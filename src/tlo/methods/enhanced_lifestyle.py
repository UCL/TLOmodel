...
class Lifestyle(Module):
    ...
    PARAMETERS = {
        ...
        "start_date_campaign_exercise_increase": Parameter(Types.DATE, "Date of campaign start for increased exercise"),
        "start_date_campaign_quit_smoking": Parameter(Types.DATE, "Date of campaign start to quit smoking"),
        "start_date_campaign_alcohol_reduction": Parameter(Types.DATE, "Date of campaign start for alcohol reduction"),
        ...
        "proportion_of_men_circumcised_at_initiation": Parameter(
            Types.REAL,
            "Proportion of men (of all ages) that are assumed to be circumcised at the initiation of the" "simulation.",
        ),
        "proportion_female_sex_workers": Parameter(
            Types.REAL, "proportion of women aged 15-49 years who are sex workers"
        ),
        "fsw_transition": Parameter(Types.REAL, "proportion of sex workers that stop being a sex worker each year"),
    }
    ...
    PROPERTIES = {
        ...
        "li_bmi": Property(
            Types.CATEGORICAL,
            "bmi: 1 (<18) 2 (18-24.9)  3 (25-29.9) 4 (30-34.9) 5 (35+)" "bmi is np.nan until age 15",
        ...
        "li_ed_lev": Property(
            Types.CATEGORICAL, "education level achieved as of now", categories=[1, 2, 3], ordered=True
        ),
    ...
    def read_parameters(self, data_folder):
        ...
        p["init_p_urban"] = (
            dataframes["urban_rural_by_district"]
            .drop(columns=["rural", "urban", "prop_rural"], axis=1)
            .set_index("district")
            .to_dict()
        )
    ...
    def on_birth(self, mother_id, child_id):
        ...
        _id_inherit_from = get_person_id_to_inherit_from(child_id, mother_id, df, self.rng)
...
class LifestyleModels:
    ...
    def __init__(self, module):
        ...
        self._models = {
            ...
            "li_wealth": {"init": self.wealth_level_linear_model(), "update": None},
            "li_low_ex": {
            ...
            "li_is_circ": {"init": self.male_circumcision_property_linear_model(), "update": None},
            "li_is_sexworker": {"init": self.female_sex_workers(), "update": self.female_sex_workers()},
        }
    ...
    def tobacco_use_linear_model(self) -> LinearModel:
        ...
        tobacco_use_linear_model = LinearModel(
            LinearModelType.LOGISTIC,
            tobacco_use_baseline_odds,
            Predictor("age_years").when("<15", 0.0),
            Predictor("sex").when("F", self.params["init_or_tob_f"]),
            Predictor().when(
                '(sex == "M") & (age_years >= 20) & (age_years < 40)', self.params["init_or_tob_age2039_m"]
            ),
            Predictor().when('(sex == "M") & (age_years >= 40)', self.params["init_or_tob_agege40_m"]),
    ...
    def excessive_alcohol_linear_model(self) -> LinearModel:
        ...
        excessive_alc_lm = LinearModel(
            LinearModelType.ADDITIVE,
            base_prob,
            Predictor()
            .when('(age_years >= 15) & (sex == "M")', self.params["init_p_ex_alc_m"])
            .when('(age_years >= 15) & (sex == "F")', self.params["init_p_ex_alc_f"]),
        )
    ...
    def marital_status_linear_model(self) -> LinearModel:
        ...
        def init_marital_status(self, df, rng=None, **externals) -> pd.Series:
            ...
            for lower_age, upper_age in age_ranges:
                subpopulation = df.index[df.age_years.between(lower_age, upper_age, inclusive="left") & df.is_alive]
                parameters_key = (
                    f"init_dist_mar_stat_age{lower_age}{upper_age}"
                    if upper_age != np.inf
                    else f"init_dist_mar_stat_agege{lower_age}"
                )
                mar_stat[subpopulation] = rng.choice(
                    li_mar_stat_dtype.categories, p=p[parameters_key], size=len(subpopulation)
                )
    ...
    def education_linear_models(self) -> Dict[str, LinearModel]:
        ...
        some_education_linear_model = LinearModel(
            LinearModelType.MULTIPLICATIVE,  # choose linear model type
            p_some_ed,  # intercept (default probability for all individuals)
            # adjust probability of some education based on age
            Predictor("age_years")
            .when("<13", self.params["init_rp_some_ed_age0513"])
            ...
            Predictor("li_wealth")
            .when(1, self.params["init_rp_some_ed_per_higher_wealth"] ** 4)
            .when(2, self.params["init_rp_some_ed_per_higher_wealth"] ** 3)
            .when(3, self.params["init_rp_some_ed_per_higher_wealth"] ** 2)
            .when(4, self.params["init_rp_some_ed_per_higher_wealth"] ** 1)
            .when(5, self.params["init_rp_some_ed_per_higher_wealth"] ** 0),
        )

        # calculate baseline of education level 3, and adjust for age and wealth
        level_3_education_linear_model = LinearModel(
            LinearModelType.MULTIPLICATIVE,
            p_ed_lev_3,
            # adjust baseline of education level 3 for age
            Predictor("age_years")
            .when("<13", 0.0)
            .when(".between(13, 19)", self.params["init_rp_some_ed_sec_age1320"])
            .when(".between(30, 39)", self.params["init_rp_some_ed_sec_age3040"])
            .when(".between(40, 49)", self.params["init_rp_some_ed_sec_age4050"])
            .when(".between(50, 59)", self.params["init_rp_some_ed_sec_age5060"])
            .when(">=60", self.params["init_rp_some_ed_sec_agege60"]),
            # adjust baseline of education level 3 for wealth
            Predictor("li_wealth")
            .when(1, (self.params["init_rp_some_ed_sec_per_higher_wealth"] ** 4))
            .when(2, (self.params["init_rp_some_ed_sec_per_higher_wealth"] ** 3))
            .when(3, (self.params["init_rp_some_ed_sec_per_higher_wealth"] ** 2))
            .when(4, (self.params["init_rp_some_ed_sec_per_higher_wealth"] ** 1))
            .when(5, (self.params["init_rp_some_ed_sec_per_higher_wealth"] ** 0)),
        )
    ...
    def no_clean_drinking_water_linear_model(self) -> LinearModel:
        ...
        no_clean_drinking_water_lm = LinearModel(
            LinearModelType.LOGISTIC,
            init_odds_no_clean_drinking_water,
            Predictor("li_urban").when(False, self.params["init_or_no_clean_drinking_water_rural"]),
        )
    ...
    def wood_burn_stove_linear_model(self) -> LinearModel:
        ...
        wood_burn_stove_lm = LinearModel(
            LinearModelType.LOGISTIC,
            init_odds_wood_burn_stove,
            Predictor("li_urban").when(False, self.params["init_or_wood_burn_stove_rural"]),
        )
    ...
    def no_access_hand_washing(self) -> LinearModel:
        ...
        no_access_hand_washing_lm = LinearModel(
            LinearModelType.LOGISTIC,
            odds_no_access_hand_washing,
            Predictor("li_wealth")
            .when(2, self.params["init_or_no_access_handwashing_per_lower_wealth"])
            .when(3, self.params["init_or_no_access_handwashing_per_lower_wealth"] ** 2)
            .when(4, self.params["init_or_no_access_handwashing_per_lower_wealth"] ** 3)
            .when(5, self.params["init_or_no_access_handwashing_per_lower_wealth"] ** 4),
        )
    ...
    def salt_intake_linear_model(self) -> LinearModel:
        ...
        odds_high_salt: float = self.params["init_p_high_salt_urban"] / (1 - self.params["init_p_high_salt_urban"])

        # create salt intake linear model
        high_salt_lm = LinearModel(
            LinearModelType.LOGISTIC,
            odds_high_salt,
            Predictor("li_urban").when(False, self.params["init_or_high_salt_rural"]),
        )
    ...
    def sugar_intake_linear_model(self) -> LinearModel:
        ...
        sugar_intake_lm = LinearModel(LinearModelType.ADDITIVE, base_prob)
    ...
    def bmi_linear_model(self, index, bmi_power) -> LinearModel:
        ...
        bmi_lm = LinearModel(
            ...
            Predictor("li_high_sugar").when(True, self.params["init_or_higher_bmi_high_sugar"] ** bmi_power),
            Predictor("age_years")
            .when('.between(30, 50, inclusive="right")', self.params["init_or_higher_bmi_age3049"] ** bmi_power)
            ...
            Predictor("li_wealth")
            .when(2, (self.params["init_or_higher_bmi_per_higher_wealth_level"] ** 2) ** bmi_power)
    ...
    def female_sex_workers(self) -> LinearModel:
        ...
        def predict_female_sex_workers(self, df, rng=None, **externals) -> pd.Series:
            ...
            target_n_sw = int(
                np.round(
                    len(df.loc[df.is_alive & (df.sex == "F") & df.age_years.between(15, 49)].index)
    ...
    def update_exercise_property_linear_model(self) -> LinearModel:
        ...
        def handle_low_exercise_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            eff_rate_not_low_ex.loc[df.li_exposed_to_campaign_exercise_increase] *= p[
    ...
    def update_tobacco_use_property_linear_model(self) -> LinearModel:
        ...
        def handle_all_tob_tansitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            all_idx_campaign_quit_smoking = df.index[
                df.is_alive & (externals["other"] == p["start_date_campaign_quit_smoking"])
            ]
    ...
    def update_excess_alcohol_property_linear_model(self) -> LinearModel:
        ...
        def handle_excess_alcohol_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            li_ex_alc_trans.loc[not_ex_alc_f] = rng.random_sample(len(not_ex_alc_f)) < p["r_ex_alc"] * p["rr_ex_alc_f"]
    ...
    def update_unimproved_sanitation_status_linear_model(self) -> LinearModel:
        ...
        def handle_unimproved_sanitation_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            eff_rate_improved_sanitation = pd.Series(
                data=p["r_improved_sanitation"], index=unimproved_sanitaton_idx, dtype=float
            )
    ...
    def update_no_access_hand_washing_status_linear_model(self) -> LinearModel:
        ...
        def handle_no_access_hand_washing_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            eff_rate_access_handwashing = pd.Series(
                data=p["r_access_handwashing"], index=no_access_handwashing_idx, dtype=float
            )
    ...
    def update_no_clean_drinking_water_linear_model(self) -> LinearModel:
        ...
        def handle_no_clean_drinking_water_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            eff_rate_clean_drinking_water = pd.Series(
                data=p["r_clean_drinking_water"], index=no_clean_drinking_water_idx, dtype=float
            )
            ...
            eff_prev_no_clean_drinking_water_urban = pd.Series(
                data=p["init_p_no_clean_drinking_water_urban"],
        ...
        no_clean_drinking_water_lm = LinearModel.custom(
            handle_no_clean_drinking_water_transitions, parameters=self.params
        )
        return no_clean_drinking_water_lm
    ...
    def update_wood_burn_stove_linear_model(self) -> LinearModel:
        ...
        def handle_wood_burn_stove_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            eff_rate_non_wood_burn_stove = pd.Series(
                data=p["r_non_wood_burn_stove"], index=wood_burn_stove_idx, dtype=float
            )
            ...
            trans_wood_burn_stove.loc[wood_burn_stove_newly_urban_idx] = random_draw < eff_prev_wood_burn_stove_urban
    ...
    def update_high_salt_property_linear_model(self) -> LinearModel:
        ...
        def handle_high_salt_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            eff_rate_not_high_salt.loc[df.li_exposed_to_campaign_salt_reduction] *= p[
    ...
    def update_high_sugar_property_linear_model(self) -> LinearModel:
        ...
        def handle_high_sugar_transitions(self, df, rng=None, **externals) -> pd.Series:
            ...
            eff_rate_not_high_sugar.loc[df.li_exposed_to_campaign_sugar_reduction] *= p[
            ...
            if _property in log_by_age_15up:
                if _property in cat_by_rural_urban_props:
                    data = grouped_counts_with_all_combinations(
                        df.loc[df.is_alive & (df.age_years >= 15)], ["li_urban", "sex", _property, "age_range"]
                    )
                else:
                    data = grouped_counts_with_all_combinations(
                        df.loc[df.is_alive & (df.age_years >= 15)], ["sex", _property, "age_range"]
                    )
                ...
                data = grouped_counts_with_all_combinations(df.loc[df.is_alive], ["sex", _property, "age_range"])
            # log data
            logger.info(key=_property, data=flatten_multi_index_series_into_dict_for_logging(data))
