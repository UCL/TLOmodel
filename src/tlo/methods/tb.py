...
class tb(Module):
    ...
    PARAMETERS = {
        ...
        "monthly_prob_progr_active": Parameter(
            Types.REAL, "Combined rate of progression/reinfection/relapse from Juan"
        ),
        "rr_tb_hiv_stages": Parameter(
            Types.REAL, "relative risk of tb in hiv+ compared with hiv- by cd4 stage"
        ),
        ...
        "monthly_prob_self_cure": Parameter(
            Types.REAL, "annual probability of self-cure"
        ),
        ...
        "monthly_prob_relapse_tx_complete": Parameter(
            Types.REAL, "annual probability of relapse once treatment complete"
        ),
        "monthly_prob_relapse_tx_incomplete": Parameter(
            Types.REAL, "annual probability of relapse if treatment incomplete"
        ),
        "monthly_prob_relapse_2yrs": Parameter(
            Types.REAL, "annual probability of relapse 2 years after treatment complete"
        ),
        "prob_treatment_success": Parameter(
            Types.REAL, "probability of treatment completion"
        ),
        ...
        "prop_mdr_retreated": Parameter(
            Types.REAL, "prevalence of mdr in previously treated cases"
        ),
        "tb_testing_coverage": Parameter(Types.REAL, "proportion of population tested"),
        "prop_smear_positive": Parameter(
            Types.REAL, "proportion of TB cases smear positive"
        ),
        "prop_smear_positive_hiv": Parameter(
            Types.REAL, "proportion of HIV/TB cases smear positive"
        ),
    ...
    PROPERTIES = {
        "tb_inf": Property(
            ...
            categories=[
                ...
                "latent_susc_primary",
                "active_susc_primary",
                "latent_susc_secondary",
                "active_susc_secondary",
                "latent_mdr_primary",
                "active_mdr_primary",
                "latent_mdr_secondary",
        ...
        "tb_specific_symptoms": Property(
            Types.CATEGORICAL,
            "Level of symptoms for tb",
            categories=["none", "latent", "active"],
        ),
        "tb_unified_symptom_code": Property(
            Types.CATEGORICAL,
    ...
    def read_parameters(self, data_folder):
        ...
        params["prop_fast_progressor"] = self.param_list.loc[
            "prop_fast_progressor", "value1"
        ]
        params["transmission_rate"] = self.param_list.loc["transmission_rate", "value1"]
        params["monthly_prob_progr_active"] = self.param_list.loc[
            "monthly_prob_progr_active", "value1"
        ]

        params["rr_tb_with_hiv_stages"] = self.param_list.loc[
            "transmission_rate"
        ].values
        params["rr_tb_art"] = self.param_list.loc["rr_tb_art", "value1"]
        params["rr_tb_ipt"] = self.param_list.loc["rr_tb_ipt", "value1"]
        params["rr_tb_malnourished"] = self.param_list.loc[
            "rr_tb_malnourished", "value1"
        ]
        ...
        params["rel_infectiousness_hiv"] = self.param_list.loc[
            "rel_infectiousness_hiv", "value1"
        ]
        params["monthly_prob_self_cure"] = self.param_list.loc[
            "monthly_prob_self_cure", "value1"
        ]
        params["monthly_prob_tb_mortality"] = self.param_list.loc[
            "monthly_prob_tb_mortality", "value1"
        ]
        params["monthly_prob_tb_mortality_hiv"] = self.param_list.loc[
            "monthly_prob_tb_mortality_hiv", "value1"
        ]
        params["prop_mdr2010"] = self.param_list.loc["prop_mdr2010", "value1"]
        params["prop_mdr_new"] = self.param_list.loc["prop_mdr_new", "value1"]
        params["prop_mdr_retreated"] = self.param_list.loc[
            "prop_mdr_retreated", "value1"
        ]
        params["monthly_prob_relapse_tx_complete"] = self.param_list.loc[
            "monthly_prob_relapse_tx_complete", "value1"
        ]
        params["monthly_prob_relapse_tx_incomplete"] = self.param_list.loc[
            "monthly_prob_relapse_tx_incomplete", "value1"
        ]
        params["monthly_prob_relapse_2yrs"] = self.param_list.loc[
            "monthly_prob_relapse_2yrs", "value1"
        ]
        params["prob_treatment_success"] = self.param_list.loc[
            "prob_treatment_success", "value1"
        ]

        params["Active_tb_prob"], params["Latent_tb_prob"] = (
            workbook["Active_TB_prob"],
            workbook["Latent_TB_prob"],
        )

        params["prop_smear_positive"] = self.param_list.loc[
            "prop_smear_positive", "value1"
        ]
        params["prop_smear_positive_hiv"] = self.param_list.loc[
            "prop_smear_positive_hiv", "value1"
        ]
        ...
        if "HealthBurden" in self.sim.modules.keys():
            params["daly_wt_susc_tb"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                0
            )  # Drug-susecptible tuberculosis, not HIV infected
            params["daly_wt_resistant_tb"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                1
            )  # multidrug-resistant tuberculosis, not HIV infected
            params["daly_wt_susc_tb_hiv_severe_anaemia"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                4
            )  # Drug-susecptible Tuberculosis, HIV infected and anemia, severe
            params["daly_wt_susc_tb_hiv_moderate_anaemia"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                5
            )  # Drug-susecptible Tuberculosis, HIV infected and anemia, moderate
            params["daly_wt_susc_tb_hiv_mild_anaemia"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                6
            )  # Drug-susecptible Tuberculosis, HIV infected and anemia, mild
            params["daly_wt_susc_tb_hiv"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                7
            )  # Drug-susecptible Tuberculosis, HIV infected
            params["daly_wt_resistant_tb_hiv_severe_anaemia"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                8
            )  # Multidrug resistant Tuberculosis, HIV infected and anemia, severe
            params["daly_wt_resistant_tb_hiv"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                9
            )  # Multidrug resistant Tuberculosis, HIV infected
            params["daly_wt_resistant_tb_hiv_moderate_anaemia"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                10
            )  # Multidrug resistant Tuberculosis, HIV infected and anemia, moderate
            params["daly_wt_resistant_tb_hiv_mild_anaemia"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                11
            )  # Multidrug resistant Tuberculosis, HIV infected and anemia, mild
    ...
    def initialise_population(self, population):
        ...
        if len(df[df.is_alive & (df.tb_inf == "latent_susc_primary")]) > 10:
            idx_c = (
                df[df.is_alive & (df.tb_inf == "latent_susc_primary")]
                .sample(frac=self.parameters["prop_mdr2010"], random_state=self.rng)
                .index
            )
        ...
        frac_active_tb = active_tb_prob_year.loc[
            (active_tb_prob_year.Sex == "M") & (active_tb_prob_year.ages == 0),
            "incidence_per_capita",
        ]

        active = (
            df[df.is_alive & (df.tb_inf == "uninfected")]
            .sample(frac=frac_active_tb, random_state=self.rng)
            .index
        )
        # print(active)
        ...
        if len(active) > 10:
            idx_c = (
                df[df.is_alive & (df.tb_inf == "active_susc_primary")]
                .sample(frac=self.parameters["prop_mdr2010"], random_state=self.rng)
                .index
            )
    ...
    def report_daly_values(self):
        ...
        health_values = df.loc[df.is_alive, "tb_specific_symptoms"].map(
            {"none": 0, "latent": 0, "active": params["daly_wt_susc_tb"]}
        )
...
class TbEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        active_hiv_neg = len(
            df[df["tb_inf"].str.contains("active_susc") & ~df.hv_inf & df.is_alive]
        )
        ...
        force_of_infection = (
            ...
            * active_hiv_neg
            * (active_hiv_pos * params["rel_infectiousness_hiv"])
            * uninfected_total
        ) / total_population
        ...
        at_risk = df[
            (df.tb_inf == "latent_susc_secondary")
            | df["tb_inf"].str.contains("latent_mdr") & df.is_alive
        ].index
    ...
    def apply(self, population):
        ...
        if new_latent.any():
            fast_progression = (
                ...
                .sample(
                    frac=params["prop_fast_progressor"], random_state=self.module.rng
                )
                .index
            )
            ...
            for i in df.index[fast_progression]:
                prob = self.sim.modules["HealthSystem"].get_prob_seek_care(
                    i, symptom_code=2
                )
        ...
        eff_prob_active_tb.loc[
            (df.tb_inf == "latent_susc_primary") & ~df.tb_on_ipt
        ] = params["monthly_prob_progr_active"]
        ...
        df.loc[
            relapse_tx_complete | relapse_tx_incomplete | relapse_tx_2yrs,
            "tb_specific_symptoms",
        ] = "active"
        df.loc[
            relapse_tx_complete | relapse_tx_incomplete, "tb_unified_symptom_code"
        ] = 2
...
class TbMdrEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        # no age distribution for FOI but the relative risks would affect distribution of active infections
        active_hiv_neg = len(
            df[df["tb_inf"].str.contains("active_mdr") & ~df.hv_inf & df.is_alive]
        )
...
class TbMdrActiveEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        eff_prob_active_tb = pd.Series(0, index=df.index)
        eff_prob_active_tb.loc[
            (df.tb_inf == "latent_mdr_primary") & ~df.tb_on_ipt
        ] = params["monthly_prob_progr_active"]
        # print('eff_prob_active_tb: ', eff_prob_active_tb)
...
class HSI_TbScreening(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        logger.debug(
            "This is HSI_TbScreening, a screening appointment for person %d", person_id
        )
        ...
        if (df.at[person_id, "tb_specific_symptoms"] == "active") and not (
            df.at[person_id, "hv_inf"]
        ):
            logger.debug(
                "This is HSI_Tb_SputumTest scheduling xpert test for person %d",
                person_id,
            )
        ...
        elif (df.at[person_id, "tb_specific_symptoms"] == "active") and (
            df.at[person_id, "hv_inf"]
        ):
            logger.debug(
                "This is HSI_Tb_SputumTest scheduling xpert test for person %d",
...
class HSI_Tb_SputumTest(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        logger.debug(
            "This is HSI_Tb_SputumTest, a first appointment for person %d", person_id
        )
        ...
        if (df.at[person_id, "tb_specific_symptoms"] == "active") and not df.at[
            person_id, "hv_inf"
        ]:
            diagnosed = self.module.rng.choice(
                [True, False],
                size=1,
                p=[params["prop_smear_positive"], (1 - params["prop_smear_positive"])],
            )
            if diagnosed:
        ...
        elif (df.at[person_id, "tb_specific_symptoms"] == "active") and df.at[
            person_id, "hv_inf"
        ]:
            diagnosed = self.module.rng.choice(
                [True, False],
                size=1,
                p=[
                    params["prop_smear_positive_hiv"],
        ...
        if (
            ...
            logger.debug(
                "This is HSI_Tb_XpertTest scheduling HSI_Tb_StartTreatmentChild for person %d",
                person_id,
            )
        ...
        if (
            ...
            logger.debug(
                "This is HSI_Tb_XpertTest scheduling HSI_Tb_StartTreatmentAdult for person %d",
                person_id,
            )
        ...
        if (
            ...
            logger.debug(
                "This is HSI_Tb_XpertTest scheduling HSI_Tb_RetreatmentChild for person %d",
                person_id,
            )
        ...
        if (
            ...
            logger.debug(
                "This is HSI_Tb_XpertTest scheduling HSI_Tb_RetreatmentAdult for person %d",
                person_id,
            )
            ...
            if len(
                ...
                ipt_sample = (
                    ...
                    .sample(n=5, replace=False, random_state=self.module.rng)
                    .index
                )
...
class HSI_Tb_XpertTest(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        logger.debug(
            "This is HSI_Tb_XpertTest giving xpert test for person %d", person_id
        )
        ...
        if df.at[person_id, "tb_inf"].startswith("active"):
            ...
            if len(
                ...
                ipt_event = HSI_Tb_Ipt(self.module, person_id=person_id)
                self.sim.modules["HealthSystem"].schedule_event(
                    ipt_event, priority=1, topen=self.sim.date, tclose=None
                )

            else:
                # Request the health system to give repeat xpert test
                logger.debug(
                    "This is HSI_Tb_XpertTest with negative result for person %d",
                    person_id,
                )
        ...
        if df.at[person_id, "tb_diagnosed"] & df.at[person_id, "tb_inf"].startswith(
            "active_mdr"
        ):
            # request treatment
            logger.debug(
...
class HSI_Tb_StartTreatmentAdult(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        self.sim.schedule_event(
            TbCureEvent(self, person_id), self.sim.date + DateOffset(months=6)
        )
...
class TbCureEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        if df.at[person_id, "tb_inf"].startswith("active_susc"):
            ...
            cured = (
                self.module.rng.random_sample(size=1) < params["prob_treatment_success"]
            )
...
class HSI_Tb_Ipt(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        self.sim.schedule_event(
            TbIptEndEvent(self, person_id), self.sim.date + DateOffset(months=6)
        )
...
class TbLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        active_susc = len(df[df["tb_inf"].str.contains("active_susc") & df.is_alive])

        active_mdr = len(df[df["tb_inf"].str.contains("active_mdr") & df.is_alive])

        active_total = active_susc + active_mdr

        ad_prev = len(
            df[df["tb_inf"].str.contains("active") & df.is_alive & (df.age_years >= 15)]
        ...
        ad_prev_latent = len(
            df[df["tb_inf"].str.contains("latent") & df.is_alive & (df.age_years >= 15)]
        ) / len(df[df.is_alive & (df.age_years >= 15)])

        child_prev_latent = len(
            df[df["tb_inf"].str.contains("latent") & df.is_alive & (df.age_years < 15)]
        ) / len(df[df.is_alive & (df.age_years < 15)])
