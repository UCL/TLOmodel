...
class Copd(Module):
    ...
    CAUSES_OF_DEATH = {
        ...
        f"COPD_cat{lung_f_category}": Cause(
            gbd_causes=sorted(gbd_causes_of_copd_represented_in_this_module),
            label="COPD",
        )
        for lung_f_category in ch_lungfunction_cats
    }
    ...
    def read_parameters(self, data_folder):
        ...
        self.load_parameters_from_dataframe(
            pd.read_csv(self.resourcefilepath / "ResourceFile_Copd.csv")
        )
    ...
    def initialise_population(self, population):
        ...
        df = population.props
        df.loc[df.is_alive, "ch_lungfunction"] = self.models.init_lung_function(
            df.loc[df.is_alive]
        )
    ...
    def report_daly_values(self):
        ...
        return df.loc[df.is_alive, "ch_lungfunction"].map(
            self.models.disability_weight_given_lungfunction
        )
    ...
    def do_logging(self):
        ...
        counts = (
            df.loc[df.is_alive]
            .groupby(by=["sex", "age_range", "li_tob", "ch_lungfunction"])
            .size()
        )
        logger.info(
            key="copd_prevalence",
    ...
    def do_when_present_with_breathless(self, person_id: int, hsi_event: HSI_Event):
        ...
        * Otherwise --> just give inhaler.
        ...
        if "breathless_severe" in self.sim.modules["SymptomManager"].has_what(
            person_id
        ):
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_Copd_TreatmentOnSevereExacerbation(
...
class CopdModels:
    ...
    def __init__(self, params, rng):
        ...
        self.__Prob_Progress_LungFunction__ = LinearModel(
            ...
            Predictor("li_tob").when(True, self.params["rel_risk_tob"]),
            Predictor("li_wood_burn_stove").when(
                True, self.params["rel_risk_wood_burn_stove"]
            ),
        )
        ...
        self.__Prob_Will_Die_SevereExacerbation__ = LinearModel.multiplicative(
            ...
            .when(
                ".between(70, 79)", self.params["prob_will_die_sev_exacerbation_7079"]
            )
            .when(
                ".between(60, 69)", self.params["prob_will_die_sev_exacerbation_6069"]
            )
            .when(
                ".between(50, 59)", self.params["prob_will_die_sev_exacerbation_5059"]
            )
            .when(
                ".between(40, 49)", self.params["prob_will_die_sev_exacerbation_4049"]
            )
            .when(
                ".between(30, 39)", self.params["prob_will_die_sev_exacerbation_3039"]
            )
    ...
    def init_lung_function(self, df: pd.DataFrame) -> pd.Series:
        ...
        cats_15_39_not_tob = dict(
            zip(
                ...
                self.rng.choice(
                    cats,
                    p=self.params["prob_not_tob_lung_func_15_39"],
        ...
        cats_40_59_not_tob = dict(
            zip(
                ...
                self.rng.choice(
                    cats,
                    p=self.params["prob_not_tob_lung_func_40_59"],
        ...
        cats_gr59_not_tob = dict(
            zip(
                ...
                self.rng.choice(
                    cats,
                    p=self.params["prob_not_tob_lung_func_gr59"],
        ...
        cats_15_39_tob = dict(
            zip(
                ...
                self.rng.choice(
                    cats,
                    p=self.params["prob_tob_lung_func_15_39"],
        ...
        cats_40_59_tob = dict(
            zip(
                ...
                self.rng.choice(
                    cats,
                    p=self.params["prob_tob_lung_func_40_59"],
        ...
        cats_gr59_tob = dict(
            zip(
                ...
                self.rng.choice(
                    cats,
                    p=self.params["prob_tob_lung_func_gr59"],
        ...
        cats_for_under15s = {
            idx: self.at_birth_lungfunction(idx) for idx in idx_notover15
        }
    ...
    def prob_livesaved_given_treatment(
        self, df: pd.DataFrame, oxygen: bool, aminophylline: bool
    ):
    ...
    def will_progres_to_next_cat_of_lungfunction(self, df: pd.DataFrame) -> List:
        ...
        """
        will_progress_to_next_category = self.__Prob_Progress_LungFunction__.predict(
            df, self.rng, squeeze_single_row_output=False
        )
        return will_progress_to_next_category[
            will_progress_to_next_category
        ].index.to_list()
    ...
    def will_get_moderate_exacerbation(self, df: pd.DataFrame) -> List:
        ...
        will_get_ex = self.__Prob_ModerateExacerbation__.predict(
            df, self.rng, squeeze_single_row_output=False
        )
    ...
    def will_get_severe_exacerbation(self, df: pd.DataFrame) -> List:
        ...
        probability of having a Severe Exacerbation in a 3-month period"""
        will_get_ex = self.__Prob_SevereExacerbation__.predict(
            df, self.rng, squeeze_single_row_output=False
        )
        return will_get_ex[will_get_ex].index.to_list()
...
class CopdPollEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        * Log current states.
        ...
        for idx in self.module.models.will_get_moderate_exacerbation(
            df.loc[df.is_alive]
        ):
            self.sim.schedule_event(
                CopdExacerbationEvent(self.module, idx, severe=False),
        ...
        for idx in self.module.models.will_get_severe_exacerbation(df.loc[df.is_alive]):
            self.sim.schedule_event(
                CopdExacerbationEvent(self.module, idx, severe=True),
                self.gen_random_date_in_next_three_months(),
            )
    ...
    def gen_random_date_in_next_three_months(self):
        ...
        return random_date(
            self.sim.date, self.sim.date + pd.DateOffset(months=3), self.module.rng
        )
    ...
    def progress_to_next_lung_function(self, df):
        ...
        idx_will_progress_to_next_category = (
            self.module.models.will_progres_to_next_cat_of_lungfunction(
                df.loc[eligible_to_progress_to_next_lung_function(df)]
            )
        )
        df.loc[idx_will_progress_to_next_category, "ch_lungfunction"] = (
            self.increment_category(
                df.loc[idx_will_progress_to_next_category, "ch_lungfunction"]
            )
        )
    ...
    * A Severe Exacerbation may cause death and may cause emergency healthcare seeking.
    ...
    def apply(self, person_id):
        ...
        self.sim.modules["SymptomManager"].change_symptom(
            person_id=person_id,
            symptom_string=(
                "breathless_severe" if self.severe else "breathless_moderate"
            ),
            add_or_remove="+",
        ...
        if self.severe:
            ...
            if self.module.models.will_die_given_severe_exacerbation(
                ...
                self.sim.schedule_event(
                    CopdDeath(self.module, person_id),
...
class CopdDeath(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        person = df.loc[
            person_id,
            ["is_alive", "age_years", "ch_will_die_this_episode", "ch_lungfunction"],
        ]
...
class HSI_Copd_TreatmentOnSevereExacerbation(HSI_Event, IndividualScopeEventMixin):
    ...
    def __init__(self, module, person_id):
        ...
        self.ACCEPTED_FACILITY_LEVEL = self.all_facility_levels[
            self.facility_levels_index
        ]
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        * Provide treatment: whatever is available at this facility at this time (no referral).
        ...
        if not self.get_consumables(self.module.item_codes["oxygen"]):
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                self, topen=self.sim.date, priority=0
            )
            ...
            prob_treatment_success = self.module.models.prob_livesaved_given_treatment(
                ...
                aminophylline=self.get_consumables(
                    self.module.item_codes["aminophylline"]
                ),
            )
