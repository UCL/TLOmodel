...
class Copd(Module, GenericFirstAppointmentsMixin):
    ...
    CAUSES_OF_DEATH = {
        ...
        f"COPD_cat{lung_f_category}": Cause(
            gbd_causes=sorted(gbd_causes_of_copd_represented_in_this_module), label="COPD"
        )
    ...
    CAUSES_OF_DISABILITY = {
        # Chronic Obstructive Pulmonary Diseases
        "COPD": Cause(gbd_causes=sorted(gbd_causes_of_copd_represented_in_this_module), label="COPD")
    }
    ...
    PARAMETERS = {
        ...
        "eff_oxygen": Parameter(Types.REAL, "the effect of oxygen on individuals with severe exacerbation "),
        ...
        "rel_risk_tob": Parameter(Types.REAL, "relative effect of tobacco on the rate of lung function progression"),
    ...
    PROPERTIES = {
        ...
        "ch_has_inhaler": Property(Types.BOOL, "Whether the person is currently using an inhaler"),
    }
        ...
        * Otherwise --> just give inhaler.
        ...
        if ("breathless_moderate" in symptoms) or ("breathless_severe" in symptoms):
            ...
            if "breathless_severe" in symptoms:
                event = HSI_Copd_TreatmentOnSevereExacerbation(module=self, person_id=person_id)
...
class CopdModels:
    ...
    def init_lung_function(self, df: pd.DataFrame) -> pd.Series:
        ...
        cats_15_39_not_tob = dict(
            zip(
                idx_15_39_not_tob,
                self.rng.choice(cats, p=self.params["prob_not_tob_lung_func_15_39"], size=len(idx_15_39_not_tob)),
            )
        )
        ...
        cats_40_59_not_tob = dict(
            zip(
                idx_40_59_not_tob,
                self.rng.choice(cats, p=self.params["prob_not_tob_lung_func_40_59"], size=len(idx_40_59_not_tob)),
            )
        )
        ...
        cats_gr59_not_tob = dict(
            zip(
                idx_gr59_not_tob,
                self.rng.choice(cats, p=self.params["prob_not_tob_lung_func_gr59"], size=len(idx_gr59_not_tob)),
            )
        )
        ...
        cats_15_39_tob = dict(
            zip(
                idx_15_39_tob, self.rng.choice(cats, p=self.params["prob_tob_lung_func_15_39"], size=len(idx_15_39_tob))
            )
        )
        ...
        cats_40_59_tob = dict(
            zip(
                idx_40_59_tob, self.rng.choice(cats, p=self.params["prob_tob_lung_func_40_59"], size=len(idx_40_59_tob))
            )
        )

        idx_gr59_tob = df.index[(df.age_years >= 60) & df.li_tob]
        cats_gr59_tob = dict(
            zip(idx_gr59_tob, self.rng.choice(cats, p=self.params["prob_tob_lung_func_gr59"], size=len(idx_gr59_tob)))
        )
...
def eligible_to_progress_to_next_lung_function(df: pd.DataFrame) -> pd.Series:
    ...
    return df.is_alive & (df.age_years >= 15) & (df["ch_lungfunction"] != ch_lungfunction_cats[-1])
        ...
        * Log current states.
        ...
        for idx in self.module.models.will_get_moderate_exacerbation(df.loc[df.is_alive]):
            self.sim.schedule_event(
                CopdExacerbationEvent(self.module, idx, severe=False), self.gen_random_date_in_next_three_months()
            )
        ...
        for idx in self.module.models.will_get_severe_exacerbation(df.loc[df.is_alive]):
            self.sim.schedule_event(
                CopdExacerbationEvent(self.module, idx, severe=True), self.gen_random_date_in_next_three_months()
            )
