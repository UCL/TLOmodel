...
def return_cons_avail(self, hsi_event, cons_dict, **info):
    ...
    if (
        "number" in info.keys()
    ):  # todo: will only work if the 'core' package contains only 1 IC?
    ...
    if (
        not ps_params["ps_analysis_in_progress"]
        and not la_params["la_analysis_in_progress"]
    ):
        available = hsi_event.get_consumables(
            item_codes=core_cons, optional_item_codes=opt_cons
        )

        if (
            not available
            and (hsi_event.target in mni)
            and (hsi_event != "AntenatalCare_Outpatient")
        ):
            mni[hsi_event.target][
                "cons_not_avail"
            ] = True  # todo: update r.e. delivery kit
...
def log_met_need(self, intervention, hsi):
    ...
    if intervention in (
        ...
        "ur_surg",
        "neo_resus",
        "neo_sep_supportive_care",
        "neo_sep_abx",
    ):
        logger.info(
            key="intervention", data={"person_id": person_id, "int": intervention}
        )
    ...
    # correct name for the logged intervention.
        ...
        if (
            (hsi.TREATMENT_ID == "DeliveryCare_Basic")
            or (hsi.TREATMENT_ID == "AntenatalCare_Inpatient")
        ) and not person.la_is_postpartum:
            logger.info(
                key="intervention",
                data={
                    "person_id": person_id,
        ...
        if (hsi.TREATMENT_ID == "DeliveryCare_Basic") or (
            hsi.TREATMENT_ID == "AntenatalCare_Inpatient"
        ):
            logger.info(
                key="intervention",
                data={"person_id": person_id, "int": "abx_an_sepsis"},
            )

        elif hsi.TREATMENT_ID == "PostnatalCare_Maternal":
            logger.info(
                key="intervention",
                data={"person_id": person_id, "int": "abx_pn_sepsis"},
            )
        ...
        if hsi.TREATMENT_ID == "AntenatalCare_Inpatient":
            logger.info(
                key="intervention",
                data={"person_id": person_id, "int": "blood_tran_anaemia"},
            )

        elif hsi.TREATMENT_ID == "DeliveryCare_Comprehensive":
            if (
                (person.la_antepartum_haem != "none")
                or (person.ps_antepartum_haemorrhage != "none")
            ) and not person.la_is_postpartum:
                logger.info(
                    key="intervention",
                    data={"person_id": person_id, "int": "blood_tran_aph"},
                )

            elif person.la_uterine_rupture and not person.la_is_postpartum:
                logger.info(
                    key="intervention",
                    data={"person_id": person_id, "int": "blood_tran_ur"},
                )

            elif (
                person.la_postpartum_haem or person.pn_postpartum_haem_secondary
            ) and person.la_is_postpartum:
                logger.info(
                    key="intervention",
                    data={"person_id": person_id, "int": "blood_tran_pph"},
                )
...
def scale_linear_model_at_initialisation(self, model, parameter_key):
    ...
    if parameter_key not in (
        ...
        if "odds" in parameter_key:
            ...
            params[parameter_key] = return_scaled_intercept(
                target, logistic_model=False
            )
...
def check_if_delayed_care_delivery(self, squeeze_factor, individual_id, hsi_type):
    ...
    if (
        squeeze_factor
        > self.current_parameters[f"squeeze_threshold_for_delay_three_{hsi_type}"]
    ):
...
def get_treatment_effect(delay_one_two, delay_three, treatment_effect, params):
    ...
    if delay_one_two and delay_three:
        treatment_effect = 1 - (
            (1 - params[treatment_effect])
            * params["treatment_effect_modifier_all_delays"]
        )
    ...
    # Otherwise, if only one type of delay is experience the treatment effect is reduced by a lesser amount
        ...
        treatment_effect = 1 - (
            (1 - params[treatment_effect])
            * params["treatment_effect_modifier_one_delay"]
        )
...
def check_for_risk_of_death_from_cause_maternal(self, individual_id):
    ...
    if (
        (mother.ps_antepartum_haemorrhage != "none")
        or (mother.la_antepartum_haem != "none")
    ) and (self != self.sim.modules["PostnatalSupervisor"]):
    ...
    if mother.la_sepsis or (
        (self == self.sim.modules["Labour"])
        and mother.ps_chorioamnionitis
        and mother.ac_admitted_for_immediate_delivery != "none"
    ):
        ...
        for cause in causes:
            if self == self.sim.modules["PregnancySupervisor"]:
                ...
                if cause == "secondary_postpartum_haemorrhage":
                    risk = {
                        cause: self.la_linear_models[
                            "postpartum_haemorrhage_death"
                        ].predict(
                            df.loc[[individual_id]],
                            received_blood_transfusion=mni[individual_id][
                                "received_blood_transfusion"
                            ],
                    ...
                    risk = {
                        cause: self.la_linear_models[f"{cause}_death"].predict(
                            df.loc[[individual_id]],
                            received_blood_transfusion=mni[individual_id][
                                "received_blood_transfusion"
                            ],
...
def check_for_risk_of_death_from_cause_neonatal(self, individual_id):
    ...
    if not nci[individual_id]["passed_through_week_one"]:
        ...
        if (
            child.nb_not_breathing_at_birth
            and (child.nb_encephalopathy == "none")
            and not child.nb_preterm_respiratory_distress
        ):
    ...
    if causes:
        ...
        for cause in causes:
            if f"{cause}_death" in self.nb_linear_models.keys():
                risk = {
                    cause: self.nb_linear_models[f"{cause}_death"].predict(
                        df.loc[[individual_id]], delay=nci[individual_id]["third_delay"]
                    )[individual_id]
                }
            else:
                risk = {cause: params[f"cfr_{cause}"]}
