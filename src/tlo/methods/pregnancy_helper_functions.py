...
def log_met_need(self, intervention, hsi):
    ...
    # correct name for the logged intervention.
        ...
        if hsi.TREATMENT_ID == "AntenatalCare_Inpatient":
            ...
            if (
                (person.la_antepartum_haem != "none") or (person.ps_antepartum_haemorrhage != "none")
...
def scale_linear_model_at_initialisation(self, model, parameter_key):
    ...
    def return_scaled_intercept(target, logistic_model):
        mean = model.predict(
            women, year=self.sim.date.year, mode_of_delivery=mode_of_delivery, delivery_setting=delivery_setting
        ).mean()
        if logistic_model:
            mean = mean / (1.0 - mean)
        ...
        for cause in causes:
            if self == self.sim.modules["PregnancySupervisor"]:
                ...
                if cause == "secondary_postpartum_haemorrhage":
                    risk = {
                        cause: self.la_linear_models["postpartum_haemorrhage_death"].predict(
    ...
    if causes:
        ...
        for cause in causes:
            if f"{cause}_death" in self.nb_linear_models.keys():
                risk = {cause: self.nb_linear_models[f"{cause}_death"].predict(df.loc[[individual_id]])[individual_id]}
            else:
                risk = {cause: params[f"cfr_{cause}"]}
