...
class Depression(Module):
    ...
    CAUSES_OF_DISABILITY = {
        "SevereDepression": Cause(
            gbd_causes="Depressive disorders", label="Depression / Self-harm"
        )
    }
    ...
    PARAMETERS = {
        ...
        "init_rp_depr_wealth45": Parameter(
            Types.REAL,
            "Initial relative prevalence of being depressed in people with wealth level 4 or 5 vs 1 or 2 "
            "or 3 ",
        ),
        ...
        "init_pr_ever_diagnosed_depression": Parameter(
            Types.REAL,
            "Initial probability of having ever been diagnosed with depression, amongst people with ever "
        ...
        "base_3m_prob_depr": Parameter(
            Types.REAL,
            "Probability of onset of depression in a 3 month period if male, wealth 1 2 or 3, no chronic "
        ...
        "rr_depr_cc": Parameter(
            Types.REAL, "Relative rate of depression if has any chronic disease"
        ),
        "rr_depr_pregnancy": Parameter(
            Types.REAL, "Relative rate of depression when pregnant or recently pregnant"
        ),
        "rr_depr_female": Parameter(
            Types.REAL, "Relative rate of depression for females vs males"
        ),
        ...
        "rr_depr_age1519": Parameter(
            Types.REAL, "Relative rate of depression associated with 15-20 year olds"
        ),
        "rr_depr_agege60": Parameter(
            Types.REAL, "Relative rate of depression associated with age > 60"
        ),
        ...
        "rr_resol_depr_current_talk_ther": Parameter(
            Types.REAL,
            "Relative rate of resolving depression if has ever had talking therapy vs has never had "
            "talking therapy ",
        ),
        "prob_3m_stop_antidepr": Parameter(
            Types.REAL,
            "Probability per 3 months of stopping antidepressants when not currently "
            "depressed.",
        ),
        "prob_3m_default_antidepr": Parameter(
            Types.REAL,
            "Probability per 3 months of stopping antidepressants when "
            "still depressed.",
        ),
        ...
        "rr_suicide_depr_f": Parameter(
            Types.REAL, "Relative risk of suicide in women compared with men"
        ),
        "prob_3m_selfharm_depr": Parameter(
            Types.REAL,
            "Probability per 3 months of non-fatal self harm in those "
            "currently depressed",
        ),
        "sensitivity_of_assessment_of_depression": Parameter(
            Types.REAL,
            "The sensitivity of the clinical assessment "
        ...
        "anti_depressant_medication_item_code": Parameter(
            Types.INT,
    ...
    PROPERTIES = {
        ...
        "de_date_init_most_rec_depr": Property(
            Types.DATE, "date this person last initiated a depression episode"
        ),
        "de_date_depr_resolved": Property(
            Types.DATE, "date this person resolved last episode of depression"
        ),
        "de_intrinsic_3mo_risk_of_depr_resolution": Property(
            Types.REAL,
            "the risk per 3 mo of an episode of depression being "
            "resolved in absence of any treatment",
        ),
        "de_ever_diagnosed_depression": Property(
            Types.BOOL, "whether ever diagnosed with depression"
        ),
        "de_on_antidepr": Property(Types.BOOL, "is currently on anti-depressants"),
        "de_ever_talk_ther": Property(
            Types.BOOL, "whether this person has ever had a session of talking therapy"
        ),
        "de_ever_non_fatal_self_harm_event": Property(
            Types.BOOL, "ever had a non-fatal self harm event"
        ),
        "de_cc": Property(Types.BOOL, "whether this person has chronic condition"),
        # TODO: <--- define and update at poll
        "de_recently_pregnant": Property(
            Types.BOOL,
            "whether this person is female and is either currently pregnant "
    ...
    def read_parameters(self, data_folder):
        ...
        self.linearModels["Depression_At_Population_Initialisation"] = LinearModel(
            ...
            Predictor().when(
                '(sex=="F") & de_recently_pregnant', p["init_rp_depr_f_rec_preg"]
            ),
            Predictor().when(
                '(sex=="F") & ~de_recently_pregnant', p["init_rp_depr_f_not_rec_preg"]
            ),
            Predictor(
                "age_years",
        ...
        self.linearModels["Depression_Ever_Diagnosed_At_Population_Initialisation"] = (
            LinearModel.multiplicative(
                Predictor("de_ever_depr")
                .when(True, p["init_pr_ever_diagnosed_depression"])
                .otherwise(0.0)
            )
        )
        ...
        self.linearModels["Using_AntiDepressants_Initialisation"] = (
            LinearModel.multiplicative(
                ...
                Predictor().when(
                    "~de_depr & de_ever_diagnosed_depression",
        ...
        self.linearModels["Risk_of_Depression_Resolution_per3mo"] = (
            LinearModel.multiplicative(
                Predictor("de_intrinsic_3mo_risk_of_depr_resolution").apply(
                    lambda x: x
                ),
                Predictor("de_cc").when(True, p["rr_resol_depr_cc"]),
                Predictor("de_on_antidepr").when(True, p["rr_resol_depr_on_antidepr"]),
                Predictor("de_ever_talk_ther").when(
                    True, p["rr_resol_depr_current_talk_ther"]
                ),
            )
        )

        self.linearModels["Risk_of_Stopping_Antidepressants_per3mo"] = (
            LinearModel.multiplicative(
                Predictor("de_depr")
                .when(True, p["prob_3m_default_antidepr"])
                .when(False, p["prob_3m_stop_antidepr"])
            )
        )

        self.linearModels["Risk_of_SelfHarm_per3mo"] = LinearModel(
            LinearModelType.MULTIPLICATIVE, p["prob_3m_selfharm_depr"]
        )
        ...
        self.sim.modules["SymptomManager"].register_symptom(
            Symptom.emergency(name="Injuries_From_Self_Harm", which="adults")
        )
    ...
    def initialise_population(self, population):
        ...
        df.loc[df["is_alive"] & df["de_depr"], "de_date_init_most_rec_depr"] = (
            self.sim.date
        )
        df.loc[
            df["is_alive"] & df["de_depr"], "de_intrinsic_3mo_risk_of_depr_resolution"
        ] = self.rng.choice(
        ...
        df.loc[(df["is_alive"] & (df["sex"] == "M")), "de_ever_depr"] = (
            self.apply_linear_model(
                self.linearModels["Depression_Ever_At_Population_Initialisation_Males"],
                df.loc[(df["is_alive"] & (df["sex"] == "M"))],
            )
        )
        df.loc[(df["is_alive"] & (df["sex"] == "F")), "de_ever_depr"] = (
            self.apply_linear_model(
                self.linearModels[
                    "Depression_Ever_At_Population_Initialisation_Females"
                ],
                df.loc[(df["is_alive"] & (df["sex"] == "F"))],
            )
        )

        df.loc[(df["is_alive"] & df["de_depr"]), "de_ever_depr"] = (
            True  # For logical consistency
        )
        df.loc[
            (df["is_alive"] & ~df["de_depr"] & df["de_ever_depr"]),
            "de_date_depr_resolved",
        ] = self.sim.date - DateOffset(
            days=1
        )  # If ever had depression, needs a resolution date in the past

        # Assign initial 'ever diagnosed' status
        df.loc[df["is_alive"], "de_ever_diagnosed_depression"] = (
            self.apply_linear_model(
                self.linearModels[
                    "Depression_Ever_Diagnosed_At_Population_Initialisation"
                ],
                df.loc[df["is_alive"]],
            )
        )
        ...
        df.loc[df["is_alive"], "de_ever_non_fatal_self_harm_event"] = (
            self.apply_linear_model(
        ...
        df.loc[
            df["is_alive"] & df["de_depr"] & df["de_ever_diagnosed_depression"],
            "de_on_antidepr",
        ] = self.apply_linear_model(
            self.linearModels["Using_AntiDepressants_Initialisation"],
        ...
        if df["de_on_antidepr"].sum():
            for person_id in df.loc[df["de_on_antidepr"]].index:
                ...
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    hsi_event=HSI_Depression_Refill_Antidepressant(
    ...
    def report_daly_values(self):
        ...
        def right_censor(obs, window_close):
            return obs.apply(
                lambda x: window_close if pd.isnull(x) else min(x, window_close)
            )
        ...
        any_depr_in_the_last_month = (
            ...
            & (
                ...
                | (
                    df["de_date_depr_resolved"]
                    >= (self.sim.date - DateOffset(months=1))
                )
            )
        )

        start_depr = left_censor(
            df.loc[any_depr_in_the_last_month, "de_date_init_most_rec_depr"],
            self.sim.date - DateOffset(months=1),
        )
        end_depr = right_censor(
            df.loc[any_depr_in_the_last_month, "de_date_depr_resolved"], self.sim.date
        )
        ...
        av_daly_wt_last_month = pd.Series(
            index=df.loc[df.is_alive].index, name="SevereDepression", data=0.0
        ).add(
            fraction_of_month_depr
            * self.daly_wts["average_per_day_during_any_episode"],
            fill_value=0.0,
        )
    ...
    def do_on_presentation_to_care(self, person_id, hsi_event):
        ...
        if hsi_event.TREATMENT_ID == "FirstAttendance_NonEmergency":
            ...
            hsi_event.TREATMENT_ID == "AntenatalCare_Outpatient"
    ...
    def do_when_suspected_depression(self, person_id, hsi_event):
        ...
        if self.sim.modules["HealthSystem"].dx_manager.run_dx_test(
            dx_tests_to_run="assess_depression", hsi_event=hsi_event
        ):
...
class DepressionPollingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        assert not (
            df["de_recently_pregnant"] & pd.isnull(df["date_of_last_pregnancy"])
        ).any()
        assert (
            df.loc[
                ((~df["is_pregnant"]) & df["de_recently_pregnant"]),
                "date_of_last_pregnancy",
            ]
            > (self.sim.date - DateOffset(years=1))
        ).all()
        ...
        df.loc[
            onset_depression.loc[onset_depression].index, "de_date_init_most_rec_depr"
        ] = self.sim.date
        df.loc[
            onset_depression.loc[onset_depression].index, "de_date_depr_resolved"
        ] = pd.NaT

        # Set the rate of depression resolution for each person who is onset with depression
        df.loc[
            onset_depression.loc[onset_depression].index,
            "de_intrinsic_3mo_risk_of_depr_resolution",
        ] = self.module.rng.choice(
            p["depr_resolution_rates"], len(onset_depression.loc[onset_depression])
        )
        ...
        resolved_depression = apply_linear_model(
            ...
            df.loc[
                df["is_alive"]
                & df["de_depr"]
                & ~df.index.isin(onset_depression.loc[onset_depression].index)
            ],
        )
        df.loc[resolved_depression.loc[resolved_depression].index, "de_depr"] = False
        df.loc[
            resolved_depression.loc[resolved_depression].index, "de_date_depr_resolved"
        ] = self.sim.date
        df.loc[
            resolved_depression.loc[resolved_depression].index,
            "de_intrinsic_3mo_risk_of_depr_resolution",
        ] = np.nan
        ...
        df.loc[
            stop_using_antidepressants.loc[stop_using_antidepressants].index,
            "de_on_antidepr",
        ] = False
        ...
        for person_id in will_self_harm_in_next_3mo.loc[
            will_self_harm_in_next_3mo
        ].index:
...
class DepressionSelfHarmEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        self.sim.population.props.at[person_id, "de_ever_non_fatal_self_harm_event"] = (
            True
        )
...
class DepressionSuicideEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        self.sim.modules["Demography"].do_death(
            individual_id=person_id, cause="Suicide", originating_module=self.module
        )
...
class DepressionLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    * counts of events of self-harm and suicide that have occurred in the 3 months prior
    ...
    def apply(self, population):
        ...
        n_ge15_m_depr = (
            df.is_alive & (df.age_years >= 15) & (df.sex == "M") & df.de_depr
        ).sum()
        n_ge15_f_depr = (
            df.is_alive & (df.age_years >= 15) & (df.sex == "F") & df.de_depr
        ).sum()
        n_ever_depr = (df.de_ever_depr & df.is_alive & (df.age_years >= 15)).sum()
        n_age_50_ever_depr = (
            df.is_alive & (df.age_years == 50) & df.de_ever_depr
        ).sum()
        ...
        n_ever_diagnosed_depression = (
            df.is_alive & df.de_ever_diagnosed_depression & (df.age_years >= 15)
        ).sum()
        n_antidepr_depr = (
            df.is_alive & df.de_on_antidepr & df.de_depr & (df.age_years >= 15)
        ).sum()
        n_antidepr_ever_depr = (
            df.is_alive & df.de_on_antidepr & df.de_ever_depr & (df.age_years >= 15)
        ).sum()
        ...
        dict_for_output = {
            ...
            "prop_age_50_ever_depr": zero_out_nan(
                safe_divide(n_age_50_ever_depr, n_age_50)
            ),
            "p_ever_diagnosed_depression_if_ever_depressed": zero_out_nan(
                safe_divide(n_ever_diagnosed_depression, n_ever_depr)
            ),
            "prop_antidepr_if_curr_depr": zero_out_nan(
                safe_divide(n_antidepr_depr, n_ge15_depr)
            ),
            "prop_antidepr_if_ever_depr": zero_out_nan(
                safe_divide(n_antidepr_ever_depr, n_ever_depr)
            ),
            "prop_ever_talk_ther_if_ever_depr": zero_out_nan(
                safe_divide(n_ever_talk_ther, n_ever_depr)
            ),
            "prop_ever_self_harmed": zero_out_nan(
                safe_divide(n_ever_self_harmed, n_ever_depr)
            ),
        }
...
class HSI_Depression_TalkingTherapy(HSI_Event, IndividualScopeEventMixin):
    ...
    def __init__(self, module, person_id):
        ...
        self.num_of_sessions_had = (
            0  # A counter for the number of sessions of talking therapy had
        )
...
class HSI_Depression_Start_Antidepressant(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        assert df.at[person_id, "de_ever_diagnosed_depression"], (
...
class HSI_Depression_Refill_Antidepressant(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if self.get_consumables(
            self.module.parameters["anti_depressant_medication_item_code"]
        ):
...
def compute_key_outputs_for_last_3_years(parsed_output):
    ...
    result["Current prevalence of depression, aged 15+"] = depr.loc[
        period, "prop_ge15_depr"
    ].mean()

    result["Current prevalence of depression, aged 15+ males"] = depr.loc[
        period, "prop_ge15_m_depr"
    ].mean()

    result["Current prevalence of depression, aged 15+ females"] = depr.loc[
        period, "prop_ge15_f_depr"
    ].mean()

    # Ever depression in people age 50:
    result["Ever depression, aged 50y"] = depr.loc[
        period, "prop_age_50_ever_depr"
    ].mean()

    # Prevalence of antidepressant use amongst age 15+ year olds ever depressed
    result["Proportion of 15+ ever depressed using anti-depressants, aged 15+y"] = (
        depr.loc[period, "prop_antidepr_if_ever_depr"].mean()
    )

    # Prevalence of antidepressant use amongst people currently depressed
    result[
        "Proportion of 15+ currently depressed using anti-depressants, aged 15+y"
    ] = depr.loc[period, "prop_antidepr_if_curr_depr"].mean()
    ...
    depr_events = depr_events.groupby(by="year")[
        ["SelfHarmEvents", "SuicideEvents"]
    ].sum()
    ...
    def get_15plus_pop_by_year(df):
        ...
        cols_for_15plus = [
            int(x[0]) >= 15 for x in df.columns.str.strip("+").str.split("-")
        ]
        return df[df.columns[cols_for_15plus]].sum(axis=1)

    tot_pop = get_15plus_pop_by_year(
    ...
    result["Rate of non-fatal self-harm incidence per 100k persons aged 15+"] = (
        1e5 * depr_event_rate["SelfHarmEvents"].mean()
    )

    # Rate of suicide per 100,000 adults age 15+ per year
    result["Rate of suicide incidence per 100k persons aged 15+"] = (
        1e5 * depr_event_rate["SuicideEvents"].mean()
    )
