...
class Depression(Module, GenericFirstAppointmentsMixin):
    ...
    INIT_DEPENDENCIES = {"Demography", "Contraception", "HealthSystem", "Lifestyle", "SymptomManager"}
    ...
    PARAMETERS = {
        ...
        "rr_depr_wealth45": Parameter(
            Types.REAL, "Relative rate of depression when in wealth level 4 or 5 vs 1 or 2 " "or 3"
        ),
        ...
        "rr_depr_on_antidepr": Parameter(Types.REAL, "Relative rate of depression episode if on antidepressants"),
        ...
        "rr_resol_depr_cc": Parameter(Types.REAL, "Relative rate of resolving depression if has any chronic disease"),
        ...
        "prob_3m_stop_antidepr": Parameter(
            Types.REAL, "Probability per 3 months of stopping antidepressants when not currently " "depressed."
        ),
        "prob_3m_default_antidepr": Parameter(
            Types.REAL, "Probability per 3 months of stopping antidepressants when " "still depressed."
        ),
        "prob_3m_suicide_depr_m": Parameter(
            Types.REAL, "Probability per 3 months of suicide in currently depressed " "men"
        ),
        ...
        "prob_3m_selfharm_depr": Parameter(
            Types.REAL, "Probability per 3 months of non-fatal self harm in those " "currently depressed"
        ),
        ...
        "pr_assessed_for_depression_in_generic_appt_level1": Parameter(
            Types.REAL,
            "Probability that a person is assessed for depression during a non-emergency generic appointment" "level 1",
        ),
        "anti_depressant_medication_item_code": Parameter(
            Types.INT, "The item code used for one month of anti-depressant " "treatment"
        ),
    ...
    PROPERTIES = {
        ...
        "de_intrinsic_3mo_risk_of_depr_resolution": Property(
            Types.REAL, "the risk per 3 mo of an episode of depression being " "resolved in absence of any treatment"
        ),
        ...
        "de_ever_talk_ther": Property(Types.BOOL, "whether this person has ever had a session of talking therapy"),
    ...
    def read_parameters(self, data_folder):
        ...
        self.load_parameters_from_dataframe(
            read_csv_files(Path(self.resourcefilepath) / "ResourceFile_Depression", files="parameter_values")
        )
        p = self.parameters
        ...
        conditional_predictors = (
            [
                Predictor().when("hv_inf & hv_diagnosed", p["rr_depr_hiv"]),
            ]
        ...
        self.linearModels["Risk_of_Depression_Onset_per3mo"] = LinearModel(
            LinearModelType.MULTIPLICATIVE, p["base_3m_prob_depr"], *(predictors + conditional_predictors)
        )
        ...
        self.linearModels["Risk_of_Stopping_Antidepressants_per3mo"] = LinearModel.multiplicative(
            Predictor("de_depr").when(True, p["prob_3m_default_antidepr"]).when(False, p["prob_3m_stop_antidepr"])
        )
    ...
    def initialise_population(self, population):
        ...
        df.loc[df["is_alive"], "de_depr"] = self.apply_linear_model(
            self.linearModels["Depression_At_Population_Initialisation"], df.loc[df["is_alive"]]
        )
        ...
        df.loc[df["is_alive"] & df["de_depr"], "de_intrinsic_3mo_risk_of_depr_resolution"] = self.rng.choice(
            self.parameters["depr_resolution_rates"], (df["is_alive"] & df["de_depr"]).sum()
        )
        ...
        df.loc[df["is_alive"], "de_ever_diagnosed_depression"] = self.apply_linear_model(
            self.linearModels["Depression_Ever_Diagnosed_At_Population_Initialisation"], df.loc[df["is_alive"]]
        )

        # Assign initial 'de_ever_talk_ther' status
        df.loc[df["is_alive"], "de_ever_talk_ther"] = self.apply_linear_model(
            self.linearModels["Ever_Talking_Therapy_Initialisation"], df.loc[df["is_alive"]]
        )
        ...
        df.loc[df["is_alive"], "de_ever_non_fatal_self_harm_event"] = self.apply_linear_model(
            self.linearModels["Ever_Self_Harmed_Initialisation"], df.loc[df["is_alive"]]
        )

        # Assign initial 'using anti-depressants' status to those who are currently depressed and diagnosed
        df.loc[
            df["is_alive"] & df["de_depr"] & df["de_ever_diagnosed_depression"], "de_on_antidepr"
    ...
    def report_daly_values(self):
        ...
        start_depr = left_censor(
            df.loc[any_depr_in_the_last_month, "de_date_init_most_rec_depr"], self.sim.date - DateOffset(months=1)
        )
    ...
    def _check_for_suspected_depression(self, symptoms: List[str], treatment_id: str, has_even_been_diagnosed: bool):
        """
    ...
    def do_on_presentation_to_care(self, person_id: int, hsi_event: HSI_Event):
        ...
        if self._check_for_suspected_depression(
            ...
            self.sim.population.props.loc[person_id, individual_properties.keys()] = individual_properties.values()
        return
    ...
    def do_when_suspected_depression(
        ...
        if diagnosis_function is None:
            assert isinstance(hsi_event, HSI_Event), "No diagnosis test function, nor HSI_Event instance, supplied."
    ...
    def do_at_generic_first_appt(self, individual_properties: IndividualProperties, **kwargs) -> None:
...
class DepressionPollingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        assert (
            df.loc[((~df["is_pregnant"]) & df["de_recently_pregnant"]), "date_of_last_pregnancy"]
        ...
        onset_depression = apply_linear_model(
            self.module.linearModels["Risk_of_Depression_Onset_per3mo"], df.loc[df["is_alive"] & ~df["de_depr"]]
        )
        ...
        df.loc[
            onset_depression.loc[onset_depression].index, "de_intrinsic_3mo_risk_of_depr_resolution"
        ...
        will_self_harm_in_next_3mo = apply_linear_model(
            self.module.linearModels["Risk_of_SelfHarm_per3mo"], df.loc[df["is_alive"] & df["de_depr"]]
        )
        ...
        will_suicide_in_next_3mo = apply_linear_model(
            self.module.linearModels["Risk_of_Suicide_per3mo"], df.loc[df["is_alive"] & df["de_depr"]]
        )
...
class DepressionSelfHarmEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        self.sim.modules["SymptomManager"].change_symptom(
            person_id=person_id, disease_module=self.module, add_or_remove="+", symptom_string="Injuries_From_Self_Harm"
        )
...
class HSI_Depression_TalkingTherapy(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if self.num_of_sessions_had < 5:
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=self, topen=self.sim.date + pd.DateOffset(months=6), tclose=None, priority=1
            )
...
def compute_key_outputs_for_last_3_years(parsed_output):
    ...
    result["Proportion of 15+ ever depressed using anti-depressants, aged 15+y"] = depr.loc[
        period, "prop_antidepr_if_ever_depr"
    ].mean()
    ...
    result["Proportion of 15+ currently depressed using anti-depressants, aged 15+y"] = depr.loc[
        period, "prop_antidepr_if_curr_depr"
    ].mean()

    # Proportion of those persons ever depressed that have ever been diagnosed
    result["Proportion of those persons ever depressed that have ever been diagnosed"] = depr.loc[
        period, "p_ever_diagnosed_depression_if_ever_depressed"
    ].mean()
    ...
    tot_pop = get_15plus_pop_by_year(parsed_output["tlo.methods.demography"]["age_range_m"]) + get_15plus_pop_by_year(
    ...
    result["Rate of suicide incidence per 100k persons aged 15+"] = 1e5 * depr_event_rate["SuicideEvents"].mean()
