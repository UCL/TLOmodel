...
class Schisto(Module):
    ...
    PROPERTIES = {
        f"{module_prefix}_last_PZQ_date": Property(
            Types.DATE, "Day of the most recent treatment with PZQ"
        )
    }

    PARAMETERS = {
        "prob_sent_to_lab_test_children": Parameter(
            Types.REAL, "Probability that infected child gets sent to lab test"
        ),
        "prob_sent_to_lab_test_adults": Parameter(
            Types.REAL, "Probability that an infected adults gets sent to lab test"
        ),
        ...
        "MDA_coverage_prognosed": Parameter(
            Types.DATA_FRAME,
    ...
    def __init__(self, name=None, resourcefilepath=None, mda_execute=True):
        ...
        self.species = {
            _name: SchistoSpecies(self, name=_name)
            for _name in ("mansoni", "haematobium")
        }
        ...
        self.cols_of_infection_status = [
            _spec.infection_status_property for _spec in self.species.values()
        ]
    ...
    def read_parameters(self, data_folder):
        ...
        workbook = pd.read_excel(
            Path(self.resourcefilepath) / "ResourceFile_Schisto.xlsx", sheet_name=None
        )
        ...
        self._register_symptoms(
            symptoms_df.set_index("Symptom")["HSB_mapped_symptom"].to_dict()
        )
    ...
    def report_daly_values(self):
        ...
        disability_weights_for_each_person_with_symptoms = pd.Series(
            symptoms_being_caused
        ).apply(get_total_disability_weight)
    ...
    def do_on_presentation_with_symptoms(
        self,
        person_id: int,
        person_details: NamedTuple,
        symptoms: Union[list, set, tuple],
    ) -> HSI_Event:
        """
        ...
        set_of_symptoms_indicative_of_schisto = {
            "anemia",
            "haematuria",
            "bladder_pathology",
        }
        ...
        event = (
            HSI_Schisto_TestingFollowingSymptoms(
                module=self, person_id=person_id, person_age=person_details.age_years
            )
    ...
    def do_effect_of_treatment(self, person_id: Union[int, Sequence[int]]) -> None:
        ...
        for spec_prefix in [_spec.prefix for _spec in self.species.values()]:
            ...
            df.loc[
                person_id, f"{self.module_prefix}_{spec_prefix}_infection_status"
            ] = "Non-infected"
    ...
    def _load_parameters_from_workbook(self, workbook) -> dict:
        ...
        historical_mda = workbook["MDA_historical_Coverage"].set_index(
            ["District", "Year"]
        ...
        prognosed_mda = workbook["MDA_prognosed_Coverage"].set_index(
            ["District", "Frequency"]
        )[["Coverage PSAC", "Coverage SAC", "Coverage Adults"]]
    ...
    def _register_symptoms(self, symptoms: dict) -> None:
        ...
        self.sim.modules["SymptomManager"].register_symptom(
            *[
                Symptom(name=_symp)
                for _symp in symptoms
                if _symp not in generic_symptoms
            ]
        )
    ...
    def _get_disability_weight(self) -> dict:
        ...
        get_daly_weight = lambda _code: (
            self.sim.modules["HealthBurden"].get_daly_weight(_code)  # noqa: E731
            if _code is not None
            else 0.0
        )

        return {
            symptom: get_daly_weight(dw_code)
    ...
    def _get_item_code_for_praziquantel(self) -> int:
        ...
        return self.sim.modules["HealthSystem"].get_item_code_from_item_name(
            "Praziquantel, 600 mg (donated)"
        )
    ...
    def _schedule_mda_events(self) -> None:
        ...
        for (district, year), cov in self.parameters[
            "MDA_coverage_historical"
        ].iterrows():
            assert (
                district in self.sim.modules["Demography"].districts
            ), f"District {district} is not recognised."
        ...
        year_last_historical_mda = (
            self.parameters["MDA_coverage_historical"].reset_index().Year.max()
        )
        year_first_simulated_mda = year_last_historical_mda + 1

        for (district, frequency_in_months), cov in self.parameters[
            ...
            self.sim.schedule_event(
                SchistoMDAEvent(
                    ...
                    district=district,
                    coverage=cov.to_dict(),
                    months_between_repeats=(
                        frequency_in_months if frequency_in_months > 0 else None
                    ),
                ),
...
class SchistoSpecies:
    ...
    def get_parameters(self):
        ...
        params = {
            ...
            "worm_lifespan": Parameter(
                Types.REAL, "Lifespan of the worm in human host given in years"
            ),
            "high_intensity_threshold": Parameter(
                Types.REAL,
            ...
            "high_intensity_threshold_PSAC": Parameter(
                Types.REAL, "Worm burden threshold for high intensity infection in PSAC"
            ),
            "reservoir_2010": Parameter(
                Types.DATA_FRAME,
            ...
            "R0": Parameter(
                Types.DATA_FRAME, "Effective reproduction number, for the FOI"
            ),
        }
    ...
    def prefix_species_property(self, generic_property_name: str) -> str:
        ...
        species."""
        return (
            f"{self.schisto_module.module_prefix}_{self.prefix}_{generic_property_name}"
        )
    ...
    def load_parameters_from_workbook(self, workbook) -> dict:
        ...
        schisto_initial_reservoir = workbook[f"District_Params_{self.name}"].set_index(
            "District"
        )
        ...
        symptoms_df = workbook["Symptoms"]
        parameters["symptoms"] = (
            symptoms_df.loc[symptoms_df["Infection_type"].isin(["both", self.name])]
            .set_index("Symptom")["Prevalence"]
            .to_dict()
        )
    ...
    def initialise_simulation(self, sim):
        ...
        * Schedule the WormBurdenEvent for this species. (A recurring instance of this event will be scheduled for
        ...
        sim.schedule_event(
            SchistoInfectionWormBurdenEvent(module=self.schisto_module, species=self),
    ...
    def on_birth(self, mother_id, child_id):
        ...
        df.at[child_id, prop("harbouring_rate")] = rng.gamma(
            params["gamma_alpha"][district], size=1
        )
    ...
    def update_infectious_status_and_symptoms(self, idx: pd.Index) -> None:
        ...
        * Removes the symptoms if a person no longer has a high intensity infection (from any species)
        ...
        newly_have_high_infection = (original_status != "High-infection") & (
            correct_status == "High-infection"
        )
        idx_newly_have_high_infection = newly_have_high_infection.index[
            newly_have_high_infection
        ]
        ...
        df.loc[idx_changing, prop("infection_status")] = correct_status.loc[
            idx_changing
        ]
        ...
        high_infection_any_other_species = (
            df.loc[idx, cols_of_infection_status_for_other_species] == "High-infection"
        ).any(axis=1)
        ...
        sm.clear_symptoms(
            person_id=no_longer_high_infection, disease_module=schisto_module
        )
    ...
    def update_parameters_from_schisto_module(self) -> None:
        ...
        self.params = {
            remove_prefix(k, f"{self.prefix}_"): v
            for k, v in self.schisto_module.parameters.items()
    ...
    def _assign_initial_harbouring_rate(self, population) -> None:
        ...
        for district in districts:
            ...
            df.loc[in_the_district, prop("harbouring_rate")] = rng.gamma(
                hr, size=len(in_the_district)
            )
    ...
    def _assign_initial_worm_burden(self, population) -> None:
        ...
        for district in districts:
            ...
            for age_group in ["PSAC", "SAC", "Adults"]:
                ...
                in_the_district_and_age_group = df.index[
                    (df["district_of_residence"] == district)
                    & (df["age_years"].between(age_range[0], age_range[1]))
                ]
                contact_rates.loc[in_the_district_and_age_group] *= params[
                    f"beta_{age_group}"
                ]

            if len(in_the_district):
                harbouring_rates = df.loc[
                    in_the_district, prop("harbouring_rate")
                ].values
                ...
                if (reservoir_distr > 0) and (rates.sum() > 0):
                    chosen = rng.choice(
                        in_the_district, reservoir_distr, p=rates / rates.sum()
                    )
        ...
        for person_id in people_with_worms:
            ...
            self.schisto_module.sim.schedule_event(
                SchistoWormsNatDeath(
                    ...
                    species=self,
                    person_id=person_id,
                    number_of_worms_that_die=df.at[
                        person_id, prop("aggregate_worm_burden")
                    ],
                ),
    ...
    def log_infection_status(self) -> None:
        ...
        age_grp = df.loc[df.is_alive].age_years.map(
            self.schisto_module.age_group_mapper
        )
    ...
    * Schedules `SchistoMatureWorms` for when the worms mature to adult worms."""
    ...
    def apply(self, population):
        ...
        mean_count_burden_district_age_group = (
            df.loc[where]
            .groupby(["district_of_residence", age_group])[
                prop("aggregate_worm_burden")
            ]
            .agg([np.mean, np.size])
        )
        district_count = (
            df.loc[where]
            .groupby(by="district_of_residence")["district_of_residence"]
            .count()
        )
        beta_contribution_to_reservoir = (
            mean_count_burden_district_age_group["mean"] * beta_by_age_group
        )
        to_get_weighted_mean = (
            mean_count_burden_district_age_group["size"] / district_count
        )
        ...
        draw_worms = pd.Series(
            rng.poisson(
                (
                    df.loc[where, "district_of_residence"].map(worms_total) * rates
                ).fillna(0.0)
            ),
        ...
        for person_id, num_new_worms in to_establish.items():
            date_of_maturation = random_date(
                self.sim.date + pd.DateOffset(days=30),
                self.sim.date + pd.DateOffset(days=55),
                rng,
            )
            self.sim.schedule_event(
                SchistoMatureWorms(
...
class SchistoMatureWorms(Event, IndividualScopeEventMixin):
    ...
    * Updates the infection status and symptoms of the person accordingly."""
    ...
    def __init__(
        self,
        module: Module,
        species: SchistoSpecies,
        person_id: int,
        number_of_worms_that_mature: int,
    ):
    ...
    def apply(self, person_id):
        ...
        df.loc[
            person_id, prop("aggregate_worm_burden")
        ] += self.number_of_worms_that_mature
...
class SchistoWormsNatDeath(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        date_worm_acquisition = self.sim.date - pd.DateOffset(
            years=params["worm_lifespan"]
        )
        has_had_treatment_since_worm_acquisition = (
            date_last_pzq >= date_worm_acquisition
        )
        ...
        if not has_had_treatment_since_worm_acquisition:
            ...
            df.loc[person_id, prop("aggregate_worm_burden")] = max(
                0, worms_now - self.number_of_worms_that_die
            )
            self.species.update_infectious_status_and_symptoms(
...
class SchistoMDAEvent(Event, PopulationScopeEventMixin):
    ...
    def __init__(
        ...
        district: str,
        coverage: dict,
        months_between_repeats: Optional[int],
    ):
        ...
        * Schedules the recurrence of this event, if the MDA is to be repeated in the future.
        ...
        for age_group, cov in self.coverage.items():
            idx_to_receive_mda.extend(
                self._select_recipients(
                    district=self.district, age_group=age_group, coverage=cov
                )
            )
        ...
        if self.months_between_repeats is not None:
            self.sim.schedule_event(
                self, self.sim.date + pd.DateOffset(months=self.months_between_repeats)
            )
...
class HSI_Schisto_TestingFollowingSymptoms(HSI_Event, IndividualScopeEventMixin):
    ...
    def __init__(self, module, person_id: int, person_age: int):
        super().__init__(module, person_id=person_id)
        assert isinstance(module, Schisto)

        under_5 = person_age <= 5
        self.TREATMENT_ID = "Schisto_Treatment"
        self.EXPECTED_APPT_FOOTPRINT = self.make_appt_footprint(
            {"Under5OPD" if under_5 else "Over5OPD": 1}
        )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        will_test = self.module.rng.random_sample() < (
            params["prob_sent_to_lab_test_children"]
            if under_15
            else params["prob_sent_to_lab_test_adults"]
        )
        ...
        if will_test:
            ...
            if self._num_occurrences <= 1:
                next_occurence = self.sim.date + pd.DateOffset(
                    days=int(
                        self.module.rng.uniform(
                            params["delay_till_hsi_a_repeated"],
                ...
                self.module.sim.modules["HealthSystem"].schedule_hsi_event(
                    self, topen=next_occurence, tclose=None, priority=0
                )
...
class HSI_Schisto_MDA(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        beneficiaries_still_alive = list(
            set(self.beneficiaries_ids).intersection(
        ...
        if self.get_consumables(
            optional_item_codes={
                self.module.item_code_for_praziquantel: len(beneficiaries_still_alive)
            }
        ):
