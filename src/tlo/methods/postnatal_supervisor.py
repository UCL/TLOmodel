...
class PostnatalSupervisor(Module):
    ...
    }  # declare that this is a disease module (leave as empty set otherwise)
    ...
    PARAMETERS = {
        ...
        "rr_secondary_pph_endometritis": Parameter(
            Types.LIST,
            "relative risk of secondary postpartum haemorrhage in women with sepsis secondary to "
            "endometritis",
        ),
        ...
        "weekly_prob_gest_htn_pn": Parameter(
            Types.LIST,
            "weekly probability of a woman developing gestational hypertension during the postnatal "
            "period",
        ),
        ...
        "probs_for_mpe_matrix_pn": Parameter(
            Types.LIST,
            "probability of mild pre-eclampsia moving between states: gestational "
            "hypertension, severe gestational hypertension, mild pre-eclampsia, severe pre-eclampsia, "
            "eclampsia",
        ),
        "probs_for_spe_matrix_pn": Parameter(
            Types.LIST,
            "probability of severe pre-eclampsia moving between states: gestational "
            "hypertension, severe gestational hypertension, mild pre-eclampsia, severe pre-eclampsia, "
            "eclampsia",
        ),
        ...
        "aor_cs_fistula_age_lowest_education": Parameter(
            Types.LIST,
            "odds ratio for care seeking for treatment of obstetric fistula in women in the lowest "
            "education quantile",
        ),
        # TREATMENT EFFECTS
        ...
        "treatment_effect_anti_htns_progression_pn": Parameter(
            Types.LIST,
            "Treatment effect of oral anti hypertensives on progression from mild/mod to severe gestational"
            "hypertension",
        ),
    }
    ...
    PROPERTIES = {
        ...
        "pn_neonatal_sepsis_disab": Property(
            Types.CATEGORICAL,
            "Level of disability experience from a neonate post " "sepsis",
            categories=[
                "none",
                "mild_motor_and_cog",
                "mild_motor",
        ...
        "pn_deficiencies_following_pregnancy": Property(
            Types.INT,
            "bitset column, stores types of anaemia causing "
    ...
    def read_parameters(self, data_folder):
        parameter_dataframe = pd.read_excel(
            Path(self.resourcefilepath) / "ResourceFile_PostnatalSupervisor.xlsx",
    ...
    def initialise_simulation(self, sim):
        ...
        sim.schedule_event(
            PostnatalSupervisorEvent(self), sim.date + DateOffset(days=0)
        )
        ...
        self.pn_linear_models = {
            ...
            "obstetric_fistula": LinearModel.custom(
                postnatal_supervisor_lm.predict_obstetric_fistula, parameters=params
            ),
            # This equation is used to determine a mothers risk of secondary postpartum haemorrhage
            "secondary_postpartum_haem": LinearModel.custom(
                postnatal_supervisor_lm.predict_secondary_postpartum_haem,
                parameters=params,
            ),
            ...
            "sepsis_endometritis_late_postpartum": LinearModel.custom(
                postnatal_supervisor_lm.predict_sepsis_endometritis_late_postpartum,
                parameters=params,
            ),
            ...
            "sepsis_sst_late_postpartum": LinearModel.custom(
                postnatal_supervisor_lm.predict_sepsis_sst_late_postpartum,
                parameters=params,
            ),
            # This equation is used to determine a mothers risk of developing gestational hypertension in the postnatal
            # period
            "gest_htn_pn": LinearModel.custom(
                postnatal_supervisor_lm.predict_gest_htn_pn, parameters=params
            ),
            # This equation is used to determine a mothers risk of developing pre-eclampsia in in the postnatal
            # period
            "pre_eclampsia_pn": LinearModel.custom(
                postnatal_supervisor_lm.predict_pre_eclampsia_pn, module=self
            ),
            # This equation is used to determine a mothers risk of developing anaemia postnatal
            "anaemia_after_pregnancy": LinearModel.custom(
                postnatal_supervisor_lm.predict_anaemia_after_pregnancy, module=self
            ),
            ...
            "early_onset_neonatal_sepsis_week_1": LinearModel.custom(
                postnatal_supervisor_lm.predict_early_onset_neonatal_sepsis_week_1,
                parameters=params,
            ),
            ...
            "late_onset_neonatal_sepsis": LinearModel.custom(
                postnatal_supervisor_lm.predict_late_onset_neonatal_sepsis,
                parameters=params,
            ),
            ...
            "care_seeking_for_fistula_repair": LinearModel.custom(
                postnatal_supervisor_lm.predict_care_seeking_for_fistula_repair,
                parameters=params,
            ),
        }
    ...
    def further_on_birth_postnatal_supervisor(self, mother_id):
        ...
        if df.at[mother_id, "is_alive"]:
            ...
            risk_of_fistula = self.pn_linear_models["obstetric_fistula"].predict(
                df.loc[[mother_id]]
            )[mother_id]
            ...
            if self.rng.random_sample() < risk_of_fistula:
                ...
                store_dalys_in_mni(
                    mother_id, mni, f"{fistula_type}_fistula_onset", self.sim.date
                )
                ...
                care_seeking_for_repair = self.pn_linear_models[
                    "care_seeking_for_fistula_repair"
                ].predict(df.loc[[mother_id]])[mother_id]
                ...
                if care_seeking_for_repair:
                    ...
                    repair_date = self.sim.date + DateOffset(
                        days=(self.rng.randint(7, 42))
                    )
            ...
            if df.at[mother_id, "ps_htn_disorders"] != "none":
                df.at[mother_id, "pn_htn_disorders"] = df.at[
                    mother_id, "ps_htn_disorders"
                ]

            #  ANAEMIA...
            if df.at[mother_id, "ps_anaemia_in_pregnancy"] != "none":
                df.at[mother_id, "pn_anaemia_following_pregnancy"] = df.at[
                    mother_id, "ps_anaemia_in_pregnancy"
                ]

    def on_hsi_alert(self, person_id, treatment_id):
        logger.debug(
            key="message",
            data=f"This is PostnatalSupervisor, being alerted about a health system "
            f"interaction person {person_id} for: {treatment_id}",
        )

    def report_daly_values(self):
        logger.debug(
            key="message", data="This is PostnatalSupervisor reporting my health values"
        )
        df = self.sim.population.props
    ...
    def apply_linear_model(self, lm, df):
        ...
        if "abx_for_prom_given" in mni_df.columns:
            received_abx_for_prom = pd.Series(
                mni_df["abx_for_prom_given"], index=df.index
            )
        ...
        maternal_prom = pd.Series(
            df["ps_premature_rupture_of_membranes"], index=df.index
        )

        return self.rng.random_sample(len(df)) < lm.predict(
            df,
    ...
    def set_postnatal_complications_mothers(self, week):
        ...
        onset_sepsis_urinary = pd.Series(
            ...
            < params["prob_late_sepsis_urinary_tract"],
            index=at_sepsis_urinary.loc[at_sepsis_urinary].index,
        )

        # Iterate over each collection of women who will develop sepsis due to one of these reasons and update the
        # appropriate variables
        for index_slice in [
            onset_sepsis_endo.loc[onset_sepsis_endo].index,
        ...
        for person in new_sepsis.loc[new_sepsis].index:
            ...
            logger.info(
                key="maternal_complication",
                data={"person": person, "type": "sepsis", "timing": "postnatal"},
            )
        ...
        df.loc[
            onset_anaemia.loc[onset_anaemia].index, "pn_anaemia_following_pregnancy"
        ...
        for person in onset_anaemia.loc[onset_anaemia].index:
            store_dalys_in_mni(
                person,
                mni,
                f'{df.at[person, "pn_anaemia_following_pregnancy"]}_anaemia_pp_onset',
                self.sim.date,
            )
        ...
        women_with_htn = df.loc[
            ...
            & ~df["hs_is_inpatient"]
            & (
                df["pn_htn_disorders"].str.contains(
                    "gest_htn|severe_gest_htn|mild_pre_eclamp|severe_pre_eclamp|"
                    "eclampsia"
                )
            )
        ]
        ...
        resolvers = pd.Series(
            self.rng.random_sample(len(women_with_htn)) < params["prob_htn_resolves"],
        ...
        for person in resolvers.loc[resolvers].index:
            if not pd.isnull(mni[person]["hypertension_onset"]):
                store_dalys_in_mni(
                    person, mni, "hypertension_resolution", self.sim.date
                )
        ...
        def apply_risk(selected, risk_of_gest_htn_progression):
            ...
            risk_ghtn_remains_mild = 1.0 - (
                risk_of_gest_htn_progression + params["probs_for_mgh_matrix_pn"][2]
            )
            ...
            prob_matrix["gest_htn"] = [
                ...
                params["probs_for_mgh_matrix_pn"][2],
                0.0,
                0.0,
            ]
            ...
            def log_new_progressed_cases(disease):
                ...
                if not new_onset_disease.empty:
                    if disease == "severe_pre_eclamp":
                        df.loc[new_onset_disease.index, "pn_emergency_event_mother"] = (
                            True
                        )
        ...
        df.loc[pre_eclampsia.loc[pre_eclampsia].index, "pn_htn_disorders"] = (
            "mild_pre_eclamp"
        )
        ...
        df.loc[gest_hypertension.loc[gest_hypertension].index, "pn_htn_disorders"] = (
            "gest_htn"
        )
        ...
        die_from_htn = pd.Series(
            self.rng.random_sample(len(at_risk_of_death_htn))
            < params["weekly_prob_death_severe_gest_htn"],
            index=at_risk_of_death_htn.index,
        )
        ...
        care_seekers = pd.Series(
            self.rng.random_sample(len(can_seek_care))
        ...
        for person in care_seekers.loc[care_seekers].index:
            ...
            if (
                ...
                < self.sim.modules["Labour"].current_parameters["prob_delay_one_two_fd"]
            ):
        ...
        for person in care_seekers.loc[~care_seekers].index:
            self.apply_risk_of_maternal_or_neonatal_death_postnatal(
                mother_or_child="mother", individual_id=person
            )
        ...
        if week == 6:
            ...
            ].care_of_women_in_pregnancy_property_reset(
                id_or_index=week_6_women.loc[week_6_women].index
            )
    ...
    def apply_risk_of_neonatal_complications_in_week_one(self, child_id, mother_id):
        ...
        received_abx_for_prom = pd.Series(
            nci_df.at[child_id, "abx_for_prom_given"], index=df.loc[[child_id]].index
        )

        if mother_id in mni_df.index:
            chorio_in_preg = pd.Series(
                mni_df.at[mother_id, "chorio_in_preg"], index=df.loc[[child_id]].index
            )
        else:
        ...
        if self.rng.random_sample() < risk_eons:
            ...
            self.sim.modules["NewbornOutcomes"].newborn_care_info[child_id][
                "sepsis_postnatal"
            ] = True
    ...
    def set_postnatal_complications_neonates(self, upper_and_lower_day_limits):
        ...
        for person in onset_sepsis.loc[onset_sepsis].index:
            ...
            logger.info(
                key="newborn_complication",
                data={"newborn": person, "type": "late_onset_sepsis"},
            )

        # Then we determine if care will be sought for newly septic newborns
        care_seeking = pd.Series(
            self.rng.random_sample(len(onset_sepsis.loc[onset_sepsis]))
        ...
        for person in care_seeking.loc[care_seeking].index:
            ...
            from tlo.methods.newborn_outcomes import (
        ...
        for person in care_seeking.loc[~care_seeking].index:
            self.apply_risk_of_maternal_or_neonatal_death_postnatal(
                mother_or_child="child", individual_id=person
            )

    def apply_risk_of_maternal_or_neonatal_death_postnatal(
        self, mother_or_child, individual_id
    ):
        """
        ...
        if mother_or_child == "mother":
            ...
            if potential_cause_of_death:
                ...
                if (
                    mother.pn_htn_disorders == "severe_pre_eclamp"
...
class PostnatalSupervisorEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        ppp_in_days = (
            self.sim.date
            - df.loc[alive_and_recently_delivered, "la_date_most_recent_delivery"]
        )
        ppp_in_weeks = ppp_in_days / np.timedelta64(1, "W")
        rounded_weeks = np.ceil(ppp_in_weeks)

        df.loc[alive_and_recently_delivered, "pn_postnatal_period_in_weeks"] = (
            rounded_weeks
        )
        logger.debug(
            key="message", data=f"updating postnatal periods on date {self.sim.date}"
        )

        # Check that all women are week 1 or above
        if (
            not (
                df.loc[alive_and_recently_delivered, "pn_postnatal_period_in_weeks"] > 0
            )
            .all()
            .all()
        ):
        ...
        for upper_and_lower_day_limits in [[7, 15], [14, 22], [21, 29]]:
            self.module.set_postnatal_complications_neonates(
                upper_and_lower_day_limits=upper_and_lower_day_limits
            )
        ...
        week_8_postnatal_women_htn = (
            df.is_alive
            & df.la_is_postpartum
            & (df.pn_postnatal_period_in_weeks == 8)
            & (
                df.pn_htn_disorders.str.contains(
                    "gest_htn|severe_gest_htn|mild_pre_eclamp|severe_pre_eclamp|eclampsia"
                )
            )
        )
        ...
        for person in week_8_postnatal_women_anaemia.loc[
            week_8_postnatal_women_anaemia
        ].index:
            store_dalys_in_mni(
                person,
                mni,
                f'{df.at[person, "pn_anaemia_following_pregnancy"]}_anaemia'
                f"_pp_resolution",
                self.sim.date,
            )

        week_8_postnatal_women = (
        ...
        for person in week_5_postnatal_neonates.loc[week_5_postnatal_neonates].index:
            ...
            logger.info(
                ...
                data={"child": person, "visits": df.at[person, "nb_pnc_check"]},
            )
...
class PostnatalWeekOneMaternalEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, individual_id):
        ...
        if (
            not mother.la_is_postpartum
            or not (self.sim.date - mother.la_date_most_recent_delivery)
            < pd.to_timedelta(7, unit="d")
            or mni[individual_id]["passed_through_week_one"]
        ):
            logger.info(
                key="error",
                data="Mother incorrectly scheduled to arrive at PostnatalWeekOneMaternalEvent",
            )
            return
        ...
        mode_of_delivery = pd.Series(
            mni[individual_id]["mode_of_delivery"], index=df.loc[[individual_id]].index
        )

        # Determine individual risk of sepsis for each possible cause
        risk_sepsis_endometritis = self.module.pn_linear_models[
            "sepsis_endometritis_late_postpartum"
        ].predict(df.loc[[individual_id]], mode_of_delivery=mode_of_delivery)[
            individual_id
        ]

        risk_sepsis_skin_soft_tissue = self.module.pn_linear_models[
            "sepsis_sst_late_postpartum"
        ].predict(df.loc[[individual_id]], mode_of_delivery=mode_of_delivery)[
            individual_id
        ]
        ...
        if endo_result or ut_result or ssti_result:
            ...
            logger.info(
                key="maternal_complication",
                data={"person": individual_id, "type": "sepsis", "timing": "postnatal"},
            )
        ...
        endometritis = pd.Series(
            mni[individual_id]["endo_pp"], index=df.loc[[individual_id]].index
        )

        risk_secondary_pph = self.module.pn_linear_models[
            "secondary_postpartum_haem"
        ].predict(df.loc[[individual_id]], endometritis=endometritis)[individual_id]
        ...
        if mother.pn_anaemia_following_pregnancy == "none":
            risk_anaemia_after_pregnancy = self.module.pn_linear_models[
                "anaemia_after_pregnancy"
            ].predict(df.loc[[individual_id]])[individual_id]

            if risk_anaemia_after_pregnancy > self.module.rng.random_sample():
                random_choice_severity = self.module.rng.choice(
                    ["mild", "moderate", "severe"],
                    p=params["prob_type_of_anaemia_pn"],
                    size=1,
                )

                df.at[individual_id, "pn_anaemia_following_pregnancy"] = (
                    random_choice_severity
                )
                ...
                store_dalys_in_mni(
                    individual_id,
                    mni,
                    f'{df.at[individual_id, "pn_anaemia_following_pregnancy"]}_'
                    f"anaemia_pp_onset",
                    self.sim.date,
                )
        ...
        if "none" not in mother.pn_htn_disorders:
            if "resolved" in mother.pn_htn_disorders:
                ...
                if not pd.isnull(mni[individual_id]["hypertension_onset"]):
                    ...
                    # weight definition)
                    store_dalys_in_mni(
                        individual_id, mni, "hypertension_resolution", self.sim.date
                    )
                ...
                if df.at[individual_id, "la_gest_htn_on_treatment"]:
                    treatment_reduced_risk = (
                        prob_matrix["gest_htn"]["severe_gest_htn"]
                        * params["treatment_effect_anti_htns_progression_pn"]
                    )
                    prob_matrix.at["severe_gest_htn", "gest_htn"] = (
                        treatment_reduced_risk
                    )
                    prob_matrix.at["gest_htn", "gest_htn"] = 1.0 - (
                        treatment_reduced_risk
                        + prob_matrix["gest_htn"]["mild_pre_eclamp"]
                    )

                current_status = df.loc[[individual_id], "pn_htn_disorders"]
                new_status = util.transition_states(
                    current_status, prob_matrix, self.module.rng
                )
        ...
        if df.at[individual_id, "pn_htn_disorders"] == "none":
            risk_pe_after_pregnancy = self.module.pn_linear_models[
                "pre_eclampsia_pn"
            ].predict(df.loc[[individual_id]])[individual_id]
            ...
            if risk_pe_after_pregnancy > self.module.rng.random_sample():
                ...
                risk_gh_after_pregnancy = self.module.pn_linear_models[
                    "gest_htn_pn"
                ].predict(df.loc[[individual_id]])[individual_id]
        ...
        if (
            df.at[individual_id, "pn_sepsis_late_postpartum"]
            or df.at[individual_id, "pn_postpartum_haem_secondary"]
            or (
                (df.at[individual_id, "pn_htn_disorders"] == "severe_pre_eclamp")
                and mni[individual_id]["new_onset_spe"]
            )
            or (df.at[individual_id, "pn_htn_disorders"] == "eclampsia")
        ):
            ...
            # If she will not receive treatment for her complications we apply risk of death now
                ...
                self.module.apply_risk_of_maternal_or_neonatal_death_postnatal(
                    mother_or_child="mother", individual_id=individual_id
                )
        else:
            ...
            if mni[individual_id]["will_receive_pnc"] == "late":
                appt_date = self.sim.date + pd.DateOffset(
                    self.module.rng.randint(0, 35)
                )
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    pnc_one_maternal,
                    priority=0,
                    topen=appt_date,
                    tclose=appt_date + pd.DateOffset(days=2),
                )
...
class PostnatalWeekOneNeonatalEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, individual_id):
        ...
        if not child.age_days < 7 or nci[individual_id]["passed_through_week_one"]:
            logger.info(
                key="error",
                data="Child incorrectly scheduled for PostnatalWeekOneNeonatalEvent",
            )
            return
        ...
        self.module.apply_risk_of_neonatal_complications_in_week_one(
            child_id=individual_id, mother_id=mother_id
        )
        ...
        if df.at[individual_id, "pn_sepsis_early_neonatal"]:
            if (nci[individual_id]["will_receive_pnc"] == "late") or (
                self.module.rng.random_sample()
                < params["prob_care_seeking_postnatal_" "emergency_neonate"]
            ):
                ...
                self.module.apply_risk_of_maternal_or_neonatal_death_postnatal(
                    mother_or_child="child", individual_id=individual_id
                )

        # Finally we determine if newborns without infection will receive PNC
        elif not df.at[individual_id, "pn_sepsis_early_neonatal"] and (
            nci[individual_id]["will_" "receive_pnc"] == "late"
        ):
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                ...
                priority=0,
                topen=self.sim.date
                + pd.DateOffset(self.module.rng.randint(0, days_till_day_7)),
                tclose=None,
            )


class HSI_PostnatalSupervisor_TreatmentForObstetricFistula(
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        pregnancy_helper_functions.store_dalys_in_mni(
            ...
            f'{df.at[person_id, "pn_obstetric_fistula"]}_fistula_resolution',
            self.sim.date,
        )
