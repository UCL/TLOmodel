...
class PostnatalSupervisor(Module):
    ...
    PARAMETERS = {
        ...
        "prevalence_type_of_fistula": Parameter(Types.LIST, "prevalence of 1.) vesicovaginal 2.)rectovaginal fistula "),
        # SECONDARY POSTPARTUM HAEMORRHAGE
        "prob_secondary_pph": Parameter(Types.LIST, "baseline probability of secondary PPH"),
        ...
        "prob_secondary_pph_severity": Parameter(Types.LIST, "probability of mild, moderate or severe secondary PPH"),
        "cfr_secondary_postpartum_haemorrhage": Parameter(Types.LIST, "case fatality rate for secondary pph"),
        # HYPERTENSIVE DISORDERS
        "prob_htn_resolves": Parameter(Types.LIST, "Weekly probability of resolution of hypertension"),
        ...
        "cfr_eclampsia": Parameter(Types.LIST, "case fatality rate of eclampsia in the postnatal period"),
        ...
        # ANAEMIA
        "baseline_prob_anaemia_per_week": Parameter(Types.LIST, "Weekly probability of anaemia in pregnancy"),
        ...
        "cfr_postpartum_sepsis": Parameter(Types.LIST, "case fatality rate for postnatal sepsis"),
        # NEWBORN SEPSIS
        ...
        "rr_eons_maternal_prom": Parameter(Types.LIST, "relative risk of EONS in newborns whose mothers have PROM"),
        "rr_eons_preterm_neonate": Parameter(Types.LIST, "relative risk of EONS in preterm newborns"),
        "cfr_early_onset_neonatal_sepsis": Parameter(Types.LIST, "case fatality for early onset neonatal sepsis"),
        "prob_late_onset_neonatal_sepsis": Parameter(
            Types.LIST, "probability of late onset neonatal sepsis (all cause)"
        ),
        "cfr_late_neonatal_sepsis": Parameter(Types.LIST, "Risk of death from late neonatal sepsis"),
    ...
    PROPERTIES = {
        "pn_postnatal_period_in_weeks": Property(
            Types.REAL, "The number of weeks a woman is in the postnatal period " "(1-6)"
        ),
        ...
        "pn_postpartum_haem_secondary": Property(
            Types.BOOL, "Whether this woman is experiencing a secondary " "postpartum haemorrhage"
        ),
        ...
        "pn_sepsis_early_neonatal": Property(
            Types.BOOL, "Whether this neonate has developed early onset neonatal" " sepsis during week one of life"
        ),
        "pn_sepsis_late_neonatal": Property(
            Types.BOOL, "Whether this neonate has developed late neonatal sepsis " "following discharge"
        ),
    ...
    def read_parameters(self, data_folder):
        parameter_dataframe = read_csv_files(
            Path(self.resourcefilepath) / "ResourceFile_PostnatalSupervisor", files="parameter_values"
        )
    ...
    def initialise_simulation(self, sim):
        ...
        sim.schedule_event(PostnatalSupervisorEvent(self), sim.date + DateOffset(days=0))
        ...
        self.pn_linear_models = {
            ...
            "secondary_postpartum_haem": LinearModel.custom(
                postnatal_supervisor_lm.predict_secondary_postpartum_haem, parameters=params
            ),
    ...
    def further_on_birth_postnatal_supervisor(self, mother_id):
        ...
        if df.at[mother_id, "is_alive"]:
            ...
            risk_of_fistula = self.pn_linear_models["obstetric_fistula"].predict(df.loc[[mother_id]])[mother_id]
            ...
            if self.rng.random_sample() < risk_of_fistula:
                ...
                fistula_type = self.rng.choice(
                    ["vesicovaginal", "rectovaginal"], p=params["prevalence_type_of_fistula"]
                )
                ...
                logger.info(
                    ...
                    data={"person": mother_id, "type": f"{fistula_type}_fistula", "timing": "postnatal"},
                )

                # Determine if she will seek care for repair
                care_seeking_for_repair = self.pn_linear_models["care_seeking_for_fistula_repair"].predict(
                    df.loc[[mother_id]]
                )[mother_id]

                # Schedule repair to occur for 1 week postnatal
                if care_seeking_for_repair:
                    repair_hsi = HSI_PostnatalSupervisor_TreatmentForObstetricFistula(self, person_id=mother_id)
                    repair_date = self.sim.date + DateOffset(days=(self.rng.randint(7, 42)))

                    self.sim.modules["HealthSystem"].schedule_hsi_event(
                        repair_hsi, priority=0, topen=repair_date, tclose=repair_date + DateOffset(days=7)
                    )
    ...
    def set_postnatal_complications_mothers(self, week):
        ...
        for person in new_sepsis.loc[new_sepsis].index:
            ...
            logger.info(key="maternal_complication", data={"person": person, "type": "sepsis", "timing": "postnatal"})
        ...
        for person in onset_pph.loc[onset_pph].index:
            ...
            logger.info(
                key="maternal_complication",
                data={"person": person, "type": "secondary_postpartum_haemorrhage", "timing": "postnatal"},
            )
        ...
        for person in onset_anaemia.loc[onset_anaemia].index:
            store_dalys_in_mni(
                person, mni, f'{df.at[person, "pn_anaemia_following_pregnancy"]}_anaemia_pp_onset', self.sim.date
            )
            ...
            logger.info(
                ...
                data={
                    "person": person,
                    "type": f'{df.at[person, "pn_anaemia_following_pregnancy"]}' f'_anaemia',
                    "timing": "postnatal",
                },
            )
        ...
        resolvers = pd.Series(
            self.rng.random_sample(len(women_with_htn)) < params["prob_htn_resolves"], index=women_with_htn.index
        )
            ...
            def log_new_progressed_cases(disease):
                ...
                if not new_onset_disease.empty:
                    ...
                    for person in new_onset_disease.index:
                        logger.info(
                            key="maternal_complication", data={"person": person, "type": disease, "timing": "postnatal"}
                        )
        ...
        apply_risk(
            women_with_htn_on_anti_htns,
            (risk_progression_mild_to_severe_htn * params["treatment_effect_anti_htns_progression_pn"]),
        )
        ...
        for person in pre_eclampsia.loc[pre_eclampsia].index:
            logger.info(
                key="maternal_complication", data={"person": person, "type": "mild_pre_eclamp", "timing": "postnatal"}
            )
        ...
        for person in gest_hypertension.loc[gest_hypertension].index:
            logger.info(
                key="maternal_complication", data={"person": person, "type": "mild_gest_htn", "timing": "postnatal"}
            )
        ...
        die_from_htn = pd.Series(
            self.rng.random_sample(len(at_risk_of_death_htn)) < params["weekly_prob_death_severe_gest_htn"],
        ...
        for person in care_seekers.loc[care_seekers].index:
            ...
            postnatal_check = HSI_Labour_ReceivesPostnatalCheck(self.sim.modules["Labour"], person_id=person)

            self.sim.modules["HealthSystem"].schedule_hsi_event(
                postnatal_check, priority=0, topen=self.sim.date, tclose=self.sim.date + DateOffset(days=2)
            )
    ...
    def apply_risk_of_neonatal_complications_in_week_one(self, child_id, mother_id):
        ...
        if self.rng.random_sample() < risk_eons:
            ...
            logger.info(key="newborn_complication", data={"newborn": child_id, "type": "early_onset_sepsis"})
    ...
    def set_postnatal_complications_neonates(self, upper_and_lower_day_limits):
        ...
        for person in onset_sepsis.loc[onset_sepsis].index:
            ...
            logger.info(key="newborn_complication", data={"newborn": person, "type": "late_onset_sepsis"})
        ...
        for person in care_seeking.loc[care_seeking].index:
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                postnatal_check, priority=0, topen=self.sim.date, tclose=self.sim.date + DateOffset(days=1)
            )
    ...
    def apply(self, population):
        ...
        for person in week_8_postnatal_women_anaemia.loc[week_8_postnatal_women_anaemia].index:
            store_dalys_in_mni(
                person,
                mni,
                f'{df.at[person, "pn_anaemia_following_pregnancy"]}_anaemia' f'_pp_resolution',
                self.sim.date,
            )
        ...
        week_5_postnatal_neonates = (
            df.is_alive & (df.age_days > 28) & (df.age_days < 36) & (df.date_of_birth > self.sim.start_date)
        )
        ...
        for person in week_5_postnatal_neonates.loc[week_5_postnatal_neonates].index:
            ...
            logger.info(key="total_neo_pnc_visits", data={"child": person, "visits": df.at[person, "nb_pnc_check"]})
...
class PostnatalWeekOneMaternalEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, individual_id):
        ...
        if (
            ...
            or not (self.sim.date - mother.la_date_most_recent_delivery) < pd.to_timedelta(7, unit="d")
        ...
        risk_sepsis_endometritis = self.module.pn_linear_models["sepsis_endometritis_late_postpartum"].predict(
            df.loc[[individual_id]], mode_of_delivery=mode_of_delivery
        )[individual_id]
        ...
        if endo_result or ut_result or ssti_result:
            ...
            logger.info(
                key="maternal_complication", data={"person": individual_id, "type": "sepsis", "timing": "postnatal"}
            )
        ...
        risk_secondary_pph = self.module.pn_linear_models["secondary_postpartum_haem"].predict(
            df.loc[[individual_id]], endometritis=endometritis
        )[individual_id]
        ...
        if risk_secondary_pph > self.module.rng.random_sample():
            ...
            logger.info(
                key="maternal_complication",
                data={"person": individual_id, "type": "secondary_postpartum_haemorrhage", "timing": "postnatal"},
            )
        ...
        if mother.pn_anaemia_following_pregnancy == "none":
            ...
            if risk_anaemia_after_pregnancy > self.module.rng.random_sample():
                random_choice_severity = self.module.rng.choice(
                    ["mild", "moderate", "severe"], p=params["prob_type_of_anaemia_pn"], size=1
                )
                ...
                store_dalys_in_mni(
                    individual_id,
                    mni,
                    f'{df.at[individual_id, "pn_anaemia_following_pregnancy"]}_' f'anaemia_pp_onset',
                    self.sim.date,
                )
            ...
            if "resolved" in mother.pn_htn_disorders:
                ...
                if df.at[individual_id, "la_gest_htn_on_treatment"]:
                    treatment_reduced_risk = (
                        prob_matrix["gest_htn"]["severe_gest_htn"] * params["treatment_effect_anti_htns_progression_pn"]
                    )
                    prob_matrix.at["severe_gest_htn", "gest_htn"] = treatment_reduced_risk
                    prob_matrix.at["gest_htn", "gest_htn"] = 1.0 - (
                        treatment_reduced_risk + prob_matrix["gest_htn"]["mild_pre_eclamp"]
                    )
                ...
                def log_new_progressed_cases(disease):
                    ...
                    if not new_onset_disease.empty:
                        for person in new_onset_disease.index:
                            logger.info(
                                key="maternal_complication",
                                data={"person": person, "type": disease, "timing": "postnatal"},
                            )
                            if disease == "severe_pre_eclamp":
                                mni[person]["new_onset_spe"] = True
        ...
        if df.at[individual_id, "pn_htn_disorders"] == "none":
            risk_pe_after_pregnancy = self.module.pn_linear_models["pre_eclampsia_pn"].predict(df.loc[[individual_id]])[
                individual_id
            ]
            ...
            if risk_pe_after_pregnancy > self.module.rng.random_sample():
                ...
                logger.info(
                    key="maternal_complication",
                    data={"person": individual_id, "type": "mild_pre_eclamp", "timing": "postnatal"},
                )

            else:
                risk_gh_after_pregnancy = self.module.pn_linear_models["gest_htn_pn"].predict(df.loc[[individual_id]])[
                    individual_id
                ]
                if risk_gh_after_pregnancy > self.module.rng.random_sample():
                    ...
                    logger.info(
                        key="maternal_complication",
                        data={"person": individual_id, "type": "mild_gest_htn", "timing": "postnatal"},
                    )
        ...
        pnc_one_maternal = HSI_Labour_ReceivesPostnatalCheck(self.sim.modules["Labour"], person_id=individual_id)
    ...
    def apply(self, individual_id):
        ...
        if df.at[individual_id, "pn_sepsis_early_neonatal"]:
            if (nci[individual_id]["will_receive_pnc"] == "late") or (
                self.module.rng.random_sample() < params["prob_care_seeking_postnatal_" "emergency_neonate"]
            ):
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    pnc_one_neonatal, priority=0, topen=self.sim.date, tclose=self.sim.date + pd.DateOffset(days=1)
                )
            else:
