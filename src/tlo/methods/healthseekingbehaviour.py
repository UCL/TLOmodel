...
class HealthSeekingBehaviour(Module, GenericFirstAppointmentsMixin):
    ...
    PARAMETERS = {
        ...
        "odds_ratio_children_sex_Female": Parameter(
            Types.REAL, "odds ratio for health-care seeking (children) if sex" " is Female"
        ),
        ...
        "odds_ratio_children_setting_urban": Parameter(
            Types.REAL, "odds ratio for health-care seeking (children) if" " setting is Urban"
        ),
        "odds_ratio_children_region_Central": Parameter(
            Types.REAL, "odds ratio for health-care seeking (children) if" " region is Central"
        ),
        "odds_ratio_children_region_Southern": Parameter(
            Types.REAL, "odds ratio for health-care seeking (children) if" " region is Southern"
        ),
        "odds_ratio_children_wealth_higher": Parameter(
            Types.REAL, "odds ratio for health-care seeking (children) if " "wealth is in categories 4 or 5"
        ),
        ...
        "odds_ratio_adults_sex_Female": Parameter(
            Types.REAL, "odds ratio for health-care seeking (adults) if sex is" " Female"
        ),
        ...
        "odds_ratio_adults_setting_urban": Parameter(
            Types.REAL, "odds ratio for health-care seeking (adults) if " "setting is Urban"
        ),
        "odds_ratio_adults_region_Central": Parameter(
            Types.REAL, "odds ratio for health-care seeking (adults) if " "region is Central"
        ),
        "odds_ratio_adults_region_Southern": Parameter(
            Types.REAL, "odds ratio for health-care seeking (adults) if " "region is Southern"
        ),
        "odds_ratio_adults_wealth_higher": Parameter(
            Types.REAL, "odds ratio for health-care seeking (adults) if wealth" " is in categories 4 or 5"
        ),
    ...
    def read_parameters(self, data_folder):
        ...
        wb.loc[wb["parameter_name"] == "force_any_symptom_to_lead_to_healthcareseeking", "value"] = wb.loc[
            wb["parameter_name"] == "force_any_symptom_to_lead_to_healthcareseeking", "value"
        ].apply(pd.eval)
        ...
        * assemble the health-care seeking information from the registered symptoms
        ...
        for symptom in self.sim.modules["SymptomManager"].all_registered_symptoms:
            ...
            if not symptom.no_healthcareseeking_in_children:
                ...
                self.prob_seeks_emergency_appt_in_children[symptom.name] = symptom.prob_seeks_emergency_appt_in_children
            # Adults:
            if not symptom.no_healthcareseeking_in_adults:
                ...
                self.prob_seeks_emergency_appt_in_adults[symptom.name] = symptom.prob_seeks_emergency_appt_in_adults
        # Define the linear models that govern healthcare seeking
    ...
    def define_linear_models(self):
        ...
        def predict_healthcareseeking(self, df, rng=None, subgroup=None, care_seeking_odds_ratios=None):
        ...
        for subgroup, prob_emergency_appt in zip(
            ("children", "adults"),
            (self.prob_seeks_emergency_appt_in_children, self.prob_seeks_emergency_appt_in_adults),
        ):
...
class HealthSeekingBehaviourPoll(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        person_ids_with_newly_onset_symptoms = sorted(symptom_manager.get_persons_with_newly_onset_symptoms())
        ...
        for subgroup, subgroup_name, care_seeking_odds_ratios, hsb_model, emergency_appt_model in zip(
            ...
            (module.emergency_appt_linear_models["children"], module.emergency_appt_linear_models["adults"]),
        ):
