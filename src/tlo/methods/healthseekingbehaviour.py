...
class HealthSeekingBehaviour(Module):
    ...
    PARAMETERS = {
        "force_any_symptom_to_lead_to_healthcareseeking": Parameter(
            Types.BOOL,
            "Whether every symptom [except those that declare they should not lead to any healthcare "
            "seeking] should always lead to healthcare seeking immediately.",
        ),
        "prob_non_emergency_care_seeking_by_level": Parameter(
            Types.LIST,
            "The probability of going to each facility-level when non-emergency care is sought. The "
        ...
        "baseline_odds_of_healthcareseeking_children": Parameter(
            Types.REAL,
            "odds of health-care seeking (children:"
            " 0-14) if male, 0-5 years-old, living in"
            " a rural setting in the Northern region,"
            " and not in the wealth categories 4 or "
            "5",
        ),
        "odds_ratio_children_sex_Female": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (children) if sex" " is Female",
        ),
        "odds_ratio_children_age_5to14": Parameter(
            Types.REAL, "odds ratio for health-care seeking (children) if aged" " 5-14"
        ),
        "odds_ratio_children_setting_urban": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (children) if" " setting is Urban",
        ),
        "odds_ratio_children_region_Central": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (children) if" " region is Central",
        ),
        "odds_ratio_children_region_Southern": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (children) if" " region is Southern",
        ),
        "odds_ratio_children_wealth_higher": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (children) if "
            "wealth is in categories 4 or 5",
        ),
        "baseline_odds_of_healthcareseeking_adults": Parameter(
            Types.REAL,
            "odds of health-care seeking (adults: 15+) "
            "if male, 15-34 year-olds, living in a rural"
            " setting in the Northern region, and not in"
            " the wealth categories 4 or 5.",
        ),
        "odds_ratio_adults_sex_Female": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (adults) if sex is" " Female",
        ),
        "odds_ratio_adults_age_35to59": Parameter(
            Types.REAL, "odds ratio for health-care seeking (adults) if aged" " 35-59"
        ),
        "odds_ratio_adults_age_60plus": Parameter(
            Types.REAL, "odds ratio for health-care seeking (adults) if aged" " 60+"
        ),
        "odds_ratio_adults_setting_urban": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (adults) if " "setting is Urban",
        ),
        "odds_ratio_adults_region_Central": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (adults) if " "region is Central",
        ),
        "odds_ratio_adults_region_Southern": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (adults) if " "region is Southern",
        ),
        "odds_ratio_adults_wealth_higher": Parameter(
            Types.REAL,
            "odds ratio for health-care seeking (adults) if wealth"
            " is in categories 4 or 5",
        ),
        "max_days_delay_to_generic_HSI_after_symptoms": Parameter(
            Types.INT,
    ...
    def __init__(
        ...
        force_any_symptom_to_lead_to_healthcareseeking=None,
    ):
        super().__init__(name)
        ...
        self.arg_force_any_symptom_to_lead_to_healthcareseeking = (
    ...
    def read_parameters(self, data_folder):
        ...
        wb.loc[
            wb["parameter_name"] == "force_any_symptom_to_lead_to_healthcareseeking",
            "value",
        ] = wb.loc[
            wb["parameter_name"] == "force_any_symptom_to_lead_to_healthcareseeking",
            "value",
        ...
        assert isinstance(
            self.parameters["force_any_symptom_to_lead_to_healthcareseeking"], bool
        )
    ...
    def define_linear_models(self):
        ...
        def predict_healthcareseeking(
            ...
            if subgroup is None or care_seeking_odds_ratios is None:
                raise ValueError(
                    "subgroup and care_seeking_odds_ratios must both be specified"
                )

            result = pd.Series(
                data=p[f"baseline_odds_of_healthcareseeking_{subgroup}"], index=df.index
            )
            # Predict behaviour due to the 'average symptom'
            if subgroup == "children":
                result[df.age_years >= 5] *= p["odds_ratio_children_age_5to14"]
            if subgroup == "adults":
                result[df.age_years.between(35, 59)] *= p[
            ...
            result[df.region_of_residence == "Central"] *= p[
                f"odds_ratio_{subgroup}_region_Central"
            ]
            result[df.region_of_residence == "Southern"] *= p[
                f"odds_ratio_{subgroup}_region_Southern"
            ]
            result[(df.li_wealth == 4) | (df.li_wealth == 5)] *= p[
                f"odds_ratio_{subgroup}_wealth_higher"
            ]
        ...
        for subgroup in ("children", "adults"):
            self.hsb_linear_models[subgroup] = LinearModel.custom(
                predict_function=predict_healthcareseeking
            )
        ...
        for subgroup, prob_emergency_appt in zip(
            ...
            self.emergency_appt_linear_models[subgroup] = LinearModel.custom(
                predict_function=custom_predict, prob_emergency_appt=prob_emergency_appt
            )
...
class HealthSeekingBehaviourPoll(RegularEvent, PopulationScopeEventMixin):
    ...
    def __init__(self, module):
        ...
        """
        super().__init__(
            module, frequency=DateOffset(days=1), priority=Priority.LAST_HALF_OF_DAY
        )
    ...
    def apply(self, population):
        ...
        newly_symptomatic_persons = population.props.loc[
            person_ids_with_newly_onset_symptoms
        ]
        alive_newly_symptomatic_persons = newly_symptomatic_persons[
            newly_symptomatic_persons.is_alive
        ]
        ...
        for (
            ...
            if module.force_any_symptom_to_lead_to_healthcareseeking:
                ...
                will_seek_care = idx_where_true(
                    self._has_any_symptoms(
                        subgroup, symptoms_that_allow_healthcareseeking
                    )
                )
            else:
                ...
                will_seek_care = will_seek_care.union(
                    idx_where_true(subgroup.hs_is_inpatient)
                )

            # Determine if the care sought will be emergency care (for those that seek care):
            will_seek_emergency_care = idx_where_true(
                emergency_appt_model.predict(
                    subgroup.loc[will_seek_care],
                    module.rng,
            ...
            will_seek_non_emergency_care = will_seek_care.difference(
                will_seek_emergency_care
            )
            ...
            if not module.force_any_symptom_to_lead_to_healthcareseeking:
                care_seeking_dates = (
                    ...
                    + module.rng.randint(
                        0, max_delay + 1, size=len(will_seek_non_emergency_care)
                    )
                    # (The +1 is because `randint` takes the upper bound to be excluded.)
                )
            else:
                care_seeking_dates = np.full(
                    len(will_seek_non_emergency_care), self.sim.date
                )
