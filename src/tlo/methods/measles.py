...
class Measles(Module):
    ...
    PROPERTIES = {
        ...
        "me_on_treatment": Property(
            Types.BOOL, "Currently on treatment for measles complications"
        ),
    }
    ...
    def __init__(self, name=None, resourcefilepath=None):
        ...
        self.symptom_probs = (
            None  # (will store the probabilities of symptom onset by age)
        )
    ...
    def read_parameters(self, data_folder):
        ...
        self.parameters["case_fatality_rate"] = (
            workbook["cfr"].set_index("age")["probability"].to_dict()
        )
        ...
        if "HealthBurden" in self.sim.modules.keys():
            self.parameters["daly_wts"] = {
                "rash": self.sim.modules["HealthBurden"].get_daly_weight(
                    sequlae_code=205
                ),
                "fever": self.sim.modules["HealthBurden"].get_daly_weight(
                    sequlae_code=205
                ),
                "diarrhoea": self.sim.modules["HealthBurden"].get_daly_weight(
                    sequlae_code=205
                ),
                "encephalitis": self.sim.modules["HealthBurden"].get_daly_weight(
                    sequlae_code=206
                ),
                "otitis_media": self.sim.modules["HealthBurden"].get_daly_weight(
                    sequlae_code=205
                ),
                "respiratory_symptoms": self.sim.modules[
                    "HealthBurden"
                ].get_daly_weight(sequlae_code=206),
                "eye_complaint": self.sim.modules["HealthBurden"].get_daly_weight(
                    sequlae_code=205
                ),
            }
        ...
        self.sim.modules["SymptomManager"].register_symptom(
            Symptom(
                ...
                odds_ratio_health_seeking_in_adults=2.5,
            )  # non-emergencies
        )
        ...
        self.sim.modules["SymptomManager"].register_symptom(
            Symptom.emergency("encephalitis")
        )
    ...
    def initialise_population(self, population):
        ...
        df.loc[df.is_alive, "me_has_measles"] = (
            False  # default: no individuals infected
        )
    ...
    def initialise_simulation(self, sim):
        ...
        self.consumables = {
            "vit_A": self.sim.modules["HealthSystem"].get_item_code_from_item_name(
                "Vitamin A, caplet, 100,000 IU"
            ),
            "severe_diarrhoea": self.sim.modules[
                "HealthSystem"
            ].get_item_code_from_item_name("ORS, sachet"),
            "severe_pneumonia": self.sim.modules[
                "HealthSystem"
    ...
    def process_parameters(self):
        ...
        probs = self.parameters["symptom_prob"].set_index(["age", "symptom"])[
            "probability"
        ]
        self.symptom_probs = {
            level: probs.loc[(level, slice(None))].to_dict()
            for level in probs.index.levels[0]
        }
    ...
    * Vaccination lowers an individual's likelihood of getting the infection (one dose will be 85% protective and two
    ...
    def apply(self, population):
        ...
        if "Epi" in self.sim.modules:
            ...
            protected_by_vaccine.loc[(df.va_measles == 1)] *= (
                1 - p["vaccine_efficacy_1"]
            )  # partially susceptible
            protected_by_vaccine.loc[(df.va_measles > 1)] *= (
                1 - p["vaccine_efficacy_2"]
            )  # partially susceptible
        ...
        if new_inf.any():
            ...
            for person_index in new_inf:
                self.sim.schedule_event(
                    ...
                    random_date(
                        start=self.sim.date,
                        end=self.sim.date + pd.DateOffset(months=1),
                        rng=rng,
                    ),
                )
...
class MeaslesOnsetEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        ref_age = df.at[person_id, "age_years"].clip(
            max=30
        )  # (For purpose of look-up age limit is 30 years)
        ...
        prob_death = self.get_prob_death(
            ref_age
        )  # Look-up the probability of death for this person
        ...
        if rng.random_sample() < prob_death:
            ...
            self.sim.schedule_event(
                MeaslesSymptomResolveEvent(self.module, person_id), symp_resolve
            )
    ...
    def assign_symptoms(self, _age):
        ...
        symptoms_to_onset = [
            ...
            for (_symp, _prob), _rand in zip(
                symptom_probs_for_this_person.items(),
                rng.random_sample(len(symptom_probs_for_this_person)),
            )
            if _rand < _prob
        ]
...
class MeaslesSymptomResolveEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        logger.debug(
            key="MeaslesSymptomResolve Event",
...
class HSI_Measles_Treatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if self.get_consumables(item_codes):
            ...
            self.sim.schedule_event(
                MeaslesSymptomResolveEvent(self.module, person_id),
                self.sim.date + DateOffset(days=7),
            )

    def did_not_run(self):
        logger.debug(
            key="HSI_Measles_Treatment", data="HSI_Measles_Treatment: did not run"
        )
        pass
...
class MeaslesLoggingAnnualEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        logger.info(
            key="pop_age_range",
            data=age_count.to_dict(),
            description="population sizes by age range",
        )

        # get the numbers infected by age group
        infected_age_counts = (
            df[(df.me_date_measles > (now - DateOffset(months=self.repeat)))]
            .groupby("age_range")
            .size()
        )
        total_infected = len(
        ...
        if total_infected:
            ...
            prop_infected_by_age = (
                infected_age_counts  # just output the series of zeros by age group
            )
