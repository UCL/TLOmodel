...
class Contraception(Module):
    ...
    PARAMETERS = {
        "Method_Use_In_2010": Parameter(
            Types.DATA_FRAME, "Proportion of women using each method in 2010, by age."
        ),
        "Pregnancy_NotUsing_In_2010": Parameter(
            Types.DATA_FRAME,
            "Probability per year of a women not on contraceptive becoming "
            "pregnant, by age.",
        ),
        "Pregnancy_NotUsing_HIVeffect": Parameter(
            Types.DATA_FRAME,
        ...
        "age_specific_fertility_rates": Parameter(
            Types.DATA_FRAME,
            "Data table from official source (WPP) for age-specific fertility rates and calendar "
            "period.",
        ),
        "scaling_factor_on_monthly_risk_of_pregnancy": Parameter(
            Types.LIST,
            "Scaling factor (by age-group: 15-19, 20-24, ..., 45-49) on the monthly risk of pregnancy and "
        ...
        "max_number_of_runs_of_hsi_if_consumable_not_available": Parameter(
            Types.INT,
            "The maximum number of time an HSI can run (repeats occur if the consumables are not "
            "available).",
        ),
        "max_days_delay_between_decision_to_change_method_and_hsi_scheduled": Parameter(
            Types.INT,
            "The maximum delay (in days) between the decision for a contraceptive to change and the `topen` "
            "date of the HSI that is scheduled to effect the change (when using the healthsystem).",
        ),
        "use_interventions": Parameter(
            Types.BOOL,
            "if True: FP interventions (pop & ppfp) are simulated from the date 'interventions_start_date',"
            " if False: FP interventions (pop & ppfp) are not simulated.",
        ),
        "interventions_start_date": Parameter(
            Types.DATE,
            "The date since which the FP interventions (pop & ppfp) are implemented, if at all (ie, if "
    ...
    def __init__(
        ...
        self.run_update_contraceptive = (
            run_update_contraceptive  # If 'False' prevents any logic occurring for
        )
        #                                                           updating/changing/maintaining contraceptive methods.

        self.states_that_may_require_HSI_to_switch_to = {
            "male_condom",
            "injections",
            "other_modern",
            "IUD",
        ...
        self.states_that_may_require_HSI_to_maintain_on = {
            "male_condom",
            "injections",
            "other_modern",
            "IUD",
        ...
        assert self.states_that_may_require_HSI_to_switch_to.issubset(
            self.all_contraception_states
        )
        assert self.states_that_may_require_HSI_to_maintain_on.issubset(
            self.states_that_may_require_HSI_to_switch_to
        )

        self.processed_params = (
            dict()
        )  # (Will store the processed data for rates/probabilities of outcomes).
        self.cons_codes = (
            dict()
        )  # (Will store the consumables codes for use in the HSI)
        self.rng2 = None  # (Will be a second random number generator, used for things to do with scheduling HSI)

        self._women_ids_sterilized_below30 = (
            set()
        )  # The ids of women who had female sterilization initiated when they
    ...
    def read_parameters(self, data_folder):
        ...
        workbook = pd.read_excel(
            Path(self.resourcefilepath) / "ResourceFile_Contraception.xlsx",
            sheet_name=None,
        )
        ...
        self.parameters["age_specific_fertility_rates"] = pd.read_csv(
            Path(self.resourcefilepath) / "demography" / "ResourceFile_ASFR_WPP.csv"
        )
    ...
    def initialise_population(self, population):
        ...
        df.loc[females1549, "co_contraception"] = df.loc[
            females1549, "age_years"
        ].apply(
            lambda _age_years: self.rng.choice(
                p_method.columns, p=p_method.loc[_age_years]
            )
        )

        # 3) Give a notional date on which the last appointment occurred for those that need them
        needs_appts = females1549 & df["co_contraception"].isin(
            self.states_that_may_require_HSI_to_maintain_on
        )
        states_to_maintain_on = sorted(self.states_that_may_require_HSI_to_maintain_on)
        df.loc[needs_appts, "co_date_of_last_fp_appt"] = (
            df.loc[needs_appts, "co_contraception"]
            .astype("string")
            .apply(
    ...
    def initialise_simulation(self, sim):
        ...
        * Schedule births to occur during the first 9 months of the simulation
        ...
        sim.schedule_event(
            ContraceptionPoll(
                self, run_update_contraceptive=self.run_update_contraceptive
            ),
            sim.date,
        )
        ...
        if self.parameters["use_interventions"]:
            ...
            sim.schedule_event(
                StartInterventions(self),
                Date(self.parameters["interventions_start_date"]),
            )

            # Log initiation date of interventions and implementation costs
            logger.info(
                key="interventions_start_date",
                data={
                    "date_co_interv_implemented": self.parameters[
    ...
    def end_pregnancy(self, person_id):
        ...
        assert (
            self.sim.population.props.at[person_id, "co_contraception"]
    ...
    def process_params(self):
        ...
        def expand_to_age_years(values_by_age_groups, ages_by_year):
            _d = dict(
                zip(
                    ...
                    values_by_age_groups,
                )
            )
            return np.array(
                [
                    _d[self.sim.modules["Demography"].AGE_RANGE_LOOKUP[_age_year]]
                    for _age_year in ages_by_year
                ]
            )
        ...
        def initial_method_use():
            ...
            p_method = (
                self.parameters["Method_Use_In_2010"]
                .set_index("age")
                .rename_axis("age_years")
            )
        ...
        def contraception_initiation():
            ...
            p_init_by_method_below30, p_init_by_method_30plus = (
                avoid_sterilization_below30(p_init_by_method)
            )

            # Effect of age
            age_effect = 1.0 + self.parameters["Initiation_ByAge"].set_index("age")[
                "r_init1_age"
            ].rename_axis("age_years")
            ...
            def apply_age_year_effects(probs_below30, probs_30plus):
                ...
                for year in year_effect.index:
                    ...
                    p_init_this_year_df = pd.DataFrame.from_dict(
                        p_init_this_year, orient="index"
                    )

                    # Check correct format of age/method data-frame
                    assert set(p_init_this_year_df.columns) == set(
        ...
        def contraception_switch():
            ...
            switching_matrix = (
                self.parameters["Prob_Switch_From_And_To"]
                .set_index("switchfrom")
                .transpose()
            )
            ...
            switching_matrix_below30 = switching_matrix_below30.apply(
            ...
            assert set(switching_matrix_below30.index) == (
                self.all_contraception_states - {"not_using"}
            )
            ...
            switching_matrix_except_fs = switching_matrix.loc[
                switching_matrix.index != "female_sterilization"
            ]
            switching_matrix_30plus = switching_matrix_except_fs.apply(
                lambda col: col / col.sum()
            )
            switching_matrix_30plus = switching_matrix_30plus * (
                1 - new_fs_probs_30plus
            )
            ...
            assert set(switching_matrix_30plus.index) == (
                self.all_contraception_states - {"not_using"}
            )
        ...
        def contraception_stop():
            ...
            age_effect = 1.0 + self.parameters["Discontinuation_ByAge"].set_index(
                "age"
            )["r_discont_age"].rename_axis("age_years")
            ...
            for year in year_effect.index:
                ...
                for a in age_effect.index:
                    p_stop_this_year[a] = (
                        p_stop_by_method * age_effect.at[a] * year_effect.at[year, a]
                    )
                p_stop_this_year_df = pd.DataFrame.from_dict(
                    p_stop_this_year, orient="index"
                )

                # Check correct format of age/method data-frame
                assert set(p_stop_this_year_df.columns) == set(
                    self.all_contraception_states - {"not_using"}
                )
        ...
        def time_age_trend_in_initiation():
            ...
            _init_over_time = np.exp(
                +0.05 * np.minimum(2020 - 2010, (_years - 2010))
            ) * np.maximum(1.0, np.exp(+0.01 * (_years - 2020)))
            _init_over_time_modification_by_age = 1.0 / expand_to_age_years(
                [1.0, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], _ages
            )
        ...
        def time_age_trend_in_stopping():
            ...
            _discont_over_time = np.exp(
                -0.05 * np.minimum(2020 - 2010, (_years - 2010))
            ) * np.minimum(1.0, np.exp(-0.01 * (_years - 2020)))
            _discont_over_time_modification_by_age = expand_to_age_years(
                [1.0, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], _ages
            )
            _discont = np.outer(
                _discont_over_time, _discont_over_time_modification_by_age
            )
        ...
        def scaling_factor_on_monthly_risk_of_pregnancy():
            ...
            return pd.Series(
                index=_ages,
                data=[
                    scaling_factor_as_dict[AGE_RANGE_LOOKUP[_age_year]]
        ...
        def pregnancy_no_contraception():
            ...
            p_pregnancy_no_contraception_per_month_nohiv = (
                self.parameters["Pregnancy_NotUsing_In_2010"]
                .set_index("age")["AnnualProb"]
                .rename_axis("age_years")
                .apply(convert_annual_prob_to_monthly_prob)
            )

            # Compute the probability of being pregnant if HIV-positive
            p_pregnancy_no_contraception_per_month_hiv = (
                p_pregnancy_no_contraception_per_month_nohiv
                * self.parameters["Pregnancy_NotUsing_HIVeffect"].set_index("age_")[
                    "RR_pregnancy"
                ]
            )
            ...
            p_pregnancy_no_contraception_per_month = pd.concat(
                {
                    ...
                    "hv_inf_True": p_pregnancy_no_contraception_per_month_hiv,
                },
                axis=1,
            )

            assert (p_pregnancy_no_contraception_per_month.index == range(15, 50)).all()
            assert set(p_pregnancy_no_contraception_per_month.columns) == {
                "hv_inf_True",
                "hv_inf_False",
            }
            assert np.isclose(
                ...
                1.0
                - np.power(
                    1.0 - p_pregnancy_no_contraception_per_month["hv_inf_False"], 12
                ),
            ).all()

            return p_pregnancy_no_contraception_per_month.mul(
                scaling_factor_on_monthly_risk_of_pregnancy(), axis=0
            )
        ...
        def pregnancy_with_contraception():
            ...
            p_pregnancy_with_contraception_per_month.loc[15, :] = (
                p_pregnancy_by_method_per_month
            )
            ...
            assert (
                p_pregnancy_with_contraception_per_month.index == range(15, 50)
            ).all()
            assert set(p_pregnancy_with_contraception_per_month.columns) == set(
                self.all_contraception_states - {"not_using"}
            )
            assert (
                0.0 == p_pregnancy_with_contraception_per_month["female_sterilization"]
            ).all()

            return p_pregnancy_with_contraception_per_month.mul(
                scaling_factor_on_monthly_risk_of_pregnancy(), axis=0
            )
        ...
        (
            ...
            processed_params["p_switching_to_below30"],
            processed_params["p_switching_to_30plus"],
        ...
        processed_params["p_pregnancy_no_contraception_per_month"] = (
            pregnancy_no_contraception()
        )
        processed_params["p_pregnancy_with_contraception_per_month"] = (
            pregnancy_with_contraception()
        )
    ...
    def update_params_for_interventions(self):
        ...
        def contraception_initiation_with_interv():
            ...
            def apply_intervention_age_year_effects(probs_below30, probs_30plus):
                ...
                probs_by_method_below30 = (
                    probs_below30.copy()
                    .drop("not_using")
                    .mul(self.parameters["Interventions_Pop"].loc[0])
                )
                probs_by_method_30plus = (
                    probs_30plus.copy()
                    .drop("not_using")
            ...
            return apply_intervention_age_year_effects(
                p_init_by_method_below30, p_init_by_method_30plus
            )
        ...
        def contraception_initiation_after_birth_with_interv():
            ...
            p_start_after_birth = (
                self.parameters["Initiation_AfterBirth"].loc[0].drop("not_using")
            )

            # Apply PPFP intervention multipliers
            p_start_after_birth = p_start_after_birth.mul(
                self.parameters["Interventions_PPFP"].loc[0]
            )
    ...
    def select_contraceptive_following_birth(self, mother_id, mother_age):
        ...
        if mother_age < 30:
            ...
            new_contraceptive = self.rng.choice(
                probs_below30.index, p=probs_below30.values
            )
        else:
            probs_30plus = self.processed_params["p_start_after_birth_30plus"]
            new_contraceptive = self.rng.choice(
                probs_30plus.index, p=probs_30plus.values
            )

        # Do the change in contraceptive
        self.schedule_batch_of_contraceptive_changes(
            ids=[mother_id], old=["not_using"], new=[new_contraceptive]
        )
    ...
    def get_item_code_for_each_contraceptive(self):
        ...
        get_items_from_pkg = self.sim.modules[
            "HealthSystem"
        ...
        assert set(_cons_codes.keys()) == set(
            self.states_that_may_require_HSI_to_switch_to
        )
    ...
    def schedule_batch_of_contraceptive_changes(self, ids, old, new):
        ...
        for _woman_id, _old, _new in zip(ids, old, new):
            if (_new == "female_sterilization") and (
                df.loc[_woman_id, "age_years"] < 30
            ):
            ...
            reqs_appt = (
                _new in self.states_that_may_require_HSI_to_switch_to
                if is_a_switch
                else _new in self.states_that_may_require_HSI_to_maintain_on
            )
            if (not is_a_switch) & reqs_appt:
                due_appt = (
                    pd.isnull(date_of_last_appt[_woman_id])
                    or (date_today - date_of_last_appt[_woman_id]).days
            ...
            if do_appt:
                ...
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    hsi_event=HSI_Contraception_FamilyPlanningAppt(
                        person_id=_woman_id, module=self, new_contraceptive=_new
                    ),
                ...
                if _old != _new:
                    self.do_and_log_individual_contraception_change(
                        woman_id=_woman_id, old=_old, new=_new
                    )
                else:
    ...
    def schedule_births_for_first_9_months(self):
        ...
        prob_birth = (
            df.loc[(df.index != 0) & (df.sex == "F") & df.is_alive & ~df.is_pregnant][
                "age_range"
            ]
            .map(risk_of_birth[self.sim.date.year])
            .fillna(0)
        )
        ...
        for _id in give_birth_women_ids:
            self.sim.schedule_event(
                DirectBirth(person_id=_id * (-1), module=self),
                random_date(
                    self.sim.date, self.sim.date + pd.DateOffset(months=9), self.rng
                ),
            )
    ...
    def on_simulation_end(self):
        ...
        if self._women_ids_sterilized_below30:
            warnings.warn(
                UserWarning(
                    f"IDs of women for whom sterilization was initiated when they were under 30:/n"
...
class DirectBirth(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        assert (
            person_id < 0
        )  # check that mother is correctly logged as direct birth mother
...
class ContraceptionPoll(RegularEvent, PopulationScopeEventMixin):
    ...
    * Determines the onset of pregnancy
    ...
    def __init__(self, module, run_do_pregnancy=True, run_update_contraceptive=True):
        ...
        self.run_update_contraceptive = (
            run_update_contraceptive  # (Provided for testing only)
        )
    ...
    def update_contraceptive(self):
        ...
        currently_using_co = df.index[
            possible_co_users
            & ~df.co_contraception.isin(["not_using", "female_sterilization"])
        ]
        currently_not_using_co = df.index[
            possible_co_users & (df.co_contraception == "not_using")
        ]
    ...
    def initiate(self, individuals_not_using: pd.Index):
        ...
        probs = (
            df.loc[individuals_not_using, ["age_years"]]
            .merge(
    ...
    def discontinue_switch_or_continue(self, individuals_using: pd.Index):
        ...
        prob = df.loc[individuals_using, ["age_years", "co_contraception"]].apply(
            lambda row: pp["p_stop_per_month"][self.sim.date.year].at[
                row.age_years, row.co_contraception
            ],
            axis=1,
        )
        ...
        individuals_eligible_for_continue_or_switch = individuals_using.drop(
            will_stop_idx
        )

        # Get the probability of switching contraceptive for all those currently using
        switch_prob = df.loc[
            individuals_eligible_for_continue_or_switch, "co_contraception"
        ].map(pp["p_switch_from_per_month"])

        # Randomly select who will switch contraceptive and who will remain on their current contraceptive
        will_switch = switch_prob > rng.random_sample(
            size=len(individuals_eligible_for_continue_or_switch)
        )
        ...
        new_co_below30 = transition_states(
            df.loc[switch_idx_below30, "co_contraception"],
            pp["p_switching_to_below30"],
            rng,
        )
        new_co_30plus = transition_states(
            df.loc[switch_idx_30plus, "co_contraception"],
            pp["p_switching_to_30plus"],
            rng,
        )
        ...
        if len(continue_idx) > 0:
            ...
            self.module.schedule_batch_of_contraceptive_changes(
                ids=continue_idx, old=current_contraception, new=current_contraception
            )
    ...
    def pregnancy_for_those_on_contraceptive(self):
        ...
        if possible_to_fail.sum():
            ...
            prob_of_failure = df.loc[
                possible_to_fail, ["age_years", "co_contraception"]
            ].apply(
                lambda row: prob_failure_per_month.at[
                    row.age_years, row.co_contraception
                ],
                axis=1,
            )
            ...
            idx_failure = prob_of_failure.index[
                prob_of_failure > rng.random_sample(size=len(prob_of_failure))
            ]
    ...
    def pregnancy_for_those_not_on_contraceptive(self):
        ...
        if subset.sum():
            ...
            idx_pregnant = prob_pregnancy.index[
                prob_pregnancy > rng.rand(len(prob_pregnancy))
            ]
    ...
    def apply(self, population):
        ...
        logger.info(
            ...
            data=flatten_multi_index_series_into_dict_for_logging(
                df.loc[df.is_alive & (df.sex == "F") & df.age_years.between(15, 49)]
                .groupby(by=["co_contraception", "age_range"])
                .size()
                .sort_index()
            ),
...
class HSI_Contraception_FamilyPlanningAppt(HSI_Event, IndividualScopeEventMixin):
    ...
    def __init__(self, module, person_id, new_contraceptive):
        ...
        _facility_level = (
            "2" if new_contraceptive in ("implant", "female_sterilization") else "1a"
        )
    ...
    def EXPECTED_APPT_FOOTPRINT(self):
        ...
        if (
            self._number_of_times_run > 0
        ):  # if it is to re-schedule due to unavailable consumables
        ...
        # if to maintain on a method
            ...
            warnings.warn(
                UserWarning(
                    "Assumed empty footprint for Contraception Routine appt because couldn't find"
                    "actual case."
                )
            )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        self.sim.population.props.at[person_id, "co_date_of_last_fp_appt"] = (
            self.sim.date
        )
        ...
        if (
            current_method
            not in self.module.contraceptives_initiated_with_additional_items
        ) and (
            self.new_contraceptive
            in self.module.contraceptives_initiated_with_additional_items
        ):
            ...
            cons_available = self.get_consumables(
                item_codes=items_essential, return_individual_results=True
            )
        ...
        co_administrated = all(
            v for k, v in cons_available.items() if k in items_essential
        )
        ...
        if co_administrated:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                ...
                items_available = {
                    k: v for k, v in items_all.items() if cons_available[k]
                }
                items_not_available = {
                    k: v for k, v in items_all.items() if not cons_available[k]
                }
                logger.debug(
                    key="Contraception_consumables",
                    data={
                        "TREATMENT_ID": (
                            self.TREATMENT_ID if self.TREATMENT_ID is not None else ""
                        ),
        ...
        if current_method != _new_contraceptive:
            ...
            self.module.do_and_log_individual_contraception_change(
                woman_id=self.target, old=current_method, new=_new_contraceptive
            )
...
class SimplifiedPregnancyAndLabour(Module):
    ...
    PARAMETERS = {
        "prob_live_birth": Parameter(
            Types.REAL, "Probability that a pregnancy results in a live birth."
        )
    }
    ...
    def read_parameters(self, *args):
        parameter_dataframe = pd.read_excel(
            self.sim.modules["Contraception"].resourcefilepath
...
def get_medium_variant_asfr_from_wpp_resourcefile(
    dat: pd.DataFrame, months_exposure: int
) -> dict:
    ...
    dat = dat.drop(
        dat[~dat.Variant.isin(["WPP_Estimates", "WPP_Medium variant"])].index
    )
    ...
    dat["asfr_per_period"] = convert_annual_prob_to_monthly_prob(
        dat["asfr"], num_months=months_exposure
    )

    asfr = dict()  # format is {year: {age-range: asfr}}
    for year in years:
        asfr[year] = (
            dat.loc[(year >= dat["Period-Start"]) & (year <= dat["Period-End"])]
            .set_index("Age_Grp")["asfr_per_period"]
            .to_dict()
        )
