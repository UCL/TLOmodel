...
class Contraception(Module):
    ...
    PARAMETERS = {
        "Method_Use_In_2010": Parameter(Types.DATA_FRAME, "Proportion of women using each method in 2010, by age."),
        "Pregnancy_NotUsing_In_2010": Parameter(
            Types.DATA_FRAME, "Probability per year of a women not on contraceptive becoming " "pregnant, by age."
        ),
        ...
        "Failure_ByMethod": Parameter(
            Types.DATA_FRAME, "Probability per month of a women on a contraceptive becoming pregnant, by " "method."
        ),
        ...
        "Interventions_Pop": Parameter(
            Types.DATA_FRAME, "Pop (population scale contraception intervention) intervention multiplier."
        ),
        "Interventions_PPFP": Parameter(
            Types.DATA_FRAME, "PPFP (post-partum family planning) intervention multiplier."
        ),
        ...
        "Discontinuation_ByMethod": Parameter(
            Types.DATA_FRAME, "The probability per month of discontinuing use of a method, by method."
        ),
    ...
    #  method').
    contraceptives_initiated_with_additional_items = {"pill", "IUD", "injections", "implant", "female_sterilization"}
    ...
    PROPERTIES = {
        "co_contraception": Property(
            Types.CATEGORICAL, "Current contraceptive method", categories=sorted(all_contraception_states)
        ),
    ...
    def read_parameters(self, data_folder):
        ...
        workbook = read_csv_files(
            Path(self.resourcefilepath) / "contraception" / "ResourceFile_Contraception", files=None
        )
        ...
        self.load_parameters_from_dataframe(
            pd.read_csv(Path(self.resourcefilepath) / "contraception" / "ResourceFile_ContraceptionParams.csv")
        )
        ...
        self.ratio_n_females_30_49_to_15_49_in_2010 = n_females_aged_30_to_49_in_2010 / n_females_aged_15_to_49_in_2010
        ...
        * Schedule births to occur during the first 9 months of the simulation
        ...
        if self.parameters["use_interventions"]:
            ...
            logger.info(
                ...
                data={"date_co_interv_implemented": self.parameters["interventions_start_date"]},
    ...
    def process_params(self):
        ...
        def avoid_sterilization_below30(probs):
            ...
            probs_30plus["female_sterilization"] = (
                probs.loc["female_sterilization"] / self.ratio_n_females_30_49_to_15_49_in_2010
            )
            # Scale so that the probability of all outcomes sum to 1.0
        ...
        def contraception_switch():
            ...
            # Increase prob of 'female_sterilization' in older women accordingly
            new_fs_probs_30plus = (
                switching_matrix.loc["female_sterilization", :] / self.ratio_n_females_30_49_to_15_49_in_2010
            )
        ...
        def time_age_trend_in_initiation():
            ...
            _init_over_time = np.exp(+0.05 * np.minimum(2020 - 2010, (_years - 2010))) * np.maximum(
                1.0, np.exp(+0.01 * (_years - 2020))
            )
        ...
        def time_age_trend_in_stopping():
            ...
            _discont_over_time = np.exp(-0.05 * np.minimum(2020 - 2010, (_years - 2010))) * np.minimum(
                1.0, np.exp(-0.01 * (_years - 2020))
            )
        ...
        def scaling_factor_on_monthly_risk_of_pregnancy():
            ...
            _ages = range(15, 50)
            return pd.Series(
                index=_ages, data=[scaling_factor_as_dict[AGE_RANGE_LOOKUP[_age_year]] for _age_year in _ages]
            )
        ...
        def pregnancy_with_contraception():
            ...
            p_pregnancy_with_contraception_per_month = pd.DataFrame(
                index=range(15, 50), columns=sorted(self.all_contraception_states - {"not_using"})
            )
    ...
    def update_params_for_interventions(self):
        ...
        def contraception_initiation_after_birth_with_interv(p_start_after_birth_without_interv):
            ...
            p_start_after_birth_with_interv = p_start_after_birth_with_interv.mul(
                self.parameters["Interventions_PPFP"].loc[0]
            )

            # Return reduced prob of 'not_using'
            p_start_after_birth_with_interv = pd.Series(
                (1.0 - p_start_after_birth_with_interv.sum()), index=["not_using"]
            ).append(p_start_after_birth_with_interv)
        ...
        processed_params["p_start_after_birth_below30"] = contraception_initiation_after_birth_with_interv(
            processed_params["p_start_after_birth_below30"]
        )
        processed_params["p_start_after_birth_30plus"] = contraception_initiation_after_birth_with_interv(
            processed_params["p_start_after_birth_30plus"]
        )
    ...
    def get_item_code_for_each_contraceptive(self):
        ...
        _cons_codes["male_condom"] = {get_item_code("Condom, male"): 30}
        _cons_codes["other_modern"] = {get_item_code("Female Condom_Each_CMST"): 30}
        _cons_codes["IUD"] = {
        ...
        _cons_codes["female_sterilization"] = {
            ...
            get_item_code("Cotton wool, 500g_1_CMST"): 100,
        }  # unit: 1 g
    ...
    def schedule_births_for_first_9_months(self):
        ...
        prob_birth = (
            df.loc[(df.index != 0) & (df.sex == "F") & df.is_alive & ~df.is_pregnant]["age_range"]
            .map(risk_of_birth[self.sim.date.year])
            .fillna(0)
        )

        # determine which women will get pregnant
        give_birth_women_ids = prob_birth.index[(self.rng.random_sample(size=len(prob_birth)) < prob_birth)]
    ...
    def on_simulation_end(self):
        ...
        if self._women_ids_sterilized_below30:
            ...
            logger.info(key="women_ids_sterilized_below30", data={"ids": self._women_ids_sterilized_below30})
    ...
    * Determines the onset of pregnancy
    ...
    def update_contraceptive(self):
        ...
        currently_using_co = df.index[
            possible_co_users & ~df.co_contraception.isin(["not_using", "female_sterilization"])
        ]
        ...
        df.loc[
            (df.sex == "F") & df.is_alive & (df.age_years > self.age_high) & (df.co_contraception != "not_using"),
            "co_contraception",
        ] = "not_using"
    ...
    def initiate(self, individuals_not_using: pd.Index):
        ...
        probs = (
            ...
            .merge(pp["p_start_per_month"][self.sim.date.year], how="left", left_on="age_years", right_index=True)
        ...
        if len(will_initiate) > 0:
            self.module.schedule_batch_of_contraceptive_changes(
                ids=list(will_initiate), old=["not_using"] * len(will_initiate), new=list(will_initiate.values())
            )
    ...
    def discontinue_switch_or_continue(self, individuals_using: pd.Index):
        ...
        prob = df.loc[individuals_using, ["age_years", "co_contraception"]].apply(
            lambda row: pp["p_stop_per_month"][self.sim.date.year].at[row.age_years, row.co_contraception], axis=1
        )
        ...
        if len(new_co) > 0:
            self.module.schedule_batch_of_contraceptive_changes(
                ids=new_co.index, old=df.loc[new_co.index, "co_contraception"].values, new=new_co.values
            )
    ...
    def pregnancy_for_those_on_contraceptive(self):
        ...
        if possible_to_fail.sum():
            ...
            prob_of_failure = df.loc[possible_to_fail, ["age_years", "co_contraception"]].apply(
                lambda row: prob_failure_per_month.at[row.age_years, row.co_contraception], axis=1
            )
...
class ContraceptionLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        logger.info(
            ...
            data=df.loc[df.is_alive & (df.sex == "F") & df.age_years.between(15, 49), "co_contraception"]
            .value_counts()
            .sort_index()
            .to_dict(),
...
class HSI_Contraception_FamilyPlanningAppt(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (current_method not in self.module.contraceptives_initiated_with_additional_items) and (
            self.new_contraceptive in self.module.contraceptives_initiated_with_additional_items
        ):
            items_optional = self.module.cons_codes["co_initiation"]
            cons_available = self.get_consumables(
                item_codes=items_essential, optional_item_codes=items_optional, return_individual_results=True
            )
        else:
            cons_available = self.get_consumables(item_codes=items_essential, return_individual_results=True)
        ...
        if co_administrated:
            ...
            if _new_contraceptive == "female_sterilization":
                self.add_equipment({"Cusco’s/ bivalved Speculum (small, medium, large)", "Lamp, Anglepoise"})
                self.add_equipment(self.healthcare_system.equipment.from_pkg_names("Minor Surgery"))
            elif _new_contraceptive == "IUD":
                self.add_equipment({"Cusco’s/ bivalved Speculum (small, medium, large)", "Sponge Holding Forceps"})
    ...
    def reschedule(self):
        ...
        self.module.sim.modules["HealthSystem"].schedule_hsi_event(
            hsi_event=self, topen=self.sim.date + pd.DateOffset(days=1), tclose=None, priority=1
        )
...
class SimplifiedPregnancyAndLabour(Module):
    ...
    PARAMETERS = {"prob_live_birth": Parameter(Types.REAL, "Probability that a pregnancy results in a live birth.")}
    ...
    def read_parameters(self, *args):
        parameter_dataframe = read_csv_files(
            self.sim.modules["Contraception"].resourcefilepath / "contraception" / "ResourceFile_Contraception",
    ...
    def set_date_of_labour(self, person_id):
        ...
        self.sim.schedule_event(
            EndOfPregnancyEvent(
                module=self, person_id=person_id, live_birth=(self.rng.rand() < self.parameters["prob_live_birth"])
            ),
            random_date(
