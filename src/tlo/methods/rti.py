...
class RTI(Module, GenericFirstAppointmentsMixin):
    ...
    INIT_DEPENDENCIES = {"SymptomManager", "HealthBurden"}
    ...
    PARAMETERS = {
        ...
        "rr_injrti_age04": Parameter(Types.REAL, "risk ratio of RTI in age 0-4 compared to base rate of RTI"),
        "rr_injrti_age59": Parameter(Types.REAL, "risk ratio of RTI in age 5-9 compared to base rate of RTI"),
        "rr_injrti_age1017": Parameter(Types.REAL, "risk ratio of RTI in age 10-17 compared to base rate of RTI"),
        ...
        "rr_injrti_excessalcohol": Parameter(
            Types.REAL, "risk ratio of RTI in those that consume excess alcohol compared to those who do not"
        ),
        ...
        "prob_bleeding_leads_to_shock": Parameter(
            Types.REAL, "The proportion of those with heavily bleeding injuries who go into shock"
        ),
        ...
        "prob_perm_disability_with_treatment_severe_TBI": Parameter(
            Types.REAL, "probability that someone with a treated severe TBI is permanently disabled"
        ),
        "prob_death_TBI_SCI_no_treatment": Parameter(
            Types.REAL, "probability that someone with a spinal cord injury will die without treatment"
        ),
        "prop_death_burns_no_treatment": Parameter(
            Types.REAL, "probability that someone with a burn injury will die without treatment"
        ),
        "prob_death_fractures_no_treatment": Parameter(
            Types.REAL, "probability that someone with a fracture injury will die without treatment"
        ),
        "prob_TBI_require_craniotomy": Parameter(
            Types.REAL, "probability that someone with a traumatic brain injury will require a craniotomy surgery"
        ),
        "prob_exploratory_laparotomy": Parameter(
            Types.REAL, "probability that someone with an internal organ injury will require a exploratory_laparotomy"
        ),
        "prob_depressed_skull_fracture": Parameter(
            Types.REAL, "Probability that a skull fracture will be depressed and therefore require surgery"
        ),
        "prob_mild_burns": Parameter(
            Types.REAL, "Probability that a burn within a region will result in < 10% total body surface area"
        ),
        "prob_dislocation_requires_surgery": Parameter(
            Types.REAL, "Probability that a dislocation will require surgery to relocate the joint."
        ),
        ...
        # Length of stay
        "mean_los_ISS_less_than_4": Parameter(Types.REAL, "Mean length of stay for someone with an ISS score < 4"),
        "sd_los_ISS_less_than_4": Parameter(
            Types.REAL, "Standard deviation in length of stay for someone with an ISS score < 4"
        ),
        "mean_los_ISS_4_to_8": Parameter(
            Types.REAL, "Mean length of stay for someone with an ISS score between 4 and 8"
        ),
        "sd_los_ISS_4_to_8": Parameter(
            Types.REAL, "Standard deviation in length of stay for someone with an ISS score between 4 and 8"
        ),
        "mean_los_ISS_9_to_15": Parameter(
            Types.REAL, "Mean length of stay for someone with an ISS score between 9 and 15"
        ),
        "sd_los_ISS_9_to_15": Parameter(
            Types.REAL, "Standard deviation in length of stay for someone with an ISS score between 9 and 15"
        ),
        "mean_los_ISS_16_to_24": Parameter(
            Types.REAL, "Mean length of stay for someone with an ISS score between 16 and 24"
        ),
        "sd_los_ISS_16_to_24": Parameter(
            Types.REAL, "Standard deviation in length of stay for someone with an ISS score between 16 and 24"
        ),
        ...
        # DALY weights
        "daly_wt_unspecified_skull_fracture": Parameter(Types.REAL, "daly_wt_unspecified_skull_fracture - code 1674"),
        "daly_wt_basilar_skull_fracture": Parameter(Types.REAL, "daly_wt_basilar_skull_fracture - code 1675"),
        "daly_wt_epidural_hematoma": Parameter(Types.REAL, "daly_wt_epidural_hematoma - code 1676"),
        "daly_wt_subdural_hematoma": Parameter(Types.REAL, "daly_wt_subdural_hematoma - code 1677"),
        "daly_wt_subarachnoid_hematoma": Parameter(Types.REAL, "daly_wt_subarachnoid_hematoma - code 1678"),
        "daly_wt_brain_contusion": Parameter(Types.REAL, "daly_wt_brain_contusion - code 1679"),
        "daly_wt_intraventricular_haemorrhage": Parameter(
            Types.REAL, "daly_wt_intraventricular_haemorrhage - code 1680"
        ),
        "daly_wt_diffuse_axonal_injury": Parameter(Types.REAL, "daly_wt_diffuse_axonal_injury - code 1681"),
        "daly_wt_subgaleal_hematoma": Parameter(Types.REAL, "daly_wt_subgaleal_hematoma - code 1682"),
        "daly_wt_midline_shift": Parameter(Types.REAL, "daly_wt_midline_shift - code 1683"),
        "daly_wt_facial_fracture": Parameter(Types.REAL, "daly_wt_facial_fracture - code 1684"),
        "daly_wt_facial_soft_tissue_injury": Parameter(Types.REAL, "daly_wt_facial_soft_tissue_injury - code 1685"),
        "daly_wt_eye_injury": Parameter(Types.REAL, "daly_wt_eye_injury - code 1686"),
        "daly_wt_neck_soft_tissue_injury": Parameter(Types.REAL, "daly_wt_neck_soft_tissue_injury - code 1687"),
        "daly_wt_neck_internal_bleeding": Parameter(Types.REAL, "daly_wt_neck_internal_bleeding - code 1688"),
        "daly_wt_neck_dislocation": Parameter(Types.REAL, "daly_wt_neck_dislocation - code 1689"),
        "daly_wt_chest_wall_bruises_hematoma": Parameter(Types.REAL, "daly_wt_chest_wall_bruises_hematoma - code 1690"),
        "daly_wt_hemothorax": Parameter(Types.REAL, "daly_wt_hemothorax - code 1691"),
        "daly_wt_lung_contusion": Parameter(Types.REAL, "daly_wt_lung_contusion - code 1692"),
        "daly_wt_diaphragm_rupture": Parameter(Types.REAL, "daly_wt_diaphragm_rupture - code 1693"),
        "daly_wt_rib_fracture": Parameter(Types.REAL, "daly_wt_rib_fracture - code 1694"),
        "daly_wt_flail_chest": Parameter(Types.REAL, "daly_wt_flail_chest - code 1695"),
        "daly_wt_chest_wall_laceration": Parameter(Types.REAL, "daly_wt_chest_wall_laceration - code 1696"),
        "daly_wt_closed_pneumothorax": Parameter(Types.REAL, "daly_wt_closed_pneumothorax - code 1697"),
        "daly_wt_open_pneumothorax": Parameter(Types.REAL, "daly_wt_open_pneumothorax - code 1698"),
        "daly_wt_surgical_emphysema": Parameter(
            Types.REAL, "daly_wt_surgical_emphysema aka subcuteal emphysema - code 1699"
        ),
        "daly_wt_abd_internal_organ_injury": Parameter(Types.REAL, "daly_wt_abd_internal_organ_injury - code 1700"),
        "daly_wt_spinal_cord_lesion_neck_with_treatment": Parameter(
            Types.REAL, "daly_wt_spinal_cord_lesion_neck_with_treatment - code 1701"
        ),
        "daly_wt_spinal_cord_lesion_neck_without_treatment": Parameter(
            Types.REAL, "daly_wt_spinal_cord_lesion_neck_without_treatment - code 1702"
        ),
        "daly_wt_spinal_cord_lesion_below_neck_with_treatment": Parameter(
            Types.REAL, "daly_wt_spinal_cord_lesion_below_neck_with_treatment - code 1703"
        ),
        "daly_wt_spinal_cord_lesion_below_neck_without_treatment": Parameter(
            Types.REAL, "daly_wt_spinal_cord_lesion_below_neck_without_treatment - code 1704"
        ),
        "daly_wt_vertebrae_fracture": Parameter(Types.REAL, "daly_wt_vertebrae_fracture - code 1705"),
        "daly_wt_clavicle_scapula_humerus_fracture": Parameter(
            Types.REAL, "daly_wt_clavicle_scapula_humerus_fracture - code 1706"
        ),
        "daly_wt_hand_wrist_fracture_with_treatment": Parameter(
            Types.REAL, "daly_wt_hand_wrist_fracture_with_treatment - code 1707"
        ),
        "daly_wt_hand_wrist_fracture_without_treatment": Parameter(
            Types.REAL, "daly_wt_hand_wrist_fracture_without_treatment - code 1708"
        ),
        "daly_wt_radius_ulna_fracture_short_term_with_without_treatment": Parameter(
            Types.REAL, "daly_wt_radius_ulna_fracture_short_term_with_without_treatment - code 1709"
        ),
        "daly_wt_radius_ulna_fracture_long_term_without_treatment": Parameter(
            Types.REAL, "daly_wt_radius_ulna_fracture_long_term_without_treatment - code 1710"
        ),
        "daly_wt_dislocated_shoulder": Parameter(Types.REAL, "daly_wt_dislocated_shoulder - code 1711"),
        "daly_wt_amputated_finger": Parameter(Types.REAL, "daly_wt_amputated_finger - code 1712"),
        "daly_wt_amputated_thumb": Parameter(Types.REAL, "daly_wt_amputated_thumb - code 1713"),
        "daly_wt_unilateral_arm_amputation_with_treatment": Parameter(
            Types.REAL, "daly_wt_unilateral_arm_amputation_with_treatment - code 1714"
        ),
        "daly_wt_unilateral_arm_amputation_without_treatment": Parameter(
            Types.REAL, "daly_wt_unilateral_arm_amputation_without_treatment - code 1715"
        ),
        "daly_wt_bilateral_arm_amputation_with_treatment": Parameter(
            Types.REAL, "daly_wt_bilateral_arm_amputation_with_treatment - code 1716"
        ),
        "daly_wt_bilateral_arm_amputation_without_treatment": Parameter(
            Types.REAL, "daly_wt_bilateral_arm_amputation_without_treatment - code 1717"
        ),
        "daly_wt_foot_fracture_short_term_with_without_treatment": Parameter(
            Types.REAL, "daly_wt_foot_fracture_short_term_with_without_treatment - code 1718"
        ),
        "daly_wt_foot_fracture_long_term_without_treatment": Parameter(
            Types.REAL, "daly_wt_foot_fracture_long_term_without_treatment - code 1719"
        ),
        "daly_wt_patella_tibia_fibula_fracture_with_treatment": Parameter(
            Types.REAL, "daly_wt_patella_tibia_fibula_fracture_with_treatment - code 1720"
        ),
        "daly_wt_patella_tibia_fibula_fracture_without_treatment": Parameter(
            Types.REAL, "daly_wt_patella_tibia_fibula_fracture_without_treatment - code 1721"
        ),
        "daly_wt_hip_fracture_short_term_with_without_treatment": Parameter(
            Types.REAL, "daly_wt_hip_fracture_short_term_with_without_treatment - code 1722"
        ),
        "daly_wt_hip_fracture_long_term_with_treatment": Parameter(
            Types.REAL, "daly_wt_hip_fracture_long_term_with_treatment - code 1723"
        ),
        "daly_wt_hip_fracture_long_term_without_treatment": Parameter(
            Types.REAL, "daly_wt_hip_fracture_long_term_without_treatment - code 1724"
        ),
        "daly_wt_pelvis_fracture_short_term": Parameter(Types.REAL, "daly_wt_pelvis_fracture_short_term - code 1725"),
        "daly_wt_pelvis_fracture_long_term": Parameter(Types.REAL, "daly_wt_pelvis_fracture_long_term - code 1726"),
        "daly_wt_femur_fracture_short_term": Parameter(Types.REAL, "daly_wt_femur_fracture_short_term - code 1727"),
        "daly_wt_femur_fracture_long_term_without_treatment": Parameter(
            Types.REAL, "daly_wt_femur_fracture_long_term_without_treatment - code 1728"
        ),
        "daly_wt_dislocated_hip": Parameter(Types.REAL, "daly_wt_dislocated_hip - code 1729"),
        "daly_wt_dislocated_knee": Parameter(Types.REAL, "daly_wt_dislocated_knee - code 1730"),
        "daly_wt_amputated_toes": Parameter(Types.REAL, "daly_wt_amputated_toes - code 1731"),
        "daly_wt_unilateral_lower_limb_amputation_with_treatment": Parameter(
            Types.REAL, "daly_wt_unilateral_lower_limb_amputation_with_treatment - code 1732"
        ),
        "daly_wt_unilateral_lower_limb_amputation_without_treatment": Parameter(
            Types.REAL, "daly_wt_unilateral_lower_limb_amputation_without_treatment - code 1733"
        ),
        "daly_wt_bilateral_lower_limb_amputation_with_treatment": Parameter(
            Types.REAL, "daly_wt_bilateral_lower_limb_amputation_with_treatment - code 1734"
        ),
        "daly_wt_bilateral_lower_limb_amputation_without_treatment": Parameter(
            Types.REAL, "daly_wt_bilateral_lower_limb_amputation_without_treatment - code 1735"
        ),
        ...
        "femur_fracture_skeletal_traction_mean_los": Parameter(
            Types.INT, "The mean length of stay for a person with a femur fracture being treated with skeletal traction"
        ),
        ...
        "prob_foot_frac_require_cast": Parameter(
            Types.REAL, "The probability that a person with a foot fracture will be treated with a plaster cast"
        ),
        "prob_foot_frac_require_maj_surg": Parameter(
            Types.REAL, "The probability that a person with a foot fracture will be treated with a major surgery"
        ),
        ...
        "prob_tib_fib_frac_require_cast": Parameter(
            Types.REAL, "The probability that a person with a tibia/fibula fracture will be treated with a plaster cast"
        ),
        ...
        "prob_femural_fracture_require_major_surgery": Parameter(
            Types.REAL, "The probability that a person with a femur fracture will be treated with major surgery"
        ),
        "prob_femural_fracture_require_minor_surgery": Parameter(
            Types.REAL, "The probability that a person with a femur fracture will be treated with minor surgery"
        ),
        "prob_femural_fracture_require_cast": Parameter(
            Types.REAL, "The probability that a person with a femur fracture will be treated with a plaster cast"
        ),
        ...
        "prob_femural_fracture_require_traction": Parameter(
            Types.REAL, "The probability that a person with a femur fracture will be treated with skeletal traction"
        ),
        "prob_pelvis_fracture_traction": Parameter(
            Types.REAL, "The probability that a person with a pelvis fracture will be treated with skeletal traction"
        ),
        "prob_pelvis_frac_major_surgery": Parameter(
            Types.REAL, "The probability that a person with a pelvis fracture will be treated with major surgery"
        ),
        "prob_pelvis_frac_minor_surgery": Parameter(
            Types.REAL, "The probability that a person with a pelvis fracture will be treated with minor surgery"
        ),
        "prob_pelvis_frac_cast": Parameter(
            Types.REAL, "The probability that a person with a pelvis fracture will be treated with a cast"
        ),
        "prob_dis_hip_require_maj_surg": Parameter(
            Types.REAL, "The probability that a person with a dislocated hip will be treated with a major surgery"
        ),
        "prob_dis_hip_require_cast": Parameter(
            Types.REAL, "The probability that a person with a dislocated hip will be treated with a plaster cast"
        ),
        "prob_hip_dis_require_traction": Parameter(
            Types.REAL, "The probability that a person with a dislocated hip will be treated with skeletal traction"
        ),
        "hdu_cut_off_iss_score": Parameter(
            Types.INT, "The ISS score used as a criteria to admit patients to the HDU/ICU units"
        ),
        "mean_icu_days": Parameter(Types.REAL, "The mean length of stay in the ICUfor those without TBI"),
        "sd_icu_days": Parameter(
            Types.REAL, "The standard deviation in length of stay in the ICU for those without TBI"
        ),
        "mean_tbi_icu_days": Parameter(Types.REAL, "The mean length of stay in the ICU for those with TBI"),
        "sd_tbi_icu_days": Parameter(
            Types.REAL, "The standard deviation in length of stay in the ICU for those with TBI"
        ),
        "prob_open_fracture_contaminated": Parameter(
            Types.REAL, "The probability that an open fracture will be contaminated"
        ),
        "allowed_interventions": Parameter(
            Types.LIST, "List of additional interventions that can be included when performing model analysis"
        ),
        "head_prob_112": Parameter(Types.REAL, "The probability that this person's head injury is a skull fracture"),
        "head_prob_113": Parameter(
            Types.REAL, "The probability that this person's head injury is a basilar skull fracture"
        ),
        "head_prob_133a": Parameter(
            Types.REAL, "The probability that this person's head injury is a Subarachnoid hematoma"
        ),
        "head_prob_133b": Parameter(Types.REAL, "The probability that this person's head injury is a Brain contusion"),
        "head_prob_133c": Parameter(
            Types.REAL, "The probability that this person's head injury is an Intraventricular haemorrhage"
        ),
        "head_prob_133d": Parameter(
            Types.REAL, "The probability that this person's head injury is a Subgaleal hematoma"
        ),
        "head_prob_134a": Parameter(
            Types.REAL, "The probability that this person's head injury is an Epidural hematoma"
        ),
        "head_prob_134b": Parameter(
            Types.REAL, "The probability that this person's head injury is a Subdural hematoma"
        ),
        "head_prob_135": Parameter(
            Types.REAL, "The probability that this person's head injury is a Diffuse axonal injury/midline shift"
        ),
        "head_prob_1101": Parameter(Types.REAL, "The probability that this person's head injury is a laceration"),
        "head_prob_1114": Parameter(Types.REAL, "The probability that this person's head injury is a burn"),
        "face_prob_211": Parameter(
            Types.REAL, "The probability that this person's face injury is a Facial fracture (nasal/unspecified)"
        ),
        "face_prob_212": Parameter(
            Types.REAL, "The probability that this person's face injury is a Facial fracture (mandible/zygomatic)"
        ),
        "face_prob_241": Parameter(
            Types.REAL, "The probability that this person's face injury is a soft tissue injury"
        ),
        "face_prob_2101": Parameter(Types.REAL, "The probability that this person's face injury is a laceration"),
        "face_prob_2114": Parameter(Types.REAL, "The probability that this person's face injury is a burn"),
        "face_prob_291": Parameter(Types.REAL, "The probability that this person's face injury is an eye injury"),
        "neck_prob_3101": Parameter(Types.REAL, "The probability that this person's neck injury is a laceration"),
        "neck_prob_3113": Parameter(Types.REAL, "The probability that this person's neck injury is a burn"),
        ...
        "neck_prob_322": Parameter(
            Types.REAL, "The probability that this person's neck injury is an Atlanto-occipital subluxation"
        ),
        "neck_prob_323": Parameter(
            Types.REAL, "The probability that this person's neck injury is an Atlanto-axial subluxation"
        ),
        "thorax_prob_4101": Parameter(Types.REAL, "The probability that this person's thorax injury is a laceration"),
        "thorax_prob_4113": Parameter(Types.REAL, "The probability that this person's thorax injury is a burn"),
        "thorax_prob_461": Parameter(
            Types.REAL, "The probability that this person's thorax injury is Chest wall bruises/haematoma"
        ),
        "thorax_prob_463": Parameter(Types.REAL, "The probability that this person's thorax injury is Haemothorax"),
        "thorax_prob_453a": Parameter(
            Types.REAL, "The probability that this person's thorax injury is a Lung contusion"
        ),
        "thorax_prob_453b": Parameter(
            Types.REAL, "The probability that this person's thorax injury is a Diaphragm rupture"
        ),
        "thorax_prob_412": Parameter(Types.REAL, "The probability that this person's thorax injury is a rib fracture"),
        "thorax_prob_414": Parameter(Types.REAL, "The probability that this person's thorax injury is flail chest"),
        "thorax_prob_441": Parameter(
            Types.REAL, "The probability that this person's thorax injury is a Chest wall lacerations/avulsions"
        ),
        "thorax_prob_442": Parameter(
            Types.REAL, "The probability that this person's thorax injury is a Surgical emphysema"
        ),
        "thorax_prob_443": Parameter(
            Types.REAL, "The probability that this person's thorax injury is a Closed pneumothorax/ open pneumothorax"
        ),
        "abdomen_prob_5101": Parameter(Types.REAL, "The probability that this person's abdomen injury is a laceration"),
        "abdomen_prob_5113": Parameter(Types.REAL, "The probability that this person's thorax injury is a burn"),
        "abdomen_prob_552": Parameter(
            Types.REAL, "The probability that this person's thorax injury is a skull fracture"
        ),
        "abdomen_prob_553": Parameter(
            Types.REAL, "The probability that this person's thorax injury is an Injury to stomach/intestines/colon"
        ),
        ...
        "spine_prob_612": Parameter(
            Types.REAL, "The probability that this person's spine injury is a vertabrae fracture"
        ),
        "spine_prob_673a": Parameter(
            Types.REAL, "The probability that this person's spine injury is a Spinal cord injury at neck level"
        ),
        "spine_prob_673b": Parameter(
            Types.REAL, "The probability that this person's spine injury is a Spinal cord injury below neck level"
        ),
        ...
        "upper_ex_prob_7101": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a laceration"
        ),
        "upper_ex_prob_7113": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a burn"
        ),
        ...
        "upper_ex_prob_712b": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a Fracture to Hand/wrist"
        ),
        "upper_ex_prob_712c": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a Fracture to Radius/ulna"
        ),
        "upper_ex_prob_722": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a dislocated shoulder"
        ),
        "upper_ex_prob_782a": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is an Amputated finger"
        ),
        "upper_ex_prob_782b": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a Unilateral arm amputation"
        ),
        "upper_ex_prob_782c": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a Thumb amputation"
        ),
        "upper_ex_prob_783": Parameter(
            Types.REAL, "The probability that this person's upper extremity injury is a bilateral arm amputation"
        ),
        "lower_ex_prob_8101": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a laceration"
        ),
        "lower_ex_prob_8113": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a burn"
        ),
        "lower_ex_prob_811": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a foot fracture"
        ),
        "lower_ex_prob_813do": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is an open foot fracture"
        ),
        ...
        "lower_ex_prob_813a": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a Hip fracture"
        ),
        "lower_ex_prob_813b": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a Pelvis fracture"
        ),
        "lower_ex_prob_813bo": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is an open Pelvis fracture"
        ),
        "lower_ex_prob_813c": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a Femur fracture"
        ),
        "lower_ex_prob_813co": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is an open Femur fracture"
        ),
        "lower_ex_prob_822a": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a Dislocated hip"
        ),
        "lower_ex_prob_822b": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a Dislocated knee"
        ),
        "lower_ex_prob_882": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is an Amputation of toes"
        ),
        "lower_ex_prob_883": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a Unilateral leg amputation"
        ),
        "lower_ex_prob_884": Parameter(
            Types.REAL, "The probability that this person's lower extremity injury is a Bilateral leg amputation "
        ),
        "blocked_interventions": Parameter(Types.LIST, "A list of interventions that are blocked in a simulation"),
        ...
        "maximum_number_of_times_HSI_events_should_run": Parameter(
            Types.INT, "limit on the number of times an HSI event can run"
        ),
    }
    ...
    PROPERTIES = {
        ...
        **{
            ...
            for injury_index, categories in zip(INJURY_INDICES, [INJURY_CODES] * len(INJURY_INDICES))
        },
        **{
        ...
        "rt_injuries_for_minor_surgery": Property(
            Types.LIST, "A list of injuries that are to be treated with a minor" "surgery"
        ),
        ...
        "rt_ISS_score": Property(
            Types.INT, "The ISS score associated with the injuries resulting from a road traffic" "accident"
        ),
        ...
        "rt_unavailable_med_death": Property(
            Types.BOOL, "death in the following month without medical intervention " "being able to be provided"
        ),
        ...
        "rt_date_death_no_med": Property(
            Types.DATE, "the date which the person has is scheduled to die without medical" "intervention"
        ),
        "rt_debugging_DALY_wt": Property(Types.REAL, "The true value of the DALY weight burden"),
        "rt_injuries_left_untreated": Property(
            Types.LIST, "A list of injuries that have been left untreated due to a " "blocked intervention"
        ),
    }
    ...
    CAUSES_OF_DISABILITY = {"RTI": Cause(gbd_causes="Road injuries", label="Transport Injuries")}
    ...
    def read_parameters(self, data_folder):
        ...
        assert (
            0.9999 < sum(p["number_of_injured_body_regions_distribution"][1]) < 1.0001
        ), "The number of injured body region distribution doesn't sum to one"
        # test the injury location distribution
        assert (
            0.9999 < sum(p["injury_location_distribution"][1]) < 1.0001
        ...
        for body_part in body_part_strings:
            ...
            assert (sum_probabilities % 1 < 0.0001) or (sum_probabilities % 1 > 0.9999), (
                "The probabilities" "chosen for assigning" "injuries don't" "sum to one"
            )
        ...
        self.ASSIGN_INJURIES_AND_DALY_CHANGES = {
            ...
            "112": [
                [p["head_prob_112"], 1, 1, 2, 3],
                p["daly_wt_unspecified_skull_fracture"],
                0,
                -p["daly_wt_unspecified_skull_fracture"],
            ],
            "113": [
                [p["head_prob_113"], 1, 1, 3, 4],
                p["daly_wt_basilar_skull_fracture"],
                0,
                -p["daly_wt_basilar_skull_fracture"],
            ],
            "133a": [
                [p["head_prob_133a"], 1, 3, 3, 4],
            ...
            "241": [
                [p["face_prob_241"], 2, 4, 1, 2],
                p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_facial_soft_tissue_injury"],
            ],
            "2101": [
                [p["face_prob_2101"], 2, 10, 1, 2],
                p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_facial_soft_tissue_injury"],
            ],
            "2114": [
                [p["face_prob_2114"], 2, 11, 4, 5],
                p["daly_wt_burns_greater_than_20_percent_body_area"],
                0,
                -p["daly_wt_burns_greater_than_20_percent_body_area"],
            ],
            ...
            "3101": [
                [p["neck_prob_3101"], 3, 10, 1, 2],
                p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_facial_soft_tissue_injury"],
            ],
            "3113": [
                [p["neck_prob_3113"], 3, 11, 3, 4],
            ...
            "342": [
                [p["neck_prob_342"], 3, 4, 2, 3],
                p["daly_wt_neck_internal_bleeding"],
                0,
                -p["daly_wt_neck_internal_bleeding"],
            ],
            "343": [
                [p["neck_prob_343"], 3, 4, 3, 4],
                p["daly_wt_neck_internal_bleeding"],
                0,
                -p["daly_wt_neck_internal_bleeding"],
            ],
            "361": [
                [p["neck_prob_361"], 3, 6, 1, 2],
                p["daly_wt_neck_internal_bleeding"],
                0,
                -p["daly_wt_neck_internal_bleeding"],
            ],
            "363": [
                [p["neck_prob_363"], 3, 6, 3, 4],
                p["daly_wt_neck_internal_bleeding"],
                0,
                -p["daly_wt_neck_internal_bleeding"],
            ],
            ...
            "4101": [
                [p["thorax_prob_4101"], 4, 10, 1, 2],
                p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_facial_soft_tissue_injury"],
            ],
            "4113": [
                [p["thorax_prob_4113"], 4, 11, 3, 4],
                p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"]
                + p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
            ...
            "441": [
                [p["thorax_prob_441"], 4, 4, 1, 2],
                p["daly_wt_closed_pneumothorax"],
                0,
                -p["daly_wt_closed_pneumothorax"],
            ],
            "442": [
                [p["thorax_prob_442"], 4, 4, 2, 3],
                p["daly_wt_surgical_emphysema"],
                0,
                -p["daly_wt_surgical_emphysema"],
            ],
            "443": [
                [p["thorax_prob_443"], 4, 4, 3, 4],
            ...
            "5101": [
                [p["abdomen_prob_5101"], 5, 10, 1, 2],
                p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_facial_soft_tissue_injury"],
            ],
            "5113": [
                [p["abdomen_prob_5113"], 5, 11, 3, 4],
                p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"]
                + p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
            ...
            "552": [
                [p["abdomen_prob_552"], 5, 5, 2, 3],
                p["daly_wt_abd_internal_organ_injury"],
                0,
                -p["daly_wt_abd_internal_organ_injury"],
            ],
            "553": [
                [p["abdomen_prob_553"], 5, 5, 3, 4],
                p["daly_wt_abd_internal_organ_injury"],
                0,
                -p["daly_wt_abd_internal_organ_injury"],
            ],
            "554": [
                [p["abdomen_prob_554"], 5, 5, 4, 5],
                p["daly_wt_abd_internal_organ_injury"],
                0,
                -p["daly_wt_abd_internal_organ_injury"],
            ],
            ...
            "612": [
                [p["spine_prob_612"], 6, 1, 2, 3],
                p["daly_wt_vertebrae_fracture"],
                0,
                -p["daly_wt_vertebrae_fracture"],
            ],
            "673a": [
                [p["spine_prob_673a"], 6, 7, 3, 4],
                p["daly_wt_spinal_cord_lesion_neck_without_treatment"],
                -p["daly_wt_spinal_cord_lesion_neck_without_treatment"]
                + p["daly_wt_spinal_cord_lesion_neck_with_treatment"],
                0,
            ],
            "673b": [
                [p["spine_prob_673b"], 6, 7, 3, 4],
                p["daly_wt_spinal_cord_lesion_below_neck_without_treatment"],
                -p["daly_wt_spinal_cord_lesion_below_neck_without_treatment"]
                + p["daly_wt_spinal_cord_lesion_below_neck_with_treatment"],
                0,
            ],
            "674a": [
                [p["spine_prob_674a"], 6, 7, 4, 5],
                p["daly_wt_spinal_cord_lesion_neck_without_treatment"],
                -p["daly_wt_spinal_cord_lesion_neck_without_treatment"]
                + p["daly_wt_spinal_cord_lesion_neck_with_treatment"],
                0,
            ...
            "7101": [
                [p["upper_ex_prob_7101"], 7, 10, 1, 2],
                p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_facial_soft_tissue_injury"],
            ],
            "7113": [
                [p["upper_ex_prob_7113"], 7, 11, 3, 4],
                p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"]
                + p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
            ...
            "712a": [
                [p["upper_ex_prob_712a"], 7, 1, 2, 3],
                p["daly_wt_clavicle_scapula_humerus_fracture"],
                0,
                -p["daly_wt_clavicle_scapula_humerus_fracture"],
            ],
            "712b": [
                [p["upper_ex_prob_712b"], 7, 1, 2, 3],
                p["daly_wt_hand_wrist_fracture_without_treatment"],
                -p["daly_wt_hand_wrist_fracture_without_treatment"] + p["daly_wt_hand_wrist_fracture_with_treatment"],
                -p["daly_wt_hand_wrist_fracture_with_treatment"],
            ],
            "712c": [
                [p["upper_ex_prob_712c"], 7, 1, 2, 3],
                p["daly_wt_radius_ulna_fracture_short_term_with_without_treatment"],
                0,
                -p["daly_wt_radius_ulna_fracture_short_term_with_without_treatment"],
            ],
            "722": [
                [p["upper_ex_prob_722"], 7, 2, 2, 3],
            ...
            "782b": [
                [p["upper_ex_prob_782b"], 7, 8, 2, 3],
                p["daly_wt_unilateral_arm_amputation_without_treatment"],
                -p["daly_wt_unilateral_arm_amputation_without_treatment"]
                + p["daly_wt_unilateral_arm_amputation_with_treatment"],
                0,
            ],
            "782c": [[p["upper_ex_prob_782c"], 7, 8, 2, 3], p["daly_wt_amputated_thumb"], 0, 0],
            "783": [
                [p["upper_ex_prob_783"], 7, 8, 3, 4],
                p["daly_wt_bilateral_arm_amputation_without_treatment"],
                -p["daly_wt_bilateral_arm_amputation_without_treatment"]
                + p["daly_wt_bilateral_arm_amputation_with_treatment"],
                0,
            ],
            # injuries to the lower extremities
            "8101": [
                [p["lower_ex_prob_8101"], 8, 10, 1, 2],
                p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_facial_soft_tissue_injury"],
            ],
            "8113": [
                [p["lower_ex_prob_8113"], 8, 11, 3, 4],
                p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_without_treatment"]
                + p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
                -p["daly_wt_burns_less_than_20_percent_body_area_with_treatment"],
            ...
            "811": [
                [p["lower_ex_prob_811"], 8, 1, 1, 2],
                p["daly_wt_foot_fracture_short_term_with_without_treatment"],
                0,
                -p["daly_wt_foot_fracture_short_term_with_without_treatment"],
            ],
            "813do": [
                [p["lower_ex_prob_813do"], 8, 1, 3, 4],
                p["daly_wt_foot_fracture_short_term_with_without_treatment"] + p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_foot_fracture_short_term_with_without_treatment"] - p["daly_wt_facial_soft_tissue_injury"],
            ],
            # lower leg fracture can be open or not
            "812": [
                [p["lower_ex_prob_812"], 8, 1, 2, 3],
            ...
            "813eo": [
                [p["lower_ex_prob_813eo"], 8, 1, 3, 4],
                p["daly_wt_patella_tibia_fibula_fracture_without_treatment"] + p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_patella_tibia_fibula_fracture_without_treatment"] - p["daly_wt_facial_soft_tissue_injury"],
            ],
            "813a": [
                [p["lower_ex_prob_813a"], 8, 1, 3, 4],
            ...
            "813b": [
                [p["lower_ex_prob_813b"], 8, 1, 3, 4],
                p["daly_wt_pelvis_fracture_short_term"],
                -p["daly_wt_pelvis_fracture_short_term"] + p["daly_wt_pelvis_fracture_long_term"],
                -p["daly_wt_pelvis_fracture_long_term"],
            ],
            "813bo": [
                [p["lower_ex_prob_813bo"], 8, 1, 3, 4],
                p["daly_wt_pelvis_fracture_short_term"] + p["daly_wt_facial_soft_tissue_injury"],
                -p["daly_wt_pelvis_fracture_short_term"] + p["daly_wt_pelvis_fracture_long_term"],
                -p["daly_wt_pelvis_fracture_long_term"] - p["daly_wt_facial_soft_tissue_injury"],
            ],
            # femur fracture can be open or closed
            "813c": [
                [p["lower_ex_prob_813c"], 8, 1, 3, 4],
                p["daly_wt_femur_fracture_short_term"],
                0,
                -p["daly_wt_femur_fracture_short_term"],
            ],
            "813co": [
                [p["lower_ex_prob_813co"], 8, 1, 3, 4],
                p["daly_wt_femur_fracture_short_term"] + p["daly_wt_facial_soft_tissue_injury"],
                0,
                -p["daly_wt_femur_fracture_short_term"] - p["daly_wt_facial_soft_tissue_injury"],
            ],
            "822a": [
                [p["lower_ex_prob_822a"], 8, 2, 2, 3],
                p["daly_wt_dislocated_hip"],
                0,
                -p["daly_wt_dislocated_hip"],
            ],
            "822b": [
                [p["lower_ex_prob_822b"], 8, 2, 2, 3],
            ...
            "883": [
                [p["lower_ex_prob_883"], 8, 8, 3, 4],
                p["daly_wt_unilateral_lower_limb_amputation_without_treatment"],
                -p["daly_wt_unilateral_lower_limb_amputation_without_treatment"]
                + p["daly_wt_unilateral_lower_limb_amputation_with_treatment"],
                0,
            ],
            "884": [
                [p["lower_ex_prob_884"], 8, 8, 4, 5],
                p["daly_wt_bilateral_lower_limb_amputation_without_treatment"],
                -p["daly_wt_bilateral_lower_limb_amputation_without_treatment"]
                + p["daly_wt_bilateral_lower_limb_amputation_with_treatment"],
                0,
            ],
        }
    ...
    def rti_do_when_diagnosed(self, person_id):
        ...
        assert (counts > 0) or self.sim.modules[
            "SymptomManager"
        ].spurious_symptoms, "This person has asked for medical treatment despite not being injured"
    ...
    def rti_do_for_major_surgeries(self, person_id, count):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            if count == 0:
                assert (
                    len(set(person.rt_injuries_for_major_surgery) & set(surgically_treated_codes)) > 0
            ...
            if "Major Surgery" not in p["blocked_interventions"]:
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    hsi_event=HSI_RTI_Major_Surgeries(module=self, person_id=person_id),
                    priority=0,
    ...
    def rti_do_for_minor_surgeries(self, person_id, count):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            assert (
                len(df.at[person_id, "rt_injuries_for_minor_surgery"]) > 0
                or len(df.at[person_id, "rt_injuries_left_untreated"]) > 0
            ...
            if count == 0:
                assert (
                    len(set(df.at[person_id, "rt_injuries_for_minor_surgery"]) & set(surgically_treated_codes)) > 0
                ), "This person has asked for a minor surgery but does not need it"
            # Isolate the relevant injury information
            person_injuries = df.loc[[person_id], RTI.INJURY_COLUMNS]
            ...
            if "Minor Surgery" not in self.parameters["blocked_interventions"]:
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    hsi_event=HSI_RTI_Minor_Surgeries(module=self, person_id=person_id),
                    priority=0,
                    topen=self.sim.date + DateOffset(days=count),
                    tclose=self.sim.date + DateOffset(days=15),
                )
    ...
    def rti_acute_pain_management(self, person_id):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            person_injuries = df.loc[[person_id], RTI.INJURY_COLUMNS]
            # check this person is injured, search they have an injury code that isn't "none".
            idx, counts = RTI.rti_find_and_count_injuries(
                person_injuries, self.PROPERTIES.get("rt_injury_1").categories[1:]
            )
            if counts == 0:
                logger.debug(
                    key="rti_general_message", data=f"person {person_id} requested pain relief but does not need it"
                )
                return
            # schedule pain management
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_RTI_Acute_Pain_Management(module=self, person_id=person_id),
                priority=0,
    ...
    def rti_ask_for_suture_kit(self, person_id):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            if counts == 0:
                logger.debug(
                    key="rti_general_message", data=f"person {person_id} requested a suture but does not need it"
                )
                return
            # request suture
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_RTI_Suture(module=self, person_id=person_id),
                priority=0,
                topen=self.sim.date + DateOffset(days=1),
                tclose=self.sim.date + DateOffset(days=15),
            )
    ...
    def rti_ask_for_shock_treatment(self, person_id):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_RTI_Shock_Treatment(module=self, person_id=person_id),
                priority=0,
                topen=self.sim.date + DateOffset(days=1),
                tclose=self.sim.date + DateOffset(days=15),
            )
    ...
    def rti_ask_for_burn_treatment(self, person_id):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            if counts == 0:
                logger.debug(
                    key="rti_general_message", data=f"person {person_id} requested burn treatment but does not need it"
                )
                return
            # if this person is alive ask for the hsi event
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_RTI_Burn_Management(module=self, person_id=person_id),
                priority=0,
                topen=self.sim.date + DateOffset(days=1),
                tclose=self.sim.date + DateOffset(days=15),
            )
    ...
    def rti_ask_for_fracture_casts(self, person_id):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            assert (
                len(set(df.loc[person_id, "rt_injuries_to_cast"]) & set(fracture_codes)) > 0
            ...
            if counts == 0:
                logger.debug(
                    key="rti_general_message", data=f"person {person_id} requested a fracture cast but does not need it"
                )
                return
            # if this person is alive request the hsi
            if "Fracture Casts" not in self.parameters["blocked_interventions"]:
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    hsi_event=HSI_RTI_Fracture_Cast(module=self, person_id=person_id),
                    priority=0,
                    topen=self.sim.date + DateOffset(days=1),
                    tclose=self.sim.date + DateOffset(days=15),
                )
    ...
    def rti_ask_for_tetanus(self, person_id):
        ...
        if df.at[person_id, "is_alive"]:
            ...
            if counts == 0:
                logger.debug(
                    key="rti_general_message",
                    data=f"This is rti_ask_for_tetanus person {person_id} asked for treatment but doesn't" f"need it.",
                )
                return

            # if this person is alive, ask for the hsi
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_RTI_Tetanus_Vaccine(module=self, person_id=person_id),
                priority=0,
                topen=self.sim.date + DateOffset(days=1),
                tclose=self.sim.date + DateOffset(days=15),
            )
    ...
    def schedule_hsi_event_for_tomorrow(self, hsi_event: HSI_Event = None):
        ...
        """
        self.sim.modules["HealthSystem"].schedule_hsi_event(
            hsi_event, topen=self.sim.date + DateOffset(days=1), tclose=self.sim.date + DateOffset(days=15), priority=0
        )
    ...
    def rti_assign_daly_weights(self, injured_index):
        ...
        daly_change = selected_for_rti_inj.applymap(lambda code: self.ASSIGN_INJURIES_AND_DALY_CHANGES[code][1]).sum(
        ...
        assert len(DALYweightunderlimit) == 0, (
            "Someone has not been given an injury burden",
    ...
    def rti_alter_daly_post_treatment(self, person_id, codes):
        ...
        df.at[person_id, "rt_debugging_DALY_wt"] += sum(
            [self.ASSIGN_INJURIES_AND_DALY_CHANGES[code][3] for code in codes]
        )
        ...
        if df.at[person_id, "rt_debugging_DALY_wt"] < 0:
            logger.debug(key="rti_general_message", data=f"person {person_id} has had too many daly weights removed")
    ...
    def rti_swap_injury_daly_upon_treatment(self, person_id, codes):
        ...
        df.at[person_id, "rt_debugging_DALY_wt"] += sum(
            [self.ASSIGN_INJURIES_AND_DALY_CHANGES[code][2] for code in relevant_codes]
        )
        df.at[person_id, "rt_debugging_DALY_wt"] = np.round(df.at[person_id, "rt_debugging_DALY_wt"], 4)
    ...
    def rti_treated_injuries(self, person_id, tlo_injury_codes):
        ...
        if df.at[person_id, "sy_injury"] != 0:
            self.sim.modules["SymptomManager"].change_symptom(
                person_id=person_id, disease_module=self.sim.modules["RTI"], add_or_remove="-", symptom_string="injury"
            )
    ...
    def rti_assign_injuries(self, number):
        ...
        for n in range(0, number):
            ...
            ninj = self.rng.choice(
                number_of_injured_body_regions_distribution[0], p=number_of_injured_body_regions_distribution[1]
            )
            ...
            for injlocs in injurylocation:
                ...
                injuries_at_location = [
                    injury for injury in self.ASSIGN_INJURIES_AND_DALY_CHANGES.keys() if injury.startswith(str(injlocs))
                ]
                # find the probability of each injury based on the above filter
                prob_of_each_injury_at_location = [
                    self.ASSIGN_INJURIES_AND_DALY_CHANGES[injury][0][0] for injury in injuries_at_location
                ]
                # make sure there are no rounding errors (meaning all probabilities sum to one)
                prob_of_each_injury_at_location = np.divide(
                    prob_of_each_injury_at_location, sum(prob_of_each_injury_at_location)
                )
        ...
        logger.info(
            ...
            data=injury_info,
            description="Relevant information on the injuries from road traffic accidents when they are " "assigned",
        )
    ...
    def _common_first_appt_steps(
        ...
        persons_injuries = [individual_properties[injury] for injury in RTI.INJURY_COLUMNS]
        if pd.isnull(individual_properties["cause_of_death"]) and not individual_properties["rt_diagnosed"]:
...
class RTIPollingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        diedfromrtiidx = df.index[
            df.rt_imm_death
            | df.rt_post_med_death
            | df.rt_no_med_death
            | df.rt_death_from_shock
        ...
        # injury.
        rt_current_non_ind = df.index[
            df.is_alive & ~df.rt_road_traffic_inc & ~df.rt_imm_death & ~df.rt_med_int & (df.rt_inj_severity == "none")
        ]
        ...
        selected_to_die = selected_for_rti[
            self.imm_death_proportion_rti > self.module.rng.random_sample(size=len(selected_for_rti))
        ]
        ...
        for individual_id in selected_to_die:
            self.sim.modules["Demography"].do_death(
                individual_id=individual_id, cause="RTI_imm_death", originating_module=self.module
            )
        ...
        df.loc[selected_for_rti_inj.index, "rt_ISS_score"] = description.loc[selected_for_rti_inj.index, "ISS"].astype(
            int
        )
        df.loc[selected_for_rti_inj.index, "rt_MAIS_military_score"] = description.loc[
            selected_for_rti_inj.index, "MAIS"
        ].astype(int)
        ...
        assert len(df.loc[df.rt_road_traffic_inc & ~df.rt_imm_death, "rt_ISS_score"] > 0) == len(
            df.loc[df.rt_road_traffic_inc & ~df.rt_imm_death]
        )
        # ========================== Determine who will experience shock from blood loss ==============================
        internal_bleeding_codes = ["361", "363", "461", "463", "813bo", "813co", "813do", "813eo"]
        df = self.sim.population.props

        potential_shock_index, _ = road_traffic_injuries.rti_find_and_count_injuries(
            df.loc[df.rt_road_traffic_inc, RTI.INJURY_COLUMNS], internal_bleeding_codes
        )
        ...
        assert sum(
            df.loc[df.rt_road_traffic_inc & ~df.rt_imm_death & (df.rt_date_inj == now), "rt_inj_severity"] != "none"
        ) == len(selected_for_rti_inj.index)
        ...
        assert sum(df.loc[condition_to_be_sent_to_em, "rt_injury_1"] != "none") == len(
            df.loc[condition_to_be_sent_to_em]
        )
        assert sum(df.loc[condition_to_be_sent_to_begin_non_emergency, "rt_injury_1"] != "none") == len(
            df.loc[condition_to_be_sent_to_begin_non_emergency]
        )
        ...
        if len(df.loc[df["rt_date_death_no_med"] == now]) > 0:
            ...
            for person in due_to_die_today_without_med_int:
                ...
                if (rand_for_death < prob_death) and (df.at[person, "rt_ISS_score"] > self.no_treatment_ISS_cut_off):
                    ...
                    self.sim.modules["Demography"].do_death(
                        individual_id=person, cause="RTI_death_without_med", originating_module=self.module
                    )
                else:
                    ...
                    if df.loc[person, "rt_med_int"]:
                        flattened_injuries = [
                            injury
                            for injury in flattened_injuries
                            if injury in df.loc[person, "rt_injuries_left_untreated"]
                        ]
                    persons_injuries = df.loc[[person], RTI.INJURY_COLUMNS]
                    for code in flattened_injuries:
                        ...
                        non_empty_injuries = persons_injuries[persons_injuries != "none"]
                        non_empty_injuries = non_empty_injuries.dropna(axis=1)
                        injury_column, _ = road_traffic_injuries.rti_find_injury_column(person, [code])
                        date_to_remove_daly_column = RTI.INJURY_DATE_COLUMN_MAP[injury_column]
                        ...
                        if not df.loc[person, "rt_med_int"]:
                            if code in self.module.NO_TREATMENT_RECOVERY_TIMES_IN_DAYS.keys():
                                df.loc[person, date_to_remove_daly_column] = self.sim.date + DateOffset(
                                    days=self.module.NO_TREATMENT_RECOVERY_TIMES_IN_DAYS[code]
                                )
                            else:
                                df.loc[person, date_to_remove_daly_column] = self.sim.end_date + DateOffset(days=1)
                        else:
...
class RTI_Recovery_Event(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        any_not_null = df.loc[df.is_alive, RTI.DATE_TO_REMOVE_DALY_COLUMNS].notnull().any(axis=1)
        ...
        for person in relevant_population:
            ...
            for date_column in RTI.DATE_TO_REMOVE_DALY_COLUMNS:
                ...
                if not pd.isnull(date_values[date_column]):
                    assert date_values[date_column] >= self.sim.date, "Recovery date assigned to past"
...
class HSI_RTI_Medical_Intervention(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        treatment_plans = {
            ...
            "811": [
                [
                    p["prob_foot_frac_require_cast"],
                    p["prob_foot_frac_require_maj_surg"],
                    p["prob_foot_frac_require_min_surg"],
                    p["prob_foot_frac_require_amp"],
                ],
            ...
            "812": [
                [
                    p["prob_tib_fib_frac_require_cast"],
                    p["prob_tib_fib_frac_require_maj_surg"],
                    p["prob_tib_fib_frac_require_min_surg"],
                    p["prob_tib_fib_frac_require_traction"],
            ...
            "813a": [
                [
                    p["prob_femural_fracture_require_major_surgery"],
                    p["prob_femural_fracture_require_minor_surgery"],
                    p["prob_femural_fracture_require_cast"],
                    p["prob_femural_fracture_require_traction"],
                    p["prob_femural_fracture_require_amputation"],
                ],
            ...
            "813b": [
                [
                    p["prob_pelvis_fracture_traction"],
                    p["prob_pelvis_frac_major_surgery"],
                    p["prob_pelvis_frac_minor_surgery"],
                    p["prob_pelvis_frac_cast"],
                ],
            ...
            "822a": [
                [
                    p["prob_dis_hip_require_maj_surg"],
                    p["prob_hip_dis_require_traction"],
                    p["prob_dis_hip_require_cast"],
                ],
                ["major", "HWT", "cast"],
            ],
            "322": [
                [self.prob_dislocation_requires_surgery, 1 - self.prob_dislocation_requires_surgery],
                ["minor", "HWT"],
            ],
            "323": [
                [self.prob_dislocation_requires_surgery, 1 - self.prob_dislocation_requires_surgery],
        ...
        if (counts > 0) & ("include_spine_surgery" in self.allowed_interventions):
            ...
            for col in columns:
                ...
                # it permanent)
                df.loc[person_id, RTI.INJURY_DATE_COLUMN_MAP[col]] = self.sim.end_date + DateOffset(days=1)
        ...
        traction_injuries = [
            injury
            for injury in df.loc[person_id, "rt_injuries_to_heal_with_time"]
            if injury in min_los_for_traction.keys()
        ]
        ...
        if df.loc[person_id, "rt_ISS_score"] > self.hdu_cut_off_iss_score:
            ...
            logger.info(key="ICU_patients", data=person_injuries, description="The injuries of ICU patients")
        # Check that each injury has only one treatment plan assigned to it
        treatment_plan = (
            person["rt_injuries_for_minor_surgery"]
            + person["rt_injuries_for_major_surgery"]
        ...
        _, counts = RTI.rti_find_and_count_injuries(
            person_injuries, self.module.PROPERTIES.get("rt_injury_1").categories[1:-1]
        )
        if counts == 0:
            logger.debug(
                key="rti_general_message",
        ...
        if len(df.at[person_id, "rt_injuries_to_heal_with_time"]) > 0:
            ...
            for code in person["rt_injuries_to_heal_with_time"]:
                ...
                if code in tbi:
                    ...
                    injury_column, _ = road_traffic_injuries.rti_find_injury_column(person_id, [code])
                    date_to_remove_daly_column = RTI.INJURY_DATE_COLUMN_MAP[injury_column]
                    df.loc[person_id, date_to_remove_daly_column] = self.sim.date + DateOffset(
                        days=heal_with_time_recovery_times_in_days[code]
                    )
        ...
        codes_treated_elsewhere = (
            p["rt_injuries_for_minor_surgery"]
            + p["rt_injuries_for_major_surgery"]
        ...
        for treatment in single_option_treatments:
            ...
            injuries_left_to_treat = [
                injury for injury in untreated_injuries if injury not in deliberately_untreated_injuries
            ]
            no_injuries_for_this_treatment = (
                len(set(injuries_left_to_treat) & set(single_option_treatments[treatment])) == 0
            )
            condition_to_skip = no_injuries_for_this_treatment & (treatment != "pain")
            if condition_to_skip:
                pass
            else:
                _, inj_counts = road_traffic_injuries.rti_find_and_count_injuries(
                    person_untreated_injuries, single_option_treatments[treatment]
                )
                if inj_counts > 0 & df.loc[person_id, "is_alive"]:
                    ...
                    if treatment == "open":
                        road_traffic_injuries.rti_ask_for_open_fracture_treatment(
                            person_id=person_id, counts=open_fractures
                        )
        ...
        self.sim.schedule_event(
            RTI_Medical_Intervention_Death_Event(self.module, person_id),
            self.sim.date + DateOffset(days=inpatient_days),
        )
        logger.debug(
            key="rti_general_message",
    ...
    def did_not_run(self):
        ...
        injurycodes = {
            "First injury": df.at[person_id, "rt_injury_1"],
...
class HSI_RTI_Shock_Treatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if is_child:
            is_cons_available = self.get_consumables(self.module.cons_item_codes["shock_treatment_child"])
        else:
            is_cons_available = self.get_consumables(self.module.cons_item_codes["shock_treatment_adult"])

        if is_cons_available:
            logger.debug(
                key="rti_general_message", data=f"Hypovolemic shock treatment available for person {person_id}"
            )
    ...
    def did_not_run(self):
        ...
        self.sim.modules["Demography"].do_death(
            individual_id=person_id, cause="RTI_death_shock", originating_module=self.module
        )
        # Log the death
        logger.debug(
            key="rti_general_message",
...
class HSI_RTI_Fracture_Cast(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if is_cons_available:
            ...
            for injury_column in columns:
                ...
                date_to_remove_daly_column = RTI.INJURY_DATE_COLUMN_MAP[injury_column]
                df.loc[person_id, date_to_remove_daly_column] = self.sim.date + DateOffset(weeks=7)
    ...
    def did_not_run(self):
        ...
        logger.debug(key="rti_general_message", data=f"Fracture casts unavailable for person {person_id}")
...
class HSI_RTI_Open_Fracture_Treatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        wound_contaminated = (open_fracture_counts > 0) and (
            self.module.parameters["prob_open_fracture_contaminated"] > self.module.rng.random_sample()
        )
        ...
        if is_cons_available:
            ...
            df.loc[person_id, date_to_remove_daly_column] = self.sim.date + DateOffset(months=7)
            assert df.loc[person_id, date_to_remove_daly_column] > self.sim.date
            assert not pd.isnull(
    ...
    def did_not_run(self):
        ...
        logger.debug(key="rti_general_message", data=f"Open fracture treatment unavailable for person {person_id}")
...
class HSI_RTI_Suture(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if lacerationcounts > 0:
            ...
            if is_cons_available:
                ...
                logger.debug(
                    key="rti_general_message", data=f"This facility treated their {lacerationcounts} open wounds"
                )
                ...
                for injury_column in columns:
                    ...
                    date_to_remove_daly_column = RTI.INJURY_DATE_COLUMN_MAP[injury_column]
                    df.loc[person_id, date_to_remove_daly_column] = self.sim.date + DateOffset(days=14)
                    assert df.loc[person_id, date_to_remove_daly_column] > self.sim.date
                df.loc[person_id, "rt_date_death_no_med"] = pd.NaT
            else:
                if self._number_of_times_this_event_has_run < self._maximum_number_times_event_should_run:
                ...
                if pd.isnull(df.loc[person_id, "rt_date_death_no_med"]):
                    df.loc[person_id, "rt_date_death_no_med"] = self.sim.date + DateOffset(days=7)
                logger.debug(
                    key="rti_general_message", data="This facility has no treatment for open wounds available."
                )
                return self.make_appt_footprint({})
    ...
    def did_not_run(self):
        ...
        logger.debug(key="rti_general_message", data=f"Suture kits unavailable for person {person_id}")
...
class HSI_RTI_Burn_Management(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if burncounts > 0:
            ...
            idx2, bigburncounts = road_traffic_injuries.rti_find_and_count_injuries(
                person_injuries, possible_large_TBSA_burn_codes
            )
            ...
            if (burncounts > 1) or ((len(idx2) > 0) & (random_for_severe_burn > self.prob_mild_burns)):
                ...
                cons_needed.update(self.module.cons_item_codes["ringers lactate for multiple burns"])

            is_cons_available = self.get_consumables(cons_needed)
            if is_cons_available:
                logger.debug(
                    key="rti_general_message",
                    data=f"This facility has burn treatment available which has been used for person " f"{person_id}",
                )
                logger.debug(key="rti_general_message", data=f"This facility treated their {burncounts} burns")
                df.at[person_id, "rt_med_int"] = True
                person = df.loc[person_id]
                injury_column, _ = road_traffic_injuries.rti_find_injury_column(person_id, codes)
                date_to_remove_daly_column = RTI.INJURY_DATE_COLUMN_MAP[injury_column]
                # estimate burns take 4 weeks to heal
                df.loc[person_id, date_to_remove_daly_column] = self.sim.date + DateOffset(weeks=4)
                assert df.loc[person_id, date_to_remove_daly_column] > self.sim.date
                persons_injuries = df.loc[[person_id], RTI.INJURY_COLUMNS]
                non_empty_injuries = persons_injuries[persons_injuries != "none"]
                non_empty_injuries = non_empty_injuries.dropna(axis=1)
                ...
                assert df.loc[person_id, date_to_remove_daly_column] > self.sim.date, "recovery date assigned to past"
                df.loc[person_id, "rt_date_death_no_med"] = pd.NaT
            else:
                if self._number_of_times_this_event_has_run < self._maximum_number_times_event_should_run:
                    self.sim.modules["RTI"].schedule_hsi_event_for_tomorrow(self)
                if pd.isnull(df.loc[person_id, "rt_date_death_no_med"]):
                    df.loc[person_id, "rt_date_death_no_med"] = self.sim.date + DateOffset(days=7)
                logger.debug(key="rti_general_message", data="This facility has no treatment for burns available.")

    def did_not_run(self):
        person_id = self.target

        logger.debug(key="rti_general_message", data=f"Burn treatment unavailable for person {person_id}")
...
class HSI_RTI_Tetanus_Vaccine(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if counts == 0:
            logger.debug(
                key="rti_general_message",
                data=f"This is RTI tetanus vaccine person {person_id} asked for treatment but doesn't" f"need it.",
            )
            return self.make_appt_footprint({})
        ...
        if counts > 0:
            ...
            if is_tetanus_available:
                logger.debug(
                    key="rti_general_message", data=f"Tetanus vaccine requested for person {person_id} and given"
                )
            else:
                if self._number_of_times_this_event_has_run < self._maximum_number_times_event_should_run:
                    self.sim.modules["RTI"].schedule_hsi_event_for_tomorrow(self)
                logger.debug(
                    key="rti_general_message", data=f"Tetanus vaccine requested for person {person_id}, not given"
                )
                return self.make_appt_footprint({})
    ...
    def did_not_run(self):
        ...
        logger.debug(key="rti_general_message", data=f"Tetanus vaccine unavailable for person {person_id}")
...
class HSI_RTI_Acute_Pain_Management(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        pain_dict = {
            "severe": [
                ...
                "552",
                "553",
                "554",  # abdominal trauma
                "463",
                "453",
                "453a",
                "453b",
                "441",
                "443",  # severe chest trauma
            ],
            "moderate": [
                "112",
                "113",
        ...
        if pain_level == "mild":
            ...
            dict_to_output = {"person": person_id, "pain level": pain_level}
            logger.info(
            ...
            if df.loc[person_id, "age_years"] < 16:
                cond = self.get_consumables(self.module.cons_item_codes["pain_management_mild_under_16"])
            else:
                ...
                if (cond1 is True) & (cond2 is True):
                    ...
                    cond = cond2
                    logger.debug(
                        key="rti_general_message", data=f"Person {person_id} requested diclofenac for their pain relief"
                    )
                else:
                    which = self.module.rng.random_sample(size=1)
                    if which <= 0.5:
            ...
            if cond:
                ...
                logger.debug(
                    key="rti_general_message",
                    data=f"This facility has no pain management available for their mild pain, person " f"{person_id}.",
                )
                return self.make_appt_footprint({})
        ...
        if pain_level == "moderate":
            ...
            logger.debug(
                key="rti_general_message", data=f"Person {person_id} has requested tramadol for moderate pain relief"
            )
        ...
        if pain_level == "severe":
            ...
            # give morphine
            is_cons_available = self.get_consumables(self.module.cons_item_codes["pain_management_severe"])
            logger.debug(
                key="rti_general_message", data=f"Person {person_id} has requested morphine for severe pain relief"
            )
            ...
            if is_cons_available:
                ...
                logger.debug(
                    key="rti_general_message",
                    data=f"This facility has no pain management available for severe pain for person " f"{person_id}.",
                )
                return self.make_appt_footprint({})
    ...
    def did_not_run(self):
        ...
        df = self.sim.population.props
        logger.debug(key="rti_general_message", data=f"Pain relief unavailable for person {person_id}")
        injurycodes = {
            "First injury": df.at[person_id, "rt_injury_1"],
            "Second injury": df.at[person_id, "rt_injury_2"],
            "Third injury": df.at[person_id, "rt_injury_3"],
        ...
        logger.debug(key="rti_general_message", data=f"Injury profile of person {person_id}, {injurycodes}")
...
class HSI_RTI_Major_Surgeries(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        request_outcome = self.get_consumables(self.module.cons_item_codes["major_surgery"])
        ...
        assert (
            len(set(injuries_to_be_treated) & set(surgically_treated_codes)) > 0
        ), "This person has asked for surgery but does not have an appropriate injury"
        ...
        if counts == 0:
            logger.debug(
                key="rti_general_message",
                data=f"This is RTI major surgery person {person_id} asked for treatment but doesn't" f"need it.",
            )
            return self.make_appt_footprint({})
        ...
        if request_outcome:
            ...
            if "include_spine_surgery" in self.allowed_interventions:
                ...
                if self.treated_code in codes:
                    ...
                    df.at[person_id, "rt_perm_disability"] = True
                    # Find the column and code where the permanent injury is stored
                    column, code = road_traffic_injuries.rti_find_injury_column(
                        person_id=person_id, codes=[self.treated_code]
                    )
                    logger.debug(
                        key="rti_general_message",
            ...
            if self.treated_code in maj_surg_recovery_time_in_days.keys():
                injury_column, _ = road_traffic_injuries.rti_find_injury_column(person_id, [self.treated_code])
                date_to_remove_daly_column = RTI.INJURY_DATE_COLUMN_MAP[injury_column]
            ...
            assert self.treated_code not in df.loc[person_id, "rt_injuries_for_major_surgery"], [
                "Treated injury code not removed",
                self.treated_code,
            ]
            df.loc[person_id, "rt_date_death_no_med"] = pd.NaT
        else:
    ...
    def did_not_run(self):
        ...
        df = self.sim.population.props
        logger.debug(key="rti_general_message", data=f"Major surgery not scheduled for person {person_id}")
        injurycodes = {
            "First injury": df.at[person_id, "rt_injury_1"],
            "Second injury": df.at[person_id, "rt_injury_2"],
            "Third injury": df.at[person_id, "rt_injury_3"],
...
class HSI_RTI_Minor_Surgeries(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if counts == 0:
            logger.debug(
                key="rti_general_message",
                data=f"This is RTI minor surgery person {person_id} asked for treatment but doesn't" f"need it.",
            )
            return self.make_appt_footprint({})
        ...
        if request_outcome:
            ...
            # stored in
            injury_column, _ = road_traffic_injuries.rti_find_injury_column(person_id, [treated_code])
            # assign a recovery date
            df.loc[person_id, RTI.INJURY_DATE_COLUMN_MAP[injury_column]] = self.sim.date + DateOffset(
                days=minor_surg_recov_time_days[treated_code]
            )
    ...
    def did_not_run(self):
        ...
        df = self.sim.population.props
        logger.debug(key="rti_general_message", data=f"Minor surgery not scheduled for person {person_id}")
        injurycodes = {
            "First injury": df.at[person_id, "rt_injury_1"],
            "Second injury": df.at[person_id, "rt_injury_2"],
            "Third injury": df.at[person_id, "rt_injury_3"],
        ...
        for range_boundaries in probabilities_of_death.keys():
            if df.loc[person_id].rt_ISS_score in probabilities_of_death[range_boundaries][0]:
                if randfordeath < probabilities_of_death[range_boundaries][1]:
                    ...
                    # Schedule the death
                    self.sim.modules["Demography"].do_death(
                        individual_id=person_id, cause="RTI_death_with_med", originating_module=self.module
                    )
                    # Log the death
                    logger.debug(
                        key="rti_general_message",
...
class RTI_No_Lifesaving_Medical_Intervention_Death_Event(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        life_threatening_injuries = [
            ...
            "343",
            "361",
            "363",  # Injuries to neck
            "414",
            "441",
            "443",
            "463",
            "453a",
            "453b",  # Severe chest trauma
            "782b",  # Unilateral arm amputation
            "783",  # Bilateral arm amputation
            "883",  # Unilateral lower limb amputation
            "884",  # Bilateral lower limb amputation
        ...
        if randfordeath < prob_death:
            ...
            self.sim.modules["Demography"].do_death(
                individual_id=person_id, cause="RTI_unavailable_med", originating_module=self.module
            )
            # Log the death
            logger.debug(
                key="rti_general_message",
            ...
            for injury in untreated_injuries:
                if injury in codes:
                    ...
                    if injury in self.module.NO_TREATMENT_RECOVERY_TIMES_IN_DAYS.keys():
                        df.loc[person_id, RTI.INJURY_DATE_COLUMN_MAP[col]] = self.sim.end_date + DateOffset(
                            days=self.module.NO_TREATMENT_RECOVERY_TIMES_IN_DAYS[injury]
                        )
...
class RTI_Logging_Event(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        proportion_severely_injured = {
            label: (len(pop_subset.loc[pop_subset["rt_inj_severity"] == "severe"]) / len(pop_subset))
            if len(pop_subset) > 0
            else float("nan")
        ...
        logger.info(key="injury_severity", data=dict_to_output, description="severity of injuries in simulation")
        # ==================================== Incidence ==============================================================
        ...
        self.death_in_denominator += (
            n_alive - (n_immediate_death + n_death_post_med + len(df.loc[df.rt_no_med_death]))
        ) * (1 / 12)
        if self.numerator > 0:
            percent_accidents_result_in_death = (
                self.deathonscene + self.deathaftermed + self.deathwithoutmed
            ) / self.numerator
        else:
        ...
        if (n_alive - len(df.loc[df.rt_post_med_death])) > 0:
            inc_post_med_death = (
                len(df.loc[df.rt_post_med_death]) / ((n_alive - len(df.loc[df.rt_post_med_death])) * (1 / 12))
            ) * 100000
        else:
            inc_post_med_death = 0
        if (n_alive - len(df.loc[df.rt_imm_death])) > 0:
            inc_imm_death = (
                len(df.loc[df.rt_imm_death]) / ((n_alive - len(df.loc[df.rt_imm_death])) * (1 / 12))
            ) * 100000
        else:
            inc_imm_death = 0.0
        if (n_alive - len(df.loc[df.rt_no_med_death])) > 0:
            inc_death_no_med = (
                len(df.loc[df.rt_no_med_death]) / ((n_alive - len(df.loc[df.rt_no_med_death])) * (1 / 12))
            ) * 100000
        else:
            inc_death_no_med = 0.0
        if (n_alive - len(df.loc[df.rt_unavailable_med_death])) > 0:
            inc_death_unavailable_med = (
                len(df.loc[df.rt_unavailable_med_death])
                / ((n_alive - len(df.loc[df.rt_unavailable_med_death])) * (1 / 12))
            ) * 100000
        else:
        ...
        died_no_healthcare = len(
            df.loc[df.rt_road_traffic_inc & df.rt_no_med_death & ~df.rt_med_int & ~df.rt_diagnosed]
        )
        ...
        logger.info(key="summary_1m", data=dict_to_output, description="Summary of the rti injuries in the last month")
        ...
        try:
            percent_related_to_alcohol = len(injuredDemographics.loc[injuredDemographics.li_ex_alc]) / len(
                injuredDemographics
            )
        ...
        logger.info(key="rti_demography", data=injured_demography_summary, description="Demographics of those in rti")

        # =================================== Flows through the model ==================================================
        dict_to_output = {
            "total_injured": self.totinjured,
        ...
        logger.info(key="model_progression", data=dict_to_output, description="Flows through the rti module")
