                ...
                if rand_for_death < prob_death:
                    ...
                    for code in flattened_injuries:
                        ...
                        if not df.loc[person, 'rt_med_int']:
                            ...
                            injury_has_associated_recovery_time = \
                                code in self.module.NO_TREATMENT_RECOVERY_TIMES_IN_DAYS.keys()
                            injury_left_untreated = code in df.loc[person, 'rt_injuries_left_untreated']
                            if injury_has_associated_recovery_time & injury_left_untreated:
                                df.loc[person, 'rt_date_to_remove_daly'][columns] = \
                                    self.sim.date + DateOffset(
                                        days=self.module.NO_TREATMENT_RECOVERY_TIMES_IN_DAYS[code]
                                    )
