...
class OesophagealCancer(Module, GenericFirstAppointmentsMixin):
    ...
    PARAMETERS = {
        ...
        "rate_palliative_care_stage4": Parameter(Types.REAL, "prob palliative care this 3 month period if stage4"),
        ...
        "r_dysphagia_stage1": Parameter(Types.REAL, "rate ratio for dysphagia if have stage 1 oesophageal cancer"),
        "rr_dysphagia_stage2": Parameter(Types.REAL, "rate ratio for dysphagia if have stage 2 oesophageal cancer"),
        "rr_dysphagia_stage3": Parameter(Types.REAL, "rate ratio for dysphagia if have stage 3 oesophageal cancer"),
        "rr_dysphagia_stage4": Parameter(Types.REAL, "rate ratio for dysphagia if have stage 4 oesophageal cancer"),
    ...
    PROPERTIES = {
        ...
        "oc_date_diagnosis": Property(
            Types.DATE, "the date of diagnsosis of the oes_cancer (pd.NaT if never diagnosed)"
        ),
    ...
    def read_parameters(self, data_folder):
        ...
        self.load_parameters_from_dataframe(
            read_csv_files(Path(self.resourcefilepath) / "ResourceFile_Oesophageal_Cancer", files="parameter_values")
        )
    ...
    def initialise_population(self, population):
        ...
        oc_status_any_dysplasia_or_cancer = lm_init_oc_status_any_dysplasia_or_cancer.predict(
        ...
        lm_init_disphagia = LinearModel.multiplicative(
            Predictor("oc_status", conditions_are_mutually_exclusive=True, conditions_are_exhaustive=True)
            .when("none", 0.0)
        ...
        lm_init_diagnosed = LinearModel.multiplicative(
            ...
            .when("none", 0.0)
            .when("low_grade_dysplasia", p["init_prop_with_dysphagia_diagnosed_oes_cancer_by_stage"][0])
            .when("high_grade_dysplasia", p["init_prop_with_dysphagia_diagnosed_oes_cancer_by_stage"][1])
    ...
    def initialise_simulation(self, sim):
        ...
        * Schedule the palliative care appointments for those that are on palliative care at initiation
        ...
        lm["low_grade_dysplasia"] = LinearModel(
            ...
            Predictor("oc_status").when("none", 1.0).otherwise(0.0),
        )
        ...
        lm["high_grade_dysplasia"] = LinearModel(
            ...
            Predictor("had_treatment_during_this_stage", external=True).when(
                True, p["rr_high_grade_dysp_undergone_curative_treatment"]
            ),
            Predictor("oc_status").when("low_grade_dysplasia", 1.0).otherwise(0.0),
        )
        ...
        lm["stage1"] = LinearModel(
            ...
            Predictor("had_treatment_during_this_stage", external=True).when(
                True, p["rr_stage1_undergone_curative_treatment"]
            ),
            Predictor("oc_status").when("high_grade_dysplasia", 1.0).otherwise(0.0),
        )
        ...
        lm["stage2"] = LinearModel(
            ...
            Predictor("oc_status").when("stage1", 1.0).otherwise(0.0),
        )
        ...
        lm["stage3"] = LinearModel(
            ...
            Predictor("oc_status").when("stage2", 1.0).otherwise(0.0),
        )
        ...
        lm["stage4"] = LinearModel(
            ...
            Predictor("oc_status").when("stage3", 1.0).otherwise(0.0),
        )
        ...
        self.lm_onset_dysphagia = LinearModel.multiplicative(
            Predictor("oc_status", conditions_are_mutually_exclusive=True, conditions_are_exhaustive=True)
            .when("low_grade_dysplasia", p["rr_dysphagia_low_grade_dysp"] * p["r_dysphagia_stage1"])
            .when("high_grade_dysplaisa", p["rr_dysphagia_high_grade_dysp"] * p["r_dysphagia_stage1"])
    ...
    def report_daly_values(self):
        ...
        disability_series_for_alive_persons.loc[
            ((df.oc_status == "stage1") | (df.oc_status == "stage2") | (df.oc_status == "stage3"))
        ...
        disability_series_for_alive_persons.loc[
            (df.oc_status == "stage4") & (pd.isnull(df.oc_date_palliative_care))
    ...
    def do_at_generic_first_appt(
        ...
        **kwargs,
        ...
        if individual_properties["age_years"] > 5 and "dysphagia" in symptoms:
            event = HSI_OesophagealCancer_Investigation_Following_Dysphagia(person_id=person_id, module=self)
    ...
    * Deaths from Oesophageal Cancer for those in stage4
    ...
    def apply(self, population):
        ...
        had_treatment_during_this_stage = (
            df.is_alive & ~pd.isnull(df.oc_date_treatment) & (df.oc_status == df.oc_stage_at_which_treatment_applied)
        )
        ...
        for person_id in selected_to_die:
            self.sim.schedule_event(InstantaneousDeath(self.module, person_id, "OesophagealCancer"), self.sim.date)
...
class HSI_OesophagealCancer_Investigation_Following_Dysphagia(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        cons_avail = self.get_consumables(
            item_codes=self.module.item_codes_oesophageal_can["screening_endoscopy_core"],
            optional_item_codes=self.module.item_codes_oesophageal_can[
        ...
        if cons_avail:
            ...
            dx_result = hs.dx_manager.run_dx_test(
                dx_tests_to_run="endoscopy_for_oes_cancer_given_dysphagia", hsi_event=self
            )
            if dx_result:
                ...
                if not in_stage4:
                    ...
                    hs.schedule_hsi_event(
                        hsi_event=HSI_OesophagealCancer_StartTreatment(module=self.module, person_id=person_id),
                        priority=0,
                        topen=self.sim.date,
                        tclose=None,
                    )
                    ...
                    hs.schedule_hsi_event(
                        hsi_event=HSI_OesophagealCancer_PalliativeCare(module=self.module, person_id=person_id),
                        priority=0,
                        topen=self.sim.date,
                        tclose=None,
                    )
...
class HSI_OesophagealCancer_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "oc_status"] == "stage4":
            logger.warning(
                key="warning",
                data="Cancer is in stage 4 - aborting HSI_OesophagaelCancer_StartTreatment,"
        ...
        cons_avail = self.get_consumables(
            item_codes=self.module.item_codes_oesophageal_can["treatment_surgery_core"],
            optional_item_codes=self.module.item_codes_oesophageal_can["treatment_surgery_optional"],
        )
...
class HSI_OesophagealCancer_PostTreatmentCheck(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "oc_status"] == "stage4":
            ...
            hs.schedule_hsi_event(
                hsi_event=HSI_OesophagealCancer_PostTreatmentCheck(module=self.module, person_id=person_id),
...
class HSI_OesophagealCancer_PalliativeCare(HSI_Event, IndividualScopeEventMixin):
    ...
    * Itself for the continuance of care.
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Check consumables are available
        cons_available = self.get_consumables(item_codes=self.module.item_codes_oesophageal_can["palliation"])
...
class OesCancerLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        out.update({f"total_{k}": v for k, v in df.loc[df.is_alive].oc_status.value_counts().items()})

        # Current counts, undiagnosed
        out.update(
            {
                f"undiagnosed_{k}": v
                for k, v in df.loc[df.is_alive].loc[pd.isnull(df.oc_date_diagnosis), "oc_status"].value_counts().items()
            }
        )
        ...
        out.update(
            {
                ...
                .loc[~pd.isnull(df.oc_date_diagnosis), "oc_status"]
                .value_counts()
                .items()
            }
        )
        ...
        out.update(
            {
                f"treatment_{k}": v
                for k, v in df.loc[df.is_alive]
                .loc[(~pd.isnull(df.oc_date_treatment) & pd.isnull(df.oc_date_palliative_care)), "oc_status"]
                .value_counts()
                .items()
            }
        )
        ...
        out.update(
            {
                f"palliative_{k}": v
                for k, v in df.loc[df.is_alive]
                .loc[~pd.isnull(df.oc_date_palliative_care), "oc_status"]
                .value_counts()
                .items()
            }
        )
