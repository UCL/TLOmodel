...
class OesophagealCancer(Module):
    ...
    CAUSES_OF_DEATH = {
        "OesophagealCancer": Cause(
            gbd_causes="Esophageal cancer", label="Cancer (Oesophagus)"
        ),
    }
    ...
    def initialise_population(self, population):
        ...
        lm_init_oc_status_any_dysplasia_or_cancer = LinearModel(
            ...
            Predictor("age_years").apply(
                lambda x: (
                    ((x - 20) ** p["rp_oes_cancer_per_year_older"]) if x > 20 else 0.0
                )
            ),
        )

        oc_status_any_dysplasia_or_cancer = (
            lm_init_oc_status_any_dysplasia_or_cancer.predict(
        ...
        if oc_status_any_dysplasia_or_cancer.sum():
            ...
            if sum_probs > 0:
                prob_by_stage_of_cancer_if_cancer = [
                    i / sum_probs for i in p["init_prop_oes_cancer_stage"]
                ]
                assert (sum(prob_by_stage_of_cancer_if_cancer) - 1.0) < 1e-10
                df.loc[oc_status_any_dysplasia_or_cancer, "oc_status"] = (
                    self.rng.choice(
        ...
        lm_init_disphagia = LinearModel.multiplicative(
            ...
            .when("none", 0.0)
            .when(
                "low_grade_dysplasia", p["init_prop_dysphagia_oes_cancer_by_stage"][0]
            )
            .when(
                "high_grade_dysplasia", p["init_prop_dysphagia_oes_cancer_by_stage"][1]
            )
        ...
        lm_init_diagnosed = LinearModel.multiplicative(
            ...
            .when(
                "stage1", p["init_prop_with_dysphagia_diagnosed_oes_cancer_by_stage"][2]
            )
            .when(
                "stage2", p["init_prop_with_dysphagia_diagnosed_oes_cancer_by_stage"][3]
            )
            .when(
                "stage3", p["init_prop_with_dysphagia_diagnosed_oes_cancer_by_stage"][4]
            )
            .when(
                "stage4", p["init_prop_with_dysphagia_diagnosed_oes_cancer_by_stage"][5]
            )
        )
        ...
        df.loc[treatment_initiated, "oc_stage_at_which_treatment_applied"] = df.loc[
            treatment_initiated, "oc_status"
        ]

        # set date at which treatment began: same as diagnosis (NB. no HSI is established for this)
        df.loc[treatment_initiated, "oc_date_treatment"] = df.loc[
            treatment_initiated, "oc_date_diagnosis"
        ]

        # -------------------- oc_date_palliative_care -----------
        in_stage4_diagnosed = df.index[
            df.is_alive & (df.oc_status == "stage4") & ~pd.isnull(df.oc_date_diagnosis)
        ]

        select_for_care = (
            self.rng.random_sample(size=len(in_stage4_diagnosed))
        ...
        df.loc[select_for_care, "oc_date_palliative_care"] = df.loc[
            select_for_care, "oc_date_diagnosis"
        ]
    ...
    def initialise_simulation(self, sim):
        ...
        * Schedule the palliative care appointments for those that are on palliative care at initiation
        ...
        sim.schedule_event(
            OesCancerMainPollingEvent(self), sim.date + DateOffset(months=0)
        )
        ...
        lm["low_grade_dysplasia"] = LinearModel(
            ...
            Predictor("age_years").apply(
                lambda x: (
                    0
                    if x < 20
                    else (x - 20) ** p["rr_low_grade_dysplasia_none_per_year_older"]
                )
            ),
        ...
        if "HealthBurden" in self.sim.modules:
            ...
            self.daly_wts["stage_1_3"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                sequlae_code=550
            ...
            self.daly_wts["stage_1_3_treated"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                sequlae_code=547
        ...
        on_palliative_care_at_initiation = df.index[
            df.is_alive & ~pd.isnull(df.oc_date_palliative_care)
        ]
...
class OesCancerMainPollingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    * Deaths from Oesophageal Cancer for those in stage4
    ...
    def apply(self, population):
        ...
        onset_dysphagia = self.module.lm_onset_dysphagia.predict(
        ...
        selected_to_die = stage4_idx[
            rng.random_sample(size=len(stage4_idx))
            < self.module.parameters["r_death_oesoph_cancer"]
        ]

        for person_id in selected_to_die:
            self.sim.schedule_event(
                InstantaneousDeath(self.module, person_id, "OesophagealCancer"),
                self.sim.date,
            )
...
class HSI_OesophagealCancer_Investigation_Following_Dysphagia(
...
class HSI_OesophagealCancer_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        df.at[person_id, "oc_stage_at_which_treatment_applied"] = df.at[
            person_id, "oc_status"
        ]
...
class OesCancerLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        out.update(
            {
                f"total_{k}": v
                for k, v in df.loc[df.is_alive].oc_status.value_counts().items()
            }
        )
        ...
        out.update(
            {
                "diagnosed_since_last_log": df.oc_date_diagnosis.between(
                    date_lastlog, date_now
                ).sum(),
                "treated_since_last_log": df.oc_date_treatment.between(
                    date_lastlog, date_now
                ).sum(),
                "palliative_since_last_log": df.oc_date_palliative_care.between(
                    date_lastlog, date_now
                ).sum(),
            }
        )
