...
# THESE METHODS SHOULD MOVE INSIDE THE HSI_GENERIC_XXX classes
...
def do_at_generic_first_appt_non_emergency(hsi_event, squeeze_factor):
        ...
        if "rash" in symptoms:
            schedule_hsi(
                HSI_Measles_Treatment(
                    person_id=person_id, module=hsi_event.sim.modules["Measles"]
                ),
                priority=0,
    ...
    if "Schisto" in sim.modules:
        sim.modules["Schisto"].do_on_presentation_with_symptoms(
    ...
    if "Malaria" in sim.modules:
        ...
        if bool(set(symptoms) & malaria_associated_symptoms):
            sim.modules["Malaria"].do_for_suspected_malaria_case(
    ...
    if age <= 5:
        ...
        if "Stunting" in sim.modules:
            sim.modules["Stunting"].do_routine_assessment_for_chronic_undernutrition(
                person_id=person_id
            )
            ...
            if "dysphagia" in symptoms:
                schedule_hsi(
                    HSI_OesophagealCancer_Investigation_Following_Dysphagia(
                        person_id=person_id, module=sim.modules["OesophagealCancer"]
                    ),
                    priority=0,
                    topen=sim.date,
                    tclose=None,
        ...
        if "BladderCancer" in sim.modules:
            ...
            if "blood_urine" in symptoms:
                schedule_hsi(
                    HSI_BladderCancer_Investigation_Following_Blood_Urine(
                        person_id=person_id, module=sim.modules["BladderCancer"]
                    ),
                    priority=0,
                    topen=sim.date,
                    tclose=None,
        ...
        if "ProstateCancer" in sim.modules:
            ...
            if "urinary" in symptoms:
                schedule_hsi(
                    HSI_ProstateCancer_Investigation_Following_Urinary_Symptoms(
                        person_id=person_id, module=sim.modules["ProstateCancer"]
                    ),
                    priority=0,
                    topen=sim.date,
                    tclose=None,
        ...
        if "OtherAdultCancer" in sim.modules:
            if "early_other_adult_ca_symptom" in symptoms:
                schedule_hsi(
                    HSI_OtherAdultCancer_Investigation_Following_early_other_adult_ca_symptom(
                        person_id=person_id, module=sim.modules["OtherAdultCancer"]
                    ),
                    priority=0,
                    topen=sim.date,
                    tclose=None,
        ...
        if "CardioMetabolicDisorders" in sim.modules:
            sim.modules["CardioMetabolicDisorders"].determine_if_will_be_investigated(
                person_id=person_id
            )
