...
def do_at_generic_first_appt_non_emergency(hsi_event, squeeze_factor):
    ...
    Person = namedtuple("Person", df.columns)
    ...
    if "Measles" in simulation_modules:
        if "rash" in symptoms:
            ...
            schedule_hsi(event, priority=0, topen=sim_date, tclose=None)
    ...
    # Sets dataframe value in method! (rt_diagnosed)
    ...
    if "Schisto" in simulation_modules:
        event = simulation_modules["Schisto"].do_on_presentation_with_symptoms(
    ...
    if "Malaria" in simulation_modules:
        ...
        if bool(set(symptoms) & malaria_associated_symptoms):
            simulation_modules["Malaria"].do_for_suspected_malaria_case(
...
def do_at_generic_first_appt_emergency(hsi_event, squeeze_factor):
    ...
    if "PregnancySupervisor" in sim.modules:
        ...
        abortion_complications = sim.modules[
            "PregnancySupervisor"
    ...
    if "Depression" in sim.modules:
        sim.modules["Depression"].do_on_presentation_to_care(
            person_id=person_id, hsi_event=hsi_event
        )

    if "Malaria" in sim.modules:
        if "severe_malaria" in symptoms:
            sim.modules["Malaria"].do_on_emergency_presentation_with_severe_malaria(
                person_id=person_id, hsi_event=hsi_event
            )
    ...
    if "CardioMetabolicDisorders" in sim.modules:
        sim.modules[
            "CardioMetabolicDisorders"
        ].determine_if_will_be_investigated_events(person_id=person_id)

    if "Epilepsy" in sim.modules:
        if "seizures" in symptoms:
            schedule_hsi(
                HSI_Epilepsy_Start_Anti_Epileptic(
                    person_id=person_id, module=sim.modules["Epilepsy"]
                ),
                priority=0,
                topen=sim.date,
                tclose=None,
    ...
    if "Copd" in sim.modules:
        if ("breathless_moderate" in symptoms) or ("breathless_severe" in symptoms):
            sim.modules["Copd"].do_when_present_with_breathless(
                person_id=person_id, hsi_event=hsi_event
            )
    ...
    if "craving_sandwiches" in symptoms:
        event = HSI_ChronicSyndrome_SeeksEmergencyCareAndGetsTreatment(
            module=sim.modules["ChronicSyndrome"], person_id=person_id
        )
    ...
    if "extreme_pain_in_the_nose" in symptoms:
        event = HSI_Mockitis_PresentsForCareWithSevereSymptoms(
            module=sim.modules["Mockitis"], person_id=person_id
        )
        schedule_hsi(event, priority=1, topen=sim.date)
