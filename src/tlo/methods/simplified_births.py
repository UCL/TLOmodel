...
class SimplifiedBirths(Module):
    ...
    ALTERNATIVE_TO = {"Contraception", "Labour", "NewbornOutcomes", "PostnatalSupervisor", "PregnancySupervisor"}
    ...
    PARAMETERS = {
        ...
        "prob_breastfeeding_type": Parameter(
            Types.LIST,
            "probabilities that a woman is: 1) not breastfeeding (none); 2) non-exclusively breastfeeding "
    ...
    PROPERTIES = {
        ...
        "date_of_last_pregnancy": Property(
            Types.DATE, "Date of the onset of the last pregnancy of this individual (if has ever " "been pregnant)."
        ),
        ...
        "nb_size_for_gestational_age": Property(
            Types.CATEGORICAL,
            "temporary property",
            categories=["small_for_gestational_age", "average_for_gestational_age", "large_for_gestational_age"],
        ),
        # (Mocked property, usually managed by Newborn_outcomes module)
...
class SimplifiedBirthsPoll(RegularEvent, PopulationScopeEventMixin):
    ...
    def set_new_pregnancies(self):
        ...
        prob_preg = (
            df.loc[(df.sex == "F") & df.is_alive & ~df.is_pregnant]["age_range"]
            .map(self.asfr[self.sim.date.year])
            .fillna(0)
        )

        # determine which woman will get pregnant
        pregnant_women_ids = prob_preg.index[(self.module.rng.random_sample(size=len(prob_preg)) < prob_preg)]
        ...
        df.loc[pregnant_women_ids, "si_date_of_last_delivery"] = self.sim.date + pd.DateOffset(
            months=self.module.parameters["months_between_pregnancy_and_delivery"]
        )
    ...
    def do_deliveries(self):
        ...
        females_to_give_birth = df.loc[
            (df.sex == "F") & df.is_alive & df.is_pregnant & (df.si_date_of_last_delivery <= self.sim.date)
        ].index
    ...
    def update_breastfed_status(self):
        ...
        df.loc[aged_6mo_to_23mo, "nb_breastfeeding_status"] = df.loc[
            aged_6mo_to_23mo, "si_breastfeeding_status_6mo_to_23mo"
        ]
