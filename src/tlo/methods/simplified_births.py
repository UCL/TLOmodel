...
class SimplifiedBirths(Module):
    ...
    def read_parameters(self, data_folder):
        ...
        self.parameters["age_specific_fertility_rates"] = pd.read_csv(
            self.resourcefilepath / "demography" / "ResourceFile_ASFR_WPP.csv"
        )
        ...
        param_as_string = rf.loc[rf.parameter_name == "prob_breastfeeding_type"][
            "value"
        ].iloc[0]
    ...
    def initialise_population(self, population):
        ...
        df = population.props
        df.loc[df.is_alive, self.default_properties.keys()] = (
            self.default_properties.values()
        )
...
class SimplifiedBirthsPoll(RegularEvent, PopulationScopeEventMixin):
    ...
    def __init__(self, module):
        ...
        self.asfr = get_medium_variant_asfr_from_wpp_resourcefile(
            ...
            months_exposure=self.months_between_polls,
        )
    ...
    def update_breastfed_status(self):
        ...
        aged_6mo_to_23mo = (
            df.is_alive & (df.age_exact_years >= 0.5) & (df.age_exact_years < 2.0)
        )
        ...
        df.loc[df.is_alive & (df.age_exact_years >= 2.0), "nb_breastfeeding_status"] = (
            "none"
        )
