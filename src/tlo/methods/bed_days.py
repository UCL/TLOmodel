...
class BedDays:
    ...
    def get_bed_types(self):
        ...
        return [
            x
            for x in self.hs_module.parameters["BedCapacity"].columns
            if x != "Facility_ID"
        ]
    ...
    def pre_initialise_population(self):
        ...
        for bed_type in self.bed_types:
            self.hs_module.PROPERTIES[f"hs_next_first_day_in_bed_{bed_type}"] = (
                Property(
                    Types.DATE,
                    f"Date when person will next enter bed_type {bed_type}. (pd.NaT) is nothing scheduled",
                )
            )
    ...
    def initialise_beddays_tracker(self, model_to_data_popsize_ratio=1.0):
        ...
        end_date = self.hs_module.sim.start_date + pd.DateOffset(
            days=max_number_of_bed_days
        )
    ...
    def move_each_tracker_by_one_day(self):
        ...
        for bed_type, tracker in self.bed_tracker.items():
            ...
            new_day = end_date + pd.DateOffset(
                days=1
            )  # the new day is the next day after the last in the tracker
    ...
    def log_todays_info_from_all_bed_trackers(self):
        ...
        self._summary_counter.record_usage_of_beds(
            {
                bed_type: (
                    self._scaled_capacity[bed_type].sum(),
                    tracker.iloc[0].sum(),
                )  # (Total, Number-available-now)
                for bed_type, tracker in self.bed_tracker.items()
            }
        )
    ...
    def check_beddays_footprint_format(self, beddays_footprint):
        ...
        assert all(
            [((v >= 0) and isinstance(v, int)) for v in beddays_footprint.values()]
        )
        if "non_bed_space" in self.bed_types:
            assert (
                beddays_footprint["non_bed_space"] == 0
            ), "A request cannot be made for a non-bed space"

    def issue_bed_days_according_to_availability(
        self, facility_id: int, footprint: dict
    ) -> dict:
        ...
        * For each type of bed specified in the footprint (in order from highest tier to lowest tier), check if there
        ...
        dates_for_bed_use_not_null = [
            _date for _date in dates_for_bed_use.values() if pd.notnull(_date)
        ]
        dates_for_bed_use_date_range = pd.date_range(
            min(dates_for_bed_use_not_null), max(dates_for_bed_use_not_null)
        )
        ...
        for _bed_type in self.bed_types:
            ...
            if hold_over_dates_for_next_bed_type:
                footprint_as_date_ranges[_bed_type].extend(
                    hold_over_dates_for_next_bed_type
                )
                footprint_as_date_ranges[_bed_type].sort()

            # Check if beds are available on each day
            tracker = self.bed_tracker[_bed_type][facility_id].loc[
                dates_for_bed_use_date_range
            ]
            ...
            if not available.all():
                ...
                hold_over_dates_for_next_bed_type = list(
                    available.loc[~available].index
                )
    ...
    def impose_beddays_footprint(self, person_id, footprint):
        ...
        * Update the property ```hs_is_inpatient``` to show that this person is now an in-patient
        ...
        if not df.at[person_id, "hs_is_inpatient"]:
            ...
            combo_footprint = {
                bed_type: max(footprint[bed_type], remaining_footprint[bed_type])
                for bed_type in self.bed_types
            }
    ...
    def apply_footprint(self, person_id, footprint):
        ...
        self.edit_bed_tracker(
            dates_of_bed_use=dates_of_bed_use, person_id=person_id, add_footprint=True
        )
    ...
    def edit_bed_tracker(self, dates_of_bed_use, person_id, add_footprint=True):
        ...
        for bed_type in self.bed_types:
            date_start_this_bed = dates_of_bed_use[
                f"hs_next_first_day_in_bed_{bed_type}"
            ]
            date_end_this_bed = dates_of_bed_use[f"hs_next_last_day_in_bed_{bed_type}"]

            if pd.isna(
                date_start_this_bed or date_end_this_bed
            ):  # filter empty bed days
                pass
            else:
                self.bed_tracker[bed_type].loc[
                    date_start_this_bed:date_end_this_bed, the_facility_id
                ] += operation
    ...
    def compute_dates_of_bed_use(self, footprint):
        ...
        for bed_type in self.bed_types:
            if footprint[bed_type] > 0:
                end_this_bed = start_this_bed + pd.DateOffset(
                    days=footprint[bed_type] - 1
                )

                # record these dates:
                dates_of_bed_use[f"hs_next_first_day_in_bed_{bed_type}"] = (
                    start_this_bed
                )
    ...
    def get_facility_id_for_beds(self, persons_id):
        ...
        the_district = self.hs_module.sim.population.props.at[
            persons_id, "district_of_residence"
        ]
        facility_level = "3"

        # Return an id of the (one) health_facility available to this person (based on their district)
        return self.hs_module._facilities_for_each_district[facility_level][
            the_district
        ][0]
    ...
    def remove_beddays_footprint(self, person_id):
        ...
        self.edit_bed_tracker(
            dates_of_bed_use=dates_of_bed_use, person_id=person_id, add_footprint=False
        )
    ...
    def refresh_in_patient_status(self):
        ...
        df.loc[df.is_alive, "hs_is_inpatient"] = (
            ...
            & (df.loc[df.is_alive, exit_cols] >= self.hs_module.sim.date)
        ).any(axis=1)
    ...
    def get_inpatient_appts(self) -> dict:
        ...
        total_inpatients = pd.DataFrame(
            [
                (
                    self._scaled_capacity[_bed_type]
                    - self.bed_tracker[_bed_type].loc[self.hs_module.sim.date]
                ).to_dict()
                for _bed_type in self.bed_types
            ]
        ).sum()
        ...
        def multiply_footprint(_footprint, _num):
            ...
            return {
                appt_type: num_needed * _num
                for appt_type, num_needed in _footprint.items()
            }

        return {
            fac_id: multiply_footprint(IN_PATIENT_DAY_SUBSEQUENT_DAYS, num_inpatients)
            for fac_id, num_inpatients in total_inpatients[total_inpatients > 0]
            .to_dict()
            .items()
        }
