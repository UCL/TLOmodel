...
class HealthSystem(Module):
    ...
    def read_parameters(self, data_folder):
        ...
        assert (
            self.parameters["HR_scaling_by_district_mode"] in self.parameters["HR_scaling_by_district_table"]
        ...
        assert (
            self.parameters["yearly_HR_scaling_mode"] in self.parameters["yearly_HR_scaling"]
    ...
    def setup_daily_capabilities(self, use_funded_or_actual_staffing):
        ...
        self._daily_capabilities, self._daily_capabilities_per_staff = self.format_daily_capabilities(
    ...
    def update_consumables_availability_to_represent_merging_of_levels_1b_and_2(self, df_original):
        ...
        facilities_with_any_differences = set(df_updated.loc[~(df_original == df_updated).all(axis=1), "Facility_ID"])
    ...
    def get_service_availability(self) -> List[str]:
        ...
        logger.info(
            key="message",
            data=f"Running Health System With the Following Service Availability: " f"{self.service_availability}",
        )
    ...
    def get_cons_availability(self) -> str:
        ...
        logger.info(
            key="message",
            data=f"Running Health System With the Following Consumables Availability: " f"{_cons_availability}",
        )
    ...
    def get_beds_availability(self) -> str:
        ...
        # Log the service_availability
        logger.info(
            key="message", data=f"Running Health System With the Following Beds Availability: " f"{_beds_availability}"
        )
    ...
    def get_equip_availability(self) -> str:
        ...
        logger.info(
            key="message",
            data=f"Running Health System With the Following Equipment Availability: " f"{_equip_availability}",
        )
    ...
    def get_squeeze_factors(
        ...
        if len(footprints_per_event) > len(self._get_squeeze_factors_store):
            ...
            new_size = (
                math.ceil(len(footprints_per_event) / self._get_squeeze_factors_store_grow)
        ...
        for i, footprint in enumerate(footprints_per_event):
            if footprint:
                ...
                if require_missing_officer:
                    ...
                    self._get_squeeze_factors_store[i] = max(load_factor[footprint.most_common()[0][0]], 0.0)
            else:
    ...
    def call_and_record_weather_cancelled_hsi_event(self, hsi_event, priority=None):
        ...
        if "HSI_CardioMetabolicDisorders_Refill_Medication" in hsi_event.as_namedtuple():
            hsi_event.never_ran_weather_event()
        ...
        if facility_id is [4, 5, 6, 7, 8, 9, 10, 11]:
            logger_summary.info(
                ...
                data={
                    ...
                    "Person_ID": person_id,
                    "priority": priority,
                    "Facility_Level": event_details.facility_level
    ...
    def log_current_capabilities_and_usage(self):
        ...
        fraction_time_used_overall = comparison["Minutes_Used"].sum() / total_available if total_available > 0 else 0
    ...
    def run_individual_level_events_in_mode_0_or_1(
        ...
        if _list_of_individual_hsi_event_tuples:
            ...
            if self.mode_appt_constraints == 0:
                ...
                squeeze_factor_per_hsi_event = np.zeros(len(footprints_of_all_individual_level_hsi_event))
            else:
                ...
                if self.sim.date > next_event_tuple.tclose:
                    ...
                    # Have enough capabilities left to run event
                        ...
                        for k in updated_call.keys():
                            updated_call[k] = updated_call[k] / (squeeze_factor + 1.0)
...
class HealthSystemSummaryCounter:
    ...
    def record_hsi_event(
        ...
        self._squeeze_factor_by_hsi_event_name[f"{treatment_id}:{hsi_event_name}"].append(squeeze_factor)
    ...
    def write_to_log_and_reset_counters(self):
        ...
        logger_summary.info(
            ...
            data=flatten_multi_index_series_into_dict_for_logging(self.frac_time_used_by_officer_type_and_level()),
        )
    ...
    def frac_time_used_by_officer_type_and_level(
        ...
        if (officer_type is not None) and (level is not None):
            ...
            return pd.Series(
                index=pd.MultiIndex.from_tuples(mean_frac_time_used.keys(), names=["OfficerType", "FacilityLevel"]),
...
class ConstantRescalingHRCapabilities(Event, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        for officer in self.module._daily_capabilities.keys():
            ...
            self.module._daily_capabilities[officer] *= HR_scaling_by_level_and_officer_type_factor.at[
