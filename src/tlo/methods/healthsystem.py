...
class HealthSystem(Module):
    ...
    def __init__(
        ...
        :param mode_appt_constraints: Integer code in ``{1, 2}`` determining mode of
            ...
            events run provided officers required have nonzero capabilities,
            2: hard constraints, only HSI events for which capabilities are available run.
    ...
    def _rescale_capabilities_to_capture_effective_capability(self):
        ...
        # for the purpose of keeping cost not scaled, need to scale down minute salary when capabilities are scaled up
        # Since now there is clinic division, we consider this type of HRH expansion only happens for GenericClinic
    ...
    def log_current_capabilities_and_usage(self, clinic_name):
        ...
        # Compute Fraction of Time Use by_facID_and_officer
        ...
        # Extract facility id, officer, level, district groups
        ...
        # Compute Fraction of Time For Each Officer and District
...
class HealthSystemSummaryCounter:
    ...
    def _reset_internal_stores(self) -> None:
        ...
        self._sum_of_daily_frac_time_used_by_officer_type_and_level = defaultdict(Counter)
        self._sum_of_daily_frac_time_used_by_officer_district = defaultdict(Counter)
        self._sum_of_daily_frac_time_used_by_officer_level_district = defaultdict(Counter)

    def record_hsi_event(
        self, treatment_id: str, hsi_event_name: str, appt_footprint: Counter, level: str, fac_id: int,
    ) -> None:
        ...
        for clinic in self._frac_time_used_overall.keys():
            logger_summary.info(
               ...
               data=flatten_multi_index_series_into_dict_for_logging(
                   self.frac_time_used_by_facID_and_officer(clinic))
            )
            ...
            logger_summary.info(
                ...
                data=flatten_multi_index_series_into_dict_for_logging(
                    self.frac_time_used_by_officer_type_and_level(clinic)),
            )
            ...
            logger_summary.info(
                ...
                data=flatten_multi_index_series_into_dict_for_logging(
                    self.frac_time_used_by_officer_district(clinic)),
            )
            ...
            logger_summary.info(
                ...
                data=flatten_multi_index_series_into_dict_for_logging(
                    self.frac_time_used_by_officer_level_district(clinic)),
            )
    ...
    def frac_time_used_by_officer_type_and_level(
        ...
        if (officer_type is not None) and (level is not None):
            return (
                self._sum_of_daily_frac_time_used_by_officer_type_and_level[clinic][officer_type, level]
                / len(self._frac_time_used_overall[clinic])
                # Use len(self._frac_time_used_overall) as proxy for number of days in past year.
            )
        else:
...
clinic: str,
        ...
        if (officer_type is not None) and (district is not None):
            return (
                self._sum_of_daily_frac_time_used_by_officer_district[clinic][officer_type, district]
                / len(self._frac_time_used_overall[clinic])
                # Use len(self._frac_time_used_overall) as proxy for number of days in past year.
            )
        else:
    ...
    def frac_time_used_by_officer_level_district(
        ...
        if (officer_type is not None) and (level is not None) and (district is not None):
            return (
                self._sum_of_daily_frac_time_used_by_officer_level_district[clinic][officer_type, level, district]
                / len(self._frac_time_used_overall[clinic])
                # Use len(self._frac_time_used_overall) as proxy for number of days in past year.
            )
        else:
...
class HRExpansionByOfficerType(Event, PopulationScopeEventMixin):
    ...
    This HRH expansion event only happens to GenericClinic as assumed."""
    def __init__(self, module):
        super().__init__(module)
