...
def adjust_facility_level_to_merge_1b_and_2(level: str) -> str:
    ...
    to level '2'"""
    return (
        level if level not in ("1b", "2") else LABEL_FOR_MERGED_FACILITY_LEVELS_1B_AND_2
    )
...
class HSI_Event:
    ...
    def initialise(self):
        ...
        * Compute appt-footprint time requirements
        ...
        self.ACCEPTED_FACILITY_LEVEL = adjust_facility_level_to_merge_1b_and_2(
...
class HealthSystem(Module):
    ...
    PARAMETERS = {
        ...
        "const_HR_scaling_table": Parameter(
            Types.DICT,
            "Factors by which capabilities of medical officer categories at different levels will be"
        ...
        "const_HR_scaling_mode": Parameter(
            Types.STRING,
            "Mode of HR scaling considered at the start of the simulation. Options are default"
    ...
    def pre_initialise_population(self):
        ...
        assert (
            self.parameters["const_HR_scaling_mode"]
            in self.parameters["const_HR_scaling_table"]
        ...
        self.bed_days = BedDays(
            hs_module=self, availability=self.get_beds_availability()
        )
    ...
    def adjust_for_const_HR_scaling(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        const_HR_scaling_factor = self.parameters["const_HR_scaling_table"][
            self.parameters["const_HR_scaling_mode"]
        ]
        ...
        scaler = df[["Officer_Category", "Facility_Level"]].apply(
            lambda row: const_HR_scaling_factor.loc[
                row["Officer_Category"], level_conversion[row["Facility_Level"]]
            ],
            axis=1,
        )
    ...
    def load_priority_policy(self, policy):
        ...
        if policy != "":
            ...
            self.priority_rank_dict = Policy_df.set_index(
                "Treatment", drop=True
                    ...
                    if sum(event.BEDDAYS_FOOTPRINT.values()):
                        event._received_info_about_bed_days = (
                            self.bed_days.issue_bed_days_according_to_availability(
                                facility_id=self.bed_days.get_facility_id_for_beds(
