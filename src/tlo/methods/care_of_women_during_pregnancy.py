...
class CareOfWomenDuringPregnancy(Module):
    ...
    PARAMETERS = {
        ...
        "prob_seek_anc2": Parameter(
            Types.LIST,
            "Probability a woman who is not predicted to attended four or more ANC visits will attend "
            "ANC2",
        ),
        ...
        "squeeze_factor_threshold_anc": Parameter(
            Types.INT,
            "squeeze factor threshold over which an ANC appointment cannot run",
        ),
        "prob_intervention_delivered_urine_ds": Parameter(
            Types.LIST,
            'probability a woman will receive the intervention "urine dipstick" given that the HSI has ran '
            "and the consumables are available (proxy for clinical quality)",
        ),
        "prob_intervention_delivered_bp": Parameter(
            Types.LIST,
            'probability a woman will receive the intervention "blood pressure measurement" given that the '
            "HSI has ran and the consumables are available (proxy for clinical quality)",
        ),
        "prob_intervention_delivered_ifa": Parameter(
            Types.LIST,
            'probability a woman will receive the intervention "iron and folic acid" given that the HSI'
            " has ran and the consumables are available (proxy for clinical quality)",
        ),
        "prob_adherent_ifa": Parameter(
            Types.LIST,
            "probability a woman who is given iron and folic acid will adhere to the treatment for their"
            " pregnancy",
        ),
        "prob_intervention_delivered_poct": Parameter(
            Types.LIST,
            'probability a woman will receive the intervention "point of care Hb testing" given that the '
            "HSI has ran and the consumables are available (proxy for clinical quality)",
        ),
        "prob_intervention_delivered_syph_test": Parameter(
            Types.LIST,
            'probability a woman will receive the intervention "Syphilis test" given that the HSI has ran '
            "and the consumables are available (proxy for clinical quality)",
        ),
        "prob_intervention_delivered_gdm_test": Parameter(
            Types.LIST,
            'probability a woman will receive the intervention "GDM screening" given that the HSI has ran '
            "and the consumables are available (proxy for clinical quality)",
        ),
        ...
        "squeeze_threshold_for_delay_three_an": Parameter(
            Types.LIST,
            "squeeze factor value over which an individual within a antenatal HSI is said to experience "
    ...
    def read_parameters(self, data_folder):
        parameter_dataframe = pd.read_excel(
            Path(self.resourcefilepath) / "ResourceFile_AntenatalCare.xlsx",
    ...
    def get_and_store_pregnancy_item_codes(self):
        ...
        # ---------------------------------- URINE DIPSTICK ----------------------------------------------------------
        self.item_codes_preg_consumables["urine_dipstick"] = get_list_of_items(
            self, ["Urine analysis"]
        )
        ...
        self.item_codes_preg_consumables["itn"] = get_list_of_items(
            self, ["Insecticide-treated net"]
        )

        # --------------------------------- CALCIUM SUPPLEMENTS -----------------------------------------------------
        self.item_codes_preg_consumables["calcium"] = get_list_of_items(
            self, ["Calcium, tablet, 600 mg"]
        )

        # -------------------------------- HAEMOGLOBIN TESTING -------------------------------------------------------
        self.item_codes_preg_consumables["hb_test"] = get_list_of_items(
            self, ["Haemoglobin test (HB)"]
        )

        # ------------------------------------------- ALBENDAZOLE -----------------------------------------------------
        self.item_codes_preg_consumables["albendazole"] = get_list_of_items(
            self, ["Albendazole 200mg_1000_CMST"]
        )
        ...
        # ----------------------------------------------- GDM TEST ----------------------------------------------------
        self.item_codes_preg_consumables["gdm_test"] = get_list_of_items(
            self, ["Blood glucose level test"]
        )

        # ------------------------------------------ FULL BLOOD COUNT -------------------------------------------------
        self.item_codes_preg_consumables["full_blood_count"] = get_list_of_items(
            self, ["Complete blood count"]
        )

        # ---------------------------------------- BLOOD TRANSFUSION -------------------------------------------------
        self.item_codes_preg_consumables["blood_transfusion"] = get_list_of_items(
            self, ["Blood, one unit"]
        )
    ...
    def initialise_simulation(self, sim):
        ...
        sim.schedule_event(
            CareOfWomenDuringPregnancyLoggingEvent(self),
            sim.date + DateOffset(days=364),
        )
    ...
    def further_on_birth_care_of_women_in_pregnancy(self, mother_id):
        ...
        if df.at[mother_id, "is_alive"]:
            ...
            if df.at[mother_id, "ac_total_anc_visits_current_pregnancy"] > 9:
                logger.info(
                    key="error",
                    data=f"Mother {mother_id} attended >8 ANC visits during her pregnancy",
                )
    ...
    def on_hsi_alert(self, person_id, treatment_id):
        logger.debug(
            key="message",
            data=f"This is CareOfWomenDuringPregnancy, being alerted about a health system "
    ...
    def antenatal_care_scheduler(
        self, individual_id, visit_to_be_scheduled, recommended_gestation_next_anc
    ):
        """
        ...
        if (
            df.at[individual_id, "ps_gestational_age_in_weeks"]
            > recommended_gestation_next_anc
        ):
            logger.info(
                key="error",
                data=f"Attempted to schedule an ANC visit for mother {individual_id} at a"
                f" gestation lower than her current gestation",
            )
            return

        visit_dict = {
            2: HSI_CareOfWomenDuringPregnancy_SecondAntenatalCareContact(
                self, person_id=individual_id
            ),
            3: HSI_CareOfWomenDuringPregnancy_ThirdAntenatalCareContact(
                self, person_id=individual_id
            ),
            4: HSI_CareOfWomenDuringPregnancy_FourthAntenatalCareContact(
                self, person_id=individual_id
            ),
            5: HSI_CareOfWomenDuringPregnancy_FifthAntenatalCareContact(
                self, person_id=individual_id
            ),
            6: HSI_CareOfWomenDuringPregnancy_SixthAntenatalCareContact(
                self, person_id=individual_id
            ),
            7: HSI_CareOfWomenDuringPregnancy_SeventhAntenatalCareContact(
                self, person_id=individual_id
            ),
            8: HSI_CareOfWomenDuringPregnancy_EighthAntenatalCareContact(
                self, person_id=individual_id
            ),
        }
        ...
        if (
            ...
            visit = HSI_CareOfWomenDuringPregnancy_FocusedANCVisit(
                self, person_id=individual_id, visit_number=visit_to_be_scheduled
            )
        ...
        elif ((visit_to_be_scheduled < 4) and not df.at[individual_id, "ps_anc4"]) or (
            visit_to_be_scheduled > 4
        ):
            if (
                self.rng.random_sample()
                < params[f"prob_seek_anc{visit_to_be_scheduled}"]
            ):
                calculate_visit_date_and_schedule_visit(visit)
    ...
    def schedule_admission(self, individual_id):
        ...
        if not df.at[individual_id, "ac_to_be_admitted"]:
            logger.info(
                key="error",
                data=f"Mother {individual_id} was scheduled for admission despite not requiring"
                f" it",
            )
            return
        ...
        logger.info(
            ...
            data={"mother": individual_id, "intervention": "admission"},
        )
    ...
    def call_if_maternal_emergency_assessment_cant_run(self, hsi_event):
        ...
        if (
            df.at[individual_id, "is_pregnant"]
            and not df.at[individual_id, "la_currently_in_labour"]
        ):
            logger.debug(
                key="message",
                data=f"HSI_CareOfWomenDuringPregnancy_MaternalEmergencyAssessment: did not"
    ...
    def check_intervention_should_run_and_update_mni(self, person_id, int_1, int2):
        ...
        # delivered
        elif (int_1 not in mni[person_id]["anc_ints"]) and (
            int2 not in mni[person_id]["anc_ints"]
        ):
        ...
        # delivered
        elif (int_1 in mni[person_id]["anc_ints"]) and int2 not in mni[person_id][
            "anc_ints"
        ]:
    ...
    def screening_interventions_delivered_at_every_contact(self, hsi_event):
        ...
        if self.rng.random_sample() < params["prob_intervention_delivered_urine_ds"]:
            ...
            if avail and self.sim.modules["HealthSystem"].dx_manager.run_dx_test(
                ...
                logger.info(
                    key="anc_interventions",
                    data={"mother": person_id, "intervention": "dipstick"},
                )
        ...
        if self.rng.random_sample() < params["prob_intervention_delivered_bp"]:
            ...
            if self.sim.modules["HealthSystem"].dx_manager.run_dx_test(
                ...
                logger.info(
                    key="anc_interventions",
                    data={"mother": person_id, "intervention": "bp_measurement"},
                )

                if (
                    not df.at[person_id, "ac_gest_htn_on_treatment"]
                    and (df.at[person_id, "ps_htn_disorders"] != "none")
                    and pd.isnull(mni[person_id]["hypertension" "_onset"])
                ):
                    ...
                    pregnancy_helper_functions.store_dalys_in_mni(
                        person_id, mni, "hypertension_onset", self.sim.date
                    )
        ...
        if hypertension_diagnosed or proteinuria_diagnosed:
            if (
                (
                    (df.at[person_id, "ps_htn_disorders"] == "severe_pre_eclamp")
                    and mni[person_id]["new_onset_spe"]
                )
                or (df.at[person_id, "ps_htn_disorders"] == "eclampsia")
                or not df.at[person_id, "ac_gest_htn_on_treatment"]
            ):
        ...
        if "Depression" in self.sim.modules:
            logger.info(
                key="anc_interventions",
                data={"mother": person_id, "intervention": "depression_screen"},
            )
    ...
    def iron_and_folic_acid_supplementation(self, hsi_event):
        ...
        if not df.at[person_id, "ac_receiving_iron_folic_acid"]:
            ...
            avail = pregnancy_helper_functions.return_cons_avail(
                self,
                hsi_event,
                self.item_codes_preg_consumables,
                core="iron_folic_acid",
                number=days,
            )
            ...
            if avail and (
                ...
                logger.info(
                    key="anc_interventions",
                    data={"mother": person_id, "intervention": "iron_folic_acid"},
                )
                ...
                if self.rng.random_sample() < params["prob_adherent_ifa"]:
                    ...
                    if (
                        ...
                        pregnancy_helper_functions.store_dalys_in_mni(
                            person_id,
                            mni,
                            f'{df.at[person_id, "ps_anaemia_in_pregnancy"]}_anaemia_resolution',
                            self.sim.date,
                        )
    ...
    def balance_energy_and_protein_supplementation(self, hsi_event):
        ...
        if not df.at[person_id, "ac_receiving_bep_supplements"]:
            ...
            if avail and (df.at[person_id, "li_bmi"] == 1):
                ...
                logger.info(
                    key="anc_interventions",
                    data={"mother": person_id, "intervention": "b_e_p"},
                )
    ...
    def tetanus_vaccination(self, hsi_event):
        ...
        if "Epi" in self.sim.modules:
            ...
            vaccine_hsi = HSI_TdVaccine(
                self.sim.modules["Epi"], person_id=person_id, suppress_footprint=True
            )

            self.sim.modules["HealthSystem"].schedule_hsi_event(
                vaccine_hsi, priority=0, topen=self.sim.date
            )
    ...
    def calcium_supplementation(self, hsi_event):
        ...
        if not df.at[person_id, "ac_receiving_calcium_supplements"] and (
            ...
            if avail:
                ...
                logger.info(
                    key="anc_interventions",
                    data={"mother": person_id, "intervention": "calcium"},
                )
    ...
    def point_of_care_hb_testing(self, hsi_event):
        ...
        if self.rng.random_sample() < params["prob_intervention_delivered_poct"]:
            logger.info(
                key="anc_interventions",
                data={"mother": person_id, "intervention": "hb_screen"},
            )

            hsi_event.get_consumables(
                item_codes=self.item_codes_preg_consumables["blood_test_equipment"]
            )

            # We run the test through the dx_manager and if a woman has anaemia and its detected she will be admitted
            # for further care
            if self.sim.modules["HealthSystem"].dx_manager.run_dx_test(
                dx_tests_to_run="point_of_care_hb_test", hsi_event=hsi_event
            ):
                df.at[person_id, "ac_to_be_admitted"] = True
    ...
    def albendazole_administration(self, hsi_event):
        ...
        avail = hsi_event.get_consumables(
            item_codes=self.item_codes_preg_consumables["albendazole"]
        )

        if avail:
            logger.info(
                key="anc_interventions",
                data={"mother": person_id, "intervention": "albendazole"},
            )
    ...
    def hep_b_testing(self, hsi_event):
        ...
        if avail:
            logger.info(
                key="anc_interventions",
                data={"mother": person_id, "intervention": "hep_b"},
            )
    ...
    def syphilis_screening_and_treatment(self, hsi_event):
        ...
        if self.rng.random_sample() < params["prob_intervention_delivered_syph_test"]:
            logger.info(
                key="anc_interventions",
                data={"mother": person_id, "intervention": "syphilis_test"},
            )
            ...
            if avail and test:
                ...
                if avail:
                    ...
                    logger.info(
                        key="anc_interventions",
                        data={"mother": person_id, "intervention": "syphilis_treat"},
                    )
    ...
    def hiv_testing(self, hsi_event):
        ...
        if "Hiv" in self.sim.modules:
            ...
            logger.info(
                key="anc_interventions",
                data={"mother": person_id, "intervention": "hiv_screen"},
            )
    ...
    def iptp_administration(self, hsi_event):
        ...
        if "Malaria" in self.sim.modules:
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                HSI_MalariaIPTp(
                    person_id=person_id, module=self.sim.modules["Malaria"]
                ),
                topen=self.sim.date,
                tclose=None,
                priority=0,
    ...
    def gdm_screening(self, hsi_event):
        ...
        if (
            ...
            or df.at[person_id, "ps_prev_gest_diab"]
            or df.at[person_id, "ps_prev_stillbirth"]
        ):

            # If they are available, the test is conducted
            if (
                self.rng.random_sample()
                < params["prob_intervention_delivered_gdm_test"]
            ):
                ...
                if avail and self.sim.modules["HealthSystem"].dx_manager.run_dx_test(
                    ...
                    logger.info(
                        key="anc_interventions",
                        data={"mother": person_id, "intervention": "gdm_screen"},
                    )
                    ...
                    if df.at[person_id, "ac_gest_diab_on_treatment"] == "none":
                        ...
                        pregnancy_helper_functions.store_dalys_in_mni(
                            person_id, mni, "gest_diab_onset", self.sim.date
                        )
                        df.at[person_id, "ac_to_be_admitted"] = True
    ...
    def check_anc1_can_run(self, individual_id, squeeze_factor, gest_age_next_contact):
        ...
        if (
            (date_difference > pd.to_timedelta(7, unit="D"))
            or (df.at[individual_id, "ac_total_anc_visits_current_pregnancy"] > 0)
            or (df.at[individual_id, "ps_gestational_age_in_weeks"] < 7)
        ):
            return False
        ...
        if df.at[individual_id, "hs_is_inpatient"]:
            ...
            logger.debug(
                key="message",
                data=f"mother {individual_id} is scheduled to attend ANC today but is "
    ...
    def check_subsequent_anc_can_run(
        self,
        individual_id,
        this_contact,
        this_visit_number,
        ...
        if (
            ...
            or (
                df.at[individual_id, "ps_gestational_age_in_weeks"]
                < ga_for_anc_dict[this_visit_number]
            )
            or (
                date_difference > pd.to_timedelta(7, unit="D")
                or not df.at[individual_id, "ac_total_anc_visits_current_pregnancy"]
                == (this_visit_number - 1)
            )
        ):
            return False
        ...
        if df.at[individual_id, "hs_is_inpatient"]:
            self.antenatal_care_scheduler(
                individual_id,
                visit_to_be_scheduled=this_visit_number,
    ...
    def antenatal_blood_transfusion(self, individual_id, hsi_event):
        ...
        sf_check = pregnancy_helper_functions.check_emonc_signal_function_will_run(
            self.sim.modules["Labour"], sf="blood_tran", hsi_event=hsi_event
        )
        ...
        if avail and sf_check:
            ...
            if (
                ...
                store_dalys_in_mni(
                    individual_id, mni, "severe_anaemia_resolution", self.sim.date
                )
                df.at[individual_id, "ps_anaemia_in_pregnancy"] = "none"

    def initiate_maintenance_anti_hypertensive_treatment(
        self, individual_id, hsi_event
    ):
        """
        ...
        cons = {
            _i: dose
            for _i in self.item_codes_preg_consumables["oral_antihypertensives"]
        }
    ...
    def initiate_treatment_for_severe_hypertension(self, individual_id, hsi_event):
        ...
        avail = hsi_event.get_consumables(
            item_codes=cons["iv_antihypertensives"],
    ...
    def treatment_for_severe_pre_eclampsia_or_eclampsia(self, individual_id, hsi_event):
        ...
        sf_check = pregnancy_helper_functions.check_emonc_signal_function_will_run(
            self.sim.modules["Labour"], sf="anticonvulsant", hsi_event=hsi_event
        )
    ...
    def ectopic_pregnancy_treatment_doesnt_run(self, hsi_event):
        ...
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_TreatmentForEctopicPregnancy: did not run",
        )
    ...
    def calculate_beddays(self, individual_id):
        ...
        # delivery
        if (
            (mother.ps_htn_disorders == "severe_pre_eclamp")
            or (mother.ps_htn_disorders == "eclampsia")
            or mother.ps_placental_abruption
            or (
                mother.ps_placenta_praevia
                and (mother.ps_antepartum_haemorrhage == "severe")
            )
            or (
                mother.ps_placenta_praevia
                and (mother.ps_antepartum_haemorrhage == "mild_moderate")
                and (mother.ps_gestational_age_in_weeks >= 37)
            )
            or (mother.ps_premature_rupture_of_membranes and mother.ps_chorioamnionitis)
            or (
                mother.ps_premature_rupture_of_membranes
                and not mother.ps_chorioamnionitis
                and (mother.ps_gestational_age_in_weeks >= 34)
            )
        ):
            beddays = 1
...
class HSI_CareOfWomenDuringPregnancy_FirstAntenatalCareContact(
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        gest_age_next_contact = self.module.determine_gestational_age_for_next_contact(
            person_id
        )

        # Check this visit can run
        can_anc1_run = self.module.check_anc1_can_run(
            person_id, squeeze_factor, gest_age_next_contact
        )
        ...
        if can_anc1_run:
            ...
            mni[person_id]["ga_anc_one"] = df.at[
                person_id, "ps_gestational_age_in_weeks"
            ]
            ...
            self.module.screening_interventions_delivered_at_every_contact(
                hsi_event=self
            )
            ...
            if mother.ps_gestational_age_in_weeks < 40:
                self.module.antenatal_care_scheduler(
                    person_id,
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FirstAntenatalCareVisit: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FirstAntenatalCareVisit: cannot not run with "
            "this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_SecondAntenatalCareContact(
    HSI_Event, IndividualScopeEventMixin
):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Run the check
        can_anc_run = self.module.check_subsequent_anc_can_run(
            individual_id=person_id,
            this_contact=this_contact,
            this_visit_number=2,
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SecondAntenatalCareVisit: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SecondAntenatalCareVisit: cannot not run with "
            "this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_ThirdAntenatalCareContact(
    HSI_Event, IndividualScopeEventMixin
):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Run the check
        can_anc_run = self.module.check_subsequent_anc_can_run(
            person_id, this_contact, 3, squeeze_factor, gest_age_next_contact
        )
        ...
        if can_anc_run:
            ...
            #  =================================== INTERVENTIONS ====================================================
            gest_age_next_contact = (
                self.module.determine_gestational_age_for_next_contact(person_id)
            )
            self.module.interventions_delivered_each_visit_from_anc2(hsi_event=self)
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_ThirdAntenatalCareContact: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_ThirdAntenatalCareContact: cannot not run "
            "with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_FourthAntenatalCareContact(
    HSI_Event, IndividualScopeEventMixin
):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Run the check
        can_anc_run = self.module.check_subsequent_anc_can_run(
            person_id, this_contact, 4, squeeze_factor, gest_age_next_contact
        )
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FourthAntenatalCareContact: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FourthAntenatalCareContact: cannot not run "
            "with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_FifthAntenatalCareContact(
    HSI_Event, IndividualScopeEventMixin
):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Run the check
        can_anc_run = self.module.check_subsequent_anc_can_run(
            person_id, this_contact, 5, squeeze_factor, gest_age_next_contact
        )
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FifthAntenatalCareContact: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FifthAntenatalCareContact: cannot not run "
            "with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_SixthAntenatalCareContact(
    HSI_Event, IndividualScopeEventMixin
):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Run the check
        can_anc_run = self.module.check_subsequent_anc_can_run(
            person_id, this_contact, 6, squeeze_factor, gest_age_next_contact
        )
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SixthAntenatalCareContact: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SixthAntenatalCareContact: cannot not run with"
            "this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_SeventhAntenatalCareContact(
    HSI_Event, IndividualScopeEventMixin
):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Run the check
        can_anc_run = self.module.check_subsequent_anc_can_run(
            person_id, this_contact, 7, squeeze_factor, gest_age_next_contact
        )
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SeventhAntenatalCareContact: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SeventhAntenatalCareContact: cannot not run"
            " with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_EighthAntenatalCareContact(
    HSI_Event, IndividualScopeEventMixin
):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Run the check
        can_anc_run = self.module.check_subsequent_anc_can_run(
            person_id, this_contact, 8, squeeze_factor, gest_age_next_contact
        )
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_EighthAntenatalCareContact: did not run",
        )

    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_EighthAntenatalCareContact: cannot not run"
            " with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_FocusedANCVisit(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def __init__(self, module, person_id, visit_number):
        ...
        self.EXPECTED_APPT_FOOTPRINT = self.make_appt_footprint(
            {("AntenatalFirst" if (self.visit_number == 1) else "ANCSubsequent"): 1}
        )
        self.ACCEPTED_FACILITY_LEVEL = "1a"
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (
            ...
            or not df.at[person_id, "is_pregnant"]
            or df.at[person_id, "la_currently_in_labour"]
            or (date_difference > pd.to_timedelta(7, unit="D"))
            or (df.at[person_id, "ac_total_anc_visits_current_pregnancy"] >= 4)
            or (df.at[person_id, "ps_gestational_age_in_weeks"] < 7)
            or self.visit_number > 4
            or self.visit_number
            != (df.at[person_id, "ac_total_anc_visits_current_pregnancy"] + 1)
        ):
        ...
        # pregnancy
        elif df.at[person_id, "hs_is_inpatient"] and (
            df.at[person_id, "ps_gestational_age_in_weeks"] < 37
        ):
            weeks_due_next_visit = int(
                recommended_gestation_next_anc
            ...
            elif (self.visit_number == 2) or (
                (mother.ps_gestational_age_in_weeks > 20) and (self.visit_number == 1)
            ):
                self.module.albendazole_administration(hsi_event=self)
                self.module.tetanus_vaccination(hsi_event=self)

            elif self.visit_number == 3 or (
                (mother.ps_gestational_age_in_weeks > 30) and (self.visit_number == 1)
            ):
                self.module.point_of_care_hb_testing(hsi_event=self)
...
class HSI_CareOfWomenDuringPregnancy_PresentsForInductionOfLabour(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (
            ...
            or not df.at[person_id, "is_pregnant"]
            or df.at[person_id, "la_currently_in_labour"]
            or df.at[person_id, "hs_is_inpatient"]
        ):
            return
        ...
        logger.debug(
            key="message",
            data=f"Mother {person_id} will move to labour ward for "
            f'{df.at[person_id, "ac_admitted_for_immediate_delivery"]} today',
        )

        self.sim.schedule_event(
            LabourOnsetEvent(self.sim.modules["Labour"], person_id), self.sim.date
        )


class HSI_CareOfWomenDuringPregnancy_AntenatalWardInpatientCare(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if not (
            mother.is_pregnant
            and not mother.la_currently_in_labour
            and not mother.hs_is_inpatient
        ):
            return

        # check if she will experience delayed care
        pregnancy_helper_functions.check_if_delayed_care_delivery(
            self.module, squeeze_factor, person_id, hsi_type="an"
        )
        ...
        if mother.ps_anaemia_in_pregnancy != "none":
            ...
            if fbc_result in ("mild", "moderate", "severe"):
                ...
                if pd.isnull(mother.ac_date_next_contact) or (
                    (mother.ac_date_next_contact - self.sim.date)
        ...
        if (mother.ps_gest_diab == "uncontrolled") and (
            mother.ac_gest_diab_on_treatment == "none"
        ):
            ...
            self.sim.schedule_event(
                GestationalDiabetesGlycaemicControlEvent(
                    self.sim.modules["PregnancySupervisor"], person_id
                ),
                self.sim.date + DateOffset(days=7),
            )
        ...
        if (
            mother.ps_htn_disorders in ("gest_htn", "mild_pre_eclamp")
            and not mother.ac_gest_htn_on_treatment
        ):
            self.module.initiate_maintenance_anti_hypertensive_treatment(
                person_id, self
            )
        ...
        # to reduce risk of death and pregnancy loss
            ...
            self.module.treatment_for_severe_pre_eclampsia_or_eclampsia(
                person_id, hsi_event=self
            )
            ...
            if df.at[person_id, "ac_admitted_for_immediate_delivery"] in (
                "caesarean_now",
                "caesarean_future",
            ):
        ...
        if mother.ps_antepartum_haemorrhage != "none":
            ...
            if df.at[person_id, "ac_admitted_for_immediate_delivery"] == "none":
                logger.info(
                    key="error",
                    data=f"Mother {person_id} was not admitted for delviery following APH",
                )
        ...
        # (i.e induction/caesarean)
        elif df.at[person_id, "ac_admitted_for_immediate_delivery"] in (
            "caesarean_future",
            "induction_future",
        ):
            ...
            if mother.ps_gestational_age_in_weeks < 37:
                days_until_safe_for_cs = int(
                    (37 * 7) - (mother.ps_gestational_age_in_weeks * 7)
                )
            else:
            ...
            self.sim.schedule_event(
                LabourOnsetEvent(self.sim.modules["Labour"], person_id), admission_date
            )
        else:
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_AntenatalWardInpatientCare: did not run",
        )
        return False
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_AntenatalWardInpatientCare: cannot not run"
            " with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfAnaemia(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfAnaemia: did "
            "not run",
        )
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfAnaemia: "
            "cannot not run with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfGestationalDiabetes(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (
            not mother.la_currently_in_labour
            and not mother.hs_is_inpatient
            and mother.ps_gest_diab != "none"
            and (mother.ac_gest_diab_on_treatment != "none")
            and (mother.ps_gestational_age_in_weeks > 21)
        ):
            ...
            if mother.ps_gest_diab == "uncontrolled":
                ...
                if mother.ac_gest_diab_on_treatment == "diet_exercise":
                    ...
                    cons = {
                        _i: dose
                        for _i in self.module.item_codes_preg_consumables[
                            "oral_diabetic_treatment"
                        ]
                    }
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfGestational"
            "Diabetes: did "
            "not run",
        )
...
class HSI_CareOfWomenDuringPregnancy_PostAbortionCaseManagement(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if not mother.is_alive or not abortion_complications.has_any(
            [person_id], "sepsis", "haemorrhage", "injury", "other", first=True
        ):
            return
        ...
        baseline_cons = pregnancy_helper_functions.return_cons_avail(
            self.module,
            self,
            self.module.item_codes_preg_consumables,
            core="post_abortion_care_core",
            optional="post_abortion_care_optional",
        )

        # Check HCW availability to deliver surgical removal of retained products
        sf_check = pregnancy_helper_functions.check_emonc_signal_function_will_run(
            self.sim.modules["Labour"], sf="retained_prod", hsi_event=self
        )
        ...
        if abortion_complications.has_any([person_id], "sepsis", first=True):
            ...
            cons_for_sepsis_pac = pregnancy_helper_functions.return_cons_avail(
                ...
                self,
                self.module.item_codes_preg_consumables,
                core="post_abortion_care_sepsis_core",
        ...
        elif abortion_complications.has_any([person_id], "other", first=True) and (
            baseline_cons or sf_check
        ):
            df.at[person_id, "ac_received_post_abortion_care"] = True
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_PostAbortionCaseManagement: did not run",
        )
        return False
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_PostAbortionCaseManagement: cannot not run "
            "with this configuration",
        )


class HSI_CareOfWomenDuringPregnancy_TreatmentForEctopicPregnancy(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        avail = pregnancy_helper_functions.return_cons_avail(
            ...
            self,
            self.module.item_codes_preg_consumables,
            core="ectopic_pregnancy_core",
            optional="ectopic_pregnancy_optional",
        )

        # If they are available then treatment can go ahead
        if avail:
            self.sim.modules["PregnancySupervisor"].mother_and_newborn_info[person_id][
                "delete_mni"
            ] = True
