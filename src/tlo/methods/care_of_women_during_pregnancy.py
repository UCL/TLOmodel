...
class CareOfWomenDuringPregnancy(Module):
    ...
    PARAMETERS = {
        ...
        "sensitivity_poc_hb_test": Parameter(Types.LIST, "sensitivity of a point of care Hb test to detect anaemia"),
        "specificity_poc_hb_test": Parameter(Types.LIST, "specificity of a point of care Hb test to detect anaemia"),
        "sensitivity_fbc_hb_test": Parameter(Types.LIST, "sensitivity of a Full Blood Count test to detect anaemia"),
        "specificity_fbc_hb_test": Parameter(Types.LIST, "specificity of a Full Blood Count test to detect anaemia"),
        ...
        "sensitivity_blood_test_syphilis": Parameter(Types.LIST, "sensitivity of a blood test to detect syphilis"),
        "specificity_blood_test_syphilis": Parameter(Types.LIST, "specificity of a blood test to detect syphilis"),
    }

    PROPERTIES = {
        "ac_total_anc_visits_current_pregnancy": Property(
            Types.INT, "rolling total of antenatal visits this woman has " "attended during her pregnancy"
        ),
        ...
        "ac_receiving_iron_folic_acid": Property(
            Types.BOOL, "whether this woman is receiving daily iron & folic acid " "supplementation"
        ),
        "ac_receiving_bep_supplements": Property(
            Types.BOOL, "whether this woman is receiving daily balanced energy " "and protein supplementation"
        ),
        "ac_receiving_calcium_supplements": Property(
            Types.BOOL, "whether this woman is receiving daily calcium " "supplementation"
        ),
        "ac_gest_htn_on_treatment": Property(
            Types.BOOL, "Whether this woman has been initiated on treatment for " "gestational hypertension"
        ),
        ...
        "ac_ectopic_pregnancy_treated": Property(
            Types.BOOL, "Whether this woman has received treatment for an ectopic " "pregnancy"
        ),
        "ac_received_post_abortion_care": Property(
            Types.BOOL, "bitset list of interventions delivered to a woman " "undergoing post abortion care"
        ),
        ...
        "ac_admitted_for_immediate_delivery": Property(
            Types.CATEGORICAL,
            "Admission type for women needing urgent " "delivery in the antenatal period",
            categories=["none", "induction_now", "induction_future", "caesarean_now", "caesarean_future", "avd_now"],
        ),
    }

    def read_parameters(self, data_folder):
        parameter_dataframe = read_csv_files(
            Path(self.resourcefilepath) / "ResourceFile_AntenatalCare", files="parameter_values"
        )
    ...
    def get_and_store_pregnancy_item_codes(self):
        ...
        # -------------------------------------------- ECTOPIC PREGNANCY ---------------------------------------------
        self.item_codes_preg_consumables["ectopic_pregnancy_core"] = {ic("Halothane (fluothane)_250ml_CMST"): 100}
        ...
        self.item_codes_preg_consumables["post_abortion_care_core"] = {ic("Misoprostol, tablet, 200 mcg"): 600}
        ...
        }  # TODO: update con requested here

        # --------------------------------- BALANCED ENERGY AND PROTEIN ----------------------------------------------
        # Dose changes at run time
        self.item_codes_preg_consumables["balanced_energy_protein"] = {ic("Dietary supplements (country-specific)"): 1}
        ...
        # Dose changes at run time
        self.item_codes_preg_consumables["oral_diabetic_treatment"] = {ic("Glibenclamide 5mg_1000_CMST"): 1}

        # ---------------------------------------- INSULIN ----------------------------------------------------------
        # Dose changes at run time
        self.item_codes_preg_consumables["insulin_treatment"] = {ic("Insulin soluble 100 IU/ml, 10ml_each_CMST"): 1}
    ...
    def antenatal_care_scheduler(self, individual_id, visit_to_be_scheduled, recommended_gestation_next_anc):
        ...
        visit_dict = {
            2: HSI_CareOfWomenDuringPregnancy_SecondAntenatalCareContact(self, person_id=individual_id),
        ...
        def calculate_visit_date_and_schedule_visit(visit):
            ...
            weeks_due_next_visit = int(
                recommended_gestation_next_anc - df.at[individual_id, "ps_gestational_age_in_weeks"]
            )
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                visit, priority=0, topen=visit_date, tclose=visit_date + DateOffset(days=7)
            )
    ...
    def schedule_admission(self, individual_id):
        ...
        if not df.at[individual_id, "ac_to_be_admitted"]:
            logger.info(
                key="error", data=f"Mother {individual_id} was scheduled for admission despite not requiring" f" it"
            )
            return
        ...
        self.sim.modules["HealthSystem"].schedule_hsi_event(
            inpatient, priority=0, topen=self.sim.date, tclose=self.sim.date + DateOffset(days=1)
        )
    ...
    def iron_and_folic_acid_supplementation(self, hsi_event):
        ...
        if not df.at[person_id, "ac_receiving_iron_folic_acid"]:
            ...
            avail = pregnancy_helper_functions.return_cons_avail(self, hsi_event, cons=updated_cons, opt_cons=None)
    ...
    def balance_energy_and_protein_supplementation(self, hsi_event):
        ...
        if not df.at[person_id, "ac_receiving_bep_supplements"]:
            ...
            days = self.get_approx_days_of_pregnancy(person_id)
            updated_cons = {
                k: v * days for (k, v) in self.item_codes_preg_consumables["balanced_energy_protein"].items()
            }
    ...
    def tb_screening(self, hsi_event):
        ...
        if "Tb" in self.sim.modules:
            tb_screen = HSI_Tb_ScreeningAndRefer(module=self.sim.modules["Tb"], person_id=hsi_event.target)

            self.sim.modules["HealthSystem"].schedule_hsi_event(
                tb_screen, priority=0, topen=self.sim.date, tclose=self.sim.date + DateOffset(days=1)
            )
    ...
    def tetanus_vaccination(self, hsi_event):
        ...
        if "Epi" in self.sim.modules:
            ...
            vaccine_hsi = HSI_TdVaccine(self.sim.modules["Epi"], person_id=person_id, suppress_footprint=True)

            self.sim.modules["HealthSystem"].schedule_hsi_event(vaccine_hsi, priority=0, topen=self.sim.date)
    ...
    def calcium_supplementation(self, hsi_event):
        ...
        if not df.at[person_id, "ac_receiving_calcium_supplements"] and (
            ...
            updated_cons = {k: v * days for (k, v) in self.item_codes_preg_consumables["calcium"].items()}
    ...
    def hep_b_testing(self, hsi_event):
        ...
        avail = hsi_event.get_consumables(
            item_codes=cons["hep_b_test"], optional_item_codes=cons["blood_test_equipment"]
        )
    ...
    def iptp_administration(self, hsi_event):
        ...
        if "Malaria" in self.sim.modules:
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                HSI_MalariaIPTp(person_id=person_id, module=self.sim.modules["Malaria"]),
                topen=self.sim.date,
                tclose=None,
                priority=0,
            )
    ...
    def check_subsequent_anc_can_run(self, individual_id, this_visit_number, gest_age_next_contact):
        ...
        if (
            ...
            or (
                date_difference > pd.to_timedelta(7, unit="D")
                or not df.at[individual_id, "ac_total_anc_visits_current_pregnancy"] == (this_visit_number - 1)
            )
        ):
            return False
    ...
    def initiate_maintenance_anti_hypertensive_treatment(self, individual_id, hsi_event):
        ...
        updated_cons = {k: v * days for (k, v) in self.item_codes_preg_consumables["oral_antihypertensives"].items()}
    ...
    def ectopic_pregnancy_treatment_doesnt_run(self, hsi_event):
        ...
        if df.at[individual_id, "ps_ectopic_pregnancy"] == "not_ruptured":
            self.sim.schedule_event(
                EctopicPregnancyRuptureEvent(self.sim.modules["PregnancySupervisor"], individual_id),
    ...
    def calculate_beddays(self, individual_id):
        ...
        if (
            mother.ps_placental_abruption
            or (
                (mother.ps_placenta_praevia and (mother.ps_antepartum_haemorrhage == "severe"))
                or mother.ps_chorioamnionitis
            )
        ) and (mother.ps_gestational_age_in_weeks < 28):
        ...
        # they have reached that gestation
            ...
            (mother.ps_placenta_praevia and (mother.ps_antepartum_haemorrhage == "mild_moderate"))
            or (mother.ps_premature_rupture_of_membranes and not mother.ps_chorioamnionitis)
...
class HSI_CareOfWomenDuringPregnancy_FirstAntenatalCareContact(HSI_Event, IndividualScopeEventMixin):
        ...
        if can_anc1_run:
            ...
            if mother.ps_gestational_age_in_weeks < 40:
                self.module.antenatal_care_scheduler(
                    person_id, visit_to_be_scheduled=2, recommended_gestation_next_anc=gest_age_next_contact
                )
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FirstAntenatalCareVisit: cannot not run with " "this configuration",
        )
...
class HSI_CareOfWomenDuringPregnancy_SecondAntenatalCareContact(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        can_anc_run = self.module.check_subsequent_anc_can_run(
            individual_id=person_id, this_visit_number=2, gest_age_next_contact=gest_age_next_contact
        )
        ...
        if can_anc_run:
            ...
            self.add_equipment(self.healthcare_system.equipment.from_pkg_names("ANC"))
            self.add_equipment({"Ultrasound, combined 2/4 pole interferential with vacuum and dual frequency 1-3MHZ"})
            ...
            if mother.ps_gestational_age_in_weeks < 40:
                self.module.antenatal_care_scheduler(
                    person_id, visit_to_be_scheduled=3, recommended_gestation_next_anc=gest_age_next_contact
                )
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SecondAntenatalCareVisit: cannot not run with " "this configuration",
        )
...
class HSI_CareOfWomenDuringPregnancy_ThirdAntenatalCareContact(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if can_anc_run:
            ...
            if mother.ps_gestational_age_in_weeks < 40:
                self.module.antenatal_care_scheduler(
                    person_id, visit_to_be_scheduled=4, recommended_gestation_next_anc=gest_age_next_contact
                )
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_ThirdAntenatalCareContact: cannot not run " "with this configuration",
        )
...
class HSI_CareOfWomenDuringPregnancy_FourthAntenatalCareContact(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if can_anc_run:
            ...
            if mother.ps_gestational_age_in_weeks < 40:
                self.module.antenatal_care_scheduler(
                    person_id, visit_to_be_scheduled=5, recommended_gestation_next_anc=gest_age_next_contact
                )
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FourthAntenatalCareContact: cannot not run " "with this configuration",
        )
...
class HSI_CareOfWomenDuringPregnancy_FifthAntenatalCareContact(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if can_anc_run:
            ...
            if mother.ps_gestational_age_in_weeks < 40:
                self.module.antenatal_care_scheduler(
                    person_id, visit_to_be_scheduled=6, recommended_gestation_next_anc=gest_age_next_contact
                )
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_FifthAntenatalCareContact: cannot not run " "with this configuration",
        )
...
class HSI_CareOfWomenDuringPregnancy_SixthAntenatalCareContact(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if can_anc_run:
            ...
            #  =================================== INTERVENTIONS ====================================================
            self.add_equipment({"Weighing scale", "Measuring tapes", "Stethoscope, foetal, monaural, Pinard, plastic"})

            self.module.interventions_delivered_each_visit_from_anc2(hsi_event=self)

            if mother.ps_gestational_age_in_weeks < 40:
                self.module.antenatal_care_scheduler(
                    person_id, visit_to_be_scheduled=7, recommended_gestation_next_anc=gest_age_next_contact
                )
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_SixthAntenatalCareContact: cannot not run with" "this configuration",
        )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if can_anc_run:
            ...
            if mother.ps_gestational_age_in_weeks < 40:
                self.module.iptp_administration(hsi_event=self)
                self.module.antenatal_care_scheduler(
                    person_id, visit_to_be_scheduled=8, recommended_gestation_next_anc=gest_age_next_contact
                )
...
class HSI_CareOfWomenDuringPregnancy_EighthAntenatalCareContact(HSI_Event, IndividualScopeEventMixin):
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_EighthAntenatalCareContact: cannot not run" " with this configuration",
        )
...
class HSI_CareOfWomenDuringPregnancy_FocusedANCVisit(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (
            ...
            or self.visit_number != (df.at[person_id, "ac_total_anc_visits_current_pregnancy"] + 1)
        ):
        ...
        # pregnancy
            ...
            visit_date = self.sim.date + DateOffset(weeks=weeks_due_next_visit)
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                self, priority=0, topen=visit_date, tclose=visit_date + DateOffset(days=7)
            )
...
class HSI_CareOfWomenDuringPregnancy_AntenatalWardInpatientCare(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if mother.ps_anaemia_in_pregnancy != "none":
            ...
            if fbc_result in ("mild", "moderate", "severe"):
                ...
                if pd.isnull(mother.ac_date_next_contact) or (
                    (mother.ac_date_next_contact - self.sim.date) > pd.to_timedelta(28, unit="D")
                ):
                    ...
                    self.sim.modules["HealthSystem"].schedule_hsi_event(
                        outpatient_checkup, priority=0, topen=follow_up_date, tclose=follow_up_date + DateOffset(days=7)
                    )
        ...
        if (mother.ps_gest_diab == "uncontrolled") and (mother.ac_gest_diab_on_treatment == "none"):
            ...
            self.sim.schedule_event(
                GestationalDiabetesGlycaemicControlEvent(self.sim.modules["PregnancySupervisor"], person_id),
                self.sim.date + DateOffset(days=7),
            )
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                outpatient_checkup, priority=0, topen=check_up_date, tclose=check_up_date + DateOffset(days=3)
            )
        ...
        # to reduce risk of death and pregnancy loss
            ...
            self.module.treatment_for_severe_pre_eclampsia_or_eclampsia(person_id, hsi_event=self)
        ...
        # (i.e induction/caesarean)
            ...
            if mother.ps_gestational_age_in_weeks < assumed_weeks_till_delivery:
                days_until_safe_for_cs = int(
                    (assumed_weeks_till_delivery * 7) - (mother.ps_gestational_age_in_weeks * 7)
                )
            else:
            ...
            self.sim.schedule_event(LabourOnsetEvent(self.sim.modules["Labour"], person_id), admission_date)
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_AntenatalWardInpatientCare: cannot not run" " with this configuration",
        )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if not mother.la_currently_in_labour and not mother.hs_is_inpatient:
            ...
            if fbc_result in ("mild", "moderate", "severe"):
                ...
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    admission, priority=0, topen=self.sim.date, tclose=self.sim.date + DateOffset(days=1)
                )

    def did_not_run(self):
        logger.debug(
            key="message", data="HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfAnaemia: did " "not run"
        )
...
class HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfGestationalDiabetes(
        ...
        if (
            ...
            if mother.ps_gest_diab == "uncontrolled":
                ...
                if mother.ac_gest_diab_on_treatment == "diet_exercise":
                    ...
                    updated_cons = {
                        k: v * days
                        for (k, v) in self.module.item_codes_preg_consumables["oral_diabetic_treatment"].items()
                    }
                ...
                if mother.ac_gest_diab_on_treatment == "orals":
                    ...
                    updated_cons = {
                        k: v * required_vials
                        for (k, v) in self.module.item_codes_preg_consumables["insulin_treatment"].items()
                    }
    ...
    def did_not_run(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfGestational" "Diabetes: did " "not run",
        )
...
class HSI_CareOfWomenDuringPregnancy_PostAbortionCaseManagement(HSI_Event, IndividualScopeEventMixin):
    ...
    def not_available(self):
        logger.debug(
            key="message",
            data="HSI_CareOfWomenDuringPregnancy_PostAbortionCaseManagement: cannot not run " "with this configuration",
        )
