        ...
        if Metadata.DISEASE_MODULE in m.METADATA:
            ...
            for tlo_cause, cause in declaration_in_module.items():
                ...
                if tlo_cause in collected_causes:
                    assert (collected_causes[tlo_cause].gbd_causes == cause.gbd_causes) and (
        ...
        for g in c.gbd_causes:
            if g in gbd_causes:
                assert gbd_causes[g] == c.label, (
                    f"The gbd cause {g} is defined under more than one label: " f"{gbd_causes[g]} and {c.label}."
                )
            else:
...
def create_mappers_from_causes_to_label(causes: dict, all_gbd_causes: set = None):
    ...
    mapper_from_tlo_causes = dict(
        (v, k) for k, v in (lookup_df.tlo_causes.apply(pd.Series).stack().reset_index(level=1, drop=True)).items()
    )

    #  - from gbd_cause --> label (key=gbd_cause, value=label)
    mapper_from_gbd_causes = dict(
        (v, k) for k, v in (lookup_df.gbd_causes.apply(pd.Series).stack().reset_index(level=1, drop=True)).items()
    )
