...
class Mockitis(Module, GenericFirstAppointmentsMixin):
    ...
    PARAMETERS = {
        ...
        "p_cure": Parameter(Types.REAL, "Probability that a treatment is successful in curing the individual"),
    ...
    PROPERTIES = {
        ...
        "mi_status": Property(
            Types.CATEGORICAL, "Historical status: N=never; C=currently; P=previously", categories=["N", "C", "P"]
        ),
    ...
    def initialise_simulation(self, sim):
        ...
        for person_id in people_who_will_die:
            self.sim.schedule_event(MockitisDeathEvent(self, person_id), df.at[person_id, "mi_scheduled_date_death"])
...
class MockitisEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        if df.is_alive.sum():
            prevalence = len(currently_infected) / (len(currently_infected) + len(currently_susc))
        else:
            prevalence = 0

        # 2. handle new infections
        now_infected = self.module.rng.choice([True, False], size=len(currently_susc), p=[prevalence, 1 - prevalence])
...
class HSI_Mockitis_PresentsForCareWithSevereSymptoms(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        self.sim.modules["HealthSystem"].schedule_hsi_event(event, priority=2, topen=self.sim.date, tclose=None)
...
class HSI_Mockitis_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        logger.debug(
            key="debug", data=f"This is HSI_Mockitis_StartTreatment: initiating treatent for person {person_id}"
        )
        ...
        if treatmentworks and self.get_consumables(self.module.cons_code):
            ...
            self.module.sim.modules["SymptomManager"].clear_symptoms(person_id=person_id, disease_module=self.module)
...
class MockitisLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        mask: pd.Series = df.loc[df.is_alive, "mi_date_infected"] > self.sim.date - DateOffset(months=self.repeat)
