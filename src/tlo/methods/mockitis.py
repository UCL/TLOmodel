...
class Mockitis(Module):
    ...
    CAUSES_OF_DISABILITY = {"Mockitis": Cause(label="Mockitis_Disability_And_Death")}
    ...
    def read_parameters(self, data_folder):
        ...
        if "HealthBurden" in self.sim.modules.keys():
            p["daly_wts"] = {
                ...
                "extreme_pain_in_the_nose": self.sim.modules[
                    "HealthBurden"
                ].get_daly_weight(50),
            }
        ...
        self.sim.modules["SymptomManager"].register_symptom(
            Symptom(
                name="weird_sense_of_deja_vu"
            ),  # will not trigger any health seeking behaviour
            Symptom(
                name="coughing_and_irritable"
            ),  # will not trigger any health seeking behaviour
...
class MockitisEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        logger.debug(
            key="debug",
            data="This is MockitisEvent, tracking the disease progression of the population.",
        )
        ...
        if now_infected.sum():
            ...
            logger.debug(
                key="debug", data="This is MockitisEvent, no one is newly infected."
            )
...
class HSI_Mockitis_PresentsForCareWithSevereSymptoms(
    ...
    def did_not_run(self):
        logger.debug(
            key="debug",
            data="HSI_Mockitis_PresentsForCareWithSevereSymptoms: did not run",
        )
...
class HSI_Mockitis_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        logger.debug(
            key="debug",
            data=(
                "....This is HSI_Mockitis_StartTreatment: "
...
class MockitisLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        mask: pd.Series = df.loc[
            df.is_alive, "mi_date_infected"
