...
class Mockitis(Module):
    ...
    def read_parameters(self, data_folder):
        ...
        if "HealthBurden" in self.sim.modules.keys():
            p["daly_wts"] = {
                "weird_sense_of_deja_vu": self.sim.modules[
                    "HealthBurden"
                ].get_daly_weight(48),
                "coughing_and_irritable": self.sim.modules[
                    "HealthBurden"
                ].get_daly_weight(49),
                "em_extreme_pain_in_the_nose": self.sim.modules[
                    "HealthBurden"
                ].get_daly_weight(50),
            }
    ...
    def initialise_population(self, population):
        ...
        df.loc[df.is_alive, "mi_is_infected"] = (
            self.rng.random_sample(size=alive_count) < initial_infected
        )
        ...
        for person_id_infected in df.index[df.mi_is_infected]:
            symptom_string_for_this_person = self.rng.choice(
                level_of_symptoms.level_of_symptoms, p=level_of_symptoms.probability
            )
        ...
        df.loc[df.mi_is_infected, "mi_scheduled_date_death"] = (
    ...
    def initialise_simulation(self, sim):
        ...
        for person_id in people_who_will_die:
            self.sim.schedule_event(
                MockitisDeathEvent(self, person_id),
    ...
    def on_birth(self, mother_id, child_id):
        ...
        child_is_infected = df.at[
            mother_id, "mi_is_infected"
        ]  # is infected if mother is infected
        ...
        if child_is_infected:
            ...
            self.sim.schedule_event(
                death_event, df.at[child_id, "mi_scheduled_date_death"]
            )
    ...
    def on_hsi_alert(self, person_id, treatment_id):
        ...
        logger.debug(
            "This is Mockitis, being alerted about a health system interaction "
            "person %d for: %s",
            person_id,
            treatment_id,
        )
...
class MockitisEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        logger.debug(
            "This is MockitisEvent, tracking the disease progression of the population."
        )
        ...
        now_infected = self.module.rng.choice(
            [True, False], size=len(currently_susc), p=[prevalence, 1 - prevalence]
        )
        ...
        if now_infected.sum():
            ...
            death_years_ahead = (
                5  # self.module.rng.exponential(scale=30, size=now_infected.sum())
            )
            death_td_ahead = pd.to_timedelta(death_years_ahead, unit="y")
            ...
            df.loc[infected_idx, "mi_scheduled_date_death"] = (
                self.sim.date + death_td_ahead
            )
            ...
            for person_index in infected_idx:
                ...
                self.sim.schedule_event(
                    death_event, df.at[person_index, "mi_scheduled_date_death"]
                )

            # assign symptoms
            level_of_symptoms = self.module.parameters["level_of_symptoms"]
            for person_id_infected in infected_idx:
                symptom_string_for_this_person = self.module.rng.choice(
                    level_of_symptoms.level_of_symptoms, p=level_of_symptoms.probability
                )
                ...
                if symptom_string_for_this_person != "none":
                    self.sim.modules["SymptomManager"].change_symptom(
                        ...
                        + DateOffset(days=1 + int(self.module.rng.rand() * 5)),
                        duration_in_days=30,
                    )
...
class HSI_Mockitis_PresentsForCareWithSevereSymptoms(
    ...
    def __init__(self, module, person_id):
        ...
        self.ACCEPTED_FACILITY_LEVEL = (
            1  # This enforces that the apppointment must be run at that facility-level
        )
        self.ALERT_OTHER_DISEASES = []

    def apply(self, person_id, squeeze_factor):
        logger.debug(
            "This is HSI_Mockitis_PresentsForCareWithSevereSymptoms, a first appointment for person %d",
            person_id,
        )
...
class HSI_Mockitis_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def __init__(self, module, person_id):
        ...
        the_appt_footprint[
            "NewAdult"
        ] = 1  # Plus, an amount of resources similar to an HIV initiation

        # Define the necessary information for an HSI
        self.TREATMENT_ID = "Mockitis_Treatment_Initiation"
        self.EXPECTED_APPT_FOOTPRINT = the_appt_footprint
        self.ACCEPTED_FACILITY_LEVEL = (
            1  # Enforces that this apppointment must happen at those facility-levels
        )
        self.ALERT_OTHER_DISEASES = []

    def apply(self, person_id, squeeze_factor):
        logger.debug(
            "This is HSI_Mockitis_StartTreatment: initiating treatent for person %d",
            person_id,
        )
        ...
        logger.debug(
            ...
            person_id,
            target_date_for_followup_appt,
        )

        followup_appt = HSI_Mockitis_TreatmentMonitoring(
...
class HSI_Mockitis_TreatmentMonitoring(HSI_Event, IndividualScopeEventMixin):
    ...
    def __init__(self, module, person_id):
        ...
        self.EXPECTED_APPT_FOOTPRINT = the_appt_footprint
        self.ACCEPTED_FACILITY_LEVEL = (
            1  # Allows this HSI to occur at any facility-level
        )
        self.ALERT_OTHER_DISEASES = ["*"]
...
class MockitisLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        mask: pd.Series = (
            df.loc[df.is_alive, "mi_date_infected"]
