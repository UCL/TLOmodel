...
def preterm_labour(self, df, rng=None, **externals):
    ...
    result[df.ps_gestational_age_in_weeks == 22] *= params[
        "baseline_prob_early_labour_onset"
    ][0]
    result[df.ps_gestational_age_in_weeks == 27] *= params[
        "baseline_prob_early_labour_onset"
    ][1]
    result[df.ps_gestational_age_in_weeks == 31] *= params[
        "baseline_prob_early_labour_onset"
    ][2]
    result[df.ps_gestational_age_in_weeks == 35] *= params[
        "baseline_prob_early_labour_onset"
    ][3]

    result[df.ps_premature_rupture_of_membranes] *= params[
...
def ectopic_pregnancy_death(self, df, rng=None, **externals):
    ...
    result[df.ac_ectopic_pregnancy_treated] *= params[
...
def induced_abortion_death(self, df, rng=None, **externals):
    ...
    if person["ac_received_post_abortion_care"]:
        treatment_effect = pregnancy_helper_functions.get_treatment_effect(
            externals["delay_one_two"],
            externals["delay_three"],
...
def spontaneous_abortion(self, df, rng=None, **externals):
    ...
    result[df.ps_gestational_age_in_weeks == 4] *= params[
        "prob_spontaneous_abortion_per_month"
    ][0]
    result[df.ps_gestational_age_in_weeks == 8] *= params[
        "prob_spontaneous_abortion_per_month"
    ][1]
    result[df.ps_gestational_age_in_weeks == 13] *= params[
        "prob_spontaneous_abortion_per_month"
    ][2]
    result[df.ps_gestational_age_in_weeks == 17] *= params[
        "prob_spontaneous_abortion_per_month"
    ][3]
    result[df.ps_gestational_age_in_weeks == 22] *= params[
        "prob_spontaneous_abortion_per_month"
    ...
    result[(df.age_years > 29) & (df.age_years < 35)] *= params[
...
def maternal_anaemia(self, df, rng=None, **externals):
    ...
    result[df.ac_receiving_iron_folic_acid] *= params[
    ...
    if "Hiv" in self.module.sim.modules:
        result[df.hv_inf & (df.hv_art != "not")] *= params[
...
def gest_htn(self, df, rng=None, **externals):
    ...
    result[df.ac_receiving_calcium_supplements] *= params[
...
def placental_abruption(self, df, rng=None, **externals):
    ...
    params = self.parameters
    result = pd.Series(
        data=params["prob_placental_abruption_per_month"], index=df.index
    )

    result[df.la_previous_cs_delivery > 0] *= params[
        "rr_placental_abruption_previous_cs"
    ]
    result[df.ps_htn_disorders != "none"] *= params[
...
def antenatal_stillbirth(self, df, rng=None, **externals):
    ...
    result[df.ac_receiving_bep_supplements] *= params[
    ...
    result[df.ps_htn_disorders == "mild_pre_eclamp"] *= params[
        "rr_still_birth_pre_eclampsia"
    ]
    result[df.ps_htn_disorders == "severe_pre_eclamp"] *= params[
        "rr_still_birth_pre_eclampsia"
    ]
    result[df.ps_htn_disorders == "gest_htn"] *= params["rr_still_birth_gest_htn"]
    result[df.ps_htn_disorders == "severe_gest_htn"] *= params[
    ...
    result[
        (df.ps_gest_diab == "controlled") & (df.ac_gest_diab_on_treatment != "none")
    ] *= (
        params["rr_still_birth_gest_diab"]
        * params["treatment_effect_gdm_case_management"]
    )
...
def early_initiation_anc4(self, df, rng=None, **externals):
    ...
    result[(df.la_parity > 1) & (df.la_parity < 4)] *= params[
        "aor_early_anc4_parity_2_3"
    ]
    result[(df.la_parity > 3) & (df.la_parity < 6)] *= params[
