...
class Stunting(Module, GenericFirstAppointmentsMixin):
    ...
    PARAMETERS = {
        ...
        "or_stunting_male": Parameter(Types.REAL, "Odds ratio of stunting if male gender"),
        ...
        "rr_stunting_untreated_HIV": Parameter(Types.REAL, "Relative risk of stunting for untreated HIV+"),
        ...
        "prob_stunting_diagnosed_at_generic_appt": Parameter(
            Types.REAL, "Probability of a stunted or severely stunted person being checked and correctly diagnosed"
        ),
    }
    ...
    def read_parameters(self, data_folder):
        self.load_parameters_from_dataframe(
            read_csv_files(Path(self.resourcefilepath) / "ResourceFile_Stunting", files="Parameter_values")
        )
    ...
    def initialise_population(self, population):
        ...
        for agegp in [(0, 5), (6, 11), (12, 23), (24, 35), (36, 47), (48, 59)]:  # in months
            ...
            severely_stunted_idx = stunted.loc[
                stunted & (self.rng.random_sample(len(stunted)) < p_stunting.prob_severe_given_stunting)
            ].index
        ...
        **kwargs,
        ...
        if individual_properties["age_years"] <= 5 and is_stunted:
            ...
            if p_stunting_diagnosed > self.rng.random_sample():
                event = HSI_Stunting_ComplementaryFeeding(module=self, person_id=person_id)
                schedule_hsi_event(
                    event,
...
class Models:
    ...
    def make_lm_prob_becomes_stunted(self):
        ...
        return LinearModel.multiplicative(
            Predictor(conditions_are_exhaustive=True, conditions_are_mutually_exclusive=True)
            ...
            Predictor("un_ever_wasted", conditions_are_exhaustive=True, conditions_are_mutually_exclusive=True)
    ...
    def make_lm_prob_progression_to_severe_stunting(self):
        ...
        return LinearModel.multiplicative(
            Predictor(conditions_are_exhaustive=True, conditions_are_mutually_exclusive=True)
            .when("age_exact_years < 0.5", p["r_progression_severe_stunting_by_agegp"][0])
            .when("0.5 <= age_exact_years < 1.0", p["r_progression_severe_stunting_by_agegp"][1])
            .when("1.0 <= age_exact_years < 2.0", p["r_progression_severe_stunting_by_agegp"][2])
            .when("2.0 <= age_exact_years < 3.0", p["r_progression_severe_stunting_by_agegp"][3])
            .when("3.0 <= age_exact_years < 4.0", p["r_progression_severe_stunting_by_agegp"][4])
            .when("4.0 <= age_exact_years < 5.0", p["r_progression_severe_stunting_by_agegp"][5])
    ...
    def make_lm_prob_natural_recovery(self):
        ...
        return LinearModel(LinearModelType.MULTIPLICATIVE, intercept=prob_recovery_per_year)
...
class StuntingLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        d_to_log = subset.groupby(by=["age_years", "un_HAZ_category"]).size().sort_index().to_dict()
        ...
        if (person.age_years < 5) and (person.un_HAZ_category in ["HAZ<-3", "-3<=HAZ<-2"]):
            ...
            if self.get_consumables(item_codes=self.module.cons_item_codes["supplementary_feeding"]):
                self.module.do_treatment(
                    person_id,
                    prob_success=self.module.parameters["effectiveness_of_food_supplementation_in_stunting_reduction"],
                )
                ...
                _ = self.get_consumables(item_codes=self.module.cons_item_codes["education_for_supplementary_feeding"])
                self.module.do_treatment(
                    person_id,
