...
class Stunting(Module):
    ...
    def read_parameters(self, data_folder):
        self.load_parameters_from_dataframe(
            pd.read_excel(
                Path(self.resourcefilepath) / "ResourceFile_Stunting.xlsx",
    ...
    def initialise_population(self, population):
        ...
        def get_probs_stunting(_agegp):
            ...
            p_severely_stunted_given_stunted = haz_distribution.cdf(
                -3.0
            ) / haz_distribution.cdf(-2.0)

            # Return results needed as named tuple:
            result = namedtuple(
                "probs", ["prob_stunting", "prob_severe_given_stunting"]
            )
            return result(p_stunted, p_severely_stunted_given_stunted)
        ...
        for agegp in [
            ...
            stunted = make_scaled_linear_model_stunting(
                target_prob=p_stunting.prob_stunting, mask=mask
            ).predict(df.loc[mask], self.rng, squeeze_single_row_output=False)

            severely_stunted_idx = stunted.loc[
                stunted
                & (
                    self.rng.random_sample(len(stunted))
                    < p_stunting.prob_severe_given_stunting
                )
            ].index
            stunted_but_not_severe_idx = stunted[stunted].index.difference(
                severely_stunted_idx
            )
    ...
    def look_up_consumable_item_codes(self):
        ...
        self.cons_item_codes["supplementary_feeding"] = get_item_codes(
            "Supplementary spread, sachet 92g/CAR-150"
        )
    ...
    def do_recovery(self, idx: Union[list, pd.Index]):
        ...
        df.loc[idx, "un_HAZ_category"] = df.loc[idx, "un_HAZ_category"].map(
            {"HAZ<-3": "-3<=HAZ<-2", "-3<=HAZ<-2": "HAZ>=-2"}
        )

    def do_routine_assessment_for_chronic_undernutrition(
        self, person_id: int, person_details: NamedTuple
    ) -> HSI_Event:
        ...
        is_stunted = person_details.un_HAZ_category in ("HAZ<-3", "-3<=HAZ<-2")
        p_stunting_diagnosed = self.parameters[
            "prob_stunting_diagnosed_at_generic_appt"
        ]

        # Do not schedule treatment if the patient is not stunted,
        # otherwise schedule the HSI for provision of treatment based on the
        # probability of stunting diagnosis
        if is_stunted and p_stunting_diagnosed > self.rng.random_sample():
            return HSI_Stunting_ComplementaryFeeding(module=self, person_id=person_id)
        else:
            return None
...
class Models:
    def __init__(self, module):
        ...
        self.lm_prob_progression_to_severe_stunting = (
            self.make_lm_prob_progression_to_severe_stunting()
        )
    ...
    def make_lm_prob_becomes_stunted(self):
        ...
        return LinearModel.multiplicative(
            ...
            .when("age_exact_years < 0.5", p["base_inc_rate_stunting_by_agegp"][0])
            .when("0.5 <= age_exact_years < 1", p["base_inc_rate_stunting_by_agegp"][1])
            .when("1 <= age_exact_years < 2", p["base_inc_rate_stunting_by_agegp"][2])
            .when("2 <= age_exact_years < 3", p["base_inc_rate_stunting_by_agegp"][3])
            .when("3 <= age_exact_years < 4", p["base_inc_rate_stunting_by_agegp"][4])
            .when("4 <= age_exact_years < 5", p["base_inc_rate_stunting_by_agegp"][5])
            .when("age_exact_years >= 5", 0.0),
            Predictor("li_wealth", conditions_are_mutually_exclusive=True)
            .when(1, 1.0)
            .otherwise(p["rr_stunting_wealth_level"]),
            Predictor("gi_number_of_episodes").apply(
                lambda x: p["rr_stunting_per_diarrhoeal_episode"] ** x
            ),
            Predictor(
                "un_ever_wasted",
            ...
            Predictor().when(
                '(hv_inf == True) & (hv_art == "not")', p["rr_stunting_untreated_HIV"]
            ),
            Predictor().when(
                '((nb_breastfeeding_status == "non_exclusive") | '
    ...
    def make_lm_prob_natural_recovery(self):
        ...
        return LinearModel(
            LinearModelType.MULTIPLICATIVE, intercept=prob_recovery_per_year
        )
...
class StuntingPollingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        * Determine who will be onset for progression (among those stunted but not severely) and effect that change;
        ...
        days_until_next_polling_event = (
            self.sim.date + self.frequency - self.sim.date
        ) / np.timedelta64(1, "D")

        # Onset of Stunting
        eligible_for_stunting = (
            df.is_alive & (df.age_exact_years < 5.0) & (df.un_HAZ_category == "HAZ>=-2")
        )
    ...
    def apply_model(self, model, mask, days_exposed_to_risk):
        ...
        * Randomly selects which individuals will have the events
        ...
        cum_prob_over_days_exposed = 1.0 - (1.0 - annual_prob) ** (
            days_exposed_to_risk / DAYS_IN_YEAR
        )

        assert pd.notnull(cum_prob_over_days_exposed).all()
        return mask[mask].index[
            cum_prob_over_days_exposed > rng.random_sample(mask.sum())
        ]
...
class StuntingLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        d_to_log = (
            df.loc[df.is_alive & (df.age_years < 5)]
            .groupby(by=["age_years", "un_HAZ_category"])
            .size()
            .sort_index()
            .to_dict()
        )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (person.age_years < 5) and (
            ...
            if self.get_consumables(
                item_codes=self.module.cons_item_codes["supplementary_feeding"]
            ):
                self.module.do_treatment(
                    person_id,
                    prob_success=self.module.parameters[
                ...
                _ = self.get_consumables(
                    item_codes=self.module.cons_item_codes[
                        "education_for_supplementary_feeding"
                    ]
                )
                self.module.do_treatment(
                    person_id,
