...
class BladderCancer(Module):
    ...
    def read_parameters(self, data_folder):
        ...
        self.load_parameters_from_dataframe(
            pd.read_excel(
                Path(self.resourcefilepath) / "ResourceFile_Bladder_Cancer.xlsx",
    ...
    def initialise_population(self, population):
        ...
        bc_status_any_stage = lm_init_bc_status_any_stage.predict(
        ...
        if bc_status_any_stage.sum():
            ...
            if sum_probs > 0:
                prob_by_stage_of_cancer_if_cancer = [
                    i / sum_probs for i in p["init_prop_bladder_cancer_stage"]
                ]
        ...
        has_blood_urine_at_init = lm_init_blood_urine.predict(
            df.loc[df.is_alive], self.rng
        )
        ...
        has_pelvic_pain_at_init = lm_init_pelvic_pain.predict(
            df.loc[df.is_alive], self.rng
        )
        self.sim.modules["SymptomManager"].change_symptom(
        ...
        df.loc[treatment_initiated, "bc_date_treatment"] = df.loc[
            treatment_initiated, "bc_date_diagnosis"
        ]
        ...
        df.loc[select_for_care, "bc_date_palliative_care"] = df.loc[
            select_for_care, "bc_date_diagnosis"
        ]
    ...
    def initialise_simulation(self, sim):
        ...
        * Schedule the palliative care appointments for those that are on palliative care at initiation
        ...
        sim.schedule_event(
            BladderCancerLoggingEvent(self), sim.date + DateOffset(months=0)
        )

        # ----- SCHEDULE MAIN POLLING EVENTS -----
        # Schedule main polling event to happen immediately
        sim.schedule_event(
            BladderCancerMainPollingEvent(self), sim.date + DateOffset(months=0)
        )
        ...
        on_palliative_care_at_initiation = df.index[
            df.is_alive & ~pd.isnull(df.bc_date_palliative_care)
        ]
    ...
    * Deaths from bladder Cancer for those in metastatic
    ...
    def apply(self, population):
        ...
        selected_to_die = metastatic_idx[
            ...
            < m.parameters["r_death_bladder_cancer"]
        ]
...
class HSI_BladderCancer_Investigation_Following_Blood_Urine(
...
class HSI_BladderCancer_Investigation_Following_pelvic_pain(
    HSI_Event, IndividualScopeEventMixin
):
...
class HSI_BladderCancer_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        df.at[person_id, "bc_stage_at_which_treatment_given"] = df.at[
            person_id, "bc_status"
        ]
...
class BladderCancerLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        out.update(
            {
                f"total_{k}": v
                for k, v in df.loc[df.is_alive].bc_status.value_counts().items()
            }
        )
        ...
        out.update(
            {
                "diagnosed_since_last_log": df.bc_date_diagnosis.between(
                    date_lastlog, date_now
                ).sum(),
                "treated_since_last_log": df.bc_date_treatment.between(
                    date_lastlog, date_now
                ).sum(),
                "palliative_since_last_log": df.bc_date_palliative_care.between(
                    date_lastlog, date_now
                ).sum(),
                "death_bladder_cancer_since_last_log": df.bc_date_death.between(
                    date_lastlog, date_now
                ).sum(),
            }
        )
