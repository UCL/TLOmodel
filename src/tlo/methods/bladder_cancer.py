...
class BladderCancer(Module, GenericFirstAppointmentsMixin):
    ...
    PROPERTIES = {
        ...
        "bc_date_diagnosis": Property(
            Types.DATE, "the date of diagnosis of the bladder cancer (pd.NaT if never diagnosed)"
        ),
        ...
        "bc_date_death": Property(Types.DATE, "date bc death"),
    }
    ...
    def read_parameters(self, data_folder):
        ...
        self.load_parameters_from_dataframe(
            read_csv_files(Path(self.resourcefilepath) / "ResourceFile_Bladder_Cancer", files="parameter_values")
        )

        # Register Symptom that this module will use
        self.sim.modules["SymptomManager"].register_symptom(
            Symptom(name="blood_urine", odds_ratio_health_seeking_in_adults=4.00, no_healthcareseeking_in_children=True)
        )
        # Register Symptom that this module will use
        self.sim.modules["SymptomManager"].register_symptom(
    ...
    def initialise_population(self, population):
        ...
        in_metastatic_diagnosed = df.index[
            df.is_alive & (df.bc_status == "metastatic") & ~pd.isnull(df.bc_date_diagnosis)
        ]
    ...
    def initialise_simulation(self, sim):
        ...
        * Schedule the palliative care appointments for those that are on palliative care at initiation
        ...
        lm["tis_t1"] = LinearModel(
            LinearModelType.MULTIPLICATIVE, p["r_tis_t1_bladder_cancer_none"], *(predictors + conditional_predictors)
        )
        ...
        lm["t2p"] = LinearModel(
            ...
            Predictor("had_treatment_during_this_stage", external=True).when(
                True, p["rr_t2p_bladder_cancer_undergone_curative_treatment"]
            ),
            Predictor("bc_status").when("tis_t1", 1.0).otherwise(0.0),
        )
        ...
        lm["metastatic"] = LinearModel(
            ...
            Predictor("bc_status").when("t2p", 1.0).otherwise(0.0),
        )
    ...
    def report_daly_values(self):
        ...
        disability_series_for_alive_persons.loc[((df.bc_status == "tis_t1") | (df.bc_status == "t2p"))] = self.daly_wts[
            "tis_t1_t2p"
        ]
        ...
        disability_series_for_alive_persons.loc[
            (df.bc_status == "metastatic") & (pd.isnull(df.bc_date_palliative_care))
        ...
        **kwargs,
        ...
        if individual_properties["age_years"] > 5:
            ...
            if "blood_urine" in symptoms:
                event = HSI_BladderCancer_Investigation_Following_Blood_Urine(person_id=person_id, module=self)
                schedule_hsi_event(event, topen=self.sim.date, priority=0)

            if "pelvic_pain" in symptoms:
                event = HSI_BladderCancer_Investigation_Following_pelvic_pain(person_id=person_id, module=self)
                schedule_hsi_event(event, topen=self.sim.date, priority=0)
    ...
    * Deaths from bladder Cancer for those in metastatic
    ...
    def apply(self, population):
        ...
        for person_id in selected_to_die:
            self.sim.schedule_event(InstantaneousDeath(m, person_id, "BladderCancer"), self.sim.date)
...
class HSI_BladderCancer_Investigation_Following_Blood_Urine(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        cons_avail = self.get_consumables(
            item_codes=self.module.item_codes_bladder_can["screening_cystoscopy_core"],
            optional_item_codes=self.module.item_codes_bladder_can["screening_biopsy_endoscopy_cystoscopy_optional"],
        )
        ...
        if cons_avail:
            ...
            dx_result = hs.dx_manager.run_dx_test(
                dx_tests_to_run="cystoscopy_for_bladder_cancer_given_blood_urine", hsi_event=self
            )
            ...
            if dx_result:
                ...
                if not in_metastatic:
                    ...
                    hs.schedule_hsi_event(
                        hsi_event=HSI_BladderCancer_StartTreatment(module=self.module, person_id=person_id),
                        priority=0,
                        topen=self.sim.date,
                        tclose=None,
                    )
                    ...
                    hs.schedule_hsi_event(
                        hsi_event=HSI_BladderCancer_PalliativeCare(module=self.module, person_id=person_id),
                        priority=0,
                        topen=self.sim.date,
                        tclose=None,
                    )
...
class HSI_BladderCancer_Investigation_Following_pelvic_pain(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if cons_avail:
            ...
            # Use a cystoscope to diagnose whether the person has bladder Cancer:
            dx_result = hs.dx_manager.run_dx_test(
                dx_tests_to_run="cystoscopy_for_bladder_cancer_given_pelvic_pain", hsi_event=self
            )
...
class HSI_BladderCancer_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "bc_status"] == "metastatic":
            logger.warning(
                key="warning",
                data="Cancer is metastatic - aborting HSI_BladderCancer_StartTreatment,"
        ...
        # Check consumables are available
        cons_avail = self.get_consumables(
            item_codes=self.module.item_codes_bladder_can["treatment_surgery_core"],
            optional_item_codes=self.module.item_codes_bladder_can["treatment_surgery_optional"],
        )
...
class HSI_BladderCancer_PostTreatmentCheck(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "bc_status"] == "metastatic":
            ...
            hs.schedule_hsi_event(
                hsi_event=HSI_BladderCancer_PostTreatmentCheck(module=self.module, person_id=person_id),
...
class HSI_BladderCancer_PalliativeCare(HSI_Event, IndividualScopeEventMixin):
    ...
    * Itself for the continuance of care.
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        # Check consumables are available
        cons_available = self.get_consumables(item_codes=self.module.item_codes_bladder_can["palliation"])
...
class BladderCancerLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        out.update({f"total_{k}": v for k, v in df.loc[df.is_alive].bc_status.value_counts().items()})

        # Current counts, undiagnosed
        out.update(
            {
                f"undiagnosed_{k}": v
                for k, v in df.loc[df.is_alive].loc[pd.isnull(df.bc_date_diagnosis), "bc_status"].value_counts().items()
            }
        )
        ...
        out.update(
            {
                ...
                .loc[~pd.isnull(df.bc_date_diagnosis), "bc_status"]
                .value_counts()
                .items()
            }
        )
        ...
        out.update(
            {
                f"treatment_{k}": v
                for k, v in df.loc[df.is_alive]
                .loc[(~pd.isnull(df.bc_date_treatment) & pd.isnull(df.bc_date_palliative_care)), "bc_status"]
                .value_counts()
                .items()
            }
        )
