...
class ChildhoodPneumonia (Module):
    ...
    PARAMETERS = {
        ...
        'rp_pneumonia_age12to23mo': Parameter(Types.REAL, 'relative prevalence of pneumonia for age 12 to 23 months'
                                             ),
        'rp_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative prevalence of pneumonia for age 24 to 59 months'
                                             ),
        ...
        'base_incidence_ri_pneumonia': Parameter(Types.REAL, 'baseline incidence of pneumonia, among children '
                                                             'aged 2-11 months, HIV negative,'
                                                             'no SAM, no indoor air pollution '
                                            ),
        'rr_pneumonia_agelt2mo': Parameter(Types.REAL, 'relative rate of pneumonia for age < 2 months'
                                           ),
        'rr_pneumonia_age12to23mo': Parameter(Types.REAL, 'relative rate of pneumonia for age 12 to 23 months'
                                               ),
        'rr_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative rate of pneumonia for age 24 to 59 months'
                                               ),
        ...
        'rp_severe_pneumonia_age12to23mo': Parameter(Types.REAL, 'relative prevalence of severe pneumonia '
                                                                  'for age 12 to 23 months'
                                                         ),
        'rp_severe_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative prevalence of severe pneumonia'
        ...
        'base_incidence_ri_severe_pneumonia': Parameter(Types.REAL, 'baseline incidence of severe pneumonia,'
        ...
        'rr_severe_pneumonia_age12to23mo': Parameter(Types.REAL,
                                                      'relative rate of severe pneumonia for age 12 to 23 months'),

        'rr_severe_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative rate of severe pneumonia'
                                                                  ' for age 24 to 59 months'
                                                      ),
        ...
        'rr_progress_severe_pneumonia_age12to23mo': Parameter(Types.REAL, 'relative rate of progression '
                                                                          'to severe pneumonia for age 12 to 23 months'
                                                         ),
        'rr_progress_severe_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative rate of progression '
        ...
        'rr_death_pneumonia_agelt2months': Parameter(Types.REAL, 'relative risk of common cold for age < 2 months'
                                          ),
        'rr_death_pneumonia_age12to23mo': Parameter(Types.REAL, 'relative risk of death from pneumonia '
                                                                    'for age 12 to 23 months'
                                             ),
        'rr_death_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative risk of death from pneumonia '
                                                                    'for age 24 to 59 months'
                                             ),
        ...
        'rr_recovery_pneumonia_age12to23mo': Parameter(Types.REAL, 'relative rate of recovery from pneumonia '
                                                           'for age between 12 to 23 months'
                                                           ),
        'rr_recovery_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative rate of recovery from pneumonia '
        ...
        'rr_recovery_severe_pneumonia_age12to23mo': Parameter(Types.REAL, 'relative rate of recovery from'
                                                                  ' severe pneumonia for age between 12 to 23 months'
                                                         ),
        'rr_recovery_severe_pneumonia_age24to59mo': Parameter(Types.REAL, 'relative rate of recovery from '
    ...
    def read_parameters(self, data_folder):
        ...
        p['base_incidence_ri_pneumonia'] = 0.0015
        ...
        p['rr_pneumonia_indoor_air_pollution'] = 1.1
        ...
        p['rr_severe_pneumonia_indoor_air_pollution'] = 1.1
        ...
        p['rr_recovery_severe_pneumonia_agelt2mo'] = 0.6
        p['rr_recovery_severe_pneumonia_age12to23mo'] = 1.2
        p['rr_recovery_severe_pneumonia_age24to59mo'] = 1.5
        ...
        p['init_prop_resp_infection_status'] = [0.2, 0.1]
    ...
    def initialise_population(self, population):
        ...
        df['ri_resp_infection_stat'] = 'none'
        df['ri_cough']= False
        df['ri_fever'] = False
        ...
        # create dataframe of the probabilities of ri_resp_infection_status for children
        # aged 2-11 months, HIV negative, normal weight, no SAM, no indoor air pollution
        p_resp_infect_stat = pd.DataFrame(data=[self.init_prop_resp_infection_status],
                                          columns=['pneumonia', 'severe pneumonia'], index=under5_idx)

        # create probabilities of pneumonia for all age under 5
        p_resp_infect_stat.loc[(df.age_exact_years < 0.1667) & df.is_alive] *= self.rp_pneumonia_agelt2mo
        p_resp_infect_stat.loc[(df.age_exact_years >= 1) & (df.age_exact_years < 2) & df.is_alive] *= self.rp_pneumonia_age12to23mo
        p_resp_infect_stat.loc[(df.age_exact_years >= 2) & (df.age_exact_years < 5) & df.is_alive] *= self.rp_pneumonia_age24to59mo
        p_resp_infect_stat.loc[(df.has_hiv == True) & (df.age_years < 5) & df.is_alive] *= self.rp_pneumonia_HIV
        p_resp_infect_stat.loc[(df.malnutrition == True) & (df.age_years < 5) & df.is_alive] *= self.rp_pneumonia_malnutrition
        p_resp_infect_stat.loc[(df.indoor_air_pollution == True) & (df.age_years <5) & df.is_alive] *= self.rp_pneumonia_indoor_air_pollution

        # create probabilities of severe pneumonia for all age under 5
        p_resp_infect_stat.loc[(df.age_exact_years < 0.1667) & df.is_alive] *= self.rp_severe_pneumonia_agelt2mo
        p_resp_infect_stat.loc[(df.age_exact_years >= 1) & (df.age_exact_years < 2) & df.is_alive] *= self.rp_severe_pneumonia_age12to23mo
        p_resp_infect_stat.loc[(df.age_exact_years >= 2) & (df.age_exact_years < 5) & df.is_alive] *= self.rp_severe_pneumonia_age24to59mo
        p_resp_infect_stat.loc[(df.has_hiv == True) & (df.age_years < 5) & df.is_alive] *= self.rp_severe_pneumonia_HIV
        p_resp_infect_stat.loc[(df.malnutrition == True) & (df.age_years < 5) & df.is_alive] *= self.rp_severe_pneumonia_malnutrition
        p_resp_infect_stat.loc[(df.indoor_air_pollution == True) & (df.age_years < 5) & df.is_alive] *= self.rp_severe_pneumonia_indoor_air_pollution
...
class RespInfectionEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        ri_current_none_indoor_air_pollution_idx = df.index[df.is_alive & (df.ri_resp_infection_stat == 'none') &
...
#       eff_prob_ri_pneumonia.loc[ri_current_none_HHhandwashing_idx] *= m.rr_pneumonia_HHhandwashing
        ...
        df.loc[idx_incident_pneumonia, 'ri_respiratory_infection_status'] = 'pneumonia'
