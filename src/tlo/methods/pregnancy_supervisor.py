...
class PregnancySupervisor(Module):
    ...
    def new_comps_dummy(self, index, month):
        ...
        # store as param
        month_6_comps = ['maternal_anaemia', 'gest_htn', 'pre_eclampsia', 'gest_diab', 'antenatal_death',
                         'antenatal_stillbirth']

        # Create a data frame of all women who are of the gestation provided in the month parameter which has all of
        # their individual risks of each possible complication for this month of pregnancy, generated by the linear
        # model

        # I feel like this could be done with a list parameter of complications and a for loop but couldnt work it out
    ...
    def set_pregnancy_complications(self, index, complication):
        ...
        if complication == 'gest_diab':
            for person in positive_index:
                assert df.at[person, 'ps_gest_diab'] == False
