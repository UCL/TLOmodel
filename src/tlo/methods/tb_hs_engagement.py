...
class health_system_tb(Module):
    ...
    PARAMETERS = {
        ...
        "testing_prob_xpert": Parameter(
            Types.REAL, "probability of individual receiving xpert test"
        ),
        "prop_xpert_positive": Parameter(
            Types.REAL,
            "proportion active tb cases tested with xpert with positive results",
        ),
        "prob_tb_treatment": Parameter(
            Types.REAL, "probability of individual starting treatment"
        ),
        "prob_mdr": Parameter(Types.REAL, "probability tb case is mdr"),
        "prob_tb_mdr_treatment": Parameter(
            Types.REAL, "probability of individual starting mdr treatment"
        ),
    }
    ...
    def initialise_simulation(self, sim):
        ...
        sim.schedule_event(
            TbHealthSystemLoggingEvent(self), sim.date + DateOffset(months=1)
        )
...
class TbTestingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        for person in undiagnosed_idx:
            ...
            referral_time = abs(
                np.random.normal(loc=(2 / 12), scale=(0.5 / 12), size=1)
            )  # in years
...
class tbXpertTest(Event, IndividualScopeEventMixin):
    ...
    def apply(self, individual_id):
        ...
        if (
            ...
            diagnosed = np.random.choice(
                [True, False],
                size=1,
                p=[params["prop_xpert_positive"], (1 - params["prop_xpert_positive"])],
            )
