...
class OtherAdultCancer(Module):
    ...
    CAUSES_OF_DEATH = {
        "OtherAdultCancer": Cause(
            gbd_causes=gbd_causes_of_cancer_represented_in_this_module,
            label="Cancer (Other)",
        )
    }
    ...
    PARAMETERS = {
        "init_prop_early_other_adult_ca_symptom_other_adult_cancer_by_stage": Parameter(
            Types.LIST,
            "initial proportions of those with other adult cancer categories that have other "
            "adult_ca_symptom",
        ),
        ...
        "init_prop_with_early_other_adult_ca_symptom_diagnosed_by_stage": Parameter(
            Types.LIST,
            "initial proportions of people that have symptom of other_adult_ca_symptom that "
            "have been diagnosed",
        ),
        ...
        "sensitivity_of_diagnostic_device_for_other_adult_cancer_with_other_adult_ca_site_confined": Parameter(
            Types.REAL,
            "sensitivity of diagnostic_device_for diagnosis of other_adult cancer for those with "
    ...
    def read_parameters(self, data_folder):
        ...
        self.load_parameters_from_dataframe(
            pd.read_excel(
                Path(self.resourcefilepath) / "ResourceFile_Other_Adult_Cancers.xlsx",
    ...
    def initialise_population(self, population):
        ...
        oac_status_ = lm_init_oac_status_any_stage.predict(
        ...
        if sum(oac_status_):
            ...
            if sum_probs > 0:
                prob_by_stage_of_cancer_if_cancer = [
                    i / sum_probs for i in p["in_prop_other_adult_cancer_stage"]
                ]
        ...
        lm_init_early_other_adult_ca_symptom = LinearModel.multiplicative(
            ...
            .when(
                "site_confined",
                p["init_prop_early_other_adult_ca_symptom_other_adult_cancer_by_stage"][
                    0
                ],
            )
            .when(
        ...
        has_early_other_adult_ca_symptom_at_init = (
            lm_init_early_other_adult_ca_symptom.predict(df.loc[df.is_alive], self.rng)
        )
        self.sim.modules["SymptomManager"].change_symptom(
            person_id=has_early_other_adult_ca_symptom_at_init.index[
                has_early_other_adult_ca_symptom_at_init
            ].tolist(),
        ...
        lm_init_treatment_for_those_diagnosed = LinearModel.multiplicative(
            ...
            .when("none", 0.0)
            .when(
                "site_confined", p["init_prop_treatment_status_other_adult_cancer"][0]
            )
        ...
        treatment_initiated = lm_init_treatment_for_those_diagnosed.predict(
            df.loc[df.is_alive], self.rng
        )
        ...
        df.loc[treatment_initiated, "oac_stage_at_which_treatment_given"] = (
            "site_confined"
        )

        # set date at which treatment began: same as diagnosis (NB. no HSI is established for this)
        df.loc[treatment_initiated, "oac_date_treatment"] = df.loc[
            treatment_initiated, "oac_date_diagnosis"
        ]
        ...
        in_metastatic_diagnosed = df.index[
            ...
            & ~pd.isnull(df.oac_date_diagnosis)
        ]

        select_for_care = (
            self.rng.random_sample(size=len(in_metastatic_diagnosed))
            < p["init_prob_palliative_care"]
        )
        ...
        df.loc[select_for_care, "oac_date_palliative_care"] = df.loc[
            select_for_care, "oac_date_diagnosis"
        ]
    ...
    def initialise_simulation(self, sim):
        ...
        * Schedule the palliative care appointments for those that are on palliative care at initiation
        ...
        sim.schedule_event(
            OtherAdultCancerLoggingEvent(self), sim.date + DateOffset(months=0)
        )

        # ----- SCHEDULE MAIN POLLING EVENTS -----
        # Schedule main polling event to happen immediately
        sim.schedule_event(
            OtherAdultCancerMainPollingEvent(self), sim.date + DateOffset(months=1)
        )
        ...
        lm["local_ln"] = LinearModel(
            ...
            Predictor("had_treatment_during_this_stage", external=True).when(
                True, p["rr_local_ln_other_adult_ca_undergone_curative_treatment"]
            ),
            Predictor("oac_status").when("site_confined", 1.0).otherwise(0.0),
        )
        ...
        lm["metastatic"] = LinearModel(
            ...
            Predictor("had_treatment_during_this_stage", external=True).when(
                True, p["rr_metastatic_undergone_curative_treatment"]
            ),
            Predictor("oac_status").when("local_ln", 1.0).otherwise(0.0),
        )
        ...
        self.sim.modules["HealthSystem"].dx_manager.register_dx_test(
            diagnostic_device_for_other_adult_cancer_given_other_adult_ca_symptom=DxTest(
                ...
                sensitivity=self.parameters[
                    "sensitivity_of_diagnostic_device_for_other_adult_cancer_with_other_"
        ...
        if "HealthBurden" in self.sim.modules:
            ...
            self.daly_wts["site_confined_local_ln"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                sequlae_code=550
            ...
            self.daly_wts["site_confined_local_ln_treated"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                sequlae_code=547
            ...
            self.daly_wts["metastatic"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(
                sequlae_code=549
            ...
            self.daly_wts["metastatic_palliative_care"] = self.daly_wts[
                "site_confined_local_ln_treated"
            ]
        ...
        on_palliative_care_at_initiation = df.index[
            df.is_alive & ~pd.isnull(df.oac_date_palliative_care)
        ]
    ...
    def report_daly_values(self):
        ...
        )  # shortcut to population properties dataframe for alive persons

        disability_series_for_alive_persons = pd.Series(
            index=df.index[df.is_alive], data=0.0
        )
        ...
        disability_series_for_alive_persons.loc[
            ((df.oac_status == "site_confined") | (df.oac_status == "local_ln"))
        ...
        disability_series_for_alive_persons.loc[
            (df.oac_status == "metastatic") & (pd.isnull(df.oac_date_palliative_care))
...
class OtherAdultCancerMainPollingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    * Deaths from Other_adult Cancer for those in metastatic
    ...
    def apply(self, population):
        ...
        onset_early_other_adult_ca_symptom = (
            self.module.lm_onset_early_other_adult_ca_symptom.predict(
                df.loc[df.is_alive], rng
            )
        )
        self.sim.modules["SymptomManager"].change_symptom(
            person_id=onset_early_other_adult_ca_symptom[
                onset_early_other_adult_ca_symptom
            ].index.tolist(),
            symptom_string="early_other_adult_ca_symptom",
            add_or_remove="+",
        ...
        selected_to_die = metastatic_idx[
            rng.random_sample(size=len(metastatic_idx))
            < self.module.parameters["r_death_other_adult_cancer"]
        ]

        for person_id in selected_to_die:
            self.sim.schedule_event(
                InstantaneousDeath(self.module, person_id, "OtherAdultCancer"),
                self.sim.date,
            )
...
class HSI_OtherAdultCancer_Investigation_Following_early_other_adult_ca_symptom(
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        assert "early_other_adult_ca_symptom" in self.sim.modules[
            "SymptomManager"
        ].has_what(person_id)
...
class HSI_OtherAdultCancer_StartTreatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "oac_status"] == "metastatic":
            logger.warning(
                key="warning",
                data="Cancer is metastatic- aborting HSI_OtherAdultCancer_StartTreatment,"
        ...
        df.at[person_id, "oac_stage_at_which_treatment_given"] = df.at[
            person_id, "oac_status"
        ]
...
class OtherAdultCancerLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        out.update(
            {
                f"total_{k}": v
                for k, v in df.loc[df.is_alive].oac_status.value_counts().items()
            }
        )
        ...
        out.update(
            {
                ...
                .loc[pd.isnull(df.oac_date_diagnosis), "oac_status"]
                .value_counts()
                .items()
            }
        )
        ...
        out.update(
            {
                f"palliative_{k}": v
                for k, v in df.loc[df.is_alive]
                .loc[~pd.isnull(df.oac_date_palliative_care), "oac_status"]
                .value_counts()
                .items()
            }
        )
        ...
        n_newly_diagnosed_site_confined = (
            df.oac_date_diagnosis.between(date_lastlog, date_now)
            & (df.oac_status == "site_confined1")
        ).sum()
        ...
        n_sy_early_other_adult_ca_symptom = (
            df.is_alive & (df.sy_early_other_adult_ca_symptom >= 1)
        ).sum()
        ...
        n_diagnosed_age_50p = (
            df.is_alive & (df.age_years >= 50) & ~pd.isnull(df.oac_date_diagnosis)
        ).sum()
        ...
        out.update(
            {
                "diagnosed_since_last_log": df.oac_date_diagnosis.between(
                    date_lastlog, date_now
                ).sum(),
                "treated_since_last_log": df.oac_date_treatment.between(
                    date_lastlog, date_now
                ).sum(),
                "palliative_since_last_log": df.oac_date_palliative_care.between(
                    date_lastlog, date_now
                ).sum(),
                "death_other_adult_cancer_since_last_log": df.oac_date_death.between(
                    date_lastlog, date_now
                ).sum(),
                "n age 15+": n_ge15,
