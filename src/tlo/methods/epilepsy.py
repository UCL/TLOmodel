...
class Epilepsy(Module, GenericFirstAppointmentsMixin):
    ...
    PARAMETERS = {
        ...
        "prob_start_anti_epilep_when_seizures_detected_in_generic_first_appt": Parameter(
            Types.REAL,
            "probability that someone who has had a seizure is started on anti-epileptics. This is "
    ...
    def read_parameters(self, data_folder):
        ...
        dfd = read_csv_files(
            Path(self.resourcefilepath) / "epilepsy" / "ResourceFile_Epilepsy", files="parameter_values"
        )
        ...
        self.sim.modules["SymptomManager"].register_symptom(Symptom.emergency("seizures"))
    ...
    def initialise_population(self, population):
        ...
        df.loc[alive_idx, "ep_seiz_stat"] = self.rng.choice(
            ["0", "1", "2", "3"], size=len(alive_idx), p=self.parameters["init_epil_seiz_status"]
        )
        ...
        self.sim.modules["SymptomManager"].change_symptom(
            person_id=dfg.to_list(), symptom_string="seizures", add_or_remove="+", disease_module=self
        )
    ...
    def transition_seizure_stat(self):
        ...
        probability_of_transition_1_2 = pd.DataFrame(
            [prop_transition_1_2] * len(random_draw_1_2), columns=["prob"], index=population_with_seizure_status_1
        )
        ...
        if len(both_up_down_seiz_stat_2) > 0:
            for person in both_up_down_seiz_stat_2:
                chosen_direction = self.rng.choice(
                    ["1", "3"],
                    p=np.divide(
                        [prop_transition_2_1, prop_transition_2_3], sum([prop_transition_2_1, prop_transition_2_3])
                    ),
                )
        ...
        if len(both_down_seiz_stat_3) > 0:
            for person in both_down_seiz_stat_3:
                chosen_direction = self.rng.choice(
                    ["1", "2"],
                    p=np.divide(
                        [prop_transition_3_1, prop_transition_3_2], sum([prop_transition_3_1, prop_transition_3_2])
                    ),
                )
                df.loc[person, "ep_seiz_stat"] = chosen_direction
    ...
    def get_best_available_medicine(self, hsi_event) -> Union[None, str]:
        ...
        possible_treatments = {
            "phenobarbitone": hsi_event.get_consumables(self.item_codes["phenobarbitone"], to_log=False),
...
class EpilepsyEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        for individual_id in alive_seiz_stat_2_or_3_idx[chosen]:
            self.sim.schedule_event(InstantaneousDeath(self.module, individual_id, "Epilepsy"), self.sim.date)

        # -------------------- UPDATING OF SYMPTOM OF seizures OVER TIME --------------------------------

        dfg = df.index[df.is_alive & ((df.ep_seiz_stat == "2") | (df.ep_seiz_stat == "3"))]
        self.sim.modules["SymptomManager"].change_symptom(
            person_id=dfg.to_list(), symptom_string="seizures", add_or_remove="+", disease_module=self.module
        )
...
class EpilepsyLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        status_groups = df[["ep_seiz_stat", "is_alive", "ep_antiep"]].groupby("ep_seiz_stat").sum()
        ...
        epi_death_rate = (n_epi_death * 4 * 1000) / n_seiz_stat_2_3 if n_seiz_stat_2_3 > 0 else 0.0

        cum_deaths = (~df.is_alive).sum()

        # Proportion of those with infrequent or frequent seizures currently on anti-epileptics
        prop_freq_or_infreq_seiz_on_antiep = (
            status_groups[2:].ep_antiep.sum() / status_groups[2:].is_alive.sum()
            if status_groups[2:].is_alive.sum() > 0
            else 0
        )
...
class HSI_Epilepsy_Start_Anti_Epileptic(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if best_available_medicine is not None:
            ...
            dose = {
                "phenobarbitone": 9131,  # 100mg per day - 3 months
                "carbamazepine": 91_311,  # 1000mg per day - 3 months
                "phenytoin": 27_393,
            }  # 300mg per day - 3 months

            self.get_consumables({self.module.item_codes[best_available_medicine]: dose[best_available_medicine]})
            ...
            self.module.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=self, topen=self.sim.date + pd.DateOffset(months=1), tclose=None, priority=0
            )
...
class HSI_Epilepsy_Follow_Up(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if best_available_medicine is not None:
            ...
            dose = {
                "phenobarbitone_3_mnths": 9131,
                "phenobarbitone_12_mnths": 36_525,  # 100mg per day - 3/12 months
                "carbamazepine_3_mnths": 91_311,
                "carbamazepine_12_mnths": 365_250,  # 1000mg per day - 3/12 months
                "phenytoin_3_mnths": 27_393,
                "phenytoin_12_mnths": 109_575,
            }  # 300mg per day - 3/12 months

            self.get_consumables(
                {self.module.item_codes[best_available_medicine]: dose[f"{best_available_medicine}_{fu_mnths}_mnths"]}
            )
            ...
            # Schedule follow-up
            hs.schedule_hsi_event(
                hsi_event=self, topen=self.sim.date + DateOffset(months=fu_mnths), tclose=None, priority=0
            )
        elif (
            self._counter_of_failed_attempts_due_to_unavailable_medicines
            < self._MAX_NUMBER_OF_FAILED_ATTEMPTS_BEFORE_DEFAULTING
            ...
            hs.schedule_hsi_event(hsi_event=self, topen=self.sim.date + DateOffset(months=1), tclose=None, priority=0)
        else:
