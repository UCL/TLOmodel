...
class Epilepsy(Module):
    ...
    PARAMETERS = {
        "init_epil_seiz_status": Parameter(
            Types.LIST, "Proportions in each seizure status category at baseline"
        ),
        ...
        "base_3m_prob_epilepsy": Parameter(
            Types.REAL, "base probability of epilepsy per 3 month period if age < 20"
        ),
        "rr_epilepsy_age_ge20": Parameter(
            Types.REAL, "relative rate of epilepsy if age over 20"
        ),
        ...
        "base_prob_3m_epi_death": Parameter(
            Types.REAL, "base probability per 3 months of epilepsy death"
        ),
        ...
        "daly_wt_epilepsy_severe": Parameter(
            Types.REAL,
            "disability weight for severe epilepsy" "controlled phase - code 860",
        ),
        "daly_wt_epilepsy_less_severe": Parameter(
            Types.REAL,
            "disability weight for less severe epilepsy" "controlled phase - code 861",
        ),
        "daly_wt_epilepsy_seizure_free": Parameter(
            Types.REAL,
            "disability weight for less severe epilepsy" "controlled phase - code 862",
        ),
    }
    ...
    PROPERTIES = {
        ...
        "ep_unified_symptom_code": Property(
            Types.CATEGORICAL, "", categories=["0", "1", "2", "3"]
        ),
    ...
    def read_parameters(self, data_folder):
        ...
        dfd = pd.read_excel(
            Path(self.resourcefilepath) / "epilepsy" / "ResourceFile_Epilepsy.xlsx",
        ...
        if "HealthBurden" in self.sim.modules.keys():
            ...
            p["daly_wt_epilepsy_severe"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=860)
            p["daly_wt_epilepsy_less_severe"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=861)
            p["daly_wt_epilepsy_seizure_free"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=862)
    ...
    def initialise_population(self, population):
        ...
        df.loc[alive_idx, "ep_seiz_stat"] = self.rng.choice(
            ["0", "1", "2", "3"],
            size=len(alive_idx),
        ...
        dfg = df.index[
            df.is_alive & ((df.ep_seiz_stat == "2") | (df.ep_seiz_stat == "3"))
        ]
    ...
    def initialise_simulation(self, sim):
        ...
        self.item_codes = dict()
        self.item_codes["phenobarbitone"] = hs.get_item_code_from_item_name(
            "Phenobarbital, 100 mg"
        )
        self.item_codes["carbamazepine"] = hs.get_item_code_from_item_name(
            "Carbamazepine 200mg_1000_CMST"
        )
        self.item_codes["phenytoin"] = hs.get_item_code_from_item_name(
            "Phenytoin sodium 100mg_1000_CMST"
        )
    ...
    def transition_seizure_stat(self):
        ...
        df = self.sim.population.props
        population_with_seizure_status_1 = df.index[
            df.is_alive & (df.ep_seiz_stat == "1")
        ]
        seizure_status_1_on_anti_epileptics = df.index[
            df.is_alive & (df.ep_seiz_stat == "1") & df.ep_antiep
        ]
        population_with_seizure_status_2 = df.index[
            df.is_alive & (df.ep_seiz_stat == "2")
        ]
        seizure_status_2_on_anti_epileptics = df.index[
            df.is_alive & (df.ep_seiz_stat == "2") & df.ep_antiep
        ]
        population_with_seizure_status_3 = df.index[
            df.is_alive & (df.ep_seiz_stat == "3")
        ]
        seizure_status_3_on_anti_epileptics = df.index[
            df.is_alive & (df.ep_seiz_stat == "3") & df.ep_antiep
        ]
        ...
        probability_of_transition_1_2 = pd.DataFrame(
            [prop_transition_1_2] * len(random_draw_1_2),
            columns=["prob"],
            index=population_with_seizure_status_1,
        )

        # Reduce the risk of worsening seizures if on anti epileptics
        probability_of_transition_1_2.loc[
            seizure_status_1_on_anti_epileptics, "prob"
        ] /= rr_effectiveness_antiepileptics

        # determine who will transition between seizure state 1 to 2
        changing_1_2 = population_with_seizure_status_1[
            probability_of_transition_1_2["prob"] > random_draw_1_2
        ]
        ...
        # transition state
        random_draw_2_1 = self.rng.random_sample(
        ...
        probability_of_transition_2 = pd.DataFrame(
            {
                "prob_down": [prop_transition_2_1] * len(random_draw_2_1),
        ...
        probability_of_transition_2.loc[
            seizure_status_2_on_anti_epileptics, "prob_down"
        ] *= rr_effectiveness_antiepileptics

        # Decrease the likelihood of reducing seizure status for those on anti epileptics
        probability_of_transition_2.loc[
            seizure_status_2_on_anti_epileptics, "prob_up"
        ] /= rr_effectiveness_antiepileptics

        # Establish who has a changing seizure status
        changing_2_1 = population_with_seizure_status_2[
            probability_of_transition_2["prob_down"] > random_draw_2_1
        ]
        changing_2_3 = population_with_seizure_status_2[
            probability_of_transition_2["prob_up"] > random_draw_2_3
        ]
        ...
        if len(both_up_down_seiz_stat_2) > 0:
            for person in both_up_down_seiz_stat_2:
                chosen_direction = self.rng.choice(
                    ["1", "3"],
                    p=np.divide(
                        [prop_transition_2_1, prop_transition_2_3],
        ...
        # transition state
        random_draw_3_1 = self.rng.random_sample(
            size=len(population_with_seizure_status_3)
        )
        random_draw_3_2 = self.rng.random_sample(
            size=len(population_with_seizure_status_3)
        )

        # create a dataframe for the transition probabilities
        probability_of_transition_3 = pd.DataFrame(
            {
                "prob_down_1": [prop_transition_3_2] * len(random_draw_3_2),
        ...
        # Increase the likelihood of reducing seizure status for those on anti epileptics
        probability_of_transition_3.loc[
            seizure_status_3_on_anti_epileptics, "prob_down_1"
        ] *= rr_effectiveness_antiepileptics

        # Decrease the likelihood of reducing seizure status for those on anti epileptics
        probability_of_transition_3.loc[
            seizure_status_3_on_anti_epileptics, "prob_down_2"
        ] /= rr_effectiveness_antiepileptics

        # Establish who has a changing seizure status
        changing_3_2 = population_with_seizure_status_3[
            probability_of_transition_3["prob_down_1"] > random_draw_3_2
        ]
        changing_3_1 = population_with_seizure_status_3[
            probability_of_transition_3["prob_down_2"] > random_draw_3_1
        ]
        ...
        if len(both_down_seiz_stat_3) > 0:
            for person in both_down_seiz_stat_3:
                chosen_direction = self.rng.choice(
                    ["1", "2"],
                    p=np.divide(
                        [prop_transition_3_1, prop_transition_3_2],
    ...
    def stop_antiep(self, indices, probability):
        ...
        df = self.sim.population.props
        df.loc[indices, "ep_antiep"] = probability < self.rng.random_sample(
            size=len(indices)
        )
    ...
    def get_best_available_medicine(self, hsi_event) -> Union[None, str]:
        ...
        possible_treatments = {
            "phenobarbitone": hsi_event.get_consumables(
                self.item_codes["phenobarbitone"], to_log=False
            ),
            "carbamazepine": hsi_event.get_consumables(
                self.item_codes["carbamazepine"], to_log=False
            ),
            "phenytoin": hsi_event.get_consumables(
                self.item_codes["phenytoin"], to_log=False
            ),
        }
        ...
        best_option_index = next(
            (i for i, j in enumerate(possible_treatments.values()) if j), None
        )
...
class EpilepsyEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def __init__(self, module):
        ...
        self.rr_stop_antiepileptic_seiz_infreq_or_freq = p[
    ...
    def apply(self, population):
        ...
        ge20_seiz_stat_0_idx = df.index[
            df.is_alive & (df.ep_seiz_stat == "0") & (df.age_years >= 20)
        ]
        ...
        seiz_stat_3_idx = alive_seiz_stat_0_idx[
            epi_now & (self.prop_inc_epilepsy_seiz_freq > random_draw_02)
        ]
        ...
        seiz_stat_2_idx = alive_seiz_stat_0_idx[
            epi_now & (self.prop_inc_epilepsy_seiz_freq <= random_draw_02)
        ]
        ...
        incidence_epilepsy = (
            (n_incident_epilepsy * 4 * 100000) / n_alive if n_alive > 0 else 0
        )
        ...
        alive_seiz_stat_1_antiep_idx = df.index[
            df.is_alive & (df.ep_seiz_stat == "1") & df.ep_antiep
        ]
        alive_seiz_stat_2_or_3_antiep_idx = df.index[
            df.is_alive & (df.ep_seiz_stat.isin(["2", "3"])) & df.ep_antiep
        ]

        # rate of stop ep_antiep if ep_seiz_stat = 1
        ep.stop_antiep(
            alive_seiz_stat_1_antiep_idx, self.base_prob_3m_stop_antiepileptic
        )

        # rate of stop ep_antiep if ep_seiz_stat = 2 or 3
        ep.stop_antiep(
            alive_seiz_stat_2_or_3_antiep_idx,
        ...
        df.loc[df.is_alive & (df.ep_seiz_stat == "1"), "ep_disability"] = (
            self.daly_wt_epilepsy_seizure_free
        )
        df.loc[df.is_alive & (df.ep_seiz_stat == "2"), "ep_disability"] = (
            self.daly_wt_epilepsy_less_severe
        )
        df.loc[df.is_alive & (df.ep_seiz_stat == "3"), "ep_disability"] = (
            self.daly_wt_epilepsy_severe
        )

        # Work out who is having seizures which may lead to death
        alive_seiz_stat_2_or_3_idx = df.index[
            df.is_alive & (df.ep_seiz_stat.isin(["2", "3"]))
        ]
        # determine if those having seizures will die as a result of epilepsy
        chosen = self.base_prob_3m_epi_death > self.module.rng.random_sample(
            size=len(alive_seiz_stat_2_or_3_idx)
        )
        ...
        for individual_id in alive_seiz_stat_2_or_3_idx[chosen]:
            self.sim.schedule_event(
                InstantaneousDeath(self.module, individual_id, "Epilepsy"),
                self.sim.date,
            )
        ...
        dfh = df.index[
            df.is_alive & ((df.ep_seiz_stat == "0") | (df.ep_seiz_stat == "1"))
        ]
        self.sim.modules["SymptomManager"].change_symptom(
...
class EpilepsyLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        status_groups = (
            df[["ep_seiz_stat", "is_alive", "ep_antiep"]].groupby("ep_seiz_stat").sum()
        )
        ...
        status_groups["prop_seiz_stats"] = status_groups.is_alive / sum(
            status_groups.is_alive
        )

        status_groups["prop_seiz_stat_on_anti_ep"] = (
            status_groups["ep_antiep"] / status_groups.is_alive
        )
        status_groups["prop_seiz_stat_on_anti_ep"] = status_groups[
            "prop_seiz_stat_on_anti_ep"
        ].fillna(0)
        epi_death_rate = (
        ...
        logger.info(
            ...
            data={
                ...
                "prop_antiepilep_seiz_stat_0": status_groups[
                    "prop_seiz_stat_on_anti_ep"
                ].iloc[0],
                "prop_antiepilep_seiz_stat_1": status_groups[
                    "prop_seiz_stat_on_anti_ep"
                ].iloc[1],
                "prop_antiepilep_seiz_stat_2": status_groups[
                    "prop_seiz_stat_on_anti_ep"
                ].iloc[2],
                "prop_antiepilep_seiz_stat_3": status_groups[
                    "prop_seiz_stat_on_anti_ep"
                ].iloc[3],
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if best_available_medicine is not None:
            ...
            hs.schedule_hsi_event(
                ...
                topen=self.sim.date
                + DateOffset(
                    months=3 if df.at[person_id, "ep_seiz_stat"] == "3" else 12
                ),
                tclose=None,
                priority=0,
            )
