...
class Demography(Module):
    ...
    def __init__(self, name=None, resourcefilepath=None):
        ...
        self.causes_of_death = (
            dict()
        )  # will store all the causes of death that are possible in the simulation
        self.gbd_causes_of_death = (
            set()
        )  # will store all the causes of death defined in the GBD data
    ...
    OPTIONAL_INIT_DEPENDENCIES = {
        "ScenarioSwitcher"
    }  # <-- Forces the 'ScenarioSwitcher' to be the first registered
    ...
    PARAMETERS = {
        "max_age_initial": Parameter(
            Types.INT, "The oldest age (in whole years) in the initial population"
        ),
        "pop_2010": Parameter(
            Types.DATA_FRAME, "Population in 2010 for initialising population"
        ),
        "district_num_to_district_name": Parameter(
            Types.DICT, "Mapping from district_num to district name"
        ),
        "district_num_to_region_name": Parameter(
            Types.DICT, "Mapping from district_num to region name"
        ),
        "districts_in_region": Parameter(
            Types.DICT, "Set of districts (by name) that are within a region (by name)"
        ),
        "all_cause_mortality_schedule": Parameter(
            Types.DATA_FRAME, "All-cause age-specific mortality rates from WPP"
        ),
        "fraction_of_births_male": Parameter(Types.REAL, "Birth Sex Ratio"),
        "gbd_causes_of_death_data": Parameter(
            Types.DATA_FRAME,
    ...
    def read_parameters(self, data_folder):
        ...
        self.load_parameters_from_dataframe(
            pd.read_csv(
                ...
                / "demography"
                / "ResourceFile_Demography_parameters.csv"
            )
        )
        ...
        self.districts = (
            self.parameters["pop_2010"]["District"].drop_duplicates().to_list()
        )
        self.parameters["district_num_to_district_name"] = (
            self.parameters["pop_2010"][["District_Num", "District"]]
            .drop_duplicates()
        ...
        self.parameters["district_num_to_region_name"] = (
            self.parameters["pop_2010"][["District_Num", "Region"]]
            .drop_duplicates()
        ...
        for _district in (
            self.parameters["pop_2010"][["District", "Region"]]
            .drop_duplicates()
            .itertuples()
        ):
        ...
        self.parameters["gbd_causes_of_death_data"] = pd.read_csv(
            Path(self.resourcefilepath)
            / "gbd"
            / "ResourceFile_CausesOfDeath_GBD2019.csv"
    ...
    def pre_initialise_population(self):
        ...
        # 1) Store all the cause of death represented in the imported GBD data
        self.gbd_causes_of_death = set(
            self.parameters["gbd_causes_of_death_data"].columns
        )
    ...
    def initialise_population(self, population):
        ...
        init_pop = self._edit_init_pop_to_prevent_persons_greater_than_max_age(
            init_pop, max_age=self.parameters["max_age_initial"]
        )
        ...
        df.loc[df.is_alive, "mother_id"] = (
            DEFAULT_MOTHER_ID  # Motherless, and their characterists are not inherited
        )
        df.loc[df.is_alive, "district_num_of_residence"] = demog_char_to_assign[
            "District_Num"
        ].values[:]
        df.loc[df.is_alive, "district_of_residence"] = demog_char_to_assign[
            "District"
        ].values[:]
        df.loc[df.is_alive, "region_of_residence"] = demog_char_to_assign[
            "Region"
        ].values[:]
    ...
    def on_birth(self, mother_id, child_id):
        ...
        _district_num_of_residence = df.at[
            _id_inherit_from, "district_num_of_residence"
        ]
        ...
        # Log the birth:
        _mother_age_at_birth = df.at[
            abs(mother_id), "age_years"
        ]  # Log age of mother whether true or direct birth
        _mother_age_at_pregnancy = (
            int(
                age_at_date(
        ...
        )  # No pregnancy for direct birth

        logger.info(
            key="on_birth",
            data={
                "mother": mother_id,  # Keep track of whether true or direct birth by using mother_id
                "child": child_id,
    ...
    def _edit_init_pop_to_prevent_persons_greater_than_max_age(self, df, max_age: int):
        ...
        _df = df.drop(
            df.index[df.Age > max_age]
        )  # Remove characteristics with age greater than max_age
    ...
    def process_causes_of_death(self):
        ...
        self.gbd_causes_of_death_not_represented_in_disease_modules = (
            get_gbd_causes_not_represented_in_disease_modules(
                causes=self.causes_of_death, gbd_causes=self.gbd_causes_of_death
            )
        )
        ...
        logger.info(
            key="mapper_from_tlo_cause_to_common_label", data=mapper_from_tlo_causes
        )
        logger.info(
            key="mapper_from_gbd_cause_to_common_label", data=mapper_from_gbd_causes
        )
    ...
    def do_death(self, individual_id: int, cause: str, originating_module: Module):
        ...
        * 'originating_module' is the disease module that is causing the death.
        ...
        assert not hasattr(
            individual_id, "__iter__"
        ), "do_death must be called for one individual at a time."
        ...
        assert (
            cause in self.causes_of_death
        ), f"The cause of death {cause} is not declared."
        if originating_module is not self:
            assert (
                cause in originating_module.CAUSES_OF_DEATH
        ...
        if ("Contraception" in self.sim.modules) or (
            "SimplifiedBirths" in self.sim.modules
        ):
        ...
        if "HealthSystem" in self.sim.modules:
            if person.hs_is_inpatient:
                self.sim.modules["HealthSystem"].remove_beddays_footprint(
                    person_id=individual_id
                )
    ...
    def create_mappers_from_causes_of_death_to_label(self):
        ...
        return create_mappers_from_causes_to_label(
            causes=self.causes_of_death, all_gbd_causes=self.gbd_causes_of_death
        )
    ...
    def calc_py_lived_in_last_year(self, delta=pd.DateOffset(years=1), mask=None):
        ...
        df_py = df.loc[
            condition, ["sex", "age_exact_years", "age_years", "date_of_birth"]
        ]

        # renaming columns for clarity
        df_py = df_py.rename(
            {"age_exact_years": "age_exact_end", "age_years": "age_years_end"}, axis=1
        )

        # exact age at the start
        df_py["age_exact_start"] = age_at_date(one_year_ago, df_py.date_of_birth)
        df_py["age_years_start"] = np.floor(df_py.age_exact_start).astype(
            np.int64
        )  # int age at start of the period
        df_py["years_in_age_start"] = (
            df_py.age_years_end - df_py.age_exact_start
        )  # time spent in age at start
        df_py["years_in_age_end"] = (
            df_py.age_exact_end - df_py.age_years_end
        )  # time spent in age at end
        ...
        df_py.loc[condition, "years_in_age_start"] = (
            df_py.age_exact_end - df_py.age_exact_start
        )
        ...
        df1 = (
            df_py[["sex", "years_in_age_start", "age_years_start"]]
            .groupby(by=["sex", "age_years_start"])
            .sum()
        )
        df1 = df1.unstack("sex")
        ...
        df2 = (
            df_py[["sex", "years_in_age_end", "age_years_end"]]
            .groupby(by=["sex", "age_years_end"])
            .sum()
        )
        df2 = df2.unstack("sex")
...
class OtherDeathPoll(RegularEvent, PopulationScopeEventMixin):
    ...
    def __init__(self, module):
        ...
        self.causes_to_represent = (
            self.module.gbd_causes_of_death_not_represented_in_disease_modules
        )
    ...
    def get_mort_risk_per_poll(self):
        ...
        prop_of_deaths_to_represent = (
            self.get_proportion_of_deaths_to_represent_as_other_deaths()
        )
        logger.info(
            key="other_deaths",
        ...
        all_cause_mort_risk["age_group"] = all_cause_mort_risk["age_years"].map(
            self.module.AGE_RANGE_LOOKUP
        )
        mort_risk = all_cause_mort_risk.merge(
            prop_of_deaths_to_represent.reset_index(name="prop"),
        ...
        return mort_risk[
            ["fallbackyear", "sex", "age_years", "prob_of_dying_before_next_poll"]
        ]

    def get_all_cause_mort_risk_per_poll(self):
        """Compute the all-cause risk of death per poll"""
        # Get time elapsed between each poll:
        dur_in_years_between_polls = (
            self.frequency.months * DAYS_IN_MONTH / DAYS_IN_YEAR
        )

        # Compute all-cause mortality risk per poll
        return (
            self.module.parameters["all_cause_mortality_schedule"]
            .assign(
                prob_of_dying_before_next_poll=lambda x: (
                    1.0 - np.exp(-x.death_rate * dur_in_years_between_polls)
                )
            )
    ...
    def apply(self, population):
        ...
        mort_risk = self.mort_risk_per_poll.loc[
            ...
            ["age_years", "sex", "prob_of_dying_before_next_poll"],
        ].copy()
        ...
        will_die = (
            ...
            < alive.prob_of_dying_before_next_poll
        )
...
class InstantaneousDeath(Event, IndividualScopeEventMixin):
    ...
    def apply(self, individual_id):
        self.sim.modules["Demography"].do_death(
            individual_id, cause=self.cause, originating_module=self.module
        )
