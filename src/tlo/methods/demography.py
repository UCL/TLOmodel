    ...
    def report_prevalence(self):
        ...
        if 'SimplifiedBirths' in self.sim.modules:
            ...
            if len(df[(df['age_days'] < 29) & (df['date_of_death'] >= (self.sim.date - DateOffset(months=1)))]) == 0:
                ...
                neonatal_deaths = (len(df[(df['age_days'] < 29) & (df['age_years'] == 0) & ~(df['is_alive']) & (df['date_of_death'] >= (self.sim.date - DateOffset(months=1)))]) / len(df[df['age_days'] < 29])) * 1000
                ...
                indirect_deaths_non_hiv = len(df.loc[
                                                  ...
                                                  (df['cause_of_death'].str.contains(
                                                   ...
                                                   (df['cause_of_death'] == 'TB')) & (df['date_of_death'] >= (self.sim.date - DateOffset(months=1)))
                                                  ])

                direct_deaths_non_hiv = len(df.loc[
                                                (df['is_pregnant'] | df['la_is_postpartum']) &
                                                df['cause_of_death'].str.contains('AIDS_non_TB|AIDS_TB') & (df['date_of_death'] >= (self.sim.date - DateOffset(months=1)))
                                                ])
