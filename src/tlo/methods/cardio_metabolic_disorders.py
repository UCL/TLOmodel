...
class CardioMetabolicDisorders(Module, GenericFirstAppointmentsMixin):
    ...
    other_params_dict = {
        ...
        "pr_bmi_reduction": Parameter(
            Types.INT, "probability of an individual having a reduction in BMI following " "weight loss treatment"
        ),
    }
    ...
    condition_list = {f"nc_{p}": Property(Types.BOOL, f"Whether or not someone has {p}") for p in conditions}
    condition_diagnosis_list = {
        f"nc_{p}_ever_diagnosed": Property(Types.BOOL, f"Whether or not someone has ever been diagnosed with {p}")
        for p in conditions
    }
    condition_date_diagnosis_list = {
        f"nc_{p}_date_diagnosis": Property(Types.DATE, f"When someone has been diagnosed with {p}") for p in conditions
    }
    condition_date_of_last_test_list = {
        f"nc_{p}_date_last_test": Property(Types.DATE, f"When someone has last been tested for {p}") for p in conditions
    }
    condition_medication_list = {
        f"nc_{p}_on_medication": Property(Types.BOOL, f"Whether or not someone is on medication for {p}")
        for p in conditions
    }
    condition_medication_death_list = {
        f"nc_{p}_medication_prevents_death": Property(
            Types.BOOL, f"Whether or not medication (if provided) will " f"prevent death from {p}"
        )
        for p in conditions
    }
    event_list = {f"nc_{p}": Property(Types.BOOL, f"Whether or not someone has had a {p}") for p in events}
    event_date_last_list = {f"nc_{p}_date_last_event": Property(Types.DATE, f"Date of last {p}") for p in events}
    event_diagnosis_list = {
        f"nc_{p}_ever_diagnosed": Property(Types.BOOL, f"Whether or not someone has ever been diagnosed with {p}")
        for p in events
    }
    event_date_diagnosis_list = {
        f"nc_{p}_date_diagnosis": Property(Types.DATE, f"When someone has last been diagnosed with {p}") for p in events
    }
    event_medication_list = {
        f"nc_{p}_on_medication": Property(Types.BOOL, f"Whether or not someone has ever been diagnosed with {p}")
        for p in events
    }
    event_scheduled_date_death_list = {
        f"nc_{p}_scheduled_date_death": Property(Types.DATE, f"Scheduled date of death from {p}") for p in events
    }
    ...
    PROPERTIES = {
        ...
        **event_scheduled_date_death_list,
        "nc_ever_weight_loss_treatment": Property(
            Types.BOOL, "whether or not the person has ever had weight loss " "treatment"
        ),
        "nc_weight_loss_worked": Property(Types.BOOL, "whether or not weight loss treatment worked"),
        ...
        for condition in self.conditions:
            ...
            for symptom in self.prob_symptoms[condition].keys():
                lm_init_symptoms = LinearModel(
                    ...
                    Predictor(f"nc_{condition}", conditions_are_mutually_exclusive=True, conditions_are_exhaustive=True)
                    .when(True, 1.0)
                    .when(False, 0.0),
                )
    ...
    def initialise_simulation(self, sim):
        ...
        * Build the LinearModels for the onset/removal of each condition:
        """
        sim.schedule_event(
            CardioMetabolicDisorders_MainPollingEvent(self, self.parameters["interval_between_polls"]), sim.date
        )
        sim.schedule_event(CardioMetabolicDisorders_LoggingEvent(self), sim.date)

        # Get DALY weights
        if "HealthBurden" in self.sim.modules.keys():
            self.daly_wts["daly_diabetes_uncomplicated"] = self.sim.modules["HealthBurden"].get_daly_weight(
                sequlae_code=971
            )
            self.daly_wts["daly_diabetes_complicated"] = self.sim.modules["HealthBurden"].get_daly_weight(
                sequlae_code=970
            )
            self.daly_wts["daly_chronic_kidney_disease_moderate"] = self.sim.modules["HealthBurden"].get_daly_weight(
                sequlae_code=978
            )
            self.daly_wts["daly_chronic_ischemic_hd"] = self.sim.modules["HealthBurden"].get_daly_weight(
                sequlae_code=697
            )
            self.daly_wts["daly_chronic_lower_back_pain"] = self.sim.modules["HealthBurden"].get_daly_weight(
                sequlae_code=1180
            )
            self.daly_wts["daly_stroke"] = self.sim.modules["HealthBurden"].get_daly_weight(sequlae_code=701)
            self.daly_wts["daly_heart_attack_acute_days_1_2"] = self.sim.modules["HealthBurden"].get_daly_weight(
                sequlae_code=689
            )
            self.daly_wts["daly_heart_attack_acute_days_3_28"] = self.sim.modules["HealthBurden"].get_daly_weight(
                sequlae_code=693
            )
            ...
            self.daly_wts["daly_heart_attack_avg"] = (2 / 28) * self.daly_wts["daly_heart_attack_acute_days_1_2"] + (
                26 / 28
        ...
        self.sim.modules["HealthSystem"].dx_manager.register_dx_test(
            assess_diabetes=DxTest(
                property="nc_diabetes", item_codes=self.parameters["diabetes_hsi"]["test_item_code"].astype(int)
            )
        )
    ...
    def build_linear_model(self, condition, interval_between_polls, lm_type):
        ...
        predictors = [
            ...
            Predictor("li_bmi", conditions_are_mutually_exclusive=True, conditions_are_exhaustive=True)
            .when("1", p["rr_bmi_1"])
            .when("2", p["rr_bmi_2"])
            .when("3", p["rr_bmi_3"])
            .when("4", p["rr_bmi_4"])
            ...
            Predictor("li_mar_stat", conditions_are_mutually_exclusive=True, conditions_are_exhaustive=True)
            ...
            Predictor("li_ed_lev", conditions_are_mutually_exclusive=True, conditions_are_exhaustive=True)
        ...
        conditional_predictors = (
            [
                Predictor().when("hv_inf & " '(hv_art != "on_VL_suppressed")', p["rr_hiv"]),
            ]
        ...
        linearmodel = LinearModel(
            LinearModelType.MULTIPLICATIVE, baseline_annual_probability, *(predictors + conditional_predictors)
        )
    ...
    def update_risk_score(self):
        ...
        df.loc[df.is_alive, "nc_risk_score"] = (
            df[["li_low_ex", "li_high_salt", "li_high_sugar", "li_tob", "li_ex_alc"]] > 0
        ).sum(1)
    ...
    def report_daly_values(self):
        ...
        dw["diabetes"].loc[self.sim.modules["SymptomManager"].who_has("diabetes_symptoms")] = self.daly_wts[
            "daly_diabetes_complicated"
        ]

        # Chronic Lower Back Pain: give those who have symptoms moderate weight
        dw["lower_back_pain"].loc[
            self.sim.modules["SymptomManager"].who_has("chronic_lower_back_pain_symptoms")
        ...
        dw["chronic_kidney_disease"].loc[
            self.sim.modules["SymptomManager"].who_has("chronic_kidney_disease_symptoms")
        ...
        dur_heart_attack_in_days = (
            (self.sim.date - start_heart_attack).dt.days.clip(lower=0, upper=days_in_last_month).fillna(0.0)
        )
    ...
    def do_at_generic_first_appt(
        ...
        **kwargs,
        ...
        for condition in self.conditions:
            is_already_diagnosed = individual_properties[f"nc_{condition}_ever_diagnosed"]
            has_symptom = f"{condition}_symptoms" in symptoms
            date_of_last_test = individual_properties[f"nc_{condition}_date_last_test"]
            next_test_due = pd.isnull(date_of_last_test) or (self.sim.date - date_of_last_test).days > DAYS_IN_YEAR / 2
        ...
        **kwargs,
        ...
        for ev in self.events:
            ...
            if f"{ev}_damage" in symptoms and (
                (self.sim.date - individual_properties[f"nc_{ev}_date_last_event"]).days <= 3
            ):
        ...
        for condition in self.module.conditions:
            ...
            if len(self.module.lms_symptoms[condition]) > 0:
                symptom_eligible_population = (
                    df.is_alive
                    & df[f"nc_{condition}"]
                    & ~df.index.isin(self.sim.modules["SymptomManager"].who_has(f"{condition}_symptoms"))
                )
...
class CardioMetabolicDisordersEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        self.sim.modules["SymptomManager"].change_symptom(
            person_id=person_id, disease_module=self.module, add_or_remove="+", symptom_string=f"{self.event}_damage"
        )
        ...
        if self.module.rng.random_sample() < prob_death[person_id]:
            ...
            self.sim.schedule_event(
                CardioMetabolicDisordersDeathEvent(self.module, person_id, originating_cause=self.event),
                date_of_outcome,
            )
        ...
        if f"{self.originating_cause}" in self.module.events:
            if self.sim.date == person[f"nc_{self.originating_cause}_scheduled_date_death"]:
                self.sim.modules["Demography"].do_death(
                    individual_id=person_id, cause=f"{self.originating_cause}", originating_module=self.module
                )
        else:
...
class CardioMetabolicDisorders_LoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        def age_cats(ages_in_years):
            ...
            _age_cats = pd.Series(
                pd.Categorical(ages_in_years.map(AGE_RANGE_LOOKUP), categories=AGE_RANGE_CATEGORIES, ordered=True)
            )
            return _age_cats
        ...
        def proportion_of_something_in_a_groupby_ready_for_logging(_df, something, groupbylist):
            dfx = _df.groupby(groupbylist).apply(
                lambda dft: pd.Series({"something": dft[something].sum(), "not_something": (~dft[something]).sum()})
            )
        ...
        for condition in self.module.conditions:
            ...
            adult_prevalence = {
                "prevalence": len(df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)])
            ...
            if len(df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)]) > 0:
                diagnosed = {
                    f"{condition}_diagnosis_prevalence": len(
                        df[df[f"nc_{condition}_ever_diagnosed"] & df.is_alive & (df.age_years >= 20)]
                    )
                    / len(df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)])
                }
            else:
            ...
            if len(df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)]) > 0:
                on_medication = {
                    f"{condition}_medication_prevalence": len(
                        df[df[f"nc_{condition}_on_medication"] & df.is_alive & (df.age_years >= 20)]
                    )
                    / len(df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)])
                }
            else:
        ...
        for event in self.module.events:
            ...
            adult_prevalence = {
                "prevalence": len(df[df[f"nc_{event}"] & df.is_alive & (df.age_years >= 20)])
                / len(df[df.is_alive & (df.age_years >= 20)])
            }
            logger.info(
        ...
        if self.module.do_condition_combos:
            ...
            n_comorbidities_all = pd.DataFrame(
                index=self.module.age_cats, columns=list(range(0, len(self.module.condition_list) + 1))
            )
            ...
            for i in range(0, len(condition_combos)):
                df.loc[df.is_alive, "nc_condition_combos"] = np.where(
                    df.loc[df.is_alive, f"{condition_combos[i][0]}"] & df.loc[df.is_alive, f"{condition_combos[i][1]}"],
                    True,
                    False,
                )
            ...
            logger.info(
                key="prop_combos", description="proportion of combinations of morbidities", data=prop_combos.to_dict()
            )
...
class HSI_CardioMetabolicDisorders_CommunityTestingForHypertension(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        dx_result = hs.dx_manager.run_dx_test(dx_tests_to_run="assess_hypertension", hsi_event=self)
...
class HSI_CardioMetabolicDisorders_Investigations(HSI_Event, IndividualScopeEventMixin):
    ...
    def do_for_each_condition(self, _c) -> bool:
        ...
        # Run a test to diagnose whether the person has condition:
        dx_result = hs.dx_manager.run_dx_test(dx_tests_to_run=f"assess_{_c}", hsi_event=self)
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (
            ...
            self.sim.schedule_event(
                ...
                random_date(
                    self.sim.date + pd.DateOffset(months=6), self.sim.date + pd.DateOffset(months=12), self.module.rng
                ),
            )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if self.condition != "chronic_kidney_disease" and (df.at[person_id, "li_bmi"] > 2):
            ...
            self.sim.schedule_event(
                ...
                random_date(self.sim.date + pd.DateOffset(months=6), self.sim.date + pd.DateOffset(months=12), m.rng),
            )
    ...
    def do_for_each_event_to_be_investigated(self, _ev):
        ...
        if dx_result:
            ...
            if self.module.parameters["prob_care_provided_given_seek_emergency_care"] > self.module.rng.random_sample():
                ...
                if self.get_consumables(
                    item_codes={
                        self.module.parameters[f"{_ev}_hsi"].get("emergency_medication_item_code").astype(int): dose
                    }
                ):
