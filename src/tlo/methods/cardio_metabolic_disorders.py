...
class CardioMetabolicDisorders(Module):
    ...
    CAUSES_OF_DEATH = {
        "diabetes": Cause(gbd_causes="Diabetes mellitus", label="Diabetes"),
        "chronic_ischemic_hd": Cause(
            gbd_causes={"Ischemic heart disease", "Hypertensive heart disease"},
            label="Heart Disease",
        ),
        ...
        "ever_stroke": Cause(gbd_causes="Stroke", label="Stroke"),
    ...
    CAUSES_OF_DISABILITY = {
        ...
        "stroke": Cause(gbd_causes="Stroke", label="Stroke"),
        "chronic_kidney_disease": Cause(
            gbd_causes="Chronic kidney disease", label="Kidney Disease"
        ),
        "lower_back_pain": Cause(gbd_causes={"Low back pain"}, label="Lower Back Pain"),
    }
    ...
    onset_conditions_param_dicts = {
        f"{p}_onset": Parameter(
            Types.DICT,
            f"all the parameters that specify the linear models for onset of {p}",
        )
        for p in conditions
    }
    removal_conditions_param_dicts = {
        f"{p}_removal": Parameter(
            Types.DICT,
            f"all the parameters that specify the linear models for removal of {p}",
        )
        for p in conditions
    }
    hsi_conditions_param_dicts = {
        f"{p}_hsi": Parameter(
            Types.DICT,
            f"all the parameters that specify diagnostic tests and treatments for {p}",
        )
        for p in conditions
    }
    ...
    death_conditions_param_dicts = {
        f"{p}_death": Parameter(
            Types.DICT,
            f"all the parameters that specify the linear models for death from {p}",
        )
        for p in conditions
    }
    ...
    initial_prev_param_dicts = {
        f"{p}_initial_prev": Parameter(Types.DICT, "initial prevalence of condition")
        for p in conditions
    }
    other_params_dict = {
        "interval_between_polls": Parameter(
            Types.INT, "months between the main polling event"
        ),
        "pr_bmi_reduction": Parameter(
            Types.INT,
            "probability of an individual having a reduction in BMI following "
    ...
    PARAMETERS = {
        ...
        **other_params_dict,
        "prob_care_provided_given_seek_emergency_care": Parameter(
            Types.REAL,
            "The probability that correct care is fully provided to persons that have sought emergency care"
    ...
    condition_list = {
        f"nc_{p}": Property(Types.BOOL, f"Whether or not someone has {p}")
        for p in conditions
    }
    condition_diagnosis_list = {
        f"nc_{p}_ever_diagnosed": Property(
            Types.BOOL, f"Whether or not someone has ever been diagnosed with {p}"
        )
        for p in conditions
    }
    condition_date_diagnosis_list = {
        f"nc_{p}_date_diagnosis": Property(
            Types.DATE, f"When someone has been diagnosed with {p}"
        )
        for p in conditions
    }
    condition_date_of_last_test_list = {
        f"nc_{p}_date_last_test": Property(
            Types.DATE, f"When someone has last been tested for {p}"
        )
        for p in conditions
    }
    condition_medication_list = {
        f"nc_{p}_on_medication": Property(
            Types.BOOL, f"Whether or not someone is on medication for {p}"
        )
        for p in conditions
    }
    condition_medication_death_list = {
        f"nc_{p}_medication_prevents_death": Property(
            Types.BOOL,
            f"Whether or not medication (if provided) will " f"prevent death from {p}",
        )
        for p in conditions
    }
    event_list = {
        f"nc_{p}": Property(Types.BOOL, f"Whether or not someone has had a {p}")
        for p in events
    }
    event_date_last_list = {
        f"nc_{p}_date_last_event": Property(Types.DATE, f"Date of last {p}")
        for p in events
    }
    ...
    event_date_diagnosis_list = {
        f"nc_{p}_date_diagnosis": Property(
            Types.DATE, f"When someone has last been diagnosed with {p}"
        )
        for p in events
    }
    ...
    event_scheduled_date_death_list = {
        f"nc_{p}_scheduled_date_death": Property(
            Types.DATE, f"Scheduled date of death from {p}"
        )
        for p in events
    }
    event_medication_death_list = {
        f"nc_{p}_medication_prevents_death": Property(
            Types.BOOL, f"Whether or not medication will prevent death from " f"{p}"
        )
        for p in events
    }
    ...
    PROPERTIES = {
        ...
        **event_medication_death_list,
        **event_scheduled_date_death_list,
        "nc_ever_weight_loss_treatment": Property(
            Types.BOOL,
            "whether or not the person has ever had weight loss " "treatment",
        ),
        "nc_weight_loss_worked": Property(
            Types.BOOL, "whether or not weight loss treatment worked"
        ),
        "nc_risk_score": Property(
            Types.INT, "score to represent number of risk conditions the person has"
        ),
    }
    ...
    def __init__(
        ...
        do_log_df: bool = False,
        do_condition_combos: bool = False,
    ):
        ...
        self.condition_list = [
            "nc_" + cond for cond in CardioMetabolicDisorders.conditions
        ]

        # Store the symptoms that this module will use (for conditions only):
        self.symptoms = {
            f"{s}_symptoms" for s in self.conditions if s != "hypertension"
        }
    ...
    def read_parameters(self, data_folder):
        ...
        cond_onset = pd.read_excel(
            cmd_path / "ResourceFile_cmd_condition_onset.xlsx", sheet_name=None
        )
        cond_removal = pd.read_excel(
            cmd_path / "ResourceFile_cmd_condition_removal.xlsx", sheet_name=None
        )
        cond_death = pd.read_excel(
            cmd_path / "ResourceFile_cmd_condition_death.xlsx", sheet_name=None
        )
        cond_prevalence = pd.read_excel(
            cmd_path / "ResourceFile_cmd_condition_prevalence.xlsx", sheet_name=None
        )
        cond_symptoms = pd.read_excel(
            cmd_path / "ResourceFile_cmd_condition_symptoms.xlsx", sheet_name=None
        )
        cond_hsi = pd.read_excel(
            cmd_path / "ResourceFile_cmd_condition_hsi.xlsx", sheet_name=None
        )
        cond_testing = pd.read_excel(
            cmd_path / "ResourceFile_cmd_condition_testing.xlsx", sheet_name=None
        )
        events_onset = pd.read_excel(
            cmd_path / "ResourceFile_cmd_events.xlsx", sheet_name=None
        )
        events_death = pd.read_excel(
            cmd_path / "ResourceFile_cmd_events_death.xlsx", sheet_name=None
        )
        events_symptoms = pd.read_excel(
            cmd_path / "ResourceFile_cmd_events_symptoms.xlsx", sheet_name=None
        )
        events_hsi = pd.read_excel(
            cmd_path / "ResourceFile_cmd_events_hsi.xlsx", sheet_name=None
        )

        self.load_parameters_from_dataframe(
            pd.read_csv(cmd_path / "ResourceFile_cmd_parameters.csv")
        )
        ...
        for param_name in self.PARAMETERS:
            assert (
                self.parameters[param_name] is not None
            ), f'Parameter "{param_name}" has not been set.'
        ...
        for symptom_name in self.symptoms:
            if symptom_name == "chronic_lower_back_pain_symptoms":
                self.sim.modules["SymptomManager"].register_symptom(
                    Symptom(name=symptom_name, no_healthcareseeking_in_adults=True)
                )
            else:
                self.sim.modules["SymptomManager"].register_symptom(
                    Symptom(name=symptom_name, odds_ratio_health_seeking_in_adults=1.0)
                )
        # Register symptoms from events and make them emergencies
        for event in self.events:
            self.sim.modules["SymptomManager"].register_symptom(
                Symptom.emergency(name=f"{event}_damage", which="adults"),
            )
    ...
    def initialise_population(self, population):
        ...
        def sample_eligible(_filter, _p, _condition):
            ...
            init_prev = self.rng.choice(
                [True, False], size=len(eligible), p=[_p, 1 - _p]
            )
            if sum(init_prev):
        ...
        def sample_eligible_diagnosis_medication(_filter, _p, _condition):
            ...
            using p"""
            eligible = df.index[_filter]
            init_diagnosis = self.rng.choice(
                [True, False], size=len(eligible), p=[_p, 1 - _p]
            )
            if sum(init_diagnosis):
                df.loc[eligible[init_diagnosis], f"nc_{_condition}_ever_diagnosed"] = (
                    True
                )
                df.loc[eligible[init_diagnosis], f"nc_{_condition}_date_diagnosis"] = (
                    self.sim.date
                )
                df.loc[eligible[init_diagnosis], f"nc_{_condition}_date_last_test"] = (
                    self.sim.date
                )
                df.loc[eligible[init_diagnosis], f"nc_{_condition}_on_medication"] = (
                    True
                )
        ...
        def sample_eligible_treatment_success(_filter, _p, _condition):
            ...
            using p"""
            eligible = df.index[_filter]
            init_treatment_works = self.rng.choice(
                [True, False], size=len(eligible), p=[_p, 1 - _p]
            )
            if sum(init_treatment_works):
                df.loc[
                    eligible[init_treatment_works],
                    f"nc_{_condition}_medication_prevents_death",
                ] = True
        ...
        for condition in self.conditions:
            ...
            for _age_range in self.age_cats:
                ...
                sample_eligible(
                    men_wo_cond & (df.age_range == _age_range),
                    p[f"m_{_age_range}"],
                    condition,
                )
                sample_eligible(
                    women_wo_cond & (df.age_range == _age_range),
                    p[f"f_{_age_range}"],
                    condition,
                )
            ...
            p_treatment_works = self.parameters[f"{condition}_hsi"][
                "pr_treatment_works"
            ]
            ...
            for symptom in self.prob_symptoms[condition].keys():
                ...
                has_symptom_at_init = lm_init_symptoms.predict(
        ...
        * Build the LinearModels for the onset/removal of each condition:
        ...
        sim.schedule_event(
            CardioMetabolicDisorders_MainPollingEvent(
                self, self.parameters["interval_between_polls"]
            ),
            sim.date,
        )
        ...
        if "HealthBurden" in self.sim.modules.keys():
            self.daly_wts["daly_diabetes_uncomplicated"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=971)
            self.daly_wts["daly_diabetes_complicated"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=970)
            self.daly_wts["daly_chronic_kidney_disease_moderate"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=978)
            self.daly_wts["daly_chronic_ischemic_hd"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=697)
            self.daly_wts["daly_chronic_lower_back_pain"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=1180)
            self.daly_wts["daly_stroke"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=701)
            self.daly_wts["daly_heart_attack_acute_days_1_2"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=689)
            self.daly_wts["daly_heart_attack_acute_days_3_28"] = self.sim.modules[
                "HealthBurden"
            ].get_daly_weight(sequlae_code=693)
            ...
            self.daly_wts["daly_heart_attack_avg"] = (2 / 28) * self.daly_wts[
                "daly_heart_attack_acute_days_1_2"
            ] + (26 / 28) * self.daly_wts["daly_heart_attack_acute_days_3_28"]

        self.trackers = {
            "onset_condition": Tracker(
                conditions=self.conditions, age_groups=self.age_cats
            ),
            "incident_event": Tracker(conditions=self.events, age_groups=self.age_cats),
            "prevalent_event": Tracker(
                conditions=self.events, age_groups=self.age_cats
            ),
        }
        ...
        for condition in self.conditions:
            self.lms_onset[condition] = self.build_linear_model(
                condition, self.parameters["interval_between_polls"], lm_type="onset"
            )
            self.lms_removal[condition] = self.build_linear_model(
                condition, self.parameters["interval_between_polls"], lm_type="removal"
            )
            self.lms_death[condition] = self.build_linear_model(
                condition, self.parameters["interval_between_polls"], lm_type="death"
            )
            self.lms_symptoms[condition] = self.build_linear_model_symptoms(
                condition, self.parameters["interval_between_polls"]
            )

        # Hypertension is the only condition for which we assume some community-based testing occurs; build LM based on
        # age / sex
        self.lms_testing["hypertension"] = self.build_linear_model(
            "hypertension", self.parameters["interval_between_polls"], lm_type="testing"
        )

        for event in self.events:
            self.lms_event_onset[event] = self.build_linear_model(
                event, self.parameters["interval_between_polls"], lm_type="onset"
            )
            self.lms_event_death[event] = self.build_linear_model(
                event, self.parameters["interval_between_polls"], lm_type="death"
            )
            self.lms_event_symptoms[event] = self.build_linear_model_symptoms(
                event, self.parameters["interval_between_polls"]
            )
        ...
        self.sim.modules["HealthSystem"].dx_manager.register_dx_test(
            assess_diabetes=DxTest(
                ...
                item_codes=self.parameters["diabetes_hsi"]["test_item_code"].astype(
                    int
                ),
            )
        )
        ...
        self.sim.modules["HealthSystem"].dx_manager.register_dx_test(
            assess_hypertension=DxTest(property="nc_hypertension")
        )
        # Create the diagnostic representing the assessment for whether a person is diagnosed with back pain
        self.sim.modules["HealthSystem"].dx_manager.register_dx_test(
            assess_chronic_lower_back_pain=DxTest(property="nc_chronic_lower_back_pain")
        )
        ...
        self.sim.modules["HealthSystem"].dx_manager.register_dx_test(
            assess_chronic_kidney_disease=DxTest(
                ...
                item_codes=self.parameters["chronic_kidney_disease_hsi"][
                    "test_item_code"
                ].astype(int),
            )
        )
    ...
    def build_linear_model(self, condition, interval_between_polls, lm_type):
        ...
        if condition.startswith("ever_") and lm_type == "death":
            ...
            baseline_annual_probability = 1 - math.exp(
                -interval_between_polls / 12 * p["baseline_annual_probability"]
            )
        ...
        linearmodel = LinearModel(
            ...
            Predictor("li_unimproved_sanitation").when(
                True, p["rr_unimproved_sanitation"]
            ),
            Predictor("li_no_access_handwashing").when(
                True, p["rr_no_access_handwashing"]
            ),
            Predictor("li_no_clean_drinking_water").when(
                True, p["rr_no_clean_drinking_water"]
            ),
            ...
            Predictor("nc_chronic_kidney_disease").when(
                True, p["rr_chronic_kidney_disease"]
            ),
            Predictor("nc_chronic_lower_back_pain").when(
                True, p["rr_chronic_lower_back_pain"]
            ),
            Predictor("nc_chronic_ischemic_hd").when(
                True, p["rr_chronic_ischemic_heart_disease"]
            ),
            Predictor("nc_ever_stroke").when(True, p["rr_ever_stroke"]),
            Predictor("nc_ever_heart_attack").when(True, p["rr_ever_heart_attack"]),
            Predictor("nc_diabetes_on_medication").when(
                True, p["rr_diabetes_on_medication"]
            ),
            Predictor("nc_hypertension_on_medication").when(
                True, p["rr_hypertension_on_medication"]
            ),
            Predictor("nc_chronic_lower_back_pain_on_medication").when(
                True, p["rr_chronic_lower_back_pain_on_medication"]
            ),
            Predictor("nc_chronic_kidney_disease_on_medication").when(
                True, p["rr_chronic_kidney_disease_on_medication"]
            ),
            Predictor("nc_chronic_ischemic_hd_on_medication").when(
                True, p["rr_chronic_ischemic_heart_disease_on_medication"]
            ),
            Predictor("nc_ever_stroke_on_medication").when(
                True, p["rr_stroke_on_medication"]
            ),
            Predictor("nc_ever_heart_attack_on_medication").when(
                True, p["rr_heart_attack_on_medication"]
            ),
        )
    ...
    def build_linear_model_symptoms(self, condition, interval_between_polls):
        ...
        for symptom in p.keys():
            p_symptom_onset = 1 - math.exp(
                -interval_between_polls / 12 * p.get(f"{symptom}")
            )
            lms_symptoms_dict[condition][f"{symptom}"] = LinearModel(
                LinearModelType.MULTIPLICATIVE,
                p_symptom_onset,
                Predictor(f"nc_{condition}").when(True, 1.0).otherwise(0.0),
            )
    ...
    def update_risk_score(self):
        ...
        """
        df = self.sim.population.props
        df.loc[df.is_alive, "nc_risk_score"] = (
            df[["li_low_ex", "li_high_salt", "li_high_sugar", "li_tob", "li_ex_alc"]]
            > 0
        ).sum(1)
    ...
    def report_daly_values(self):
        ...
        dw["lower_back_pain"].loc[
            self.sim.modules["SymptomManager"].who_has(
        ...
        dw["chronic_kidney_disease"].loc[
            self.sim.modules["SymptomManager"].who_has(
                "chronic_kidney_disease_symptoms"
            )
        ...
        days_in_last_month = (
            self.sim.date - (self.sim.date - DateOffset(months=1))
        ).days
        start_heart_attack = left_censor(
            df.loc[df.is_alive, "nc_ever_heart_attack_date_last_event"],
            self.sim.date - DateOffset(months=1),
        )
        dur_heart_attack_in_days = (
            (self.sim.date - start_heart_attack)
            .dt.days.clip(lower=0, upper=days_in_last_month)
            .fillna(0.0)
        )
        fraction_of_month_heart_attack = dur_heart_attack_in_days / days_in_last_month
        dw["heart_attack"] = (
            fraction_of_month_heart_attack * self.daly_wts["daly_heart_attack_avg"]
        )
    ...
    def determine_if_will_be_investigated(
        self, person_id: int, person_details: NamedTuple, symptoms: List[str]
    ) -> HSI_Event:
        """
        ...
        has_any_cmd_symptom = (
            False  # Marker for whether the person has any symptoms of interest
        )

        # Determine if there are any conditions that should be investigated:
        for condition in self.conditions:
            is_already_diagnosed = getattr(
                person_details, f"nc_{condition}_ever_diagnosed"
            )
            ...
            next_test_due = is_next_test_due(
                ...
                date_of_last_test=getattr(
                    person_details, f"nc_{condition}_date_last_test"
                ),
            )
        ...
        if conditions_to_investigate:
            return HSI_CardioMetabolicDisorders_Investigations(
                    module=self,
                    person_id=person_id,
    ...
    def determine_if_will_be_investigated_events(self, person_id):
        ...
        for ev in self.events:
            ...
            if f"{ev}_damage" in symptoms and (
                (
                    self.sim.date
                    - self.sim.population.props.at[
                        person_id, f"nc_{ev}_date_last_event"
                    ]
                ).days
                <= 3
            ):
...
class CardioMetabolicDisorders_MainPollingEvent(
    RegularEvent, PopulationScopeEventMixin
):
    ...
    def apply(self, population):
        ...
        for person_id in idx_will_test:
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_CardioMetabolicDisorders_CommunityTestingForHypertension(
                    person_id=person_id, module=self.module
                ),
                priority=1,
                topen=random_date(
                    self.sim.date,
                    self.sim.date + self.frequency - pd.DateOffset(days=2),
                    m.rng,
                ),
                tclose=self.sim.date
                + self.frequency
                - pd.DateOffset(days=1),  # (to occur before next polling)
            )
        ...
        for condition in self.module.conditions:
            ...
            self.module.trackers["onset_condition"].add(
                condition,
                df.loc[idx_acquires_condition].groupby("age_range").size().to_dict(),
            )
            ...
            if len(self.module.lms_symptoms[condition]) > 0:
                symptom_eligible_population = (
                    df.is_alive
                    & df[f"nc_{condition}"]
                    & ~df.index.isin(
                        self.sim.modules["SymptomManager"].who_has(
                            f"{condition}_symptoms"
                        )
                    )
                )
                symptom_onset = self.module.lms_symptoms[condition][
                    f"{condition}_symptoms"
                ].predict(
            ...
            eligible_population = df.is_alive & df[f"nc_{condition}"]
            selected_to_die = self.module.lms_death[condition].predict(
                df.loc[eligible_population], rng, squeeze_single_row_output=False
            )
        ...
        for event in self.module.events:
            ...
            has_event = self.module.lms_event_onset[event].predict(
                df.loc[eligible_population_for_event], rng
            )
            if has_event.any():  # catch in case no one has an event
                idx_has_event = has_event[has_event].index
                for person_id in idx_has_event:
                    self.sim.schedule_event(
                        CardioMetabolicDisordersEvent(self.module, person_id, event),
                        random_date(
                            self.sim.date,
                            self.sim.date + self.frequency - pd.DateOffset(days=1),
                            m.rng,
                    ...
                    if df.at[person_id, f"nc_{event}"]:
                        self.module.trackers["prevalent_event"].add(
                            event, {df.at[person_id, "age_range"]: 1}
                        )
                    else:
                        self.module.trackers["incident_event"].add(
                            event, {df.at[person_id, "age_range"]: 1}
                        )
...
class CardioMetabolicDisordersEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        prob_death = self.module.lms_event_death[self.event].predict(
            df.loc[[person_id]]
        )
        ...
        if self.module.rng.random_sample() < prob_death[person_id]:
            ...
            self.sim.schedule_event(
                CardioMetabolicDisordersDeathEvent(
                    self.module, person_id, originating_cause=self.event
                ),
                date_of_outcome,
            )
...
class CardioMetabolicDisordersDeathEvent(Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id):
        ...
        if person[f"nc_{self.originating_cause}"]:
            ...
            if person[f"nc_{self.originating_cause}_on_medication"]:
                if not df.at[
                    person_id, f"nc_{self.originating_cause}_medication_prevents_death"
                ]:
    ...
    def check_if_event_and_do_death(self, person_id):
        ...
        if f"{self.originating_cause}" in self.module.events:
            if (
                self.sim.date
                == person[f"nc_{self.originating_cause}_scheduled_date_death"]
            ):
...
class CardioMetabolicDisorders_LoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        def age_cats(ages_in_years):
            ...
            _age_cats = pd.Series(
                pd.Categorical(
                    ages_in_years.map(AGE_RANGE_LOOKUP),
        ...
        def proportion_of_something_in_a_groupby_ready_for_logging(
            _df, something, groupbylist
        ):
            dfx = _df.groupby(groupbylist).apply(
                lambda dft: pd.Series(
                    {
                        "something": dft[something].sum(),
                        "not_something": (~dft[something]).sum(),
                    }
                )
            )
            ...
            for _i in range(len(pr)):
                pr.at[_i, "flat_index"] = "__".join(
                    [f"{_col}={pr.at[_i, _col]}" for _col in groupbylist]
                )
        ...
        for condition in self.module.conditions:
            prev_age_sex = proportion_of_something_in_a_groupby_ready_for_logging(
                df, f"nc_{condition}", ["sex", "age_range"]
            )
            ...
            adult_prevalence = {
                "prevalence": len(
                    df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)]
                )
            ...
            if len(df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)]) > 0:
                diagnosed = {
                    f"{condition}_diagnosis_prevalence": len(
                        df[
                            df[f"nc_{condition}_ever_diagnosed"]
                            & df.is_alive
                            & (df.age_years >= 20)
                        ]
                    )
            ...
            if len(df[df[f"nc_{condition}"] & df.is_alive & (df.age_years >= 20)]) > 0:
                on_medication = {
                    f"{condition}_medication_prevalence": len(
                        df[
                            df[f"nc_{condition}_on_medication"]
                            & df.is_alive
                            & (df.age_years >= 20)
                        ]
                    )
        ...
        for event in self.module.events:
            ...
            adult_prevalence = {
                "prevalence": len(
                    df[df[f"nc_{event}"] & df.is_alive & (df.age_years >= 20)]
                )
                / len(df[df.is_alive & (df.age_years >= 20)])
            }
            logger.info(
        ...
        if self.module.do_condition_combos:
            df.loc[df.is_alive, "nc_n_conditions"] = df.loc[
                df.is_alive, self.module.condition_list
            ].sum(axis=1)
            n_comorbidities_all = pd.DataFrame(
                index=self.module.age_cats,
            ...
            for num in range(0, len(self.module.condition_list) + 1):
                col = (
                    df.loc[df["nc_n_conditions"] == num]
                    .groupby(["age_range"])
                    .apply(lambda x: pd.Series({"count": x["nc_n_conditions"].count()}))
                )
                n_comorbidities_all.loc[:, num] = col["count"]

            prop_comorbidities_all = n_comorbidities_all.div(
                n_comorbidities_all.sum(axis=1), axis=0
            )

            logger.info(
                key="mm_prevalence_by_age_all",
            ...
            n_combos = pd.DataFrame(
                index=df["age_range"].value_counts().sort_index().index
            )
            ...
            for i in range(0, len(condition_combos)):
                ...
                col = (
                    df.loc[df.is_alive]
                    .groupby(["age_range"])["nc_condition_combos"]
                    .count()
                )
                n_combos.reset_index()
                n_combos.loc[
                    :, (f"{condition_combos[i][0]}" + "_" + f"{condition_combos[i][1]}")
                ] = col.values

            # output proportions of different combinations of conditions

            prop_combos = n_combos.div(
                df.groupby(["age_range"])["age_range"].count(), axis=0
            )
...
# ---------------------------------------------------------------------------------------------------------
class HSI_CardioMetabolicDisorders_CommunityTestingForHypertension(
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        dx_result = hs.dx_manager.run_dx_test(
            dx_tests_to_run="assess_hypertension", hsi_event=self
        )
        ...
        if dx_result:
            ...
            hs.schedule_hsi_event(
                hsi_event=HSI_CardioMetabolicDisorders_StartWeightLossAndMedication(
                    module=self.module, person_id=person_id, condition="hypertension"
                ),
                priority=0,
                topen=self.sim.date,
                tclose=None,
...
class HSI_CardioMetabolicDisorders_Investigations(HSI_Event, IndividualScopeEventMixin):
    ...
    def __init__(
        ...
        module,
        person_id,
        conditions_to_investigate: List,
        has_any_cmd_symptom: bool = False,
    ):
        super().__init__(module, person_id=person_id)
    ...
    def do_for_each_condition(self, _c) -> bool:
        ...
        # Run a test to diagnose whether the person has condition:
        dx_result = hs.dx_manager.run_dx_test(
            dx_tests_to_run=f"assess_{_c}", hsi_event=self
        )
        ...
        if dx_result:
            ...
            hs.schedule_hsi_event(
                hsi_event=HSI_CardioMetabolicDisorders_StartWeightLossAndMedication(
                    module=self.module, person_id=person_id, condition=f"{_c}"
                ),
                priority=0,
                topen=self.sim.date,
                tclose=None,
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        hsi_scheduled = [
            self.do_for_each_condition(_c) for _c in self.conditions_to_investigate
        ]
        ...
        if (
            ...
            self.sim.schedule_event(
                CardioMetabolicDisordersWeightLossEvent(self.module, person_id),
                random_date(
        ...
        if (
            ...
            and (
                ...
                < self.module.parameters["hypertension_hsi"][
                    "pr_assessed_other_symptoms"
                ]
            )
        ):
...
class HSI_CardioMetabolicDisorders_StartWeightLossAndMedication(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if self.condition != "chronic_kidney_disease" and (
            df.at[person_id, "li_bmi"] > 2
        ):
            self.sim.population.props.at[person_id, "nc_ever_weight_loss_treatment"] = (
                True
            )
        ...
        assert person[f"nc_{self.condition}_ever_diagnosed"], (
            "The person is not diagnosed and so should not be " "receiving an HSI."
        )
        # Check availability of medication for condition
        if self.get_consumables(
            item_codes=self.module.parameters[f"{self.condition}_hsi"]
            .get("medication_item_code")
            .astype(int)
        ):
            ...
            df.at[person_id, f"nc_{self.condition}_medication_prevents_death"] = (
                self.module.rng.rand()
                < self.module.parameters[f"{self.condition}_hsi"].pr_treatment_works
            )
            # Schedule their next HSI for a refill of medication in one month
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                hsi_event=HSI_CardioMetabolicDisorders_Refill_Medication(
                    person_id=person_id, module=self.module, condition=self.condition
                ),
                priority=1,
...
class HSI_CardioMetabolicDisorders_Refill_Medication(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def did_not_run(self):
        ...
        person_id = self.target
        self.sim.population.props.at[
            person_id, f"nc_{self.condition}_on_medication"
        ] = False


class HSI_CardioMetabolicDisorders_SeeksEmergencyCareAndGetsTreatment(
    HSI_Event, IndividualScopeEventMixin
):
    """
    ...
    def do_for_each_event_to_be_investigated(self, _ev):
        ...
        dx_result = self.sim.modules["HealthSystem"].dx_manager.run_dx_test(
            dx_tests_to_run=f"assess_{_ev}", hsi_event=self
        )
        if dx_result:
            # Record date of diagnosis
            ...
            if (
                self.module.parameters["prob_care_provided_given_seek_emergency_care"]
                > self.module.rng.random_sample()
            ):
                # If care is provided....
                if self.get_consumables(
                    item_codes=self.module.parameters[f"{_ev}_hsi"]
                    .get("emergency_medication_item_code")
                    .astype(int)
                ):
                    ...
                    df.at[person_id, f"nc_{_ev}_medication_prevents_death"] = (
                        self.module.rng.rand()
                        < self.module.parameters[f"{_ev}_hsi"].pr_treatment_works
                    )
                    ...
                    logger.debug(
                        key="debug",
                        data="Treatment will not be provided due to no available consumables",
                    )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (
            self.module.rng.rand()
            < self.module.parameters["hypertension_hsi"]["pr_assessed_other_symptoms"]
        ):
            # Run a test to diagnose whether the person has condition:
            dx_result = hs.dx_manager.run_dx_test(
                dx_tests_to_run="assess_hypertension", hsi_event=self
    ...
    def did_not_run(self):
        logger.debug(
            key="debug",
            data="HSI_CardioMetabolicDisorders_SeeksEmergencyCareAndGetsTreatment: did not run",
        )
