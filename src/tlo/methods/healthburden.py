...
class HealthBurden(Module):
    ...
    def read_parameters(self, data_folder):
        ...
        p["Age_Limit_For_YLL"] = (
            70.0  # Assumption that only deaths younger than 70y incur years of lost life
        )
        p["gbd_causes_of_disability"] = set(
            pd.read_csv(
                Path(self.resourcefilepath)
                / "gbd"
                / "ResourceFile_CausesOfDALYS_GBD2019.csv",
                header=None,
            )[0].values
        )
    ...
    def initialise_simulation(self, sim):
        ...
        self.years_life_lost = pd.DataFrame(
            index=self.multi_index_for_age_and_wealth_and_time
        )
        ...
        for module_name in self.recognised_modules_names:
            assert getattr(
                self.sim.modules[module_name], "report_daly_values", None
            ) and callable(self.sim.modules[module_name].report_daly_values), (
    ...
    def process_causes_of_dalys(self):
        ...
        def merge_dicts_of_causes(d1: Dict, d2: Dict) -> Dict:
            ...
            labels_seen = (
                dict()
            )  # Look-up of the form {label: tlo_cause_name} for the tlo_cause_name associated
            #                       (first) with a label.
            merged_causes = (
                dict()
            )  # Dict that will build-up as {tlo_cause_name: Cause} and be returned
            for d in (d1, d2):
                for _tlo_cause_name, _cause in d.items():
                    if _cause.label not in labels_seen:
                        ...
                        merged_causes[_tlo_cause_name] = copy(
                            _cause
                        )  # Use copy to avoid the merged dict being linked
        ...
        all_gbd_causes_of_death_and_disability = set(
            self.parameters["gbd_causes_of_disability"]
        ).union(self.sim.modules["Demography"].gbd_causes_of_death)
    ...
    def get_dalys(self, yld: pd.DataFrame, yll: pd.DataFrame) -> pd.DataFrame:
        ...
        tots = pd.concat(
            [
                yld_with_label.set_index(
                    sorted(
                        [
                            i
                            for i in yld_with_label.columns
                            if i not in self._causes_of_dalys
                        ]
                    )
                ),
                yll_with_label.set_index(
                    sorted(
                        [
                            i
                            for i in yll_with_label.columns
                            if i not in self._causes_of_dalys
                        ]
                    )
                ),
    ...
    def report_live_years_lost(
        ...
        assert self.years_life_lost.index.equals(
            self.multi_index_for_age_and_wealth_and_time
        )
        ...
        yll_stacked_by_age_and_time = (
            pd.DataFrame(yll_stacked_by_time.groupby(level=[0, 2, 3]).sum())
        ...
        self.years_life_lost_stacked_time[cause_of_death] = (
            self.years_life_lost_stacked_time[cause_of_death].add(
                yll_stacked_by_time, fill_value=0
            )
        )
        self.years_life_lost_stacked_age_and_time[cause_of_death] = (
            self.years_life_lost_stacked_age_and_time[cause_of_death].add(
    ...
    def write_to_log(self, year: int):
        ...
        def summarise_results_for_this_year(df, level=[0, 1]) -> pd.DataFrame:
            ...
            """
            return (
                df.loc[(slice(None), slice(None), slice(None), year)]
                .groupby(level=level)
                .sum()
                .reset_index()
                .assign(year=year)
        ...
        def log_df_line_by_line(key, description, df, force_cols=None) -> None:
            ...
            df[sorted(set(force_cols) - set(df.columns))] = (
                0.0  # Force the addition of any missing causes
            )
            df = df[
                sorted(df.columns)
            ]  # sort the columns so that they are always in same order
        ...
        log_df_line_by_line(
            ...
            df=(
                yld := summarise_results_for_this_year(self.years_lived_with_disability)
            ),
        ...
        log_df_line_by_line(
            ...
            df=(
                yll_stacked_by_time := summarise_results_for_this_year(
        ...
        log_df_line_by_line(
            ...
            df=(
                yll_stacked_by_age_and_time := summarise_results_for_this_year(
...
class Get_Current_DALYS(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        if (not self.module.recognised_modules_names) or (
            not self.module.causes_of_disability
        ):
            return
        ...
        for disease_module_name in self.module.recognised_modules_names:
            ...
            declared_causes_of_disability_module = (
                disease_module.CAUSES_OF_DISABILITY.keys()
            )
            ...
            if declared_causes_of_disability_module:
                ...
                if isinstance(dalys_from_disease_module, pd.Series):
                    ...
                    dalys_from_disease_module.name = list(
                        declared_causes_of_disability_module
                    )[0]
                ...
                assert set(dalys_from_disease_module.columns) == set(
                    declared_causes_of_disability_module
                )
                ...
                assert (
                    (
                        (dalys_from_disease_module >= 0)
                        & (dalys_from_disease_module <= 1)
                    )
                    .all()
                    .all()
                )
        ...
        disease_specific_daly_values_this_month = (
            pd.concat(dalys_from_each_disease_module, axis=1)
        ...
        disease_specific_daly_values_this_month = (
            disease_specific_daly_values_this_month.multiply(scaling_factor, axis=0)
        )
        assert (
            (disease_specific_daly_values_this_month.sum(axis=1) - 1.0) < 1e-6
        ).all()
        ...
        disease_specific_daly_values_this_month = (
            disease_specific_daly_values_this_month.merge(
                df.loc[idx_alive, ["sex", "li_wealth", "age_range"]],
                left_index=True,
                right_index=True,
                how="left",
            )
        ...
        dalys_current = (
            self.module.years_lived_with_disability.sum().sum()
        )  # for checking
        ...
        self.module.years_lived_with_disability = pd.DataFrame(
            index=self.module.multi_index_for_age_and_wealth_and_time
        ...
        assert self.module.years_lived_with_disability.index.equals(
            self.module.multi_index_for_age_and_wealth_and_time
        )
        assert (
            abs(
                self.module.years_lived_with_disability.sum().sum()
...
class Healthburden_WriteToLog(RegularEvent, PopulationScopeEventMixin):
    ...
    def __init__(self, module):
        super().__init__(
            module, frequency=DateOffset(years=1), priority=Priority.END_OF_DAY
        )
