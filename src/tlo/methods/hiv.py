...
class Hiv(Module):
    ...
    def initialise_simulation(self, sim):
        ...
        logger.debug('HSI_Hiv_PopulationWideBehaviourChange has been scheduled successfully!')
...
class HivEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        # ----------------------------------- PROGRESSION TO SYMPTOMATIC -----------------------------------
        df.loc[newly_infected_index, 'hv_proj_date_symp'] = df.loc[newly_infected_index,
        ...
        # ----------------------------------- PROGRESSION TO AIDS -----------------------------------
        df.loc[newly_infected_index, 'hv_proj_date_aids'] = df.loc[newly_infected_index,
...
class HSI_Hiv_PresentsForCareWithSymptoms(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if is_cons_available:
            logger.debug('HSI_Hiv_PresentsForCareWithSymptoms: all consumables available')
...
class HSI_Hiv_InfantScreening(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        logger.debug('HSI_Hiv_InfantScreening: a first appointment for infant %d', person_id)
        ...
        if is_cons_available:
            ...
            if df.at[person_id, 'hv_inf']:
                ...
                # request treatment
                logger.debug('HSI_Hiv_InfantScreening: scheduling hiv treatment for person %d on date %s',
                             person_id, self.sim.date)
...
class HSI_Hiv_PopulationWideBehaviourChange(HSI_Event, PopulationScopeEventMixin):
    ...
    def apply(self, population, squeeze_factor):
        ...
        self.module.parameters['hv_behav_mod'] = self.module.parameters['hv_behav_mod'] * 0.9
...
class HSI_Hiv_StartInfantProphylaxis(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        item_code3 = pd.unique(consumables.loc[consumables[
                                                   'Items'] == 'Cotrimoxazole preventive therapy for TB HIV+ patients', 'Item_Code'])[
            0]
    ...
    def apply(self, population):
        ...
        if len(df[df.is_alive & (df.hv_on_art == 2)]) > 1:
            ...
            if len(poor):
                ...
                for person in poor:
                    logger.debug('This is ArtGoodToPoorAdherenceEvent: transitioning to poor adherence for person %d',
                                 person)
                    ...
                    if df.at[person, 'hv_specific_symptoms'] == 'none':
                        df.at[person, 'hv_proj_date_symp'] = df.at[person,
                                                                   'hv_proj_date_death'] - DateOffset(days=732.5)
                    ...
                    if df.at[person, 'hv_specific_symptoms'] != 'aids':
                        df.at[person, 'hv_proj_date_aids'] = df.at[person,
                                                                   'hv_proj_date_death'] - DateOffset(days=365.25)
...
class HivLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        logger.info('%s|hiv_adult_prev_m|%s', self.sim.date,
                    m_adult_prev_out.to_dict())

        logger.info('%s|hiv_adult_prev_f|%s', self.sim.date,
                    f_adult_prev_out.to_dict())
