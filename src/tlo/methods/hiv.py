...
class Hiv(Module, GenericFirstAppointmentsMixin):
    ...
    def initialise_baseline_art(self, population):
        ...
        for person in art_idx:
            days = self.rng.randint(low=1, high=self.parameters['initial_dispensation_period_months'] * 30.5,
                                    dtype=np.int64)
    ...
    def initialise_simulation(self, sim):
        ...
        * 7) Look-up and save the codes for consumables
        ...
        # 1) Schedule the Main HIV Regular Polling Event
        sim.schedule_event(
            HivRegularPollingEvent(self), sim.date + DateOffset(years=1)
        )
    ...
    def update_parameters_for_program_scaleup(self):
        ...
        if p['type_of_scaleup'] == 'target_VL':
            # this is enacted within the HSI_Start_Or_Continue_Treatment
            # and limits VL to young people and/or pregnant women
            return
...
class HivLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        df = population.props
        now = self.sim.date
        # todo several loggers clear the counters so check when commenting / uncommenting
        ...
        #         "n_fsw": n_fsw,
        #         "prop_fsw_on_prep_oral": prop_fsw_on_prep_oral,
        #         "prop_fsw_on_prep_inj": prop_fsw_on_prep_inj,
