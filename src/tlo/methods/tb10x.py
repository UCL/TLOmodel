...
class Tb(Module):
    ...
    def initialise_simulation(self, sim):
        ...
        * 3) Define the DxTests and treatment options
        ...
        sim.schedule_event(ScenarioSetupEvent(self), self.parameters["outreach_xray_start_date"])

        # 4) Define the DxTests and get the consumables required
...
class HSI_Tb_ScreeningAndRefer(HSI_Event, IndividualScopeEventMixin):
    ...
    * give IPT for paediatric contacts of diagnosed case
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        person_has_tb_symptoms = any(symptom in persons_symptoms for symptom in self.module.symptom_list)

        #if not any(x in self.module.symptom_list for x in persons_symptoms):
        if not person_has_tb_symptoms:
...
class TbCommunityXray(RegularEvent, PopulationScopeEventMixin):
    ...
    * Excludes people with a current diagnosis of TB
    ...
    def apply(self, population):
        ...
        logger.debug(key="message", data=f"Selecting persons for community chest x-ray screening")
        print("DEBUG: Entering TbCommunityXray apply method", flush=True)

        prob_screening = p['probability_community_chest_xray']
        print(f"DEBUG: Probability of community screening: {prob_screening}")
        ...
        if select_for_screening.sum():
            ...
            for person_id in screen_idx:
                ...
                if any(x in self.module.symptom_list for x in persons_symptoms):
                    print(f"DEBUG: Community CXR scheduled for person {person_id} due to TB symptoms.")
                    self.sim.modules["HealthSystem"].schedule_hsi_event(
...
class HSI_Tb_CommunityXray(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        print("DEBUG: Entering HSI_Tb_CommunityXray apply method")
        ...
        print(f"DEBUG: Test result for {person_id}: {test_result}")
