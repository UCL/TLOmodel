...
class CervicalCancer(Module, GenericFirstAppointmentsMixin):
    ...
    INIT_DEPENDENCIES = {'Demography', 'HealthSystem', 'Lifestyle', 'SymptomManager', 'Hiv', 'HPV'}
    ...
    PARAMETERS = {
        "init_prev_cervical_states_hiv": Parameter(
            Types.LIST,
            "Initial probabilities of cervical lesion/cancer states among women with HIV"
        ),
        "init_prev_cervical_states_nhiv": Parameter(
            Types.LIST,
            "Initial probabilities of cervical lesion/cancer states among women without HIV"
        ),

        "r_cin1_persistent_base": Parameter(
            Types.REAL,
            "Monthly probability of incident CIN1 attributable to one persistent reference-group (hr6) HPV infection",
        ),
        "rr_cin1_persistent_hr1": Parameter(Types.REAL, "Relative CIN1 onset hazard for persistent hr1 vs hr6"),
        "rr_cin1_persistent_hr2": Parameter(Types.REAL, "Relative CIN1 onset hazard for persistent hr2 vs hr6"),
        "rr_cin1_persistent_hr3": Parameter(Types.REAL, "Relative CIN1 onset hazard for persistent hr3 vs hr6"),
        "rr_cin1_persistent_hr4": Parameter(Types.REAL, "Relative CIN1 onset hazard for persistent hr4 vs hr6"),
        "rr_cin1_persistent_hr5": Parameter(Types.REAL, "Relative CIN1 onset hazard for persistent hr5 vs hr6"),

        "prob_revert_from_cin1": Parameter(
            Types.REAL,
            "probability of reverting from cin1 to none",
        ),
        "prob_revert_from_cin2": Parameter(
            Types.REAL,
            "monthly probability of spontaneous regression from CIN2 to CIN1",
        ),
        ...
        "rr_progress_cc_hiv": Parameter(
            Types.REAL, "hazard ratio for CIN lesion progression with unsuppressed HIV"
        ),
        ...
        "stage4_survival_median_months": Parameter(
            Types.REAL,
            "Median survival time (months) after entry into stage 4; used by Weibull time-to-death model",
        ),
        "stage4_survival_weibull_shape": Parameter(
            Types.REAL,
            "Weibull shape parameter for survival time after entry into stage 4",
        ),
        ...
        "sensitivity_of_xpert_for_current_hrhpv": Parameter(
            Types.REAL, "Sensitivity of Xpert HPV for detecting current high-risk HPV infection"
        ),
        "specificity_of_xpert_for_current_hrhpv": Parameter(
            Types.REAL, "Specificity of Xpert HPV for excluding current high-risk HPV infection"
        ),
    ...
    PROPERTIES = {
        "ce_hpv_cc_status": Property(
            Types.CATEGORICAL,
            "Current cervical lesion or invasive-cancer status",
            categories=["none", "cin1", "cin2", "cin3", "stage1", "stage2a", "stage2b", "stage3", "stage4"],
        ),
        "ce_cc_driver": Property(
            Types.CATEGORICAL,
            "HPV high-risk group attributed as the driver of the current cervical lesion episode",
            categories=["none", "hr1", "hr2", "hr3", "hr4", "hr5", "hr6"],
        ),
    ...
    EXPECTED_HPV_GROUPS = ("hr1", "hr2", "hr3", "hr4", "hr5", "hr6")
    def __init__(self, name=None, resourcefilepath=None):
        super().__init__(name)
        self.resourcefilepath = resourcefilepath
        self.polling_event = None

        self.cin_states = ["cin1", "cin2", "cin3",]
        self.cancer_states = ["stage1", "stage2a", "stage2b", "stage3", "stage4",]
    ...
    def _get_hpv_module(self):
        """Return HPV module and validate the public interface required by CervicalCancer."""
        if "HPV" not in self.sim.modules:
            raise RuntimeError("CervicalCancer requires the HPV module.")

        hpv_module = self.sim.modules["HPV"]
        expected = set(self.EXPECTED_HPV_GROUPS)
        actual = set(getattr(hpv_module, "HPV_GROUPS", []))
        if actual != expected:
            raise RuntimeError(
                f"CervicalCancer expects HPV groups {sorted(expected)}, "
                f"but HPV exposes {sorted(actual)}."
            )

        required_methods = (
            "get_current_hpv_matrix",
            "get_any_current_hpv",
            "get_persistent_hpv_matrix",
            "get_any_persistent_hpv",
        )
        missing_methods = [
            method_name for method_name in required_methods
            if not callable(getattr(hpv_module, method_name, None))
        ]
        if missing_methods:
            raise RuntimeError(
                "HPV public interface is incomplete. "
                f"Missing methods: {missing_methods}"
            )
        return hpv_module

    def _validate_natural_history_configuration(self):

        hpv_module = self._get_hpv_module()
        p = self.parameters

        if float(p["main_polling_frequency_months"]) != 1.0:
            raise ValueError(
                "main_polling_frequency_months must equal 1 because all cervical "
                "natural-history probabilities are monthly."
            )

        monthly_probabilities = (
            "r_cin1_persistent_base",
            "prob_revert_from_cin1",
            "prob_revert_from_cin2",
            "r_cin2_cin1",
            "r_cin3_cin2",
            "r_stage1_cin3",
            "r_stage2a_stage1",
        ...
        for parameter_name in monthly_probabilities:
            value = float(p[parameter_name])
            if not np.isfinite(value) or not (0.0 <= value < 1.0):
                raise ValueError(
                    f"{parameter_name} must be a finite monthly probability in [0, 1)."
                )

        relative_risks = ["rr_progress_cc_hiv"] + [
            f"rr_cin1_persistent_{group}" for group in hpv_module.HPV_GROUPS if group != "hr6"
        ]
        for parameter_name in relative_risks:
            value = float(p[parameter_name])
            if not np.isfinite(value) or value < 0.0:
                raise ValueError(f"{parameter_name} must be finite and >= 0.")

        median_months = float(p["stage4_survival_median_months"])
        weibull_shape = float(p["stage4_survival_weibull_shape"])
        if not np.isfinite(median_months) or median_months <= 0.0:
            raise ValueError("stage4_survival_median_months must be finite and > 0.")
        if not np.isfinite(weibull_shape) or weibull_shape <= 0.0:
            raise ValueError("stage4_survival_weibull_shape must be finite and > 0.")

    def _set_post_resolution_status(self, person_ids):
        """Resolve a cervical lesion/cancer episode without clearing HPV infection."""
        df = self.sim.population.props
        if np.isscalar(person_ids):
            idx = pd.Index([person_ids])
        else:
            idx = pd.Index(person_ids)
        if len(idx) == 0:
            return

        df.loc[idx, "ce_hpv_cc_status"] = "none"
        df.loc[idx, "ce_cc_driver"] = "none"

    def _get_cin1_group_rr(self, hpv_module=None):
        if hpv_module is None:
            hpv_module = self._get_hpv_module()
        p = self.parameters
        return pd.Series(
            {
                "hr1": float(p["rr_cin1_persistent_hr1"]),
                "hr2": float(p["rr_cin1_persistent_hr2"]),
                "hr3": float(p["rr_cin1_persistent_hr3"]),
                "hr4": float(p["rr_cin1_persistent_hr4"]),
                "hr5": float(p["rr_cin1_persistent_hr5"]),
                "hr6": 1.0,
            },
            dtype=float,
        ).reindex(hpv_module.HPV_GROUPS)

    def _initialise_baseline_cc_drivers(self):
        """Assign a driver HPV group to prevalent CIN/cancer at simulation baseline.

        UPDATED v2:
        * If one or more persistent HPV groups are present, sample the driver among
          those groups in proportion to their CIN1 cause-specific hazard weights.
        * If no persistent HPV is currently present, assign hr1-hr6 uniformly.

        The second rule is an explicit provisional modelling assumption requested
        for the baseline joint HPV/lesion initialisation. It attributes the lesion
        episode but does NOT create a current HPV infection.
        """
        df = self.sim.population.props
        hpv_module = self._get_hpv_module()
        lesion_states = set(self.cin_states + self.cancer_states)
        idx = df.index[
            df.is_alive
            & (df.sex == "F")
            & df.ce_hpv_cc_status.isin(lesion_states)
        ]
        if len(idx) == 0:
            return

        persistent = hpv_module.get_persistent_hpv_matrix(index=idx)
        group_rr = self._get_cin1_group_rr(hpv_module)
        all_groups = np.asarray(hpv_module.HPV_GROUPS, dtype=object)

        for person_id in idx:
            persistent_groups = persistent.columns[
                persistent.loc[person_id].to_numpy(dtype=bool)
            ].to_numpy(dtype=object)

            if len(persistent_groups) > 0:
                weights = group_rr.loc[persistent_groups].to_numpy(dtype=float)
                if weights.sum() > 0.0:
                    weights = weights / weights.sum()
                else:
                    weights = np.repeat(1.0 / len(persistent_groups), len(persistent_groups))
                driver = self.rng.choice(persistent_groups, p=weights)
            else:
                # PROVISIONAL BASELINE ASSUMPTION: uniform attribution when the
                # prevalent lesion has no currently persistent hrHPV group.
                driver = self.rng.choice(all_groups)

            df.at[person_id, "ce_cc_driver"] = str(driver)

    def _sample_stage4_survival_days(self):
        """Sample individual survival after entry into Stage IV from a Weibull distribution."""
        p = self.parameters
        median_months = float(p["stage4_survival_median_months"])
        shape = float(p["stage4_survival_weibull_shape"])
        scale_months = median_months / (np.log(2.0) ** (1.0 / shape))
        survival_months = scale_months * float(self.rng.weibull(shape))
        return max(1, int(round(survival_months * DAYS_IN_YEAR / 12.0)))

    def _schedule_stage4_death(self, person_id):
        """Schedule one individual cervical-cancer death after entry into Stage IV."""
        death_date = self.sim.date + DateOffset(days=self._sample_stage4_survival_days())
        self.sim.schedule_event(
            CervicalCancer_DeathInStage4(self, person_id),
            death_date,
        )

    def _apply_persistent_hpv_to_cin1(self, eligible_none_at_start=None):
        """Apply group-specific cause-specific hazards for incident CIN1.

        Each current persistent HPV group contributes an independent cause-specific hazard.
        If CIN1 occurs, ce_cc_driver is sampled in proportion to those hazards.
        """
        df = self.sim.population.props
        p = self.parameters
        rng = self.rng

        if eligible_none_at_start is None:
            idx = df.index[
                df.is_alive
                & (df.sex == "F")
            ...
            idx = pd.Index(eligible_none_at_start)
            if len(idx) > 0:
                still_eligible = (
                ...
                idx = idx[still_eligible.to_numpy()]
        ...
        hpv_module = self._get_hpv_module()
        persistent = hpv_module.get_persistent_hpv_matrix(index=idx)
        has_persistent_hpv = persistent.any(axis=1)
        idx = idx[has_persistent_hpv.to_numpy()]
        if len(idx) == 0:
            return
        persistent = persistent.loc[idx]

        base_probability = float(p["r_cin1_persistent_base"])
        if base_probability <= 0.0:
            return
        base_hazard = -np.log1p(-base_probability)
        group_rr = self._get_cin1_group_rr(hpv_module)

        cause_specific_hazards = persistent.astype(float).mul(group_rr, axis=1) * base_hazard
        total_hazard = cause_specific_hazards.sum(axis=1)
        probability_cin1 = 1.0 - np.exp(-total_hazard)
        progresses = pd.Series(rng.random(size=len(idx)), index=idx) < probability_cin1
        progressed_idx = progresses.index[progresses]

        for person_id in progressed_idx:
            person_hazards = cause_specific_hazards.loc[person_id]
            person_hazards = person_hazards[person_hazards > 0.0]
            if person_hazards.empty:
                continue
            driver_probabilities = (person_hazards / person_hazards.sum()).to_numpy(dtype=float)
            driver = rng.choice(person_hazards.index.to_numpy(), p=driver_probabilities)
            df.at[person_id, "ce_hpv_cc_status"] = "cin1"
            df.at[person_id, "ce_cc_driver"] = str(driver)


    def _apply_cin1_competing_transitions(self, cin1_at_start):
        """Apply mutually exclusive monthly CIN1 -> CIN2 / regression transitions."""
        df = self.sim.population.props
        p = self.parameters
        rng = self.rng
        if len(cin1_at_start) == 0:
            return

        idx = pd.Index(cin1_at_start)
        eligible = (
            df.loc[idx, "is_alive"].fillna(False).astype(bool)
            & (df.loc[idx, "sex"] == "F")
            & (df.loc[idx, "ce_hpv_cc_status"] == "cin1")
        )
        idx = idx[eligible.to_numpy()]
        if len(idx) == 0:
            return

        p_progress_base = float(p["r_cin2_cin1"])
        p_regress_base = float(p["prob_revert_from_cin1"])
        for parameter_name, probability in (
            ("r_cin2_cin1", p_progress_base),
            ("prob_revert_from_cin1", p_regress_base),
        ):
            if not np.isfinite(probability) or not (0.0 <= probability < 1.0):
                raise ValueError(f"{parameter_name} must be a finite monthly probability in [0, 1).")

        progression_hazard = pd.Series(-np.log1p(-p_progress_base), index=idx, dtype=float)
        regression_hazard = pd.Series(-np.log1p(-p_regress_base), index=idx, dtype=float)

        hiv_unsuppressed = (
            df.loc[idx, "hv_inf"].fillna(False).astype(bool)
            & df.loc[idx, "hv_art"].isin(["not", "on_not_VL_suppressed"])
        )
        progression_hazard.loc[hiv_unsuppressed] *= float(p["rr_progress_cc_hiv"])

        total_hazard = progression_hazard + regression_hazard
        probability_any_transition = 1.0 - np.exp(-total_hazard)
        transition_occurs = pd.Series(rng.random(size=len(idx)), index=idx) < probability_any_transition
        transition_idx = transition_occurs.index[transition_occurs]
        if len(transition_idx) == 0:
            return

        probability_progress_given_transition = (
            progression_hazard.loc[transition_idx] / total_hazard.loc[transition_idx]
        )
        progresses = (
            pd.Series(rng.random(size=len(transition_idx)), index=transition_idx)
            < probability_progress_given_transition
        )
        progresses_idx = progresses.index[progresses]
        regresses_idx = progresses.index[~progresses]

        if len(progresses_idx) > 0:
            # Preserve ce_cc_driver through the lesion episode.
            df.loc[progresses_idx, "ce_hpv_cc_status"] = "cin2"
        if len(regresses_idx) > 0:
            self._set_post_resolution_status(regresses_idx)

    def _apply_cin2_competing_transitions(self, cin2_at_start):
        """Apply mutually exclusive monthly CIN2 -> CIN3 / CIN1 regression transitions.

        UPDATED v2: CIN2 spontaneous regression is explicitly represented. HIV
        modifies the progression hazard only; the regression hazard is unchanged.
        """
        df = self.sim.population.props
        p = self.parameters
        rng = self.rng
        if len(cin2_at_start) == 0:
            return

        idx = pd.Index(cin2_at_start)
        eligible = (
            df.loc[idx, "is_alive"].fillna(False).astype(bool)
            & (df.loc[idx, "sex"] == "F")
            & (df.loc[idx, "ce_hpv_cc_status"] == "cin2")
        )
        idx = idx[eligible.to_numpy()]
        if len(idx) == 0:
            return

        p_progress_base = float(p["r_cin3_cin2"])
        p_regress_base = float(p["prob_revert_from_cin2"])
        ...
        if len(progresses_idx) > 0:
            # Preserve the lesion driver through progression.
            df.loc[progresses_idx, "ce_hpv_cc_status"] = "cin3"
        if len(regresses_idx) > 0:
            # CIN2 -> CIN1 remains within the same lesion episode; retain the driver.
            df.loc[regresses_idx, "ce_hpv_cc_status"] = "cin1"

    @staticmethod
    def _prepare_initial_cervical_lesion_distribution(raw_probabilities, parameter_name):
        """Validate final 9-category baseline cervical lesion/cancer probabilities."""
        categories = [
            "none", "cin1", "cin2", "cin3", "stage1", "stage2a", "stage2b", "stage3", "stage4"
        ]
        probabilities = np.asarray(raw_probabilities, dtype=float)
        if probabilities.shape != (len(categories),):
            raise ValueError(f"{parameter_name} must contain {len(categories)} probabilities.")
        if not np.all(np.isfinite(probabilities)):
            raise ValueError(f"{parameter_name} contains non-finite values.")
        if np.any(probabilities < 0.0):
            raise ValueError(f"{parameter_name} contains negative probabilities.")
        if not np.isclose(probabilities.sum(), 1.0):
            raise ValueError(
                f"{parameter_name} must sum to 1.0, but sums to {probabilities.sum()}."
            )
        return categories, probabilities

    def _run_xpert_hpv_test(self, person_id):
        """Simulate Xpert HPV using CURRENT hrHPV truth from the HPV module."""
        hpv_module = self._get_hpv_module()
        current_hpv = hpv_module.get_any_current_hpv(index=[person_id])
        true_hpv_positive = bool(current_hpv.loc[person_id])

        sensitivity = float(self.parameters["sensitivity_of_xpert_for_current_hrhpv"])
        specificity = float(self.parameters["specificity_of_xpert_for_current_hrhpv"])
        for parameter_name, value in (
            ("sensitivity_of_xpert_for_current_hrhpv", sensitivity),
            ("specificity_of_xpert_for_current_hrhpv", specificity),
        ):
            if not np.isfinite(value) or not (0.0 <= value <= 1.0):
                raise ValueError(f"{parameter_name} must be a finite probability in [0, 1].")

        random_draw = self.rng.random()
        if true_hpv_positive:
            return bool(random_draw < sensitivity)
        return bool(random_draw < (1.0 - specificity))
    ...
    def initialise_population(self, population):
        ...
        df.loc[df.is_alive, "ce_cc_driver"] = "none"
        ...
        lesion_categories, probs_nhiv = self._prepare_initial_cervical_lesion_distribution(
            p["init_prev_cervical_states_nhiv"], "init_prev_cervical_states_nhiv"
        )
        _, probs_hiv = self._prepare_initial_cervical_lesion_distribution(
            p["init_prev_cervical_states_hiv"], "init_prev_cervical_states_hiv"
        )

        alive_female_15plus = (
            df.is_alive & (df["sex"] == "F") & (df["age_years"] >= 15)
        )
        hiv_positive = df["hv_inf"].fillna(False).astype(bool)
        women_15plus_nhiv_idx = df.index[alive_female_15plus & (~hiv_positive)]
        women_15plus_hiv_idx = df.index[alive_female_15plus & hiv_positive]

        if len(women_15plus_nhiv_idx) > 0:
            df.loc[women_15plus_nhiv_idx, "ce_hpv_cc_status"] = rng.choice(
                lesion_categories, size=len(women_15plus_nhiv_idx), p=probs_nhiv
            )
        if len(women_15plus_hiv_idx) > 0:
            df.loc[women_15plus_hiv_idx, "ce_hpv_cc_status"] = rng.choice(
                lesion_categories, size=len(women_15plus_hiv_idx), p=probs_hiv
            )
    ...
    def initialise_simulation(self, sim):
        """Schedule CervicalCancer events, diagnostics and disability weights."""

        p = self.parameters
        sim = self.sim

        self._validate_natural_history_configuration()
        self._initialise_baseline_cc_drivers()
        ...
        baseline_stage4_idx = self.sim.population.props.index[
            self.sim.population.props.is_alive
            & (self.sim.population.props.ce_hpv_cc_status == "stage4")
        ]
        for person_id in baseline_stage4_idx:
            self._schedule_stage4_death(person_id)
    ...
    def build_linear_models(self) -> None:
        ...
        expected_lm_labels = {'stage1', 'stage2a', 'stage2b', 'stage3', 'stage4'}
        if set(lm) != expected_lm_labels:
            raise RuntimeError(
                "Unexpected cervical progression LinearModel configuration. "
                f"Expected {sorted(expected_lm_labels)}, found {sorted(lm)}."
            )

        # the exhaustive vaginal-bleeding predictor at the no-cancer background rate.
        ...
        self.lm_onset_vaginal_bleeding = LinearModel.multiplicative(
            Predictor('sex', conditions_are_mutually_exclusive=True, conditions_are_exhaustive=True)
                .when('M', 0.0).when('F', 1.0),
            Predictor(
                'ce_hpv_cc_status',
    ...
    def on_birth(self, mother_id, child_id):
        ...
        df.at[child_id, "ce_cc_driver"] = "none"
...
class CervicalCancerMainPollingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    * Deaths from cervical cancer for those in stage4
    ...
    def apply(self, population):
        ...
        none_at_start = df.index[
            df.is_alive & (df.sex == 'F') & (df.ce_hpv_cc_status == 'none')
        ]
        cin1_at_start = df.index[
            df.is_alive & (df.sex == 'F') & (df.ce_hpv_cc_status == 'cin1')
        ]
        cin2_at_start = df.index[
            df.is_alive & (df.sex == 'F') & (df.ce_hpv_cc_status == 'cin2')
        ]
        stage4_at_start = set(df.index[
            df.is_alive & (df.ce_hpv_cc_status == 'stage4')
        ])

        for stage, lm in reversed(list(m.linear_models_for_progression_of_hpv_cc_status.items())):
        ...
        m._apply_cin2_competing_transitions(cin2_at_start)
        m._apply_cin1_competing_transitions(cin1_at_start)
        m._apply_persistent_hpv_to_cin1(eligible_none_at_start=none_at_start)
        current_stage4 = set(df.index[df.is_alive & (df.ce_hpv_cc_status == 'stage4')])
        for person_id in (current_stage4 - stage4_at_start):
            m._schedule_stage4_death(person_id)
...
class HSI_CervicalCancer_Screening_Xpert(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if cons_avail:
            ...
            dx_result = self.module._run_xpert_hpv_test(person_id)

            df.loc[person_id, [
                "ce_date_last_screened", "ce_date_xpert", "ce_ever_screened"
            ]] = [self.sim.date, self.sim.date, True]
...
class HSI_CervicalCancer_CINRemoval_Cryotherapy(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if cons_avail:
            ...
            if current_status in self.module.cin_states:
                ...
                if rng.random_sample() <= p['prob_cryotherapy_successful']:
                    ...
                    self.module._set_post_resolution_status(person_id)

            elif current_status in self.module.cancer_states and not df.at[person_id, "ce_current_cc_diagnosed"]:
                hs.schedule_hsi_event(
                    hsi_event=HSI_CervicalCancer_Biopsy(module=self.module, person_id=person_id),
                    priority=0,
                    topen=self.sim.date,
                    tclose=None,
                )
...
class HSI_CervicalCancer_Biopsy(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if not df.at[person_id, "is_alive"] or df.at[person_id, "ce_current_cc_diagnosed"]:
        ...
        if cons_avail:
            ...
            # Biopsy does not show cancer, but the woman has CIN:
            # proceed to CIN treatment.
            if (
                (not dx_result)
                and current_status in self.module.cin_states
            ):
                self.module.choose_cin_procedure_and_schedule_it(person_id)


            # Biopsy detects cancer:
            # proceed through the cancer diagnosis/treatment pathway.
            elif (
                dx_result
                and current_status in self.module.cancer_states
            ):
                # Record date of diagnosis
                if pd.isnull(df.at[person_id, "ce_date_diagnosis"]):
                    df.at[person_id, "ce_date_diagnosis"] = self.sim.date
                    df.at[person_id, "ce_stage_at_diagnosis"] = current_status
...
class HSI_CervicalCancer_Treatment_Start(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "ce_hpv_cc_status"] not in ['stage1', 'stage2a', 'stage2b', 'stage3']:
            return self.make_appt_footprint({})

        if pd.notnull(df.at[person_id, "ce_date_treatment"]):
            return self.make_appt_footprint({})

        if (
            not df.at[person_id, "is_alive"]
            or not df.at[person_id, "ce_current_cc_diagnosed"]
        ):
            return self.make_appt_footprint({})
...
class HSI_CervicalCancer_PalliativeCare(HSI_Event, IndividualScopeEventMixin):
    ...
    * Itself for the continuance of care.
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if not df.at[person_id, "is_alive"] or df.at[person_id, "ce_hpv_cc_status"] != 'stage4':
...
class CervicalCancerLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        current_lesion = df_alive_females.loc[
            df_alive_females.ce_hpv_cc_status.isin(
                ['cin1', 'cin2', 'cin3', 'stage1', 'stage2a', 'stage2b', 'stage3', 'stage4']
            )
        ]
        out.update({
            f'current_lesion_driver_{k}': int(v)
            for k, v in current_lesion.ce_cc_driver.value_counts().items()
        })
        ...
        rate_diagnosed_cc = (
            n_diagnosed_past_year / n_women_alive
            if n_women_alive > 0
            else np.nan
        )

        living_with_diagnosed_cc = (
            df['is_alive']
            & df['ce_current_cc_diagnosed']
            & df['ce_hpv_cc_status'].isin(self.module.cancer_states)
        )

        n_women_living_with_diagnosed_cc = living_with_diagnosed_cc.sum()

        n_women_living_with_diagnosed_cc_age_lt_30 = (
            living_with_diagnosed_cc & (df['age_years'] < 30)
        ).sum()

        n_women_living_with_diagnosed_cc_age_3050 = (
            living_with_diagnosed_cc
            & df['age_years'].between(30, 49, inclusive='both')
        ).sum()

        n_women_living_with_diagnosed_cc_age_gt_50 = (
            living_with_diagnosed_cc & (df['age_years'] >= 50)
        ).sum()
