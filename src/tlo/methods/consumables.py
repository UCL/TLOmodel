...
class Consumables:
    ...
    def __init__(self, data: pd.DataFrame = None, rng: np.random = None, availability: str = 'default') -> None:
        ...
        self.is_unknown_item_available = None  # Whether an unknown item is available, by facility_id

        self.__process_consumables_df(data)
    ...
    def processing_at_start_of_new_day(self, date: datetime.datetime) -> None:
        ...
        * Update the availability of the consumables
        """
        self.__refresh_availability_of_consumables(date)

    def __process_consumables_df(self, df: pd.DataFrame) -> None:
        """Helper function for processing the consumables data.
    ...
    def __refresh_availability_of_consumables(self, date: datetime.datetime):
        ...
        self.is_unknown_item_available = (average_availability_of_items_by_facility_id >
                                          self.rng.random_sample(len(average_availability_of_items_by_facility_id))
                                          ).to_dict()

    @staticmethod
    def __determine_default_return_value(cons_availability, default_return_value):
    ...
    def __request_consumables(self, facility_id: int, item_codes: dict, to_log: bool = True,
        ...
        if self.cons_availability == 'all':
            ...
            available = self.__lookup_availability_of_consumables(item_codes=item_codes, facility_id=facility_id)
        ...
        if to_log:
            logger.info(key='Consumables',
                        data={
                            ...
                            # N.B. Casting the data to strings because logger complains with dict of varying sizes/keys.
                        },
    ...
    def __lookup_availability_of_consumables(self, facility_id: int, item_codes: dict) -> dict:
        """Return dict indicating whether each item_code is in the set of currently available items for a facility_id
         (in `self.is_available`). If any code is not recognised, the value of `is_unknown_item_available` is used."""
        avail = dict()
        for _i in item_codes.keys():
            if _i in self.item_codes:
                avail.update({_i: _i in self.is_available[facility_id]})
            else:
                avail.update({_i: self.is_unknown_item_available[facility_id]})
        return avail

    @staticmethod
    def __get_item_codes_from_package_name(lookup_df: pd.DataFrame, package: str) -> dict:
    ...
    @staticmethod
    def __get_item_code_from_item_name(lookup_df: pd.DataFrame, item: str) -> int:
