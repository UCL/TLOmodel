...
class Consumables:
    ...
    def _update_prob_item_codes_available(self, availability: str):
        ...
        if availability not in alt_scenarios:
            ...
            # select column for the given scenario. Take copy in case it's changed later on
            self._prob_item_codes_available = self._processed_consumables_data[f'available_prop_{availability}'].copy()
