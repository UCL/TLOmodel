...
class Epi(Module):
    ...
    PARAMETERS = {
        ...
        "prob_facility_level_for_vaccine": Parameter(
            Types.LIST,
    ...
    PROPERTIES = {
        ...
        "va_hep": Property(
            Types.INT, "number of doses of HepB vaccine received (infant series)"
        ),
        "va_pneumo": Property(
        ...
        "va_measles": Property(
            Types.INT, "number of doses of measles vaccine received"
        ),
        "va_rubella": Property(
            Types.INT, "number of doses of rubella vaccine received"
        ),
        ...
        "va_bcg_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the vaccine bcg"
        ),
        "va_opv_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the OPV vaccine"
        ),
        "va_dtp_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the DTP vaccine"
        ),
        "va_hib_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the Hib vaccine"
        ),
        "va_hep_all_doses": Property(
            Types.BOOL,
            "whether all doses have been received of the HepB vaccine (infant series)",
        ),
        "va_pneumo_all_doses": Property(
            Types.BOOL,
            "whether all doses have been received of the pneumococcal vaccine",
        ),
        "va_rota_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the rotavirus vaccine"
        ),
        "va_measles_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the measles vaccine"
        ),
        "va_rubella_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the rubella vaccine"
        ),
        "va_hpv_all_doses": Property(
            Types.BOOL, "whether all doses have been received of the HPV vaccine"
        ),
        "va_td_all_doses": Property(
            Types.BOOL,
    ...
    def __init__(self, name=None, resourcefilepath=None):
        ...
        self.all_doses = dict()
        self.cons_item_codes = (
            dict()
        )  # (will store dict giving item_codes for each vaccine)
    ...
    def read_parameters(self, data_folder):
        ...
        p["vaccine_schedule"] = workbook["vaccine_schedule"].set_index("vaccine")[
            "date_administration_days"
        ]
    ...
    def initialise_population(self, population):
        ...
        df.loc[
            df.is_alive & (random_draw < df_vaccine_baseline["MCV1"]), "va_measles"
        ] = 1

        # update the 'all_doses' properties for initial population
        for vaccine, max_dose in self.all_doses.items():
            df.loc[df.is_alive, f"va_{vaccine}_all_doses"] = (
                df.loc[df.is_alive, f"va_{vaccine}"] >= max_dose
            )
    ...
    def on_birth(self, mother_id, child_id):
        ...
        facility_level_for_vaccines = self.rng.choice(
            facility_levels, p=self.parameters["prob_facility_level_for_vaccine"]
        )

        for each_vax in vax_schedule:
            vax_hsi_event, admin_schedule = each_vax
            for admin_key in admin_schedule:
                vax_event_instance = vax_hsi_event(
                    self,
                    person_id=child_id,
    ...
    def get_item_codes(self):
        ...
        get_item_code_from_item_name = self.sim.modules[
            "HealthSystem"
        ].get_item_code_from_item_name

        self.cons_item_codes["bcg"] = get_item_code_from_item_name(
        ...
        self.cons_item_codes["pneumo"] = get_item_code_from_item_name(
            "Pneumococcal vaccine"
        )
        self.cons_item_codes["measles_and_rubella"] = get_item_code_from_item_name(
            "Measles vaccine"
        )
        self.cons_item_codes["hpv"] = get_item_code_from_item_name("HPV vaccine")
        self.cons_item_codes["td"] = get_item_code_from_item_name(
...
class HpvScheduleEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        logger.debug(
            key="debug",
            data="HpvScheduleEvent selecting eligible 9-yr olds for HPV vaccine",
        )
        ...
        for index, person_id in enumerate(hpv_vax):
            logger.debug(
                key="debug",
                data=f"HpvScheduleEvent scheduling HPV vaccine for {person_id}",
            )
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "va_bcg"] < self.module.all_doses["bcg"]:
            if self.get_consumables(
                item_codes=self.module.cons_item_codes["bcg"],
...
class HSI_opv(HsiBaseVaccine):
    ...
    def apply(self, person_id, squeeze_factor):
        if self.get_consumables(
            item_codes=self.module.cons_item_codes["opv"],
            optional_item_codes=self.module.cons_item_codes["syringes"],
        ):
...
class HSI_DtpHibHepVaccine(HsiBaseVaccine):
    ...
    def apply(self, person_id, squeeze_factor):
        if self.get_consumables(
            item_codes=self.module.cons_item_codes["pentavalent_vaccine"],
            optional_item_codes=self.module.cons_item_codes["syringes"],
        ):
...
class HSI_RotaVaccine(HsiBaseVaccine):
    ...
    def apply(self, person_id, squeeze_factor):
        logger.debug(
            key="debug", data=f"HSI_RotaVaccine: requesting vaccines for {person_id}"
        )
        ...
        if df.at[person_id, "va_rota"] < self.module.all_doses["rota"]:
            if self.get_consumables(
                item_codes=self.module.cons_item_codes["rota"],
                optional_item_codes=self.module.cons_item_codes["syringes"],
            ):
...
class HSI_PneumoVaccine(HsiBaseVaccine):
    ...
    def apply(self, person_id, squeeze_factor):
        if self.get_consumables(
            item_codes=self.module.cons_item_codes["pneumo"],
            optional_item_codes=self.module.cons_item_codes["syringes"],
        ):
...
class HSI_MeaslesRubellaVaccine(HsiBaseVaccine):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (df.at[person_id, "va_measles"] == 0) or (
            ...
            if self.get_consumables(
                item_codes=self.module.cons_item_codes["measles_and_rubella"],
                optional_item_codes=self.module.cons_item_codes["syringes"],
            ):
...
class HSI_HpvVaccine(HsiBaseVaccine):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if df.at[person_id, "va_hpv"] < self.module.all_doses["hpv"]:
            if self.get_consumables(
                item_codes=self.module.cons_item_codes["hpv"],
                optional_item_codes=self.module.cons_item_codes["syringes"],
            ):
...
class HSI_TdVaccine(HsiBaseVaccine):
    ...
    def apply(self, person_id, squeeze_factor):
        if self.get_consumables(
            item_codes=self.module.cons_item_codes["td"],
            optional_item_codes=self.module.cons_item_codes["syringes"],
        ):
...
class EpiLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        from_14_weeks_to_one = (
            df.is_alive & (df.age_years <= 1) & (df.age_exact_years >= 0.27)
        )
        ...
        from_15_months_to_two = (
            df.is_alive & (df.age_exact_years >= 1.25) & (df.age_years <= 2)
        )
        ...
        girls_from_ten_to_twelve = (
            df.is_alive
            & (df.sex == "F")
            & (df.age_exact_years >= 10)
            & (df.age_years <= 12)
        )
