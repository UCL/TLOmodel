...
class Malaria(Module):
    ...
    def read_parameters(self, data_folder):
        workbook = pd.read_excel(
            self.resourcefilepath / "ResourceFile_malaria.xlsx", sheet_name=None
        )
        ...
        p["inf_inc"] = pd.read_csv(
            self.resourcefilepath / "ResourceFile_malaria_InfInc_expanded.csv"
        )
        p["clin_inc"] = pd.read_csv(
            self.resourcefilepath / "ResourceFile_malaria_ClinInc_expanded.csv"
        )
        p["sev_inc"] = pd.read_csv(
            self.resourcefilepath / "ResourceFile_malaria_SevInc_expanded.csv"
        )
    ...
    def check_if_fever_is_caused_by_malaria(self, person_id, hsi_event):
        ...
        dx_result = self.sim.modules["HealthSystem"].dx_manager.run_dx_test(
            dx_tests_to_run="malaria_rdt", hsi_event=hsi_event
        )
        ...
        true_malaria_infection_type = self.sim.population.props.at[
    ...
    def do_for_suspected_malaria_case(self, person_id, hsi_event):
        ...
        if df.at[person_id, "ma_tx"] == "none":
            ...
            if malaria_test_result == "severe_malaria":
                ...
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    HSI_Malaria_Treatment_Complicated(person_id=person_id, module=self),
                    priority=0,
                    topen=self.sim.date,
                    tclose=None,
                )
            ...
            # return type 'clinical_malaria' includes asymptomatic infection
                ...
                df.at[person_id, "ma_dx_counter"] += 1
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    HSI_Malaria_Treatment(person_id=person_id, module=self),
                    priority=1,
                    topen=self.sim.date,
                    tclose=None,
                )
