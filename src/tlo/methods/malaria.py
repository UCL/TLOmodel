...
class Malaria(Module, GenericFirstAppointmentsMixin):
    ...
    INIT_DEPENDENCIES = {"Contraception", "Demography", "HealthSystem", "SymptomManager"}
    ...
    PARAMETERS = {
        ...
        "dur_clin": Parameter(Types.REAL, "duration (days) of clinical symptoms of malaria"),
        "dur_clin_para": Parameter(Types.REAL, "duration (days) of parasitaemia for clinical malaria cases"),
        ...
        "itn_proj": Parameter(Types.REAL, "coverage of ITN for projections 2020 onwards"),
        "mortality_adjust": Parameter(Types.REAL, "adjustment of case-fatality rate to match WHO/MAP"),
        "data_end": Parameter(Types.REAL, "final year of ICL malaria model outputs, after 2018 = projections"),
        "irs_rates_boundary": Parameter(Types.REAL, "threshold for indoor residual spraying coverage"),
        "irs_rates_upper": Parameter(Types.REAL, "indoor residual spraying high coverage"),
        "irs_rates_lower": Parameter(Types.REAL, "indoor residual spraying low coverage"),
        "prob_malaria_case_tests": Parameter(Types.REAL, "probability that a malaria case will have a scheduled rdt"),
        ...
        "duration_iptp_protection_weeks": Parameter(
            Types.REAL, "duration of protection against clinical malaria conferred by each dose of IPTp"
        ),
        "rr_clinical_malaria_hiv_under5": Parameter(
            Types.REAL, "relative risk of clinical malaria if HIV+ and aged under 5 years"
        ),
        "rr_clinical_malaria_hiv_over5": Parameter(
            Types.REAL, "relative risk of clinical malaria if HIV+ and aged over 5 years"
        ),
        ...
        "rr_clinical_malaria_art": Parameter(
            Types.REAL, "relative risk of clinical malaria if HIV+ and on ART and virally suppressed"
        ),
        "rr_clinical_malaria_iptp": Parameter(Types.REAL, "relative risk of clinical malaria with each dose of IPTp"),
        ...
        "rr_severe_malaria_iptp": Parameter(Types.REAL, "relative risk of severe malaria with each dose of IPTp"),
        "prob_of_treatment_success": Parameter(Types.REAL, "probability that treatment will clear malaria symptoms"),
    ...
    PROPERTIES = {
        ...
        "ma_date_symptoms": Property(Types.DATE, "Date of symptom start for clinical infection"),
        ...
        "ma_date_tx": Property(Types.DATE, "Date treatment started for most recent malaria episode"),
        "ma_inf_type": Property(
            Types.CATEGORICAL,
            "specific symptoms with malaria infection",
            categories=["none", "asym", "clinical", "severe"],
        ),
        "ma_age_edited": Property(Types.REAL, "age values redefined to match with malaria data"),
        "ma_clinical_counter": Property(Types.INT, "annual counter for malaria clinical episodes"),
        "ma_dx_counter": Property(Types.INT, "annual counter for malaria diagnoses"),
        "ma_tx_counter": Property(Types.INT, "annual counter for malaria treatment episodes"),
    ...
    def read_parameters(self, data_folder):
        ...
        self.itn_irs = (
            itn_irs.reset_index()
            .assign(District_Num=lambda x: x["District"].map(mapper_district_name_to_num))
    ...
    def _build_linear_models(self):
        ...
        conditional_predictors = (
            [
                Predictor().when(
                    "(hv_inf == True) & (age_years <= 5) & (is_pregnant == False)", p["rr_clinical_malaria_hiv_under5"]
                ),
                Predictor().when(
                    "(hv_inf == True) & (age_years > 5) & (is_pregnant == False)", p["rr_clinical_malaria_hiv_over5"]
                ),
                Predictor().when("(hv_inf == True) & (is_pregnant == True)", p["rr_clinical_malaria_hiv_pregnant"]),
                # treatment effects
        ...
        self.lm["rr_of_clinical_malaria"] = LinearModel.multiplicative(*(predictors + conditional_predictors))
        ...
        conditional_predictors = (
            [
                Predictor().when(
                    "(hv_inf == True) & (age_years <= 5) & (is_pregnant == False)", p["rr_severe_malaria_hiv_under5"]
                ),
                Predictor().when(
                    "(hv_inf == True) & (age_years > 5) & (is_pregnant == False)", p["rr_severe_malaria_hiv_over5"]
                ),
                Predictor().when("(hv_inf == True) & (is_pregnant == True)", p["rr_severe_malaria_hiv_pregnant"]),
            ]
            if "Hiv" in self.sim.modules
            else []
        )

        self.lm["rr_of_severe_malaria"] = LinearModel.multiplicative(*(predictors + conditional_predictors))
    ...
    def malaria_poll2(self, population):
        ...
        for person in death:
            logger.debug(key="message", data=f"MalariaEvent: scheduling malaria death for person {person}")
            ...
            death_event = MalariaDeathEvent(self, person_id=person, cause="Malaria")  # make that death event
    ...
    def update_parameters_for_program_scaleup(self):
        ...
        mask = self.itn_irs["irs_rate"].index.get_level_values("District_Num").isin(highrisk_distr_num)
    ...
    def do_at_generic_first_appt(
        ...
        **kwargs,
        ...
        if bool(set(symptoms) & malaria_associated_symptoms) and individual_properties["ma_tx"] == "none":
...
class MalariaIPTp(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        p1_condition = (
            ...
            & ~df.ma_is_infected
            & ~df.ma_iptp
            & (~df.hv_on_cotrimoxazole if "Hiv" in self.sim.modules else True)
        )

        p1 = df.index[p1_condition]

        for person_index in p1:
            logger.debug(key="message", data=f"MalariaIPTp: scheduling HSI_Malaria_IPTp for person {person_index}")

            event = HSI_MalariaIPTp(self.module, person_id=person_index)
            self.sim.modules["HealthSystem"].schedule_hsi_event(event, priority=1, topen=now, tclose=None)
...
class HSI_Malaria_rdt(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        logger.debug(
            key="message", data=f"HSI_Malaria_rdt: rdt test for person {person_id} " f"in district num {district}"
        )

        # call the DxTest RDT to diagnose malaria
        dx_result = hs.dx_manager.run_dx_test(dx_tests_to_run="malaria_rdt", hsi_event=self)
        ...
        if dx_result:
            ...
            if df.at[person_id, "ma_inf_type"] == "severe":
                ...
                treat = HSI_Malaria_Treatment_Complicated(self.sim.modules["Malaria"], person_id=person_id)
                self.sim.modules["HealthSystem"].schedule_hsi_event(treat, priority=0, topen=self.sim.date, tclose=None)
            ...
            # this will allow those with asym malaria (positive RDT) to also be treated
                ...
                self.sim.modules["HealthSystem"].schedule_hsi_event(treat, priority=1, topen=self.sim.date, tclose=None)
...
class HSI_Malaria_Treatment(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if person["ma_tx"] == "none":
            logger.debug(key="message", data=f"HSI_Malaria_Treatment: requesting malaria treatment for {person_id}")

            # Check if drugs are available, and provide drugs:
            drugs_available = self.get_drugs(age_of_person=person["age_years"])

            if drugs_available:
                logger.debug(key="message", data=f"HSI_Malaria_Treatment: giving malaria treatment for {person_id}")
    ...
    def get_drugs(self, age_of_person):
        ...
        if age_of_person < 5:
            ...
            drugs_available = self.get_consumables(
                ...
                optional_item_codes={
                    self.module.item_codes_for_consumables_required["paracetamol_syrup"]: 120,
            ...
            drugs_available = self.get_consumables(
                ...
                optional_item_codes={
                    self.module.item_codes_for_consumables_required["paracetamol_syrup"]: 180,
                    self.module.item_codes_for_consumables_required["malaria_rdt"]: 1,
                },
            )
            ...
            drugs_available = self.get_consumables(
                ...
                optional_item_codes={
                    self.module.item_codes_for_consumables_required["paracetamol"]: 12_000,
                    self.module.item_codes_for_consumables_required["malaria_rdt"]: 1,
                },
            )
...
class HSI_Malaria_Treatment_Complicated(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        if (df.at[person_id, "ma_tx"] == "none") and df.at[person_id, "is_alive"]:
            ...
            if self.get_consumables(
                ...
                # Add used equipment
                self.add_equipment({"Drip stand", "Haemoglobinometer", "Analyser, Combined Chemistry and Electrolytes"})
    ...
    def did_not_run(self):
        logger.debug(key="message", data="HSI_Malaria_Treatment_Complicated: did not run")
...
class HSI_MalariaIPTp(HSI_Event, IndividualScopeEventMixin):
    ...
    def apply(self, person_id, squeeze_factor):
        ...
        logger.debug(key="message", data=f"HSI_MalariaIPTp: requesting IPTp for person {person_id}")
        ...
        if self.get_consumables(self.module.item_codes_for_consumables_required["malaria_iptp"]):
            logger.debug(key="message", data=f"HSI_MalariaIPTp: giving IPTp for person {person_id}")
...
class MalariaUpdateEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        * clears parasites if treated
        ...
        new_symptomatic_severe = df.loc[df.is_alive & (df.ma_inf_type == "severe") & (df.ma_date_symptoms == now)].index
        ...
        for idx in eligible_for_rdt[selected_for_rdt]:
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                ...
                priority=1,
                topen=random_date(now + DateOffset(days=1), now + DateOffset(days=4), self.module.rng),
                tclose=None,
            )
        ...
        self.sim.modules["SymptomManager"].clear_symptoms(person_id=infections_to_clear, disease_module=self.module)
        ...
        clinical_not_treated = df.index[
            df.is_alive & (df.ma_inf_type == "clinical") & (df.ma_date_symptoms < (self.sim.date - DateOffset(days=6)))
        ]

        self.sim.modules["SymptomManager"].clear_symptoms(person_id=clinical_not_treated, disease_module=self.module)
...
class MalariaLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        tmp2 = len(
            df.loc[(df.age_years.between(2, 10)) & (df.ma_date_symptoms > (now - DateOffset(months=self.repeat)))]
        )
        ...
        logger.info(key="incidence", data=summary, description="Summary of incident malaria cases")
        ...
        logger.info(key="status_counts", data=counts, description="Running counts of incident malaria cases")

        # ------------------------------------ PARASITE PREVALENCE BY AGE ------------------------------------

        # includes all parasite positive cases: some may have low parasitaemia (undetectable)
        child2_10_inf = len(df[df.is_alive & (df.ma_inf_type != "none") & (df.age_years.between(2, 10))])
        ...
        if "Hiv" in self.sim.modules:
            ...
            coinfection_num = len(df[df.is_alive & (df.ma_inf_type != "none") & df.hv_inf])
            ...
            logger.info(
                key="coinfection_prevalence", data=coinfection_prevalence, description="Co-infection prevalence"
            )
...
class MalariaTxLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        logger.info(key="tx_coverage", data=treatment, description="Treatment of malaria cases")

        # reset all counters
        logger.debug(key="message", data=f"Resetting the malaria counter {self.sim.date}")
...
class MalariaPrevDistrictLoggingEvent(RegularEvent, PopulationScopeEventMixin):
    ...
    def apply(self, population):
        ...
        infected = df.district_num_of_residence[df.is_alive & df.ma_is_infected].value_counts()
        ...
        logger.info(key="prev_district", data=prev_ed.to_dict(), description="District estimates of malaria prevalence")

        logger.info(key="pop_district", data=pop.to_dict(), description="District population sizes")
