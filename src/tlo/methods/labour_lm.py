...
def predict_eclampsia_death(self, df, rng=None, **externals):
    ...
    if (
        person["la_maternal_hypertension_treatment"]
        or person["ac_iv_anti_htn_treatment"]
    ):
        treatment_effect = pregnancy_helper_functions.get_treatment_effect(
            externals["delay_one_two"],
            externals["delay_three"],
...
def predict_postpartum_haem_pp_death(self, df, rng=None, **externals):
    ...
    person = df.iloc[0]
    params = self.module.current_parameters
    treatment = self.module.pph_treatment.to_strings(
        person.la_postpartum_haem_treatment
    )
    result = params["cfr_pp_pph"]

    if ("surgery" in treatment) or (
        "hysterectomy" in treatment
    ):  # todo: replace bitset property with bool?
        treatment_effect = pregnancy_helper_functions.get_treatment_effect(
            externals["delay_one_two"],
            externals["delay_three"],
...
def predict_probability_delivery_health_centre(self, df, rng=None, **externals):
    ...
    result[(df.age_years > 19) & (df.age_years < 25)] *= params[
        "rrr_hc_delivery_age_20_24"
    ]
    result[(df.age_years > 24) & (df.age_years < 30)] *= params[
        "rrr_hc_delivery_age_25_29"
    ]
    result[(df.age_years > 29) & (df.age_years < 35)] *= params[
        "rrr_hc_delivery_age_30_34"
    ]
    result[(df.age_years > 34) & (df.age_years < 40)] *= params[
        "rrr_hc_delivery_age_35_39"
    ]
    result[(df.age_years > 39) & (df.age_years < 45)] *= params[
        "rrr_hc_delivery_age_40_44"
    ]
    result[(df.age_years > 44) & (df.age_years < 50)] *= params[
    ...
    result[(df.la_parity > 2) & (df.la_parity < 5)] *= params[
...
def predict_postnatal_check(self, df, rng=None, **externals):
    ...
    result[externals["mode_of_delivery"] == "caesarean_section"] *= params[
        "or_pnc_caesarean_delivery"
    ]
    result[
        (externals["delivery_setting"] == "health_centre")
        | (externals["delivery_setting"] == "hospital")
