...
def get_hsi_event_classes_per_module(
    ...
    for _, module_name, _ in pkgutil.iter_modules([methods_package_path]):
        ...
        tlo_module_classes = [obj for _, obj in inspect.getmembers(module) if is_valid_tlo_module_subclass(obj, {})]
        hsi_event_classes = [
            obj
...
def sort_hsi_event_details(set_of_hsi_event_details: Iterable[HSIEventDetails]) -> List[HSIEventDetails]:
...
def _format_appt_footprint(appt_footprint, inline_code_formatter):
    return ", ".join(f"{inline_code_formatter(appt_type)}" for appt_type, _ in appt_footprint)


def _format_beddays_footprint(beddays_footprint, inline_code_formatter):
    return ", ".join(
        f"{inline_code_formatter(bedtype)} ({days} days)" for bedtype, days in beddays_footprint if days > 0
    )


def _format_treatment_id(treatment_id, module_name, inline_code_formatter):
    prefixes = [f"{module_name}_", f"HSI_{module_name}_", f"{module_name[0].lower()}{module_name[1:]}_"]
...
def format_hsi_event_details_as_csv(hsi_event_details: Iterable[HSIEventDetails]) -> str:
    """Format HSI event details list as comma-separated value string."""
    with io.StringIO(newline="") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(
            ["Module", "Event", "Treatment", "Facility level", "Appointment footprint", "Bed days footprint"]
        )
        for event_details in hsi_event_details:
            writer.writerow(
                [
                    ...
                    _format_beddays_footprint(event_details.beddays_footprint, lambda s: s),
                ]
            )
        return csv_file.getvalue()


def format_hsi_event_details_as_table(hsi_event_details: Iterable[HSIEventDetails], text_format: str = "rst") -> str:
    ...
    for event_details in hsi_event_details:
        table_string += formatters["table_row"](
            [
                ...
                _format_treatment_id(event_details.treatment_id, event_details.module_name, formatters["inline_code"]),
                _format_facility_level(event_details.facility_level),
                _format_appt_footprint(event_details.appt_footprint, formatters["inline_code"]),
                _format_beddays_footprint(event_details.beddays_footprint, formatters["inline_code"]),
            ]
        )
    return table_string


def format_hsi_event_details_as_list(hsi_event_details: Iterable[HSIEventDetails], text_format: str = "rst") -> str:
    ...
    for module_name, module_event_details in event_details_by_module.items():
        ...
        for event_details in module_event_details:
            appt_footprint_string = _format_appt_footprint(event_details.appt_footprint, formatters["inline_code"])
            treatment_id_string = _format_treatment_id(
                event_details.treatment_id, event_details.module_name, formatters["inline_code"]
            )
...
def merge_hsi_event_details(
    ...
    for event_details in inspect_hsi_event_details:
        if not (
            ...
            and without_facility_level(event_details) in run_hsi_event_details_without_facility_level
        ):
...
def _parse_command_line_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate formatted description of HSI events.")
    parser.add_argument(
        "--json-file",
        type=Path,
        help=(
...
def main():
    ...
    if args.json_file is not None:
        ...
        json_hsi_event_details = set(
            HSIEventDetails(**{key: recursive_list_to_tuple(value) for key, value in event_details.items()})
            for event_details in hsi_event_details
        )
    ...
    if args.merge_json_details:
        hsi_event_details = merge_hsi_event_details(inspect_hsi_event_details, json_hsi_event_details)
