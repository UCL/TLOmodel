...
def unflatten_flattened_multi_index_in_logging(_x: pd.DataFrame) -> pd.DataFrame:
    """Helper function that recreate the multi-index of logged results from a pd.DataFrame generated by `parse_log`.
    If a pd.Series with a multi-index was transformed before logging using
    `flatten_multi_index_series_into_dict_for_logging` then those flattened labels will be the columns of the
    pd.DataFrame generated by `parse_log`. This converts those columns into the original multi-index from which the
    flattened labels were created.
    """
    cols = _x.columns
