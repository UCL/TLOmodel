...
def get_class_output_string(classinfo):
    ...
    for name, obj in classdat:
        ...
        elif (ismodule(obj) or isclass(obj) or ismethod(obj) or istraceback(obj) or isframe(obj) or iscode(obj)) and (
    ...
    if func_objects_to_document:
        mystr += f"{spacer}**Functions (defined or overridden in " f"class {class_name}):**\n\n"
    ...
    for func_name, func_obj in class_functions.items():
        ...
        for baseclass in clazz.__mro__:
            ...
            if baseclass != clazz:
                ...
                functions_base_class = dict(inspect.getmembers(baseclass, predicate=inspect.isfunction))
