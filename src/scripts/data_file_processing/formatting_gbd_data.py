...
gbd["Age_Grp"] = gbd["Age_Grp_GBD"].str.replace("1-4", "0-4").str.replace("<1year", "0-4")
...
cod = (
    cod.loc[cod["Year"] == latest_year].groupby(by=["Sex", "Age_Grp", "cause_name"], as_index=False)[["GBD_Est"]].sum()
)
...
causes_of_disability = (
    gbd.loc[lambda df: df["measure_name"] == "DALYs (Disability-Adjusted Life Years)"]["cause_name"].unique().tolist()
)
pd.Series(causes_of_disability).to_csv(
    path_for_saved_files / "ResourceFile_CausesOfDALYS_GBD2019.csv", index=False, header=False
)
