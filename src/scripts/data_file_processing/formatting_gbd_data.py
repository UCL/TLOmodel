...
gbd["Age_Grp"] = (
    gbd["Age_Grp_GBD"].str.replace("1-4", "0-4").str.replace("<1year", "0-4")
)
...
# Rename the 'variants'
gbd = gbd.rename(columns={"val": "GBD_Est", "upper": "GBD_Upper", "lower": "GBD_Lower"})
...
gbd.to_csv(
    path_for_saved_files / "ResourceFile_Deaths_And_DALYS_GBD2019.csv", index=False
)
...
gbd_deaths = gbd_deaths.groupby(by=["Year", "Sex", "Age_Grp"], as_index=False)[
    ["GBD_Est", "GBD_Lower", "GBD_Upper"]
].sum()
gbd_deaths = gbd_deaths.melt(
    id_vars=["Year", "Sex", "Age_Grp"], var_name="Variant", value_name="Count"
)
gbd_deaths.to_csv(
    path_for_saved_files / "ResourceFile_TotalDeaths_GBD2019.csv", index=False
)
...
cod = (
    cod.loc[cod["Year"] == latest_year]
    .groupby(by=["Sex", "Age_Grp", "cause_name"], as_index=False)[["GBD_Est"]]
    .sum()
)
cod = cod.pivot(
    index=["Sex", "Age_Grp"], columns="cause_name", values="GBD_Est"
).fillna(0)
...
prop_deaths.reset_index().to_csv(
    path_for_saved_files / "ResourceFile_CausesOfDeath_GBD2019.csv", index=False
)

# %% Make: ResourceFile_CausesOfDALYS_GBD2019
causes_of_disability = (
    gbd.loc[lambda df: df["measure_name"] == "DALYs (Disability-Adjusted Life Years)"][
        "cause_name"
    ]
    .unique()
    .tolist()
)
pd.Series(causes_of_disability).to_csv(
    path_for_saved_files / "ResourceFile_CausesOfDALYS_GBD2019.csv",
    index=False,
    header=False,
)
