...
district_nums = pd.DataFrame(data={"Region": a1["Region"], "District_Num": np.arange(len(a1["Region"]))})
...
a7 = pd.read_excel(
    workingfile_popsizes, sheet_name="A7", usecols=[0] + list(range(2, 10)) + list(range(12, 21)), header=1, index_col=0
)
...
table.rename(
    columns={"district": "District", "age_grp": "Age_Grp_Special", "sex": "Sex", "number": "Count"}, inplace=True
)
...
# Re-order columns
table = table[["Variant", "District", "District_Num", "Region", "Year", "Period", "Age_Grp", "Sex", "Count"]]
...
from_pop_data = (
    table.loc[
        (table["Sex"] == "F") & (table["Age_Grp"].isin(["15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"]))
    ]
...
for Sex in ["M", "F"]:
    for Age in range(101):
        for District in district_nums.index:
            ...
            record = {"District": District, "Sex": Sex, "Age": Age, "Count": tot_agesex_pop_this_district}
...
for year in range(1950, 2100):
    frac_this_year = frac_birth_male.loc[
        (int(year) >= frac_birth_male["low_year"]) & (int(year) <= frac_birth_male["high_year"]), "frac_births_male"
    ].values[0]
...
lt_males = pd.concat(
    [
        pd.read_excel(lt_males_file, sheet_name="ESTIMATES", header=16, usecols="B,C,H,I,J,K"),
...
lt_females = pd.concat(
    [
        pd.read_excel(lt_females_file, sheet_name="ESTIMATES", header=16, usecols="B,C,H,I,J,K"),
...
lt["Age_Grp"] = (
    lt["Age (x)"].astype(int).astype(str) + "-" + (lt["Age (x)"] + lt["Age interval (n)"] - 1).astype(int).astype(str)
)
...
mort_sched_expanded = pd.DataFrame(
    mort_sched_expanded_as_list, columns=["fallbackyear", "sex", "age_years", "death_rate"]
)
