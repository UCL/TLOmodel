...
assert (
    females.sum(axis=1).astype("float32") == a1["Female_2018"].astype("float32")
).all()
females["district"] = females.index
females_melt = females.melt(
    id_vars=["district"], var_name="age_grp", value_name="number"
)
females_melt["sex"] = "F"
...
table["Count_By_Age_Grp"] = table.groupby(by=["Age_Grp", "District", "Sex"])[
    "Count"
].transform("sum")
table = table.drop_duplicates(subset=["Age_Grp", "District", "Sex"])
table = table.rename(
    columns={"Count_By_Age_Grp": "Count", "Count": "Count_By_Age_Grp_Special"}
)
...
# Save
table.to_csv(
    path_for_saved_files / "ResourceFile_PopulationSize_2018Census.csv", index=False
)
...
b1 = b1.rename(
    columns={
        b1.columns[0]: "Age/Region",
        b1.columns[1]: "Num_Women_1549",
        b1.columns[2]: "Live_Births",
        b1.columns[3]: "Babies_Live_12m",
        b1.columns[4]: "Babies_Dead_12m",
    }
)
...
# save the file:
b1[["Variant", "Year", "Period", "Region", "Count"]].to_csv(
    path_for_saved_files / "ResourceFile_Births_2018Census.csv", index=False
)
...
k2_melt = k2.melt(
    id_vars=["Variant", "Year", "Period", "Age_Grp", "Region"],
...
ests[ests.columns[2:103]] = (
    ests[ests.columns[2:103]] * 1000
)  # given numbers are in 1000's, so multiply by 1000 to give actual
ests = ests.rename(columns={ests.columns[1]: "Year"})
...
(__tmp__, age_grp_lookup) = create_age_range_lookup(
    min_age=0, max_age=100, range_size=5
)
...
pop_2010 = (
    ests_melt.loc[ests_melt["Year"] == 2010, ["Sex", "Age", "Count"]]
    .copy()
...
pop_2010.sum()  # 14M
district_breakdown = (
    table[["District", "Count"]].groupby(["District"]).sum() / table["Count"].sum()
)
    ...
    for Age in range(101):
        for District in district_nums.index:
            tot_agesex_pop_across_districts = pop_2010.loc[
                (pop_2010["Sex"] == Sex) & (pop_2010["Age"] == Age), "Count"
            ].values[0]

            frac_in_district = district_breakdown.loc[
                district_breakdown.index == District
            ]["Count"].values[0]

            tot_agesex_pop_this_district = (
                tot_agesex_pop_across_districts * frac_in_district
            )
...
init_pop = init_pop[
    ["District", "District_Num", "Region", "Sex", "Age", "Count"]
].reset_index(drop=True)
...
tot_births = (
    tot_births.loc[tot_births[tot_births.columns[2]] == "Malawi"]
    .copy()
    .reset_index(drop=True)
)
...
tot_births["Total_Births"] = (
    1000 * tot_births["Total_Births"]
)  # Imported units are 1000's
...
sex_ratio = (
    sex_ratio.loc[sex_ratio[sex_ratio.columns[2]] == "Malawi"]
    .copy()
    .reset_index(drop=True)
)
...
def reformat_date_period_for_wpp(wpp_import):
    ...
    wpp_import["Period"] = (
        wpp_import["t_lo"].astype(str) + "-" + wpp_import["t_hi"].astype(str)
    )
...
frac_birth_male["frac_births_male"] = frac_birth_male["M_to_F_Sex_Ratio"] / (
    1 + frac_birth_male["M_to_F_Sex_Ratio"]
)
frac_birth_male.drop(
    frac_birth_male.index[frac_birth_male["Variant"] == "Low variant"],
    axis=0,
    inplace=True,
)
frac_birth_male.drop(
    frac_birth_male.index[frac_birth_male["Variant"] == "High variant"],
    axis=0,
    inplace=True,
)
frac_birth_male.drop(
    ["Variant", "Total_Births", "M_to_F_Sex_Ratio"], axis=1, inplace=True
)
frac_birth_male["low_year"], frac_birth_male["high_year"] = (
    frac_birth_male["Period"].str.split("-", 1).str
)
...
for year in range(1950, 2100):
    frac_this_year = frac_birth_male.loc[
        (int(year) >= frac_birth_male["low_year"])
        & (int(year) <= frac_birth_male["high_year"]),
        "frac_births_male",
    ].values[0]

    record = {"Year": year, "frac_births_male": frac_this_year}
...
frac_birth_male_for_export.to_csv(
    path_for_saved_files / "ResourceFile_Pop_Frac_Births_Male.csv", index=False
)
...
asfr[asfr.columns[2:9]] = (
    asfr[asfr.columns[2:9]] / 1000
)  # given numbers are per 1000, so divide by 1000 to make 'per woman'
...
asfr_melt = asfr.melt(
    id_vars=["Variant", "Period"], value_name="asfr", var_name="Age_Grp"
)
...
deaths_males = (
    deaths_males.loc[deaths_males[deaths_males.columns[2]] == "Malawi"]
    .copy()
    .reset_index(drop=True)
)
deaths_males["Sex"] = "M"
...
deaths_females = (
    deaths_females.loc[deaths_females[deaths_females.columns[2]] == "Malawi"]
    .copy()
    .reset_index(drop=True)
)
...
deaths[deaths.columns[2:22]] = (
    deaths[deaths.columns[2:22]] * 1000
)  # given numbers are in 1000's, so multiply by 1000 to give actual
...
deaths_melt.to_csv(
    path_for_saved_files / "ResourceFile_TotalDeaths_WPP.csv", index=False
)
...
lt_males = pd.concat(
    [
        pd.read_excel(
            lt_males_file, sheet_name="ESTIMATES", header=16, usecols="B,C,H,I,J,K"
        ),
        pd.read_excel(
            lt_males_file,
            sheet_name="MEDIUM 2020-2050",
            header=16,
            usecols="B,C,H,I,J,K",
        ),
        pd.read_excel(
            lt_males_file,
            sheet_name="MEDIUM 2050-2100",
            header=16,
            usecols="B,C,H,I,J,K",
        ),
    ]
...
lt_males = (
    lt_males.loc[lt_males[lt_males.columns[1]] == "Malawi"]
    .copy()
    .reset_index(drop=True)
)
lt_males["Sex"] = "M"

lt_females = pd.concat(
    [
        pd.read_excel(
            lt_females_file, sheet_name="ESTIMATES", header=16, usecols="B,C,H,I,J,K"
        ),
        pd.read_excel(
            lt_females_file,
            sheet_name="MEDIUM 2020-2050",
...
lt_females = (
    lt_females.loc[lt_females[lt_females.columns[1]] == "Malawi"]
    .copy()
    .reset_index(drop=True)
)
lt_females["Sex"] = "F"
...
lt = lt.rename(
    columns={"Central death rate m(x,n)": "death_rate"}
)  # NB. it is indeed an instantaneous rate
...
mort_sched["low_age"], mort_sched["high_age"] = (
    mort_sched["Age_Grp"].str.split("-", 1).str
)
...
mort_sched_expanded = pd.DataFrame(
    mort_sched_expanded_as_list,
    columns=["fallbackyear", "sex", "age_years", "death_rate"],
)

mort_sched_expanded.to_csv(
    path_for_saved_files / "ResourceFile_Pop_DeathRates_Expanded_WPP.csv", index=False
)
...
dhs_asfr[dhs_asfr.columns[1:]] = (
    dhs_asfr[dhs_asfr.columns[1:]] / 1000
)  # to make the ASFR per women
dhs_asfr.to_csv(path_for_saved_files / "ResourceFile_ASFR_DHS.csv", index=False)


dhs_u5 = pd.read_excel(
    dhs_working_file, sheet_name="UNDER_5_MORT", header=1, index=False
)
dhs_u5["Year"] = dhs_u5.index
dhs_u5 = dhs_u5.reset_index(drop=True)
dhs_u5 = dhs_u5[dhs_u5.columns[[3, 0, 1, 2]]]
dhs_u5[dhs_u5.columns[1:]] = (
    dhs_u5[dhs_u5.columns[1:]] / 1000
)  # to make it mortality risk per person
dhs_u5.to_csv(
    path_for_saved_files / "ResourceFile_Under_Five_Mortality_DHS.csv", index=False
)
