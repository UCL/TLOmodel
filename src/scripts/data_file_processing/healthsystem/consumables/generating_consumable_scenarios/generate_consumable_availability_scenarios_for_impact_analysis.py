...
def generate_alternative_availability_scenarios(tlo_availability_df: pd.DataFrame = None) -> pd.DataFrame:
    ...
    # Return updated consumable availability resource file with scenario data
    return full_df_with_scenario

def generate_descriptive_consumable_availability_plots(tlo_availability_df: pd.DataFrame = None,
                                                       figurespath: str = None,
                                                       mfl: pd.DataFrame = None,
                                                       program_item_mapping: pd.DataFrame = None,
                                                       chosen_availability_columns:list = None,
                                                       scenario_names_dict:dict = None,):
    # Prepare the availability dataframe for descriptive plots
    df_for_plots = tlo_availability_df.merge(mfl[['Facility_ID', 'Facility_Level']], on = 'Facility_ID', how = 'left', validate = "m:1")
    df_for_plots = df_for_plots.merge(program_item_mapping, on = 'item_code', how = 'left', validate = "m:1")
    if chosen_availability_columns is None:
        # All availability columns
        chosen_availability_columns = [c for c in df_for_plots.columns if c.startswith("available_prop")]

    # recreate the chosen columns list based on the mapping above
    if scenario_names_dict is not None:
    ...
    color_mapping = {
        ...
        'EPI supply \n chain': '#17becf',
        'HIV moved to \n Govt supply chain \n (Avg by Level)': '#ff6347',
        'HIV moved to \n Govt supply chain  \n (Avg by Facility_ID)': '#ff7f50',
        'HIV moved to \n Govt supply chain  \n (Avg by Facility_ID times 1.25)': '#fa8072',
        'HIV moved to \n Govt supply chain  \n (Avg by Facility_ID times 0.75)': '#cd5c5c',
        'Perfect': '#31a354'
    }

    # Create a color list for the bars
    colors = [
        color_mapping.get(scenario, '#808080')  # default to grey if scenario not in mapping
        for scenario in average_availability['scenario']
    ]
    ...
    # Plot 4
    # Create heatmap of average availability by item for HIV program across chosen scenarios
    # Pivot the DataFrame
