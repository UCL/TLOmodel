...
# 1. Load HHFA data#
####################
df_orig <- read_csv("1 data/cleaned_hhfa_2019_onlyobserved.csv", col_names = TRUE)
...
df <- df_orig %>%
  filter(service_consumable_stock == 1, # Facilities which do not store consumables
         ...
         fac_daily_opening_hours != 0, 
         !(item %in% second_line_arvs) # Facilities which reporting 0 opening hours per day
  )
...
# drug_resupply_calculation_system - this variable has been dropped since it's not clear whether this was 
# accurately reported
