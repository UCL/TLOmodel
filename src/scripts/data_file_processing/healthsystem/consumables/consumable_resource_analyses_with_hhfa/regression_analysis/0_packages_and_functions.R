...
custom_forest_plot <- function(model, ylimit= 4){
  ...
  reg_table_rownames <- data.frame(term = c("fac_typeFacility_level_1b", "fac_ownerCHAM", "fac_ownerNGO","fac_ownerPrivate for profit",                          
                                            "fac_urban",                                            
                                            "functional_computer",                                  
  ...
  forestdf <- structure(list(labels = structure(1:27, .Label = forest_matrix$term, class = "factor"), 
                             rr = forest_matrix$estimate, rrhigh = forest_matrix$conf.high, rrlow = forest_matrix$conf.low), 
                        class = "data.frame", row.names = c(NA, -27L)) # changes from factor to character
  
  
  # Create the dataframe "fpplot" for the data table
  #-----------------------------------------------------------
  fplottable <- structure(list(labels = structure(1:27, .Label = forest_matrix$term, class = "factor"), 
  ...
  p <<- ggplot(forestdf, aes(x = rr, y = labels, xmin = rrlow, xmax = rrhigh)) +
    ...
    scale_x_log10(limits = c(0.25, ylimit*(1.15)), 
...
custom_forest_plot_by_level <- function(model, xlab = "Odds ratio with 95% Confidence Interval", ylimit = 4){
  ...
  forestdf <- structure(list(labels = structure(1:(position_last_var-1), .Label = forest_matrix$term, class = "factor"), 
                             rr = forest_matrix$estimate, rrhigh = forest_matrix$conf.high, rrlow = forest_matrix$conf.low), 
                        class = "data.frame", row.names = c(NA, -(position_last_var-1))) # changes from factor to character
  
  
  # Create the dataframe "fpplot" for the data table
  #-----------------------------------------------------------
  fplottable <- structure(list(labels = structure(1:(position_last_var-1), .Label = forest_matrix$term, class = "factor"), 
                               ci = paste0(forest_matrix$estimate, " (", forest_matrix$conf.low, " - ",forest_matrix$conf.high, ")"),
                               p = forest_matrix$p.value),  
                          class = "data.frame", row.names = c(NA,-(position_last_var-1)))
