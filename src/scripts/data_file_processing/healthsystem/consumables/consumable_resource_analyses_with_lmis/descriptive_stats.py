...
stkout_df = pd.read_csv(
    path_for_new_resourcefiles / "ResourceFile_Consumables_availability_and_usage.csv", low_memory=False
)
...
consumables_df = pd.merge(
    stkout_df.drop(columns=["district", "Unnamed: 0"]), fac_gis.drop(columns=["Unnamed: 0"]), how="left", on="fac_name"
)
