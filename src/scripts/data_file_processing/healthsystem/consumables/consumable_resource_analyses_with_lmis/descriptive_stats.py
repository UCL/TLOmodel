...
regsubset_cond2 = (
    stkout_df["fac_type_tlo"] == "Facility_level_0"
)  # since only one facility from Mchinji reported in OpenLMIS
...
rename_districts = {"Nkhota Kota": "Nkhotakota", "Nkhata bay": "Nkhata Bay"}
stkout_df["district"] = stkout_df["district"].replace(rename_districts)

# --- 1.2 Import GIS data --- #
fac_gis = pd.read_csv(
    path_to_files_in_the_tlo_dropbox / "gis_data/facility_distances.csv"
)

# --- 1.3 Merge cleaned LMIS data with GIS data --- #
consumables_df = pd.merge(
    stkout_df.drop(columns=["district", "Unnamed: 0"]),
    fac_gis.drop(columns=["Unnamed: 0"]),
    how="left",
    on="fac_name",
)
