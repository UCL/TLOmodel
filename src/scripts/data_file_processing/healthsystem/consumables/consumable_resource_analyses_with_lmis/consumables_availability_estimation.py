...
def rename_items_to_address_inconsistentencies(_df, item_dict):
    ...
    columns_to_sum = [
        col for col in _df.columns if col[0].startswith(("amc", "closing_bal", "dispensed", "received", "stkout_days"))
    ]
    columns_to_preserve = [col for col in _df.columns if col[0].startswith(("data_source"))]
    ...
    def custom_agg(x):
        ...
        # is a 0 or positive value for one or both item name variation, the sum is taken.
            ...
            return x.str.cat(sep="")  # for the data_source column, this function concatenates the string values
...
list_of_items_with_inconsistent_names_zipped = set(
...
df_with_consistent_item_names = lmis_df_wide_flat[
    ~lmis_df_wide_flat[("item",)].isin(list_of_items_with_inconsistent_names)
]
df_without_consistent_item_names = lmis_df_wide_flat[
    lmis_df_wide_flat[("item",)].isin(list_of_items_with_inconsistent_names)
]
...
lmis_df_wide_flat = pd.concat(
    [df_without_consistent_item_names_corrected, df_with_consistent_item_names], ignore_index=True
)
...
consumables_df = pd.read_csv(
    path_for_new_resourcefiles / "ResourceFile_consumables_matched.csv", low_memory=False, encoding="ISO-8859-1"
)
...
def collapse_stockout_data(_df, groupby_list, var):
    ...
    _collapsed_df = (
        ...
        .agg({col: custom_agg_stkout for col in columns_to_multiply + columns_to_sum + columns_to_preserve})
        .reset_index()
    )
...
stkout_df = (
    stkout_df.groupby(["fac_type_tlo", "fac_name", "district", "month", "item_code"], as_index=False)
    .agg(collapse_dict)
    .reset_index()
)
...
unmatched_consumables = pd.merge(
    unmatched_consumables, matched_consumables[["item", "item_code"]], how="left", on="item_code"
)
...
cond_ncds = (stkout_df["category"] == "epilepsy") | (stkout_df["category"] == "depression")
...
for fac in fac_ids:
    for item in item_codes:
        ...
        if pd.notnull(_monthly_records).any():
            ...
            recorded_at_other_facilities_of_same_level = pd.notnull(full_set.loc[(facilities, slice(None), item)]).any()
...
full_set_interpolated.reset_index().to_csv(
    path_for_new_resourcefiles / "ResourceFile_Consumables_availability_small.csv", index=False
)
...
final_availability_df = (
    final_availability_df.groupby(["Facility_Level", "item_code"]).agg({"available_prop": "mean"}).reset_index()
)
...
def comparison_plot(level_of_disaggregation, group_by_var, colour):
    ...
    for i, label in enumerate(comparison_df_agg["labels"]):
        plt.annotate(
            label,
            (comparison_df_agg["available_prop"][i] + 0.005, comparison_df_agg["available_prop_hhfa"][i] + 0.005),
            fontsize=6,
            rotation=38,
        )
...
def comparison_plot_by_level(fac_type):
    ...
    plt.scatter(comparison_df_by_level["available_prop"], comparison_df_by_level["available_prop_hhfa"])
    plt.axline([0, 0], [1, 1])
