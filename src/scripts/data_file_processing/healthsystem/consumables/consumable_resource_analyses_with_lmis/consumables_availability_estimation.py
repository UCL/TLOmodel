...
path_to_files_in_the_tlo_dropbox = (
    path_to_dropbox / "05 - Resources/Module-healthsystem/consumables raw files/"
)
...
# Import 2018 data
lmis_df = pd.read_csv(
    path_to_files_in_the_tlo_dropbox / "ResourceFile_LMIS_2018.csv", low_memory=False
)
...
cond_level1a = (
    (lmis_df["fac_type"] == "Clinic")
    | (lmis_df["fac_type"] == "Health Centre")
    | (lmis_df["fac_name"].str.contains("Clinic"))
    | (lmis_df["fac_name"].str.contains("Health Centre"))
    | (lmis_df["fac_name"].str.contains("Maternity"))
    | (lmis_df["fac_name"] == "Chilaweni")
    | (lmis_df["fac_name"] == "Chimwawa")
    | (lmis_df["fac_name"].str.contains("Dispensary"))
)
cond_level1b = (
    (lmis_df["fac_name"].str.contains("Community Hospital"))
    | (lmis_df["fac_type"] == "Rural/Community Hospital")
    | (lmis_df["fac_name"] == "St Peters Hospital")
    | (lmis_df["fac_name"] == "Police College Hospital")
    | (lmis_df["fac_type"] == "CHAM")
    | (lmis_df["fac_owner"] == "CHAM")
)
cond_level2 = (lmis_df["fac_type"] == "District Health Office") | (
    lmis_df["fac_name"].str.contains("DHO")
)
...
num_cols = ["year", "closing_bal", "dispensed", "stkout_days", "amc", "received"]
...
programs_lmis_dict = {
    ...
    5: "RH",
    6: "TB",
}  # programs listed in the OpenLMIS data
...
lmis_df_wide = lmis_df.pivot_table(
    index=["district", "fac_type_tlo", "fac_name", "program", "item"],
    columns="month",
...
for m in range(1, 13):
    ...
    lmis_df_wide_flat.loc[cond1, [("data_source", months_dict[m])]] = (
        "original_lmis_data"
    )

    # First update received to zero if other columns are avaialble
    cond1 = (
        lmis_df_wide_flat["closing_bal", months_dict[m]].notna()
        & lmis_df_wide_flat["amc", months_dict[m]].notna()
    )
...
inconsistent_item_names_mapping = {
    "Zidovudine/Lamivudine (AZT/3TC), 300+150mg": """Zidovudine (AZT) + Lamivudine (3TC), 300mg+150mg, 60''s (4A, 8A)""",
    ...
    "Atazanavir /Ritonavir (ATV/r), 300+100mg": "Atazanavir +  Ritonavir, 300mg + 100mg, 30"
    "S (7A)",
...
def rename_items_to_address_inconsistentencies(_df, item_dict):
    ...
    for item in item_dict:
        print(
            len(_df[_df.item == item_dict[item]]),
            ''' instances of "''',
            item_dict[item],
            '''"''' ''' changed to "''',
            item,
    ...
    columns_to_sum = [
        ...
        if col[0].startswith(
            ("amc", "closing_bal", "dispensed", "received", "stkout_days")
        )
    ]
    columns_to_preserve = [
        col for col in _df.columns if col[0].startswith(("data_source"))
    ]
    ...
    def custom_agg(x):
        if x.name in columns_to_sum:
            return (
                x.sum(skipna=True) if np.any(x.notnull() & (x >= 0)) else np.nan
            )  # this ensures that the NaNs are retained
        ...
        # is a 0 or positive value for one or both item name variation, the sum is taken.
            ...
            return x.str.cat(
                sep=""
            )  # for the data_source column, this function concatenates the string values

    # Collapse dataframe
    _collapsed_df = (
        _df.groupby(["program", "item", "district", "fac_type_tlo", "fac_name"])
        .agg({col: custom_agg for col in columns_to_preserve + columns_to_sum})
        .reset_index()
    )
...
lmis_df_wide_flat = rename_items_to_address_inconsistentencies(
    lmis_df_wide_flat, inconsistent_item_names_mapping
)
...
for m in range(2, 13):
    ...
    cond1 = (
        lmis_df_wide_flat["closing_bal", months_dict[m - 1]].notna()
        & lmis_df_wide_flat["amc", months_dict[m]].notna()
        & lmis_df_wide_flat["received", months_dict[m]].notna()
    )
    cond2 = lmis_df_wide_flat["stkout_days", months_dict[m]].notna()
    cond3 = lmis_df_wide_flat["amc", months_dict[m]] != 0
    lmis_df_wide_flat.loc[cond1 & ~cond2 & cond3, [("data_source", months_dict[m])]] = (
        "lmis_interpolation_rule1"
    )

    if months_dict[m] in months_dict31:
        lmis_df_wide_flat.loc[
            cond1 & ~cond2 & cond3, [("stkout_days", months_dict[m])]
        ] = (
            (
...
lmis_df_wide_flat["consumable_reporting_freq"] = lmis_df_wide_flat["closing_bal"].count(
    axis=1
)  # generate a column
...
lmis_df_wide_flat.columns = [
    " ".join(col).strip() for col in lmis_df_wide_flat.columns.values
]
...
for m in range(1, 13):
    ...
    lmis_df_wide_flat[var_name] = lmis_df_wide_flat.groupby("fac_name")[
        month
    ].transform(
        "count"
    )  # generate a column
...
for m in range(1, 13):
    ...
    lmis_df_wide_flat.loc[
        cond1 & cond2 & ~cond3, [("data_source " + months_dict[m])]
    ] = "lmis_interpolation_rule3"
...
for m in range(1, 13):
    count_stkout_entries = (
        count_stkout_entries
        + lmis_df_wide_flat["stkout_days " + months_dict[m]].count().sum()
    )
...
for m in range(1, 13):
    if months_dict[m] in months_dict31:
        lmis["stkout_prop " + months_dict[m]] = (
            lmis["stkout_days " + months_dict[m]] / 31
        )
    elif months_dict[m] in months_dict30:
        lmis["stkout_prop " + months_dict[m]] = (
            lmis["stkout_days " + months_dict[m]] / 30
        )
    else:
        lmis["stkout_prop " + months_dict[m]] = (
            lmis["stkout_days " + months_dict[m]] / 28
        )

# Reshape data
lmis = pd.wide_to_long(
    lmis,
    stubnames=[
        "closing_bal",
        "received",
        ...
        "stkout_prop",
        "data_source",
        "consumables_reported_in_mth",
    ],
    i=["district", "fac_type_tlo", "fac_name", "program", "item"],
    j="month",
    sep=" ",
    suffix=r"\w+",
)
...
consumables_df = pd.read_csv(
    path_for_new_resourcefiles / "ResourceFile_consumables_matched.csv",
    low_memory=False,
    encoding="ISO-8859-1",
)
cond = consumables_df["matching_status"] == "Remove"
consumables_df = consumables_df[
    ~cond
]  # Remove items which were removed due to updates or the existence of duplicates
...
matched_consumables = replace_old_item_names_in_lmis_data(
    matched_consumables, inconsistent_item_names_mapping
)
...
def collapse_stockout_data(_df, groupby_list, var):
    ...
    def custom_agg_stkout(x):
        if x.name in columns_to_multiply:
            return (
                x.prod(skipna=True) if np.any(x.notnull() & (x >= 0)) else np.nan
            )  # this ensures that the NaNs are retained
    ...
    # Collapse dataframe
    _collapsed_df = (
        _df.groupby(groupby_list)
        .agg(
            {
                col: custom_agg_stkout
                for col in columns_to_multiply + columns_to_sum + columns_to_preserve
            }
        )
        .reset_index()
    )
...
collapse_dict = {
    ...
    "dispensed": "mean",
    "received": "mean",
    "consumable_reporting_freq": "mean",
    "consumables_reported_in_mth": "mean",
    "module_name": "first",
...
stkout_df = (
    stkout_df.groupby(
        ["fac_type_tlo", "fac_name", "district", "month", "item_code"], as_index=False
    )
    .agg(collapse_dict)
    .reset_index()
)
...
unmatched_consumables = pd.merge(
    unmatched_consumables,
    matched_consumables[["item", "item_code"]],
    how="left",
    on="item_code",
)
...
hhfa_df = pd.read_excel(
    path_to_files_in_the_tlo_dropbox / "ResourceFile_hhfa_consumables.xlsx",
...
availratio_2to1b = (
    stkout_df[cond2]["available_prop"].mean()
    / stkout_df[cond1b]["available_prop"].mean()
)
availratio_3to1b = (
    stkout_df[cond3]["available_prop"].mean()
    / stkout_df[cond1b]["available_prop"].mean()
)
...
hhfa_df["available_prop_hhfa_Facility_level_1b"] = (
    scaleparam * hhfa_df["available_prop_hhfa_Facility_level_1b"]
)
hhfa_df["available_prop_hhfa_Facility_level_2"] = (
    availratio_2to1b * hhfa_df["available_prop_hhfa_Facility_level_1b"]
)
hhfa_df["available_prop_hhfa_Facility_level_3"] = (
    availratio_3to1b * hhfa_df["available_prop_hhfa_Facility_level_1b"]
)

for var in [
    "available_prop_hhfa_Facility_level_2",
    "available_prop_hhfa_Facility_level_3",
]:
    cond = hhfa_df[var] > 1
    hhfa_df.loc[cond, var] = 1

# Add further assumptions on consumable availability from other sources
assumptions_df = pd.read_excel(
    open(path_to_files_in_the_tlo_dropbox / "ResourceFile_hhfa_consumables.xlsx", "rb"),
    sheet_name="availability_assumptions",
)
assumptions_df = assumptions_df[
    [
        "item_code",
        "available_prop_Facility_level_0",
        "available_prop_Facility_level_1a",
        "available_prop_Facility_level_1b",
        "available_prop_Facility_level_2",
        "available_prop_Facility_level_3",
    ]
]

# Merge HHFA data with the list of unmatched consumables from the TLO model
unmatched_consumables_df = pd.merge(
    unmatched_consumables, hhfa_df, how="left", on="item_code"
)
unmatched_consumables_df = pd.merge(
    unmatched_consumables_df, assumptions_df, how="left", on="item_code"
)
# when not missing, replace with assumption

for level in ["0", "1a", "1b", "2", "3"]:
    cond = unmatched_consumables_df[
        "available_prop_hhfa_Facility_level_" + level
    ].notna()
    ...
    unmatched_consumables_df.loc[
        cond, "available_prop_hhfa_Facility_level_" + level
    ] = unmatched_consumables_df["available_prop_Facility_level_" + level]

unmatched_consumables_df = unmatched_consumables_df[
    [
        "module_name",
        "item_code",
        "consumable_name_tlo_x",
        "available_prop_hhfa_Facility_level_0",
        "available_prop_hhfa_Facility_level_1a",
        "available_prop_hhfa_Facility_level_1b",
        "available_prop_hhfa_Facility_level_2",
        "available_prop_hhfa_Facility_level_3",
        "fac_count_Facility_level_0",
        "fac_count_Facility_level_1a",
        "fac_count_Facility_level_1b",
        "data_source",
    ]
]
...
# on OpenLMIS
unmatched_consumables_df = pd.wide_to_long(
    unmatched_consumables_df,
    stubnames=["available_prop_hhfa", "fac_count"],
    i=["item_code", "consumable_name_tlo_x"],
    j="fac_type_tlo",
    sep="_",
    suffix=r"\w+",
)
...
# Final cleaning
NameChangeList = [
    ("consumable_name_tlo_x", "consumable_name_tlo"),
...
NameChangeList = [
    ("fac_count_Facility_level_0", "fac_count"),
...
hhfa_fac0 = pd.merge(
    hhfa_fac0, consumables_df[["item_code", "module_name"]], on="item_code", how="inner"
)
...
cond_childhood = (
    (stkout_df["category"] == "acute lower respiratory infections")
...
diagnostics_cons_list = [
    ...
    184,
    187,
]  # for now these have not been applied because most diagnostics are program specific
...
stkout_df.to_csv(
    path_for_new_resourcefiles / "ResourceFile_Consumables_availability_and_usage.csv"
)
...
print(
    stkout_df.loc[
        (~(stkout_df.available_prop >= 0.0)) | (~(stkout_df.available_prop <= 1.0))
    ].available_prop
)
assert not stkout_df.duplicated(
    ["fac_type_tlo", "fac_name", "district", "month", "item_code"]
).any(), "No duplicates"
...
districts = set(
    pd.read_csv(resourcefilepath / "demography" / "ResourceFile_Population_2010.csv")[
        "District"
    ]
)
...
sf = stkout_df[
    ["item_code", "month", "district", "fac_type_tlo", "available_prop"]
].dropna()
sf.loc[sf.month == "Aggregate", "month"] = (
    "January"  # Assign arbitrary month to data only available at aggregate level
)
sf.loc[sf.district == "Aggregate", "district"] = (
    "Balaka"  # Assign arbitrary district to data only available at # aggregate level
)
...
sf = (
    sf.groupby(by=["district_std", "fac_type_tlo", "month", "item_code"])[
        "available_prop"
    ]
    .mean()
    .reset_index()
...
means = (
    sf.loc[sf.fac_type_tlo.isin(["1a", "1b", "2"])]
    .groupby(by=["fac_type_tlo", "month", "item_code"])["available_prop"]
    .mean()
    .reset_index()
)
new_rows = means.copy()
...
mwanza_1b = (
    sf.loc[(sf.district_std == "Mwanza") & (sf.fac_type_tlo == "1a")]
    .copy()
    .assign(fac_type_tlo="1b")
)
...
districts_with_regional_level_fac = pd.unique(
    sf_merge.loc[sf_merge.fac_type_tlo == "3"].district_std
)
...
for _district in districts_with_regional_level_fac:
    ...
    _fac_id = mfl.loc[
        (mfl.Facility_Level == "3") & (mfl.Region == _region)
    ].Facility_ID.values[0]
    sf_merge.loc[
        (sf_merge.fac_type_tlo == "3") & (sf_merge.district_std == _district),
        "Facility_ID",
    ] = _fac_id
...
sf_final = (
    sf_merge.groupby(by=["Facility_ID", "month", "item_code"])["available_prop"]
    .mean()
    .reset_index()
)
...
full_set = pd.Series(
    index=pd.MultiIndex.from_product(
        [fac_ids, months, item_codes], names=["Facility_ID", "month", "item_code"]
    ),
    data=np.nan,
    name="available_prop",
)
...
full_set = full_set.combine_first(
    sf_final.set_index(["Facility_ID", "month", "item_code"])["available_prop"]
)
...
check_format_of_consumables_file(
    df=full_set_interpolated.reset_index(), fac_ids=fac_ids
)
...
hhfa_comparison_df = pd.wide_to_long(
    hhfa_comparison_df.dropna(),
    stubnames="available_prop_hhfa",
    i=["consumable_name_tlo", "item_code", "item_hhfa"],
    j="fac_type_tlo",
    sep="_",
    suffix=r"\w+",
)
hhfa_comparison_df = hhfa_comparison_df.reset_index()
hhfa_comparison_df["fac_type_tlo"] = hhfa_comparison_df["fac_type_tlo"].str.replace(
    "Facility_level_", ""
)
hhfa_comparison_df = hhfa_comparison_df.rename(
    {"fac_type_tlo": "Facility_Level"}, axis=1
)
...
final_availability_df = pd.merge(
...
final_availability_df = (
    final_availability_df.groupby(["Facility_Level", "item_code"])
    .agg({"available_prop": "mean"})
    .reset_index()
)
...
comparison_df = pd.merge(
    ...
    hhfa_comparison_df,
    how="inner",
    on=["item_code", "Facility_Level"],
)
comparison_df["difference"] = (
    comparison_df["available_prop_hhfa"] - comparison_df["available_prop"]
)

# --- 7.2 Compare OpenLMIS estimates with HHFA estimates (CALIBRATION) --- ##
# Summary results by level of care
comparison_df.groupby(["Facility_Level"])[
    ["available_prop", "available_prop_hhfa", "difference"]
].mean()
...
def comparison_plot(level_of_disaggregation, group_by_var, colour):
    comparison_df_agg = comparison_df.groupby([group_by_var], as_index=False).agg(
        {
            "available_prop": "mean",
    ...
    ax = comparison_df_agg.plot.scatter(
        "available_prop", "available_prop_hhfa", c=colour
    )
    ax.axline([0, 0], [1, 1])
    ...
    if level_of_disaggregation != "aggregate":
        plt.title(
            "Disaggregated by " + level_of_disaggregation, fontsize=size, weight="bold"
        )
    else:
    ...
    save_name = (
        "comparison_plots/calibration_to_hhfa_" + level_of_disaggregation + ".png"
    )
...
def comparison_plot_by_level(fac_type):
    ...
    plt.scatter(
        comparison_df_by_level["available_prop"],
    ...
    for i, label in enumerate(comparison_df_by_level["consumable_labels"]):
        plt.annotate(
            label,
            (
                comparison_df_by_level["available_prop"][i] + 0.005,
...
plt.savefig(
    outputfilepath / "comparison_plots/calibration_to_hhfa_fac_type_and_consumable.png"
)
