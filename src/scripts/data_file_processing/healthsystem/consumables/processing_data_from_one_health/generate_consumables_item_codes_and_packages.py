...
workingfile_ehp_consumables = (
    path_to_dropbox
    / "05 - Resources/Module-healthsystem/From Matthias Arnold/ORIGINAL_Intervention input.xlsx"
)

workingfile_one_health = (
    path_to_dropbox / "07 - Data/OneHealth projection files/OneHealth commodities.xlsx"
)
...
wb_import = pd.read_excel(
    workingfile_ehp_consumables, sheet_name="Intervention input costs", header=None
)
...
block = wb.loc[
    wb["Intervention"]
    == "Treatment for those with cerebrovascular disease and post-stroke"
].copy()
...
block_no_diabetes.loc[:, "Intervention"] = (
    block.at[559, "Intervention"] + "_ No Diabetes"
)
...
block_diabetes.loc[:, "Intervention"] = (
    block.at[559, "Intervention"] + "_ With Diabetes"
)
...
wb.loc[:, "Expected_Units_Per_Case"] = (
    (wb.loc[:, "Proportion of patients receiving this input"] / 100)
...
wb.loc[:, "Expected_Cost_Per_Case"] = (
    wb.loc[:, "Expected_Units_Per_Case"] * wb.loc[:, "Unit_Cost"]
)

wb = wb.rename(
    columns={
        "Drug/Supply Inputs": "Items",
...
intv_codes = pd.DataFrame(
    {
        "Intervention_Pkg": unique_intvs,
...
item_codes = pd.DataFrame(
    {"Items": unique_items, "Item_Code": np.arange(0, len(unique_items))}
)
...
oh = oh[["Drug or supply", 2010.0]].rename(
    columns={"Drug or supply": "Items", 2010.0: "Unit_Cost"}
)
...
comparison = pd.DataFrame(list_of_consumables_orig).merge(
    list_of_consumables_oh, on="Items", indicator=True, how="outer"
)

# get the ones which are only in the one health dataset
only_in_oh = comparison.loc[comparison["_merge"] == "right_only", "Items"].reset_index(
    drop=True
)
...
# Save:
cons.to_csv(
    path_for_new_resourcefiles / "ResourceFile_Consumables_Items_and_Packages.csv",
    index=False,
)
