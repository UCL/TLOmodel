...
# Reshape data
hhfa_equipment = pd.melt(
    hhfa_equipment_wide, id_vars=["fac_code"], var_name="equipment_availability_var", value_name="response"
)
...
# Reshape data
unique_equipment_df_for_model = hhfa_equipment_df_for_model.pivot(
    index=["fac_code", "equipment"], columns="availability_var", values="response"
)
...
new_column_names = {
    ...
    "previous-prep": "date_last_prepared",
    "today": "available",
    "today-functional": "functional_today",
}
...
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["functional"].isin(["Yes", "No"]), "available"
] = "Yes"  # if there is a valid response to "functional" the equipment must be available
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["functional_today"].isin(
        ["AVAILABLE  AND  FUNCTIONAL", "AVAILABLE  NOT  FUNCTIONAL", "AVAILABLE DON'T KNOW IF  FUNCTIONAL", "Yes"]
    ),
    "available",
] = "Yes"  # if equipment is functional then it is available
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["functional_today"].isin(["NOT AVAILABLE", "No"]), "available"
] = "No"  # if equipment is functional then it is available
available_missing_after_extrapolation = len(
    unique_equipment_df_for_model[unique_equipment_df_for_model.available.isna()]
)
print(
    (available_missing_before_extrapolation - available_missing_after_extrapolation)
    / len(unique_equipment_df_for_model)
    * 100,
    "% values for 'available' extrapolated from other survey questions",
)
...
unique_equipment_df_for_model.loc[
    (
        ...
        & unique_equipment_df_for_model["functional"].isna()
    ),
    "functional",
] = "Yes"
...
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["available"].isin(["AT LEAST ONE VALID", "Available and functional"]), "functional"
] = "Yes"  # information under the available
#                                                                question captures whether equipment is functional
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["available"].isin(["AVAILABLE NON VALID", "Available not functional"]), "functional"
] = "No"  # information under the available
#                                                               question captures whether equipment is functional
functional_missing_after_extrapolation = len(
    unique_equipment_df_for_model[unique_equipment_df_for_model.functional.isna()]
)
print(
    (functional_missing_before_extrapolation - functional_missing_after_extrapolation)
    / len(unique_equipment_df_for_model)
    * 100,
    "% values for 'functional' extrapolated from other survey questions",
)
...
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["available"].isin(
        [
            "AT LEAST ONE VALID",
...
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["available"].isin(
        ["NOT AVAILABLE TODAY", "Not available", "NEVER AVAILABLE", "Not observed", "No"]
    ),
    "available",
] = 0
unique_equipment_df_for_model.loc[
    unique_equipment_df_for_model["functional"] == 3, "functional"
] = np.nan  # 3 implies "Don't know"
unique_equipment_df_for_model.loc[unique_equipment_df_for_model["functional"] == "Yes", "functional"] = 1
unique_equipment_df_for_model.loc[
    (unique_equipment_df_for_model["functional"] == "No") | (unique_equipment_df_for_model["available"] == 0),
    "functional",
] = 0
...
equipment_crosswalk_only_matched_items = equipment_crosswalk[
    equipment_crosswalk.Equipment_name_HHFA.notna() & equipment_crosswalk.Equipment_name.notna()
]
tlo_equipment_availability = pd.merge(
    ...
    left_on="Equipment_name_HHFA",
    right_on="equipment",
)

tlo_equipment_availability_duplicates_collapsed = (
    tlo_equipment_availability.groupby(["Item_code", "Equipment_name", "fac_code"])[["available", "functional"]]
    .agg(max)
    .reset_index()
)
...
hhfa_facility_data = pd.read_csv(
    path_to_dropbox / "07 - Data/HHFA_2018-19/2 clean/cleaned_hhfa_2019.csv",
...
final_equipment_availability_df = pd.merge(
    tlo_equipment_availability_duplicates_collapsed,
...
for district, city in district_mapping.items():
    ...
    final_equipment_availability_df = pd.concat(
        [final_equipment_availability_df, duplicated_district_rows], ignore_index=True
    )
...
final_equipment_availability_df.loc[
    (final_equipment_availability_df.District == "Mzimba North")
    | (final_equipment_availability_df.District == "Mzimba South"),
    "District",
] = "Mzimba"
...
model_district_list = pd.read_csv(resourcefilepath / "demography/ResourceFile_PopulationSize_2018Census.csv")[
    "District"
].unique()  # Check that the new list of district matches with the model
...
assert final_equipment_availability_export["Facility_Level"].isin(["0", "1a", "1b", "2", "3", "4", "5"]).all()
...
region_lookup = (
    pd.read_csv(resourcefilepath / "demography/ResourceFile_PopulationSize_2018Census.csv")
    .set_index("District")["Region"]
    .to_dict()
)
...
reconciled_level_3_data = (
    final_equipment_availability_export.loc[mask_to_level_3]
    .groupby(["Region", "Facility_Level", "Item_Code"])["Pr_Available"]
    .mean()
    .reset_index()
)
...
final_equipment_availability_export = pd.concat(
    [final_equipment_availability_export, reconciled_level_3_data], ignore_index=True
)

# Set national-level facility_ids (levels 4/5) (depends on only on facility_level)
levels_4_and_5_lookup = (
    mfl.loc[mfl["Facility_Level"].isin(["4", "5"])].set_index("Facility_Level")["Facility_ID"].to_dict()
)
mask_to_levels_4_and_5 = final_equipment_availability_export["Facility_Level"].isin(["4", "5"])
final_equipment_availability_export.loc[
    mask_to_levels_4_and_5, "Facility_ID"
...
final_equipment_availability_export_full = pd.Series(
    index=pd.MultiIndex.from_product([_all_fac_ids, _all_item_codes], names=["Facility_ID", "Item_Code"]),
    data=float("nan"),
    name="Pr_Available",
...
pc_missing_data = final_equipment_availability_export_full.isnull().sum() / len(
...
equipment_price_category = final_equipment_availability_export_full.index.get_level_values("Item_Code").map(
    equipment_price_category_mapper
)
final_equipment_availability_export_full = final_equipment_availability_export_full.groupby(
    [equipment_price_category, facility_levels]
).transform(lambda x: x.fillna(x.mean()))
...
final_equipment_availability_export_full[
    final_equipment_availability_export_full.index.get_level_values(level=0) == list(level_5_lookup.values())[0]
] = 0
