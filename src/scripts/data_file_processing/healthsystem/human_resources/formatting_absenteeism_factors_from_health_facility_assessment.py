...
merged_df = pd.merge(
    survey_daily_capabilities, officer_types, on="Officer_Type_Code", how="left"
)
...
survey_daily_capabilities = (
    survey_daily_capabilities.groupby("Officer_Category").mean().reset_index()
)
...
summarise_daily_capabilities["Officer_Category"] = survey_daily_capabilities[
    "Officer_Category"
]
...
summarise_daily_capabilities = summarise_daily_capabilities.set_index(
    "Officer_Category"
)

# If not data is available, assume scaling factor of 1
absenteeism_factor = (survey_daily_capabilities / summarise_daily_capabilities).fillna(
    1.0
)
