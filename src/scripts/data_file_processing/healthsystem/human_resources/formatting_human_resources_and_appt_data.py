...
path_to_dropbox = Path(
    "/Users/jdbb1/Dropbox/Thanzi La Onse"
...
for d in pop_by_district.index:
    pop_by_district.loc[d, "Region"] = population.loc[
        population["District"] == d, "Region"
    ].values[0]
...
officer_types_table = (
    wb_import.loc[2:3, 64:84].transpose().reset_index(drop=True).copy()
)
...
compiled_staff_return = pd.read_excel(
    path_to_auxiliaryfiles / "Compiled Staff Returns.xlsx",
    sheet_name="Compiled Staff Returns",
    skiprows=range(5),
)

# Get relevant columns
curr_staff_return = compiled_staff_return[
    [
...
curr_staff_return.drop(
    curr_staff_return[curr_staff_return["Cadre"] == "_NOT INCLUDED"].index, inplace=True
)
curr_staff_return.drop(
    curr_staff_return[curr_staff_return["Cadre"] == "_MISSING"].index, inplace=True
)
...
curr_staff_return.drop(
    curr_staff_return[curr_staff_return["Cadre"] == "Home Craft Worker"].index,
    inplace=True,
)
curr_staff_return.drop(
    curr_staff_return[
        curr_staff_return["Cadre"] == "Educ/Environ Health Officer"
    ].index,
    inplace=True,
)
...
idx_Karonga = curr_staff_return[
    curr_staff_return["Health Facility Type"] == "Karonga Hospital"
].index
...
idx_ZMH = curr_staff_return[
    curr_staff_return["Health Facility"] == "Zomba Mental Hospital"
].index
...
fac_types_list = pd.unique(
    curr_staff_return["Health Facility Type"]
)  # Level_0 Facs and Headquarter not included
cadre_list = pd.unique(
    curr_staff_return["Cadre"]
)  # Radiotherapy Technician/Sonographer/Dental Assistant not included

# Add column 'Facility_Level'; HQ not listed in compiled staff return table
idx_urbhc = curr_staff_return[
    curr_staff_return["Health Facility Type"] == "Urban Health Center"
].index
curr_staff_return.loc[idx_urbhc, "Facility_Level"] = (
    "Facility_Level_1a"  # Including CHAM HCs
)

idx_rurhc = curr_staff_return[
    curr_staff_return["Health Facility Type"] == "Rural Health Center"
].index
curr_staff_return.loc[idx_rurhc, "Facility_Level"] = (
    "Facility_Level_1a"  # Including CHAM HCs
)

idx_comhos = curr_staff_return[
    curr_staff_return["Health Facility Type"] == "Community Hospital"
].index
curr_staff_return.loc[idx_comhos, "Facility_Level"] = (
    "Facility_Level_1b"  # Including CHAM community hospitals
)

idx_dishos = curr_staff_return[
    curr_staff_return["Health Facility Type"] == "District Hospital"
].index
curr_staff_return.loc[idx_dishos, "Facility_Level"] = "Facility_Level_2"

idx_cenhos = curr_staff_return[
    curr_staff_return["Health Facility Type"] == "Central Hospital"
].index
curr_staff_return.loc[idx_cenhos, "Facility_Level"] = "Facility_Level_3"

idx_zmhfac = curr_staff_return[
    curr_staff_return["Health Facility Type"] == "Zomba Mental Hospital"
].index
curr_staff_return.loc[idx_zmhfac, "Facility_Level"] = "Facility_Level_4"

# Add column 'Cadre_Code'
for c in cadre_list:
    curr_staff_return.loc[curr_staff_return["Cadre"] == c, "Cadre_Code"] = (
        officer_types_table.loc[
            officer_types_table["Officer_Type"] == c, "Officer_Type_Code"
        ]
        .copy()
        .values[0]
    )
...
curr_staff_district = curr_staff_return[
    ["Facility_Level", "Cadre_Code", "Staff_Count"]
].copy()
...
cadre_faclevel = pd.melt(
    cadre_faclevel,
    id_vars="Cadre_Code",
    value_vars=cadre_faclevel.columns[1:],
...
idx_keep = curr_staff_distribution[
    (curr_staff_distribution["Facility_Level"] == "Facility_Level_0")
    | (curr_staff_distribution["Facility_Level"] == "Facility_Level_1a")
    | (curr_staff_distribution["Facility_Level"] == "Facility_Level_1b")
    | (curr_staff_distribution["Facility_Level"] == "Facility_Level_2")
].index
...
for i in range(21):
    ...
    if (
        curr_staff_distribution.loc[4 * i + 1 : 4 * i + 3, "Staff_Count"].sum() > 0
    ):  # sum of 4i+1,4i+2,4i+3
...
fund_staff_2018_raw = pd.read_excel(
    path_to_auxiliaryfiles / "2018-03-09 Facility-level establishment MOH & CHAM.xlsx",
    sheet_name="Establishment listing",
)

# Get relevant columns
fund_staff_2018 = fund_staff_2018_raw[
    ["Number of positions", "Facility", "Facility Type", "WFOM Cadre"]
].copy()

# Drop rows with missing/blank elements
fund_staff_2018.dropna(inplace=True)
# Drop rows that associate to '_NOT INCLUDED'
fund_staff_2018.drop(
    fund_staff_2018[fund_staff_2018["WFOM Cadre"] == "_NOT INCLUDED"].index,
    inplace=True,
)
# Drop rows for 'Training Institution'
fund_staff_2018.drop(
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Training Institution"].index,
    inplace=True,
)
...
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre_Code"] == "C02"].index, "Cadre_Code"
] = "C01"
...
# as these cadres are not included in 'Time_Base' and 'PFT'.
fund_staff_2018.drop(
    fund_staff_2018[fund_staff_2018["Cadre"] == "Home Craft Worker"].index, inplace=True
)
fund_staff_2018.drop(
    fund_staff_2018[fund_staff_2018["Cadre"] == "Educ/Environ Health Officer"].index,
    inplace=True,
)
fund_staff_2018.drop(
    fund_staff_2018[fund_staff_2018["Cadre"] == "Community Midwife Assistant"].index,
    inplace=True,
)
# Reset index
fund_staff_2018.reset_index(drop=True, inplace=True)
...
# to be consistent with officer_types_table
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "HSA"].index, "Cadre"
] = "DCSA"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre_Code"] == "E02"].index, "Cadre_Code"
] = "E01"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Medical Assistant"].index, "Cadre"
] = "Med. Assistant"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Laboratory Officer"].index, "Cadre"
] = "Lab Officer"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Laboratory Technician"].index, "Cadre"
] = "Lab Technician"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Laboratory Assistant"].index, "Cadre"
] = "Lab Assistant"
fund_staff_2018.loc[
    fund_staff_2018[
        fund_staff_2018["Cadre"] == "Nursing Officer/Registered Nurse"
    ].index,
    "Cadre",
] = "Nurse Officer"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Dentist"].index, "Cadre"
] = "Dental Officer"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Nutrition Officer"].index, "Cadre"
] = "Nutrition Staff"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Pharmacy Technician"].index, "Cadre"
] = "Pharm Technician"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Pharmacy Assistant"].index, "Cadre"
] = "Pharm Assistant"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Pharmacy Officer"].index, "Cadre"
] = "Pharmacist"
...
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility"] == "Zomba Mental Hospital"].index,
    "Facility Type",
...
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Urban Health Center"].index,
    "Facility_Level",
] = "Facility_Level_1a"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Rural Health Center"].index,
    "Facility_Level",
] = "Facility_Level_1a"
fund_staff_2018.loc[
    fund_staff_2018[
        fund_staff_2018["Facility Type"] == "Health Center (with maternity)"
    ].index,
    "Facility_Level",
] = "Facility_Level_1a"
fund_staff_2018.loc[
    fund_staff_2018[
        fund_staff_2018["Facility Type"] == "Health Center (without maternity)"
    ].index,
    "Facility_Level",
] = "Facility_Level_1a"
...
fund_staff_2018.loc[
    fund_staff_2018[
        fund_staff_2018["Facility Type"] == "Rural/Community Hospital"
    ].index,
    "Facility_Level",
] = "Facility_Level_1b"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "District Hospital"].index,
    "Facility_Level",
] = "Facility_Level_2"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "DHO"].index, "Facility_Level"
] = "Facility_Level_2"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Central Hospital"].index,
    "Facility_Level",
] = "Facility_Level_3"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Zomba Mental Hospital"].index,
    "Facility_Level",
] = "Facility_Level_4"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Headquarters"].index,
    "Facility_Level",
] = "Facility_Level_5"
...
fund_staff_district = fund_staff_2018[
    ["Facility_Level", "Cadre_Code", "Number of positions"]
].copy()
...
fund_cadre_faclevel = pd.DataFrame(
    columns=[
        "Cadre_Code",
        "Facility_Level_0",
        "Facility_Level_1a",
...
fund_cadre_faclevel = pd.melt(
    fund_cadre_faclevel,
    id_vars="Cadre_Code",
    value_vars=fund_cadre_faclevel.columns[1:],
    var_name="Facility_Level",
)
# Merge
fund_staff_distribution = fund_staff_distribution.merge(
    fund_cadre_faclevel, how="right"
)
# Fill null with 0
...
# Keep and focus on rows of levels 0, 1a, 1b, and 2
fund_idx_keep = fund_staff_distribution[
    (fund_staff_distribution["Facility_Level"] == "Facility_Level_0")
    | (fund_staff_distribution["Facility_Level"] == "Facility_Level_1a")
    | (fund_staff_distribution["Facility_Level"] == "Facility_Level_1b")
    | (fund_staff_distribution["Facility_Level"] == "Facility_Level_2")
].index
...
for i in range(21):
    ...
    if (
        ...
        fund_staff_distribution.loc[4 * i + 1, "Proportion_Fund"] = (
            ...
            / fund_staff_distribution.loc[
                4 * i + 1 : 4 * i + 3, "Number of positions"
            ].sum()
        )
...
# For DCSA individually, reassign their proportions since we assume all DCSAs are located at level 0
fund_idx_dcsa = fund_staff_distribution[
    fund_staff_distribution["Cadre_Code"] == "E01"
].index
...
opt_workforce = pd.read_excel(
    path_to_auxiliaryfiles / "MalawiOptimization_OUTPUT2022 SH 2019-10-19.xlsx",
...
opt_workforce_distribution = opt_workforce.loc[
    1:4, cols_matter
].copy()  # drop row Facility_Level_3
opt_workforce_distribution = pd.DataFrame(
    opt_workforce_distribution.groupby(by=["Facility_Level"], sort=False).sum()
)
...
opt_workforce_distribution = pd.melt(
    opt_workforce_distribution,
    id_vars="Facility_Level",
    value_vars=cols_matter[1:],
    var_name="Cadre_Opt",
    value_name="Staff_Count_Opt",
)

# Add column 'Cadre_Code'
for i in range(63):
    opt_workforce_distribution.loc[i, "Cadre_Code"] = str(
        opt_workforce_distribution.loc[i, "Cadre_Opt"]
    )[7:10]
...
for i in range(21):
    if (
        opt_workforce_distribution.loc[3 * i : 3 * i + 2, "Staff_Count_Opt"].sum() > 0
    ):  # sum of 3i,3i+1,3i+2
...
immed_need = pd.read_excel(
    path_to_auxiliaryfiles / "MalawiOptimization_OUTPUT_ALLYEARS_Curr.xlsx",
    sheet_name="CurrBase Output",
)

# Select relevant data
idx_year = immed_need[immed_need["OutputYear"] == 2016].index
immed_need_distribution = immed_need.loc[
    idx_year, immed_need.columns[np.r_[1, 3, 49:70]]
]
immed_need_distribution.dropna(inplace=True)

# Add column 'Facility_Level'
immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "UrbHC"].index,
    "Facility_Level",
] = "Facility_Level_1a"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "RurHC"].index,
    "Facility_Level",
] = "Facility_Level_1a"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "ComHos"].index,
    "Facility_Level",
] = "Facility_Level_1b"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "DisHos"].index,
    "Facility_Level",
] = "Facility_Level_2"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "CenHos"].index,
    "Facility_Level",
] = "Facility_Level_3"
...
immed_need_distribution = pd.melt(
    immed_need_distribution,
    id_vars="Facility_Level",
    value_vars=cols_matter[1:],
    var_name="Cadre_ImmedNeed",
    value_name="Staff_Count_ImmedNeed",
)

# Add column 'Cadre_Code'
for i in range(63):
    immed_need_distribution.loc[i, "Cadre_Code"] = str(
        immed_need_distribution.loc[i, "Cadre_ImmedNeed"]
    )[7:10]
...
for i in range(21):
    if (
        ...
        immed_need_distribution.loc[3 * i, "Proportion_ImmedNeed"] = (
            ...
            / immed_need_distribution.loc[
                3 * i : 3 * i + 2, "Staff_Count_ImmedNeed"
            ].sum()
        )
...
idx_level0 = curr_staff_distribution[
    curr_staff_distribution["Facility_Level"] == "Facility_Level_0"
].index
compare_staff_distribution = curr_staff_distribution.drop(
    idx_level0, axis=0, inplace=False
).copy()
# Merge
compare_staff_distribution = curr_staff_distribution.merge(
    opt_workforce_distribution, how="right"
)

# Check before adding ImmedNeed data
assert (
    compare_staff_distribution["Cadre_Code"] == immed_need_distribution["Cadre_Code"]
).all()
assert (
    compare_staff_distribution["Facility_Level"]
    == immed_need_distribution["Facility_Level"]
).all()
# Add Staff_Count_ImmedNeed and Proportion_ImmedNeed to the merged table
compare_staff_distribution["Staff_Count_ImmedNeed"] = immed_need_distribution[
    "Staff_Count_ImmedNeed"
].copy()
compare_staff_distribution["Proportion_ImmedNeed"] = immed_need_distribution[
    "Proportion_ImmedNeed"
].copy()

# Add fund data
# First, drop rows of level_0 of fund_staff_distribution
fund_idx_level0 = fund_staff_distribution[
    fund_staff_distribution["Facility_Level"] == "Facility_Level_0"
].index
fund_staff_distribution_nolevel0 = fund_staff_distribution.drop(
    fund_idx_level0, axis=0, inplace=False
).copy()
fund_staff_distribution_nolevel0.reset_index(drop=True, inplace=True)
# Check before combination
assert (
    compare_staff_distribution["Cadre_Code"]
    == fund_staff_distribution_nolevel0["Cadre_Code"]
).all()
assert (
    compare_staff_distribution["Facility_Level"]
    == fund_staff_distribution_nolevel0["Facility_Level"]
).all()
# Add Number of positions and Proportion_Fund to the merged table
compare_staff_distribution.insert(
    4,
    "Staff_Count_Fund",
    fund_staff_distribution_nolevel0["Number of positions"].values,
)
compare_staff_distribution.insert(
    5, "Proportion_Fund", fund_staff_distribution_nolevel0["Proportion_Fund"].values
)
...
fund_staffing_table.loc[
    fund_staffing_table["District_Or_Hospital"] == "HQ", "District_Or_Hospital"
] = "Headquarter"
fund_staffing_table.loc[
    fund_staffing_table["District_Or_Hospital"] == "KCH", "District_Or_Hospital"
] = "Referral Hospital_Central"
fund_staffing_table.loc[
    fund_staffing_table["District_Or_Hospital"] == "MCH", "District_Or_Hospital"
] = "Referral Hospital_Northern"
fund_staffing_table.loc[
    fund_staffing_table["District_Or_Hospital"] == "QECH", "District_Or_Hospital"
] = "Referral Hospital_Southern"
# fund_staffing_table.loc[
# fund_staffing_table['District_Or_Hospital'] == 'QECH', 'District_Or_Hospital'] = 'Referral Hospital_Southwest'
fund_staffing_table.loc[
    fund_staffing_table["District_Or_Hospital"] == "ZCH", "District_Or_Hospital"
] = "Referral Hospital_Southern"
# fund_staffing_table.loc[
# fund_staffing_table['District_Or_Hospital'] == 'ZCH', 'District_Or_Hospital'] = 'Referral Hospital_Southeast'
fund_staffing_table.loc[
    fund_staffing_table["District_Or_Hospital"] == "ZMH", "District_Or_Hospital"
] = "Zomba Mental Hospital"

# Group the referral hospitals QECH and ZCH as Referral Hospital_Southern
Is_DistrictLevel = fund_staffing_table[
    "Is_DistrictLevel"
].values  # Save the column 'Is_DistrictLevel' first
fund_staffing_table = pd.DataFrame(
    fund_staffing_table.groupby(by=["District_Or_Hospital"], sort=False).sum()
).reset_index()
fund_staffing_table.insert(
    1, "Is_DistrictLevel", Is_DistrictLevel[:-1]
)  # Add the column 'Is_DistrictLevel'
...
for i in np.arange(0, len(split_districts)):
    ...
    cols = set(fund_staffing_table.columns).intersection(
        set(officer_types_table.Officer_Type_Code)
    )

    total_staff = fund_staffing_table.loc[
        fund_staffing_table["District_Or_Hospital"] == super_district, cols
    ].values.squeeze()

    # get the weight; The original weights w0 for the 4 new districts in order are 0.60,0.24,0.14,1.77(> 1)
    w0 = (
        pop_by_district.loc[new_district, "Count"]
        / pop_by_district.loc[super_district, "Count"]
    )
    if w0 < 1:
        w = w0
    else:
...
assert set(pop["District"].values) == set(
    fund_staffing_table.loc[
        fund_staffing_table["Is_DistrictLevel"], "District_Or_Hospital"
    ]
)
...
chai_districts = pd.DataFrame(
    {
        "District": fund_staffing_table.loc[
            fund_staffing_table["Is_DistrictLevel"], "District_Or_Hospital"
        ]
    }
)
...
district_faclevel = pd.DataFrame(
    columns=[
        "District_Or_Hospital",
        "Facility_Level_0",
        "Facility_Level_1a",
        "Facility_Level_1b",
        "Facility_Level_2",
...
district_faclevel = pd.melt(
    district_faclevel,
    id_vars="District_Or_Hospital",
    value_vars=district_faclevel.columns[1:],
    var_name="Facility_Level",
)
...
for district in pop["District"]:
    for cadre in set(fund_staffing_table.columns[3:]):
        # The proportions
        weight = fund_staff_distribution.loc[
            fund_staff_distribution["Cadre_Code"] == cadre,
            ["Facility_Level", "Proportion_Fund"],
        ].copy()
        # The staff count before splitting
        old_count = fund_staffing_table.loc[
            fund_staffing_table["District_Or_Hospital"] == district,
            ["Facility_Level", cadre],
        ].copy()

        # Check that Facility levels of weight and old_count are consistent
        assert (
            weight["Facility_Level"].values == old_count["Facility_Level"].values
        ).all()
        ...
        # Split
        fund_staffing_table.loc[
            fund_staffing_table["District_Or_Hospital"] == district, cadre
        ] = (old_count[cadre].values * weight["Proportion_Fund"].values)

# Add facility levels for HQ, CenHos and ZMH
fund_staffing_table.loc[128:132, "Facility_Level"] = [
    "Facility_Level_5",
    "Facility_Level_3",
    "Facility_Level_3",
    "Facility_Level_3",
...
fund_staffing_table_to_save = fund_staffing_table.drop(
    columns="Is_DistrictLevel", inplace=False
)
...
curr_staffing_table.loc[
    curr_staffing_table["District_Or_Hospital"] == "HQ", "District_Or_Hospital"
] = "Headquarter"
curr_staffing_table.loc[
    curr_staffing_table["District_Or_Hospital"] == "KCH", "District_Or_Hospital"
] = "Referral Hospital_Central"
curr_staffing_table.loc[
    curr_staffing_table["District_Or_Hospital"] == "MCH", "District_Or_Hospital"
] = "Referral Hospital_Northern"
curr_staffing_table.loc[
    curr_staffing_table["District_Or_Hospital"] == "QECH", "District_Or_Hospital"
] = "Referral Hospital_Southern"
curr_staffing_table.loc[
    curr_staffing_table["District_Or_Hospital"] == "ZCH", "District_Or_Hospital"
] = "Referral Hospital_Southern"
curr_staffing_table.loc[
    curr_staffing_table["District_Or_Hospital"] == "ZMH", "District_Or_Hospital"
] = "Zomba Mental Hospital"

# Group the referral hospitals QECH and ZCH as Referral Hospital_Southern
Is_DistrictLevel = curr_staffing_table[
    "Is_DistrictLevel"
].values  # Save the column 'Is_DistrictLevel' first
curr_staffing_table = pd.DataFrame(
    curr_staffing_table.groupby(by=["District_Or_Hospital"], sort=False).sum()
).reset_index()
curr_staffing_table.insert(
    1, "Is_DistrictLevel", Is_DistrictLevel[:-1]
)  # Add the column 'Is_DistrictLevel'
...
for i in np.arange(0, len(split_districts)):
    ...
    # get total staff level from the super districts
    cols = set(curr_staffing_table.columns).intersection(
        set(officer_types_table.Officer_Type_Code)
    )

    total_staff = curr_staffing_table.loc[
        curr_staffing_table["District_Or_Hospital"] == super_district, cols
    ].values.squeeze()
    ...
    w0 = (
        pop_by_district.loc[new_district, "Count"]
        / pop_by_district.loc[super_district, "Count"]
    )  # The values in order are 0.05,0.60,0.24,0.14,1.77
    if w0 < 1:
        w = w0
    else:
        w = 0.5
...
assert set(pop["District"].values) == set(
    curr_staffing_table.loc[
        curr_staffing_table["Is_DistrictLevel"], "District_Or_Hospital"
    ]
)
assert len(pop["District"].values) == len(
    curr_staffing_table.loc[
...
chai_districts = pd.DataFrame(
    {
        "District": curr_staffing_table.loc[
            curr_staffing_table["Is_DistrictLevel"], "District_Or_Hospital"
        ]
    }
)
...
for district in pop["District"]:
    for cadre in set(curr_staffing_table.columns[3:]):
        # The proportions
        weight = curr_staff_distribution.loc[
            curr_staff_distribution["Cadre_Code"] == cadre,
            ["Facility_Level", "Proportion"],
        ].copy()
        # The staff count before splitting
        old_count = curr_staffing_table.loc[
            curr_staffing_table["District_Or_Hospital"] == district,
            ["Facility_Level", cadre],
        ].copy()
        ...
        # Split
        curr_staffing_table.loc[
            curr_staffing_table["District_Or_Hospital"] == district, cadre
        ] = (old_count[cadre].values * weight["Proportion"].values)

# Add facility levels for HQ, CenHos and ZMH
curr_staffing_table.loc[128:133, "Facility_Level"] = [
    "Facility_Level_5",
    "Facility_Level_3",
    "Facility_Level_3",
    "Facility_Level_3",
...
# Save the table without column 'Is_DistrictLevel'; staff counts in floats
curr_staffing_table_to_save = curr_staffing_table.drop(
    columns="Is_DistrictLevel", inplace=False
)
...
pop_districts = pop[
    "District"
].values  # array; the 'pop_districts' used in previous lines is a DataFrame
...
for d in pop_districts:
    df = pd.DataFrame(
        {
            "Facility_Level": Facility_Levels[0:4],
            "District": d,
...
name.loc[mfl["Facility_Level"] == 3] = (
    "Referral Hospital" + "_" + mfl.loc[mfl["Facility_Level"] == 3, "Region"]
)
...
# Save
mfl.to_csv(
    outputlocation / "organisation" / "ResourceFile_Master_Facilities_List.csv",
    index=False,
)
...
for d in pop_districts:
    ...
    district_facs = mfl.loc[
        mfl["District"] == d
    ]  # Include facs from level 0 to level 2

    region_fac = (
        mfl.loc[pd.isnull(mfl["District"]) & (mfl["Region"] == the_region)]
        .copy()
    ...
    zmh_fac = (
        mfl.loc[
            pd.isnull(mfl["District"])
            & pd.isnull(mfl["Region"])
            & (mfl["Facility_Name"] == "Zomba Mental Hospital")
        ]
        .copy()
        .reset_index(drop=True)
    )
    ...
    facilities_by_district = pd.concat(
        [facilities_by_district, district_facs, region_fac, zmh_fac, headquarter_fac],
        ignore_index=True,
    )
...
data_import = pd.DataFrame(
    data=trimmed.iloc[1:, 2:].values,
    columns=trimmed.iloc[0, 2:],
    index=trimmed.iloc[1:, 1],
)

data_import = data_import.dropna(
    axis="columns", how="all"
)  # get rid of the 'spacer' columns
...
new_appt_for_CHW = pd.Series(
    index=data_import.index,
...
new_appt_for_pharmdispensing_p03 = pd.Series(
    index=data_import.index,
...
new_appt_for_pharmdispensing_p02 = pd.Series(
    index=data_import.index,
...
data_import = pd.concat(
    [data_import, new_appt_for_pharmdispensing_p03, new_appt_for_pharmdispensing_p02],
    axis=1,
)
...
new_rows_for_ZMH = pd.DataFrame(
    index=["ZMH", "ZMH_Per"], columns=data_import.columns.copy(), data=0
)
new_rows_for_ZMH.loc[:, ["C01_MentOPD", "C01_MentClinic"]] = (
    data_import.loc[["CenHos", "CenHos_Per"], ["C01_MentOPD", "C01_MentClinic"]]
    .copy()
    .values
)
...
assert set(chai_code["Officer_Type_Code"]).issubset(
    set(officer_types_table["Officer_Type_Code"])
)
...
appt_types_table_import = (
    sheet.loc[(1, 2, 6), 2:].transpose().reset_index(drop=True).copy()
)
appt_types_table_import = appt_types_table_import.rename(
    columns={1: "Appt_Cat", 2: "Appt_Type", 6: "Appt_Type_Code"}
)
appt_types_table_import["Appt_Cat"] = pd.Series(
    appt_types_table_import["Appt_Cat"]
).fillna(method="ffill")
appt_types_table_import["Appt_Type"] = pd.Series(
    appt_types_table_import["Appt_Type"]
).fillna(method="ffill")
appt_types_table_import["Appt_Type_Code"] = pd.Series(
    appt_types_table_import["Appt_Type_Code"]
).fillna(method="ffill")
appt_types_table_import = appt_types_table_import.drop_duplicates().reset_index(
    drop=True
)

# starting with the retained appt codes, merge in these descriptions
appt_types_table = pd.DataFrame(data={"Appt_Type_Code": retained_appt_type_code}).merge(
    appt_types_table_import, on="Appt_Type_Code", how="left", indicator=True
)

# Fill in the missing information about the appointment type that was added above
appt_types_table.loc[
    appt_types_table["Appt_Type_Code"] == new_appt_for_CHW.name.split("_")[1],
    "Appt_Cat",
...
appt_types_table.loc[
    appt_types_table["Appt_Type_Code"]
    == new_appt_for_pharmdispensing_p03.name.split("_")[1],
    "Appt_Cat",
] = new_appt_for_pharmdispensing_p03.name.split("_")[1]
appt_types_table.loc[
    appt_types_table["Appt_Type_Code"]
...
appt_types_table["Appt_Cat"].replace(
    to_replace="  ", value="_", regex=True, inplace=True
)
appt_types_table["Appt_Cat"].replace(
    to_replace=" ", value="_", regex=True, inplace=True
)
...
Avg_Level1a_ExpectTime = (
    Urban_HealthCentre_ExpecTime + Rural_HealthCentre_ExpecTime
) / 2
...
ApptTimeTable = pd.melt(
    Y,
    id_vars=["Officer_Type_Code", "Appt_Type_Code"],
    var_name="Facility_Level",
    value_name="Time_Taken_Mins",
)
...
ApptTimeTable = ApptTimeTable.drop(
    ApptTimeTable[ApptTimeTable["Time_Taken_Mins"] == 0].index
)
ApptTimeTable = ApptTimeTable.drop(
    ApptTimeTable[pd.isnull(ApptTimeTable["Time_Taken_Mins"])].index
)
# reset index
...
appt_time_table_coarse = pd.DataFrame(
    ApptTimeTable.groupby(
        ["Appt_Type_Code", "Facility_Level", "Officer_Category"]
    ).sum()
).reset_index()
...
ApptType_By_FacLevel = pd.DataFrame(
    index=appt_types_table["Appt_Type_Code"],
...
for appt_type in ApptType_By_FacLevel.index:
    for fac_level in ApptType_By_FacLevel.columns:
        ...
        ApptType_By_FacLevel.at[appt_type, fac_level] = (
            (ApptTimeTable["Facility_Level"] == fac_level)
            & (ApptTimeTable["Appt_Type_Code"] == appt_type)
        ).any()
...
appt_type_by_level_coarse = pd.DataFrame(
    index=appt_types_table["Appt_Type_Code"],
    columns=Facility_Levels,
    data=False,
    dtype=bool,
...
for appt_type in appt_type_by_level_coarse.index:
    for fac_level in appt_type_by_level_coarse.columns:
        ...
        appt_type_by_level_coarse.at[appt_type, fac_level] = (
            ...
            & (appt_time_table_coarse["Appt_Type_Code"] == appt_type)
        ).any()
...
Officers_Need_For_Appt = pd.DataFrame(
    columns=["Facility_Level", "Appt_Type_Code", "Officer_Type_Codes"]
)
...
FacLevel_By_Officer = pd.DataFrame(
    columns=Facility_Levels, index=officer_types_table["Officer_Type_Code"].values
)
...
for o in officer_types_table["Officer_Type_Code"].values:
    ...
    for i in Officers_Need_For_Appt.index:
        ...
        if (
            officer_types is not False
        ):  # (i.e. such an appointment at such a a facility is possible)

            if o in officer_types:
                FacLevel_By_Officer.loc[(FacLevel_By_Officer.index == o), fac_level] = (
                    True
                )
...
fund_staff_HQ = fund_staffing_table[
    fund_staffing_table["District_Or_Hospital"] == "Headquarter"
].copy()
curr_staff_HQ = curr_staffing_table[
    curr_staffing_table["District_Or_Hospital"] == "Headquarter"
].copy()
fund_staff_HQ.drop(
    columns=["District_Or_Hospital", "Facility_Level", "Is_DistrictLevel"], inplace=True
)
curr_staff_HQ.drop(
    columns=["District_Or_Hospital", "Facility_Level", "Is_DistrictLevel"], inplace=True
)
...
fr_nonpregwomen = (
    pft_sheet.iloc[68, np.arange(2, 23)] - pft_sheet.iloc[71, np.arange(2, 23)]
)

# Total average working days
workingdays = (
    (fr_men * days_per_year_men)
    + (fr_nonpregwomen * days_per_year_women)
...
fr_pregwomen_old = (
    pft_old.iloc[55, np.arange(2, 23)] * pft_old.iloc[57, np.arange(2, 23)]
)
fr_nonpregwomen_old = pft_old.iloc[55, np.arange(2, 23)] * (
    1 - pft_old.iloc[57, np.arange(2, 23)]
)

# Total average working days
working_days_old = (
    (fr_men_old * days_men_old)
    + (fr_nonpregwomen_old * days_women_old)
...
mins_daily_hc_old = (
    pft_old.iloc[26, np.arange(2, 23)] - pft_old.iloc[34, np.arange(2, 23)]
)
...
assert (
    HosHC_patient_facing_time_old["Officer_Type_Code"]
    == HosHC_patient_facing_time["Officer_Type_Code"]
).all()
assert (
    HosHC_patient_facing_time_old.columns == HosHC_patient_facing_time.columns
).all()

# check new and old pft difference
HosHC_pft_diff = pd.DataFrame(columns=HosHC_patient_facing_time.columns)
HosHC_pft_diff["Officer_Type_Code"] = HosHC_patient_facing_time[
    "Officer_Type_Code"
].values
...
HosHC_pft_diff = HosHC_pft_diff.append(
    HosHC_pft_diff.iloc[:, 1:].mean(axis=0), ignore_index=True
)
...
HosHC_patient_facing_time.iloc[11:, :] = HosHC_patient_facing_time_old.iloc[
    11:, :
].copy()
...
for i in funded_daily_minutes.index:
    ...
    for officer in officer_types_table["Officer_Type_Code"]:
        if the_level in [
            "Facility_Level_0",
            "Facility_Level_1a",
        ]:  # Levels 0, 1a; HC minutes
            t = (
                funded_staff_floats.loc[i, officer]
                * HosHC_patient_facing_time.loc[
                    HosHC_patient_facing_time["Officer_Type_Code"] == officer,
                    "HC_Av_Mins_Per_Day",
                ]
            )
...
# Long format
funded_staff_floats = pd.melt(
    funded_staff_floats,
    id_vars=["District_Or_Hospital", "Facility_Level"],
    var_name="Officer_Type_Code",
    value_name="Staff_Count",
)
funded_daily_minutes = pd.melt(
    funded_daily_minutes,
    id_vars=["District_Or_Hospital", "Facility_Level"],
    var_name="Officer_Type_Code",
    value_name="Total_Mins_Per_Day",
)
...
assert funded_daily_capability[
    funded_daily_capability["Total_Mins_Per_Day"] == 0
].index.equals(
...
for i in funded_daily_capability.index:
    ...
    if the_level in ["0", "1a", "1b", "2"]:
        ...
        funded_daily_capability.loc[i, "Facility_Name"] = (
            "Facility_Level_" + str(the_level) + "_" + the_district
        )
    else:
        funded_daily_capability.loc[i, "Facility_Name"] = funded_daily_capability.loc[
            i, "District_Or_Hospital"
        ]
...
funded_daily_capability = funded_daily_capability.merge(
    officer_types_table, on="Officer_Type_Code", how="left"
)
...
funded_daily_capability_coarse = pd.DataFrame(
    funded_daily_capability.groupby(
        ...
        dropna=False,
    )[["Total_Mins_Per_Day", "Staff_Count"]].sum()
).reset_index()
...
assert set(mfl.Facility_ID) == set(
    funded_daily_capability_coarse.Facility_ID.drop_duplicates()
)
assert len(funded_daily_capability_coarse) == len(mfl) * len(
    officer_types_table.Officer_Category.drop_duplicates()
)
...
for i in curr_daily_minutes.index:
    ...
    for officer in officer_types_table["Officer_Type_Code"]:
        if the_level in [
            ...
            t = (
                curr_staff_floats.loc[i, officer]
                * HosHC_patient_facing_time.loc[
                    HosHC_patient_facing_time["Officer_Type_Code"] == officer,
                    "HC_Av_Mins_Per_Day",
                ]
...
# Long format
curr_staff_floats = pd.melt(
    curr_staff_floats,
    id_vars=["District_Or_Hospital", "Facility_Level"],
    var_name="Officer_Type_Code",
    value_name="Staff_Count",
)
curr_daily_minutes = pd.melt(
    curr_daily_minutes,
    id_vars=["District_Or_Hospital", "Facility_Level"],
    var_name="Officer_Type_Code",
    value_name="Total_Mins_Per_Day",
...
assert curr_daily_capability[
    curr_daily_capability["Total_Mins_Per_Day"] == 0
].index.equals(curr_daily_capability[curr_daily_capability["Staff_Count"] == 0].index)
# Reset index
...
for i in curr_daily_capability.index:
    ...
    if the_level in ["0", "1a", "1b", "2"]:
        ...
        curr_daily_capability.loc[i, "Facility_Name"] = (
            "Facility_Level_" + str(the_level) + "_" + the_district
        )
    else:
        curr_daily_capability.loc[i, "Facility_Name"] = curr_daily_capability.loc[
            i, "District_Or_Hospital"
        ]
# Drop column 'District_Or_Hospital'
...
# check that there are entries for all coarse cadres and all facility id
assert set(mfl.Facility_ID) == set(
    curr_daily_capability_coarse.Facility_ID.drop_duplicates()
)
assert len(curr_daily_capability_coarse) == len(mfl) * len(
    officer_types_table.Officer_Category.drop_duplicates()
)
...
def all_appts_can_run(capability):
    ...
    assert (
        appt_have_or_miss_capability["Time_Taken_Mins"] > 0
    ).all()  # ensure that each row/appt is required
        ...
        if L in Facility_Levels[0:4]:  # Levels 0, 1a, 1b, 2
            ...
            for D in pop_districts:
                idx = capability[
                    ...
                    & (capability["Total_Mins_Per_Day"] > 0)
                ].index
                if idx.size == 0:
            ...
            assert (
                0 == 1
            )  # There should be no 'else'; otherwise, the generated tables above is incorrect
...
def find_districts_with_no_required_hcw(capability, scenario):
    ...
    appt_miss_cap = appt_miss_cap.loc[
        ~(appt_miss_cap["fail_district"] == "All districts pass")
    ]
    ...
    appt_miss_cap = appt_miss_cap.merge(
        fail_district_split, how="outer", left_index=True, right_index=True
    )

    appt_miss_cap = pd.melt(
        appt_miss_cap,
...
df_funded = find_districts_with_no_required_hcw(
    funded_daily_capability_coarse, "funded"
)
appts_with_no_required_hcw = pd.merge(
    df_actual,
    df_funded,
    on=["Appt_Type_Code", "Facility_Level", "Officer_Category"],
    how="outer",
)
appts_with_no_required_hcw = pd.melt(
    appts_with_no_required_hcw,
    id_vars=["Appt_Type_Code", "Facility_Level", "Officer_Category"],
...
appts_with_no_required_hcw.to_csv(
    ...
    / "human_resources"
    / "definitions"
    / "ResourceFile_Appts_That_Require_HCW_Who_Are_Not_Present.csv",
    index=False,
)
...
funded_daily_capability_compare = funded_daily_capability_coarse.drop(
    columns=["Facility_ID", "Facility_Name"], inplace=False
).copy()
funded_daily_capability_compare.set_index(
    ["Facility_Level", "District", "Region", "Officer_Category"], inplace=True
)
funded_daily_capability_compare.rename(
    columns={
        "Total_Mins_Per_Day": "Funded_Total_Mins_Per_Day",
        "Staff_Count": "Funded_Staff_Count",
    },
    inplace=True,
)
curr_daily_capability_compare = curr_daily_capability_coarse.drop(
    columns=["Facility_ID", "Facility_Name"], inplace=False
).copy()
curr_daily_capability_compare.set_index(
    ["Facility_Level", "District", "Region", "Officer_Category"], inplace=True
)
curr_daily_capability_compare.rename(
    columns={
        "Total_Mins_Per_Day": "Curr_Total_Mins_Per_Day",
        "Staff_Count": "Curr_Staff_Count",
    },
    inplace=True,
)
diff_capability = funded_daily_capability_compare.join(
    curr_daily_capability_compare, how="outer"
)
diff_capability.fillna(0, inplace=True)
diff_capability["Diff_Staff_Count"] = (
    diff_capability["Funded_Staff_Count"] - diff_capability["Curr_Staff_Count"]
)
diff_capability["Diff_Total_Mins_Per_Day"] = (
    diff_capability["Funded_Total_Mins_Per_Day"]
