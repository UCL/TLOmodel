...
path_to_onedrive = Path(
    "/Users/jdbb1/Imperial College London/TLOModel - WP - Documents"
)  # <-- point to the TLO onedrive locally
...
officer_types_table.to_csv(
    outputlocation / "human_resources" / "definitions" / "ResourceFile_Officer_Types_Table.csv", index=False
)
...
compiled_staff_return = pd.read_excel(
    path_to_auxiliaryfiles / "Compiled Staff Returns.xlsx", sheet_name="Compiled Staff Returns", skiprows=range(5)
)
...
curr_staff_return.drop(
    curr_staff_return[curr_staff_return["Cadre"] == "Educ/Environ Health Officer"].index, inplace=True
)
...
for c in cadre_list:
    curr_staff_return.loc[curr_staff_return["Cadre"] == c, "Cadre_Code"] = (
        officer_types_table.loc[officer_types_table["Officer_Type"] == c, "Officer_Type_Code"].copy().values[0]
    )
...
cadre_faclevel = pd.melt(
    cadre_faclevel, id_vars="Cadre_Code", value_vars=cadre_faclevel.columns[1:], var_name="Facility_Level"
)
# Merge
...
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Cadre"] == "Nursing Officer/Registered Nurse"].index, "Cadre"
] = "Nurse Officer"
...
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility"] == "Zomba Mental Hospital"].index, "Facility Type"
...
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Urban Health Center"].index, "Facility_Level"
] = "Facility_Level_1a"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Rural Health Center"].index, "Facility_Level"
] = "Facility_Level_1a"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Health Center (with maternity)"].index, "Facility_Level"
] = "Facility_Level_1a"
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Health Center (without maternity)"].index, "Facility_Level"
] = "Facility_Level_1a"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Rural/Community Hospital"].index, "Facility_Level"
] = "Facility_Level_1b"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "District Hospital"].index, "Facility_Level"
] = "Facility_Level_2"
fund_staff_2018.loc[
...
fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Central Hospital"].index, "Facility_Level"
] = "Facility_Level_3"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Zomba Mental Hospital"].index, "Facility_Level"
] = "Facility_Level_4"

fund_staff_2018.loc[
    fund_staff_2018[fund_staff_2018["Facility Type"] == "Headquarters"].index, "Facility_Level"
] = "Facility_Level_5"
...
fund_cadre_faclevel = pd.melt(
    fund_cadre_faclevel, id_vars="Cadre_Code", value_vars=fund_cadre_faclevel.columns[1:], var_name="Facility_Level"
)
# Merge
...
opt_workforce = pd.read_excel(
    path_to_auxiliaryfiles / "MalawiOptimization_OUTPUT2022 SH 2019-10-19.xlsx", sheet_name="Sums by facility type"
)
# Drop redundant row
...
opt_workforce.insert(
    2,
    "Facility_Level",
    ["Facility_Level_3", "Facility_Level_1b", "Facility_Level_2", "Facility_Level_1a", "Facility_Level_1a"],
)
...
immed_need = pd.read_excel(
    path_to_auxiliaryfiles / "MalawiOptimization_OUTPUT_ALLYEARS_Curr.xlsx", sheet_name="CurrBase Output"
)
...
immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "UrbHC"].index, "Facility_Level"
] = "Facility_Level_1a"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "RurHC"].index, "Facility_Level"
] = "Facility_Level_1a"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "ComHos"].index, "Facility_Level"
] = "Facility_Level_1b"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "DisHos"].index, "Facility_Level"
] = "Facility_Level_2"

immed_need_distribution.loc[
    immed_need_distribution[immed_need_distribution["FacilityType"] == "CenHos"].index, "Facility_Level"
] = "Facility_Level_3"

# Group staff by levels
immed_need_distribution = pd.DataFrame(immed_need_distribution.groupby(by=["Facility_Level"], sort=False).sum()).drop(
    columns=["FacilityType", "FacilityName"]
)
# Drop level 3
...
for i in range(63):
    ...
    if compare_staff_distribution.loc[i, "Proportion_ImmedNeed"] > 0:
        compare_staff_distribution.loc[i, "Curr_vs_ImmedNeed"] = (
            compare_staff_distribution.loc[i, "Proportion"] - compare_staff_distribution.loc[i, "Proportion_ImmedNeed"]
    ...
    if compare_staff_distribution.loc[i, "Proportion_Opt"] > 0:
        compare_staff_distribution.loc[i, "Fund_vs_Opt"] = (
            compare_staff_distribution.loc[i, "Proportion_Fund"] - compare_staff_distribution.loc[i, "Proportion_Opt"]
        ) / compare_staff_distribution.loc[i, "Proportion_Opt"]
...
fund_staffing_table.loc[fund_staffing_table["District_Or_Hospital"] == "HQ", "District_Or_Hospital"] = "Headquarter"
...
fund_staffing_table = (
    pd.DataFrame(fund_staffing_table.groupby(by=["District_Or_Hospital"], sort=False).sum())
    .reset_index()
    .drop(columns=["Is_DistrictLevel"])
)
...
for i in np.arange(0, len(split_districts)):
    ...
    fund_staffing_table.loc[fund_staffing_table["District_Or_Hospital"] == super_district, cols] = (
        fund_staffing_table.loc[fund_staffing_table["District_Or_Hospital"] == super_district, cols] - record.loc[cols]
    )
...
district_faclevel = pd.DataFrame(
    columns=["District_Or_Hospital", "Facility_Level_0", "Facility_Level_1a", "Facility_Level_1b", "Facility_Level_2"]
)
...
for district in pop["District"]:
    for cadre in set(fund_staffing_table.columns[3:]):
        ...
        weight = fund_staff_distribution.loc[
            fund_staff_distribution["Cadre_Code"] == cadre, ["Facility_Level", "Proportion_Fund"]
        ].copy()
        # The staff count before splitting
        old_count = fund_staffing_table.loc[
            fund_staffing_table["District_Or_Hospital"] == district, ["Facility_Level", cadre]
        ].copy()
...
curr_staffing_table.loc[curr_staffing_table["District_Or_Hospital"] == "HQ", "District_Or_Hospital"] = "Headquarter"
...
curr_staffing_table = (
    pd.DataFrame(curr_staffing_table.groupby(by=["District_Or_Hospital"], sort=False).sum())
    .reset_index()
    .drop(columns="Is_DistrictLevel")
)
...
for i in np.arange(0, len(split_districts)):
    ...
    # get the weight
    w0 = (
        pop_by_district.loc[new_district, "Count"] / pop_by_district.loc[super_district, "Count"]
    ...
    curr_staffing_table.loc[curr_staffing_table["District_Or_Hospital"] == super_district, cols] = (
        curr_staffing_table.loc[curr_staffing_table["District_Or_Hospital"] == super_district, cols] - record.loc[cols]
    )
...
for district in pop["District"]:
    for cadre in set(curr_staffing_table.columns[3:]):
        ...
        weight = curr_staff_distribution.loc[
            curr_staff_distribution["Cadre_Code"] == cadre, ["Facility_Level", "Proportion"]
        ].copy()
        # The staff count before splitting
        old_count = curr_staffing_table.loc[
            curr_staffing_table["District_Or_Hospital"] == district, ["Facility_Level", cadre]
        ].copy()
...
for r in pop_regions:
    df = pd.DataFrame({"Facility_Level": Facility_Levels[4], "District": None, "Region": r}, index=[0])
    mfl = pd.concat([mfl, df], ignore_index=True, sort=True)

# Add the ZMH
df = pd.DataFrame({"Facility_Level": Facility_Levels[5], "District": None, "Region": None}, index=[0])
mfl = pd.concat([mfl, df], ignore_index=True, sort=True)

# Add the HQ
df = pd.DataFrame({"Facility_Level": Facility_Levels[6], "District": None, "Region": None}, index=[0])
mfl = pd.concat([mfl, df], ignore_index=True, sort=True)
...
name.loc[mfl["Facility_Level"] == 3] = "Referral Hospital" + "_" + mfl.loc[mfl["Facility_Level"] == 3, "Region"]
...
for d in pop_districts:
    ...
    zmh_fac = (
        mfl.loc[
            pd.isnull(mfl["District"]) & pd.isnull(mfl["Region"]) & (mfl["Facility_Name"] == "Zomba Mental Hospital")
        ]
        .copy()
    ...
    headquarter_fac = (
        mfl.loc[pd.isnull(mfl["District"]) & pd.isnull(mfl["Region"]) & (mfl["Facility_Name"] == "Headquarter")]
        .copy()
        .reset_index(drop=True)
    )
    ...
    facilities_by_district = pd.concat(
        [facilities_by_district, district_facs, region_fac, zmh_fac, headquarter_fac], ignore_index=True
    )
...
new_rows_for_ZMH = pd.DataFrame(index=["ZMH", "ZMH_Per"], columns=data_import.columns.copy(), data=0)
new_rows_for_ZMH.loc[:, ["C01_MentOPD", "C01_MentClinic"]] = (
    data_import.loc[["CenHos", "CenHos_Per"], ["C01_MentOPD", "C01_MentClinic"]].copy().values
)
...
appt_types_table.loc[
    appt_types_table["Appt_Type_Code"] == new_appt_for_CHW.name.split("_")[1], "Appt_Cat"
] = new_appt_for_CHW.name.split("_")[1]
appt_types_table.loc[
    appt_types_table["Appt_Type_Code"] == new_appt_for_CHW.name.split("_")[1], "Appt_Type"
] = new_appt_for_CHW.name.split("_")[1]
appt_types_table.loc[
    appt_types_table["Appt_Type_Code"] == new_appt_for_pharmdispensing_p03.name.split("_")[1], "Appt_Cat"
] = new_appt_for_pharmdispensing_p03.name.split("_")[1]
appt_types_table.loc[
    appt_types_table["Appt_Type_Code"] == new_appt_for_pharmdispensing_p03.name.split("_")[1], "Appt_Type"
] = new_appt_for_pharmdispensing_p03.name.split("_")[1]
...
appt_types_table.to_csv(
    outputlocation / "human_resources" / "definitions" / "ResourceFile_Appt_Types_Table.csv", index=False
)
...
ApptTimeTable = pd.melt(
    Y, id_vars=["Officer_Type_Code", "Appt_Type_Code"], var_name="Facility_Level", value_name="Time_Taken_Mins"
)
...
appt_time_table_coarse = (
    pd.DataFrame(ApptTimeTable.groupby(["Appt_Type_Code", "Facility_Level", "Officer_Category"]).sum())
    .reset_index()
    .drop(columns=["Officer_Type", "Officer_Type_Code"])
)
...
appt_time_table_coarse.to_csv(
    outputlocation / "human_resources" / "definitions" / "ResourceFile_Appt_Time_Table.csv", index=False
)
...
ApptType_By_FacLevel = pd.DataFrame(
    index=appt_types_table["Appt_Type_Code"], columns=Facility_Levels, data=False, dtype=bool
)
...
for appt_type in ApptType_By_FacLevel.index:
    for fac_level in ApptType_By_FacLevel.columns:
        ...
        ApptType_By_FacLevel.at[appt_type, fac_level] = (
            (ApptTimeTable["Facility_Level"] == fac_level) & (ApptTimeTable["Appt_Type_Code"] == appt_type)
        ).any()
...
for a in appt_types_table["Appt_Type_Code"].values:
    for f in Facility_Levels:
        ...
        if len(block) == 0:
            ...
            df = pd.DataFrame(
                {"Facility_Level": f, "Appt_Type_Code": a, "Officer_Type_Codes": need_officer_types},
                index=range(len(block)),
            )
            Officers_Need_For_Appt = pd.concat([Officers_Need_For_Appt, df], ignore_index=True)

# Turn this into the the set of staff that are required for each type of appointment
FacLevel_By_Officer = pd.DataFrame(columns=Facility_Levels, index=officer_types_table["Officer_Type_Code"].values)
...
workingdays = (
    (fr_men * days_per_year_men) + (fr_nonpregwomen * days_per_year_women) + (fr_pregwomen * days_per_year_pregwomen)
)
...
# Total average working days
working_days_old = (
    (fr_men_old * days_men_old) + (fr_nonpregwomen_old * days_women_old) + (fr_pregwomen_old * days_pregwomen_old)
)
...
HosHC_pft_diff.iloc[:, 1:] = (
    HosHC_patient_facing_time.iloc[:, 1:].values - HosHC_patient_facing_time_old.iloc[:, 1:].values
        ...
        if the_level in ["Facility_Level_0", "Facility_Level_1a"]:  # Levels 0, 1a; HC minutes
            t = (
                ...
                * HosHC_patient_facing_time.loc[
                    HosHC_patient_facing_time["Officer_Type_Code"] == officer, "HC_Av_Mins_Per_Day"
                ]
            )
            ...
            t = (
                funded_staff_floats.loc[i, officer]
                * HosHC_patient_facing_time.loc[
                    HosHC_patient_facing_time["Officer_Type_Code"] == officer, "ComHos_Av_Mins_Per_Day"
                ]
            )
            funded_daily_minutes.loc[i, officer] = t.values[0]
            ...
            t = (
                funded_staff_floats.loc[i, officer]
                * HosHC_patient_facing_time.loc[
                    HosHC_patient_facing_time["Officer_Type_Code"] == officer, "DisHos_Av_Mins_Per_Day"
                ]
            )
            funded_daily_minutes.loc[i, officer] = t.values[0]
...
funded_daily_capability_coarse = pd.DataFrame(
    funded_daily_capability.groupby(
        ["Facility_ID", "Facility_Name", "Facility_Level", "District", "Region", "Officer_Category"], dropna=False
...
def find_districts_with_no_required_hcw(capability, scenario):
    ...
    appt_miss_cap = pd.melt(
        appt_miss_cap, id_vars=["Appt_Type_Code", "Facility_Level", "Officer_Category"], value_name=scenario
    )
...
appts_with_no_required_hcw = pd.merge(
    df_actual, df_funded, on=["Appt_Type_Code", "Facility_Level", "Officer_Category"], how="outer"
)
...
funded_daily_capability_compare.rename(
    columns={"Total_Mins_Per_Day": "Funded_Total_Mins_Per_Day", "Staff_Count": "Funded_Staff_Count"}, inplace=True
)
...
curr_daily_capability_compare.rename(
    columns={"Total_Mins_Per_Day": "Curr_Total_Mins_Per_Day", "Staff_Count": "Curr_Staff_Count"}, inplace=True
)
...
diff_capability["Diff_Total_Mins_Per_Day"] = (
    diff_capability["Funded_Total_Mins_Per_Day"] - diff_capability["Curr_Total_Mins_Per_Day"]
)
