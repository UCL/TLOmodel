...
)  # <-- point to the TLO dropbox locally
...
def estimate_beds_by_facility_id(
    beds_by_district: pd.Series, beds_by_level: pd.Series
) -> pd.Series:
    ...
    beds_by_district = beds_by_district.append(
        pd.Series(
            {
    ...
    pc_of_beds_by_region = {
        _region: (
            beds_by_district[
                beds_by_district.index.isin(districts_in_region[_region])
            ].sum()
            / beds_by_district.sum()
        )
    ...
    for _fac_id, _info in fac_id.iterrows():
        ...
        if _info.Facility_Level == "0":
            ...
            _num_beds = (
                pc_of_beds_by_district[_info.District]
                * beds_by_level[_info.Facility_Level]
            )

        elif _info.Facility_Level == "3":
            _num_beds = (
                pc_of_beds_by_region[_info.Region] * beds_by_level[_info.Facility_Level]
            )
    ...
    x = pd.Series(
        data=beds_by_fac_id
    )  # Make into pd.Series to make these logical checks easier.
...
beds_by_district = tab501.iloc[19:][
    ["", "all_beds", "maternity_beds", "kangaroo_beds", "delivery_beds"]
]
...
def compute_bed_number_custom_type(_df: pd.DataFrame, _bed_types: dict) -> pd.DataFrame:
    ...
    for _custom_type, _types_to_combine in _bed_types.items():
        if _types_to_combine is not None:
            ...
            df[_custom_type] = _df["all_beds"] - _df[
                {x for v in _bed_types.values() if v is not None for x in v}
            ].sum(axis=1)
...
for _bed_type in bed_types:
    print(
        f"{_bed_type}: difference={beds_by_level[_bed_type].sum() - beds_by_district[_bed_type].sum()}"
    )
...
beds_by_district = (
    ...
    .groupby(by="District")[list(bed_types.keys())]
    .sum()
    .sum(axis=1)
)  # excluding those at the regional level
beds_per_10k = beds_by_district.div(pop_by_district / 10_000)

# Load comparator data:
tab38 = pd.read_excel(
    workingfile, sheet_name="Table_38"
)  # <-- "Malawi Harmonized Health Facility Assesement 2018/2019 Report"
tab38.columns = clean_up_index(tab38.columns)
tab38 = tab38.rename(columns={tab38.columns[0]: "District"})
in_patient_beds_per_10k = tab38[
    ["District", "number_of_inpatient_beds/10_000_population"]
].copy()
...
in_patient_beds_per_10k = in_patient_beds_per_10k.sort_index()[
    "number_of_inpatient_beds/10_000_population"
]
...
beds_by_region_and_level = (
    num_beds.merge(mfl, left_index=True, right_on="Facility_ID")
    .groupby(by=["Region", "Facility_Level"])[list(bed_types.keys())]
    .sum()
    .sum(axis=1)
)
...
bed_by_type_and_level = (
    num_beds.merge(mfl, left_index=True, right_on="Facility_ID")
    .groupby(by=["Facility_Level"])[list(bed_types.keys())]
    .sum()
)
