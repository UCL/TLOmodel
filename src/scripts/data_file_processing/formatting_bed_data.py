...
workingfile = path_to_dropbox / "05 - Resources" / "Module-healthsystem" / "beds" / "extracted_data_on_beds.xlsx"
...
def estimate_beds_by_facility_id(beds_by_district: pd.Series, beds_by_level: pd.Series) -> pd.Series:
    ...
    cities = {"Lilongwe": "Lilongwe City", "Blantyre": "Blantyre City", "Zomba": "Zomba City", "Mzimba": "Mzuzu City"}
    ...
    beds_by_district = beds_by_district.append(
        pd.Series({_city: 0.5 * total_for_district[_district] for _district, _city in cities.items()})
    )
...
def compute_bed_number_custom_type(_df: pd.DataFrame, _bed_types: dict) -> pd.DataFrame:
    ...
    for _custom_type, _types_to_combine in _bed_types.items():
        if _types_to_combine is not None:
            ...
            df[_custom_type] = _df["all_beds"] - _df[{x for v in _bed_types.values() if v is not None for x in v}].sum(
                axis=1
            )
...
num_beds = pd.DataFrame(
    {
        _bed_type: estimate_beds_by_facility_id(
            beds_by_district=beds_by_district[_bed_type], beds_by_level=beds_by_level[_bed_type]
        )
        for _bed_type in bed_types
    }
)
...
beds_by_district = (
    num_beds.merge(mfl[["Facility_ID", "District"]], left_index=True, right_on="Facility_ID")
