...
sim.modules["BladderCancer"].parameters[
    "init_prop_blood_urine_bladder_cancer_by_stage"
] = [0.0] * 4
sim.modules["BladderCancer"].parameters[
    "init_prop_with_blood_urine_diagnosed_bladder_cancer_by_stage"
] = [0.0] * 4
sim.modules["BladderCancer"].parameters["init_prop_treatment_status_bladder_cancer"] = [
    0.0
] * 4
...
treated = pd.DataFrame(
    df.loc[
        df.index.isin(cohort) & ~pd.isnull(df.bc_date_treatment), "bc_date_treatment"
    ].copy()
)
...
cohort_treated["bc_date_treatment"] = pd.to_datetime(
    cohort_treated["bc_date_treatment"]
)
cohort_treated["days_treatment_to_death"] = (
    cohort_treated["date"] - cohort_treated["bc_date_treatment"]
).dt.days
...
1 - (
    len(
        cohort_treated.loc[
            (cohort_treated["cause"] == "BladderCancer")
