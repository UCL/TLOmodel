...
counts.plot(y=["total_tis_t1", "total_t2p", "total_metastatic"])
...
results_no_healthsystem["dalys"].plot.bar(
    y=["YLD_BladderCancer_0", "YLL_BladderCancer_BladderCancer"], stacked=True
)
...
counts = results_with_healthsystem["total_counts_by_stage_over_time"][
    ["total_tis_t1", "total_t2p", "total_metastatic"]
].iloc[-1]
totpopsize = (
    results_with_healthsystem["total_counts_by_stage_over_time"][
        ["total_none", "total_tis_t1", "total_t2p", "total_metastatic"]
    ]
    .iloc[-1]
    .sum()
)
