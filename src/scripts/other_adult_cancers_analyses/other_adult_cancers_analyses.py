...
def get_summary_stats(logfile):
    ...
    summary = {
        "total": counts_by_stage[
            get_cols_excl_none(counts_by_stage.columns, "total_")
        ].sum(axis=1),
        "udx": counts_by_stage[
            get_cols_excl_none(counts_by_stage.columns, "undiagnosed_")
        ].sum(axis=1),
        "dx": counts_by_stage[
            get_cols_excl_none(counts_by_stage.columns, "diagnosed_")
        ].sum(axis=1),
        "tr": counts_by_stage[
            get_cols_excl_none(counts_by_stage.columns, "treatment_")
        ].sum(axis=1),
        "pc": counts_by_stage[
            get_cols_excl_none(counts_by_stage.columns, "palliative_")
        ].sum(axis=1),
    }
    ...
    deaths["age_group"] = deaths["age"].map(
        demography.Demography(resourcefilepath=resourcefilepath).AGE_RANGE_LOOKUP
    )
    ...
    death_counts_by_age_group = death_counts_by_age_group.reindex(
        pd.Index.intersection(
            make_age_grp_types().categories, death_counts_by_age_group.index
        )
    )

    # 5) Rates of diagnosis per year:
    counts_by_stage["year"] = counts_by_stage.index.year
    annual_count_of_dxtr = counts_by_stage.groupby(by="year")[
        [
            "diagnosed_since_last_log",
            "treated_since_last_log",
            "palliative_since_last_log",
        ]
    ].sum()
...
results_no_healthsystem["dalys"].plot.bar(y=[CAUSE_NAME], stacked=True)
...
totdeaths = pd.Series(
    index=pd.Index.intersection(make_age_grp_types().categories, deaths.index),
    data=np.nan,
)
...
counts = results_with_healthsystem["total_counts_by_stage_over_time"][
    ["total_site_confined", "total_local_ln", "total_metastatic"]
].iloc[-1]

totpopsize = (
    results_with_healthsystem["total_counts_by_stage_over_time"][
        ["total_none", "total_site_confined", "total_local_ln", "total_metastatic"]
    ]
    .iloc[-1]
    .sum()
)
...
comparison = compare_number_of_deaths(
    logfile=logfile_with_healthsystem, resourcefilepath=resourcefilepath
)
...
x = compare_number_of_deaths(
    logfile=logfile_no_healthsystem, resourcefilepath=resourcefilepath
)["model"]
