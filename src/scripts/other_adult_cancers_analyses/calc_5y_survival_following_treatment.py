...
treated = pd.DataFrame(
    df.loc[
        df.index.isin(cohort) & ~pd.isnull(df.oac_date_treatment), "oac_date_treatment"
    ].copy()
)
...
cohort_treated["oc_date_treatment"] = pd.to_datetime(
    cohort_treated["oac_date_treatment"]
)
cohort_treated["days_treatment_to_death"] = (
    cohort_treated["date"] - cohort_treated["oac_date_treatment"]
).dt.days
...
print(
    ...
    - (
        len(
            cohort_treated.loc[cohort_treated["days_treatment_to_death"] < (5 * 365.25)]
        )
        / len(cohort_treated)
    )
)
...
# cause):
condition = (cohort_treated["cause"] == "OtherAdultCancer") & (
    cohort_treated["days_treatment_to_death"] < (5 * 365.25)
)
1 - (len(cohort_treated.loc[condition]) / len(cohort_treated))  # 0.87
