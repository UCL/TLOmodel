...
def plot_staff_counts_by_cadre_across_scenarios_by_district(
        staff_counts_summary,
        output_folder
):
    ...
    districts = (
        staff_counts_summary.index
        .get_level_values("District")
        .unique()
    )

    cadres = (
        staff_counts_summary.index
        .get_level_values("Cadre")
        .unique()
    )

    for district in districts:

        district_df = staff_counts_summary.xs(
            district,
            level="District"
        )
        ...
        for cadre in cadres:
            ...
            if cadre not in district_df.index.get_level_values("Cadre"):
                continue
            ...
            for scenario in scenario_names:
                ...
                central = district_df[(scenario, "mean")].xs(
                    cadre,
                    level="Cadre"
                )

                lower = district_df[(scenario, "lower")].xs(
                    cadre,
                    level="Cadre"
                )

                upper = district_df[(scenario, "upper")].xs(
                    cadre,
                    level="Cadre"
                )
            ...
            ax.set_title(
                f"{cadre} Staff Counts Across Scenarios ({district})"
            )

            ax.set_xlabel("Year")
            ax.set_ylabel("Average Number of Health Workers")
            ...
            fig.savefig(
                output_folder /
                f"{district}_{cadre}_staff_counts_across_scenarios.png"
            )
...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    print(staff_counts_summary.index.names)
    ...
    # STEP 4: plot
    plot_staff_counts_by_cadre_across_scenarios_by_district(
        staff_counts_summary,
        output_folder
    )
