...
if [[ -z "$SCENARIO" || -z "$TIME" || -z "$MEMORY" || -z "$DRAWS" || -z "$RUNS" || -z "$SHIFT" || -z "$POPSIZE" ]]; then
  echo "Usage: $0 SCENARIO=<scenario> TIME=<time> MEMORY=<memory> DRAWS=<draws> RUNS=<runs> SHIFT=<shift> POPSIZE=<popsize>"
  exit 1
fi
...
qsub -t 1-${JOBS} -N wasting_analysis__full_model_${SCENARIO} \
 ...
 job_script.sh $SCENARIO $TIME $MEMORY $DRAWS $RUNS $SHIFT $POPSIZE
