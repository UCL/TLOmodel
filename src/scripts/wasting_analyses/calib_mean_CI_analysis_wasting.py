"""
An analysis file for the wasting module (mean over multiple runs, 1 draw)
"""
# %% Import statements
import glob
import gzip
...
class WastingAnalyses:
    """
    This class looks at plotting all important calibration outputs from the wasting module
    """
    ...
    def __init__(self, sim_results_folder_path_str, in_datestamp, in_draw_nmb, in_run_nmb, in_png=False):
        ...
        sim_results_folder_draw_x_run_0_path_str = self.outcomes_folder_path + f'/{draw_nmb}/{run_nmb}/'
        ...
        self.calib_outcomes_folder_path_name = (
            Path(self.outcomes_folder_path).parent / f"calib_outcome_figures_SQ-{self.datestamp}"
        )
   ...
   # def plot_wasting_prevalence_per_year(self):
    ...
    #     w_prev_calib_data_years_only_df = pd.DataFrame({
    #         'sev_wast_calib': [0.006, 0.007],
    #         'mod_wast_calib': [0.021, 0.019]
    #     }, index=[2016, 2020])
    #     date_range = pd.Index(range(2015, 2032), name='date')
    ...
    #     w_prev_plot_df = w_prev_df.loc[lambda df: df.index >= 2015]
    #     columns_to_plot = [
    ...
    def plot_wasting_prevalence_by_age_group(self):
        ...
        # init_w_prev_2010_only_df = init_w_prev_2010_only_df.set_index(init_w_prev_2010_only_df.date.dt.year)
        # init_w_prev_2010_only_df = init_w_prev_2010_only_df.drop(columns='date')
        # from year 2011-- now all from 2010 here
        ...
        # )[age_groups]
        # from year 2011 -- now all from 2010
...
if __name__ == "__main__":
    ...
    # Analyse each draw
    # for now, we always have just one draw, draw 0
    draw_nmb = 0
    for run_nmb in range(0, len(folders)):
        print(f"Analysing {run_nmb=} ...")
        time_start = time.time()

        print(f"\n(str(sim_results_folder_path): {str(sim_results_folder_path)}")
