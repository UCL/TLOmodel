...
class WastingScenarioAnalyses:
    """
    This class enables plotting of all important outcomes for a single scenario (i.e. one draw) with each run being
    plotted separately.
    """
    ...
    def __init__(self, in_scenario_type, sim_results_folder_path_str, in_datestamp, in_draw_nmb, in_png=False):
        ...
        runs_folders = [name for name in os.listdir(draw_folder_path) if \
                        os.path.isdir(os.path.join(draw_folder_path, name)) and name.isdigit()]
        self.total_runs = len(runs_folders)

        self.__w_logs_dict = {}
        for run_nmb in range(0, self.total_runs):
            print(f"run {run_nmb} ...")
            sim_results_folder_draw_run_path_str = sim_results_folder_path_str + f'/{draw_nmb}/{run_nmb}/'
            sim_results_file_name_prefix = scenario_filename
            sim_results_file_name_extension = '.log.gz'
            gz_results_file_path = Path(glob.glob(os.path.join(sim_results_folder_draw_run_path_str,
            ...
            # Parse wasting logs for each run
            self.__w_logs_dict[run_nmb] = parse_log_file(self.__log_file_path)['tlo.methods.wasting']
    ...
    def save_fig__store_pdf_file(self, fig, fig_output_name: str, run_nmb) -> None:
        full_path_and_file_name = self.outcomes_draw_folder_path_str + f'/{self.draw_nmb}/{run_nmb}/' + fig_output_name + \
                    f'_{self.draw_nmb}_{run_nmb}'
    ...
    def plot_wasting_incidence(self):
        ...
        for run_nmb in range(0, self.total_runs):
            w_inc_df = self.__w_logs_dict[run_nmb]['wasting_incidence_count']
            ...
            pop_sizes_df = self.__w_logs_dict[run_nmb]['pop sizes']
            ...
            self.save_fig__store_pdf_file(fig, fig_output_name, run_nmb)
            # plt.show()
    ...
    def plot_wasting_initial_overall_prevalence(self):
        ...
        for run_nmb in range(0, self.total_runs):
            ...
            init_w_prev_2010_only_df = self.__w_logs_dict[run_nmb]["wasting_init_prevalence_props"]
    ...
    def plot_wasting_initial_prevalence_by_age_group(self):
        ...
        for run_nmb in range(0, self.total_runs):
            # Initial prevalence at the beginning of 2010 - model
            w_prev_df = self.__w_logs_dict[run_nmb]["wasting_init_prevalence_props"]
    ...
    def plot_wasting_prevalence_per_year(self):
        ...
        for run_nmb in range(0, self.total_runs):
            ...
            w_prev_df = self.__w_logs_dict[run_nmb]["wasting_prevalence_props"]
    ...
    def plot_wasting_prevalence_by_age_group(self):
        ...
        for run_nmb in range(0, self.total_runs):
            ...
            w_prev_model_df = self.__w_logs_dict[run_nmb]["wasting_prevalence_props"]
            ...
            pop_sizes_model_df = self.__w_logs_dict[run_nmb]['pop sizes']
    ...
    def plot_all_figs_in_one_pdf(self, in_outcome_figs_folder):
        for run_nmb in range(0, self.total_runs):
            output_file_path = \
                in_outcome_figs_folder / f'wasting_all_figures__{self.datestamp}_{self.draw_nmb}_{run_nmb}.pdf'
