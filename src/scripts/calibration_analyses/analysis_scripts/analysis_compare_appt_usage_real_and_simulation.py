...
def get_annual_num_appts_by_level(results_folder: Path) -> pd.DataFrame:
    ...
    def get_counts_of_appts(_df):
        ...
        def unpack_nested_dict_in_series(_raw: pd.Series):
            return (
                pd.concat({idx: pd.DataFrame.from_dict(mydict) for idx, mydict in _raw.items()})
                .unstack()
                .fillna(0.0)
                .astype(int)
            )
        ...
        return (
            _df.loc[pd.to_datetime(_df["date"]).between(*TARGET_PERIOD), "Number_By_Appt_Type_Code_And_Level"]
...
def get_annual_num_appts_by_level_with_confidence_interval(results_folder: Path) -> pd.DataFrame:
    ...
    def get_counts_of_appts(_df):
        ...
        def unpack_nested_dict_in_series(_raw: pd.Series):
            return (
                pd.concat({idx: pd.DataFrame.from_dict(mydict) for idx, mydict in _raw.iteritems()})
                .unstack()
                .fillna(0.0)
                .astype(int)
            )
...
def get_real_usage(resourcefilepath, adjusted=True) -> pd.DataFrame:
    ...
    real_delivery = (
        real_usage.loc[(real_usage.Appt_Type == "Delivery") | (real_usage.Appt_Type == "Csection")]
        .groupby(["Year", "Month", "Facility_ID"])
        .agg({"Usage": "sum"})
        .reset_index()
    )
    real_delivery["Appt_Type"] = "Delivery"
    real_usage = pd.concat([real_usage.drop(real_usage[real_usage.Appt_Type == "Delivery"].index), real_delivery])
    ...
    real_usage_TB = real_usage_TB.merge(
        mfl[["Facility_ID", "Facility_Level"]], left_on="Facility_ID", right_on="Facility_ID"
    )
    ...
    annual_usage_by_level = (
        annual_usage_by_level.groupby(["Year", "Appt_Type", "Facility_Level"])["Usage"].sum().reset_index()
    )
    ...
    annual_usage_by_level_with_ci = pd.melt(
        annual_usage_by_level_with_ci, id_vars=["Appt_Type", "Facility_Level"], var_name="value_type"
    )
    annual_usage_by_level_with_ci.value_type = annual_usage_by_level_with_ci.value_type.replace(
    ...
    annual_usage_with_ci = pd.melt(annual_usage_with_ci, id_vars="Appt_Type", var_name="value_type")
...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    # format data
    rel_diff_all_levels = (simulation_usage.sum(axis=0) / real_usage.sum(axis=0)).clip(lower=0.1, upper=10.0)
    ...
    for idx in rel_diff_all_levels.index:
        if not pd.isna(rel_diff_all_levels[idx]):
            ax.text(
                idx, rel_diff_all_levels[idx] * (1 + 0.2), round(rel_diff_all_levels[idx], 1), ha="left", fontsize=8
            )
    format_and_save(fig, ax, name_of_plot, legend=False)

    # plot for each level
    rel_diff_by_levels = (simulation_usage / real_usage).clip(upper=10, lower=0.1).dropna(how="all", axis=0)

    name_of_plot = (
        "Model vs Data usage per appt type per facility level" "\n[Model average annual, Adjusted Data average annual]"
    )
    fig, ax = plt.subplots()
    marker_dict = {"0": 0, "1a": 4, "1b": 5, "2": 6, "3": 7, "4": 1}  # Note that level 0/3/4 has very limited data
    ...
    usage_all = (
        ...
        .melt(id_vars="Appt_Type", value_vars=["Model", "Data"], var_name="Type", value_name="Value")
    ...
    fig, ax = plt.subplots()
    ax.errorbar(
        rel_diff_real.index.values, rel_diff_real["mean"].values, err_real, fmt=".", capsize=3.0, label="All levels"
    )
    ...
    def format_simulation_usage_fraction():
        ...
        _usage_all_levels = (
            _usage.sum(axis=0).reset_index().rename(columns={0: "_usage_all_levels", "index": "appt_type"})
        )

        _usage = pd.melt(_usage.reset_index(), id_vars="index", var_name="appt_type", value_name="_usage").rename(
            columns={"index": "facility_level"}
        )

        _usage_fraction = _usage.merge(_usage_all_levels, on="appt_type", how="outer")
        _usage_fraction["ratio"] = _usage_fraction["_usage"] / _usage_fraction["_usage_all_levels"]
    ...
    def format_real_usage_fraction(adjusted=True):
        ...
        _usage_all_levels = (
            _usage.sum(axis=0).reset_index().rename(columns={0: "_usage_all_levels", "Appt_Type": "appt_type"})
        )

        _usage = pd.melt(
            _usage.reset_index(), id_vars="Facility_Level", var_name="appt_type", value_name="_usage"
    ...
    simulation_usage_plot.plot(kind="bar", stacked=True, width=0.3, edgecolor="dimgrey", hatch="", ax=ax, position=0)
    real_usage_plot.plot(kind="bar", stacked=True, width=0.25, edgecolor="dimgrey", hatch=".", ax=ax, position=1)
    unadjusted_real_usage_plot.plot(
        kind="bar", stacked=True, width=0.25, edgecolor="dimgrey", hatch="//", ax=ax, position=2
    )
    ...
    legend_1 = plt.legend(
        simulation_usage_plot.columns, loc="upper left", bbox_to_anchor=(1.0, 0.5), title="Facility Level"
    )
    ...
    plt.legend(
        handles=[patch_unadjusted_real, patch_real, patch_simulation], loc="lower left", bbox_to_anchor=(1.0, 0.6)
    )
    ...
    rel_diff_unadjusted_real.plot(
        kind="bar", yerr=err_unadjusted_real, width=0.4, ax=ax, position=0, bottom=1.0, legend=False, color="salmon"
    )
    rel_diff_real.plot(
        kind="bar", yerr=err_real, width=0.4, ax=ax, position=1, bottom=1.0, legend=False, color="yellowgreen"
    )
    ax.axhline(1.0, color="r")
