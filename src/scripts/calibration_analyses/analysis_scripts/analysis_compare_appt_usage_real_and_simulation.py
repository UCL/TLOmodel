    ...
    def get_counts_of_appts(_df):
        ...
        return (
            _df.loc[
                ...
                "Number_By_Appt_Type_Code_And_Level",
            ]
...
def get_annual_num_appts_by_level_with_confidence_interval(
    results_folder: Path,
) -> pd.DataFrame:
    """Return pd.DataFrame gives the (mean) simulated annual number of appointments of each type at each level,
        ...
        def unpack_nested_dict_in_series(_raw: pd.Series):
            return (
                pd.concat(
                    {
                        idx: pd.DataFrame.from_dict(mydict)
                        for idx, mydict in _raw.iteritems()
                    }
                )
                .unstack()
                .fillna(0.0)
...
def get_simulation_usage_by_level_with_confidence_interval(
    results_folder: Path,
) -> pd.DataFrame:
    ...
    # Get model outputs
    model_output = get_annual_num_appts_by_level_with_confidence_interval(
        results_folder=results_folder
    )

    # Reformat
    model_output.columns = [
        " ".join(col).strip() for col in model_output.columns.values
    ]
    model_output = model_output.melt(
        var_name="name", value_name="value", ignore_index=False
    )
...
def get_simulation_usage_with_confidence_interval(results_folder: Path) -> pd.DataFrame:
    ...
    # Get model outputs
    model_output = get_annual_num_appts_with_confidence_interval(
        results_folder=results_folder
    )
...
def adjust_real_usage_on_mentalall(real_usage_df) -> pd.DataFrame:
    ...
    rr = pd.DataFrame(
        index=["1a", "1b", "2", "3"],
        columns=list(range(2015, 2020)),
        data=[
            [44.00, 39.33, 79.00, 97.33, 95.00],
    ...
    for level in ["1a", "1b", "2", "3"]:
        for y in range(2015, 2020):
            real_usage_df.loc[
                (real_usage_df.Facility_Level == level)
                & (real_usage_df.Year == y)
                & (real_usage_df.Appt_Type == "MentalAll"),
                "Usage",
            ] = (
                real_usage_df.loc[
                    (real_usage_df.Facility_Level == level)
...
def get_real_usage(resourcefilepath, adjusted=True) -> pd.DataFrame:
    ...
    real_delivery = (
        real_usage.loc[
            (real_usage.Appt_Type == "Delivery") | (real_usage.Appt_Type == "Csection")
        ]
        .groupby(["Year", "Month", "Facility_ID"])
        .agg({"Usage": "sum"})
        .reset_index()
    )
    real_delivery["Appt_Type"] = "Delivery"
    real_usage = pd.concat(
        [
            real_usage.drop(real_usage[real_usage.Appt_Type == "Delivery"].index),
            real_delivery,
        ]
    )

    # get facility_level for each record
    real_usage = real_usage.merge(
        mfl[["Facility_ID", "Facility_Level"]],
        left_on="Facility_ID",
    ...
    real_usage["date"] = pd.to_datetime(
        {"year": real_usage["Year"], "month": real_usage["Month"], "day": 1}
    )

    # Produce table of the AVERAGE NUMBER PER YEAR DURING THE TIME_PERIOD of appointment type by level
    # limit to date
    totals_by_year = (
        real_usage.loc[real_usage["date"].between(*TARGET_PERIOD)]
        .groupby(["Year", "Appt_Type", "Facility_Level"])["Usage"]
        .sum()
    )
    ...
    real_usage_TB = pd.read_csv(
        ...
        / "healthsystem"
        / "real_appt_usage_data"
        / "real_yearly_usage_of_TBNotifiedAll.csv"
    )
    real_usage_TB = real_usage_TB.loc[real_usage_TB["Year"].isin([2017, 2018, 2019])]
    real_usage_TB = real_usage_TB.merge(
        mfl[["Facility_ID", "Facility_Level"]],
        left_on="Facility_ID",
        right_on="Facility_ID",
    )
    totals_by_year_TB = real_usage_TB.groupby(["Year", "Appt_Type", "Facility_Level"])[
        "Usage"
    ].sum()

    annual_usage_by_level = pd.concat(
        [totals_by_year.reset_index(), totals_by_year_TB.reset_index()], axis=0
    )

    # group levels 1b and 2 into 2
    annual_usage_by_level["Facility_Level"] = annual_usage_by_level[
        "Facility_Level"
    ].replace({"1b": "2"})
    annual_usage_by_level = (
        annual_usage_by_level.groupby(["Year", "Appt_Type", "Facility_Level"])["Usage"]
        .sum()
        .reset_index()
    )

    # prepare annual usage by level with mean, 97.5% percentile, and 2.5% percentile
    annual_usage_by_level_with_ci = (
        annual_usage_by_level.drop(columns="Year")
        .groupby(["Appt_Type", "Facility_Level"])
        .describe(percentiles=[0.025, 0.975])
        .stack(level=[0])[["mean", "2.5%", "97.5%"]]
        .reset_index()
        .drop(columns="level_2")
    )

    average_annual_by_level = (
        annual_usage_by_level_with_ci[["Appt_Type", "Facility_Level", "mean"]]
        .set_index(["Appt_Type", "Facility_Level"])
        .unstack()
    )
    average_annual_by_level.columns = average_annual_by_level.columns.get_level_values(
        1
    )
    average_annual_by_level = average_annual_by_level.T

    annual_usage_by_level_with_ci = pd.melt(
        annual_usage_by_level_with_ci,
        id_vars=["Appt_Type", "Facility_Level"],
        var_name="value_type",
    )
    annual_usage_by_level_with_ci.value_type = (
        annual_usage_by_level_with_ci.value_type.replace(
            {"2.5%": "lower", "97.5%": "upper"}
        )
    )

    # prepare annual usage at all levels with mean, 97.5% percentile, and 2.5% percentile
    annual_usage_with_ci = (
        annual_usage_by_level.groupby(["Year", "Appt_Type"])["Usage"]
        .sum()
        .reset_index()
        .drop(columns="Year")
        .groupby("Appt_Type")
    ...
    annual_usage_with_ci = pd.melt(
        annual_usage_with_ci, id_vars="Appt_Type", var_name="value_type"
    )
    annual_usage_with_ci.value_type = annual_usage_with_ci.value_type.replace(
        {"2.5%": "lower", "97.5%": "upper"}
    )
...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_{stub}.png"
    )  # noqa: E731
    ...
    appts_real_only = set(real_usage.columns.values) - set(
        simulation_usage.columns.values
    )
    appts_simulation_only = set(simulation_usage.columns.values) - set(
        real_usage.columns.values
    )

    # format data
    rel_diff_all_levels = (simulation_usage.sum(axis=0) / real_usage.sum(axis=0)).clip(
        lower=0.1, upper=10.0
    )

    # plot for all levels
    name_of_plot = (
        "Model vs Data usage per appt type at all facility levels"
        "\n[Model average annual, Adjusted Data average annual]"
    )
    fig, ax = plt.subplots()
    ax.stem(
        rel_diff_all_levels.index,
        rel_diff_all_levels.values,
        bottom=1.0,
        label="All levels",
    )
    for idx in rel_diff_all_levels.index:
        if not pd.isna(rel_diff_all_levels[idx]):
            ax.text(
                idx,
                rel_diff_all_levels[idx] * (1 + 0.2),
                round(rel_diff_all_levels[idx], 1),
                ha="left",
                fontsize=8,
            )
    ...
    name_of_plot = (
        "Model vs Data usage per appt type per facility level"
        "\n[Model average annual, Adjusted Data average annual]"
    )
    fig, ax = plt.subplots()
    marker_dict = {
        ...
        "3": 7,
        "4": 1,
    }  # Note that level 0/3/4 has very limited data
    ...
    simulation_usage_all = (
        simulation_usage.sum(axis=0)
        .reset_index()
        .rename(columns={"index": "Appt_Type", 0: "Model"})
    )
    usage_all = (
        simulation_usage_all.merge(real_usage_all, on="Appt_Type", how="inner")
        .melt(
            id_vars="Appt_Type",
            value_vars=["Model", "Data"],
            var_name="Type",
            value_name="Value",
        )
        .pivot(index="Type", columns="Appt_Type", values="Value")
    )
    ...
    def format_rel_diff(adjusted=True):
        def format_real_usage():
            ...
            _real_usage_all_levels = (
                _real_usage.sum(axis=0)
                .reset_index()
                .rename(columns={0: "real_usage_all_levels", "Appt_Type": "appt_type"})
            )
            return _real_usage_all_levels

        simulation_usage_all_levels_with_ci = (
            get_simulation_usage_with_confidence_interval(results_folder)
        )
        _rel_diff = simulation_usage_all_levels_with_ci.merge(
            format_real_usage(), on="appt_type", how="outer"
        )

        _rel_diff["ratio"] = _rel_diff["value"] / _rel_diff["real_usage_all_levels"]

        _rel_diff = (
            _rel_diff[["appt_type", "value_type", "ratio"]]
            .pivot(index="appt_type", columns="value_type", values="ratio")
        ...
        _asymmetric_error = [
            _rel_diff["lower_error"].values,
            _rel_diff["upper_error"].values,
        ]
    ...
    def format_simulation_usage_fraction():
        ...
        _usage_all_levels = (
            _usage.sum(axis=0)
            .reset_index()
            .rename(columns={0: "_usage_all_levels", "index": "appt_type"})
        )
        ...
        _usage_fraction["ratio"] = (
            _usage_fraction["_usage"] / _usage_fraction["_usage_all_levels"]
        )

        _usage_fraction = pd.pivot(
            _usage_fraction, index="appt_type", columns="facility_level", values="ratio"
        )

        # add nan rows of appts_real_only
        nan_df = pd.DataFrame(
            index=sorted(appts_real_only), columns=_usage_fraction.columns
        )
        _usage_fraction = pd.concat([_usage_fraction, nan_df]).sort_index()

        # make row of appts_simulation_only nan
        _usage_fraction.loc[_usage_fraction.index.isin(appts_simulation_only), :] = (
            np.NaN
        )
    ...
    def format_real_usage_fraction(adjusted=True):
        ...
        _usage_all_levels = (
            _usage.sum(axis=0)
            .reset_index()
            .rename(columns={0: "_usage_all_levels", "Appt_Type": "appt_type"})
        )

        _usage = pd.melt(
            _usage.reset_index(),
            id_vars="Facility_Level",
            var_name="appt_type",
            value_name="_usage",
        ...
        nan_df = pd.DataFrame(
            index=sorted(appts_simulation_only), columns=_usage_fraction.columns
        )
        _usage_fraction = pd.concat([_usage_fraction, nan_df]).sort_index()
    ...
    # plot
    name_of_plot = (
        "Model vs Data usage per appointment type on fraction per level"
        "\n[Model average annual, Adjusted & Unadjusted Data average annual]"
    )
    fig, ax = plt.subplots(figsize=(12, 5))
    simulation_usage_plot.plot(
        kind="bar",
        stacked=True,
        width=0.3,
        edgecolor="dimgrey",
    ...
    unadjusted_real_usage_plot.plot(
        kind="bar",
        stacked=True,
        width=0.25,
        edgecolor="dimgrey",
    ...
    legend_1 = plt.legend(
        simulation_usage_plot.columns,
        loc="upper left",
        bbox_to_anchor=(1.0, 0.5),
        title="Facility Level",
    )
    patch_simulation = matplotlib.patches.Patch(
        facecolor="lightgrey", hatch="", edgecolor="dimgrey", label="Model"
    )
    patch_real = matplotlib.patches.Patch(
        facecolor="lightgrey", hatch="...", edgecolor="dimgrey", label="Adjusted Data"
    )
    patch_unadjusted_real = matplotlib.patches.Patch(
        facecolor="lightgrey", hatch="///", edgecolor="dimgrey", label="Unadjusted Data"
    )

    plt.legend(
        handles=[patch_unadjusted_real, patch_real, patch_simulation],
        loc="lower left",
    ...
    rel_diff_unadjusted_real.plot(
        kind="bar",
        yerr=err_unadjusted_real,
        width=0.4,
        ax=ax,
        position=0,
        bottom=1.0,
    ...
    ax.set_title(name_of_plot)
    patch_real = matplotlib.patches.Patch(
        facecolor="yellowgreen", label="Adjusted Data"
    )
    patch_unadjusted_real = matplotlib.patches.Patch(
        facecolor="salmon", label="Unadjusted Data"
    )
    legend = plt.legend(
        handles=[patch_real, patch_unadjusted_real],
        loc="center left",
        bbox_to_anchor=(1.0, 0.5),
    )
