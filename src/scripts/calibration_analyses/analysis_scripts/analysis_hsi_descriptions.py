...
def figure1_distribution_of_hsi_event_by_treatment_id(
    ...
    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_Fig1_{stub}.png"
    )  # noqa: E731
...
def figure2_appointments_used(
    ...
    df["Appt_Type"] = df.level_1.astype(
        pd.CategoricalDtype(
            categories=list(COARSE_APPT_TYPE_TO_COLOR_MAP.keys()), ordered=True
        )
    )
    df["TREATMENT_ID"] = df.level_0.map(
        lambda x: _standardize_short_treatment_id(x)
    ).astype(
        pd.CategoricalDtype(
            categories=list(SHORT_TREATMENT_ID_TO_COLOR_MAP.keys()), ordered=True
        )
    )
    df = df.sort_values(by=["Appt_Type", "TREATMENT_ID"])
    df = (
        df.groupby(by=["Appt_Type", "TREATMENT_ID"])[0]
        .sum()
...
def figure3_fraction_of_time_of_hcw_used_by_treatment(
    ...
    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_Fig3_{stub}.png"
    )  # noqa: E731
    ...
    for (
        _,
        times_by_officer_category_treatment_id,
    ) in times_by_officer_category_treatment_id_per_run.items():
        ...
        for (
            cat,
            times_per_treatment_id,
        ) in times_per_treatment_id_by_officer_category.items():
    ...
    for (
        officer_category,
        proportions_per_treatment_id,
    ) in proportions_per_treatment_id_by_officer_category.items():
...
def figure4_hr_use_overall(
    results_folder: Path, output_folder: Path, resourcefilepath: Path
):
    """'Figure 4': The level of usage of the HealthSystem HR Resources"""

    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_Fig4_{stub}.png"
    )  # noqa: E731
    ...
    def get_share_of_time_for_hw_in_each_facility_by_short_treatment_id(_df):
        ...
        _df["All"] = _all
        return (
            _df.groupby(pd.Grouper(freq="M")).mean().stack()
        )  # find monthly averages and stack into series

    def get_share_of_time_used_for_each_officer_at_each_level(_df):
        _df = drop_outside_period(_df)
        _df = _df.set_index("date")
        _df = (
            _df["Frac_Time_Used_By_OfficerType"].apply(pd.Series).mean()
        )  # find mean over the period
...
def figure5_bed_use(results_folder: Path, output_folder: Path, resourcefilepath: Path):
    ...
    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_Fig5_{stub}.png"
    )  # noqa: E731
...
def figure6_cons_use(results_folder: Path, output_folder: Path, resourcefilepath: Path):
    ...
    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_Fig6_{stub}.png"
    )  # noqa: E731
    ...
    def get_counts_of_items_requested(_df):
        ...
        return (
            pd.concat(
                {
                    "Available": pd.Series(counts_of_available),
    ...
    cons = (
        cons.merge(cons_names, left_index=True, right_index=True, how="left")
        .set_index("Items")
        .astype(int)
    )
    ...
    def get_treatment_id_affecting_by_missing_consumables(_df):
        ...
        _df = drop_outside_period(_df)
        _df = _df.loc[
            (_df["Item_NotAvailable"] != "{}"), ["TREATMENT_ID", "Item_NotAvailable"]
        ]
...
def figure7_squeeze_factors(
    results_folder: Path, output_folder: Path, resourcefilepath: Path
):
    """'Figure 7': Squeeze Factors for the HSIs"""
    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_Fig7_{stub}.png"
    )  # noqa: E731
    ...
    def get_mean_squeeze_factor_by_hsi(_df):
        ...
        return (
            _df.loc[
                pd.to_datetime(_df["date"]).between(*TARGET_PERIOD), "squeeze_factor"
            ]
            .apply(pd.Series)
            .mean()
        )
    ...
    squeeze_factor_by_hsi.index = squeeze_factor_by_hsi.index.str.split(
        ":", expand=True
    )
    ...
    squeeze_factor_by_hsi["TREATMENT_ID"] = squeeze_factor_by_hsi["_TREATMENT_ID"].map(
        lambda x: x.split("_")[0] + "*"
    )

    # Sort to collect the same TREATMENT_ID together
    squeeze_factor_by_hsi = squeeze_factor_by_hsi.sort_values(
        "TREATMENT_ID", key=order_of_short_treatment_ids, ascending=False
    ).reset_index(drop=True)
