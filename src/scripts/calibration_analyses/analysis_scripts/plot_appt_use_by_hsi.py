...
def figure5_proportion_of_hsi_events_per_appt_type(results_folder: Path, output_folder: Path, resourcefilepath: Path):
    ...
    counts_by_treatment_id_and_appt_type = compute_mean_across_runs(
        bin_hsi_event_details(
            ...
            lambda event_details, count: sum(
                [
                    Counter({(event_details["treatment_id"], appt_type): count * appt_number})
    ...
    counter_to_df = counter_to_df.groupby(["TREATMENT_ID", "Appt_Type"])["Count"].sum().reset_index()
    ...
    fig.tight_layout()
    fig.savefig(output_folder / f"{PREFIX_ON_FILENAME}_{name_of_figure.replace(' ', '_')}.png")
    plt.close(fig)
