...
def figure5_proportion_of_hsi_events_per_appt_type(
        ...
        bin_hsi_event_details(
            ...
            lambda event_details, count: sum(
                [
                    Counter(
                        {
                            (event_details["treatment_id"], appt_type): count
                            * appt_number
                        }
                    )
    ...
    counter_to_df = pd.DataFrame.from_dict(
        counts_by_treatment_id_and_appt_type, orient="index"
    ).reset_index()
    counter_to_df["TREATMENT_ID"], counter_to_df["Appt_Type"] = zip(
        *counter_to_df["index"]
    )
    counter_to_df = (
        counter_to_df.rename(columns={0: "Count"}).drop(columns="index").copy()
    )
    ...
    counter_to_df = (
        counter_to_df.groupby(["TREATMENT_ID", "Appt_Type"])["Count"]
        .sum()
        .reset_index()
    )
    ...
    name_of_figure = (
        "Proportion of Appointment Use by TREATMENT_ID per Appointment Type"
    )
    for idx in range(len(appts)):
        ...
        axs[idx].legend(
            handles=axs[idx].containers[0][3:10],  # legend for top 10
            labels=list(df_to_plot.index[3:10]),
            handlelength=1,
            handleheight=1,
            fontsize=10,
            title="Unlabelled Treatment ID",
            title_fontsize=11,
            ncol=1,
            loc="center left",
    ...
    fig.tight_layout()
    fig.savefig(
        output_folder / f"{PREFIX_ON_FILENAME}_{name_of_figure.replace(' ', '_')}.png"
    )
    plt.close(fig)
    ...
    fig, axs = plt.subplots(len(appts), 1, figsize=(11, 40))
    name_of_figure = (
        "Proportion of Appointment Use by SHORT_TREATMENT_ID per Appointment Type"
    )
    for idx in range(len(appts)):
        df_to_plot = counter_to_df[counter_to_df.Appt_Type == appts[idx]].copy()
        df_to_plot["SHORT_TREATMENT_ID"] = df_to_plot["TREATMENT_ID"].apply(
