...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    gbd_all["measure_name"] = gbd_all["measure_name"].replace(
    ...
    def make_std_graphs(what, period):
        ...
        def get_counts_of_death_by_period_sex_agegrp_label(df):
            ...
            return df.groupby(by=["period", "sex", "age_grp", "label"])[
                "person_id"
            ].count()
        ...
        if what == "Deaths":
            ...
            mapper_from_gbd_causes = pd.Series(
                demoglog["mapper_from_gbd_cause_to_common_label"]
                .drop(columns={"date"})
                .loc[0]
            ).to_dict()
        else:
            ...
            mapper_from_gbd_causes = pd.Series(
                hblog["daly_mapper_from_gbd_cause_to_common_label"]
                .drop(columns={"date"})
                .loc[0]
            ).to_dict()
        ...
        all_causes = sorted(
            list(results.index.levels[3]), key=order_of_cause_of_death_or_daly_label
        )
        ...
        def _sort_columns(df):
            ...
            return df.sort_index(
                axis=1, key=order_of_cause_of_death_or_daly_label, ascending=False
            )
        ...
        for cause in all_causes:
            try:
                outcomes_this_cause = (
                    pd.concat(
                        {
                            dat: outcome_by_age_pt[dat].loc[:, (slice(None), cause)]
                            for dat in outcome_by_age_pt.keys()
                        },
                        axis=1,
                    ).fillna(0.0)
                    / 1e3
        ...
        tot_outcomes_by_cause = pd.concat(
            {
                dat: outcome_by_age_pt[dat].sum(axis=0)
                for dat in outcome_by_age_pt.keys()
            },
            axis=1,
        )
        ...
        ax.text(
            ...
            y=0.05,
            s=(
                "Main causes not included explicitly:\n\n"
