...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    def make_std_graphs(what, period):
        ...
        # - GBD:
        outcome_by_age_pt["GBD"] = (
            gbd.loc[gbd.period == period]
            .groupby(["sex", "age_grp", "label"])[["mean", "lower", "upper"]]
            .sum()
            .unstack()
            .div(5.0)
        )
        ...
        outcome_by_age_pt["Model"] = (
            summarize(results, collapse_columns=True)
            .reset_index()
            .loc[lambda x: (x.period == period)]
            .groupby(by=["sex", "age_grp", "label"])[["mean", "lower", "upper"]]
            .sum()
        ...
        for sex in sexes:
            for scaled in (False, True):
                _dat = {
                    _dat: outcome_by_age_pt[_dat].loc[sex].loc[:, pd.IndexSlice["mean"]].pipe(_sort_columns)
                ...
                fig.tight_layout()
                fig.savefig(
                    make_graph_file_name(f"{what}_{period}_{sex}_StackedBars_ModelvsGBD_{'scaled' if scaled else ''}")
                )
                plt.close(fig)
        ...
        slices = normalize_series(outcome_by_age_pt["Model"].sum().loc["mean"].sort_values(ascending=True))
        ...
        for cause in all_causes:
            try:
                ...
                fig.tight_layout()
                fig.savefig(
                    make_graph_file_name(f"B_{what}_{period}_AgeAndSexSpecificLineGraph_{reformat_cause(cause)}")
                )
                plt.close(fig)
        ...
        tot_outcomes_by_cause = pd.concat(
            {dat: outcome_by_age_pt[dat].sum(axis=0) for dat in outcome_by_age_pt.keys()}, axis=1
        )
        ...
        for cause in all_causes:
            ...
            xerr = np.array([x - vals.at[("lower", cause), "GBD"], vals.at[("upper", cause), "GBD"] - x]).reshape(2, 1)
            y = vals.at[("mean", cause), "Model"]
            yerr = np.array([y - vals.at[("lower", cause), "Model"], vals.at[("upper", cause), "Model"] - y]).reshape(
                2, 1
            )

            ax.errorbar(
                x=x, y=y, xerr=xerr, yerr=yerr, label=cause, color=get_color_cause_of_death_or_daly_label(cause)
            )

            # add labels to selected points
            if cause in select_labels:
                ax.annotate(cause, (x, y), textcoords="offset points", xytext=(0, 10), ha="center")
        ...
        unmodelled_causes = (
            gbd.loc[(period == period) & (gbd["measure_name"] == what)]
            .assign(frac_cause=lambda df: df["mean"] / df["mean"].sum())
            .groupby(by=["cause_name", "label"])["frac_cause"]
            .sum()
        ...
        top_five_causes_not_modelled = "".join(
            [
                ...
                for _cause, _percent_deaths in unmodelled_causes[0:10].items()
                if _percent_deaths >= 0.005
            ]
        )
        ...
        ax.axhline(
            fraction_causes_modelled_overall,
            color="b",
        ...
        ax.text(
            x=0.5,
            y=0.05,
            s=("Main causes not included explicitly:\n\n" + top_five_causes_not_modelled),
        ...
        format_to_plot = (
            total_num_dalys_by_wealth_and_label.sort_index(axis=0)
