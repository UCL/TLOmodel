...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    colors = {"Model": "royalblue", "Census": "darkred", "WPP": "forestgreen", "GBD": "plum"}
    ...
    ax.plot(pop_model.index, pop_model["mean"] / 1e6, label="Model (mean)", color=colors["Model"])
    ax.fill_between(
    ...
    pop_2018 = {
        "Census": cens_2018,
        "WPP": wpp_2018,
        "Model": pd.Series({"F": pop_model_female.loc[2018, "mean"], "M": pop_model_male.loc[2018, "mean"]}),
    }
    ...
    def plot_population_pyramid(data, fig):
        ...
        sources = data["M"].keys()
        dat = {_sex: pd.concat({_source: data["M"][_source] for _source in sources}, axis=1) for _sex in ["M", "F"]}
    ...
    for tp in time_period:
        ...
        ax.plot(births_loc.index, births_loc["Model_mean"] / 1e6, label="Model", color=colors["Model"])
        ax.fill_between(
        ...
        ax.plot(births_loc.index, births_loc["WPP_continuous"] / 1e6, color=colors["WPP"], label="WPP")
        ax.fill_between(
            (births_loc.index).to_numpy(),
    ...
    def get_usage_by_age_and_year(_df):
        _x = _df.assign(year=_df["date"].dt.year).set_index("year").drop(columns=["date"]).sort_index()
    ...
    model_asfr = (
        asfr.unstack().droplevel(axis=1, level=0).groupby(by=asfr.index.levels[0].map(calperiodlookup)).mean().stack()
    )

    data_asfr = (
        wpp.loc[wpp.Variant.isin(["WPP_Estimates", "WPP_Medium variant"]), ["Age_Grp", "Period", "asfr"]]
    ...
    gbd_deaths = pd.DataFrame(
        gbd_deaths.drop(columns=["Year"]).groupby(by=["Period", "Sex", "Age_Grp", "Variant"]).sum()
    ).reset_index()
    ...
    deaths_by_period = pd.concat(
        [deaths_model_by_period, wpp_deaths_byperiod, gbd_deaths_by_period], ignore_index=True, sort=False
    )
    ...
    # Plot:
    fig, ax = plt.subplots()
    ax.plot(deaths_by_period.index, deaths_by_period["WPP_continuous"] / 1e6, label="WPP", color=colors["WPP"])
    ax.fill_between(
    ...
    ax.plot(deaths_by_period.index, deaths_by_period["GBD_Est"] / 1e6, label="GBD", color=colors["GBD"])
    ax.fill_between(
        (deaths_by_period.index).to_numpy(),
    ...
    ax.plot(deaths_by_period.index, deaths_by_period["Model_mean"] / 1e6, label="Model", color=colors["Model"])
    ax.fill_between(
        (deaths_by_period.index).to_numpy(),
    ...
    deaths_by_agesexperiod = pd.concat(
        [deaths_model_by_agesexperiod, wpp_deaths, gbd_deaths], ignore_index=True, sort=False
    )
        ...
        for i, sex in enumerate(["F", "M"]):
            ...
            tot_deaths_byage = pd.DataFrame(
                deaths_by_agesexperiod.loc[
                    (deaths_by_agesexperiod["Period"] == period) & (deaths_by_agesexperiod["Sex"] == sex)
                ]
            ...
            if "WPP_Medium variant" in tot_deaths_byage.columns:
                ...
                ax.plot(
                    tot_deaths_byage.index, tot_deaths_byage["WPP_Estimates"] / 1e3, label="WPP", color=colors["WPP"]
                )

            if "GBD_Est" in tot_deaths_byage.columns:
                ax.plot(tot_deaths_byage.index, tot_deaths_byage["GBD_Est"] / 1e3, label="GBD", color=colors["GBD"])
                ax.fill_between(
                    (tot_deaths_byage.index).to_numpy(),
            ...
            ax.plot(tot_deaths_byage.index, tot_deaths_byage["Model_mean"] / 1e3, label="Model", color=colors["Model"])
            ax.fill_between(
                (tot_deaths_byage.index).to_numpy(),
