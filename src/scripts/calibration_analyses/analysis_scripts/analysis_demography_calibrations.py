...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    wpp_ann = pd.read_csv(
        Path(resourcefilepath) / "demography" / "ResourceFile_Pop_Annual_WPP.csv"
    )
    ...
    fig, ax = plt.subplots()
    ax.plot(wpp_ann_total.index, wpp_ann_total / 1e6, label="WPP", color=colors["WPP"])
    ax.plot(
        2018.5,
        cens_2018.sum() / 1e6,
        marker="o",
    ...
    for i, key in enumerate(pop_2018):
        ax.bar(
            x=x + (i - 1) * width * 1.05,
            height=[pop_2018[key][sex] / 1e6 for sex in labels],
            width=width,
            label=key,
            color=colors[key],
        )
    ...
    def plot_population_pyramid(data, fig):
        ...
        for _dat_source in sorted(set(sources).intersection(["WPP", "Census"])):
            ax.plot(
                data["M"][_dat_source].values / 1e3,
                dat["M"].index,
                label=_dat_source,
                color=colors[_dat_source],
            )
            ax.plot(
                -data["F"][_dat_source].values / 1e3,
                dat["F"].index,
                label="_",
                color=colors[_dat_source],
            )
        ...
        num_by_age = summarize(
            extract_results(
                ...
                custom_generate_series=(
                    lambda df_: df_.loc[pd.to_datetime(df_.date).dt.year == year]
                    .drop(columns=["date"])
    ...
    for year in [2010, 2015, 2018, 2029, 2049]:
        if year in pop_model.index:
            # Get WPP data:
            wpp_thisyr = (
                wpp_ann.loc[wpp_ann["Year"] == year]
                .groupby(["Sex", "Age_Grp"])["Count"]
                .sum()
            )
    ...
    calperiods, calperiodlookup = make_calendar_period_lookup()
    births_results.index = births_results.index.map(calperiodlookup).astype(
        make_calendar_period_type()
    )
    ...
    wpp_births = pd.read_csv(
        Path(resourcefilepath) / "demography" / "ResourceFile_TotalBirths_WPP.csv"
    )
    wpp_births = (
        wpp_births.groupby(["Period", "Variant"])["Total_Births"].sum().unstack()
    )
    ...
    wpp_births["WPP_continuous"] = wpp_births["WPP_Estimates"].combine_first(
        wpp_births["WPP_Medium variant"]
    )

    # Births in 2018 Census
    cens_births = pd.read_csv(
    ...
    births = wpp_births.merge(
        births_model, right_index=True, left_index=True, how="left"
    )
    ...
    time_period = {
        ...
        "2010-2029": [
            (2010 <= int(x[0])) & (int(x[1]) < 2030)
            for x in births.index.str.split("-")
        ],
        "2010-2049": [
            (2010 <= int(x[0])) & (int(x[1]) < 2050)
            for x in births.index.str.split("-")
        ],
    }
    ...
    def get_usage_by_age_and_year(_df):
        ...
        for _age in adult_age_groups:
            ...
            q = pd.DataFrame(
                q.apply(lambda row: row / row.sum(), axis=1)
                .groupby(q.index)
                .mean()
                .stack()
            )
    ...
    contaceptive_use_in_year_of_interest = mean_usage_by_age.loc[
        (year_of_interest, slice(None))
    ].T
    fig, ax = plt.subplots()
    ...
    fig.tight_layout()
    fig.savefig(
        make_graph_file_name(f"Contraception_use_by_age_in_year_{year_of_interest}")
    )
    plt.close(fig)
    ...
    asfr = summarize(
        births_by_mother_age_at_pregnancy.div(num_adult_women)
    ).sort_index()
    ...
    for i, _agegrp in enumerate(adult_age_groups):
        ...
        data = wpp.loc[
            (wpp.Age_Grp == _agegrp)
            & wpp.Variant.isin(["WPP_Estimates", "WPP_Medium variant"]),
            ["Period", "asfr"],
        ]
    ...
    data_asfr = (
        ...
        .groupby(by=["Age_Grp", "Period"])["asfr"]
        .mean()
        .unstack()
        .T
    )
    ...
    def get_counts_of_death_by_period_sex_agegrp(df):
        ...
        df["Period"] = (
            df["year"].map(calperiodlookup).astype(make_calendar_period_type())
        )
        df["Sex"] = df["sex"]
    ...
    # Load WPP data
    wpp_deaths = pd.read_csv(
        Path(resourcefilepath) / "demography" / "ResourceFile_TotalDeaths_WPP.csv"
    )
    ...
    # Load GBD
    gbd_deaths = format_gbd(
        pd.read_csv(resourcefilepath / "gbd" / "ResourceFile_TotalDeaths_GBD2019.csv")
    )

    # For GBD, compute sums by period
    gbd_deaths = pd.DataFrame(
        gbd_deaths.drop(columns=["Year"])
        .groupby(by=["Period", "Sex", "Age_Grp", "Variant"])
        .sum()
    ).reset_index()
    ...
    deaths_model_by_period = summarize(
        results_deaths.groupby(level=0).sum(), collapse_columns=True
    ).reset_index()
    ...
    wpp_deaths_byperiod = (
        wpp_deaths.groupby(by=["Variant", "Period"])["Count"].sum().reset_index()
    )
    gbd_deaths_by_period = (
        gbd_deaths.groupby(by=["Variant", "Period"])["Count"].sum().reset_index()
    )
    ...
    deaths_by_period["Period"] = deaths_by_period["Period"].astype(
        make_calendar_period_type()
    )

    # Total of deaths over time (summing age/sex)
    deaths_by_period = pd.DataFrame(
        deaths_by_period.groupby(by=["Period", "Variant"]).sum()
    ).unstack()
    deaths_by_period.columns = pd.Index(
        [label[1] for label in deaths_by_period.columns.tolist()]
    )
    deaths_by_period = deaths_by_period.replace(0, np.nan)

    # Make the WPP line connect to the 'medium variant' to make the lines look like they join up
    deaths_by_period["WPP_continuous"] = deaths_by_period[
        "WPP_Estimates"
    ].combine_first(deaths_by_period["WPP_Medium variant"])
    ...
    plt.xticks(
        np.arange(len(deaths_by_period.index)), deaths_by_period.index, rotation=90
    )
    fig.tight_layout()
    ...
    deaths_model_by_agesexperiod = summarize(
        results_deaths, collapse_columns=True
    ).reset_index()
    deaths_model_by_agesexperiod = deaths_model_by_agesexperiod.melt(
        id_vars=["Period", "Sex", "Age_Grp"],
        value_vars=["mean", "lower", "upper"],
        var_name="Variant",
        value_name="Count",
    )
    deaths_model_by_agesexperiod["Variant"] = (
        "Model_" + deaths_model_by_agesexperiod["Variant"]
    )

    # Combine into one large dataframe
    deaths_by_agesexperiod = pd.concat(
        [deaths_model_by_agesexperiod, wpp_deaths, gbd_deaths],
        ignore_index=True,
        sort=False,
    )
        ...
        for i, sex in enumerate(["F", "M"]):
            ...
            tot_deaths_byage = pd.DataFrame(
                deaths_by_agesexperiod.loc[
                    (deaths_by_agesexperiod["Period"] == period)
                    & (deaths_by_agesexperiod["Sex"] == sex)
                ]
                .groupby(by=["Variant", "Age_Grp"])["Count"]
                .sum()
            ).unstack()
            tot_deaths_byage.columns = pd.Index(
                [label[1] for label in tot_deaths_byage.columns.tolist()]
            )
