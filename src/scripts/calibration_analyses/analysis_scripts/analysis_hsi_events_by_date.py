...
def format_dataframe(result_folder):
    result = load_pickled_dataframes(
        result_folder, draw=0, run=0, name="tlo.methods.healthsystem"
    )
    ...
    # Produce coarse version of TREATMENT_ID (just first level, which is the module)
    hsi_event["TREATMENT_ID_SHORT"] = (
        hsi_event["TREATMENT_ID"].str.split("_").apply(lambda x: x[0])
    )
    return date, hsi_event


def figure1_distribution_of_hsi_event_by_date(
    results_folder: Path, output_folder: Path
):
...
def figure2_distribution_of_hsi_event_by_treatment(
    results_folder: Path, output_folder: Path
):
    """'Figure 2': Distribution of treatment id by date."""

    make_graph_file_name = (
        lambda stub: output_folder / f"{PREFIX_ON_FILENAME}_Fig2_{stub}.png"
    )  # noqa: E731
    ...
    def create_figure(name_of_plot, key_variable, s_variable, plot_kind, colour_kind):
        ...
        s_variable.iloc[key_variable].value_counts().plot(
            kind=plot_kind,
            color=get_color_short_treatment_id(colour_kind),
            stacked=True,
            label=colour_kind,
        )
        ax.set_title(name_of_plot, {"size": 12, "color": "black"})
    ...
    for key, value in treatment.items():
        date.iloc[value].value_counts(normalize=True).plot(
            kind="line",
            color=get_color_short_treatment_id(key),
            stacked=True,
            label=key,
        )
    ax.set_title(name_of_plot, {"size": 12, "color": "black"})
    ...
    date.iloc[treatment["Malaria"]].value_counts(normalize=True).plot(
        kind="line",
        color=get_color_short_treatment_id("Malaria"),
        stacked=True,
        label="Malaria",
    )
    ax.set_title(name_of_plot, {"size": 12, "color": "black"})
    ...
    create_figure(
        "Tb over Facility Level",
        treatment["Tb"],
        hsi_events["Facility_Level"],
        "bar",
        "Tb",
    )
    create_figure(
        "Malaria over Facility Level",
        treatment["Malaria"],
        hsi_events["Facility_Level"],
        "bar",
        "Malaria",
...
def figure3_appointments_used(results_folder: Path, output_folder: Path):
    ...
    counts_of_coarse_appt_by_date = (
        ...
        .groupby(
            axis=1, by=counts_of_appt_by_date.index.levels[1].map(get_coarse_appt_type)
        )
        .sum()
    )
    ...
    counts_of_coarse_appt_by_date.plot.bar(
        ...
        stacked=True,
        color=[
            get_color_coarse_appt(_appt)
            for _appt in counts_of_coarse_appt_by_date.columns
        ],
    )
