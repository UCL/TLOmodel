...
def format_dataframe(result_folder):
    ...
    hsi_event = result["tlo.methods.healthsystem"]["HSI_Event"].drop(
        columns=["Person_ID", "Squeeze_Factor", "did_run", "Facility_ID"]
    )
    # Produce coarse version of TREATMENT_ID (just first level, which is the module)
...
def figure1_distribution_of_hsi_event_by_date(results_folder: Path, output_folder: Path):
    ...
    for key, value in facility_id.items():
        date.iloc[value].value_counts(normalize=True).plot(
            kind="line", color=get_color_facility_id(key), stacked=True, label=key
        )
    ax.set_title(name_of_plot, {"size": 12, "color": "black"})
...
def figure2_distribution_of_hsi_event_by_treatment(results_folder: Path, output_folder: Path):
    ...
    def create_figure(name_of_plot, key_variable, s_variable, plot_kind, colour_kind):
        fig, ax = plt.subplots()
        s_variable.iloc[key_variable].value_counts().plot(
            kind=plot_kind, color=get_color_short_treatment_id(colour_kind), stacked=True, label=colour_kind
        )
        ax.set_title(name_of_plot, {"size": 12, "color": "black"})
        ax.legend(ncol=2, prop={"size": 8}, loc="upper left")
    ...
    for key, value in treatment.items():
        date.iloc[value].value_counts(normalize=True).plot(
            kind="line", color=get_color_short_treatment_id(key), stacked=True, label=key
        )
    ax.set_title(name_of_plot, {"size": 12, "color": "black"})
    ax.legend(ncol=2, prop={"size": 8}, loc="upper left")
    fig.savefig(make_graph_file_name(name_of_plot.replace(" ", "_")))
    ...
    date.iloc[treatment["Tb"]].value_counts(normalize=True).plot(
        kind="line", color=get_color_short_treatment_id("Tb"), stacked=True, label="Tb"
    )
    date.iloc[treatment["Malaria"]].value_counts(normalize=True).plot(
        kind="line", color=get_color_short_treatment_id("Malaria"), stacked=True, label="Malaria"
    )
    ax.set_title(name_of_plot, {"size": 12, "color": "black"})
    ax.legend(ncol=2, prop={"size": 8}, loc="upper left")
    fig.savefig(make_graph_file_name(name_of_plot.replace(" ", "_")))
    ...
    create_figure("Tb over Facility Level", treatment["Tb"], hsi_events["Facility_Level"], "bar", "Tb")
    create_figure("Malaria over Facility Level", treatment["Malaria"], hsi_events["Facility_Level"], "bar", "Malaria")
    create_figure("Tb over Appointment", treatment["Tb"], hsi_events["Number_By_Appt_Type_Code"], "bar", "Tb")
    create_figure(
        "Malaria over Appointment", treatment["Malaria"], hsi_events["Number_By_Appt_Type_Code"], "bar", "Malaria"
    )
...
def figure3_appointments_used(results_folder: Path, output_folder: Path):
    ...
    counts_of_coarse_appt_by_date = (
        ...
        .groupby(axis=1, by=counts_of_appt_by_date.index.levels[1].map(get_coarse_appt_type))
        .sum()
    )
    ...
    counts_of_coarse_appt_by_date.plot.bar(
        ax=ax, stacked=True, color=[get_color_coarse_appt(_appt) for _appt in counts_of_coarse_appt_by_date.columns]
    )
...
def apply(results_folder: Path, output_folder: Path):
    ...
    figure1_distribution_of_hsi_event_by_date(results_folder=results_folder, output_folder=output_folder)
    figure2_distribution_of_hsi_event_by_treatment(results_folder=results_folder, output_folder=output_folder)
    figure3_appointments_used(results_folder=results_folder, output_folder=output_folder)
