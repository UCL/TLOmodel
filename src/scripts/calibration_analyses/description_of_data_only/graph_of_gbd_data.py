...
# load gbd data
gbd = pd.read_csv(
    Path(resourcefilepath) / "gbd" / "ResourceFile_Deaths_And_DALYS_GBD2019.csv"
)
gbd = format_gbd(gbd)
...
gbd.loc[gbd.cause_name.str.contains("Lower respiratory infections"), "cause"] = (
    "Childhood Pneumonia"
)
...
gbd.loc[gbd.cause_name.str.contains("heart disease"), "cause"] = (
    "Stroke & Heart Disease"
)
gbd.loc[gbd.cause_name.str.contains("Neonatal disorders"), "cause"] = (
    "Neonatal Disorders"
)
gbd.loc[gbd.cause_name.str.contains("Self-harm"), "cause"] = (
    "Other conditions - Modelled"
)
gbd.loc[gbd.cause_name.str.contains("Epilepsy"), "cause"] = (
    "Other conditions - Modelled"
)
gbd.loc[gbd.cause_name.str.contains("Schistosomiasis"), "cause"] = (
    "Other conditions - Modelled"
)
