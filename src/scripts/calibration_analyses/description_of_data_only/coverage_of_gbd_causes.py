...
codis = pd.Series(
    (
        pd.read_csv(
            resourcefilepath / "gbd" / "ResourceFile_CausesOfDALYS_GBD2019.csv",
            header=None,
        )[0]
    )
).sort_values()
...
class Tb(Module):
    ...
    CAUSES_OF_DEATH = {
        "Tuberculosis": Cause(gbd_causes={"Tuberculosis"}, label="Tuberculosis")
    }
...
class RoadTrafficInjuries(Module):
    ...
    # Declare Causes of Death
    CAUSES_OF_DEATH = {
        "Road injuries": Cause(gbd_causes={"Road injuries"}, label="Road injuries")
    }
...
class Copd(Module):
    ...
    CAUSES_OF_DEATH = {
        "Chronic obstructive pulmonary disease": Cause(
            gbd_causes={"Chronic obstructive pulmonary disease"},
...
class Schisto(Module):
    ...
    CAUSES_OF_DEATH = {
        "Schistosomiasis": Cause(
            gbd_causes={"Schistosomiasis"}, label="Schistosomiasis"
        )
    }
...
class Backpain(Module):
    ...
    # Declare Causes of Disability
    CAUSES_OF_DISABILITY = {
        "chronic_lower_back_pain": Cause(
            gbd_causes={"Low back pain"}, label="Low back pain"
        )
    }
...
list(sim.modules["OtherAdultCancer"].CAUSES_OF_DISABILITY.values())[
    0
].label = "Other Cancer"
...
_, deaths_mapper_from_gbd_causes = sim.modules[
    "Demography"
].create_mappers_from_causes_of_death_to_label()
_, dalys_mapper_from_gbd_causes = sim.modules[
    "HealthBurden"
].create_mappers_from_causes_of_disability_to_label()

# %% Load GBD data:
gbd = format_gbd(
    pd.read_csv(resourcefilepath / "gbd" / "ResourceFile_Deaths_And_DALYS_GBD2019.csv")
)

# extract total death (all ages/sex)
deaths = (
    gbd.loc[(gbd.measure_name == "Deaths") & (gbd.Year == 2019)]
    .copy()
