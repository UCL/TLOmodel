...
cod = pd.Series(
    (
        pd.read_csv(resourcefilepath / "gbd" / "ResourceFile_CausesOfDeath_GBD2019.csv")
...
class Tb(Module):
    ...
    CAUSES_OF_DEATH = {"Tuberculosis": Cause(gbd_causes={"Tuberculosis"}, label="Tuberculosis")}

    # Declare Causes of Disability
    CAUSES_OF_DISABILITY = {"Tuberculosis": Cause(gbd_causes={"Tuberculosis"}, label="Tuberculosis")}
...
class Alri(Module):
    ...
    CAUSES_OF_DEATH = {
        "Lower respiratory infections": Cause(
            gbd_causes={"Lower respiratory infections"}, label="Lower respiratory infections"
        )
    }
...
class RoadTrafficInjuries(Module):
    METADATA = {Metadata.DISEASE_MODULE}
    # Declare Causes of Death
    CAUSES_OF_DEATH = {"Road injuries": Cause(gbd_causes={"Road injuries"}, label="Road injuries")}

    # Declare Causes of Disability
    CAUSES_OF_DISABILITY = {"Road injuries": Cause(gbd_causes={"Road injuries"}, label="Road injuries")}
...
class Copd(Module):
    ...
    CAUSES_OF_DEATH = {
        "Chronic obstructive pulmonary disease": Cause(
            gbd_causes={"Chronic obstructive pulmonary disease"}, label="Chronic obstructive pulmonary disease"
        )
    }
...
class Schisto(Module):
    METADATA = {Metadata.DISEASE_MODULE}
    # Declare Causes of Death
    CAUSES_OF_DEATH = {"Schistosomiasis": Cause(gbd_causes={"Schistosomiasis"}, label="Schistosomiasis")}

    # Declare Causes of Disability
    CAUSES_OF_DISABILITY = {"Schistosomiasis": Cause(gbd_causes={"Schistosomiasis"}, label="Schistosomiasis")}
...
class Backpain(Module):
    METADATA = {Metadata.DISEASE_MODULE}
    # Declare Causes of Disability
    CAUSES_OF_DISABILITY = {"chronic_lower_back_pain": Cause(gbd_causes={"Low back pain"}, label="Low back pain")}
...
dummies = [Tb(), Alri(), RoadTrafficInjuries(), Copd(), Schisto(), OtherInjuries(), Backpain()]
...
_, deaths_mapper_from_gbd_causes = sim.modules["Demography"].create_mappers_from_causes_of_death_to_label()
_, dalys_mapper_from_gbd_causes = sim.modules["HealthBurden"].create_mappers_from_causes_of_disability_to_label()
...
dalys = (
    gbd.loc[(gbd.measure_name == "DALYs (Disability-Adjusted Life Years)") & (gbd.Year == 2019)]
    .copy()
    .groupby(by="cause_name")["GBD_Est"]
    .sum()
)
