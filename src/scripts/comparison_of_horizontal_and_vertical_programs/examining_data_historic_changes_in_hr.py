...
step_size = (to_plot.loc[2023, 'Scale-up'] - to_plot.loc[2017, 'Scale-up']) / 7  # Step size for each year from 2023
...
# Now add the new line that applies an 8% increase after 2023
to_plot['Accelerated Scale-up (8%)'] = to_plot['Scale-up'].copy()

# Apply 8% growth starting in 2024 for the new scenario and keep constant after 2030
for year in range(2024, 2031):
    to_plot.loc[year, 'Accelerated Scale-up (8%)'] = to_plot.loc[year - 1, 'Accelerated Scale-up (8%)'] * 1.08
# Keep it constant from 2031 onwards
to_plot.loc[2031:, 'Accelerated Scale-up (8%)'] = to_plot.loc[2030, 'Accelerated Scale-up (8%)']
...
for xlim in (datetime.date(2025, 1, 1), datetime.date(2035, 1, 1)):
    ...
    ax.stairs(
        values=to_plot['Accelerated Scale-up (8%)'],
        edges=step_dates, baseline=None,
        label='Accelerated Scale-up (8%) Scenario',
        color='b',
        zorder=2,
        linewidth=3
    )
    ...
    # Shading between the original scale-up and counterfactual
    ax.stairs(
        values=to_plot['Scale-up'],
        edges=step_dates,
        baseline=1.0,
    ...
    # Plot the counterfactual scenario (green)
    ax.stairs(
...
scale_up_multipliers = dict()
scale_up_multipliers[2010] = 1.0
for idx, val in to_plot['Accelerated Scale-up (8%)'].sort_index().items():
    if idx-1 > to_plot['Accelerated Scale-up (8%)'].index[0]:
        scale_up_multipliers[idx] = val / to_plot.loc[idx-1, 'Accelerated Scale-up (8%)']
