...
logFile_without = (
    "run_analysis_contraception_no_diseases__" + datestamp_without_log + ".log"
)
...
def save_csv(in_to_save, in_file_name, in_datestamp_log):
    in_to_save.to_csv(
        Path(dataframe_folder + "/" + in_file_name + "_" + in_datestamp_log + ".csv")
    )
    return 0
...
if do_no_interv_analysis:
    ...
    (
        ...
        ID_without, logFile_without, False
    )  # no calc of intervention costs for sim without interv
    if not run_analysis:
        (
            use_without_df,
            percentage_use_without_df,
    ...
    if not run_analysis:
        ...
        np.save(
            Path(
                dataframe_folder + "/scaling_factor_with_" + datestamp_with_log + ".npy"
            ),
            scaling_factor_with,
        )
        print("Dataframes and scaling factor saved.\n")
...
if table_use_costs_bool:
    ...
    to_print = (
        "Tab: Contraception Use (total & percentage) & Costs within Time Periods "
    )
    if do_no_interv_analysis:
        if do_interv_analysis:
