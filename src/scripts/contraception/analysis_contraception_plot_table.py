...
contraceptives_order = ["pill", "IUD", "injections", "implant", "male_condom", "female_sterilization", "other_modern"]
...
def do_analysis(ID, logFile, in_calc_intervention_costs_bool):
    use_df, percentage_use_df, costs_df, interv_costs_df, scaling_factor_out = a_co.analyse_contraception(
        ID,
        logFile,
        suffix,
        # %% Plot Contraception Use Over time?
...
def load_analysis_out(in_analysis_type, in_datestamp_log):
    use_with_df_loaded = pd.read_csv(
        Path(dataframe_folder + "/use_" + in_analysis_type + "_df_" + in_datestamp_log + ".csv"), index_col=[0]
    )
    percentage_use_with_df_loaded = pd.read_csv(
        Path(dataframe_folder + "/percentage_use_" + in_analysis_type + "_df_" + in_datestamp_log + ".csv"),
        index_col=[0],
    )
    costs_with_df_loaded = pd.read_csv(
        Path(dataframe_folder + "/costs_" + in_analysis_type + "_df_" + in_datestamp_log + ".csv"), index_col=[0, 1]
    )
    interv_costs_with_df_loaded = pd.read_csv(
        Path(dataframe_folder + "/interv_costs_" + in_analysis_type + "_df_" + in_datestamp_log + ".csv"), index_col=[0]
    )
    scaling_factor_with_loaded = pd.read_csv(
        Path(dataframe_folder + "/scaling_factor_" + in_analysis_type + "_" + in_datestamp_log + ".npy")
    )
    return (
        use_with_df_loaded,
...
if do_no_interv_analysis:
    ...
    (
        use_without_df,
        percentage_use_without_df,
        costs_without_df,
        interv_costs_without_df,
    ...
    if not run_analysis:
        ...
        for to_save, file_name in (
            (use_without_df, "use_without_df"),
            (percentage_use_without_df, "percentage_use_without_df"),
            (costs_without_df, "costs_without_df"),
            (interv_costs_without_df, "interv_costs_without_df"),
        ):
            save_csv(to_save, file_name, datestamp_without_log)
        # save scaling factor (numpy float64)
        np.save(
            Path(dataframe_folder + "/scaling_factor_without_" + datestamp_without_log + ".npy"), scaling_factor_without
        )
    ...
    if do_interv_analysis:
        ...
        use_with_df, percentage_use_with_df, costs_with_df, interv_costs_with_df, scaling_factor_with = do_analysis(
            ID_with, logFile_with, calc_intervention_costs_bool
        )
        if not run_analysis:
            # load dataframes & scaling factor
            (
                use_with_df,
                percentage_use_with_df,
                costs_with_df,
                interv_costs_with_df,
                scaling_factor_with,
            ) = load_analysis_out("with", datestamp_with_log)
        else:
            # save dataframes
            for to_save, file_name in (
                (use_with_df, "use_with_df"),
                (percentage_use_with_df, "percentage_use_with_df"),
                (costs_with_df, "costs_with_df"),
                (interv_costs_with_df, "interv_costs_with_df"),
            ):
                save_csv(to_save, file_name, datestamp_with_log)
            # save scaling factor (numpy float64)
            np.save(Path(dataframe_folder + "/scaling_factor_with_" + datestamp_with_log + ".npy"), scaling_factor_with)
            print("Dataframes and scaling factor saved.\n")
    else:
...
if table_use_costs_bool:
    ...
    tables.table_use_costs__plot_costs(
        use_output,
        use_without_df,
        percentage_use_without_df,
        use_with_df,
        ...
        interv_costs_without_df,
        interv_costs_with_df,
        mwk_to_usd_exchange_rate,
        rounding_costs_mwk_to,
        rounding_costs_usd_to,
        plot_costs,
        datestamp_without_log,
        datestamp_with_log,
        suffix,
        ID_without,
        ID_with,
