...
def plot_costs(
    ...
    # bar_without
    ax.bar(x_i - width * 0.6, cons_costs_without, width, label=labels["cons"], color=colors["cons"])
    # bar_with
    ...
    ax.bar(
        x_i + width * 0.6, ppfp_interv_costs_with, width, bottom=ppfp_bottom, label=labels["ppfp"], color=colors["ppfp"]
    )
    # add legend
    ax.legend()
    plt.grid(axis="y")
    ...
    # bar_without
    ax2.bar(x2_i - width * 0.2, cons_costs_without[-1], width / 3, label=labels["cons"], color=colors["cons"])
    # bar_with
