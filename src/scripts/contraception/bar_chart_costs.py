...
def plot_costs(
    ...
    in_suffix,
    in_x_labels,
    in_cons_costs_without,
    in_cons_costs_with,
    in_pop_interv_costs_with,
    in_ppfp_interv_costs_with,
    ...
    # output file name
    output_filename = str(
        "Consumables and Interventions Costs "
        + in_id[0]
        + "_"
        + in_id[1]
        + in_suffix
    ...
    pop_interv_costs_with = reduce_magnitude(
        in_pop_interv_costs_with, in_reduce_magnitude
    )
    ppfp_interv_costs_with = reduce_magnitude(
        in_ppfp_interv_costs_with, in_reduce_magnitude
    )
    ...
    for y_label_mwk in ax.get_yticks():
        y_labels_usd.extend(
            [str(round(y_label_mwk * in_mwk_to_usd_exchange_rate * 1000))]
        )
    ...
    ax2.bar(
        x2_i - width * 0.2,
        cons_costs_without[-1],
        width / 3,
        label=labels["cons"],
        color=colors["cons"],
    )
    # bar_with
    ax2.bar(x2_i + width * 0.2, cons_costs_with[-1], width / 3, color=colors["cons"])
    # bar_with_pop_interv
    ax2.bar(
        x2_i + width * 0.2,
        pop_interv_costs_with[-1],
        width / 3,
        bottom=cons_costs_with[-1],
        label=labels["pop"],
        color=colors["pop"],
    )
    # bar_with_ppfp_interv
    ax2.bar(
        x2_i + width * 0.2,
        ppfp_interv_costs_with[-1],
        width / 3,
        bottom=ppfp_bottom[-1],
        label=labels["ppfp"],
        color=colors["ppfp"],
    )
    # add legend
    ax2.legend(loc="upper left", bbox_to_anchor=(0, -0.12))
    plt.grid(axis="y")
    ...
    for y2_label_mwk in ax2.get_yticks():
        y2_labels_usd.extend(
            [str(round(y2_label_mwk * in_mwk_to_usd_exchange_rate * 1000))]
        )
