...
avail_data = pd.read_csv(
    Path("resources/healthsystem/consumables/" + avail_data_filename)
)

items_pkgs = pd.read_csv(
    Path("resources/healthsystem/consumables/" + RF_of_interest_filename)
)
...
if var_exists("lines_of_interest") and var_exists("last_line_interv_pkg_name"):
...
elif not var_exists("lines_of_interest") and not var_exists(
    "last_line_interv_pkg_name"
):
    raise ValueError(
    ...
    last_line_nmb_data = items_pkgs.loc[
        items_pkgs["Intervention_Pkg"] == last_line_interv_pkg_name
    ].index[0]
...
if what_to_check == "lines":
    unavail_codes = set()
    nmb_unavail_items = 0
    for item_code_to_check in items_pkgs["Item_Code"][
        lines_of_interest[0] - 2 : lines_of_interest[1] - 1
    ]:
        if item_code_to_check not in avail_codes:
            unavail_codes.add(item_code_to_check)
            nmb_unavail_items += 1
    print(
        "Number of unavailable items (including repetitions): "
        + str(nmb_unavail_items)
        + " of "
