...
def table_use_costs__plot_costs(
    in_use_output,
    in_use_without_df,
    in_percentage_use_without_df,
    in_use_with_df,
    in_percentage_use_with_df,
    in_rounding_use_to,
    in_costs_without_df,
    in_costs_with_df,
    in_interv_costs_without_df,
    in_interv_costs_with_df,
    in_mwk_to_usd_exchange_rate,
    in_rounding_costs_mwk_to,
    in_rounding_costs_usd_to,
    in_plot_costs,
    in_datestamp_without_log,
    in_datestamp_with_log,
    in_suffix,
    in_ID_without,
    in_ID_with,
):
    ...
    def round_df(in_df, in_rounding_to):
        if in_rounding_to:
            round_index = math.log10(
                in_rounding_to
            )  # TODO: fix the round_index warning
        else:
    ...
    use_without_perc_val_df = (
        in_percentage_use_without_df.round(1).astype(str)
        + "%"
        + " ("
        + round_format(in_use_without_df, in_rounding_use_to).astype(str)
        + ")"
    )
    use_with_perc_val_df = (
        in_percentage_use_with_df.round(1).astype(str)
        + "%"
        + " ("
    ...
    def add_total_cons_costs_tp(in_costs_df):
        ...
        total_costs_without_tp.index = pd.MultiIndex.from_tuples(
            [(x, "co_modern_total") for x in total_costs_without_tp.index]
        )
        out_costs_df = pd.concat([in_costs_df, total_costs_without_tp]).sort_index(
        ...
        out_costs_df_reordered = pd.DataFrame(
            index=pd.MultiIndex.from_product([tp_order, out_costs_df.index.levels[1]]),
    ...
    for tp in in_interv_costs_with_df.index:
        interv_cons_total.append(
            in_interv_costs_with_df.loc[tp, "interventions_total"]
            + in_costs_with_df.loc[(tp, "co_modern_total"), "Costs"]
        )
    ...
    interv_costs_usd_without_df = (
        in_interv_costs_without_df * in_mwk_to_usd_exchange_rate
    )
    ...
    if in_rounding_costs_mwk_to:
        ...
        in_interv_costs_without_df = round_df(
            in_interv_costs_without_df, in_rounding_costs_mwk_to
        )
        in_interv_costs_with_df = round_df(
            in_interv_costs_with_df, in_rounding_costs_mwk_to
        )
    if in_rounding_costs_usd_to:
        ...
        interv_costs_usd_without_df = round_df(
            interv_costs_usd_without_df, in_rounding_costs_usd_to
        )
        interv_costs_usd_with_df = round_df(
            interv_costs_usd_with_df, in_rounding_costs_usd_to
        )
    ...
    if in_plot_costs:
        ...
        all_cons_costs_without_tp = (
            in_costs_without_df.loc[(slice(None), "co_modern_total"), "Costs"]
            .reset_index(level=1, drop=True)
            .tolist()
        )
        all_cons_costs_with_tp = (
            in_costs_with_df.loc[(slice(None), "co_modern_total"), "Costs"]
            .reset_index(level=1, drop=True)
            .tolist()
        )
        # create lists with interv costs
        pop_interv_costs_with_tp_l = in_interv_costs_with_df[
            "pop_intervention_cost"
        ].tolist()
        ppfp_interv_costs_with_tp_l = in_interv_costs_with_df[
            "ppfp_intervention_cost"
        ].tolist()
        bar_chart_costs.plot_costs(
            [in_datestamp_without_log, in_datestamp_with_log],
            in_suffix,
            list(in_interv_costs_with_df.index),
            all_cons_costs_without_tp,
            all_cons_costs_with_tp,
            pop_interv_costs_with_tp_l,
            ppfp_interv_costs_with_tp_l,
    ...
    costs_mwk_usd_without_df = (
        in_costs_without_df.astype(str) + " (" + costs_usd_without_df.astype(str) + ")"
    )
    costs_mwk_usd_with_df = (
        in_costs_with_df.astype(str) + " (" + costs_usd_with_df.astype(str) + ")"
    )
    interv_costs_mwk_usd_without_df = (
        in_interv_costs_without_df.astype(str)
        + " ("
        + interv_costs_usd_without_df.astype(str)
        + ")"
    )
    interv_costs_mwk_usd_with_df = (
        in_interv_costs_with_df.astype(str)
        + " ("
        + interv_costs_usd_with_df.astype(str)
        + ")"
    )
    ...
    def combine_use_costs_with_without_interv(
        in_df_use_without,
        in_df_use_perc_without,
        in_df_costs_without,
        in_df_interv_costs_without,
        in_df_use_with,
        in_df_use_perc_with,
        in_df_costs_with,
        in_df_interv_costs_with,
    ):
        data = []
        for tp in in_df_use_without.index:
            for in_df_use, in_df_use_perc, in_df_costs, in_df_interv_costs in [
                ...
                for meth in in_df_use.columns:
                    if meth in list(
                        in_df_costs.loc[tp].index.get_level_values(
                ...
                if in_df_interv_costs.empty:
                    ...
                    l_tp_costs.append(
                        in_df_interv_costs.loc[tp, "pop_intervention_cost"]
                    )
                    l_tp_costs.append(
                        in_df_interv_costs.loc[tp, "ppfp_intervention_cost"]
                    )
        ...
        df_combined = pd.DataFrame(
            data[:],
            columns=pd.Index(
                table_cols, name="Contraception method"
            ),  # TODO: maybe remove?
            index=pd.MultiIndex.from_product(
                [
                    ...
                    [
                        str(in_use_output).capitalize()
                        + " % of\nwomen using\n("
                        + rounding_name(in_rounding_use_to)
                        + " users)",
                        "Costs in\n"
                        + rounding_name(in_rounding_costs_mwk_to)
                        + " MWK\n"
                        + "("
                        + rounding_name(in_rounding_costs_mwk_to / 1000)
                        + " USD)",
                    ],
                ]
            ),
    ...
    use_costs_table_df = use_costs_table_df.rename(
        index={
            "co_modern_total": "modern contraceptives\nTOTAL",
    ...
    use_costs_table_df.index = use_costs_table_df.index.map(
        lambda s: s.replace("_", " ")
    )

    output_table_file = (
        r"outputs/output_table_"
        + in_use_output
        + "-use_costs"
        + "__"
        + in_ID_without
...
def table_cons(
    ...
    assert (
        np.count_nonzero(
            resource_items_pkgs_df["Intervention_Pkg"] == last_line_interv_pkg_name
        )
        == 1
    )
    # TODO: This works only if there is only one item for the last pkg. Needs to be improved to work with a pkg with
    #  more items.
    last_line_nmb_data = resource_items_pkgs_df.loc[
        resource_items_pkgs_df["Intervention_Pkg"] == last_line_interv_pkg_name
    ].index[0]
    co_pkgs_df = resource_items_pkgs_df.loc[
        0:last_line_nmb_data,
    ...
    if "male_sterilization" not in in_contraceptives_order:
        co_pkgs_df = co_pkgs_df.loc[
            co_pkgs_df["Intervention_Pkg"] != "Male sterilization", :
        ]
    # Rename the columns
    co_pkgs_df.columns = [
        "\nContraception\npackage",
    ...
    def pkg_name_to_method(in_co_pkg_name):
        ...
        if in_co_pkg_name == "Contraception initiation":
            ...
            raise ValueError(
                "There is an unrecognised co. Intervention_Pkg name: "
                + str(in_co_pkg_name)
                + "."
            )
    ...
    co_pkgs_df["\nContraception\npackage"] = co_pkgs_df[
        "\nContraception\npackage"
    ].apply(pkg_name_to_method)
    co_pkgs_df = co_pkgs_df.sort_values(
        by="\nContraception\npackage",
