...
def table_use_costs__plot_costs(
    ...
    def add_total_cons_costs_tp(in_costs_df):
        ...
        out_costs_df = pd.concat([in_costs_df, total_costs_without_tp]).sort_index(level=[0, 1], sort_remaining=False)
        ...
        out_costs_df_reordered = pd.DataFrame(
            index=pd.MultiIndex.from_product([tp_order, out_costs_df.index.levels[1]]), columns=out_costs_df.columns
        )
    ...
    def combine_use_costs_with_without_interv(
        ...
        df_combined = pd.DataFrame(
            data[:],
            columns=pd.Index(table_cols, name="Contraception method"),  # TODO: maybe remove?
...
def table_cons(
    ...
    co_pkgs_df = resource_items_pkgs_df.loc[
        0:last_line_nmb_data, ["Intervention_Pkg", "Items", "Expected_Units_Per_Case", "Unit_Cost"]
    ]
    ...
    def pkg_name_to_method(in_co_pkg_name):
        ...
        if in_co_pkg_name == "Contraception initiation":
            ...
            raise ValueError("There is an unrecognised co. Intervention_Pkg name: " + str(in_co_pkg_name) + ".")
    ...
    co_pkgs_df = co_pkgs_df.sort_values(
        by="\nContraception\npackage", key=lambda x: x.map(lambda v: contraception_pkgs_order.index(v))
    )
