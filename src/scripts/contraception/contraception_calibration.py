...
sim.register(
    ...
    contraception.Contraception(
        resourcefilepath=resourcefilepath, use_healthsystem=False
    ),
...
# Helper Functions
AGE_RANGE_CATEGORIES, AGE_RANGE_LOOKUP = (
    sim.modules["Demography"].AGE_RANGE_CATEGORIES,
    sim.modules["Demography"].AGE_RANGE_LOOKUP,
)
...
def get_asfr_per_month_implied_by_contraceptive_use(
    contraceptive_use: pd.DataFrame,
) -> dict:
    ...
    preg_per_month["not_using"] = pp["p_pregnancy_no_contraception_per_month"][
        "hv_inf_False"
    ]  # (for simplicity, assume all persons HIV-negative)
    preg_per_month.loc[:, sorted(states - {"not_using"})] = pp[
        "p_pregnancy_with_contraception_per_month"
    ].loc[:, sorted(states - {"not_using"})]
    preg_per_month = preg_per_month.groupby(
        by=preg_per_month.index.map(AGE_RANGE_LOOKUP)
    ).mean()

    # Total live births per month: sum across risk of pregnancy from all births and multiply by prob of live birth.
    return (
        (contraceptive_use * preg_per_month).sum(axis=1) * prob_live_births
    ).to_dict()
...
assumption_init_use = (
    pp["initial_method_use"]
    .groupby(by=pp["initial_method_use"].index.map(AGE_RANGE_LOOKUP))
    .mean()
)
assert np.isclose(1.0, assumption_init_use.sum(axis=1)).all()

# Get the model-induced age-specific fertility rate, per month
asfr_per_month_init = get_asfr_per_month_implied_by_contraceptive_use(
    assumption_init_use
)
...
wpp_fert_per_month_2010 = get_medium_variant_asfr_from_wpp_resourcefile(
    wpp, months_exposure=1
)[2010]

# Plot
plt.plot(
    asfr_per_month_init.keys(),
    asfr_per_month_init.values(),
    "k",
    label="Model (Expectation)",
)
plt.plot(
    wpp_fert_per_month_2010.keys(), wpp_fert_per_month_2010.values(), "r-", label="WPP"
)
...
current_sf = (sim.modules["Contraception"]).parameters[
    "scaling_factor_on_monthly_risk_of_pregnancy"
]
y = np.array(list(wpp_fert_per_month_2010.values())) / (
    np.array(list(asfr_per_month_init.values())) / current_sf
)
print(
    f"Approximate scaling factor on pregnancy should be (by age_group): {[round(_x, 2) for _x in y]}"
)
...
def run_sim(end_date=Date(2029, 12, 31)):
    ...
    poll = contraception.ContraceptionPoll(
        module=sim.modules["Contraception"], run_do_pregnancy=False
    )
    age_update_event = demography.AgeUpdateEvent(
        sim.modules["Demography"], sim.modules["Demography"].AGE_RANGE_LOOKUP
    )
    ...
    for date in pd.date_range(sim.date, end_date, freq=pd.DateOffset(months=1)):
        ...
        _usage_by_age[date] = (
            sim.population.props.loc[
                (sim.population.props.sex == "F")
                & (sim.population.props.age_years.between(15, 49))
            ]
        ...
        df.loc[(df.age_exact_years > 50.0), "date_of_birth"] = sim.date - pd.DateOffset(
            years=15
        )
...
def plot(df, title=""):
    ...
    fig, ax = plt.subplots()
    df.loc[spacing].apply(lambda row: row / row.sum(), axis=1).plot.bar(
...
for age_grp in ["15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49"]:
    plot(
        pd.DataFrame.from_dict(
            {
                _date.date(): usage_by_age[_date].unstack()[age_grp]
                for _date in usage_by_age
            },
            orient="index",
        ),
        title=age_grp,
...
#  WPP ASFR per month:
wpp_fert_per_month = pd.DataFrame(
    get_medium_variant_asfr_from_wpp_resourcefile(wpp, months_exposure=1)
)

# ASFR per month in model:
model = dict()
for _d in usage_by_age:
    model[_d] = get_asfr_per_month_implied_by_contraceptive_use(
        format_usage_results(usage_by_age[_d])
    )
...
for i, agegrp in enumerate(adult_age_groups):
    ...
    # Comparative plot:
    l1 = ax[i].plot(
        model_this_agegrp.index.year, model_this_agegrp.values, "r-", label="Model"
    )
