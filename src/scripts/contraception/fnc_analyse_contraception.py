...
def analyse_contraception(
    ...
    if in_plot_use_time_bool or in_plot_use_time_method_bool or in_plot_pregnancies_bool:
        ...
        if in_plot_use_time_bool:
            ...
            plt.savefig(
                outputpath / ("Contraception Use " + in_id + "_UpTo" + str(plot_months.year[-1]) + in_suffix + ".png"),
                format="png",
            )
        ...
        if in_plot_use_time_method_bool:
            ...
            # Plot absolut values
            fig, ax = plt.subplots()
            ax.plot(np.asarray(plot_months), Model_pill * scaling_factor, color=colours_all_meths[0], ls=line_style)
            ax.plot(np.asarray(plot_months), Model_IUD * scaling_factor, color=colours_all_meths[1], ls=line_style)
            ax.plot(
                np.asarray(plot_months), Model_injections * scaling_factor, color=colours_all_meths[2], ls=line_style
            )
            ax.plot(np.asarray(plot_months), Model_implant * scaling_factor, color=colours_all_meths[3], ls=line_style)
            ax.plot(
                np.asarray(plot_months), Model_male_condom * scaling_factor, color=colours_all_meths[4], ls=line_style
            )
            ax.plot(
            ...
            ax.plot(
                np.asarray(plot_months), Model_other_modern * scaling_factor, color=colours_all_meths[6], ls=line_style
            )
            ax.plot(
                np.asarray(plot_months),
            ...
            ax.plot(
                np.asarray(plot_months), Model_withdrawal * scaling_factor, color=colours_all_meths[8], ls=line_style
            )
            ax.plot(
                np.asarray(plot_months),
            ...
            plt.savefig(
                outputpath
                / ("Contraception Use By Method " + in_id + "_UpTo" + str(plot_months.year[-1]) + in_suffix + ".png"),
                format="png",
            )
            ...
            plt.savefig(
                outputpath
                / ("Prop Fem1549 Using Method " + in_id + "_UpTo" + str(plot_months.year[-1]) + in_suffix + ".png"),
                format="png",
            )
        ...
        if in_plot_pregnancies_bool:
            ...
            plt.savefig(
                outputpath / ("Pregnancies per Year " + in_id + "_UpTo" + str(plot_years[-1]) + in_suffix + ".png"),
                format="png",
            )
    ...
    if in_plot_depend_ratio_bool:
        ...
        def demog_gend_labor(in_demog_gend_df_by_years):
            ...
            out_demog_gend_df_by_years["non-labor_force"] = out_demog_gend_df_by_years.loc[
                :, ["0-4", "5-9", "10-14", "65-69", "70-74", "75-79", "80-84", "85-89"]
            ].sum(axis=1)
        ...
        plt.savefig(
            outputpath / ("Dependency Ratio over Time " + in_id + "_UpTo" + str(plot_years[-1]) + in_suffix + ".png"),
            format="png",
        )
    ...
    if in_calc_use_costs_bool:
        ...
        print("Years simulated:", co_use_modern_df.loc[1, "year"], "-", last_year_simulated)
        print()
        ...
        def create_time_period_data(in_l_time_period_start, in_df):
            ...
            tp_df = in_df.loc[
                (in_l_time_period_start[0] <= in_df["year"]) & (in_df["year"] < in_l_time_period_start[-1])
            ].copy()
        ...
        co_use_modern_tp_df = create_time_period_data(in_required_time_period_starts, co_use_modern_df)
        ...
        def create_percentage_use_df(in_df_use_incl_women_total):
            ...
            df_percentage_use.iloc[:, :-1] = (
                df_percentage_use.iloc[:, :-1].div(df_percentage_use["women_total"], axis=0).mul(100, axis=0)
            )
        ...
        co_use_modern_tp_df = co_use_modern_tp_df.loc[:, co_use_modern_tp_df.columns != "women_total"]
        ...
        if in_use_output == "mean":
            co_output_use_modern_tp_df = co_use_modern_tp_df.groupby("Time_Period").mean()
            ...
            co_output_percentage_use_df = co_percentage_use_df.groupby("Time_Period").mean()
            # Include the output summation for all time periods:
            ...
            co_output_use_modern_tp_df = co_use_modern_tp_df.groupby("Time_Period").max()
            co_output_use_modern_tp_df = co_output_use_modern_tp_df.append(
                sum_use_all_times(co_output_use_modern_tp_df, in_use_output)
            )
            co_output_percentage_use_df = co_percentage_use_df.groupby("Time_Period").max()
            co_output_percentage_use_df = co_output_percentage_use_df.append(
                sum_use_all_times(co_output_percentage_use_df, in_use_output)
            )
        ...
        def join_avail_notavail_items(in_df):
            ...
            for i in range(len(in_df.index)):
                l_requests.append(
                    merge_dicts([eval(in_df.loc[i]["Item_Available"]), eval(in_df.loc[i]["Item_NotAvailable"])])
                )
            return l_requests
        ...
        def get_contraceptive_method_for_request(in_d):
            ...
            if (
                dict(
                    {8: 1, 307: 0.5, 15: 1, 1960: 3, 75: 2, 2676: 3, 2677: 3, 21: 0.25, 112: 2, 23: 8, 5: 2, 49: 0.2}
                ).items()
                <= in_d.items()
            ):
        ...
        cons_time_and_method_df = create_time_period_data(in_required_time_period_starts, cons_processed_df)

        # Group consumables data by time period and contraceptive method:
        cons_time_and_method_df = cons_time_and_method_df.set_index(["Time_Period", "Contraceptive_Method"])
        ...
        def get_intervention_pkg_name(in_co_meth_name):
            ...
            if in_co_meth_name == "female_sterilization":
                ...
                raise ValueError("There is an unrecognised co. method name: " + str(in_co_meth_name) + ".")
        ...
        def calculate_costs(in_df_resource_items_pkgs, in_df_cons_avail_by_time_and_method, in_df_mean_use):
            """
            ...
            for i in in_df_cons_avail_by_time_and_method.index:
                ...
                item_avail_dict = in_df_cons_avail_by_time_and_method.loc[i, "Item_Available_summation"]
        ...
        cons_avail_grouped_by_time_and_method_df["Costs"] = calculate_costs(
            resource_items_pkgs_df, cons_avail_grouped_by_time_and_method_df, mean_use_df
        )
        ...
        cons_costs_by_time_and_method_df = cons_avail_grouped_by_time_and_method_df.loc[:, "Costs"].copy().to_frame()
        ...
        if in_calc_intervention_costs_bool:
            interv_info_df = log_df["tlo.methods.contraception"]["interventions_start_date"].set_index("date").copy()
        ...
        if calc_intervention_costs_bool:
            ...
            pop_interv_cost_2016 = (
                float(co_analysis_params["value"].loc[co_analysis_params["parameter_name"] == "pop_intervention_cost"])
                * 1.81
            )
            ppfp_interv_cost_2016 = (
                float(co_analysis_params["value"].loc[co_analysis_params["parameter_name"] == "ppfp_intervention_cost"])
                * 1.81
            )
            ...
            co_interv_costs_tp_df = create_time_period_data(in_required_time_period_starts, popsize1549)
