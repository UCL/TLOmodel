...
with open(output_path / f"depression_analysis_structured_logs.pickle", "wb") as handle:
    pickle.dump(output, handle)

structured_log = output

with open(output_path / f"depression_analysis_parsed_logs.pickle", "rb") as handle:
    stdlib_log = pickle.load(handle)

for key in stdlib_log['tlo.methods.depression']:
    ...
    stdlib_table = stdlib_log['tlo.methods.depression'][key].round(6).astype('object')
    structured_table = structured_log['tlo.methods.depression'][key].round(6).astype('object')
