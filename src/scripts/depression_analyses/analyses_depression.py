...
def run_simulation_with_set_service_coverage_parameter(
    service_availability, healthsystemdisable
):
    """
...
results_health_system_disabled = compute_key_outputs_for_last_3_years(
    parse_log_file(
        run_simulation_with_set_service_coverage_parameter(
            service_availability=["*"], healthsystemdisable=True
        )
    )
)
...
calibration_df.at["Current prevalence of depression, aged 15+ females", "Data"] = [
    0.10,
    0.08,
]
calibration_df.at["Rate of suicide incidence per 100k persons aged 15+", "Data"] = [
    26.1,
    8.0,
    3.7,
]
...
results_no_intvs = compute_key_outputs_for_last_3_years(
    parse_log_file(
        run_simulation_with_set_service_coverage_parameter(
            service_availability=[], healthsystemdisable=False
        )
    )
)

# Make a table to compare the effects of having vs not having any interventions
effect_of_intvs_df = pd.DataFrame(
    data={"Intvs_On": results_health_system_disabled, "Intvs_Off": results_no_intvs}
)
...
def run_simulation_with_intvs_maximised():
    ...
    sim.register(
        ...
        enhanced_lifestyle.Lifestyle(resourcefilepath=resourcefilepath),
        healthsystem.HealthSystem(
            resourcefilepath=resourcefilepath, service_availability=["*"], disable=True
        ),
        symptommanager.SymptomManager(resourcefilepath=resourcefilepath),
        healthseekingbehaviour.HealthSeekingBehaviour(
            resourcefilepath=resourcefilepath
    ...
    sim.modules["Depression"].parameters[
...
results_max_intvs = compute_key_outputs_for_last_3_years(
    parse_log_file(run_simulation_with_intvs_maximised())
)
