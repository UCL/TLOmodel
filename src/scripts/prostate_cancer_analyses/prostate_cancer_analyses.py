...
def run_sim(service_availability):
    log_config = {"filename": "LogFile", "directory": outputpath}
...
def get_summary_stats(logfile):
    ...
    annual_count_of_dxtr = counts_by_stage.groupby(by="year")[
        ["diagnosed_since_last_log", "treated_since_last_log", "palliative_since_last_log"]
    ].sum()
...
results_no_healthsystem["dalys"].plot.bar(y=["YLD_ProstateCancer_0", "YLL_ProstateCancer_ProstateCancer"], stacked=True)
