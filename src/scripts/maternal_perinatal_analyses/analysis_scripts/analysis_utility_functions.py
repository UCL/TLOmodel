...
def line_graph_with_ci_and_target_rate(
    sim_years, data, target_data_dict, ylim, y_label, title, graph_location, file_name
):
    ...
    ax.fill_between(sim_years, data[1], data[2], label="95% CI", color="b", alpha=0.1)
...
def simple_line_chart_with_ci(sim_years, data, y_title, title, file_name, graph_location):
    ...
    ax.fill_between(sim_years, data[1], data[2], color="b", alpha=0.1, label="95% CI")
    plt.ylabel(y_title)
    plt.xlabel("Year")
    plt.title(title)
    plt.legend()
...
def comparison_graph_multiple_scenarios_multi_level_dict(
    colours, intervention_years, data_dict, key, y_label, title, graph_location, save_name
):
    """Outputs and saves line plot of an outcome over time, including uncertainty, in addition to a pre-determined
    ...
    for k, colour in zip(data_dict, colours):
        ...
        ax.fill_between(intervention_years, data_dict[k][key][1], data_dict[k][key][2], color=colour, alpha=0.1)
...
def comparison_bar_chart_multiple_bars(
    data, dict_name, intervention_years, colours, y_title, title, plot_destination_folder, save_name
):
    ...
    for k, position, colour in zip(data, [ind - width, ind, ind + width, ind + width * 2, ind + width * 3], colours):
...
def return_birth_data_from_multiple_scenarios(results_folders, sim_years, intervention_years):
    ...
    def extract_births(folder):
        ...
        agg_births_data = get_mean_from_columns(
            births_results.loc[intervention_years[0] : intervention_years[-1]], "agg"
        )
        agg_births = get_mean_95_CI_from_list(agg_births_data)

        return {
            "total_births": total_births_per_year,
...
def return_pregnancy_data_from_multiple_scenarios(results_folders, sim_years, intervention_years):
    ...
    def extract_pregnancies(folder):
        ...
        return {
            "total_preg": total_pregnancies_per_year,
