...
def get_comp_mean_and_rate_across_multiple_dataframes(
    complication, denominators, rate, dataframes, sim_years
):
        ...
        for year, denominator in zip(sim_years, denominators):
            if year in df.index:
                if complication in df.loc[year].index:
                    ...
                    lq = (
                        df.loc[year, complication].quantile(0.025) / denominator
                    ) * rate
                    uq = (
                        df.loc[year, complication].quantile(0.925) / denominator
                    ) * rate
    ...
    if len(dataframes) == 2:
        ...
        total_rates = [
            x + y + z for x, y, z in zip(df_1_data[0], df_2_data[0], df_3_data[0])
        ]
        total_lq = [
            x + y + z for x, y, z in zip(df_1_data[1], df_2_data[1], df_3_data[1])
        ]
        total_uq = [
            x + y + z for x, y, z in zip(df_1_data[2], df_2_data[2], df_3_data[2])
        ]
...
def line_graph_with_ci_and_target_rate(
    sim_years,
    mean_list,
    lq_list,
    uq_list,
    ...
    ax.fill_between(
        sim_years, lq_list, uq_list, color="b", alpha=0.1, label="UI (2.5-92.5)"
    )

    if target_data_dict["double"]:
        plt.errorbar(
            target_data_dict["first"]["year"],
            target_data_dict["first"]["value"],
            label=target_data_dict["first"]["label"],
            yerr=target_data_dict["first"]["ci"],
            fmt="o",
            color="darkseagreen",
            ecolor="green",
            elinewidth=3,
        ...
        plt.errorbar(
            target_data_dict["second"]["year"],
            target_data_dict["second"]["value"],
            label=target_data_dict["second"]["label"],
            yerr=target_data_dict["second"]["ci"],
            fmt="o",
            color="red",
            ecolor="mistyrose",
            elinewidth=3,
...
def basic_comparison_graph(
    intervention_years, bdata, idata, x_label, title, graph_location, save_name
):
    fig, ax = plt.subplots()
    ...
    ax.plot(
        intervention_years, idata[0], label="Intervention (mean)", color="olivedrab"
    )
    ax.fill_between(
        intervention_years,
        idata[1],
...
def simple_line_chart_with_target(
    sim_years, model_rate, target_rate, y_title, title, file_name, graph_location
):
    plt.plot(sim_years, model_rate, "o-g", label="Model", color="deepskyblue")
...
def simple_line_chart_with_ci(
    sim_years, data, y_title, title, file_name, graph_location
):
    fig, ax = plt.subplots()
    ax.plot(sim_years, data[0], label="Model (mean)", color="deepskyblue")
    ax.fill_between(
        sim_years, data[1], data[2], color="b", alpha=0.1, label="UI (2.5-92.5)"
    )
    plt.ylabel(y_title)
    plt.xlabel("Year")
    plt.title(title)
...
def simple_bar_chart(
    model_rates, x_title, y_title, title, file_name, sim_years, graph_location
):
    bars = sim_years
...
def return_median_and_mean_squeeze_factor_for_hsi(
    folder, hsi_string, sim_years, graph_location
):
    hsi_med = extract_results(
        ...
        custom_generate_series=(
            lambda df: df.loc[
                df["TREATMENT_ID"].str.contains(hsi_string) & df["did_run"]
            ]
            .assign(year=df["date"].dt.year)
            .groupby(["year"])["Squeeze_Factor"]
            .median()
        ),
    )
    ...
    simple_line_chart_with_ci(
        sim_years,
        data,
        "Median Squeeze Factor",
        f"Median Yearly Squeeze for HSI {hsi_string}",
        f"median_sf_{hsi_string}",
        graph_location,
    )
    ...
    simple_line_chart_with_ci(
        ...
        data,
        "Mean Squeeze Factor",
        f"Mean Yearly Squeeze for HSI {hsi_string}",
        f"mean_sf_{hsi_string}",
        graph_location,
    )
...
def return_squeeze_plots_for_hsi(folder, hsi_string, sim_years, graph_location):
    ...
    lq_squeeze_per_year = [
        np.percentile(hsi.loc[year].to_numpy(), 2.5) for year in sim_years
    ]
    uq_squeeze_per_year = [
        np.percentile(hsi.loc[year].to_numpy(), 92.5) for year in sim_years
    ]
    ...
    hsi_squeeze = extract_results(
        ...
        custom_generate_series=(
            lambda df: df.loc[
                (df["TREATMENT_ID"].str.contains(hsi_string))
                & df["did_run"]
                & (df["Squeeze_Factor"] > 0)
            ]
            .assign(year=df["date"].dt.year)
    ...
    prop_squeeze_year = [
        (
            hsi_squeeze.loc[year].to_numpy().mean()
            / hsi_count.loc[year].to_numpy().mean()
        )
        * 100
        for year in sim_years
    ]
    prop_squeeze_lq = [
        (
            ...
            / np.percentile(hsi_count.loc[year].to_numpy(), 2.5)
        )
        * 100
        for year in sim_years
    ]
    ...
    prop_squeeze_uq = [
        (
            ...
            / np.percentile(hsi_count.loc[year].to_numpy(), 92.5)
        )
        * 100
        for year in sim_years
    ]
    ...
    simple_line_chart_with_ci(
        ...
        prop_data,
        "% HSIs",
        f"Proportion of HSI {hsi_string} where squeeze > 0",
        f"prop_sf_{hsi_string}",
        graph_location,
    )


def comparison_graph_multiple_scenarios(
    intervention_years, data_dict, y_label, title, graph_location, save_name
):
    fig, ax = plt.subplots()

    for k, colour in zip(
        data_dict, ["deepskyblue", "olivedrab", "darksalmon", "orchid"]
    ):
...
def comparison_graph_multiple_scenarios_multi_level_dict(
    ...
    for k, colour in zip(
        data_dict, ["deepskyblue", "olivedrab", "darksalmon", "darkviolet"]
    ):
...
def comparison_bar_chart_multiple_bars(
    data,
    dict_name,
    intervention_years,
    y_title,
    ...
    for k, position, colour in zip(
        data,
        [ind - width, ind, ind + width, ind + width * 2],
...
def return_pregnancy_data_from_multiple_scenarios(results_folders, intervention_years):
    ...
    def extract_pregnancies(folder):
        ...
        total_pregnancies_per_year = get_mean_and_quants(
            preg_results, intervention_years
        )
...
def return_death_data_from_multiple_scenarios(
    results_folders, births_dict, intervention_years, detailed_log
):
    """
    ...
    def extract_deaths(folder, births):
        ...
        other_preg_deaths = extract_results(
            ...
            custom_generate_series=(
                lambda df: df.assign(year=df["date"].dt.year)
                .groupby(["year", "label", "pregnancy"])["year"]
                .count()
            ),
            do_scaling=True,
        )
        ...
        crude_m_deaths = get_mean_and_quants_from_str_df(
            death_results_labels, "Maternal Disorders", intervention_years
        )
            ...
            for year in intervention_years:
                ...
                for cause in indirect_causes:
                    if cause in other_preg_deaths.loc[year, :, True].index:
                        id_deaths_per_year += other_preg_deaths.loc[
                            year, cause, True
                        ].mean()
                        id_lq_py += other_preg_deaths.loc[year, cause, True].quantile(
                            0.025
                        )
                        id_uq_pu += other_preg_deaths.loc[year, cause, True].quantile(
                            0.925
                        )
        ...
        total_mmr = [
            [
                ((x + y) / z) * 100000
                for x, y, z in zip(indirect_deaths[0], crude_m_deaths[0], births[0])
            ],
            [
                ((x + y) / z) * 100000
                for x, y, z in zip(indirect_deaths[1], crude_m_deaths[1], births[1])
            ],
            [
                ((x + y) / z) * 100000
                for x, y, z in zip(indirect_deaths[2], crude_m_deaths[2], births[2])
            ],
        ]

        # Extract NMR
        nmr = get_comp_mean_and_rate(
            "Neonatal Disorders",
            births[0],
            death_results_labels,
            1000,
        ...
        crude_n_deaths = get_mean_and_quants_from_str_df(
            death_results_labels, "Neonatal Disorders", intervention_years
        )
    ...
    return {
        k: extract_deaths(results_folders[k], births_dict[k]) for k in results_folders
    }
...
def get_differences_between_two_outcomes(baseline_data, comparator):
    ...
    percentage_diff = [
        100 - ((x / y) * 100) for x, y in zip(comparator[0], baseline_data[0])
    ]
...
def return_stillbirth_data_from_multiple_scenarios(
    ...
    def extract_stillbirths(folder, births):
        ...
        # Get stillbirths
        an_still_birth_data = get_mean_and_quants(
            an_stillbirth_results, intervention_years
        )
        ip_still_birth_data = get_mean_and_quants(
            ip_stillbirth_results, intervention_years
        )

        # Store mean number of stillbirths, LQ, UQ
        crude_sb = [
            [x + y for x, y in zip(an_still_birth_data[0], ip_still_birth_data[0])],
        ...
        # Then generate SBR
        an_sbr = [
            [(x / y) * 1000 for x, y in zip(an_still_birth_data[0], births[0])],
            [(x / y) * 1000 for x, y in zip(an_still_birth_data[1], births[1])],
            [(x / y) * 1000 for x, y in zip(an_still_birth_data[2], births[2])],
        ]

        ip_sbr = [
            [(x / y) * 1000 for x, y in zip(ip_still_birth_data[0], births[0])],
        ...
        total_sbr = [
            [
                ((x + y) / z) * 1000
                for x, y, z in zip(
                    an_still_birth_data[0], ip_still_birth_data[0], births[0]
                )
            ],
            [
                ((x + y) / z) * 1000
                for x, y, z in zip(
                    an_still_birth_data[1], ip_still_birth_data[1], births[1]
                )
            ],
            [
                ((x + y) / z) * 1000
                for x, y, z in zip(
                    an_still_birth_data[2], ip_still_birth_data[2], births[2]
                )
            ],
        ]
    ...
    return {
        k: extract_stillbirths(results_folders[k], births_dict[k])
...
def return_dalys_from_multiple_scenarios(results_folders, intervention_years):
    ...
    def get_dalys_from_scenario(results_folder, intervention_years):
        ...
        person_years_total = extract_results(
            ...
            custom_generate_series=(
                lambda df: df.assign(
                    total=(df["M"].apply(lambda x: sum(x.values())))
                    + df["F"].apply(lambda x: sum(x.values()))
                )
                .assign(year=df["date"].dt.year)
                .groupby(["year"])["total"]
                .sum()
        ...
        dalys_mat = get_comp_mean_and_rate(
            "Maternal Disorders", denom[0], dalys_stacked, 100000, intervention_years
        )
        dalys_neo = get_comp_mean_and_rate(
            "Neonatal Disorders", denom[0], dalys_stacked, 100000, intervention_years
        )
        ...
        return {
            "maternal_dalys_crude": extract_dalys_tlo_model("Maternal"),
    ...
    return {
        k: get_dalys_from_scenario(results_folders[k], intervention_years)
        for k in results_folders
    }
