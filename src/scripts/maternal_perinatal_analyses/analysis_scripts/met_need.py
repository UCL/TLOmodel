...
def met_need_and_contributing_factors_for_deaths(
    scenario_file_dict, outputspath, intervention_years, service_of_interest
):
    """
    ...
    neo_comp_dfs = {
        k: analysis_utility_functions.get_modules_neonatal_complication_dataframes(
            results_folders[k]
        )
        for k in results_folders
    }
    ...
    # ============================================ MET NEED ==========================================================
    def get_total_interventions_delivered(
        results_folder, interventions, intervention_years
    ):
        ...
        for treatment in interventions:
            if treatment == "neo_sep_treat":
                ...
                treatment_dict.update(
                    {
                        treatment: analysis_utility_functions.get_mean_and_quants_from_str_df(
    ...
    ints = {
        k: get_total_interventions_delivered(
            results_folders[k], treatments, intervention_years
        )
        for k in results_folders
    }
    ...
    def get_crude_complication_numbers(mat_comps, neo_comps, intervention_years):
        ...
        crude_comps.update(
            {
                "ectopic": analysis_utility_functions.get_mean_and_quants_from_str_df(
                    mat_comps["pregnancy_supervisor"],
                    "ectopic_unruptured",
                    intervention_years,
                )
            }
        ...
        crude_comps.update(
            {"an_ip_sepsis": sum_lists(incidence_an_sep, incidence_la_sep)}
        )

        # Antenatal/Intrapartum Haemorrhage
        incidence_an_haem_mm = (
        ...
        for lp in [0, 1, 2]:
            data = [
                incidence_an_haem_mm[lp],
                incidence_an_haem_s[lp],
                incidence_la_haem_mm[lp],
        ...
        crude_comps.update(
            {"pp_sepsis": sum_lists(incidence_pn_l_sep, incidence_pn_p_sep)}
        )
        ...
        crude_comps.update(
            {"pph_uterine_atony": sum_lists(incidence_ua_pph, incidence_oth_pph)}
        )
        ...
        incidence_s_rp = analysis_utility_functions.get_mean_and_quants_from_str_df(
            mat_comps["postnatal_supervisor"],
            "secondary_postpartum_haemorrhage",
            intervention_years,
        )
        crude_comps.update(
            {"pph_retained_p": sum_lists(incidence_p_rp, incidence_s_rp)}
        )
        ...
        for list_pos in [0, 1, 2]:
            surg_data.append(
                [x * 0.43 for x in crude_comps["pph_uterine_atony"][list_pos]]
            )
            surg_data_rp.append(
                [x * 0.3 for x in crude_comps["pph_retained_p"][list_pos]]
            )
        ...
        crude_comps.update(
            {
                "uterine_rupture": analysis_utility_functions.get_mean_and_quants_from_str_df(
        ...
        crude_comps.update(
            {
                "spe_pn": analysis_utility_functions.get_mean_and_quants_from_str_df(
                    mat_comps["postnatal_supervisor"],
                    "severe_pre_eclamp",
                    intervention_years,
                )
        ...
        crude_comps.update(
            {
                "sgh_pn": analysis_utility_functions.get_mean_and_quants_from_str_df(
                    mat_comps["postnatal_supervisor"],
                    "severe_gest_htn",
                    intervention_years,
                )
        ...
        crude_comps.update(
            {
                "ec_pn": analysis_utility_functions.get_mean_and_quants_from_str_df(
        ...
        crude_comps.update(
            {
                "obs_labour": analysis_utility_functions.get_mean_and_quants_from_str_df(
        ...
        crude_comps.update(
            {"obs_labour_cpd": sum_lists(incidence_a_ec, incidence_p_ec)}
        )
        crude_comps.update(
            {"obs_labour_other": sum_lists(incidence_a_ec, incidence_p_ec)}
        )
        ...
        for lp in [0, 1, 2]:
            data = [
                incidence_prds[lp],
                incidence_nbab[lp],
                incidence_enceph_mi[lp],
                incidence_enceph_mo[lp],
    ...
    comp_numbers = {
        k: get_crude_complication_numbers(
            comp_dfs[k], neo_comp_dfs[k], intervention_years
        )
        for k in results_folders
    }
    ...
    def get_cs_indication_counts(folder):
        ...
        for indication in ["ol", "ur"]:  # 'spe_ec', 'other', 'previous_scar'
            cs_id_counts.update(
                {
                    f"{indication}_cs": analysis_utility_functions.get_mean_and_quants_from_str_df(
        ...
        pa_cs = analysis_utility_functions.get_mean_and_quants_from_str_df(
            cs_results, "an_aph_pa", intervention_years
        )
        pp_cs = analysis_utility_functions.get_mean_and_quants_from_str_df(
            cs_results, "an_aph_pp", intervention_years
        )
        la_aph_cs = analysis_utility_functions.get_mean_and_quants_from_str_df(
            cs_results, "la_aph", intervention_years
        )
    ...
    cs_indication = {
        k: get_cs_indication_counts(results_folders[k]) for k in results_folders
    }
    ...
    def get_met_need(ints, cs_data, crude_comps):
        ...
        def update_met_need_dict(comp, treatment, cs):
            ...
            mean_met_need = [
                (x / y) * 100 for x, y in zip(iv[treatment][0], crude_comps[comp][0])
            ]
    ...
    met_need = {
        k: get_met_need(ints[k], cs_indication[k], comp_numbers[k]) for k in ints
    }

    for t in treatments:
        fig, ax = plt.subplots()
        for k, colour in zip(
            met_need, ["deepskyblue", "olivedrab", "darksalmon", "darkviolet"]
        ):
    ...
    def get_factors_impacting_death(results_folder, factors, intervention_years):
        ...
        deaths = analysis_utility_functions.get_mean_and_quants(
            total_deaths, intervention_years
        )
        ...
        for factor in factors:
            factor_df = extract_results(
                ...
                key="death_mni",
                custom_generate_series=(
                    lambda df: df.loc[df[factor]]
                    .assign(year=df["date"].dt.year)
                    .groupby(["year", factor])["year"]
                    .count()
                ),
                do_scaling=True,
            )
            ...
            year_means = [
                death_factors.loc[year, True].mean()
                for year in intervention_years
                if year in death_factors.index
            ]
            lower_quantiles = [
                death_factors.loc[year, True].quantile(0.025)
                for year in intervention_years
                if year in death_factors.index
            ]
            upper_quantiles = [
                death_factors.loc[year, True].quantile(0.925)
                for year in intervention_years
                if year in death_factors.index
            ]
    ...
    death_causes = {
        k: get_factors_impacting_death(results_folders[k], factors, intervention_years)
        for k in results_folders
    }
    ...
    for f in factors:
        fig, ax = plt.subplots()
        for k, colour in zip(
            death_causes, ["deepskyblue", "olivedrab", "darksalmon", "darkviolet"]
        ):
