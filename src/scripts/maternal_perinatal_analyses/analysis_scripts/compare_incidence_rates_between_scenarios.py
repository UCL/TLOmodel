...
def compare_key_rates_between_multiple_scenarios(
    scenario_file_dict, service_of_interest, outputspath, intervention_years
):
    """
    ...
    comp_dfs = {
        k: analysis_utility_functions.get_modules_maternal_complication_dataframes(
            results_folders[k]
        )
    ...
    neo_comp_dfs = {
        k: get_neonatal_comp_dfs(results_folders[k]) for k in results_folders
    }

    # ------------------------------------ PREGNANCIES AND BIRTHS ----------------------------------------------------
    # Extract and plot pregnancies and births across scenarios
    preg_dict = (
        analysis_utility_functions.return_pregnancy_data_from_multiple_scenarios(
    ...
    def get_completed_pregnancies(comps_df, total_births_per_year, results_folder):
        ...
        ia_mean_numbers_per_year = (
            analysis_utility_functions.get_mean_and_quants_from_str_df(
                comps_df["pregnancy_supervisor"], "induced_abortion", intervention_years
            )[0]
        )
        ...
        an_still_birth_data = analysis_utility_functions.get_mean_and_quants(
            an_stillbirth_results, intervention_years
        )
    ...
    comp_pregs = {
        k: get_completed_pregnancies(comp_dfs[k], births_dict[k][0], results_folders[k])
        for k in results_folders
    }
    ...
    def get_twin_data(results_folder, total_births_per_year):
        ...
        twin_data = analysis_utility_functions.get_mean_and_quants(
            twins_results, intervention_years
        )

        total_deliveries = [x - y for x, y in zip(total_births_per_year, twin_data[0])]
        final_twining_rate = [
            (x / y) * 100 for x, y in zip(twin_data[0], total_deliveries)
        ]
    ...
    twin_data = {
        k: get_twin_data(results_folders[k], births_dict[k][0]) for k in results_folders
    }
    ...
    ectopic_data = {
        k: analysis_utility_functions.get_comp_mean_and_rate(
            "ectopic_unruptured",
            preg_dict[k][0],
            comp_dfs[k]["pregnancy_supervisor"],
            1000,
            intervention_years,
        )
        for k in results_folders
    ...
    def get_anaemia_output_at_birth(results_folder, total_births_per_year):
        ...
        no_anaemia_data = analysis_utility_functions.get_mean_and_quants_from_str_df(
            anaemia_results, "none", intervention_years
        )

        prevalence_of_anaemia_per_year = [
            100 - ((x / y) * 100)
            for x, y in zip(no_anaemia_data[0], total_births_per_year)
        ]
        no_anaemia_lqs = [
            100 - ((x / y) * 100)
            for x, y in zip(no_anaemia_data[1], total_births_per_year)
        ]
        no_anaemia_uqs = [
            100 - ((x / y) * 100)
            for x, y in zip(no_anaemia_data[2], total_births_per_year)
        ]

        return [prevalence_of_anaemia_per_year, no_anaemia_lqs, no_anaemia_uqs]

    anaemia_birth_data = {
        k: get_anaemia_output_at_birth(results_folders[k], births_dict[k][0])
        for k in results_folders
    }
    ...
    def get_anaemia_output_at_delivery(results_folder, total_births_per_year):
        ...
        no_anaemia_data = analysis_utility_functions.get_mean_and_quants_from_str_df(
            pnc_anaemia, "none", intervention_years
        )
        prevalence_of_anaemia_per_year = [
            100 - ((x / y) * 100)
            for x, y in zip(no_anaemia_data[0], total_births_per_year)
    ...
    anaemia_delivery_data = {
        k: get_anaemia_output_at_delivery(results_folders[k], births_dict[k][0])
        for k in results_folders
    }
    ...
    def get_htn_disorders_outputs(comps_df, total_births_per_year):
        ...
        output["gh"] = (
            analysis_utility_functions.get_comp_mean_and_rate_across_multiple_dataframes(
                ...
                total_births_per_year,
                1000,
                [comps_df["pregnancy_supervisor"], comps_df["postnatal_supervisor"]],
                intervention_years,
            )
        )
    ...
    htn_data = {
        k: get_htn_disorders_outputs(comp_dfs[k], births_dict[k][0])
        for k in results_folders
    }
    ...
    abruption = {
        k: analysis_utility_functions.get_comp_mean_and_rate_across_multiple_dataframes(
            "placental_abruption",
            births_dict[k][0],
            1000,
    ...
    aph_data = {
        k: get_aph_data(comp_dfs[k], births_dict[k][0]) for k in results_folders
    }
    ...
    ptl_data = {
        k: get_ptl_data(births_dict[k][0], comp_dfs[k]) for k in results_folders
    }
    ...
    lbw = {
        k: analysis_utility_functions.get_comp_mean_and_rate(
            "low_birth_weight",
            births_dict[k][0],
            neo_comp_dfs[k]["newborn_outcomes"],
            100,
            intervention_years,
        )
        for k in results_folders
    ...
    def get_delivery_data(results_folder, total_births_per_year):
        ...
        return {"cs": cs_data, "avd": avd_data}

    delivery_data = {
        k: get_delivery_data(results_folders[k], births_dict[k][0])
        for k in results_folders
    }
    ...
    def get_total_sepsis_rates(total_births_per_year, comps_df):
        ...
        an_number = analysis_utility_functions.get_mean_and_quants_from_str_df(
            comps_df["pregnancy_supervisor"],
            "clinical_chorioamnionitis",
            intervention_years,
        )
        ...
        la_number = analysis_utility_functions.get_mean_and_quants_from_str_df(
            comps_df["labour"], "sepsis", intervention_years
        )
        ...
        pn_number = analysis_utility_functions.get_mean_and_quants_from_str_df(
            comps_df["postnatal_supervisor"], "sepsis", intervention_years
        )

        complete_pn_sep_data = [
            x + y for x, y in zip(pn_la_sep_data[0], pn_sep_data[0])
        ]
        ...
        sep_lq = [
            x + y + z
            for x, y, z in zip(an_sep_data[1], la_sep_data[1], complete_pn_sep_lq)
        ]
        sep_uq = [
            x + y + z
            for x, y, z in zip(an_sep_data[2], la_sep_data[2], complete_pn_sep_up)
        ]

        return {
            "total": [total_sep_rates, sep_lq, sep_uq],
            "an": an_sep_data,
            "la": la_sep_data,
    ...
    sep_data = {
        k: get_total_sepsis_rates(births_dict[k][0], comp_dfs[k])
        for k in results_folders
    }
    ...
    pph_data = {
        k: get_pph_data(births_dict[k][0], comp_dfs[k]) for k in results_folders
    }
    ...
    def get_neonatal_sepsis(total_births_per_year, nb_comp_dfs):
        ...
        total_ns_rates = [
            x + y + z
            for x, y, z in zip(early_ns_data[0], early_ns_pn[0], late_ns_data[0])
        ]
        ns_lqs = [
            x + y + z
            for x, y, z in zip(early_ns_data[1], early_ns_pn[1], late_ns_data[1])
        ]
        ns_uqs = [
            x + y + z
            for x, y, z in zip(early_ns_data[2], early_ns_pn[2], late_ns_data[2])
        ]

        return [total_ns_rates, ns_lqs, ns_uqs]

    neo_sep_data = {
        k: get_neonatal_sepsis(births_dict[k][0], neo_comp_dfs[k])
        for k in results_folders
    }
    ...
    def get_neonatal_encephalopathy(total_births_per_year, nb_comp_dfs):
        ...
        enceph_lq = [
            x + y + z for x, y, z in zip(mild_data[1], mod_data[1], sev_data[1])
        ]
        enceph_uq = [
            x + y + z for x, y, z in zip(mild_data[2], mod_data[2], sev_data[2])
        ]

        return [total_enceph_rates, enceph_lq, enceph_uq]

    neo_enceph_data = {
        k: get_neonatal_encephalopathy(births_dict[k][0], neo_comp_dfs[k])
        for k in results_folders
    }
        ...
        for cause in simplified_causes:
            if (
                ...
                def get_htn_mmr(death_results):
                    ...
                    deaths = [
                        x + y + z
                        for x, y, z in zip(spe_deaths[0], ec_deaths[0], sgh_deaths[0])
                    ]
                    lq = [
                        x + y + z
                        for x, y, z in zip(spe_deaths[1], ec_deaths[1], sgh_deaths[1])
                    ]
                    uq = [
                        x + y + z
                        for x, y, z in zip(spe_deaths[2], ec_deaths[2], sgh_deaths[2])
                    ]
                ...
                def get_sep_mmr(death_results):
                    ...
                    deaths = [
                        x + y + z
                        for x, y, z in zip(a_deaths[0], i_deaths[0], p_deaths[0])
                    ]
                    lq = [
                        x + y + z
                        for x, y, z in zip(a_deaths[1], i_deaths[1], p_deaths[1])
                    ]
                    uq = [
                        x + y + z
                        for x, y, z in zip(a_deaths[2], i_deaths[2], p_deaths[2])
                    ]
            ...
            mmr_mean = [
                (x / y) * 100000 for x, y in zip(deaths_data[0], total_births[0])
            ]
    ...
    comp_mmrs = {
        k: get_death_data(results_folders[k], births_dict[k]) for k in results_folders
    }
    ...
    for cause in simplified_causes:
        fig, ax = plt.subplots()
        for k, colour in zip(
            comp_mmrs, ["deepskyblue", "olivedrab", "darksalmon", "darkviolet"]
        ):
            ax.plot(
                intervention_years,
                comp_mmrs[k]["mmr_dict"][cause][0],
                label=k,
                color=colour,
            )
            ax.fill_between(
                intervention_years,
                comp_mmrs[k]["mmr_dict"][cause][1],
                comp_mmrs[k]["mmr_dict"][cause][2],
                color=colour,
                alpha=0.1,
            )
    ...
    def get_neo_death_data(results_folder, births):
        ...
        for cause in simplified_causes_neo:
            if (cause == "encephalopathy") or (
                cause == "neonatal_respiratory_depression"
            ):
                ...
                def get_neo_sep_deaths(death_results):
                    ...
                    deaths = [
                        x + y + z for x, y, z in zip(early1[0], early2[0], late[0])
                    ]
    ...
    nmr_data = {
        k: get_neo_death_data(results_folders[k], births_dict[k])
        for k in results_folders
    }
    ...
    for cause in simplified_causes_neo:
        ...
        for k, colour in zip(
            ...
            ax.plot(
                intervention_years,
                nmr_data[k]["nmr_dict"][cause][0],
                label=k,
                color=colour,
            )
            ax.fill_between(
                intervention_years,
                nmr_data[k]["nmr_dict"][cause][1],
                nmr_data[k]["nmr_dict"][cause][2],
                color=colour,
                alpha=0.1,
            )
