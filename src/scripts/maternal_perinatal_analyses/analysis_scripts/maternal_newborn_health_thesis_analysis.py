...
def run_maternal_newborn_health_thesis_analysis(
    scenario_file_dict, outputspath, sim_years, intervention_years, service_of_interest, scen_colours
):
    """
    ...
    path = (
        f"{outputspath}/{service_of_interest}_thesis_analysis_graphs_and_results_"
    ...
    def get_diff_between_runs(dfs, baseline, intervention, keys, intervention_years, output_df):
        ...
        for k in keys:
            ...
            sq_mean_outcome_list_int = get_med_or_mean_from_columns(
                dfs["Status Quo"][k].loc[intervention_years[0] : intervention_years[-1]], operation
            )
            ...
            mean_outcome_list_int = get_med_or_mean_from_columns(
                dfs[intervention][k].loc[intervention_years[0] : intervention_years[-1]], operation
            )
            ...
            mean_diff_list = get_med_or_mean_from_columns(
                int_diff.loc[intervention_years[0] : intervention_years[-1]], operation
            )
            skew_diff_list = scipy.stats.skew(mean_diff_list)
            mean_outcome_diff = get_mean_and_confidence_interval(mean_diff_list)

            median_outcome_diff = [
                round(np.median(mean_diff_list), 2),
    ...
    # complications
    comps_dfs = {
        k: analysis_utility_functions.get_modules_maternal_complication_dataframes(results_folders[k])
        for k in results_folders
    }

    neo_comps_dfs = {
        k: analysis_utility_functions.get_modules_neonatal_complication_dataframes(results_folders[k])
        for k in results_folders
    }

    comp_pregs_dict = {
        k: analysis_utility_functions.get_completed_pregnancies_from_multiple_scenarios(
            comps_dfs[k], births_dict[k], results_folders[k]
        )
        for k in results_folders
    }
    ...
    def extract_death_and_stillbirth_data_frames_and_outcomes(folder, birth_df):
        ...
        nd = extract_results(
            ...
            key="properties_of_deceased_persons",
            custom_generate_series=(
                lambda df: df.loc[(df["age_days"] < 29)]
                .assign(year=df["date"].dt.year)
                .groupby(["year"])["year"]
                .count()
            ),
        ...
        avg_isbr_df = (
            ip_stillbirth_results.loc[intervention_years[0] : intervention_years[-1]]
            / birth_df.loc[intervention_years[0] : intervention_years[-1]]
        ) * 1000
        ip_sbr_means = get_mean_from_columns(avg_isbr_df, "avg")

        avg_asbr_df = (
            an_stillbirth_results.loc[intervention_years[0] : intervention_years[-1]]
            / birth_df.loc[intervention_years[0] : intervention_years[-1]]
        ) * 1000
    ...
    death_data = {
        k: extract_death_and_stillbirth_data_frames_and_outcomes(
            results_folders[k], births_dict[k]["births_data_frame"]
        )
        for k in results_folders
    }

    # Plot key mortality/stillbirth outcomes starting with average outcomes across the intervention period
    for data, title, y_lable in zip(
        [
            "agg_dir_m_deaths",
            "agg_dir_mr",
            "agg_ind_m_deaths",
    ...
    analysis_utility_functions.comparison_graph_multiple_scenarios_multi_level_dict(
        ...
        "Direct Maternal Mortality Ratio per Year by Scenario ",
        primary_oc_path,
        "maternal_mr_direct",
    )
    ...
    for group, abrv in zip(["Maternal", "Neonatal"], ["dir_m", "n"]):
        analysis_utility_functions.comparison_bar_chart_multiple_bars(
            death_data,
            f"crude_{abrv}_deaths",
            sim_years,
            scen_colours,
            f"Total {group} Deaths (scaled)",
            f"Total {group} Deaths per by Scenario ",
            primary_oc_path,
            f"{group}_crude_deaths_comparison.png",
        )
    ...
    def extract_deaths_by_cause(results_folder, births_df, intervention_years):
        ...
        dd = extract_results(
            ...
            key="properties_of_deceased_persons",
            custom_generate_series=(
                lambda df: df.assign(year=df["date"].dt.year).groupby(["year", "cause_of_death"])["year"].count()
            ),
            do_scaling=True,
        )
    ...
    def save_mr_by_cause_data_and_output_graphs(group, cause_d):
        ...
        if d[0] == "m":
            ...
            reformat_labels = [
                ...
                "Malaria",
                "Other",
                "Congenital anomaly (limb)",
                "Congenital anomaly (heart)",
    ...
    cod_data = {
        k: extract_deaths_by_cause(results_folders[k], births_dict[k]["births_data_frame"], intervention_years)
        for k in results_folders
    }

    cod_neo_data = {
        k: extract_neonatal_deaths_by_cause(
            results_folders[k], births_dict[k]["births_data_frame"], sim_years, intervention_years
        )
        for k in results_folders
    }
    ...
    analysis_utility_functions.comparison_bar_chart_multiple_bars(
        ...
        sim_years,
        scen_colours,
        "Total Stillbirths (scaled)",
        "Yearly Baseline Stillbirths Compared to Intervention",
        primary_oc_path,
        "crude_stillbirths_comparison.png",
    )

    for data, title, y_lable in zip(
        ["avg_sbr", "avg_i_sbr", "avg_a_sbr"],
        [
        ...
        ["Stillbirths per 1000 births", "Stillbirths per 1000 births", "Stillbirths per 1000 births"],
    ):
        labels = results_folders.keys()
    ...
    # is plotted
    for data, title, y_lable in zip(
        ["agg_mat_dalys_stacked", "agg_neo_dalys_stacked", "avg_mat_dalys_rate_stacked", "avg_neo_dalys_rate_stacked"],
        [
        ...
        ["DALYs", "DALYs", "DALYs per 100,000 Person-Years", "DALYs per 100,000 Person-Years"],
    ):
        labels = results_folders.keys()
    ...
    for dict_key, axis, title, save_name in zip(
        [
            "maternal_dalys_crude_stacked",
            "maternal_dalys_rate_stacked",
            "maternal_yll_crude_stacked",
            "maternal_yll_rate_stacked",
            "maternal_yld_crude_unstacked",
            "maternal_yld_rate_unstacked",
            "neonatal_dalys_crude_stacked",
            "neonatal_dalys_rate_stacked",
            "neonatal_yll_crude_stacked",
            "neonatal_yll_rate_stacked",
            "neonatal_yld_crude_unstacked",
            "neonatal_yld_rate_unstacked",
        ],
        [
            "DALYs",
        ...
        [
            "DALYs per Year Attributable to Maternal Disorders by " "Scenario ",
            "DALYs per 100,000 Person-Years per Year Attributable to Maternal" " Disorders by Scenario ",
            "Years of Life Lost per Year Attributable to Maternal Disorders " "by Scenario ",
            "Years of Life Lost per 100,000 Person-Years per Year Attributable to " "Maternal Disorders by Scenario ",
            "Years Lived with Disability per Year Attributable to Maternal " "Disorders by Scenario ",
            ...
            "DALYs per 100,000 Person-Years per Year Attributable to Neonatal" " Disorders by Scenario ",
            "Years of Life Lost per Year Attributable to Neonatal disorders by " "Scenario ",
            "Years of Life Lost per 100,000 Person-Years per Year Attributable " "to Neonatal Disorders by Scenario ",
            "Years Lived with Disability per Year Attributable to Neonatal " "Disorders by Scenario ",
            "Years Lived with Disability per 100,000 Person-Years per Year "
            "Attributable to Neonatal Disorders by Scenario ",
        ],
        [
            "maternal_dalys_stacked",
            "maternal_dalys_rate",
            "maternal_yll",
            "maternal_yll_rate",
            "maternal_yld",
            "maternal_yld_rate",
            "neonatal_dalys_stacked",
            "neonatal_dalys_rate",
            "neonatal_yll",
            "neonatal_yll_rate",
            "neonatal_yld",
    ...
    # calculated and these outcomes are saved to an Excel file
    keys = [
        "maternal_total_dalys_df_stacked",
        "neonatal_total_dalys_df_stacked",
        "maternal_dalys_rate_df_stacked",
        "neonatal_dalys_rate_df_stacked",
        "all_total_dalys_df_stacked",
        "all_dalys_rate_df_stacked",
    ]
    ...
    def extract_comp_inc_folders(folder, comps_df, neo_comps_df, pregnancy_df, births_df, comp_preg_df):
        ...
        m_aph_ps = (
            comps_df["pregnancy_supervisor"]
            .loc[(slice(None), "mild_mod_antepartum_haemorrhage"), slice(None)]
            .droplevel(1)
        )
        m_aph_la = comps_df["labour"].loc[(slice(None), "mild_mod_antepartum_haemorrhage"), slice(None)].droplevel(1)
        s_aph_ps = (
            comps_df["pregnancy_supervisor"]
            .loc[(slice(None), "severe_antepartum_haemorrhage"), slice(None)]
            .droplevel(1)
        )
        s_aph_la = comps_df["labour"].loc[(slice(None), "severe_antepartum_haemorrhage"), slice(None)].droplevel(1)
        ...
        an_sep = (
            comps_df["pregnancy_supervisor"].loc[(slice(None), "clinical_chorioamnionitis"), slice(None)].droplevel(1)
        )
        ...
        l_pph = (
            ...
            .loc[(slice(None), "primary_postpartum_haemorrhage"), slice(None)]
            .droplevel(1)
        )
        p_pph = (
            comps_df["postnatal_supervisor"]
            .loc[(slice(None), "secondary_postpartum_haemorrhage"), slice(None)]
            .droplevel(1)
        )
        ...
        pnc_anaemia = extract_results(
            ...
            custom_generate_series=(
                lambda df_: df_.loc[df_["anaemia"] != "none"]
                .assign(year=df_["date"].dt.year)
                .groupby(["year"])["mother"]
                .count()
            ),
            do_scaling=True,
        ...
        resp_distress = (
            neo_comps_df["newborn_outcomes"].loc[(slice(None), "not_breathing_at_birth"), slice(None)].droplevel(1)
        )
        ...
        rds = (
            neo_comps_df["newborn_outcomes"]
            .loc[(slice(None), "respiratory_distress_syndrome"), slice(None)]
            .droplevel(1)
        )
        ...
        eons_pn = (
            neo_comps_df["postnatal_supervisor"].loc[(slice(None), "early_onset_sepsis"), slice(None)].droplevel(1)
        )
    ...
    comp_inc_folders = {
        k: extract_comp_inc_folders(
            results_folders[k],
            comps_dfs[k],
            neo_comps_dfs[k],
            preg_dict[k]["preg_data_frame"],
            births_dict[k]["births_data_frame"],
            comp_pregs_dict[k]["comp_preg_data_frame"],
        )
        for k in results_folders
    }
    ...
    def get_coverage_of_key_maternity_services(folder, births_df):
        ...
        anc_coverage = extract_results(
            ...
            custom_generate_series=(
                lambda df: df.assign(year=df["date"].dt.year).groupby(["year"])["person_id"].count()
            ),
            do_scaling=True,
        )
        ...
        an = extract_results(
            ...
            key="anc_count_on_birth",
            custom_generate_series=(
                lambda df: df.loc[df["total_anc"] >= 4]
                .assign(year=df["date"].dt.year)
        ...
        all_deliveries = extract_results(
            ...
            custom_generate_series=(
                lambda df_: df_.assign(year=df_["date"].dt.year).groupby(["year"])["mother"].count()
            ),
            do_scaling=True,
        )
        ...
        all_surviving_mothers = extract_results(
            ...
            module="tlo.methods.postnatal_supervisor",
            key="total_mat_pnc_visits",
            custom_generate_series=(lambda df: df.assign(year=df["date"].dt.year).groupby(["year"])["mother"].count()),
            do_scaling=True,
        )
        ...
        all_surviving_newborns = extract_results(
            ...
            custom_generate_series=(lambda df: df.assign(year=df["date"].dt.year).groupby(["year"])["child"].count()),
            do_scaling=True,
        )
    ...
    cov_data = {
        k: get_coverage_of_key_maternity_services(results_folders[k], births_dict[k]["births_data_frame"])
        for k in results_folders
    }
    ...
    def extract_secondary_outcomes_dfs(folder, births):
        ...
        results.update({"total_anc_df": anc_count, "anc_contacts_trend": hsi_data, "agg_anc_contacts": agg})
        ...
        an_ip = extract_results(
            ...
            key="HSI_Event",
            custom_generate_series=(
                lambda df: pd.concat([df, df["TREATMENT_ID"].apply(pd.Series)], axis=1)
                .assign(year=df["date"].dt.year)
                .groupby(["year"])["AntenatalCare_Inpatient"]
                .sum()
            ),
            do_scaling=True,
        )
        ...
        pnc_mat_count = extract_results(
            ...
            custom_generate_series=(
                lambda df: pd.concat([df, df["TREATMENT_ID"].apply(pd.Series)], axis=1)
                .assign(year=df["date"].dt.year)
                .groupby(["year"])["PostnatalCare_Maternal"]
                .sum()
            ),
            do_scaling=True,
        )
        ...
        pnc_neo_count = extract_results(
            ...
            custom_generate_series=(
                lambda df: pd.concat([df, df["TREATMENT_ID"].apply(pd.Series)], axis=1)
                .assign(year=df["date"].dt.year)
                .groupby(["year"])["PostnatalCare_Neonatal"]
                .sum()
            ),
            do_scaling=True,
        )
        ...
        results.update(
            {
                "total_mat_pnc_count_df": pnc_mat_count,
        ...
        cs_delivery = extract_results(
            ...
            key="delivery_setting_and_mode",
            custom_generate_series=(
                lambda df_: df_.loc[df_["mode"] == "caesarean_section"]
                .assign(year=df_["date"].dt.year)
                .groupby(["year"])["mother"]
                .count()
            ),
        ...
        results.update({"cs_rate_df": cs_rate_df, "cs_rate_per_year": cs_rate_per_year, "avg_cs_rate_int": avg_cs})
        ...
        avd_delivery = extract_results(
            ...
            key="delivery_setting_and_mode",
            custom_generate_series=(
                lambda df_: df_.loc[df_["mode"] == "instrumental"]
                .assign(year=df_["date"].dt.year)
                .groupby(["year"])["mother"]
                .count()
            ),
        ...
        results.update({"avd_rate_df": avd_rate_df, "avd_rate_per_year": avd_rate, "avg_avd_rate_int": avg_avd})
        ...
        incidence_dates = extract_results(
            folder, module="tlo.methods.malaria", key="incidence", column="inc_1000py", index="date", do_scaling=True
        )
        ...
        results.update(
            {
                "mal_total_clinical_df": preg_clin_counter_years,
        ...
        tb_new_diag_dates = extract_results(
            folder, module="tlo.methods.tb", key="tb_treatment", column="tbNewDiagnosis", index="date", do_scaling=True
        )
        ...
        results.update(
            {
                "tb_treatment_df": tb_treatment_years,
        ...
        results.update(
            {
                "depression_treatment_tt_df": tt_data,
        ...
        results.update(
            {
                "hiv_prog_cov_df": hiv_tests_years,
    ...
    sec_outcomes_df = {
        k: extract_secondary_outcomes_dfs(results_folders[k], births_dict[k]["births_data_frame"])
        for k in results_folders
    }
    ...
    plot_agg_graph(
        ...
        "agg_anc_contacts",
        "Total ANC contacts",
        "Total Number of ANC visits per Scenario",
        "agg_anc_contacts",
        secondary_oc_path,
    )
    ...
    plot_agg_graph(
        sec_outcomes_df,
        "mal_clin_counter_agg",
        "Number of Clinical Cases",
        "Total Clinical Cases of Malaria During Pregnancy Per Scenario",
        "mal_agg_clin_cases",
        secondary_oc_path,
    )
    ...
    plot_agg_graph(
        ...
        "tb_diagnosis_agg",
        "Number of Tb Diagnoses",
        "Total New Tb Cases Diagnosed per Scenario",
        "tb_diagnoses_agg",
        secondary_oc_path,
    )
    ...
    plot_agg_graph(
        ...
        "art_number_agg",
        "Number of Women",
        "Total Number of Women Receiving ART per Per Scenario",
        "hiv_women_art_agg",
        secondary_oc_path,
    )
