...
def run_maternal_newborn_health_analysis(
    scenario_file_dict,
    outputspath,
    intervention_years,
    service_of_interest,
    ...
    results_folders = {
        k: get_scenario_outputs(scenario_file_dict[k], outputspath)[-1]
        for k in scenario_file_dict
    }
    ...
    if not os.path.isdir(path):
        os.makedirs(
            f'{outputspath}/{service_of_interest}_analysis_output_graphs_{results_folders["Status Quo"].name}'
        )
    ...
    births_dict = analysis_utility_functions.return_birth_data_from_multiple_scenarios(
        results_folders, intervention_years
    )
    ...
    if service_of_interest == "anc" or show_all_results:
        ...
        def get_anc_coverage(folder, service_structure):
            ...
            anc_cov_of_interest = extract_results(
                ...
                key="anc_count_on_birth",
                custom_generate_series=(
                    lambda df: df.loc[df["total_anc"] >= service_structure]
            ...
            mean_total_anc = analysis_utility_functions.get_mean_and_quants(
                anc_coverage, intervention_years
            )
            mean_cov_anc = analysis_utility_functions.get_mean_and_quants(
                anc_cov_of_interest, intervention_years
            )

            # The calculate the mean proportion of women receiving the ANC coverage of interest
            result_m = [
                (x / y) * 100 for x, y in zip(mean_cov_anc[0], mean_total_anc[0])
            ]
            result_lq = [
                (x / y) * 100 for x, y in zip(mean_cov_anc[1], mean_total_anc[1])
            ]
            result_uq = [
                (x / y) * 100 for x, y in zip(mean_cov_anc[2], mean_total_anc[2])
            ]

            return [result_m, result_lq, result_uq]

        cov_data_4 = {
            k: get_anc_coverage(results_folders[k], 4) for k in results_folders
        }
        cov_data_8 = {
            k: get_anc_coverage(results_folders[k], 8) for k in results_folders
        }
        ...
        for service_structure, cov_data in zip([4, 8], [cov_data_4, cov_data_8]):
            analysis_utility_functions.comparison_graph_multiple_scenarios(
                ...
                plot_destination_folder,
                f"anc{service_structure}_cov",
            )
    ...
    if service_of_interest == "sba" or show_all_results:
        ...
        def get_delivery_place_info(folder, intervention_years):
            ...
            mean_total_deliveries = [
                x + y + z for x, y, z in zip(hb_data[0], hc_data[0], hp_data[0])
            ]

            home_birth_rate = [
                (x / y) * 100 for x, y in zip(hb_data[0], mean_total_deliveries)
            ]
            home_birth_lq = [
                (x / y) * 100 for x, y in zip(hb_data[1], mean_total_deliveries)
            ]
            home_birth_uq = [
                (x / y) * 100 for x, y in zip(hb_data[2], mean_total_deliveries)
            ]

            health_centre_rate = [
                (x / y) * 100 for x, y in zip(hc_data[0], mean_total_deliveries)
            ]
            health_centre_lq = [
                (x / y) * 100 for x, y in zip(hc_data[1], mean_total_deliveries)
            ]
            health_centre_uq = [
                (x / y) * 100 for x, y in zip(hc_data[2], mean_total_deliveries)
            ]

            hospital_rate = [
                (x / y) * 100 for x, y in zip(hp_data[0], mean_total_deliveries)
            ]
            hospital_lq = [
                (x / y) * 100 for x, y in zip(hp_data[1], mean_total_deliveries)
            ]
            hospital_uq = [
                (x / y) * 100 for x, y in zip(hp_data[2], mean_total_deliveries)
            ]
            ...
            return {
                "hb": [home_birth_rate, home_birth_lq, home_birth_uq],
        ...
        delivery_data = {
            k: get_delivery_place_info(results_folders[k], intervention_years)
    ...
    if service_of_interest == "pnc" or show_all_results:
        ...
        def get_pnc_coverage(folder, birth_data):
            ...
            pn_mat_data = analysis_utility_functions.get_mean_and_quants(
                pnc_results_maternal, intervention_years
            )
            pn_neo_data = analysis_utility_functions.get_mean_and_quants(
                pnc_results_newborn, intervention_years
            )

            # Use birth data to calculate coverage as a proportion of total births
            pnc_1_plus_rate_mat = [
                (x / y) * 100 for x, y in zip(pn_mat_data[0], birth_data[0])
            ]
            ...
            pnc1_plus_rate_neo = [
                (x / y) * 100 for x, y in zip(pn_neo_data[0], birth_data[0])
            ]
            ...
            return {
                "maternal": [pnc_1_plus_rate_mat, pnc_mat_lqs, pnc_mat_uqs],
                "neonatal": [pnc1_plus_rate_neo, pnc_neo_lqs, pnc_neo_uqs],
            }

        coverage_data = {
            k: get_pnc_coverage(results_folders[k], births_dict[k])
            for k in results_folders
        }
    ...
    death_data = analysis_utility_functions.return_death_data_from_multiple_scenarios(
        results_folders, births_dict, intervention_years, detailed_log=True
    )
    ...
    for group, group_label in zip(["Maternal", "Neonatal"], ["m", "n"]):
        analysis_utility_functions.comparison_bar_chart_multiple_bars(
            death_data,
            f"crude_{group_label}_deaths",
            intervention_years,
            f"Total {group} Deaths (scaled)",
            f"Yearly Baseline {group} Deaths Compared to Intervention",
            plot_destination_folder,
    ...
    if (service_of_interest != "pnc") or show_all_results:
        sbr_data = (
            analysis_utility_functions.return_stillbirth_data_from_multiple_scenarios(
    ...
    dalys_data = analysis_utility_functions.return_dalys_from_multiple_scenarios(
        results_folders, intervention_years
    )
    for dict_key, axis, title, save_name in zip(
        [
            "maternal_dalys_crude",
            "maternal_dalys_rate",
            "maternal_yll_crude",
            "maternal_yll_rate",
            "maternal_yld_crude",
            "maternal_yld_rate",
            "neonatal_dalys_crude",
            "neonatal_dalys_rate",
            "neonatal_yll_crude",
            "neonatal_yll_rate",
            "neonatal_yld_crude",
        ...
        [
            "DALYs",
            "DALYs per 100k Person-Years",
            "YLL",
            "YLL per 100k Person-Years",
            "YLD",
            "YLD per 100k Person-Years",
            "DALYs",
            "DALYs per 100k Person-Years",
            "YLL",
            "YLL per 100k Person-Years",
    ...
    for output, data in zip(output_list, data_list):
        diff_data = {
            k: analysis_utility_functions.get_differences_between_two_outcomes(
                data["Status Quo"][output], data[k][output]
            )
            for k in keys_list
        }  # todo: not ideal to have to change key names
        ...
        for dict_name in ["crude", "percentage"]:
            for k, position, colour in zip(
                diff_data, [ind, ind + width], ["bisque", "powderblue"]
            ):
            ...
            plt.xlabel("Years")
            plt.title(
                f"Mean Reduction of {output} from Baseline per Year ({dict_name})"
            )
            plt.legend(loc="best")
    ...
    if service_of_interest == "anc" or show_all_results:
        ...
        def get_hsi_counts_from_cowdp_logger(folder, intervention_years):
            ...
            hsi_data = analysis_utility_functions.get_mean_and_quants(
                hsi, intervention_years
            )

            return hsi_data

        hs_data = {
            k: get_hsi_counts_from_cowdp_logger(results_folders[k], intervention_years)
            for k in results_folders
        }
    ...
    if service_of_interest == "pnc" or show_all_results:
        ...
        def get_hsi_counts_from_summary_logger(folder, intervention_years):
            hsi = extract_results(
                ...
                custom_generate_series=(
                    lambda df: pd.concat(
                        [df, df["TREATMENT_ID"].apply(pd.Series)], axis=1
                    )
                    .assign(year=df["date"].dt.year)
            ...
            hsi_data_neo = analysis_utility_functions.get_mean_and_quants(
                hsi_n, intervention_years
            )

            return {"mat": hsi_data, "neo": hsi_data_neo}

        hs_data = {
            k: get_hsi_counts_from_summary_logger(
                results_folders[k], intervention_years
            )
            for k in results_folders
        }
        ...
        analysis_utility_functions.comparison_graph_multiple_scenarios_multi_level_dict(
            ...
            plot_destination_folder,
            f"{service_of_interest}_mat_visits",
        )
        ...
        analysis_utility_functions.comparison_graph_multiple_scenarios_multi_level_dict(
            ...
            plot_destination_folder,
            f"{service_of_interest}_neo_visits",
        )
    ...
    if service_of_interest == "anc" or show_all_results:
        ...
        def get_malaria_incidence_in_pregnancy(folder):
            ...
            preg_clinical_counter = analysis_utility_functions.get_mean_and_quants(
                preg_clin_counter_years, intervention_years
            )
            ...
            incidence = analysis_utility_functions.get_mean_and_quants(
                incidence_years, intervention_years
            )

            return {"clin_counter": preg_clinical_counter, "incidence": incidence}

        mal_data = {
            k: get_malaria_incidence_in_pregnancy(results_folders[k])
            for k in results_folders
        }
        ...
        def get_tb_info_in_pregnancy(folder):
            ...
            tb_diagnosis = analysis_utility_functions.get_mean_and_quants(
                tb_new_diag_years, intervention_years
            )
            # Treatment coverage
            ...
            tb_treatment = analysis_utility_functions.get_mean_and_quants(
                tb_treatment_years, intervention_years
            )

            return {"diagnosis": tb_diagnosis, "treatment": tb_treatment}

        tb_data = {
            k: get_tb_info_in_pregnancy(results_folders[k]) for k in results_folders
        }
    ...
    if (service_of_interest != "sba") or show_all_results:
        ...
        def get_hiv_information(folder):
            ...
            hiv_tests = analysis_utility_functions.get_mean_and_quants(
                hiv_tests_years, intervention_years
            )
            ...
            hiv_test_rate = analysis_utility_functions.get_mean_and_quants(
                hiv_tests_rate_years, intervention_years
            )
            ...
            art = analysis_utility_functions.get_mean_and_quants(
                art_years, intervention_years
            )

            return {
                "testing_prop": hiv_tests,
    ...
    if service_of_interest != "sba" or show_all_results:
        ...
        def get_depression_info_in_pregnancy(folder, intervention_years):
            ...
            diag_final = analysis_utility_functions.get_mean_and_quants(
                diag_data, intervention_years
            )
            ...
            ad_data = anti_depress_reindexed.groupby(
                anti_depress_reindexed.index
            ).mean()
            ad_final = analysis_utility_functions.get_mean_and_quants(
                ad_data, intervention_years
            )
            ...
            tt_final = analysis_utility_functions.get_mean_and_quants(
                tt_data, intervention_years
            )

            return {"diag": diag_final, "ad": ad_final, "tt": tt_final}

        depression_data = {
            k: get_depression_info_in_pregnancy(results_folders[k], intervention_years)
            for k in results_folders
        }
