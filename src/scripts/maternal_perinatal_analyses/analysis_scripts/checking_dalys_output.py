...
num_deaths_mat_disorders = (
    (pd.to_datetime(deaths.date).dt.year == 2010)
    & (deaths.label == "Maternal " "Disorders")
).sum()
# 49 deaths

# number of YLL (stacked onto year 2010)
yll = output["tlo.methods.healthburden"]["yll_by_causes_of_death_stacked"]
map_to_label_deaths = pd.Series(
    output["tlo.methods.demography"]["mapper_from_tlo_cause_to_common_label"]
    .drop(columns={"date"})
    .loc[0]
).to_dict()
...
yll_mat_disorders = sum_yll.groupby(level=0).sum()[
    "Maternal Disorders"
]  # 2038 life-years
yll_mat_disorders_per_death = (
    yll_mat_disorders / num_deaths_mat_disorders
)  # 40.1 YLL per death


yld = output["tlo.methods.healthburden"]["yld_by_causes_of_disability"]
map_to_label_disability = pd.Series(
    output["tlo.methods.healthburden"]["mapper_from_tlo_cause_to_common_label"]
    .drop(columns={"date"})
    .loc[0]
).to_dict()
