...
def output_all_death_calibrations(
    scenario_filename, outputspath, pop_size, sim_years, daly_years
):
    ...
    if not os.path.isdir(path):
        os.makedirs(
            f"{outputspath}/calibration_output_graphs_{pop_size}_{results_folder.name}/death"
        )

    graph_location = path

    # read in daly data
    dalys_data = pd.read_csv(
        Path("./resources/gbd") / "ResourceFile_Deaths_and_DALYS_GBD2019.CSV"
    )
    gbd_years = [2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019]

    # ============================================HELPER FUNCTIONS... =================================================
    def simple_line_chart_two_targets(
        model_rate, target_rate_one, target_rate_two, x_title, y_title, title, file_name
    ):
        plt.plot(sim_years, model_rate, "o-g", label="Model", color="deepskyblue")
        plt.plot(
            sim_years, target_rate_one, "o-g", label="Target", color="darkseagreen"
        )
        plt.plot(
            sim_years, target_rate_two, "o-g", label="Target (adj.)", color="powderblue"
        )
        plt.xlabel(x_title)
        plt.ylabel(y_title)
        plt.title(title)
    ...
    total_births_per_year_ex2010_data = analysis_utility_functions.get_mean_and_quants(
        births_results_exc_2010, sim_years
    )
    ...
    death_results = extract_results(
        ...
        custom_generate_series=(
            ...
            .groupby(["year", "label"])["year"]
            .count()
            .fillna(0)
        ),
    )
    death_results_labels = death_results.fillna(0)
    mm = analysis_utility_functions.get_comp_mean_and_rate(
        "Maternal Disorders",
        total_births_per_year_ex2010,
        death_results_labels,
        100000,
        sim_years,
    )
    ...
    id_preg_data = analysis_utility_functions.get_mean_and_quants(
        indirect_deaths, sim_years
    )

    id_mmr_data = [
        [
            (x / y) * 100000
            for x, y in zip(id_preg_data[0], total_births_per_year_ex2010)
        ],
        [
            (x / y) * 100000
            for x, y in zip(id_preg_data[1], total_births_per_year_ex2010)
        ],
        [
            (x / y) * 100000
            for x, y in zip(id_preg_data[2], total_births_per_year_ex2010)
        ],
    ]

    total_mmr_data = [
        [x + y for x, y in zip(id_mmr_data[0], mm[0])],
    ...
    for data, title, l_colour, f_colour in zip(
        ...
        ax.fill_between(sim_years, data[1], data[2], color=f_colour, alpha=0.1)
        plt.errorbar(
            2010,
            (675 * mp),
            yerr=((780 * mp) - (570 * mp)) / 2,
            label="DHS 2010",
            fmt="o",
            color="green",
        ...
        ax.plot(
            [2011, 2015, 2017],
            [(444 * mp), (370 * mp), (349 * mp)],
            label="WHO MMEIG",
            color="red",
        )
        ax.fill_between(
            [2011, 2015, 2017],
            [(347 * mp), (269 * mp), (244 * mp)],
            [(569 * mp), (517 * mp), (507 * mp)],
            color="pink",
            alpha=0.1,
        )
        ax.plot(
    ...
    target_indirect_mmr_dict = {
        ...
        "first": {
            ...
            "value": 675 * 0.3,
            "label": "DHS 2010",
            "ci": ((780 * 0.3) - (570 * 0.3)) / 2,
        },
        "second": {
            "year": 2015,
            "value": 439 * 0.3,
            "label": "DHS 2015",
            "ci": ((531 * 0.3) - (348 * 0.3)) / 2,
        },
    }
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_indirect_mmr_dict,
        "% of total births",
        "Indirect Maternal Mortality Ratio per Year (w/v Target)",
        graph_location,
        "indirect_mmr_w_target",
    )
    ...
    ax.bar(
        ...
        bottom=[
            a + b
            for a, b in zip(
                indirect_deaths_means["AIDS_non_TB"], indirect_deaths_means["Malaria"]
            )
        ],
        color="darkmagenta",
    )
    ...
    m_deaths = analysis_utility_functions.get_mean_and_quants_from_str_df(
        scaled_deaths, "Maternal Disorders", gbd_years
    )

    def extract_deaths_gbd_data(group):
        dalys_df = dalys_data.loc[
            (dalys_data["measure_name"] == "Deaths")
            & (dalys_data["cause_name"] == group)
            & (dalys_data["Year"] > 2009)
        ]
        gbd_deaths = list()
        gbd_deaths_lq = list()
        gbd_deaths_uq = list()
        ...
        for year in daly_years:
            ...
            gbd_deaths_lq.append(
                dalys_df.loc[(dalys_df["Year"] == year)].sum()["GBD_Lower"]
            )
            gbd_deaths_uq.append(
                dalys_df.loc[(dalys_df["Year"] == year)].sum()["GBD_Upper"]
            )
    ...
    gbd_ci = [
        (x - y) / 2
        for x, y in zip(gbd_deaths_2010_2019_data[2], gbd_deaths_2010_2019_data[1])
    ]
    ...
    for cause, tr in zip(simplified_causes, trs):
        if (
            ...
            or (cause == "antepartum_haemorrhage")
            or (cause == "uterine_rupture")
        ):
            deaths = analysis_utility_functions.get_mean_and_quants_from_str_df(
                death_results, cause, sim_years
            )[0]
    ...
    def pie_prop_cause_of_death(values, years, labels, title):
        ...
        plt.savefig(
            f"{graph_location}/mat_death_by_cause_{title}_{years}.png",
            bbox_inches="tight",
        )
        plt.show()
    ...
    for index in props_df.index:
        ...
        labels_10.append(
            f"{index} ({round(props_df.loc[index, slice(2010, 2014)].mean(), 2)} %)"
        )
        values_15.append(props_df.loc[index, slice(2015, 2020)].mean())
        labels_15.append(
            f"{index} ({round(props_df.loc[index, slice(2015, 2020)].mean(), 2)} %)"
        )
    ...
    simplified_df["Abortion"] = (
        simplified_df["induced_abortion"] + simplified_df["spontaneous_abortion"]
    )
    simplified_df["Severe PE/Eclampsia"] = (
        simplified_df["severe_pre_eclampsia"] + simplified_df["eclampsia"]
    )
    simplified_df["PPH"] = (
        simplified_df["postpartum_haemorrhage"]
        + simplified_df["secondary_postpartum_haemorrhage"]
    )

    simplified_df["Sepsis"] = pd.Series(0, index=sim_years)
    if "postpartum_sepsis" in simplified_df.columns:
        simplified_df["Sepsis"] = (
            simplified_df["Sepsis"] + simplified_df["postpartum_sepsis"]
        )
    if "intrapartum_sepsis" in simplified_df.columns:
        simplified_df["Sepsis"] = (
            simplified_df["Sepsis"] + simplified_df["intrapartum_sepsis"]
        )
    if "antenatal_sepsis" in simplified_df.columns:
        simplified_df["Sepsis"] = (
            simplified_df["Sepsis"] + simplified_df["antenatal_sepsis"]
        )
    ...
    for column in simplified_df.columns:
        ...
        labels_10.append(
            f"{column} ({round(simplified_df.loc[slice(2010, 2014), column].mean(), 2)} %)"
        )
        values_15.append(simplified_df.loc[slice(2015, 2020), column].mean())
        labels_15.append(
            f"{column} ({round(simplified_df.loc[slice(2015, 2020), column].mean(), 2)} %)"
        )
    ...
    mean_ec = (
        analysis_utility_functions.get_comp_mean_and_rate_across_multiple_dataframes(
            "eclampsia", dummy_denom, 1, [an_comps, la_comps, pn_comps], sim_years
        )[0]
    )

    mean_sgh = (
        analysis_utility_functions.get_comp_mean_and_rate_across_multiple_dataframes(
            "severe_gest_htn", dummy_denom, 1, [an_comps, la_comps, pn_comps], sim_years
        )[0]
    )
    ...
    for inc_list, complication in zip(
        [
            mean_ep,
            mean_sa,
            mean_ia,
        ...
        cfr = analysis_utility_functions.get_comp_mean_and_rate(
            complication, inc_list, death_results, 100, sim_years
        )[0]
    ...
    analysis_utility_functions.simple_line_chart_with_target(
        ...
        tr,
        "Total CFR",
        "Yearly CFR for antenatal/intrapartum sepsis",
    ...
    analysis_utility_functions.simple_line_chart_with_target(
        ...
        tr,
        "Total CFR",
        "Yearly CFR for Sepsis (combined)",
        "combined_sepsis_cfr_per_year",
        graph_location,
    )
    ...
    analysis_utility_functions.simple_line_chart_with_target(
        ...
        tr,
        "Total CFR",
        "Yearly CFR for PPH (combined)",
        "combined_pph_cfr_per_year",
        graph_location,
    )
    ...
    analysis_utility_functions.simple_line_chart_with_target(
        ...
        tr,
        "Total CFR",
        "Yearly CFR for Abortion (combined)",
    ...
    total_spec_deaths = [
        x + y + z for x, y, z in zip(spe_deaths, ec_deaths, sgh_deaths)
    ]
    ...
    analysis_utility_functions.simple_line_chart_with_target(
        ...
        tr,
        "Total CFR",
        "Yearly CFR for Severe Pre-eclampsia/Eclampsia",
    ...
    tnd = extract_results(
        ...
        key="properties_of_deceased_persons",
        custom_generate_series=(
            lambda df: df.loc[(df["age_days"] < 28) & (df["cause_of_death"] != "Other")]
            .assign(year=df["date"].dt.year)
            .groupby(["year"])["year"]
            .count()
        ),
    ...
    t_nm = analysis_utility_functions.get_mean_and_quants(
        total_neonatal_deaths, sim_years
    )
    tnmr = [x / y * 1000 for x, y in zip(t_nm[0], total_births_per_year_ex2010_data[0])]
    tnmr_lq = [
        x / y * 1000 for x, y in zip(t_nm[1], total_births_per_year_ex2010_data[1])
    ]
    tnmr_uq = [
        x / y * 1000 for x, y in zip(t_nm[2], total_births_per_year_ex2010_data[2])
    ]
    ...
    id_nd = extract_results(
        ...
        custom_generate_series=(
            lambda df: df.loc[
                (df["age_days"] < 28)
                & df["cause_of_death"].str.contains(
                    "AIDS_non_TB|AIDS_TB|TB|ALRI|Diarrhoea|Malaria|anomaly"
                )
            ]
            .assign(year=df["date"].dt.year)
            .groupby(["year"])["year"]
    ...
    i_nm = analysis_utility_functions.get_mean_and_quants(
        indirect_neonatal_deaths, sim_years
    )
    i_nmr = [
        x / y * 1000 for x, y in zip(i_nm[0], total_births_per_year_ex2010_data[0])
    ]
    i_tnmr_lq = [
        x / y * 1000 for x, y in zip(i_nm[1], total_births_per_year_ex2010_data[1])
    ]
    i_tnmr_uq = [
        x / y * 1000 for x, y in zip(i_nm[2], total_births_per_year_ex2010_data[2])
    ]
    ...
    def get_nmr_graphs(data, colours, title, save_name):
        ...
        if save_name != "neonatal_disorders_nmr":
            data = {
                ...
                "un": {
                    "mean": [28, 27, 26, 25, 24, 23, 22, 22, 20, 20],
            ...
            data = {
                "dhs": {"mean": [25, 22], "lq": [22, 18], "uq": [28, 28]},
                "hug": {"mean": 18.6, "lq": 13, "uq": 24},
                "gbd": {"mean": 20, "lq": 17.12, "uq": 21.3},
                "un": {
                    "mean": [22.4, 21.6, 20.8, 20, 19.2, 18.4, 17.6, 17.6, 16, 16],
    ...
    get_nmr_graphs(
        nd_nmr,
        ["deepskyblue", "b"],
        'Yearly NMR due to GBD "Neonatal Disorders"',
        "neonatal_disorders_nmr",
    )
    get_nmr_graphs(
        [tnmr, tnmr_lq, tnmr_uq],
        ["coral", "lightcoral"],
        "Yearly Total NMR",
        "total_nmr",
    )
    get_nmr_graphs(
        [i_nmr, i_tnmr_lq, i_tnmr_uq],
        ["seagreen", "mediumseagreen"],
        'Yearly NMR due to causes other than "Neonatal Disorders"',
        "other_nmr",
    )

    # ------------------------------------------ CRUDE DEATHS PER YEAR ------------------------------------------------
    # Neonatal Disorders...
    neo_deaths = analysis_utility_functions.get_mean_and_quants_from_str_df(
        scaled_deaths, "Neonatal Disorders", daly_years
    )
    ...
    gbd_bar_chart(
        neo_deaths,
        gbd_neo_deaths,
        'Total Deaths Attributable to "Neonatal Disorders" per Year',
        "crude_deaths_nd",
    )
    gbd_bar_chart(
        ca_deaths,
    ...
    for year, dictionary in zip(sim_years, list_of_proportions_dicts_nb):
        ...
        plt.savefig(
            f"{graph_location}/neo_death_by_cause_{year}.png", bbox_inches="tight"
        )
        plt.show()
    ...
    early_ns = analysis_utility_functions.get_mean_and_quants_from_str_df(
        nb_outcomes_df, "early_onset_sepsis", sim_years
    )[0]
    early_ns_pn = analysis_utility_functions.get_mean_and_quants_from_str_df(
        nb_outcomes_pn_df, "early_onset_sepsis", sim_years
    )[0]

    total_ens = [x + y for x, y in zip(early_ns, early_ns_pn)]

    late_ns_nb = analysis_utility_functions.get_mean_and_quants_from_str_df(
        nb_outcomes_df, "late_onset_sepsis", sim_years
    )[0]
    late_ns_pn = analysis_utility_functions.get_mean_and_quants_from_str_df(
        nb_outcomes_pn_df, "late_onset_sepsis", sim_years
    )[0]

    late_ns = [x + y for x, y in zip(late_ns_nb, late_ns_pn)]

    mild_en = analysis_utility_functions.get_mean_and_quants_from_str_df(
        nb_outcomes_df, "mild_enceph", sim_years
    )[0]
    mod_en = analysis_utility_functions.get_mean_and_quants_from_str_df(
        nb_outcomes_df, "moderate_enceph", sim_years
    )[0]
    sev_en = analysis_utility_functions.get_mean_and_quants_from_str_df(
        nb_outcomes_df, "severe_enceph", sim_years
    )[0]
    ...
    early_ptl_data = analysis_utility_functions.get_mean_and_quants_from_str_df(
        la_comps, "early_preterm_labour", sim_years
    )[0]
    late_ptl_data = analysis_utility_functions.get_mean_and_quants_from_str_df(
        la_comps, "late_preterm_labour", sim_years
    )[0]
    ...
    for cause, tr in zip(simplified_causes, trs):
        ...
        analysis_utility_functions.simple_line_chart_with_target(
            ...
            f"Neonatal Mortality Ratio per Year for {cause}",
            f"nmr_{cause}",
            graph_location,
        )
    ...
    dalys_stacked = extract_results(
        ...
        key="dalys_stacked",
        custom_generate_series=(
            lambda df: df.drop(columns="date").groupby(["year"]).sum().stack()
        ),
        do_scaling=True,
    )
    ...
    def extract_dalys_tlo_model(group):
        ...
        for year in sim_years:
            if year in dalys_stacked.index:
                stacked_dalys.append(
                    dalys_stacked.loc[year, f"{group} Disorders"].mean()
                )
                stacked_dalys_lq.append(
                    dalys_stacked.loc[year, f"{group} Disorders"].quantile(0.025)
                )
                stacked_dalys_uq.append(
                    dalys_stacked.loc[year, f"{group} Disorders"].quantile(0.925)
                )
    ...
    def get_daly_graphs(group, dalys, gbd_estimate):
        ...
        ax.fill_between(sim_years, dalys[1], dalys[2], color="b", alpha=0.1)

        ax.plot(daly_years, gbd_estimate[0], label="GBD DALY Est.", color="olivedrab")
        ax.fill_between(
            daly_years, gbd_estimate[1], gbd_estimate[2], color="g", alpha=0.1
        )
        plt.xlabel("Year")
