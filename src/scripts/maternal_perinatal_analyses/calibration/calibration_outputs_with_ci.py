...
def output_incidence_for_calibration(scenario_filename, pop_size, outputspath, sim_years):
    ...
    births_results_exc_2010 = extract_results(
        ...
        key="on_birth",
        custom_generate_series=(
            lambda df: df.loc[(df["mother"] >= 0)].assign(year=df["date"].dt.year).groupby(["year"])["year"].count()
        ),
    )

    lb_data = analysis_utility_functions.return_95_CI_across_runs(births_results_exc_2010, sim_years)
    analysis_utility_functions.simple_line_chart_with_ci(
        sim_years, lb_data, "Births (mean)", "Mean number of Live Births per Year", "live_births", graph_location
    )
    ...
    an_sb = extract_results(
        ...
        custom_generate_series=(lambda df: df.assign(year=df["date"].dt.year).groupby(["year"])["year"].count()),
    )
    ...
    analysis_utility_functions.simple_bar_chart(
        percent_live_birth[0],
        "Year",
        "% of Total Pregnancies",
        "Percentage of Total Pregnancies Ending in a Live Birth per Year",
        "preg_loss_proportion",
        alt_years,
        graph_location,
    )
    ...
    anc_coverage = extract_results(
        ...
        custom_generate_series=(lambda df: df.assign(year=df["date"].dt.year).groupby(["year"])["person_id"].count()),
        do_scaling=True,
    )
    ...
    def get_anc_df(visit_number):
        an = extract_results(
            ...
            key="anc_count_on_birth",
            custom_generate_series=(
                lambda df: df.loc[df["total_anc"] >= visit_number]
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        "Percentage of women who gave birth that attended one or more ANC contacts per year",
        graph_location,
        "anc_prop_anc1",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        100,
        "% of women who gave birth",
        "Percentage of women who gave birth that attended four or more ANC contacts per year",
        graph_location,
        "anc_prop_anc4",
    )
    ...
    anc1_for_bc = [
        [anc1_coverage[0][0], anc1_coverage[0][5]],
        [anc1_coverage[1][0], anc1_coverage[1][5]],
        [anc1_coverage[2][0], anc1_coverage[2][5]],
    ]
    anc4_for_bc = [
        [anc4_coverage[0][0], anc4_coverage[0][5]],
    ...
    for model_data, calib_data, title, save_name in zip(
        [anc1_for_bc, anc4_for_bc],
        [[94, 95], [45.5, 51]],
        [
    ...
    ax.bar(
        labels,
        new_anc_count_df.loc[4]
        + new_anc_count_df.loc[5]
        + new_anc_count_df.loc[6]
        + new_anc_count_df.loc[7]
        + new_anc_count_df.loc[8],
        width,
        label="4+",
    ...
    anc_ga = extract_results(
        ...
        key="anc_count_on_birth",
        custom_generate_series=(
            lambda df_: df_.assign(year=df_["date"].dt.year)
            .groupby(["year", "total_anc", "ga_anc_one"])["person_id"]
            .count()
        ),
        do_scaling=False,
    )
    ...
    a_5 = [
        anc_before_four_five_months[0],
        anc_before_four_five_months[1],
        anc_before_four_five_months[6],
        anc_before_four_five_months[8],
    ]
    a_6 = [
        anc_before_six_seven_months[0],
        anc_before_six_seven_months[1],
        anc_before_six_seven_months[6],
        anc_before_six_seven_months[8],
    ]
    a_8 = [
        anc_before_eight_plus_months[0],
        anc_before_eight_plus_months[1],
        anc_before_eight_plus_months[6],
        anc_before_eight_plus_months[8],
    ]

    labels = ["2010 (DHS)", "2010 (Model)", "2015 (DHS)", "2016 (Model)"]
    fig, ax = plt.subplots()
    ax.bar(labels, a_8, width, label=">8m", bottom=[x + y + z for x, y, z in zip(a_4, a_5, a_6)])
    ax.bar(labels, a_6, width, label="6-7m", bottom=[x + y for x, y in zip(a_4, a_5)])
    ax.bar(labels, a_5, width, label="4-5m", bottom=a_4)
    ...
    def get_delivery_rate(facility_type):
        ...
        if facility_type == "facility":
            deliver_setting_results = extract_results(
                ...
                custom_generate_series=(
                    lambda df: df.loc[df["facility_type"] != "home_birth"]
                    .assign(year=df["date"].dt.year)
                    .groupby(["year"])["mother"]
                    .count()
                ),
            ...
            deliver_setting_results = extract_results(
                ...
                key="delivery_setting_and_mode",
                custom_generate_series=(
                    lambda df: df.loc[df["facility_type"] == facility_type]
                    .assign(year=df["date"].dt.year)
                    .groupby(["year"])["mother"]
                    .count()
                ),
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        100,
        "% of total births",
        "Percentage of births occurring in a health facility per year ",
        graph_location,
    ...
    ax.bar(labels, hp_data, width, label="Hospital", bottom=[x + y for x, y in zip(hb_data, hc_data)])
    ax.bar(labels, hc_data, width, label="Health Centre", bottom=hb_data)
    ...
    fd_for_bc = [[fd_data[0][0], fd_data[0][5]], [fd_data[1][0], fd_data[1][5]], [fd_data[2][0], fd_data[2][5]]]
    ...
    target_npnc_dict = {"double": False, "first": {"year": 2015, "value": 60, "label": "DHS (2015)", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        100,
        "% of total births",
        "Percentage of total births after which the mother received any PNC per year",
        graph_location,
        "pnc_mat",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        100,
        "% of total births",
        "Percentage of total births after which the neonate received any PNC per year",
        graph_location,
        "pnc_neo",
    )
    ...
    m_pnc_for_bc = [
        [mat_birth_pnc_data[0][1], mat_birth_pnc_data[0][5]],
        [mat_birth_pnc_data[1][1], mat_birth_pnc_data[1][5]],
        [mat_birth_pnc_data[2][1], mat_birth_pnc_data[2][5]],
    ]

    cov_bar_chart(
        m_pnc_for_bc, [50, 48], "Percentage of Women Receiving Any Postnatal Care Following Birth", "mat_pnc_cov_bar"
    )
    ...
    plt.legend()
    plt.savefig(f"{graph_location}/pnc_neo_2015" f".png", bbox_inches="tight")
    plt.show()
    ...
    def get_early_late_pnc_split(module, target, file_name):
        p = extract_results(
            ...
            custom_generate_series=(
                ...
                .groupby(["year", "timing", "visit_number"])["person_id"]
                .count()
            ),
        )
        pnc = p.fillna(0)
    ...
    target_twin_dict = {"double": False, "first": {"year": 2011, "value": 3.9, "label": "Monden & Smits", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        5,
        "Rate per 100 births",
        "Twin Births per 100 Births per Year",
        graph_location,
        "twin_rate",
    )
    ...
    target_ect_dict = {"double": False, "first": {"year": 2015, "value": 10, "label": "Panelli et al.,", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        "Ectopic Pregnancies per 1000 Pregnancies per Year",
        graph_location,
        "ectopic_rate",
    )
    ...
    target_rate_rup = {"double": False, "first": {"year": 2015, "value": 92, "label": "Est.", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        "Proportion of Ectopic Pregnancies ending in rupture",
        graph_location,
        "prop_rupture",
    )
    ...
    target_sa_dict = {"double": False, "first": {"year": 2015, "value": 153, "label": "Polis et al.", "ci": 0}}

    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        alt_years,
        spotaneous_abortion_data,
        target_sa_dict,
        165,
        "Rate per 1000 completed pregnancies",
        "Spontaneous Abortions per 1000 Completed Pregnancies per Year",
        graph_location,
        "miscarriage_rate",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        170,
        "Rate per 1000 completed pregnancies",
        "Induced Abortions per 1000 Completed Pregnancies per Year",
        graph_location,
        "abortion_rate",
    )
    ...
    analysis_utility_functions.simple_bar_chart(
        proportion_of_complicated_sa_per_year[0],
        "Year",
        "% of Spontaneous Abortions",
        "Proportion of Spontaneous Abortions Leading to Complications",
        "miscarriage_prop_complicated",
        alt_years,
        graph_location,
    )
    ...
    analysis_utility_functions.simple_bar_chart(
        proportion_of_complicated_ia_per_year[0],
        "Year",
        "% of Induced Abortions",
        "Proportion of Induced Abortions Leading to Complications",
        "abortion_prop_complicated",
        alt_years,
        graph_location,
    )

    # --------------------------------------------------- Syphilis Rate... --------------------------------------------
    syphilis_data = return_rate(
        an_comps.loc[(slice(None), "syphilis"), slice(None)].droplevel(1),
        total_completed_pregnancies_df,
        1000,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        30,
        "Rate per 1000 completed pregnancies",
        "Syphilis Cases in Pregnancy per 1000 Completed Pregnancies",
        graph_location,
        "syphilis_rate",
    )

    # ------------------------------------------------ Gestational Diabetes ------------------------------------------
    gdm_data = return_rate(
        an_comps.loc[(slice(None), "gest_diab"), slice(None)].droplevel(1),
        total_completed_pregnancies_df,
        1000,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        20,
        "Rate per 1000 completed pregnancies",
        "Gestational Diabetes Cases per 1000 Completed Pregnancies",
        graph_location,
        "gest_diab_rate",
    )

    # --------------------------------- Premature Rupture of Membranes (PROM)-----------------------------------------
    prom_data = return_rate(
        an_comps.loc[(slice(None), "PROM"), slice(None)].droplevel(1), births_results_exc_2010, 1000, alt_years
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        "Premature Rupture of Membranes (PROM) per 1000 Births",
        graph_location,
        "prom_rate",
    )
    ...
    anaemia_results = extract_results(
        ...
        custom_generate_series=(
            lambda df: df.loc[df["anaemia_status"] != "none"]
            .assign(year=df["date"].dt.year)
            .groupby(["year"])["year"]
            .count()
        ),
    ...
    pnc_anaemia = extract_results(
        ...
        key="total_mat_pnc_visits",
        custom_generate_series=(
            lambda df: df.loc[df["anaemia"] != "none"].assign(year=df["date"].dt.year).groupby(["year"])["year"].count()
        ),
    )
    ...
    def get_anaemia_graphs(df, timing, severity_df):
        ...
        prevalence_of_mod_anaemia_per_year = return_rate(
            severity_df.loc[(slice(None), "moderate"), slice(None)].droplevel(1),
            births_results_exc_2010,
            100,
            alt_years,
        )
    ...
    gh_df = an_comps.loc[(slice(None), "mild_gest_htn"), slice(None)].droplevel(1) + pn_comps.loc[
        (slice(None), "mild_gest_htn"), slice(None)
    ].droplevel(1)
    gh_data = return_rate(gh_df, births_results_exc_2010, 1000, alt_years)

    sgh_df = (
        an_comps.loc[(slice(None), "severe_gest_htn"), slice(None)].droplevel(1)
    ...
    mpe_df = an_comps.loc[(slice(None), "mild_pre_eclamp"), slice(None)].droplevel(1) + pn_comps.loc[
        (slice(None), "mild_pre_eclamp"), slice(None)
    ].droplevel(1)
    mpe_data = return_rate(mpe_df, births_results_exc_2010, 1000, alt_years)

    spe_df = (
        an_comps.loc[(slice(None), "severe_pre_eclamp"), slice(None)].droplevel(1)
    ...
    ec_df = (
        an_comps.loc[(slice(None), "eclampsia"), slice(None)].droplevel(1)
    ...
    target_gh_dict = {"double": False, "first": {"year": 2019, "value": 43.8, "label": "Noubiap et al.", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        55,
        "Rate per 1000 births",
        "Mild Gestational Hypertension Cases per 1000 Births per Year",
        graph_location,
        "gest_htn_rate",
    )

    target_sgh_dict = {"double": False, "first": {"year": 2019, "value": 5.98, "label": "Noubiap et al.", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        8,
        "Rate per 1000 births",
        "Severe Gestational Hypertension Cases per 1000 Births per Year",
        graph_location,
        "severe_gest_htn_rate",
    )

    target_mpe_dict = {"double": False, "first": {"year": 2019, "value": 44, "label": "Noubiap et al", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        55,
        "Rate per 1000 births",
        "Mild Pre-eclampsia Cases per 1000 Births per Year",
        graph_location,
        "mild_pre_eclampsia_rate",
    )

    target_spe_dict = {"double": False, "first": {"year": 2019, "value": 22, "label": "Noubiap et al", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        35,
        "Rate per 1000 births",
        "Severe Pre-eclampsia Cases per 1000 Births per Year",
        graph_location,
        "severe_pre_eclampsia_rate",
    )

    target_ec_dict = {"double": False, "first": {"year": 2019, "value": 10, "label": "Vousden et al.", "ci": 0}}
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        alt_years,
        ec_data,
        target_ec_dict,
        15,
        "Rate per 1000 births",
        "Eclampsia Cases per 1000 Births per Year",
        graph_location,
        "eclampsia_rate",
    )
    ...
    #  ---------------------------------------------Placenta praevia... ----------------------------------------------
    pp_data = return_rate(
        an_comps.loc[(slice(None), "placenta_praevia"), slice(None)].droplevel(1),
        pregnancy_poll_results,
        1000,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        8,
        "Rate per 1000 pregnancies",
        "Cases of Placenta Praevia per 1000 Pregnancies per Year",
        graph_location,
        "praevia_rate",
    )
    ...
    pa_df = an_comps.loc[(slice(None), "placental_abruption"), slice(None)].droplevel(1) + la_comps.loc[
        (slice(None), "placental_abruption"), slice(None)
    ].droplevel(1)
    pa_data = return_rate(pa_df, births_results_exc_2010, 1000, alt_years)

    target_pa_dict = {"double": False, "first": {"year": 2015, "value": 3, "label": "Macheku et al.", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        5,
        "Rate per 1000 births",
        "Cases of Placental Abruption per 1000 Births Per Year",
        graph_location,
        "abruption_rate",
    )
    ...
    aph_df = (
        an_comps.loc[(slice(None), "mild_mod_antepartum_haemorrhage"), slice(None)].droplevel(1)
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        7,
        "Rate per 1000 births",
        "Cases of Antepartum & Intrapartum Haemorrhages per 1000 Births per Year",
        graph_location,
        "aph_rate",
    )
    ...
    ptl_df = la_comps.loc[(slice(None), "early_preterm_labour"), slice(None)].droplevel(1) + la_comps.loc[
        (slice(None), "late_preterm_labour"), slice(None)
    ].droplevel(1)
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        15,
        "Rate per 100 births",
        "Preterm Births per 100 Births per Year",
        graph_location,
        "ptb_rate",
    )
    ...
    prop_early = return_rate(
        la_comps.loc[(slice(None), "early_preterm_labour"), slice(None)].droplevel(1), ptl_df, 100, alt_years
    )
    prop_late = return_rate(
        la_comps.loc[(slice(None), "late_preterm_labour"), slice(None)].droplevel(1), ptl_df, 100, alt_years
    )
    labels = alt_years
    width = 0.35
    fig, ax = plt.subplots()
    ax.bar(labels, prop_early[0], width, label="Early Preterm", bottom=prop_late[0])
    ...
    # --------------------------------------------- Post term birth ... -----------------------------------------------
    potl_data = return_rate(
        la_comps.loc[(slice(None), "post_term_labour"), slice(None)].droplevel(1),
        births_results_exc_2010,
        100,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        5,
        "Rate per 100 Births",
        "Post term Births per 100 Births per Year",
        graph_location,
        "potl_rate",
    )
    ...
    # --------------------------------------------- Obstructed Labour... ----------------------------------------------
    ol_data = return_rate(
        la_comps.loc[(slice(None), "obstructed_labour"), slice(None)].droplevel(1),
        births_results_exc_2010,
        1000,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        45,
        "Rate per 1000 births",
        "Cases of Obstructed Labour per 1000 Births per Year",
        graph_location,
        "ol_rate",
    )
    ...
    # --------------------------------------------- Uterine rupture... -----------------------------------------------
    ur_data = return_rate(
        la_comps.loc[(slice(None), "uterine_rupture"), slice(None)].droplevel(1),
        births_results_exc_2010,
        1000,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        3,
        "Rate per 1000 births",
        "Cases of Uterine Rupture per 1000 Births per Year",
        graph_location,
        "ur_rate",
    )
    ...
    # ------------------------------------------ Maternal sepsis  --------------------------------------------------
    sepsis_df = (
        an_comps.loc[(slice(None), "clinical_chorioamnionitis"), slice(None)].droplevel(1)
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        3.5,
        "Rate per 1000 births",
        "Cases of Maternal Sepsis per 1000 Births per Year",
        graph_location,
        "sepsis_rate",
    )
    ...
    pph_data = pn_comps.loc[(slice(None), "primary_postpartum_haemorrhage"), slice(None)].droplevel(1) + pn_comps.loc[
        (slice(None), "secondary_postpartum_haemorrhage"), slice(None)
    ].droplevel(1)
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        15,
        "Rate per 1000 births",
        "Cases of Postpartum Haemorrhage per 1000 births Year",
        graph_location,
        "pph_rate",
    )
    ...
    of_data = pn_comps.loc[(slice(None), "vesicovaginal_fistula"), slice(None)].droplevel(1) + pn_comps.loc[
        (slice(None), "rectovaginal_fistula"), slice(None)
    ].droplevel(1)

    total_fistula_rates = return_rate(of_data, births_results_exc_2010, 1000, alt_years)

    target_fistula_dict = {"double": False, "first": {"year": 2015, "value": 6, "label": "DHS 2015", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        8,
        "Rate per 1000 births",
        "Cases of Obstetric Fistula per 1000 Births per Year",
        graph_location,
        "fistula_rate",
    )
    ...
    # calibration targets
    cs_data = return_rate(
        delivery_mode.loc[(slice(None), "caesarean_section"), slice(None)].droplevel(1),
        births_results_exc_2010,
        100,
        alt_years,
    )

    avd_data = return_rate(
        delivery_mode.loc[(slice(None), "instrumental"), slice(None)].droplevel(1),
        births_results_exc_2010,
        100,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        5,
        "% of total births",
        "Percentage of total births delivered via caesarean section",
        graph_location,
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        4,
        "% of total births",
        "Percentage of Total Births Delivered via Assisted Vaginal Delivery ",
        graph_location,
        "avd_rate",
    )

    # Then the CS rate for 2010 and 2015 is compared to the calibration data from the EmONC survey via a barchart
    mdata = [[cs_data[0][0], cs_data[0][5]], [cs_data[1][0], cs_data[1][5]], [cs_data[2][0], cs_data[2][5]]]
    cdata = [3.7, 4]
    ...
    un_igcme_data = [
        [20, 19, 19, 18, 18, 17, 17, 17, 17, 16, 16, 16],
    ...
    def get_stillbirth_graphs(rate_data, calib_data, group):
        ...
        ax.fill_between(sim_years, rate_data[1], rate_data[2], color="b", alpha=0.1)
        ax.plot(
            [2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021],
            calib_data[0],
            label="UN IGCME 2020 (Uncertainty interval)",
            color="green",
        )
        ax.fill_between(
            [2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021],
    ...
    # ------------------------------------------------- Birth weight... -----------------------------------------------
    lbw_data = return_rate(
        nb_outcomes_df.loc[(slice(None), "low_birth_weight"), slice(None)].droplevel(1),
        births_results_exc_2010,
        100,
        alt_years,
    )

    macro_data = return_rate(
        nb_outcomes_df.loc[(slice(None), "macrosomia"), slice(None)].droplevel(1),
        births_results_exc_2010,
        100,
        alt_years,
    )

    sga_data = return_rate(
        nb_outcomes_df.loc[(slice(None), "small_for_gestational_age"), slice(None)].droplevel(1),
        births_results_exc_2010,
        100,
        alt_years,
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        20,
        "Rate per 100 births",
        "Cases of Low Birth Weight per 100 Births per Year",
        graph_location,
        "neo_lbw_prev",
    )

    target_mac_dict = {"double": False, "first": {"year": 2019, "value": 5.13, "label": "Ngwira et al.", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        7,
        "Rate per 100 births",
        "Cases of Macrosomia per 100 Births per Year",
        graph_location,
        "neo_macrosomia_prev",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        30,
        "Rate per 100 births",
        "Cases of Small for Gestational Age per 100 Births Per Year",
        graph_location,
        "neo_sga_prev",
    )
    ...
    ns_df = (
        nb_outcomes_df.loc[(slice(None), "early_onset_sepsis"), slice(None)].droplevel(1)
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        85,
        "Rate per 1000 births",
        "Cases of Neonatal Sepsis per 1000 Births per Year",
        graph_location,
        "neo_sepsis_rate",
    )
    ...
    ne_df = (
        nb_outcomes_df.loc[(slice(None), "mild_enceph"), slice(None)].droplevel(1)
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        25,
        "Rate per 1000 births",
        "Cases of Neonatal Encephalopathy per 1000 Births per year",
        graph_location,
        "neo_enceph_rate",
    )
    ...
    dummy_dict = {"double": False, "first": {"year": 2011, "value": 0, "label": "UNK.", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        15,
        "Rate per 1000 births",
        "Cases of Neonatal Respiratory Depression per 1000 Births per Year",
        graph_location,
        "neo_resp_depression_rate",
    )
    ...
    target_rds_dict = {"double": False, "first": {"year": 2019, "value": 180, "label": "Estimated (see text)", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        "Rate of Preterm Respiratory Distress Syndrome per 1000 Preterm Births per Year",
        graph_location,
        "neo_rds_rate_ptb",
    )
    ...
    target_nb_dict = {"double": False, "first": {"year": 2014, "value": 5.7, "label": "Vossius et al.", "ci": 0}}
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        7,
        "Rate per 100 births",
        "Total Cases of All Respiratory Complications per 100 Births Per Year",
        graph_location,
    ...
    cba_df = (
        nb_outcomes_df.loc[(slice(None), "congenital_heart_anomaly"), slice(None)].droplevel(1)
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        30,
        "Rate per 1000 births",
        "Total Cases of Congenital Birth Anomalies per 1000 Births Per Year",
        graph_location,
        "cba_total_rate",
    )
    ...
    def get_maternal_death_dfs(scaled):
        ...
        direct_deaths = extract_results(
            ...
            key="death",
            custom_generate_series=(
                lambda df: df.loc[(df["label"] == "Maternal Disorders")]
                .assign(year=df["date"].dt.year)
                .groupby(["year"])["year"]
                .count()
            ),
                ...
                lambda df: df.loc[
                    ...
                    & (
                        df["cause_of_death"].str.contains(
                            "Malaria|Suicide|ever_stroke|diabetes|"
        ...
        return {
            "direct_deaths_final": direct_deaths_final,
    ...
    direct_mmr_by_year = return_rate(mat_d_unscaled["direct_deaths_final"], births_results_exc_2010, 100_000, sim_years)
    indirect_mmr_by_year = return_rate(
        mat_d_unscaled["indirect_deaths_final"], births_results_exc_2010, 100_000, sim_years
    )
    total_mmr_by_year = return_rate(mat_d_unscaled["total_deaths"], births_results_exc_2010, 100_000, sim_years)
    ...
    for data, title in zip(
        [direct_mmr_by_year, indirect_mmr_by_year, total_mmr_by_year], ["Direct", "Indirect", "Total"]
    ):
        ...
        ax.fill_between(sim_years, data[1], data[2], color="b", alpha=0.1)
        plt.errorbar(
            2010,
            dhs_10_adj[0],
            yerr=(dhs_10_adj[2] - dhs_10_adj[1]) / 2,
            label="DHS 2010 (95% CI)",
            fmt="o",
            color="green",
        ...
        plt.errorbar(
            2015,
            dhs_15_adj[0],
            yerr=(dhs_15_adj[2] - dhs_15_adj[1]) / 2,
            label="DHS 2015 (95% CI)",
            fmt="o",
            color="green",
            ecolor="mediumseagreen",
            elinewidth=3,
        ...
        ax.fill_between(unmmeig_yrs, unmmeig_l_adj, unmmeig_u_adj, color="cadetblue", alpha=0.1)
        ax.plot(gbd_years_mmr, gbd_adj, label="GBD 2019 (lower & upper bounds)", color="darkslateblue")
        ax.fill_between(gbd_years_mmr, gbd_l_adj, gbd_u_adj, color="slateblue", alpha=0.1)
    ...
    def extract_deaths_gbd_data(group):
        ...
        dalys_df = dalys_data.loc[
            (dalys_data["measure_name"] == "Deaths") & (dalys_data["cause_name"] == group) & (dalys_data["Year"] > 2010)
        ]
        gbd_deaths = list()
        gbd_deaths_lq = list()
        gbd_deaths_uq = list()
    ...
    plt.bar(
        ind + width,
        gbd_deaths_2010_2019_data[0],
        width,
        label="GBD (upper & lower bounds)",
        yerr=gbd_ci,
        color="olivedrab",
    )
    ...
    ec_tr = {
        "double": True,
        "first": {"year": 2011, "value": 1.4, "label": "UNK.", "ci": 0},
        "second": {"year": 2015, "value": 3, "label": "UNK.", "ci": 0},
    }
    ab_tr = {
        "double": True,
        "first": {"year": 2011, "value": 36.8, "label": "UNK.", "ci": 0},
        "second": {"year": 2015, "value": 20.9, "label": "UNK.", "ci": 0},
    }
    spe_ec_tr = {
        "double": True,
        "first": {"year": 2011, "value": 50.6, "label": "UNK.", "ci": 0},
        "second": {"year": 2015, "value": 55.3, "label": "UNK.", "ci": 0},
    }
    sep_tr = {
        "double": True,
        "first": {"year": 2011, "value": 91, "label": "UNK.", "ci": 0},
        "second": {"year": 2015, "value": 67.6, "label": "UNK.", "ci": 0},
    }
    ur_tr = {
        "double": True,
        "first": {"year": 2011, "value": 55, "label": "UNK.", "ci": 0},
        "second": {"year": 2015, "value": 43, "label": "UNK.", "ci": 0},
    }
    pph_tr = {
        "double": True,
        "first": {"year": 2011, "value": 174.8, "label": "UNK.", "ci": 0},
    ...
    for cause, tr in zip(simplified_causes, trs):
        ...
        analysis_utility_functions.line_graph_with_ci_and_target_rate(
            ...
            f"Maternal Mortality Ratio per Year for {cause}",
            graph_location,
            f"mmr_{cause}",
        )
        ...
        for cause in simp_causes:
            ...
            if cause == "abortion":
                new_df.loc[year, cause] = (
                    death_results.loc[year, "induced_abortion"] + death_results.loc[year, "spontaneous_abortion"]
                )
                new_df.loc[year, cause] = (
                    new_df.loc[year, cause] / mat_d_unscaled["direct_deaths_final"].loc[year]
                ) * 100
                t.append(new_df)
            elif cause == "severe_pe_ec":
                new_df.loc[year, cause] = (
                    death_results.loc[year, "severe_pre_eclampsia"] + death_results.loc[year, "eclampsia"]
                )
                new_df.loc[year, cause] = (
                    new_df.loc[year, cause] / mat_d_unscaled["direct_deaths_final"].loc[year]
                ) * 100
                ...
                new_df.loc[year, cause] = (
                    death_results.loc[year, "postpartum_haemorrhage"]
                ...
                new_df.loc[year, cause] = (
                    death_results.loc[year, "postpartum_sepsis"]
                ...
                new_df.loc[year, cause] = (
                    death_results.loc[year, "antepartum"] + death_results.loc[year, "secondary_postpartum_haemorrhage"]
                )
                new_df.loc[year, cause] = (
                    new_df.loc[year, cause] / mat_d_unscaled["direct_deaths_final"].loc[year]
                ) * 100
    ...
    ax.bar(
        ...
        bottom=[
            a + b + c
            for a, b, c in zip(
                indirect_deaths_means["AIDS_non_TB"], indirect_deaths_means["Malaria"], indirect_deaths_means["TB"]
            )
        ],
    ...
    ax.bar(
        ...
        width,
        label="TB",
        bottom=[a + b for a, b in zip(indirect_deaths_means["AIDS_non_TB"], indirect_deaths_means["Malaria"])],
        color="darkmagenta",
    )
    ...
    nd_nmr = return_rate(
        death_results_labels.loc[(slice(None), "Neonatal Disorders"), slice(None)].droplevel(1),
        births_results_exc_2010,
        1000,
        sim_years,
    )
    ...
    def get_nmr_graphs(data, colours, title, save_name):
        ...
        ax.fill_between(sim_years, data[1], data[2], color=colours[1], alpha=0.1)
        un_yrs = [2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021]

        data = {
            "dhs": {"mean": [31, 27], "lq": [26, 22], "uq": [34, 34]},
            "un": {
                "mean": [28, 27, 26, 25, 24, 23, 22, 22, 20, 20, 20, 19],
        ...
        plt.errorbar(
            ...
            data["dhs"]["mean"][0],
            label="DHS 2010 (95% CI)",
            yerr=(data["dhs"]["uq"][0] - data["dhs"]["lq"][0]) / 2,
            fmt="o",
            color="mediumseagreen",
            ecolor="green",
            elinewidth=3,
        ...
        plt.errorbar(
            ...
            data["dhs"]["mean"][1],
            label="DHS 2015 (95% CI)",
            yerr=(data["dhs"]["uq"][1] - data["dhs"]["lq"][1]) / 2,
            fmt="o",
            color="mediumseagreen",
            ecolor="green",
            elinewidth=3,
        ...
        ax.fill_between(un_yrs, data["un"]["lq"], data["un"]["uq"], color="grey", alpha=0.1)
        ax.set(ylim=(0, 35))
        plt.xlabel("Year")
    ...
    ax.fill_between(sim_years, tnmr[1], tnmr[2], color="b", alpha=0.1)
    ...
    def gbd_bar_chart(data, gbd_data, title, save_name):
        ...
        plt.bar(
            ind + width, gbd_data[0], width, label="GBD (Lower & Upper bounds)", yerr=gbd_ci_neo, color="forestgreen"
        )
    ...
    gbd_bar_chart(
        neo_deaths, gbd_neo_deaths, 'Total Deaths Attributable to "Neonatal Disorders" per Year', "crude_deaths_nd"
    )
    gbd_bar_chart(
        ca_deaths,
        gbd_cba_deaths,
        'Total Deaths Attributable to "Congenital Birth Defects" per Year',
        "crude_deaths_cba",
    )
    ...
    for year in sim_years:
        for cause in simp_causes_neo:
            ...
            if cause == "Prematurity":
                new_df.loc[year, cause] = (
                    death_results.loc[year, "respiratory_distress_syndrome"] + death_results.loc[year, "preterm_other"]
                )
                new_df.loc[year, cause] = (new_df.loc[year, cause] / total_neonatal_deaths.loc[year]) * 100
                t.append(new_df)
            elif cause == "Birth Asphyxia":
                new_df.loc[year, cause] = (
                    death_results.loc[year, "encephalopathy"]
                ...
                new_df.loc[year, cause] = (
                    death_results.loc[year, "early_onset_sepsis"] + death_results.loc[year, "late_onset_sepsis"]
                )
                new_df.loc[year, cause] = (new_df.loc[year, cause] / total_neonatal_deaths.loc[year]) * 100
                t.append(new_df)
    ...
    ptb_tr = {
        "double": True,
        "first": {"year": 2011, "value": 6.75, "label": "UNK.", "ci": 0},
        "second": {"year": 2015, "value": 5.94, "label": "UNK.", "ci": 0},
    }
    enceph_tr = {
        "double": True,
        "first": {"year": 2011, "value": 6.25, "label": "UNK.", "ci": 0},
        "second": {"year": 2015, "value": 5.5, "label": "UNK.", "ci": 0},
    }
    sep = {
        "double": True,
        "first": {"year": 2011, "value": 2, "label": "UNK.", "ci": 0},
    ...
    for cause, tr in zip(simplified_causes, trs):
        ...
        analysis_utility_functions.line_graph_with_ci_and_target_rate(
            ...
            f"Neonatal Mortality Rate per Year for {cause}",
            graph_location,
            f"nmr_{cause}",
        )
    ...
    def extract_dalys_gbd_data(group):
        ...
        dalys_df = dalys_data.loc[
            (dalys_data["measure_name"] == "DALYs (Disability-Adjusted Life Years)")
            & (dalys_data["cause_name"] == f"{group} disorders")
            & (dalys_data["Year"] > 2010)
        ]
        ...
        if group == "Maternal":
            ...
            rate = [
                [7754, 7388, 7039, 6667, 6323, 6051, 5637, 5402, 5199],
            ...
            yll = [
                ...
                [926580.96, 898355.11, 868688.00, 838840.05, 806310.38, 806310.38, 730865.10, 707276.07, 693996.45],
                [
                    1325484.32,
                    1303762.52,
                    1278501.75,
            ...
            yld = [
                [22980.05, 27396.62, 32943.15, 38722.71, 43760.64, 48920.63, 52681.07, 52985.03, 50919.88],
                [17003.09, 20257.22, 24621.60, 28670.33, 32352.21, 36966.02, 39836.19, 39901.68, 37275.59],
                [31058.88, 36616.19, 43246.68, 51007.54, 57403.37, 61380.25, 66633.24, 66586.96, 67266.20],
            ]

            return {"total": [gbd_dalys, gbd_dalys_lq, gbd_dalys_uq], "rate": rate, "yll": yll, "yld": yld}
    ...
    dalys_stacked = extract_results(
        ...
        key="dalys_stacked",
        custom_generate_series=(
            lambda df: df.drop(columns=["date", "sex", "age_range"]).groupby(["year"]).sum().stack()
        ),
        do_scaling=True,
    )
    ...
    person_years_total = extract_results(
        ...
        custom_generate_series=(
            lambda df: df.assign(
                total=(df["M"].apply(lambda x: sum(x.values()))) + df["F"].apply(lambda x: sum(x.values()))
            )
            .assign(year=df["date"].dt.year)
            .groupby(["year"])["total"]
            .sum()
    ...
    mat_model_dalys_data.update(
        {
            "rate": return_rate(
                dalys_stacked.loc[(slice(None), "Maternal Disorders"), slice(None)].droplevel(1),
                person_years_total,
                100_000,
                alt_years,
            )
    ...
    neo_model_dalys_data.update(
        {
            "rate": return_rate(
                dalys_stacked.loc[(slice(None), "Neonatal Disorders"), slice(None)].droplevel(1),
                person_years_total,
                100_000,
                alt_years,
            )
    ...
    mat_model_dalys_data.update(
        {
            "total": analysis_utility_functions.return_95_CI_across_runs(
    ...
    neo_causes_death = [
        ...
        "neonatal_respiratory_depression",
    ]
    ...
    neo_model_dalys_data.update({"yll": analysis_utility_functions.return_95_CI_across_runs(neo_yll_s_df, alt_years)})

    neo_model_dalys_data.update({"yld": analysis_utility_functions.return_95_CI_across_runs(neo_yld_df, alt_years)})
    ...
    def get_daly_graphs(group, model_data, gbd_data):
        ...
        ax.fill_between(alt_years, model_data["total"][1], model_data["total"][2], color="b", alpha=0.1)
        ax.plot(gbd_years, gbd_data["total"][0], label="GBD (Lower & Upper bounds)", color="olivedrab")
        ax.fill_between(gbd_years, gbd_data["total"][1], gbd_data["total"][2], color="g", alpha=0.1)
        if group == "Maternal":
        ...
        ax.fill_between(alt_years, model_data["rate"][1], model_data["rate"][2], color="b", alpha=0.1)
        ax.plot(gbd_years, gbd_data["rate"][0], label="GBD (Lower & Upper bounds)", color="olivedrab")
        ax.fill_between(gbd_years, gbd_data["rate"][1], gbd_data["rate"][2], color="g", alpha=0.1)
        if group == "Maternal":
            ax.set(ylim=(0, 950))
        else:
            ax.set(ylim=0)
        ...
        if group == "Neonatal":
            ...
            ax.fill_between(alt_years, model_data["yll"][1], model_data["yll"][2], color="b", alpha=0.1)
            ax.plot(gbd_years, gbd_data["yll"][0], label="GBD (Lower & Upper bounds)", color="olivedrab")
            ax.fill_between(gbd_years, gbd_data["yll"][1], gbd_data["yll"][2], color="g", alpha=0.1)
            plt.xlabel("Year")
            ...
            ax.fill_between(alt_years, model_data["yld"][1], model_data["yld"][2], color="b", alpha=0.1)
            ax.plot(gbd_years, gbd_data["yld"][0], label="GBD (Lower & Upper bounds)", color="olivedrab")
            ax.fill_between(gbd_years, gbd_data["yld"][1], gbd_data["yld"][2], color="g", alpha=0.1)
            plt.xlabel("Year")
