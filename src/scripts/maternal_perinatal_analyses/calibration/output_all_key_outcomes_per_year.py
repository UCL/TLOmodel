...
def output_incidence_for_calibration(
    scenario_filename, pop_size, outputspath, sim_years
):
    """
    ...
    if not os.path.isdir(path):
        os.makedirs(
            f"{outputspath}/calibration_output_graphs_{pop_size}_{results_folder.name}"
        )
    ...
    pregnancy_poll_results = extract_results(
        ...
        key="pregnancy",
        custom_generate_series=(
            lambda df: df.assign(year=pd.to_datetime(df["date"]).dt.year)
            .groupby(["year"])["year"]
            .count()
        ),
    )

    preg_data = analysis_utility_functions.get_mean_and_quants(
        pregnancy_poll_results, sim_years
    )
    ...
    birth_data = analysis_utility_functions.get_mean_and_quants(
        births_results, sim_years
    )
    ...
    ectopic_mean_numbers_per_year = (
        analysis_utility_functions.get_mean_and_quants_from_str_df(
    ...
    ia_mean_numbers_per_year = (
        analysis_utility_functions.get_mean_and_quants_from_str_df(
            an_comps, "induced_abortion", sim_years
        )[0]
    )

    sa_mean_numbers_per_year = (
        analysis_utility_functions.get_mean_and_quants_from_str_df(
            an_comps, "spontaneous_abortion", sim_years
        )[0]
    )
    ...
    an_still_birth_data = analysis_utility_functions.get_mean_and_quants(
        an_stillbirth_results, sim_years
    )
    ip_still_birth_data = analysis_utility_functions.get_mean_and_quants(
        ip_stillbirth_results, sim_years
    )

    total_completed_pregnancies_per_year = [
        a + b + c + d + e
    ...
    prop_lost_pregnancies = [
        (x / y) * 100 for x, y in zip(total_pregnancy_losses, preg_data[0])
    ]
    ...
    analysis_utility_functions.simple_bar_chart(
        ...
        "Proportion of total pregnancies ending in pregnancy loss",
    ...
    results = extract_results(
        ...
        custom_generate_series=(
            lambda df_: df_.assign(year=df_["date"].dt.year)
            .groupby(["year", "total_anc"])["person_id"]
            .count()
        ),
        do_scaling=False,
    )
    ...
    for year in sim_years:
        ...
        yearly_anc1_rates.append(
            100 - ((anc_count_df.at[0, year][0] / anc_total) * 100)
        )
    ...
    for year in sim_years:
        ...
        if total_per_year != 0:
            for index in anc_count_df.index:
                anc_count_df.at[index, year] = (
                    anc_count_df.at[index, year][0] / total_per_year
                ) * 100
    ...
    ax.bar(
        ...
        width,
        label=4,
        bottom=anc_count_df.loc[1] + anc_count_df.loc[2] + anc_count_df.loc[3],
    )
    ax.bar(
        labels,
        anc_count_df.loc[3],
        width,
        label=3,
        bottom=anc_count_df.loc[1] + anc_count_df.loc[2],
    )
    ...
    for year in sim_years:
        ...
        four_to_five = year_df.loc[
            (slice(1, 8), slice(14, 22)), 0 : len(year_df.columns)
        ]
        six_to_seven = year_df.loc[
            (slice(1, 8), slice(23, 31)), 0 : len(year_df.columns)
        ]
    ...
    ax.bar(
        labels,
        anc_before_eight_plus_months,
        width,
        label=">8m",
        bottom=[
            x + y + z
            for x, y, z in zip(
                anc_before_four_months,
                anc_before_four_five_months,
    ...
    ax.bar(
        ...
        label="6-7m",
        bottom=[
            x + y for x, y in zip(anc_before_four_months, anc_before_four_five_months)
        ],
    )
    ax.bar(
        labels,
    ...
    analysis_utility_functions.simple_line_chart_with_target(
        ...
        target_rate_eanc4,
        "% total deliveries",
        "Proportion of women attending attending ANC4+ with first visit early",
        "anc_prop_early_anc4",
        graph_location,
    )
    ...
    birth_data_ex2010 = analysis_utility_functions.get_mean_and_quants(
    ...
    deliver_setting_results = extract_results(
        ...
        custom_generate_series=(
            lambda df_: df_.assign(year=df_["date"].dt.year)
            .groupby(["year", "facility_type"])["mother"]
            .count()
        ),
        do_scaling=False,
    )
    ...
    pn_mat_data = analysis_utility_functions.get_mean_and_quants(
        pnc_results_maternal, sim_years
    )
    pn_neo_data = analysis_utility_functions.get_mean_and_quants(
        pnc_results_newborn, sim_years
    )

    pnc_1_plus_rate_mat = [
        (x / y) * 100 for x, y in zip(pn_mat_data[0], birth_data_ex2010[0])
    ]
    ...
    pnc1_plus_rate_neo = [
        (x / y) * 100 for x, y in zip(pn_neo_data[0], birth_data_ex2010[0])
    ]
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_npnc_dict,
        "% of total births",
        "Proportion of Neonates per year attending PNC",
        graph_location,
        "pnc_neo",
    )
    ...
    def get_early_late_pnc_split(module, target, file_name):
        ...
        for year in sim_years:
            total = (
                pnc.loc[year, "early", 0].mean()
                + pnc.loc[year, "late", 0].mean()
                + pnc.loc[year, "none", 0].mean()
            )
            early = (pnc.loc[year, "early", 0].mean() / total) * 100
            late = (
                (pnc.loc[year, "late", 0].mean() + pnc.loc[year, "none", 0].mean())
                / total
            ) * 100
        ...
        ax.set_title(
            f"Proportion of {target} PNC1 Visits occurring pre/post 48hrs Postnatal"
        )
        ax.legend()
    ...
    proportion_of_ectopics_that_rupture_per_year = (
        analysis_utility_functions.get_comp_mean_and_rate(
            "ectopic_ruptured", mean_unrup_ectopics, an_comps, 100, sim_years
        )[0]
    )

    proportion_of_ectopics_that_rupture_per_year = [
        100 if i > 100 else i for i in proportion_of_ectopics_that_rupture_per_year
    ]
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        sim_years,
        spotaneous_abortion_data[0],
        spotaneous_abortion_data[1],
        spotaneous_abortion_data[2],
        target_sa_dict,
    ...
    proportion_of_complicated_sa_per_year = (
        analysis_utility_functions.get_comp_mean_and_rate(
            "complicated_spontaneous_abortion",
            mean_complicated_sa,
            an_comps,
            100,
            sim_years,
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_ia_dict,
        "Rate per 1000 completed pregnancies",
        "Yearly rate of Induced Abortion",
        graph_location,
        "abortion_rate",
    )
    ...
    proportion_of_complicated_ia_per_year = (
        analysis_utility_functions.get_comp_mean_and_rate(
    ...
    # ------------------------------------------------ PROM... --------------------------------------------------------
    prom_data = analysis_utility_functions.get_comp_mean_and_rate(
        "PROM", birth_data_ex2010[0], an_comps, 1000, sim_years
    )
    ...
    pnc_anaemia = extract_results(
        ...
        key="total_mat_pnc_visits",
        custom_generate_series=(
            lambda df: df.loc[df["anaemia"] != "none"]
            .assign(year=df["date"].dt.year)
            .groupby(["year"])["year"]
            .count()
        ),
    ...
    preg_an_severity = extract_results(
        ...
        custom_generate_series=(
            lambda df: df.assign(year=df["date"].dt.year)
            .groupby(["year", "anaemia_status"])["year"]
            .count()
        ),
    )
    ...
    def get_anaemia_graphs(data, timing, severity_df):
        ...
        mild_anaemia_at_birth = (
            analysis_utility_functions.get_mean_and_quants_from_str_df(
                severity_df, "mild", sim_years
            )[0]
        )
        moderate_anaemia_at_birth = (
            analysis_utility_functions.get_mean_and_quants_from_str_df(
                severity_df, "moderate", sim_years
            )[0]
        )
        severe_anaemia_at_birth = (
            analysis_utility_functions.get_mean_and_quants_from_str_df(
                severity_df, "severe", sim_years
            )[0]
        )

        prevalence_of_mild_anaemia_per_year = [
            (x / y) * 100 for x, y in zip(mild_anaemia_at_birth, birth_data_ex2010[0])
        ]
        prevalence_of_mod_anaemia_per_year = [
            (x / y) * 100
            for x, y in zip(moderate_anaemia_at_birth, birth_data_ex2010[0])
        ]
        prevalence_of_sev_anaemia_per_year = [
            (x / y) * 100 for x, y in zip(severe_anaemia_at_birth, birth_data_ex2010[0])
        ]
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_gh_dict,
        "Rate per 1000 births",
        "Rate of Gestational Hypertension per Year",
        graph_location,
        "gest_htn_rate",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_sgh_dict,
        "Rate per 1000 births",
        "Rate of Severe Gestational Hypertension per Year",
        graph_location,
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_mpe_dict,
        "Rate per 1000 births",
        "Rate of Mild pre-eclampsia per Year",
        graph_location,
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_spe_dict,
        "Rate per 1000 births",
        "Rate of Severe pre-eclampsia per Year",
        graph_location,
    ...
    ax.bar(labels, prop_early, width, label="Early Preterm", bottom=prop_late)
    ...
    an_sbr_per_year = [
        (x / y) * 1000 for x, y in zip(an_still_birth_data[0], birth_data_ex2010[0])
    ]
    an_sbr_lqs = [
        (x / y) * 1000 for x, y in zip(an_still_birth_data[1], birth_data_ex2010[0])
    ]
    an_sbr_uqs = [
        (x / y) * 1000 for x, y in zip(an_still_birth_data[2], birth_data_ex2010[0])
    ]
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_lbw_dict,
        "Proportion of total births",
        "Yearly Prevalence of Low Birth Weight",
        graph_location,
        "neo_lbw_prev",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_mac_dict,
        "Proportion of total births",
        "Yearly Prevalence of Macrosomia",
        graph_location,
        "neo_macrosomia_prev",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_sga_dict,
        "Proportion of total births",
        "Yearly Prevalence of Small for Gestational Age",
        graph_location,
        "neo_sga_prev",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_ol_dict,
        "Rate per 1000 births",
        "Obstructed Labour Rate per Year",
        graph_location,
        "ol_rate",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_ur_dict,
        "Rate per 1000 births",
        "Rate of Uterine Rupture per Year",
        graph_location,
        "ur_rate",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_cs_dict,
        "Proportion of total births",
        "Caesarean Section Rate per Year",
        graph_location,
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_avd_dict,
        "Proportion of total births",
        "Assisted Vaginal Delivery Rate per Year",
        graph_location,
        "avd_rate",
    )
    ...
    total_sep_rates = [
        x + y + z
        for x, y, z in zip(an_sep_data[0], la_sep_data[0], complete_pn_sep_data)
    ]
    sep_lq = [
        x + y + z for x, y, z in zip(an_sep_data[1], la_sep_data[1], complete_pn_sep_lq)
    ]
    sep_uq = [
        x + y + z for x, y, z in zip(an_sep_data[2], la_sep_data[2], complete_pn_sep_up)
    ]
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_sep_dict,
        "Rate per 1000 births",
        "Rate of Maternal Sepsis per Year",
        graph_location,
        "sepsis_rate",
    )
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_pph_dict,
        "Rate per 1000 births",
        "Rate of Postpartum Haemorrhage per Year",
        graph_location,
        "pph_rate",
    )
    ...
    ip_sbr_per_year = [
        (x / y) * 1000 for x, y in zip(ip_still_birth_data[0], birth_data_ex2010[0])
    ]
    ip_sbr_lqs = [
        (x / y) * 1000 for x, y in zip(ip_still_birth_data[1], birth_data_ex2010[0])
    ]
    ip_sbr_uqs = [
        (x / y) * 1000 for x, y in zip(ip_still_birth_data[2], birth_data_ex2010[0])
    ]
    ...
    ax.fill_between(sim_years, total_lqs, total_uqs, color="b", alpha=0.1)
    plt.errorbar(
        2010,
        20,
        yerr=(23 - 17) / 2,
    ...
    analysis_utility_functions.line_graph_with_ci_and_target_rate(
        ...
        target_fistula_dict,
        "Rate per 1000 births",
        "Rate of Obstetric Fistula per Year",
        graph_location,
        "fistula_rate",
    )
    ...
    total_ns_rates = [
        x + y + z for x, y, z in zip(early_ns_data[0], early_ns_pn[0], late_ns_data[0])
    ]
    ns_lqs = [
        x + y + z for x, y, z in zip(early_ns_data[1], early_ns_pn[1], late_ns_data[1])
    ]
    ns_uqs = [
        x + y + z for x, y, z in zip(early_ns_data[2], early_ns_pn[2], late_ns_data[2])
    ]
    ...
    total_enceph_rates = [
        x + y + z for x, y, z in zip(mild_data[0], mod_data[0], sev_data[0])
    ]
    ...
    rate = [
        x + y + z
        for x, y, z in zip(rds_data_over_births[0], total_enceph_rates, rd_data[0])
    ]
    ...
    analysis_utility_functions.simple_bar_chart(
        ...
        "Year",
        "Rate per 1000 births",
        "Rate of Not Breathing Newborns per year",
    ...
    rate_of_ua = analysis_utility_functions.get_comp_mean_and_rate(
        "urogenital_anomaly", birth_data_ex2010[0], nb_outcomes_df, 1000, sim_years
    )[0]

    rate_of_da = analysis_utility_functions.get_comp_mean_and_rate(
        "digestive_anomaly", birth_data_ex2010[0], nb_outcomes_df, 1000, sim_years
    )[0]

    rate_of_oa = analysis_utility_functions.get_comp_mean_and_rate(
        "other_anomaly", birth_data_ex2010[0], nb_outcomes_df, 1000, sim_years
    )[0]
