...
for _row, period in enumerate(("2010-2014", "2015-2019")):
    ax = axs[_row]
    comparison.loc[(period, slice(None), slice(None), CAUSE_NAME)].groupby(axis=0, level=1).sum().plot(
...
plt.bar(
    [1, 2],
    [mean_inc_death_gbd / mean_inc_gbd, np.mean(epi_death_rate) / np.mean(incidence_epilepsy)],
