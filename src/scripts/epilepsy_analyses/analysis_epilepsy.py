...
mean_proportion_in_sim = [
    np.mean(prop_seiz_stat_1),
    np.mean(prop_seiz_stat_2),
    np.mean(prop_seiz_stat_3),
]
plt.bar(
    np.arange(len(mean_proportion_in_sim)),
    mean_proportion_in_sim,
...
prop_antiepilep_seiz_stat_1 = pd.Series(
    output["tlo.methods.epilepsy"]["epilepsy_logging"][
...
for _row, period in enumerate(("2010-2014", "2015-2019")):
    ax = axs[_row]
    comparison.loc[(period, slice(None), slice(None), CAUSE_NAME)].droplevel(
        [0, 1, 3]
    ).groupby(axis=0, level=0).sum().plot(use_index=True, ax=ax)
...
gbd_data = pd.read_csv(
    resourcefilepath / "epilepsy" / "IHME-GBD_2019_DATA-4a82f00e-1.csv"
)
...
plt.bar(
    np.arange(2),
    [mean_inc_gbd, mean_inc_death_gbd],
    width=0.4,
...
plt.bar(
    np.arange(2) + 0.4,
    [np.mean(incidence_epilepsy), np.mean(epi_death_rate)],
    width=0.4,
    color="lightsalmon",
    label="Model",
)
...
plt.title(
    "The epilepsy model's estimated incidence of epilepsy\nand epilepsy death compared to the GBD study"
)
plt.show()
plt.clf()
...
plt.bar(
    [1, 2],
    [
        mean_inc_death_gbd / mean_inc_gbd,
        np.mean(epi_death_rate) / np.mean(incidence_epilepsy),
    ],
...
gbd_est_inc = gbd_inc_data.loc[
    gbd_inc_data["year"] <= incidence_epilepsy_df.index.max(), "val"
]
plt.plot(
    np.arange(len(incidence_epilepsy_df.index)),
    incidence_epilepsy_df["inc"],
    color="lightsalmon",
    label="model",
)
plt.plot(
...
incidence_epilepsy_death_df = incidence_epilepsy_death_df.rename(
    columns={0: "inc_death"}
)
...
gbd_est_inc_death = gbd_inc_death_data.loc[
    gbd_inc_death_data["year"] <= incidence_epilepsy_death_df.index.max(), "val"
]
plt.plot(
    np.arange(len(incidence_epilepsy_death_df.index)),
    incidence_epilepsy_death_df["inc_death"],
    color="lightsalmon",
    label="model",
)
plt.plot(
...
plt.legend()
plt.xticks(
    np.arange(len(incidence_epilepsy_death_df.index)), incidence_epilepsy_df.index
)
