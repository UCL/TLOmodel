...
def run_simulation(popsize=popsize, mda_execute=True):
    ...
    sim = Simulation(
        ...
        seed=0,
        log_config={"filename": __file__[-19:-3], "custom_levels": custom_levels},
    )
    sim.register(
...
def construct_dfs(schisto_log) -> dict:
    ...
    return {
        ...
        for k, v in schisto_log.items()
        if k in [f"infection_status_{s}" for s in species]
    }
...
def get_model_prevalence_by_district(spec: str):
    ...
    return (
        (counts["High-infection"] + counts["Low-infection"]) / counts.sum(axis=1)
    ).to_dict()
...
def get_expected_prevalence_by_district(species: str):
    ...
    expected_district_prevalence = pd.read_excel(
        resourcefilepath / "ResourceFile_Schisto.xlsx",
...
def get_model_dalys_schisto_2010():
    ...
    scaling_factor = (
        14.5e6 / popsize
    )  # todo - this properly once dealt with https://github.com/UCL/TLOmodel/issues/536
    dalys_schisto = (
        dalys.set_index("year")
        .loc[2010]
        .groupby(by="age_range")["Schistosomiasis"]
        .sum()
    )
...
def get_gbd_dalys_schisto_2010():
    ...
    return (
        gbd_all.loc[(gbd_all.cause_name == "Schistosomiasis") & (gbd_all.Year == 2010)]
        .groupby(by="Age_Grp")[["GBD_Est", "GBD_Lower", "GBD_Upper"]]
        .sum()
    )
...
ax.fill_between(
    dat.index, dat.GBD_Lower.values, dat.GBD_Upper.values, alpha=0.5, color="b"
)
