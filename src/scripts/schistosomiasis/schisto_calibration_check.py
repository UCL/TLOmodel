...
def run_simulation(popsize=popsize, mda_execute=True):
    ...
    sim = Simulation(
        start_date=start_date, seed=0, log_config={"filename": __file__[-19:-3], "custom_levels": custom_levels}
    )
    sim.register(
...
def get_expected_prevalence_by_district(species: str):
    ...
    expected_district_prevalence = read_csv_files(
        resourcefilepath / "ResourceFile_Schisto", files="District_Params_" + species.lower()
    )
...
for i, _spec in enumerate(species):
    ax = axes[i]
    pd.DataFrame(
        data={"Data": get_expected_prevalence_by_district(_spec), "Model": get_model_prevalence_by_district(_spec)}
