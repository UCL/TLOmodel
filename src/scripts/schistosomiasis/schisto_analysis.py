...
def save_inf_outputs(infection, save_the_districts=False):
    ...
    for age_group in ["PSAC", "SAC", "Adults", "All"]:
        ...
        output_states = output_states.append(
            output["tlo.methods.schisto"][age_group + "_" + infection], ignore_index=True
        )
    ...
    if save_the_districts:
        ...
        for distr in districts:
            ...
            new_row = {"District": distr, "Prevalence": prev, "MWB": mwb, "High-infection-prevalence": high_inf_p}
...
def save_general_outputs_and_params():
    ...
    for age_group in ["PSAC", "SAC", "Adults", "All"]:
        ...
        output_states = output_states.append(
            output["tlo.methods.schisto"][age_group + "_PrevalentYears"], ignore_index=True
        )
...
def get_logger(infection, cut_off_left=False):
    ...
    log_dict = {"loger_PSAC": loger_PSAC, "loger_SAC": loger_SAC, "loger_Adults": loger_Adults, "loger_All": loger_All}
    return log_dict
...
def get_expected_prevalence(infection):
    expected_district_prevalence = read_csv_files(
        Path("./resources") / "ResourceFile_Schisto", files="District_Params_" + infection.lower()
    )
