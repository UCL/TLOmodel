...
def run_simulation(popsize=10000, haem=True, mansoni=True, mda_execute=True):
    ...
    if haem:
        sim.register(
            schisto.Schisto_Haematobium(
                resourcefilepath=resourcefilepath, symptoms_and_HSI=False
            )
        )
    if mansoni:
        sim.register(
            schisto.Schisto_Mansoni(
                resourcefilepath=resourcefilepath, symptoms_and_HSI=False
            )
        )
...
def save_inf_outputs(infection, save_the_districts=False):
    ...
    savepath_districts_prev = (
        output_path + "output_districts_prev_" + infection + "_" + timestamp + ".csv"
    )

    output_states = pd.DataFrame([])
    for age_group in ["PSAC", "SAC", "Adults", "All"]:
        output["tlo.methods.schisto"][age_group + "_" + infection][
            "Age_group"
        ] = age_group
        output_states = output_states.append(
            output["tlo.methods.schisto"][age_group + "_" + infection],
            ignore_index=True,
        )
    ...
    if save_the_districts:
        ...
        distr_end_data = pd.DataFrame(
            columns=["District", "Prevalence", "MWB", "High-infection-prevalence"]
        )
        for distr in districts:
            prev = output["tlo.methods.schisto"][distr + "_" + infection][
                "Prevalence"
            ].values[-1]
            mwb = output["tlo.methods.schisto"][distr + "_" + infection][
                "MeanWormBurden"
            ].values[-1]
            high_inf = output["tlo.methods.schisto"][distr + "_" + infection][
                "High_infections"
            ].values[-1]
            low_inf = output["tlo.methods.schisto"][distr + "_" + infection][
                "Low_infections"
            ].values[-1]
            non_inf = output["tlo.methods.schisto"][distr + "_" + infection][
                "Non_infected"
            ].values[-1]
...
def save_general_outputs_and_params():
    ...
    savepath_prevalent_years = (
        output_path + "output_prevalent_years_" + timestamp + ".csv"
    )
    ...
    # Prevalent years
    for age_group in ["PSAC", "SAC", "Adults", "All"]:
        output["tlo.methods.schisto"][age_group + "_PrevalentYears"][
            "Age_group"
        ] = age_group
        output_states = output_states.append(
            output["tlo.methods.schisto"][age_group + "_PrevalentYears"],
    ...
    parameters_used = pd.read_excel(
        Path("./resources/ResourceFile_Schisto.xlsx"), sheet_name=None
    )
...
df = df[df["is_alive"]]
df = df[
    df["district_of_residence"].isin(
        ["Blantyre", "Chiradzulu", "Mulanje", "Nsanje", "Nkhotakota", "Phalombe"]
    )
]
...
def get_logger(infection, cut_off_left=False):
    ...
    if cut_off_left:
        ...
        # remove the period of "equilibrating"
        loger_PSAC = loger_PSAC[
            loger_PSAC.date >= pd.Timestamp(datetime.date(2014, 1, 1))
        ]
        loger_SAC = loger_SAC[loger_SAC.date >= pd.Timestamp(datetime.date(2014, 1, 1))]
        loger_Adults = loger_Adults[
            loger_Adults.date >= pd.Timestamp(datetime.date(2014, 1, 1))
        ]
...
def plot_high_inf_prevalence(loger_inf, infection):
    ...
    for log in loger_inf.keys():
        ax.plot(
            loger_inf[log].date, loger_inf[log]["High-inf_Prevalence"], label=log[6:]
        )
        ax.xaxis_date()
    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=12))
    ax.xaxis.set_major_formatter(DateFormatter("%Y"))
...
def get_values_per_district(infection):
    ...
    for distr in districts:
        prev = output["tlo.methods.schisto"][distr + "_" + infection].Prevalence.values[
            -1
        ]
        mwb = output["tlo.methods.schisto"][
            distr + "_" + infection
        ].MeanWormBurden.values[-1]
        high_inf = output["tlo.methods.schisto"][distr + "_" + infection][
            "High_infections"
        ].values[-1]
        low_inf = output["tlo.methods.schisto"][distr + "_" + infection][
...
def get_expected_prevalence(infection):
    expected_district_prevalence = pd.read_excel(
        Path("./resources") / "ResourceFile_Schisto.xlsx",
        sheet_name="District_Params_" + infection.lower(),
    )
    expected_district_prevalence = expected_district_prevalence[
        expected_district_prevalence["District"].isin(
            ["Blantyre", "Chiradzulu", "Mulanje", "Nsanje", "Nkhotakota", "Phalombe"]
        )
    ]
    expected_district_prevalence.set_index("District", inplace=True)
    expected_district_prevalence = expected_district_prevalence.loc[
        :, "Prevalence"
    ].to_dict()
...
districts_prevalence, districts_mwb, districts_high_inf_prev = get_values_per_district(
    "Haematobium"
)
expected_district_prevalence = get_expected_prevalence("Haematobium")
plot_prevalence_per_district(
    "Haematobium", districts_prevalence, expected_district_prevalence
)
...
def plot_mwb_bar_plots(infection):
    ...
    mwb_adults = df[df["age_group"] == "Adults"][
        prefix + "_aggregate_worm_burden"
    ].values.mean()
    mwb_sac = df[df["age_group"] == "SAC"][
        prefix + "_aggregate_worm_burden"
    ].values.mean()
    mwb_psac = df[df["age_group"] == "PSAC"][
        prefix + "_aggregate_worm_burden"
    ].values.mean()
...
def plot_prevalent_years():
    ...
    fig, ax = plt.subplots(figsize=(9, 7))
    ax.plot(
        loger_PrevalentYears_All.date,
        loger_PrevalentYears_All.Prevalent_years_this_year_total,
        label="All",
    )
    ax.xaxis_date()
...
def plot_high_inf_years():
    ...
    fig, ax = plt.subplots(figsize=(9, 7))
    ax.plot(
        loger_High_inf_years.date,
        loger_High_inf_years.High_infection_years_this_year_total,
        label="All",
    )
    ax.xaxis_date()
    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=60))
...
# this adds M and F
loger_daly = loger_daly.groupby(["date", "age_range"], as_index=False)[
    "YLD_Schisto_Haematobium_Schisto_Haematobium_Symptoms"
].sum()
age_map = {"0-4": "PSAC", "5-9": "SAC", "10-14": "SAC"}
...
plt.scatter(
    loger_daly.date[loger_daly.age_group == "Adults"],
    loger_daly.YLD_Schisto_Haematobium_Schisto_Haematobium_Symptoms[
        loger_daly.age_group == "Adults"
    ],
    label="Adults",
)
plt.scatter(
    loger_daly.date[loger_daly.age_group == "PSAC"],
    loger_daly.YLD_Schisto_Haematobium_Schisto_Haematobium_Symptoms[
        loger_daly.age_group == "PSAC"
    ],
    label="PSAC",
)
plt.scatter(
    loger_daly.date[loger_daly.age_group == "SAC"],
    loger_daly.YLD_Schisto_Haematobium_Schisto_Haematobium_Symptoms[
        loger_daly.age_group == "SAC"
    ],
    label="SAC",
)
plt.xticks(rotation="vertical")
...
def count_district_states(df, district):
    ...
    count_states.update(
        {"infected_any": count_states["Low-infection"] + count_states["High-infection"]}
    )
    count_states.update(
        {"total_pop_alive": count_states["infected_any"] + count_states["Non-infected"]}
    )
    if count_states["total_pop_alive"] != 0.0:
        count_states.update(
            {
                "Prevalence": count_states["infected_any"]
                / count_states["total_pop_alive"]
            }
        )
    else:
