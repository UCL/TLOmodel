...
def impose_bd_footprint(person_id, dur_bed):
    ...
    hs.bed_days.impose_beddays_footprint(
        person_id=person_id, footprint={"bedtype1": dur_bed}
    )
...
cp.run(
    "sim.simulate(end_date=end_date)", filename=outputpath / "bed_days_profiling.prof"
)
