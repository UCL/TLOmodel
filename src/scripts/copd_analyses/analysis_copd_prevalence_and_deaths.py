...
class CopdAnalyses:
    ...
    def plot_lung_function(self):
        ...
        for _tob in ["True", "False"]:
            ...
            for _lung_func, _ in enumerate(self.__lung_func_cats):
                plot_df[_lung_func] = re_ordered_copd_prev[_tob][f"{_lung_func}"].sum(
                    axis=1
                )
            # get totals per year
    ...
    def plot_lung_function_by_gender(self):
        ...
        for _gender in ["M", "F"]:
            plot_df = pd.DataFrame()
            for _lung_func, _ in enumerate(self.__lung_func_cats):
                plot_df[_lung_func] = re_ordered_df[_gender][f"{_lung_func}"].sum(
                    axis=1
                )
            _df_dict[_gender] = plot_df
            # get totals per year
            _df_dict[_gender] = (
                _df_dict[_gender].groupby(_df_dict[_gender].index.year).sum()
            )
            # convert totals into proportions
            _df_dict[_gender] = _df_dict[_gender].apply(
        ...
        plt.tight_layout()
        plt.savefig(
            outputpath / ("lung_function_categories_by_gender" + datestamp + ".pdf"),
            format="pdf",
        )
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def plot_lung_function_categories_by_age_group(self):
        ...
        for _tob in ["True", "False"]:
            for _lung_func, _ in enumerate(self.__lung_func_cats):
                plot_df[f"{_lung_func}"] = re_ordered_df[f"{_lung_func}"]["M"][
                    _tob
                ].sum(axis=0) + re_ordered_df[f"{_lung_func}"]["F"][_tob].sum(axis=0)
        ...
        plt.savefig(
            outputpath / ("lung_function_categories_by_age_group" + datestamp + ".pdf"),
            format="pdf",
        )
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def plot_copd_deaths_by_lungfunction(self):
        ...
        plot_lung_func_deaths = plot_lung_func_deaths.apply(
            lambda row: row / row.sum(), axis=1
        )
        # do plotting
        fig, ax = plt.subplots()
        ...
        plt.savefig(
            outputpath / ("copd_deaths_by_lung_function" + datestamp + ".pdf"),
            format="pdf",
        )
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def plot_modal_gbd_deaths_by_gender(self):
        ...
        for _col, sex in enumerate(("M", "F")):
            plot_df = (
                death_compare.loc[
                    (["2010-2014", "2015-2019"], sex, slice(None), "COPD")
                ]
                .groupby("period")
                .sum()
            )
            ax = plot_df["model"].plot.bar(
                color=self.__plot_colors[6], label="Model", ax=axs[_col], rot=0
            )
            ax.errorbar(
        ...
        plt.tight_layout()
        plt.savefig(
            outputpath / ("modal_gbd_deaths_by_gender" + datestamp + ".pdf"),
            format="pdf",
        )
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def plot_modal_gbd_deaths_by_age_group(self):
        ...
        plot_df = (
            death_compare.loc[
                (["2010-2014", "2015-2019"], slice(None), slice(None), "COPD")
            ]
            .groupby("age_grp")
            .sum()
        )
        ...
        ax = __plot_df["model"].plot.bar(
            color=self.__plot_colors[6], label="Model", rot=0
        )
        ax.errorbar(
        ...
        plt.tight_layout()
        plt.savefig(
            outputpath / ("modal_gbd_deaths_by_age_groups" + datestamp + ".pdf"),
            format="pdf",
        )
        plt.savefig(Path(".outputs"))
        plt.show()
