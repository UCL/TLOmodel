...
class CopdAnalyses:
    ...
    def plot_lung_function(self):
        ...
        plt.legend(
            self.__lung_func_cats,
            title="lung function categories",
            bbox_to_anchor=(0.7, 0.74),
            loc="upper left",
            prop=fontP,
        )
        plt.tight_layout()
        plt.savefig(outputpath / ("lung_function_categories" + datestamp + ".pdf"), format="pdf")
    ...
    def mod_sev_exacerb_lung_function_smokers(self):
        ...
        for _tob in ["True", "False"]:
            ...
            plot_df[[5, 6]].plot(
                ...
                title=f"{self.__smokers_desc[_tob].lower()} " f"in lung function categories 5 or 6",
                xlabel="Year",
                ylabel="Proportions",
                ylim=[0, 0.23],
            )
    ...
    def plot_lung_function_by_gender(self):
        ...
        for _gender in ["M", "F"]:
            ...
            # do plotting
            ax = _df_dict[_gender].plot(
                kind="bar",
                stacked=True,
                ax=axes[_rows_counter],
                color=self.__plot_colors,
                title=f"{self.__gender_desc[_gender]} " f"proportion in each Lung Function Category",
            )
            ...
            _rows_counter += 1
        plt.tight_layout()
        plt.savefig(outputpath / ("lung_function_categories_by_gender" + datestamp + ".pdf"), format="pdf")
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def sev_mod_exacerb_lung_function_by_gender(self):
        ...
        for _gender in ["M", "F"]:
            ...
            _df_dict[_gender] = _df_dict[_gender].apply(lambda row: row / row.sum(), axis=1)
            # do plotting
            ax = _df_dict[_gender][[5, 6]].plot(
                kind="bar",
                stacked=True,
                ax=axes[_rows_counter],
                ylim=[0, 0.02],
                color=self.__plot_colors[5:],
                title=f"{self.__gender_desc[_gender]} " f"in lung function categories 5 or 6",
            )
    ...
    def plot_lung_function_categories_by_age_group(self):
        ...
        for _tob in ["True", "False"]:
            for _lung_func, _ in enumerate(self.__lung_func_cats):
                plot_df[f"{_lung_func}"] = re_ordered_df[f"{_lung_func}"]["M"][_tob].sum(axis=0) + re_ordered_df[
                    f"{_lung_func}"
                ]["F"][_tob].sum(axis=0)
        ...
        plt.savefig(outputpath / ("lung_function_categories_by_age_group" + datestamp + ".pdf"), format="pdf")
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def plot_copd_deaths_by_lungfunction(self):
        ...
        deaths_lung_func = parse_log_file(self.__logfile_path)["tlo.methods.demography.detail"][
            "properties_of_deceased_persons"
        ]
        _plot_deaths = deaths_lung_func.loc[
            (deaths_lung_func.cause_of_death == "COPD_cat5") | (deaths_lung_func.cause_of_death == "COPD_cat6")
        ]
        ...
        # do plotting
        fig, ax = plt.subplots()
        plot_lung_func_deaths.plot(
            kind="bar",
            stacked=True,
            ax=ax,
            title="COPD deaths by lung function",
            color=self.__plot_colors[5:],
            xlabel="Year",
        ...
        plt.savefig(outputpath / ("copd_deaths_by_lung_function" + datestamp + ".pdf"), format="pdf")
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def plot_modal_gbd_deaths_by_gender(self):
        ...
        for _col, sex in enumerate(("M", "F")):
            ...
            ax.legend(loc=2)
        plt.tight_layout()
        plt.savefig(outputpath / ("modal_gbd_deaths_by_gender" + datestamp + ".pdf"), format="pdf")
        plt.savefig(Path(".outputs"))
        plt.show()
    ...
    def plot_modal_gbd_deaths_by_age_group(self):
        ...
        death_compare = compare_number_of_deaths(self.__logfile_path, resourcefilepath)
        plot_df = (
            death_compare.loc[(["2010-2014", "2015-2019"], slice(None), slice(None), "COPD")].groupby("age_grp").sum()
        )
        __plot_df = pd.DataFrame(
            index=["0-29", "30-39", "40-49", "50-59", "60-69", "70-79", "80+"],
            data=[
                plot_df.iloc[:6].sum(axis=0),
                plot_df.iloc[6:8].sum(axis=0),
                plot_df.iloc[8:10].sum(axis=0),
                plot_df.iloc[10:12].sum(axis=0),
                plot_df.iloc[12:14].sum(axis=0),
                plot_df.iloc[14:16].sum(axis=0),
        ...
        ax.errorbar(
            x=__plot_df["model"].index,
            y=__plot_df.GBD_mean,
        ...
        ax.legend(loc=1)
        plt.tight_layout()
        plt.savefig(outputpath / ("modal_gbd_deaths_by_age_groups" + datestamp + ".pdf"), format="pdf")
        plt.savefig(Path(".outputs"))
        plt.show()
