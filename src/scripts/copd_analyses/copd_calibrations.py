...
class CopdCalibrations:
    ...
    def copd_prevalence(self):
        ...
        for _lung_func in [3, 4, 5, 6]:
            ...
            plot_df.plot.line(y=[f"observed_data_{_lung_func}"], marker="^", color="red", ax=ax)
            ax.legend(["modal", "observed data"])
            _col_counter += 1  # increment column counter
        plt.figtext(
            0.5,
            0.01,
            "Prevalence of mild, moderate, severe, very severe COPD at mean age 49 (SD 17 years)*",
            ha="center",
    ...
    def rate_of_death_by_lungfunction_category(self):
        ...
        all_deaths = (
            demog.assign(year=lambda x: x["date"].dt.year, cat=lambda x: x["ch_lungfunction"])
            .groupby(["year", "cat"])["cause_of_death"]
            .count()
        )
        ...
        prev = (
            prev.groupby(axis=1, by=prev.columns.droplevel([0, 1, 2])).sum().groupby(axis=0, by=prev.index.year).mean()
        )
        prev["none"] = prev["0"]
        ...
        model_and_observed_data_dict = {
            "model": [
                death_rate_per100.mild,
                death_rate_per100.moderate,
                death_rate_per100.severe,
                death_rate_per100.very_severe,
            ],
            "data": [3.8, 5.1, 15.3, 27.8],
        }

        # plot rate of death (per 100 per year) by COPD stage (mild, moderate, severe, very severe)
        plot_rate_df = pd.DataFrame(
            index=["mild", "moderate", "severe", "very_severe"], data=model_and_observed_data_dict
        )
        fig, axes = plt.subplots(ncols=4, sharey=True)
        _col_counter = 0
        for _label in plot_rate_df.index:
            # do plotting
            ax = plot_rate_df.iloc[_col_counter : 1 + _col_counter].plot.line(
                ax=axes[_col_counter],
                title=f"{_label} COPD",
                y="model",
                marker="o",
                color="blue",
                ylabel="rate of COPD death per 100 " "person years",
            )

            plot_rate_df.iloc[_col_counter : 1 + _col_counter].plot.line(y="data", marker="^", color="red", ax=ax)
            _col_counter += 1  # increment column counter
        ...
        plt.figtext(
            0.5,
            0.01,
            "Rate of death (per 100 per year) by COPD stage (mild, moderate, severe, very severe)",
            ha="center",
            bbox={"facecolor": "grey", "alpha": 0.5, "pad": 5},
        )
        ...
        rr_model_and_observed_data_dict = {
            "model": [rr_rates.none, rr_rates.mild, rr_rates.moderate, rr_rates.severe + rr_rates.very_severe],
            "data": [1.0, 2.4, 3.5, 6.6],
        }

        # plot relative rate of (all cause) death according to COPD stage (none, mild, moderate, severe + v severe)
        plot_rr_rate_df = pd.DataFrame(
            index=["none", "mild", "moderate", "severe_and_very_severe"], data=rr_model_and_observed_data_dict
        )
        fig, axes = plt.subplots(ncols=4, sharey=True)
        _col_counter = 0
        for _label in plot_rr_rate_df.index:
            # do plotting
            ax = plot_rr_rate_df.iloc[_col_counter : 1 + _col_counter].plot.line(
                ax=axes[_col_counter],
                title=f"{_label} COPD",
                y="model",
                marker="o",
                color="blue",
                ylabel="relative rate of COPD death per " "100 " "person years",
            )

            plot_rr_rate_df.iloc[_col_counter : 1 + _col_counter].plot.line(y="data", marker="^", color="red", ax=ax)
            _col_counter += 1  # increment column counter
        # remove all the subplot legends
        for ax in axes:
            ax.get_legend().remove()
        ...
        plt.figtext(
            0.5,
            0.01,
            "Relative rate of (all cause) death according to COPD stage (none, mild, moderate, " "severe + v severe)",
            ha="center",
            bbox={"facecolor": "grey", "alpha": 0.5, "pad": 5},
        )
        # show plot
