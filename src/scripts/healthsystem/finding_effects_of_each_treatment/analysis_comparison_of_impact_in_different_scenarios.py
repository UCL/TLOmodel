...
results_folders = {
    "Defaults": outputs / "scenario_effect_of_each_treatment_min-2023-02-03T215051Z",
    "Consumables available and Full healthcare seeking": outputs
    / "scenario_effect_of_each_treatment_max-2023-02-03T215023Z",
}
...
yy = dict(
    zip(order_of_treatment_id, 1.0 + np.arange(0, len(order_of_treatment_id)))
)  # associate a position on the
...
for _tr, ypos_central in yy.items():
    for ypos_adjustment, _sc in zip([-0.15, 0.15], _scenarios):
        ...
        level = _dat["mean"]
        error = np.array(
            [(_dat["mean"] - _dat["lower"], _dat["upper"] - _dat["mean"])]
        ).T
