...
old_labels = [
    'No growth status quo', 'GDP growth status quo', 'GDP growth fHE growth case 1 status quo',
    'GDP growth fHE growth case 3 status quo', 'GDP growth fHE growth case 4 status quo',
    'GDP growth fHE growth case 6 status quo', 'No growth perfect consumables',
    'GDP growth perfect consumables', 'GDP growth fHE growth case 1 perfect consumables',
    'GDP growth fHE growth case 3 perfect consumables', 'GDP growth fHE growth case 4 perfect consumables',
    'GDP growth fHE growth case 6 perfect consumables'
]

new_labels = [
    'No growth present-day consumables', 'GDP growth present-day consumables', '<< GDP growth present-day consumables',
    '< GDP growth present-day consumables', '> GDP growth present-day consumables', '>> GDP growth present-day consumables',
    'No growth perfect consumables', 'GDP growth perfect consumables', '<< GDP growth perfect consumables',
    '< GDP growth perfect consumables', '> GDP growth perfect consumables', '>> GDP growth perfect consumables'
]

def replace_run_labels(df, old_labels, new_labels, level_name='draw'):
    """
    Replace labels at a specific level in MultiIndex columns.

    Parameters:
        df (pd.DataFrame): DataFrame with MultiIndex columns.
        old_labels (list): List of current labels.
        new_labels (list): List of new labels (same order).
        level_name (str): Name of the MultiIndex level to replace (default is 'run').
    ...
    # Create mapping
    rename_map = dict(zip(old_labels, new_labels))

    # Extract the current MultiIndex
    cols = df.columns

    if not isinstance(cols, pd.MultiIndex):
        raise TypeError("DataFrame must have MultiIndex columns.")
    
    # Identify the level index for 'run'
    level_idx = cols.names.index(level_name)

    # Construct new MultiIndex
    new_tuples = [
        tuple(rename_map.get(val, val) if i == level_idx else val
              for i, val in enumerate(tup))
        for tup in cols
    ]

    # Assign new MultiIndex
    df.columns = pd.MultiIndex.from_tuples(new_tuples, names=cols.names)
    return df
