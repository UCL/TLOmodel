...
results_folder = get_scenario_outputs(
    "impact_of_consumables_availability.py", outputspath
)[-1]
...
def _extract_deaths_by_age_group_and_time_period(_df: pd.DataFrame) -> pd.Series:
    ...
    _df["Period"] = (
        pd.to_datetime(_df["date"])
        .dt.year.map(calperiodlookup)
        .astype(make_calendar_period_type())
    )
...
deaths_summarized = deaths_summarized.loc[
    deaths_summarized.index.isin(("2010-2014", "2015-2019"))
]
...
for i, _p in enumerate(params.values):
    central_val = (
        deaths_summarized[(i, "mean")].values / deaths_summarized[(0, "mean")].values
    )
    lower_val = (
        deaths_summarized[(i, "lower")].values / deaths_summarized[(0, "lower")].values
    )
    upper_val = (
        deaths_summarized[(i, "upper")].values / deaths_summarized[(0, "upper")].values
    )
    ...
    ax.plot(deaths_summarized.index, central_val, label=_p)
    ax.fill_between(deaths_summarized.index, lower_val, upper_val, alpha=0.5)
