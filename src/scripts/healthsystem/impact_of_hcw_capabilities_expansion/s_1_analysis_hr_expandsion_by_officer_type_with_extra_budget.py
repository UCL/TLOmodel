...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None,
    ...
    def get_num_dalys_by_cause(_df):
        ...
        assert set(_df.year.unique()).issuperset(years_needed), "Some years are not recorded."
        return (_df
                .loc[_df.year.between(*years_needed)].drop(columns=['date', 'year', 'age_range', 'sex'])  # .drop(columns=['date', 'year', 'li_wealth'])
                .sum(axis=0)
                )
    ...
    def scenario_grouping_coloring(by='design'):
        if by == 'effect':  # based on DALYs averted/whether to  expand Clinical + Pharmacy
            ...
            grouping = {
                ...
                'current_allocation': {'s_1'},
                'optimal_allocation': {'s_*'},
                'equal_allocation': {'s_3', 's_4', 's_5', 's_6', 's_7', 's_8',
                                     's_9', 's_10', 's_11', 's_12', 's_13',
                                     's_14', 's_15', 's_16', 's_17', 's_18',
                                     's_19', 's_20', 's_21', 's_22', 's_23',
                                     's_24', 's_25', 's_26', 's_27', 's_28',
                                     's_29', 's_30', 's_31', 's_32', 's_33'}
            }
            grouping_color = {
                "gap_allocation": 'skyblue',
                "equal_allocation": 'pink',
                "current_allocation": 'yellowgreen',
                "optimal_allocation": 'gold',
                "no_allocation": "silver",
            }
    ...
    def do_bar_plot_with_ci(_df, _df_percent=None, annotation=False):
        ...
        if annotation:
            ...
            for xpos, ypos1, ypos2, text1, text2, text3 in zip(xticks.keys(),
                                                               _df['upper'].values,
                                                               _df['lower'].values,
                ...
                if text1 >= 0:
                    ax.text(xpos, ypos1 + 0.2, text, horizontalalignment='center', fontsize='x-small')
                else:
                    ax.text(xpos, ypos2 - 0.5, text, horizontalalignment='center', fontsize='x-small')
        ...
        legend_list = list(scenario_groups[1].keys())
        legend_labels = [s for s in legend_list if s not in ['optimal_allocation', 'current_allocation']]
    ...
    def get_hcw_time_by_treatment():
        ...
        time_increased_by_treatment = time_by_treatment_all_scenarios.subtract(
            time_by_treatment_all_scenarios.loc['s_1', :], axis=1).drop('s_1', axis=0).add_suffix('*')
    ...
    # format num_dalys for regression analysis on draw-run results
    def format_draw_run_results(df):
        _draw_run = df.unstack()
        _draw = _draw_run.index.get_level_values('draw')
        _run = _draw_run.index.get_level_values('run')
        _draw_run = _draw_run.to_frame()
        _draw_run['draw_no'] = _draw
        _draw_run['run_no'] = _run

        return _draw_run

    percent_dalys_averted_draw_run = -1.0 * pd.DataFrame(
        find_difference_relative_to_comparison_series(
                num_dalys.loc[0],
                comparison='s_1',
                scaled=True,
                drop_comparison=False,
        )
    ).T

    dalys_averted_draw_run = -1.0 * pd.DataFrame(
        find_difference_relative_to_comparison_series(
            num_dalys.loc[0],
            comparison='s_1',
            drop_comparison=False,
    ...
    num_dalys_draw_run = format_draw_run_results(num_dalys).rename(columns={0: 'dalys'})
    percent_dalys_averted_draw_run = format_draw_run_results(percent_dalys_averted_draw_run
                                                             ).rename(columns={0: 'percent_dalys_averted'})
    dalys_averted_draw_run = format_draw_run_results(dalys_averted_draw_run
                                                     ).rename(columns={0: 'dalys_averted'})
    ...
    num_dalys_by_cause = extract_results(
        results_folder,
        module="tlo.methods.healthburden",
        key='dalys_stacked',  # "dalys_by_wealth_stacked_by_age_and_time",
    ...
    hcw_time_increased_by_treatment_type = get_hcw_time_by_treatment().reindex(num_dalys_summarized.index).drop(['s_1'])
    ...
    num_dalys_by_cause_group_averted_ci = summarize_by_cause(
        -1.0 * find_difference_relative_to_comparison_dataframe(
            num_dalys_by_cause_group,
            comparison='s_1',
        ),
    )
    draw_order = list(num_dalys_summarized.index)
    num_dalys_by_cause_group_averted_ci['draw'] = pd.Categorical(
        num_dalys_by_cause_group_averted_ci['draw'], categories=draw_order, ordered=True)
    num_dalys_by_cause_group_averted_ci = num_dalys_by_cause_group_averted_ci.sort_values(by=['cause', 'draw'])
    num_dalys_by_cause_group_averted_ci.to_csv(output_folder / 'num_dalys_by_cause_area_averted_ci.csv')
    ...
    draw_order = list(num_dalys_summarized.index)
    num_dalys_by_cause_group_averted_percent_ci['draw'] = pd.Categorical(
        num_dalys_by_cause_group_averted_percent_ci['draw'], categories=draw_order, ordered=True)
    num_dalys_by_cause_group_averted_percent_ci = num_dalys_by_cause_group_averted_percent_ci.sort_values(by=['cause', 'draw'])
    num_dalys_by_cause_group_averted_percent_ci.to_csv(output_folder / 'num_dalys_by_cause_area_averted_percent_ci.csv')
    ...
    num_treatments_group_increased_ci = summarize_by_cause(
        find_difference_relative_to_comparison_dataframe(
            num_treatments_group,
            comparison='s_1',
        ),
    ...
    draw_order = list(num_dalys_summarized.index)
    num_treatments_group_increased_ci['draw'] = pd.Categorical(
        num_treatments_group_increased_ci['draw'], categories=draw_order, ordered=True)
    num_treatments_group_increased_ci = num_treatments_group_increased_ci.sort_values(
        by=['cause', 'draw'])
    num_treatments_group_increased_ci.to_csv(output_folder / 'num_treatments_area_increased_ci.csv')

    num_treatments_group_increased_percent_ci = summarize_by_cause(
        find_difference_relative_to_comparison_dataframe(
            num_treatments_group,
            comparison='s_1',
            scaled=True
    ...
    draw_order = list(num_dalys_summarized.index)
    num_treatments_group_increased_percent_ci['draw'] = pd.Categorical(
        num_treatments_group_increased_percent_ci['draw'], categories=draw_order, ordered=True)
    num_treatments_group_increased_percent_ci = num_treatments_group_increased_percent_ci.sort_values(
        by=['cause', 'draw'])
    ...
    def hcw_time_or_cost_used(time_cost_df=appt_time, count_df=num_appts_by_level_summarized):
        ...
        use_increased = use.subtract(use.loc['s_1', :], axis=1).drop('s_1', axis=0)

        use_increase_percent = use.subtract(use.loc['s_1', :], axis=1
                                            ).divide(use.loc['s_1', :], axis=1).drop('s_1', axis=0)
    ...
    scenario_groups = scenario_grouping_coloring(by='design')
    scenario_color = {}
    ...
    # prepare extra budget allocation and HRH increase rates
    ...
    # # do linear regression on percent dalys averted against s_1: draw-run results
    # increase_rate_avg = increase_rate_avg_exp.reset_index(drop=False).rename(columns={'draw': 'draw_no'})
    # regression_draw_run = pd.merge(percent_dalys_averted_draw_run,
    #                                increase_rate_avg,
    #                                on='draw_no', how='outer')
    #
    # draw_order = list(num_dalys_summarized.index)
    # order_map = {name: i for i, name in enumerate(draw_order)}
    # regression_draw_run['sort_key'] = regression_draw_run['draw_no'].map(order_map)
    # regression_draw_run = regression_draw_run.sort_values('sort_key').drop(columns='sort_key')
    #
    # regression_draw_run['percent_dalys_averted'] = regression_draw_run['percent_dalys_averted'] * 100
    #
    # regression_draw_run.drop(index=regression_draw_run[regression_draw_run.draw_no == 's_0'].index, inplace=True)
    # # regression_draw_run.drop(index=regression_draw_run[regression_draw_run.draw_no == 's_1'].index, inplace=True)
    # cadres_to_drop_due_to_multicollinearity = ['Dental', 'Laboratory', 'Mental', 'Nutrition', 'Radiography']
    # regression_draw_run.drop(columns=cadres_to_drop_due_to_multicollinearity, inplace=True)
    #
    # predictor = regression_draw_run[['Clinical', 'DCSA', 'Nursing_and_Midwifery', 'Pharmacy', 'Other']].copy()
    # outcome = regression_draw_run['percent_dalys_averted']  # 'dalys'
    # predictor = sm.add_constant(predictor)  # add constant term
    # est = sm.OLS(outcome.astype(float), predictor.astype(float)).fit()
    # print(est.summary())  # R-squared=0.713, all coefficients significant with P<0.001
    #
    # # get prediction results with confidence and prediction intervals
    ...
    #
    # # plot data and fitted values with confidence and prediction intervals
    # regression_prediction_data = pd.merge(regression_draw_run, pred_summary,
    #                                       left_index=True, right_index=True, how='inner')
    # name_of_plot = 'Percent DALYs averted against current_allocation scenario 2025-2034'
    # fig, ax = plt.subplots(figsize=(9, 6))
    #
    # x = regression_prediction_data.draw_no
    # y_data = regression_prediction_data.percent_dalys_averted  # dalys
    # y = est.fittedvalues
    #
    # ci_lower_error = y - ci_lower
    # ci_upper_error = ci_upper - y
    # ci_yerr = np.vstack([ci_lower_error, ci_upper_error])
    #
    # pi_lower_error = y - pi_lower
    # pi_upper_error = pi_upper - y
    # pi_yerr = np.vstack([pi_lower_error, pi_upper_error])
    #
    # plt.scatter(x, y_data, label='Data', color='blue', marker='.')
    # plt.errorbar(x, y, yerr=ci_yerr, capsize=5, ecolor='green', linestyle='None', marker='o',
    #              color='red', alpha=0.3, label='Fitted value with 95% CI')
    # plt.errorbar(x, y, yerr=pi_yerr, capsize=5, ecolor='red', linestyle='None', marker='o',
    #              color='red', alpha=0.3, label='Fitted value with 95% PI')
    #
    # xtick_labels = [substitute_labels[v] for v in x.drop_duplicates()]
    ...
    #
    # plt.xlabel('Extra budget allocation scenario')
    # plt.ylabel('% DALYs averted')  # 'DALYs 2025-3034'
    # plt.legend()
    ...
    # try linear regression on dalys: draw-run results, extra budget proportions
    extra_budget_prop = extra_budget_allocation.reset_index(drop=False).rename(columns={'draw': 'draw_no'})
    regression_draw_run = pd.merge(num_dalys_draw_run,
                                   extra_budget_prop,
                                   on='draw_no', how='outer')
    draw_order = list(num_dalys_summarized.index)
    order_map = {name: i for i, name in enumerate(draw_order)}
    ...
    predictor = 100 * regression_draw_run[['Clinical', 'DCSA', 'Nursing_and_Midwifery', 'Pharmacy']]
    # predictor['C*P'] = predictor['Clinical'] * predictor['Pharmacy']
    outcome = regression_draw_run['dalys'] / 1e6  # increase rate to percentage and dalys unit in million so that the coefficients look neater
    predictor = sm.add_constant(predictor)  # add constant term
    est = sm.OLS(outcome.astype(float), predictor.astype(float)).fit()
    print(est.summary())  # Adjusted R-squared=0.415, coefficient of DCSA not significant

    # get prediction results with confidence and prediction intervals
    pred = est.get_prediction(predictor.astype(float))
    pred_summary = pred.summary_frame(alpha=0.05)
    ci_lower = pred_summary['mean_ci_lower']
    ci_upper = pred_summary['mean_ci_upper']
    pi_lower = pred_summary['obs_ci_lower']
    ...
    regression_prediction_data = pd.merge(regression_draw_run, pred_summary,
                                          left_index=True, right_index=True, how='inner')
    name_of_plot = 'Total DALYs 2025-2034 (regression on incremental budget proportions)'
    fig, ax = plt.subplots(figsize=(9, 6))

    x = regression_prediction_data.draw_no
    y_data = regression_prediction_data.dalys
    y = est.fittedvalues
    ...
    plt.scatter(x, y_data, label='Data', color='blue', marker='.')
    plt.errorbar(x, y * 1e6, yerr=ci_yerr * 1e6, capsize=5, ecolor='green', linestyle='None', marker='o',
                 color='red', alpha=0.3, label='Fitted value with 95% CI')
    plt.errorbar(x, y * 1e6, yerr=pi_yerr * 1e6, capsize=5, ecolor='red', linestyle='None', marker='o',
                 color='red', alpha=0.3, label='Fitted value with 95% PI')

    xtick_labels = [substitute_labels[v] for v in x.drop_duplicates()]
    ax.set_xticklabels(xtick_labels, rotation=90, fontsize='small')  # re-label scenarios

    plt.xlabel('Incremental budget allocation scenario')
    plt.ylabel('DALYs')
    plt.legend()
    plt.title(name_of_plot, fontsize='medium')
    fig.tight_layout()
    ...
    # do linear regression on dalys: draw-run results
    increase_rate_avg = increase_rate_avg_exp.reset_index(drop=False).rename(columns={'draw': 'draw_no'})
    regression_draw_run = pd.merge(num_dalys_draw_run,
                                   increase_rate_avg,
                                   on='draw_no', how='outer')
    ...
    predictor = 100 * regression_draw_run[['Clinical', 'DCSA', 'Nursing_and_Midwifery', 'Pharmacy', 'Other']]
    # predictor['C*P'] = predictor['Clinical'] * predictor['Pharmacy']
    outcome = regression_draw_run['dalys']/1e6  # increase rate to percentage and dalys unit in million so that the coefficients look neater
    predictor = sm.add_constant(predictor)  # add constant term
    est = sm.OLS(outcome.astype(float), predictor.astype(float)).fit()
    print(est.summary())  # R-squared=0.804, all coefficients significant with P<0.001
    # save the regression model
    # import pickle
    # with open('reg_model_max_hs_func.pkl', 'wb') as f:
    #     pickle.dump(est, f)
    ...
    regression_prediction_data = pd.merge(regression_draw_run, pred_summary,
                                          left_index=True, right_index=True, how='inner')
    name_of_plot = 'Total DALYs 2025-2034'
    fig, ax = plt.subplots(figsize=(9, 6))
    ...
    # read in random sample data and test the regression model (out of sample)
    sample_path = Path(
        './src/scripts/healthsystem/impact_of_hcw_capabilities_expansion/extra_budget_fracs_sample_17Mar2025'
    )
    rs_data = pd.read_csv(sample_path / 'dalys_and_avg_increase_rates_sample.csv')
    rs_predictor = 100 * rs_data[['Clinical', 'DCSA', 'Nursing_and_Midwifery', 'Pharmacy', 'Other']]
    # rs_predictor['C*P'] = rs_predictor['Clinical'] * rs_predictor['Pharmacy']
    rs_predictor = sm.add_constant(rs_predictor)

    # # try regression on rs_data
    # rs_outcome = rs_data['dalys']
    # rs_est = sm.OLS(rs_outcome.astype(float), rs_predictor.astype(float)).fit()
    # print(rs_est.summary())  # R-squared = 0.706, all coefficients significant with P<0.001
    #
    # # try regression on rs_data + regression_draw_run data (special scenarios)
    # all_data = pd.concat([
    #     rs_data[['dalys', 'Clinical', 'DCSA', 'Nursing_and_Midwifery', 'Pharmacy', 'Other']],
    #     regression_draw_run[['dalys', 'Clinical', 'DCSA', 'Nursing_and_Midwifery', 'Pharmacy', 'Other']]
    # ], ignore_index=True)
    #
    # all_predictor = all_data[['Clinical', 'DCSA', 'Nursing_and_Midwifery', 'Pharmacy', 'Other']].copy()
    # all_predictor = sm.add_constant(all_predictor)
    # all_outcome = all_data['dalys']
    # all_est = sm.OLS(all_outcome.astype(float), all_predictor.astype(float)).fit()
    # print(all_est.summary())  # R-squared = 0.792, all coefficients significant with P<0.001

    # use the specially designed scenarios' regression model to predict
    pred_rs = est.get_prediction(rs_predictor)
    pred_summary_rs = pred_rs.summary_frame(alpha=0.05)

    rs_data_prediction = pd.merge(rs_data['dalys'],
                                  pred_summary_rs,
                                  left_index=True, right_index=True, how='inner')
    rs_data_prediction['within_PI'] = False
    for i in rs_data_prediction.index:
        if ((rs_data_prediction.loc[i, 'dalys'] >= 1e6 * rs_data_prediction.loc[i, 'obs_ci_lower'])
...
# make the above process as a function of alpha and plot test_result against alpha
    def func_rs_test_result(_alpha=0.05):
        _pred_summary_rs = pred_rs.summary_frame(alpha=_alpha)
        _rs_data_prediction = pd.merge(rs_data['dalys'],
                                       _pred_summary_rs,
                                       left_index=True, right_index=True, how='inner')
        _rs_data_prediction['within_PI'] = False
        for i in _rs_data_prediction.index:
            if ((_rs_data_prediction.loc[i, 'dalys'] >= 1e6 * _rs_data_prediction.loc[i, 'obs_ci_lower'])
                & (_rs_data_prediction.loc[i, 'dalys'] <= 1e6 * _rs_data_prediction.loc[i, 'obs_ci_upper'])):
                _rs_data_prediction.loc[i, 'within_PI'] = True

        _rs_test_result = _rs_data_prediction['within_PI'].sum() / len(_rs_data_prediction['within_PI'])

        return _rs_test_result

    alpha_range = np.arange(0.01, 0.51, 0.01)
    rs_test_result_on_alpha = pd.DataFrame(columns=['alpha', 'result'], index=range(len(alpha_range)))
    for i in rs_test_result_on_alpha.index:
        rs_test_result_on_alpha.loc[i, 'alpha'] = alpha_range[i]
        rs_test_result_on_alpha.loc[i, 'result'] = func_rs_test_result(_alpha=alpha_range[i])

    name_of_plot = 'Proportions of random samples in (1-Alpha)% Prediction Intervals'
    fig, ax = plt.subplots()
    plt.plot(rs_test_result_on_alpha.alpha, 100 * rs_test_result_on_alpha.result,
             color='blue', marker='.', alpha=0.8)
    ax.set_yticks(np.arange(40, 110, 10))
    ax.set_xticks(np.arange(0, 0.55, 0.05))
    plt.xlabel('Alpha')
    plt.ylabel('Percentage %')
    ax.text(0.05, 96.5, '(0.05, 96)', color='green')
    ax.text(0.10, 92.5, '(0.10, 92)', color='yellowgreen')
    ax.text(0.25, 70.5, '(0.25, 70)', color='gray')
    ymin, ymax = ax.get_ylim()
    xmin, xmax = ax.get_xlim()
    ax.axvline(x=0.05, color='green', linestyle='--', ymax=(96 - ymin) / (ymax - ymin))
    ax.axhline(y=96, color='green', linestyle='--', xmax=(0.05 - xmin) / (xmax - xmin))
    ax.axvline(x=0.10, color='yellowgreen', linestyle='--', ymax=(92 - ymin) / (ymax - ymin))
    ax.axhline(y=92, color='yellowgreen', linestyle='--', xmax=(0.10 - xmin) / (xmax - xmin))
    ax.axvline(x=0.25, color='gray', linestyle='--', ymax=(70 - ymin) / (ymax - ymin))
    ax.axhline(y=70, color='gray', linestyle='--', xmax=(0.25 - xmin) / (xmax - xmin))
    # plt.legend()
    plt.title(name_of_plot, fontsize='medium')
    fig.tight_layout()
    fig.savefig(make_graph_file_name(name_of_plot.replace(' ', '_').replace(',', '')))
    ...
    # # todo: in regression analysis, need to drop "s_0" as it uses no extra budget (the budget proportions are 0).
    ...
    ax.set_xlabel('Incremental budget allocation scenario', fontsize='small')
    ...
    ax.set_xlabel('Incremental budget allocation scenario')
    xtick_labels = [substitute_labels[v] for v in data_to_plot.index]
    ...
    # Figure 1(a)
    name_of_plot = f'DALYs averted against current_allocation strategy {target_period()}'
    ...
    ax.set_xlabel('Incremental budget allocation scenario', fontsize='medium')
    fig.tight_layout()
    fig.savefig(make_graph_file_name(name_of_plot.replace(' ', '_').replace(',', '')))
    fig.show()
    plt.close(fig)
    ...
    # Figure 2(a)
    name_of_plot = f'Services increased by health area against current_allocation strategy {target_period()}'
    ...
    for xpos, ypos, text1 in zip(ax.get_xticks(),
                                 (num_treatments_total_increased['mean'] / 1e6).values,
                                 num_treatments_total_increased_percent['mean'].values):
        text = f"{int(round(text1 * 100, 2))}%"  # \n{[round(text2, 2),round(text3, 2)]}"
        if text1 >= 0:
            ax.text(xpos, ypos + 0.7, text, horizontalalignment='center', fontsize='small')
        else:
            ax.text(xpos, ypos - 2, text, horizontalalignment='center', fontsize='small')
    ...
    plt.legend(title='Health area', title_fontsize='small',
               fontsize='small', reverse=True)
    plt.title(name_of_plot, fontsize='medium')
    fig.tight_layout()
    fig.savefig(make_graph_file_name(
    ...
    # plot relative services by treatment area averted
    # Figure 2(b)/Figure A.2.2: if to remove CIs, need to comment ax.errorbar
    name_of_plot = f'Services increased by health area (%) against current_allocation strategy {target_period()}'
    data_to_plot = num_treatments_group_increased_percent_ci[
        num_treatments_group_increased_percent_ci['cause'].isin(
            ['RMNCH', 'HIV/AIDS', 'Malaria', 'TB (non-AIDS)', 'NCDs', 'Transport Injuries'])
    ].pivot(index='draw', columns='cause', values='mean')
    data_to_plot = data_to_plot * 100

    data_to_plot_with_yerr = num_treatments_group_increased_percent_ci[
        num_treatments_group_increased_percent_ci['cause'].isin(
            ['RMNCH', 'HIV/AIDS', 'Malaria', 'TB (non-AIDS)', 'NCDs', 'Transport Injuries'])
    ].copy()
    data_to_plot_with_yerr['yerr_lower'] = (data_to_plot_with_yerr['mean'] - data_to_plot_with_yerr['lower']) * 100
    data_to_plot_with_yerr['yerr_upper'] = (data_to_plot_with_yerr['upper'] - data_to_plot_with_yerr['mean']) * 100

    lower_error_to_plot = data_to_plot_with_yerr.pivot(index='draw', columns='cause', values='yerr_lower')
    upper_error_to_plot = data_to_plot_with_yerr.pivot(index='draw', columns='cause', values='yerr_upper')

    assert (lower_error_to_plot.index == data_to_plot.index).all()
    assert (lower_error_to_plot.columns == data_to_plot.columns).all()
    assert (upper_error_to_plot.index == data_to_plot.index).all()
    assert (upper_error_to_plot.columns == data_to_plot.columns).all()

    markers = ['o', 's', '^', 'D', 'P', 'X']
    cause_groups = data_to_plot.columns
    marker_map = dict(zip(cause_groups, markers))

    fig, ax = plt.subplots(figsize=(9, 6))
    x_positions = np.arange(len(data_to_plot.index))

    jitter = np.random.uniform(-0.1, 0.1, size=len(data_to_plot))
    # Plot each cause group as scatter dots
    for i, cause in enumerate(cause_groups):
        y = data_to_plot[cause].values
        yerr = np.vstack([lower_error_to_plot[cause], upper_error_to_plot[cause]])

        ax.errorbar(
            x_positions + jitter,
            y,
            yerr=yerr,
            fmt=marker_map[cause],
            color=cause_group_color[cause],
            markersize=5,
            capsize=3,
            label=None,  # legend handled by scatter below
            linestyle='none',
            alpha=0.7
        )
        ...
        ax.scatter(
            ...
            y,
            label=cause,
            marker=marker_map[cause],
            color=cause_group_color[cause],
            s=20,
            edgecolors='none'  # optional: adds border to dots
        )
    ...
    xtick_labels = [substitute_labels[v] for v in data_to_plot.index]
    ax.set_xticklabels(xtick_labels, rotation=90, fontsize='small')
    ax.set_xlim(-1, len(data_to_plot.index))

    ax.set_ylabel('Services increased in %', fontsize='medium')
    ax.set_xlabel('Incremental budget allocation scenario', fontsize='medium')
    ax.grid(axis="y")
    ...
    name_of_plot = f'HCW time-used increased by cadre against current_allocation strategy {target_period()}'
    ...
    for xpos, ypos, text1 in zip(ax.get_xticks(),
        ...
        if text1 >= 0:
            ...
            ax.text(xpos, ypos - 0.12, text, horizontalalignment='center', fontsize='small')
    ...
    name_of_plot = f'DALYs by cause area averted against current_allocation strategy {target_period()}'
    ...
    # plot relative dalys by cause group averted
    # Figure 1(b)/Figure A.2.1, if to remove CIs, need to comment ax.errorbar
    name_of_plot = f'DALYs by health area averted (%) against current_allocation strategy {target_period()}'
    data_to_plot = num_dalys_by_cause_group_averted_percent_ci[
        num_dalys_by_cause_group_averted_percent_ci['cause'].isin(
            ['RMNCH', 'HIV/AIDS', 'Malaria', 'TB (non-AIDS)', 'NCDs', 'Transport Injuries'])
    ].pivot(index='draw', columns='cause', values='mean')
    data_to_plot = data_to_plot * 100

    data_to_plot_with_yerr = num_dalys_by_cause_group_averted_percent_ci[
        num_dalys_by_cause_group_averted_percent_ci['cause'].isin(
            ['RMNCH', 'HIV/AIDS', 'Malaria', 'TB (non-AIDS)', 'NCDs', 'Transport Injuries'])
    ].copy()
    data_to_plot_with_yerr['yerr_lower'] = (data_to_plot_with_yerr['mean'] - data_to_plot_with_yerr['lower']) * 100
    ...
    ax.set_ylabel('DALYs averted in %', fontsize='medium')
    ax.set_xlabel('Incremental budget allocation scenario', fontsize='medium')
    ax.grid(axis="y")
