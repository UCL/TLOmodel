...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None, rtn_results=False):
    ...
    def get_parameter_names_from_scenario_file() -> Tuple[str]:
        ...
        e = EffectOfEachTreatmentStatusQuo()
    ...
    num_deaths_averted = (
        summarize(pd.DataFrame(find_difference_extra_relative_to_comparison(num_deaths, comparison="Everything")).T)
        .iloc[0]
        .unstack()
    ...
    pc_deaths_averted = 100.0 * summarize(
        pd.DataFrame(find_difference_extra_relative_to_comparison(num_deaths, comparison="Everything", scaled=True)).T
    ...
    num_dalys_averted = (
        summarize(pd.DataFrame(find_difference_extra_relative_to_comparison(num_dalys, comparison="Everything")).T)
        .iloc[0]
        .unstack()
        .drop(["FirstAttendance*"])
        .sort_values(by="mean", ascending=True)
    ...
    pc_dalys_averted = 100.0 * summarize(
        pd.DataFrame(find_difference_extra_relative_to_comparison(num_dalys, comparison="Everything", scaled=True)).T
    ...
    def get_total_num_death_by_agegrp_and_label(_df):
        ...
        return _df.loc[_df["date"].between(*TARGET_PERIOD)].groupby([age_group, "label"])["person_id"].size()
    ...
    def get_total_num_death_by_wealth_and_label(_df):
        ...
        wealth_group = _df["li_wealth"].map(wealth_cats).astype(pd.CategoricalDtype(wealth_cats.values(), ordered=True))

        return _df.loc[_df["date"].between(*TARGET_PERIOD)].groupby([wealth_group, "label"])["person_id"].size()
...
if __name__ == "__main__":
    ...
    if args.results_path is None:
        candidate_paths = glob.glob(str(Path("outputs") / "*effect_of_each_treatment*"), recursive=True)
        candidate_paths += glob.glob(str(Path("outputs") / "*" / "*effect_of_each_treatment*"), recursive=True)
    ...
    # Plot the legends
    plot_legends.apply(results_folder=None, output_folder=results_path, resourcefilepath=rfp)

    # Plot the organisation chart of the TREATMENT_IDs
    plot_org_chart_treatment_ids.apply(results_folder=None, output_folder=results_path, resourcefilepath=None)
