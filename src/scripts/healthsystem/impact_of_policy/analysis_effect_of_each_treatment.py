...
from scripts.healthsystem.finding_effects_of_each_treatment import (
    plot_org_chart_treatment_ids,
)
...
def apply(
    ...
    make_graph_file_name = (
        lambda stub: output_folder / f"{stub.replace('*', '_star_')}.png"
    )  # noqa: E731
    ...
    def set_param_names_as_column_index_level_0(_df):
        ...
        names_of_cols_level0 = [
            ordered_param_names_no_prefix.get(col) for col in _df.columns.levels[0]
        ]
    ...
    def find_difference_extra_relative_to_comparison(
        ...
        return (
            _ser.unstack()
            .apply(
                lambda x: (x - x[comparison]) / (x[comparison] if scaled else 1.0),
                axis=0,
            )
    ...
    def do_bar_plot_with_ci(_df, _ax):
        ...
        _df_sorted = _df.reindex(
            index=CAUSE_OF_DEATH_OR_DALY_LABEL_TO_COLOR_MAP.keys(), fill_value=0.0
        ).sort_index(
        ...
        for i, cause_label in enumerate(_df_sorted.index):
            ...
            lower_bar_yerr = np.array(
                [
                    one_cause.loc[("Everything", "mean")]
                    - one_cause.loc[("Everything", "lower")],
                    one_cause.loc[("Everything", "upper")]
                    - one_cause.loc[("Everything", "mean")],
                ]
            ).reshape(2, 1)
        ...
        _ax.legend(
            [lb, ub], ["All TREATMENT_IDs", "No TREATMENT_IDs"], loc="upper right"
        )
    ...
    def do_barh_plot_with_ci(_df, _ax):
        ...
        errors = pd.concat(
            [_df["mean"] - _df["lower"], _df["upper"] - _df["mean"]], axis=1
        ).T.to_numpy()
        _df.plot.barh(
            ax=_ax,
            y="mean",
            xerr=errors,
            legend=False,
            color=[get_color_short_treatment_id(_id) for _id in _df.index],
        )

    def do_label_barh_plot(_df, _ax):
        """Add text annotation from values in _df onto _ax"""
        y_cords = {
            ylabel.get_text(): ytick
            for ytick, ylabel in zip(_ax.get_yticks(), _ax.get_yticklabels())
        }
    ...
    num_deaths_averted = (
        summarize(
            pd.DataFrame(
                find_difference_extra_relative_to_comparison(
                    num_deaths, comparison="Everything"
                )
            ).T
        )
        .iloc[0]
        ...
        .drop(["FirstAttendance*"])
    )

    pc_deaths_averted = 100.0 * summarize(
        pd.DataFrame(
            find_difference_extra_relative_to_comparison(
                num_deaths, comparison="Everything", scaled=True
            )
        ).T
    ).iloc[0].unstack().sort_values(by="mean", ascending=True).drop(
        ["FirstAttendance*"]
    )
    ...
    num_dalys_averted = (
        summarize(
            pd.DataFrame(
                find_difference_extra_relative_to_comparison(
                    num_dalys, comparison="Everything"
                )
            ).T
        )
        .iloc[0]
    ...
    pc_dalys_averted = 100.0 * summarize(
        pd.DataFrame(
            find_difference_extra_relative_to_comparison(
                num_dalys, comparison="Everything", scaled=True
            )
        ).T
    ).iloc[0].unstack().drop(["FirstAttendance*"]).sort_values(
    ...
    def get_total_num_death_by_agegrp_and_label(_df):
        ...
        return (
            _df.loc[_df["date"].between(*TARGET_PERIOD)]
            .groupby([age_group, "label"])["person_id"]
            .size()
        )
    ...
    deaths_averted_by_agegrp_and_label = (
        find_mean_difference_extra_relative_to_comparison_dataframe(
    ...
    for _scenario_name, _deaths_av in deaths_averted_by_agegrp_and_label.T.iterrows():
        ...
        format_to_plot = (
            ...
            .reindex(
                columns=CAUSE_OF_DEATH_OR_DALY_LABEL_TO_COLOR_MAP.keys(), fill_value=0.0
            )
        ...
        fig, ax = plt.subplots()
        name_of_plot = (
            f"Deaths Averted by {_scenario_name} by Age and Cause {target_period()}"
        )
        (format_to_plot / 1000).plot.bar(
            stacked=True,
            ax=ax,
            color=[
                get_color_cause_of_death_or_daly_label(_label)
                for _label in format_to_plot.columns
            ],
        )
    ...
    def get_total_num_death_by_wealth_and_label(_df):
        ...
        return (
            _df.loc[_df["date"].between(*TARGET_PERIOD)]
            .groupby([wealth_group, "label"])["person_id"]
            .size()
        )
    ...
    deaths_averted_by_wealth_and_label = (
        find_mean_difference_extra_relative_to_comparison_dataframe(
    ...
    for _scenario_name, _deaths_av in deaths_averted_by_wealth_and_label.T.iterrows():
        ...
        fig, ax = plt.subplots()
        name_of_plot = (
            f"Deaths Averted by {_scenario_name} by Wealth and Cause {target_period()}"
        )
        (format_to_plot / 1000).plot.bar(
            stacked=True,
            ax=ax,
    ...
    dalys_averted_by_agegrp_and_label = (
        find_mean_difference_extra_relative_to_comparison_dataframe(
    ...
    for _scenario_name, _dalys_av in dalys_averted_by_agegrp_and_label.T.iterrows():
        ...
        fig, ax = plt.subplots()
        name_of_plot = (
            f"DALYS Averted by {_scenario_name} by Age and Cause {target_period()}"
        )
    ...
    dalys_averted_by_wealth_and_label = (
        find_mean_difference_extra_relative_to_comparison_dataframe(
    ...
    for _scenario_name, _dalys_av in dalys_averted_by_wealth_and_label.T.iterrows():
        ...
        fig, ax = plt.subplots()
        name_of_plot = (
            f"DALYS Averted by {_scenario_name} by Wealth and Cause {target_period()}"
        )
        (format_to_plot / 1e6).plot.bar(
            stacked=True,
            ax=ax,
    ...
    mean_num_hsi_by_short_treatment_id = summarize(
        counts_of_hsi_by_short_treatment_id, only_mean=True
    )
    ...
    delta_appts = find_mean_difference_in_appts_relative_to_comparison(
        counts_of_appts, comparison="Everything"
    )
    ...
    delta_appts_coarse = (
        delta_appts.groupby(axis=0, by=delta_appts.index.map(get_coarse_appt_type))
        .sum()
    ...
    delta_appts_coarse = delta_appts_coarse[num_dalys_averted.index].drop(
        columns=["*"]
    )  # <-- order as dalys averted
    (delta_appts_coarse / 1e6).T.plot.barh(
        stacked=True,
        legend=True,
        ax=ax,
        color=[get_color_coarse_appt(_a) for _a in delta_appts_coarse.index],
    )
    ax.set_title(name_of_plot, {"size": 12, "color": "black"})
...
if __name__ == "__main__":
    ...
    with zipfile.ZipFile(
        output_path / f"images_{output_path.parts[-1]}.zip", mode="w"
    ) as archive:
