...
def apply(
        ...
        summarize(
            ...
            * pd.DataFrame(
                find_difference_relative_to_comparison(
                    num_deaths.loc[0], comparison="Status Quo"
                )
            ).T
        )
        .iloc[0]
    ...
    pc_deaths_averted = 100.0 * summarize(
        ...
        * pd.DataFrame(
            find_difference_relative_to_comparison(
                num_deaths.loc[0], comparison="Status Quo", scaled=True
            )
        ).T
    ).iloc[0].unstack().reindex(param_names).drop(
        ["No Healthcare System", "With Hard Constraints", "Status Quo"]
    )
    ...
    num_dalys_averted = (
        summarize(
            ...
            * pd.DataFrame(
                find_difference_relative_to_comparison(
                    num_dalys.loc[0], comparison="Status Quo"
                )
            ).T
        )
        .iloc[0]
    ...
    pc_dalys_averted = 100.0 * summarize(
        ...
        * pd.DataFrame(
            find_difference_relative_to_comparison(
                num_dalys.loc[0], comparison="Status Quo", scaled=True
            )
        ).T
    ).iloc[0].unstack().reindex(param_names).drop(
        ["No Healthcare System", "With Hard Constraints", "Status Quo"]
