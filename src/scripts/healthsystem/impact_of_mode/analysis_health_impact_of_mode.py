...
def apply(
    ...
    num_dalys_averted = (
        summarize(
            -1.0
            * pd.DataFrame(
                find_difference_relative_to_comparison(num_dalys.loc[0], comparison="No Healthcare System")
            ).T
        )
        .iloc[0]
        .unstack()
    ...
    pc_dalys_averted = (
        ...
        * summarize(
            -1.0
            * pd.DataFrame(
                find_difference_relative_to_comparison(num_dalys.loc[0], comparison="No Healthcare System", scaled=True)
            ).T
        )
        .iloc[0]
        .unstack()
