...
def apply(
    ...
    def get_num_deaths(_df):
        ...
        return pd.Series(
            data=len(_df.loc[pd.to_datetime(_df.date).between(*TARGET_PERIOD)])
        )

    def get_num_dalys(_df):
        """Return total number of DALYS (Stacked) by label (total within the TARGET_PERIOD)"""
        return pd.Series(
            data=_df.loc[_df.year.between(*[i.year for i in TARGET_PERIOD])]
    ...
    pc_deaths_averted = (
        ...
        * summarize(
            ...
            * pd.DataFrame(
                find_difference_relative_to_comparison(
                    num_deaths.loc[0], comparison="No Healthcare System", scaled=True
                )
            ).T
        )
        .iloc[0]
    ...
    num_dalys_averted = (
        summarize(
            ...
            * pd.DataFrame(
                find_difference_relative_to_comparison(
                    num_dalys.loc[0], comparison="No Healthcare System"
                )
            ).T
        )
        .iloc[0]
    ...
    pc_dalys_averted = (
        ...
        * summarize(
            ...
            * pd.DataFrame(
                find_difference_relative_to_comparison(
                    num_dalys.loc[0], comparison="No Healthcare System", scaled=True
                )
            ).T
        )
        .iloc[0]
    ...
    name_of_plot = (
        f"Additional Deaths Averted vs No Healthcare System, {target_period()}"
    )
    ...
    name_of_plot = (
        f"Additional DALYs Averted vs No Healthcare System, {target_period()}"
    )
    fig, ax = do_bar_plot_with_ci(
