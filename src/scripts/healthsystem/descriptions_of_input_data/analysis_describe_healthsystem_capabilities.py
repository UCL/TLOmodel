...
data = pd.read_csv(Path(resourcefilepath) / "ResourceFile_Daily_Capabilities.csv")
...
data["Officer_Type"] = data["Officer_Type"].replace(
    ["Lab Officer", "Lab Technician", "Lab Assistant"], "Lab Support"
)
data["Officer_Type"] = data["Officer_Type"].replace(
    ["Radiographer", "Radiography Technician"], "Radiography"
)
data["Officer_Type"] = data["Officer_Type"].replace(
    ["Nurse Officer", "Nutrition Staff", "Med. Assistant"], "Nurse"
)
data["Officer_Type"] = data["Officer_Type"].replace(
    "Nurse Midwife Technician", "MidWife"
)
data["Officer_Type"] = data["Officer_Type"].replace(
    ["Pharmacist", "Pharm Technician", "Pharm Assistant"], "Pharmacy"
)
data["Officer_Type"] = data["Officer_Type"].replace(
    ["Medical Officer / Specialist", "Clinical Officer / Technician"], "Clinician"
)
data["Officer_Type"] = data["Officer_Type"].replace(["Dental Therapist"], "Dentist")

# MINUTES PER HEALTH OFFICER TYPE BY DISTRICT:
dat = pd.DataFrame(
    data.groupby(["District", "Officer_Type"], as_index=False)[
        "Total_Minutes_Per_Day"
    ].sum()
)
tab = dat.pivot(
    index="District", columns="Officer_Type", values="Total_Minutes_Per_Day"
)
