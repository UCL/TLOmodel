...
def runsim(seed=0):
    ...
    sim = Simulation(
        start_date=start_date, seed=0, log_config=log_config, show_progress_bar=True
    )
    ...
    sim.register(
        ...
        cardio_metabolic_disorders.CardioMetabolicDisorders(
            resourcefilepath=resourcefilepath, do_condition_combos=True
        ),
...
for cond in condition_names:
    ...
    for age_cat in age_cats:
        men_GBD.append(
            comparison.loc[("2015-2019", "M", f"{age_cat}", f"{cond}")]["GBD_mean"]
        )
        men_GBD_lower.append(
            comparison.loc[("2015-2019", "M", f"{age_cat}", f"{cond}")]["GBD_lower"]
        )
        men_GBD_upper.append(
            comparison.loc[("2015-2019", "M", f"{age_cat}", f"{cond}")]["GBD_upper"]
        )
        men_model.append(
            comparison.loc[("2015-2019", "M", f"{age_cat}", f"{cond}")]["model"]
        )

    men_GBD_error = [
        ([mean - lower for mean, lower in zip(men_GBD, men_GBD_lower)]),
    ...
    for age_cat in age_cats:
        women_GBD.append(
            comparison.loc[("2015-2019", "F", f"{age_cat}", f"{cond}")]["GBD_mean"]
        )
        women_GBD_lower.append(
            comparison.loc[("2015-2019", "F", f"{age_cat}", f"{cond}")]["GBD_lower"]
        )
        women_GBD_upper.append(
            comparison.loc[("2015-2019", "F", f"{age_cat}", f"{cond}")]["GBD_upper"]
        )
        women_model.append(
            comparison.loc[("2015-2019", "F", f"{age_cat}", f"{cond}")]["model"]
        )

    women_GBD_error = [
        ([mean - lower for mean, lower in zip(women_GBD, women_GBD_lower)]),
    ...
    ax.errorbar(
        x, women_GBD, yerr=women_GBD_error, fmt="o", color="#23395d", label="GBD"
    )
...
def transform_output(x):
    return pd.concat(
        [
            pd.Series(name=x.iloc[i]["date"], data=x.iloc[i]["data"])
...
def restore_multi_index(dat):
    ...
    index_name_list = tuple(
        component.split("=")[0] for component in cols[0].split("__")
    )
...
def convert_output(output_path):
    ...
    output_path.set_index("year", drop=True, inplace=True)
    return output_path
...
for condition in conditions:
    ...
    prev_condition_age_sex = restore_multi_index(
        transform_output(
            output["tlo.methods.cardio_metabolic_disorders"][
                f"{condition}_prevalence_by_age_and_sex"
            ]
        )
    )

    # get prevalence + lower and upper values
    prev_range = pd.read_excel(
        resourcefilepath / "cmd" / "ResourceFile_cmd_condition_prevalence.xlsx",
        sheet_name=None,
    )
    baseline_error = [
        (
            prev_range[f"{condition}"]["value"].values
            - prev_range[f"{condition}"]["lower"].values
        ),
        (
            prev_range[f"{condition}"]["upper"].values
            - prev_range[f"{condition}"]["value"].values
        ),
    ]
    if "gbd_value" in prev_range[f"{condition}"]:
        gbd_error = [
            (
                prev_range[f"{condition}"]["gbd_value"].values
                - prev_range[f"{condition}"]["gbd_lower"].values
            ),
            (
                prev_range[f"{condition}"]["gbd_upper"].values
                - prev_range[f"{condition}"]["gbd_value"].values
            ),
        ]
    if "steps_value" in prev_range[f"{condition}"]:
        steps_error = [
            (
                prev_range[f"{condition}"]["steps_value"].values
                - prev_range[f"{condition}"]["steps_lower"].values
            ),
            (
                prev_range[f"{condition}"]["steps_upper"].values
                - prev_range[f"{condition}"]["steps_value"].values
            ),
        ]
    ...
    if condition == "diabetes":
        ...
        plt.legend(
            [bar, scatter, scatter_price, scatter_steps],
            ["Model", "GBD 2019", "Price et al. 2018", "STEPS Survey 2017"],
        )
        ...
        plt.title(f"Prevalence of {condition_title} by Age and Sex")
        plt.legend(
            [bar, scatter_price, scatter_steps],
            ["Model", "Price et al. 2018", "STEPS Survey 2017"],
        )
        plt.savefig(outputpath / f"prevalence_{condition_title}_by_age_sex.pdf")
        # plt.ylim([0, 1])
        plt.tight_layout()
        ...
        plt.title(f"Prevalence of {condition_title} by Age and Sex")
        plt.legend(
            [bar, scatter_price, scatter], ["Model", "Nakanga et al. 2020", "GBD 2019"]
        )
        plt.savefig(outputpath / f"prevalence_{condition_title}_by_age_sex.pdf")
        # plt.ylim([0, 1])
        plt.tight_layout()
...
for condition in conditions:
    diagnosis = output["tlo.methods.cardio_metabolic_disorders"][
        f"{condition}_diagnosis_prevalence"
    ]
    diagnosis_df[f"{condition}"] = diagnosis[f"{condition}_diagnosis_prevalence"].iloc[
        -1
    ]
diagnosis_df = diagnosis_df.transpose()
bar = plt.bar(
    diagnosis_df.index,
    diagnosis_df["diagnosis_prev"],
    alpha=0.25,
    color="b",
    label="Model",
)
...
plt.errorbar(
    ["diabetes", "hypertension"], steps_data, yerr=steps_error, fmt="o", c="#23395d"
)
...
plt.legend(
    [bar, scatter_steps, scatter_price],
    ["Model", "STEPS Survey 2017", "Price et al. 2018"],
)
plt.tight_layout()
...
age_range = sim.modules["Demography"].AGE_RANGE_CATEGORIES
prev_mm_age_all = convert_output(
    output["tlo.methods.cardio_metabolic_disorders"]["mm_prevalence_by_age_all"]
)
...
for age_grp in age_range:
    n_conditions_by_age[age_grp] = (
        last_year["counts"].apply(lambda x: x.get(f"{age_grp}")).dropna()
    )
...
plt.tight_layout()
plt.savefig(
    outputpath / ("N_comorbidities_by_age_all" + datestamp + ".pdf"), format="pdf"
)
plt.show()

# ----------------------------------------------- COMBINATIONS OF CONDITIONS ----------------------------------

prop_combos = convert_output(
    output["tlo.methods.cardio_metabolic_disorders"]["prop_combos"]
)
last_year = prop_combos.iloc[-1, :].to_frame(name="props")
props_by_age = pd.DataFrame(index=last_year.index, columns=age_range)
for age_grp in age_range:
    props_by_age[age_grp] = (
        last_year["props"].apply(lambda x: x.get(f"{age_grp}")).dropna()
    )
...
def get_incidence_rate_and_death_numbers_from_logfile(logfile):
    ...
    counts = convert_output(
        output["tlo.methods.cardio_metabolic_disorders"]["incidence_count_by_condition"]
    )
    ...
    for condition in conditions:
        ...
        py_ = output["tlo.methods.cardio_metabolic_disorders"][
            f"person_years_{condition}"
        ]
        ...
        for year in years:
            tot_py = (
                (py_.loc[pd.to_datetime(py_["date"]).dt.year == year]["M"]).apply(
                    pd.Series
                )
        ...
        for age_grp in age_range:
            ...
            condition_counts[age_grp] = (
                counts[f"{condition}"].apply(lambda x: x.get(f"{age_grp}")).dropna()
            )
            individual_condition = (
                condition_counts[age_grp]
                .apply(pd.Series)
                .div(py[age_grp], axis=0)
                .dropna()
            )
...
def get_incidence_rate_and_death_numbers_from_logfile_events(logfile, type):
    ...
    incident_counts = convert_output(
        output["tlo.methods.cardio_metabolic_disorders"][
            f"incidence_count_by_{type}_event"
        ]
    )
    ...
    for event in events:
        ...
        for age_grp in age_range:
            # extract specific condition counts from counts df
            event_counts[age_grp] = (
                incident_counts[f"{event}"]
                .apply(lambda x: x.get(f"{age_grp}"))
                .dropna()
            )
            individual_condition = (
                event_counts[age_grp].apply(pd.Series).div(py[age_grp], axis=0).dropna()
            )
...
inc_by_incident_event = get_incidence_rate_and_death_numbers_from_logfile_events(
    sim.log_filepath, type="incident"
)
inc_by_prevalent_event = get_incidence_rate_and_death_numbers_from_logfile_events(
    sim.log_filepath, type="prevalent"
)
...
def make_incidence_plot(condition, type):
    ...
    if type == "incidence":
        inc_range = pd.read_excel(
            resourcefilepath
            / "cmd"
            / f"ResourceFile_cmd_condition_and_events_{type}.xlsx",
            sheet_name=None,
        )
    else:
        inc_range = pd.read_excel(
            resourcefilepath / "cmd" / f"ResourceFile_cmd_event_{type}.xlsx",
            sheet_name=None,
        )
    asymptomatic_error = [
        (
            inc_range[f"{condition}"]["value"].values
            - inc_range[f"{condition}"]["lower"].values
        ),
        (
            inc_range[f"{condition}"]["upper"].values
            - inc_range[f"{condition}"]["value"].values
        ),
    ]
    ...
    if condition == "ever_stroke":
        if type == "incidence":
            inc_df["model_incidence"] = (
                inc_by_incident_event.loc[f"{condition}"].transpose().values
            )
        else:
            inc_df["model_incidence"] = (
                inc_by_prevalent_event.loc[f"{condition}"].transpose().values
            )
    else:
        inc_df["model_incidence"] = (
            inc_by_condition.loc[f"{condition}"].transpose().values
        )
    ...
    if condition == "ever_stroke":
        if type == "incidence":
            ...
            plt.legend([bar, scatter], ["Model", "GBD 2019"])
            plt.savefig(
                outputpath / f"incidence_{condition_title}_incident_cases_by_age.pdf"
            )
            plt.tight_layout()
            plt.show()
        else:
            ...
            plt.legend([bar, scatter], ["Model", "GBD 2019"])
            plt.savefig(
                outputpath / f"incidence_{condition_title}_prevalent_cases_by_age.pdf"
            )
            plt.tight_layout()
            plt.show()
    else:
...
conditions_and_events_for_incidence = [
    "diabetes",
