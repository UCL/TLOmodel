...
def transform_output(x):
    return pd.concat(
        [pd.Series(name=x.iloc[i]["date"], data=x.iloc[i]["data"]) for i in range(len(x))], axis=1, sort=False
    ).transpose()
...
for condition in conditions:
    ...
    prev_condition_age_sex = restore_multi_index(
        transform_output(output["tlo.methods.cardio_metabolic_disorders"][f"{condition}_prevalence_by_age_and_sex"])
    )
    ...
    baseline_error = [
        (prev_range[f"{condition}"]["value"].values - prev_range[f"{condition}"]["lower"].values),
        (prev_range[f"{condition}"]["upper"].values - prev_range[f"{condition}"]["value"].values),
    ]
    if "gbd_value" in prev_range[f"{condition}"]:
        gbd_error = [
            (prev_range[f"{condition}"]["gbd_value"].values - prev_range[f"{condition}"]["gbd_lower"].values),
            (prev_range[f"{condition}"]["gbd_upper"].values - prev_range[f"{condition}"]["gbd_value"].values),
        ]
    if "steps_value" in prev_range[f"{condition}"]:
        steps_error = [
            ...
            (prev_range[f"{condition}"]["steps_upper"].values - prev_range[f"{condition}"]["steps_value"].values),
        ]
    ...
    bar = plt.bar(prev_df.index, prev_df["model_prevalence"], bar_width, alpha=opacity, color="b", label="Model")
    ...
    if condition == "diabetes":
        ...
        plt.errorbar(
            prev_range[f"{condition}"].index, prev_range[f"{condition}"]["value"].values, yerr=0, fmt="o", c="hotpink"
        )
        plt.ylabel(f"Proportion With {condition_title}")
        plt.title(f"Prevalence of {condition_title} by Age and Sex")
...
bar = plt.bar(diagnosis_df.index, diagnosis_df["diagnosis_prev"], alpha=0.25, color="b", label="Model")
steps_data = [0.5, 0.479]  # from STEP Survey 2017
scatter_steps = plt.scatter(
    ["diabetes", "hypertension"], steps_data, s=20, alpha=1.0, color="#23395d", label="STEPS Survey 2017"
)
steps_error = [[0.447, 0.16], [0.184, 0.207]]
plt.errorbar(["diabetes", "hypertension"], steps_data, yerr=steps_error, fmt="o", c="#23395d")
price_data = [0.3846, 0.3797]  # from Price et al. 2018
scatter_price = plt.scatter(
    ["diabetes", "hypertension"], price_data, s=20, alpha=1.0, color="hotpink", label="Price et al. 2018"
)
plt.xticks(rotation=90)
...
def get_incidence_rate_and_death_numbers_from_logfile(logfile):
    ...
    for condition in conditions:
        ...
        for age_grp in age_range:
            ...
            individual_condition = condition_counts[age_grp].apply(pd.Series).div(py[age_grp], axis=0).dropna()
            individual_condition.columns = [f"{condition}"]
            if condition == conditions[0]:
                inc_rate[age_grp] = condition_counts[age_grp].apply(pd.Series).div(py[age_grp], axis=0).dropna()
...
def get_incidence_rate_and_death_numbers_from_logfile_events(logfile, type):
    ...
    for event in events:
        ...
        for age_grp in age_range:
            ...
            individual_condition = event_counts[age_grp].apply(pd.Series).div(py[age_grp], axis=0).dropna()
            individual_condition.columns = [f"{event}"]
            if event == events[0]:
                inc_rate[age_grp] = event_counts[age_grp].apply(pd.Series).div(py[age_grp], axis=0).dropna()
...
def make_incidence_plot(condition, type):
    ...
    if type == "incidence":
        inc_range = read_csv_files(
            resourcefilepath / "cmd" / f"ResourceFile_cmd_condition_and_events_{type}", files=None
        )
    else:
        inc_range = read_csv_files(resourcefilepath / "cmd" / f"ResourceFile_cmd_event_{type}", files=None)
    asymptomatic_error = [
        (inc_range[f"{condition}"]["value"].values - inc_range[f"{condition}"]["lower"].values),
    ...
    bar = plt.bar(inc_df.index, inc_df["model_incidence"], bar_width, alpha=opacity, color="b", label="Model")
    scatter = plt.scatter(
