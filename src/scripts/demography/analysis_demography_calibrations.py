...
model_2018 = (
    model_df.loc[model_df["year"] == 2018, ["male", "female"]]
    .reset_index(drop=True)
    .transpose()
    .rename(index={"male": "M", "female": "F"})
)

popsize = pd.concat(
    [cens_2018, wpp_2018, model_2018], keys=["Census_2018", "WPP", "Model"]
).unstack()
...
for year in [2018, 2030]:
    ...
    model_m = (
        model_m.loc[pd.to_datetime(model_m["date"]).dt.year == year]
        .drop(columns=["date"])
        .melt(value_name="Model", var_name="Age_Grp")
    )
    ...
    model_f = (
        model_f.loc[pd.to_datetime(model_f["date"]).dt.year == year]
        .drop(columns=["date"])
        .melt(value_name="Model", var_name="Age_Grp")
    )
    ...
    wpp_m = (
        wpp_ann.loc[wpp_ann["Sex"] == "M", ["Count", "Age_Grp"]]
        .groupby("Age_Grp")["Count"]
        .sum()
    )
    ...
    wpp_f = (
        wpp_ann.loc[wpp_ann["Sex"] == "F", ["Count", "Age_Grp"]]
        .groupby("Age_Grp")["Count"]
        .sum()
    )
    ...
    plt.savefig(
        outputpath / ("Pop_Size_" + str(year) + datestamp + ".pdf"), format="pdf"
    )
    plt.show()
...
births_model = scaled_output["tlo.methods.demography"][
    "birth_groupby_scaled"
].reset_index()
...
plt.scatter(
    x=np.arange(len(births.index))[births.index == [cens["Period"][0]]],
    y=births.loc[cens["Period"][0], "Census"],
    marker="^",
    color="orange",
)
...
# Get Model ouput
deaths_model = scaled_output["tlo.methods.demography"][
    "death_groupby_scaled"
].reset_index()
...
deaths_model = pd.DataFrame(
    deaths_model.groupby(["Period", "Sex", "Age_Grp"])["Count"].sum()
).reset_index()
...
gbd = pd.DataFrame(
    gbd.drop(columns=["Year"]).groupby(by=["Period", "Sex", "Age_Grp", "Variant"]).sum()
).reset_index()
...
ax.fill_between(
    tot_deaths.index,
    tot_deaths["WPP_Low variant"],
    tot_deaths["WPP_High variant"],
    facecolor="orange",
    alpha=0.5,
)
ax.fill_between(
    tot_deaths.index,
    tot_deaths["GBD_Lower"],
    tot_deaths["GBD_Upper"],
    facecolor="green",
    alpha=0.2,
)
plt.xticks(rotation=90)
...
tot_deaths_byage = pd.DataFrame(
    deaths.loc[deaths["Period"] == period].groupby(by=["Variant", "Age_Grp"]).sum()
).unstack()
...
ax.fill_between(
    ...
    tot_deaths_byage["GBD_Lower"],
    tot_deaths_byage["GBD_Upper"],
    facecolor="orange",
    alpha=0.2,
)
plt.xticks(rotation=90)
