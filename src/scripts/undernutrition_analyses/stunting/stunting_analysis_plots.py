...
def __process(x):
    ...
    cat = [
        _.replace("'", "").strip("( )").split(",")[1].strip() for _ in list(x.columns)
    ]
...
def __get_sim():
    ...
    _sim.register(
        ...
        healthsystem.HealthSystem(
            resourcefilepath=resourcefilepath, ignore_cons_constraints=True
        ),
...
to_plot = (
    ...
    .groupby(["age", "cat"])[years_to_compare_to_data]
    .sum()
    .groupby(level=0)
    .apply(lambda x: x / x.sum())
    .unstack()[cats_in_order]
)
for _age in range(5):
    # Model
    ...
    plt.bar(
        _age,
        to_plot.loc[_age]["-3<=HAZ<-2"],
        bottom=to_plot.loc[_age]["HAZ>=-2"],
        color="goldenrod",
    )
    plt.bar(
        _age,
        to_plot.loc[_age]["HAZ<-3"],
        bottom=(to_plot.loc[_age]["HAZ>=-2"] + to_plot.loc[_age]["-3<=HAZ<-2"]),
        color="gold",
    )

    # Data
    prop_notseverely_stunted, prop_severely_stunted = __get_proportions_stunted_by_age(
        _age
    )
...
for year, ax in zip(years_to_plot, axes):
    breakdown_this_year = (
        mean_dist_draw0[["age", "cat", year]]
        .groupby(["age", "cat"])[year]
        .sum()
        .groupby(level=0)
        .apply(lambda x: x / x.sum())
    )
...
props = (
    mean_dist_draw0.groupby(
        by=[mean_dist_draw0.age, mean_dist_draw0.cat.isin(["HAZ<-3", "-3<=HAZ<-2"])]
    )
    .sum()
    .groupby(level=0)
    .apply(lambda x: x / x.sum())
