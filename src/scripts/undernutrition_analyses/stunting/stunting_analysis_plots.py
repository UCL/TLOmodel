...
# %% Extract results
results = summarize(
    extract_results(results_folder, module="tlo.methods.stunting", key="prevalence", custom_generate_series=__process)
)
...
for _age in range(5):
    ...
    plt.bar(_age, to_plot.loc[_age]["-3<=HAZ<-2"], bottom=to_plot.loc[_age]["HAZ>=-2"], color="goldenrod")
    plt.bar(
        _age,
...
props = (
    mean_dist_draw0.groupby(by=[mean_dist_draw0.age, mean_dist_draw0.cat.isin(["HAZ<-3", "-3<=HAZ<-2"])])
    .sum()
    .groupby(level=0)
    .apply(lambda x: x / x.sum())
)
