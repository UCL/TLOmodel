...
matplotlib.use("Agg")  # non-interactive backend - never opens a GUI window,
                         # avoids any rendering/display overhead, and can't
                         # hang waiting for a display on a headless machine
...
plt.savefig(results_folder / "art_coverage_adults_with_hiv.png")
...
def plot_stacked_dalys_by_cause(
    ...
    # Generate colours for each cause (matplotlib colormap, no seaborn needed)
    cmap = plt.get_cmap(cause_palette, len(df_plot.index))
    cause_colours = dict(zip(df_plot.index, [cmap(i) for i in range(len(df_plot.index))]))
...
# Scenario-name helper lists - genuinely unrelated to funding-contraction
# analysis (just param_names minus certain scenarios), restored after
# being accidentally removed along with the contraction block, since
# they're used later (e.g. by plot_combined_cadre_and_funding).
...
dalys_ci = (
    dalys_labelled_diff_from_statusquo
    .loc[["AIDS"]]
    .reindex(draw_order, axis=1, level="draw")
)
...
################################
# save HIV DALYs and costs per draw AND run, for SMAC warm-starting
################################

def save_hiv_dalys_and_costs_by_draw_and_run(
    filepath: str = "hiv_dalys_and_costs_by_draw_and_run.csv",
) -> None:
    """
    For each (draw, run) pair, save the draw name, run number, and that
    specific run's HIV-specific DALYs, HRH (staff-time) cost, and
    consumables cost - runs within a draw are NOT averaged together,
    since each run is a separate, genuine stochastic realisation that
    downstream use (warm-starting the SMAC pipeline's PRIOR_RUNS, which
    expects one entry per (config, seed) - see
    seed_history_with_prior_runs() in optimisation_pipeline.py) needs
    kept distinct, not collapsed into a single draw-level summary in
    THIS file - a separate companion file with runs averaged per draw
    is also written, see mean_filepath below.

    HRH cost and consumables cost are stored SEPARATELY (not just their
    combined total) - reuses hrh_costs_by_year and cons_summed_by_year
    (both already indexed by year, columned by (draw, run), built
    earlier in this script for the base costing pipeline, before they
    ever get added together into cons_hrh_costs_year_draw_run), each
    summed over years here to get one total per (draw, run). No other
    aggregation happens - this is a reshape of already-computed values,
    not a new calculation.
    """
    hiv_dalys_by_run = daly_by_cause.loc['AIDS']                # Series indexed by (draw, run)
    hrh_cost_by_run = hrh_costs_by_year.sum(axis=0)             # summed over years -> (draw, run)
    cons_cost_by_run = cons_summed_by_year.sum(axis=0)          # summed over years -> (draw, run)

    out = pd.DataFrame({
        "hiv_dalys": hiv_dalys_by_run,
        "hrh_cost": hrh_cost_by_run,
        "consumables_cost": cons_cost_by_run,
        "total_costs": hrh_cost_by_run + cons_cost_by_run
    })
    out.index.names = ["draw", "run"]

    # Companion file: same structure/columns, but runs averaged (mean)
    # within each draw - produced before the per-run file below.
    mean_filepath = filepath.replace(".csv", "_mean.csv")
    out.groupby(level="draw").mean().reset_index().to_csv(mean_filepath, index=False)

    out.reset_index().to_csv(filepath, index=False)


save_hiv_dalys_and_costs_by_draw_and_run()


################################
# save run-specific difference from Status Quo, for HIV DALYs and costs
################################

def save_hiv_dalys_and_costs_diff_from_status_quo(
    filepath: str = "hiv_dalys_and_costs_diff_from_status_quo.csv",
) -> None:
    """
    For each (draw, run), compute the RUN-SPECIFIC difference (draw's
    value MINUS Status Quo's value, matched by run number) for all four
    quantities: HIV DALYs, HRH cost, consumables cost, total costs.
    Uses find_difference_relative_to_comparison_series (already used
    throughout this script) - i.e. the same "X - COMPARISON" pairing by
    run number, not a comparison of draw-level means.

    Status Quo itself is EXCLUDED from the output (drop_comparison=True,
    the function's default, matching every other call site in this
    script) - Status Quo compared to itself is trivially 0 for every
    quantity by construction, so including it as a row is meaningless
    rather than informative.

    Computed from the RAW per-run values (same source as
    save_hiv_dalys_and_costs_by_draw_and_run above), BEFORE any
    run-averaging - matching runs by number (rather than comparing
    already-averaged draw means) preserves whatever run-to-run
    correlation exists between a scenario and Status Quo (e.g. shared
    background stochasticity from common random numbers), which a
    difference-of-means would not.

    Writes two files, same raw+mean pairing pattern as elsewhere in this
    script: the raw per-(draw,run) differences, and a second with those
    differences averaged (mean) across runs within each draw.
    """
    hiv_dalys_by_run = daly_by_cause.loc['AIDS']
    hrh_cost_by_run = hrh_costs_by_year.sum(axis=0)
    cons_cost_by_run = cons_summed_by_year.sum(axis=0)
    total_cost_by_run = hrh_cost_by_run + cons_cost_by_run

    diffs = pd.DataFrame({
        "hiv_dalys_diff": find_difference_relative_to_comparison_series(
            hiv_dalys_by_run, comparison="Status Quo"),
        "hrh_cost_diff": find_difference_relative_to_comparison_series(
            hrh_cost_by_run, comparison="Status Quo"),
        "consumables_cost_diff": find_difference_relative_to_comparison_series(
            cons_cost_by_run, comparison="Status Quo"),
        "total_costs_diff": find_difference_relative_to_comparison_series(
            total_cost_by_run, comparison="Status Quo"),
    })

    # find_difference_relative_to_comparison_series unstacks draw to
    # columns then re-stacks, which appends "draw" as the LAST index
    # level - i.e. its actual output order is (run, draw), not
    # (draw, run). Swap back so "draw" genuinely labels the draw name
    # (not the run number) and matches every other output in this
    # script's (draw, run) convention.
    diffs = diffs.swaplevel(0, 1).sort_index()
    diffs.index.names = ["draw", "run"]

    # Companion file: same structure/columns, but differences averaged
    # (mean) across runs within each draw - produced before the
    # per-run file below, same pattern as elsewhere in this script.
    mean_filepath = filepath.replace(".csv", "_mean.csv")
    diffs.groupby(level="draw").mean().reset_index().to_csv(mean_filepath, index=False)

    diffs.reset_index().to_csv(filepath, index=False)


save_hiv_dalys_and_costs_diff_from_status_quo()


################################
# save Status Quo HIV DALYs and costs, per year, for SMAC input
################################

def num_dalys_by_cause_and_year(_df):
    """
    Return DALYs by cause AND by year (not summed across years, unlike
    num_dalys_by_cause above), within TARGET_PERIOD. Needed because
    every other DALYs extraction in this script collapses years into a
    single total - this is the one place year-level granularity is
    preserved, specifically for save_status_quo_hiv_dalys_and_costs_by_year().

    Returns a Series with MultiIndex (year, cause) -> DALYs, matching
    the same stack-to-MultiIndex pattern used elsewhere in this script
    (e.g. make_series_treatment_counts_by_year).
    """
    years_needed = [i.year for i in TARGET_PERIOD]
    d = _df.loc[_df.year.between(*years_needed)].drop(columns=['date', 'sex', 'age_range'])
    wide = d.groupby('year').sum()  # index=year, columns=cause
    s = wide.stack()
    s.index.names = ['year', 'cause']
    return s


def save_status_quo_hiv_dalys_and_costs_by_year(
    filepath: str = "status_quo_hiv_dalys_and_costs_by_year.csv",
) -> None:
    """
    For the Status Quo scenario specifically, save HIV-specific DALYs,
    HRH cost, and consumables cost for EACH YEAR in TARGET_PERIOD
    (not summed across years, unlike save_hiv_dalys_and_costs_by_draw_and_run
    above). Runs within Status Quo are kept separate (a 'run' column),
    for the same reason given there - each run is a distinct stochastic
    realisation, not something to silently average away in THIS file -
    a separate companion file with runs averaged per year is also
    written, see mean_filepath below.

    HRH cost and consumables cost reuse hrh_costs_by_year and
    cons_summed_by_year (already year-indexed, built earlier in this
    script). DALYs use the new num_dalys_by_cause_and_year() above, via
    a fresh extract_results() call, since no existing variable in this
    script preserves both cause AND year simultaneously.
    """
    dalys_by_cause_and_year = extract_results(
        results_folder,
        module="tlo.methods.healthburden",
        key="dalys_stacked",
        custom_generate_series=num_dalys_by_cause_and_year,
        do_scaling=True,
    ).pipe(set_param_names_as_column_index_level_0)

    hiv_dalys_by_year_run = dalys_by_cause_and_year.xs('AIDS', level='cause')['Status Quo']
    hrh_cost_by_year_run = hrh_costs_by_year['Status Quo']
    cons_cost_by_year_run = cons_summed_by_year['Status Quo']

    out = pd.DataFrame({
        "hiv_dalys": hiv_dalys_by_year_run.stack(),
        "hrh_cost": hrh_cost_by_year_run.stack(),
        "consumables_cost": cons_cost_by_year_run.stack(),
    })
    out.index.names = ["year", "run"]

    # Companion file: same structure/columns, but runs averaged (mean)
    # within each year - extending the same principle applied to
    # save_hiv_dalys_and_costs_by_draw_and_run above. Draw is already
    # fixed to Status Quo in this file, so "run" (within each "year")
    # is the analogous dimension to collapse.
    mean_filepath = filepath.replace(".csv", "_mean.csv")
    out.groupby(level="year").mean().reset_index().to_csv(mean_filepath, index=False)

    out.reset_index().to_csv(filepath, index=False)


save_status_quo_hiv_dalys_and_costs_by_year()
