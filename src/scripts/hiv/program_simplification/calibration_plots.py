...
# HIV UNAIDS data
data_hiv_unaids = pd.read_csv("resources/ResourceFile_HIV/unaids_infections_art2023.csv")
data_hiv_unaids.index = data_hiv_unaids["year"]
data_hiv_unaids = data_hiv_unaids.drop(columns=["year"])

# HIV UNAIDS data
data_hiv_unaids_deaths = pd.read_csv("resources/ResourceFile_HIV/unaids_mortality_dalys2023.csv")
data_hiv_unaids_deaths.index = data_hiv_unaids_deaths["year"]
data_hiv_unaids_deaths = data_hiv_unaids_deaths.drop(columns=["year"])

# AIDSinfo (UNAIDS)
data_hiv_aidsinfo = pd.read_csv("resources/ResourceFile_HIV/children0_14_prev_AIDSinfo.csv")
...
data_hiv_program = pd.read_csv("resources/ResourceFile_HIV/unaids_program_perf.csv")
...
data_hiv_mphia_inc = pd.read_csv("resources/ResourceFile_HIV/MPHIA_incidence2020.csv")
# 2015
...
data_hiv_mphia_prev = pd.read_csv("resources/ResourceFile_HIV/MPHIA_prevalence_art2020.csv")

# DHS HIV data
data_hiv_dhs_prev = pd.read_csv("resources/ResourceFile_HIV/DHS_prevalence.csv")

# MoH HIV testing data
data_hiv_moh_tests = pd.read_csv("resources/ResourceFile_HIV/MoH_numbers_tests.csv")
...
data_hiv_moh_art = pd.read_csv("resources/ResourceFile_HIV/MoH_number_art.csv")
...
# HIV Incidence in adults aged 15-49 per 1000 population
make_plot(
    title_str="Incidence per 1000 Adults Aged 15-49",
    model=model_hiv_adult_inc["median"] * 1000,
    model_low=model_hiv_adult_inc["lower"] * 1000,
    model_high=model_hiv_adult_inc["upper"] * 1000,
    data_name="UNAIDS",
    ...
    xlab="Year",
    ylab="Incidence per 1000 population"
)

# MPHIA
plt.errorbar(
    model_hiv_adult_inc.index[6],
    data_hiv_mphia_inc_estimate[0] * 10,
    yerr=[[data_hiv_mphia_inc_yerr[0][0] * 10], [data_hiv_mphia_inc_yerr[1][0] * 10]],
    fmt="gx",
)
plt.errorbar(
    2020,
    data_hiv_mphia_inc_estimate20[0] * 10,
    yerr=[[data_hiv_mphia_inc_yerr20[0][0] * 10], [data_hiv_mphia_inc_yerr20[1][0] * 10]],
    fmt="gx",
)
plt.ylim(0, 20.0)
plt.xlim(2010, 2025)
...
plt.savefig(make_graph_file_name("Incidence_per_1000_Adults_15_49"))
...
ax.plot(data_hiv_unaids_deaths.index[2:16],
        aids_deaths["median_aids_deaths_rate_100kpy"][2:16], "-", color=blue)
ax.fill_between(data_hiv_unaids_deaths.index[2:16],
                aids_deaths["lower_aids_deaths_rate_100kpy"][2:16],
                aids_deaths["upper_aids_deaths_rate_100kpy"][2:16], color=blue, alpha=0.2)
...
death_compare = compare_number_of_deaths(
    "outputs/hiv_program_simplification-2026-01-06T151634Z/0/0/hiv_program_simplification__2026-01-06T151634.log",
    resourcefilepath)
