...
#--------------------------DALYs--------------------------------------------#
# Initialize YLL and YLD dictionaries
yll = {'active': 0, 'ds': 0, 'mdr': 0}
yll_output = output['tlo.methods.healthburden']['yll_by_causes_of_death_stacked']
yll_2010_df = yll_output.loc[yll_output['year'] == 2010]
yll= yll_2010_df['tb'].sum()

yld = {'active': 0, 'ds': 0, 'mdr': 0}
yld_output = output['tlo.methods.healthburden']['yld_by_causes_of_disability']
yld_2010_df = yld_output.loc[yld_output['year'] == 2010]
yld = yld_2010_df['tb'].sum()

# Sum total YLL and YLD for each cause
for cause in ['active', 'ds', 'mdr']:
    if cause in yll_2010_df.columns:
        yll[cause] += yll_2010_df[f'{cause}'].sum()
    yld[cause] += yld_2010_df[f'{cause}_yld'].sum()

# Calculate total DALYs for each cause
expected_dalys = {}
for cause in ['active', 'ds', 'mdr']:
    expected_dalys[cause] = yld[cause] + yll[cause]
    print(f'expected {cause} dalys:', expected_dalys[cause])

# Calculate actual DALYs from output dataframe
dalys_output = output['tlo.methods.healthburden']['dalys_stacked']
dalys_2010 = dalys_output.loc[dalys_output['year'] == 2010]
actual_dalys = dalys_2010.groupby('cause').sum()['value'].to_dict()
print('actual tb dalys', actual_dalys)

# Plot YLD, YLL, and DALYs for each cause
make_plot(
    title_str="Trends in DALYs, YLD and YLL",
    Dalytrend=[expected_dalys['active'], expected_dalys['ds'], expected_dalys['mdr']],
    YrsLD=[yld['active'], yld['ds'], yld['mdr']],
    YrsLL=[yll['active'], yll['ds'], yll['mdr']]
)
plt.show()
