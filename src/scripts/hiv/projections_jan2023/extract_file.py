...
# Print dictionary keys for the TB module
...
# Computing person years to use as denominators for mortality rate and incidence
def get_person_years(_df):
    """Extract person-years for each draw/run.
    Sums across men and women.
    Will skip column if a particular run has failed.
    """
...
def summarize_tb_deaths(results_folder, person_years):
