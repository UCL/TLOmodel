...
with open(outputpath / "default_run0509.pickle", "rb") as f:
    output = pickle.load(f)
...
#cons_available = output['tlo.methods.healthsystem.summary'].keys()
HSE_sample = output["tlo.methods.healthsystem.summary"]["hsi_event_details"]
HSExxx = HSE_sample.set_index("date")
...
# #sample_dalys= output['tlo.methods.healthburden']['dalys_stacked'].groupby(['cause', 'sex']).size()
# sample_dalys= output['tlo.methods.healthburden']['dalys_stacked'].drop(columns=[])
# sample_dalys.to_excel(outputpath / "sample_dalys_CXR_scale_up2.xlsx")
