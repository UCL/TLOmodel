...
results_folder = get_scenario_outputs("calibration_hiv.py", outputspath)[-1]
...
extracted.to_csv(outputspath / ("hiv_calibration_outputs" + ".csv"))
...
combined_output.to_csv(outputspath / ("hiv_calibration_outputs_summary" + ".csv"))


# plot the deviance against parameters
fig, ax = plt.subplots()
plt.plot(combined_output["Hiv:beta"], combined_output["deviance"], 'o')
ax.set_title("Deviance measure")
plt.xlabel("Hiv: transmission rate")
plt.ylabel("Deviance measure")
plt.show()
