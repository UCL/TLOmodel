...
# load the results
with open(outputpath / "scenario0.pickle", "rb") as f:
    scenario0 = pickle.load(f)

with open(outputpath / "scenario1.pickle", "rb") as f:
    scenario1 = pickle.load(f)

with open(outputpath / "scenario2.pickle", "rb") as f:
    scenario2 = pickle.load(f)

with open(outputpath / "scenario3.pickle", "rb") as f:
    scenario3 = pickle.load(f)

with open(outputpath / "scenario4.pickle", "rb") as f:
    scenario4 = pickle.load(f)


# baseline person-years all ages (irrespective of HIV status)
py_ = scenario0["tlo.methods.demography"]["person_years"]
...
TB_inc = scenario0["tlo.methods.tb"]["tb_incidence"]
years = pd.to_datetime(TB_inc["date"]).dt.year
TB_inc.index = pd.to_datetime(years, format="%Y")
activeTB_inc_rate0 = (TB_inc["num_new_active_tb"] / py) * 100000

TB_inc = scenario1["tlo.methods.tb"]["tb_incidence"]
years = pd.to_datetime(TB_inc["date"]).dt.year
TB_inc.index = pd.to_datetime(years, format="%Y")
activeTB_inc_rate1 = (TB_inc["num_new_active_tb"] / py) * 100000

TB_inc = scenario2["tlo.methods.tb"]["tb_incidence"]
years = pd.to_datetime(TB_inc["date"]).dt.year
TB_inc.index = pd.to_datetime(years, format="%Y")
activeTB_inc_rate2 = (TB_inc["num_new_active_tb"] / py) * 100000

TB_inc = scenario3["tlo.methods.tb"]["tb_incidence"]
years = pd.to_datetime(TB_inc["date"]).dt.year
TB_inc.index = pd.to_datetime(years, format="%Y")
activeTB_inc_rate3 = (TB_inc["num_new_active_tb"] / py) * 100000

TB_inc = scenario4["tlo.methods.tb"]["tb_incidence"]
years = pd.to_datetime(TB_inc["date"]).dt.year
TB_inc.index = pd.to_datetime(years, format="%Y")
activeTB_inc_rate4 = (TB_inc["num_new_active_tb"] / py) * 100000

make_plot(
    title_str="Active TB Incidence (per 100k person-years)",
    model=activeTB_inc_rate0,
...
plt.plot(activeTB_inc_rate1)
plt.plot(activeTB_inc_rate2)
plt.plot(activeTB_inc_rate3)
plt.plot(activeTB_inc_rate4)
...
# latest_path = "outputs\deviance__2022-01-20T105927.log"
...
# ---------------------------------------------------------------------- #
ipt0 = scenario0["tlo.methods.tb"]["tb_treatment"]["tbIptCoverage"] * 100
ipt1 = scenario1["tlo.methods.tb"]["tb_treatment"]["tbIptCoverage"] * 100
ipt2 = scenario2["tlo.methods.tb"]["tb_treatment"]["tbIptCoverage"] * 100
ipt3 = scenario3["tlo.methods.tb"]["tb_treatment"]["tbIptCoverage"] * 100
ipt4 = scenario4["tlo.methods.tb"]["tb_treatment"]["tbIptCoverage"] * 100

# TB IPT coverage
make_plot(
    title_str="Percentage of population starting IPT",
    model=ipt0,
)
plt.plot(ipt1)
plt.plot(ipt2)
