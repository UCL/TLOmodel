...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    comparison = compare_number_of_deaths(logfile=results_folder, resourcefilepath=resourcefilepath)
    ...
    for cond in condition_names:
        ...
        conditions = cardio_metabolic_disorders.conditions
        events = cardio_metabolic_disorders.events
        age_range = demography.AGE_RANGE_CATEGORIES
...
# ----------------------------------------------- CREATE PREVALENCE PLOTS ----------------------------------------------
        ...
        for condition in conditions:
            ...
            if condition == 'diabetes':
                ...
                plt.legend([bar, scatter, scatter_price, scatter_steps], ['Model', 'GBD 2019', 'Price et al. 2018',
                                                                          'STEPS Survey 2017'])
                plt.savefig(output_folder / f'prevalence_{condition_title}_by_age_sex.pdf')
                # plt.ylim([0, 1])
                plt.tight_layout()
                plt.show()
        ...
        plt.tight_layout()
        plt.savefig(output_folder / 'diagnosis_by_condition.pdf')
        plt.show()
        ...
        plt.tight_layout()
        plt.savefig(output_folder / ("N_comorbidities_by_age_all" + datestamp + ".pdf"), format='pdf')
        plt.show()
        ...
        inc_by_condition = get_incidence_rate_and_death_numbers_from_logfile(output_folder)
        inc_by_incident_event = get_incidence_rate_and_death_numbers_from_logfile_events(output_folder, type='incident')
        inc_by_prevalent_event = get_incidence_rate_and_death_numbers_from_logfile_events(output_folder, type='prevalent')
        ...
        def make_incidence_plot(condition, type):
            ...
            if condition == 'ever_stroke':
                if type == 'incidence':
                    ...
                    plt.legend([bar, scatter], ['Model', 'GBD 2019'])
                    plt.savefig(output_folder / f'incidence_{condition_title}_incident_cases_by_age.pdf')
                    plt.tight_layout()
                    plt.show()
                else:
                    ...
                    plt.legend([bar, scatter], ['Model', 'GBD 2019'])
                    plt.savefig(output_folder / f'incidence_{condition_title}_prevalent_cases_by_age.pdf')
                    plt.tight_layout()
                    plt.show()
            else:
                ...
                plt.legend([bar, scatter], ['Model', 'GBD 2019'])
                plt.savefig(output_folder / f'incidence_{condition_title}_by_age.pdf')
                plt.tight_layout()
                plt.show()
