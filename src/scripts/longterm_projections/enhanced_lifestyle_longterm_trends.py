...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    fig, ax = plt.subplots(figsize=(12, 8))  # Single panel

    # Plot YLDs by cause
    df_yld_all_draws_mean_1000.T.plot.bar(stacked=True,label = [label for label in df_all_years_yld_mean.index])

    ax.set_xlabel('Year')
    ax.set_ylabel('Total YLDs per 1,000')
    ax.legend(title='Cause', bbox_to_anchor=(1., 1), loc='upper left')

    fig.savefig(make_graph_file_name('Trend_YLDs_by_condition_All_Years'))
    plt.close(fig)
