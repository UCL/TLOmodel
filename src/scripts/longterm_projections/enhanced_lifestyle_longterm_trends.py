...
class LifeStylePlots:
    ...
    def plot_categorical_properties_by_gender(self, li_property: str):
        ...
        fig.tight_layout()
        plt.savefig(self.outputpath / (li_property + self.datestamp + '_' + str(year) + '.png'), format='png')
    ...
    def plot_categorical_properties_by_age_group(self, li_property: str):
        ...
        # select logs from the latest year. In this case we are selecting year year
        all_logs_df = self.dfs[li_property]
        mask = (all_logs_df.index > pd.to_datetime(min_year)) & (all_logs_df.index <= pd.to_datetime(max_year))
        ...
        for _key in df_dict.keys():
            ...
            df_dict[_key].plot(kind='bar', stacked=True,
                               ...
                               title=f"{_key.split('_')[0]} {self.en_props[li_property].label}"
                                     f' categories by age group in {year}' if li_property in
    ...
    def plot_non_categorical_properties_by_age_group(self, li_property):
        ...
        for _key in df_dict.keys():
            ...
            df_dict[_key].plot(kind='bar', stacked=True,
                               ...
                               legend=None,
                               color='darkturquoise',
                               title=f"{self.en_props[li_property].label} by age group in {year}, {_key.split('_')[0]}"
                                     if li_property in self.cat_by_rural_urban_props or li_property == 'li_in_ed' else
                               f"{self.en_props[li_property].label} by age group in {year}",
                               ylabel=f"{self.en_props[li_property].label} proportions", xlabel="Year"
                               )
    ...
    def male_circumcision_and_sex_workers_plot(self, _property: str = None):
        ...
        plt.tight_layout()
        plt.savefig(self.outputpath / (_property + self.datestamp + '_' + str(year) + '.png'), format='png')
...
# read the results
output = parse_log_file("/Users/rem76/PycharmProjects/TLOmodel/outputs/rm916@ic.ac.uk/longterm_trends_all_diseases-2025-01-27T125329Z/3/0/longterm_trends_all_diseases__2025-01-27T125628.log")
...
g_plots = LifeStylePlots(logs=logs_df, path="/Users/rem76/PycharmProjects/TLOmodel/outputs/rm916@ic.ac.uk/longterm_trends_all_diseases-2025-01-27T125329Z/3/0")
