...
lmis_df_wide_flat["consumable_reported_horiz"] = lmis_df_wide_flat["closing_bal"].count(
    axis=1
)  # generate a column which reports the number of entries of closing balance
...
consumables_df = pd.read_csv(
    resourcefilepath / "ResourceFile_consumables_matched.csv", low_memory=False
)
...
hhfa_df = pd.read_excel(
    open(resourcefilepath / "ResourceFile_hhfa_consumables.xlsx", "rb"),
...
unmatched_consumables_df["stkout_prop"] = (
    1 - unmatched_consumables_df["available_prop_hhfa"]
)
...
hhfa_calibration_df = pd.wide_to_long(
    hhfa_calibration_df.dropna(),
...
lmis_calibration_df = (
    stkout_df[~cond1]
    .groupby(["module_name", "fac_type_tlo", "item_code"])
    .mean()
    .reset_index()
)
...
calibration_df = pd.merge(
    ...
    hhfa_calibration_df,
    how="inner",
    on=["item_code", "fac_type_tlo"],
)
calibration_df["difference"] = (
    calibration_df["available_prop_hhfa"] - calibration_df["available_prop"]
)

# --- 8.2 Compare OpenLMIS estimates with HHFA estimates --- ##
# Summary results by level of care
calibration_df.groupby(["fac_type_tlo"])[
...
ax = calibration_df[cond].plot.line(
    x="labels", y=["available_prop", "available_prop_hhfa"]
)
