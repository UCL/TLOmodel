...
plt.figure()
plt.plot(
    year_by_year.index.to_numpy(), year_by_year.to_numpy() / year_by_year[2017], marker="o", label="Historical data"
)
...
for xlim in (datetime.date(2025, 1, 1), datetime.date(2031, 1, 1)):
    ...
    ax.plot(  # the data
        to_plot["mid-year_date"], to_plot["Data"], marker="o", linestyle="--", label="Data"
    )
