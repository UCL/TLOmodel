...
def apply(
    ...
    fig, ax = do_bar_plot_with_ci(
        num_deaths_summarized / 1e6, xticklabels_horizontal_and_wrapped=True, put_labels_in_legend=False
    )
    ...
    fig, ax = do_bar_plot_with_ci(
        num_dalys_summarized / 1e6, xticklabels_horizontal_and_wrapped=True, put_labels_in_legend=False
    )
    ax.set_title(name_of_plot)
    ax.set_ylabel("(Millions)")
    ...
    num_deaths_averted = (
        summarize(
            -1.0
            * pd.DataFrame(
                find_difference_relative_to_comparison_series(num_deaths.loc[0], comparison=counterfactual_scenario)
            ).T
        )
        .iloc[0]
        .unstack()
        .reindex(param_names)
        .drop([counterfactual_scenario])
    )
    ...
    pc_deaths_averted = 100.0 * summarize(
        ...
        * pd.DataFrame(
            find_difference_relative_to_comparison_series(
                num_deaths.loc[0], comparison=counterfactual_scenario, scaled=True
            )
        ).T
    ...
    num_dalys_averted = (
        summarize(
            -1.0
            * pd.DataFrame(
                find_difference_relative_to_comparison_series(num_dalys.loc[0], comparison=counterfactual_scenario)
            ).T
        )
        .iloc[0]
        .unstack()
    ...
    pc_dalys_averted = 100.0 * summarize(
        ...
        * pd.DataFrame(
            find_difference_relative_to_comparison_series(
                num_dalys.loc[0], comparison=counterfactual_scenario, scaled=True
            )
        ).T
    ).iloc[0].unstack().reindex(param_names).drop([counterfactual_scenario])
    ...
    make_string_number = (
        lambda row: f"{round(row['mean']/1e6,1)} ({round(row['lower']/1e6, 1)}-{round(row['upper']/1e6, 1)}) Million"
    )  # noqa: E731
    ...
    total_num_dalys_by_label_results_averted_vs_baseline_grouping_htm = (
        total_num_dalys_by_label_results_averted_vs_baseline.groupby(
            total_num_dalys_by_label_results_averted_vs_baseline.index == "Other"
        )
        .sum()
        .rename(index={False: "H/T/M", True: "Other"})
    )
    make_daly_split_by_cause_graph(
        total_num_dalys_by_label_results_averted_vs_baseline_grouping_htm, filename_suffix="_broad"
    )

    # percent of DALYS averted in H/T/M
    pc_dalys_averted_in_h_t_m = (
        100
        * (
            total_num_dalys_by_label_results_averted_vs_baseline
            / total_num_dalys_by_label_results_averted_vs_baseline.sum()
        )
    ).round(0)
    ...
    def plot_table(df, name_of_plot):
        ...
        ax.axis("off")
        pd.plotting.table(ax, df, loc="center", cellLoc="center", colWidths=list([0.2, 0.2]))
        ax.set_title(name_of_plot)
        fig.tight_layout()
        fig.savefig(make_graph_file_name(name_of_plot.replace(" ", "_").replace(",", "")))
        fig.show()
    ...
    pc_dalys_averted_in_htm = 1.0 - (
        total_num_dalys_by_label_results_averted_vs_baseline.loc["Other"]
        / total_num_dalys_by_label_results_averted_vs_baseline.sum()
    )
