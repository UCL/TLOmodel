...
MAP_comm = pd.read_excel(
    Path(resourcefilepath) / "ResourceFile_malaria.xlsx",
    sheet_name="MAP_CommoditiesData2023",
)
...
MAP_rdt_yield = (
    MAP_comm.Tested_Positive_Public / MAP_comm.RDT_Consumption_Public
) * 100
WHO_rdt_yield = (
    WHO_comm.NumPositiveCasesTestedByRDT / WHO_comm.NumSuspectedCasesTestedByRDT
) * 100
# model rdt yield
...
plt.plot(
    incMAP_data.Year, incMAP_data.IncidenceRatePer1000, color="crimson"
)  # MAP data
plt.plot(WHO_data.Year, WHO_data.IncidencePer1000, color="darkorchid")  # WHO data
plt.fill_between(
    WHO_data.Year,
    WHO_data.IncidencePer1000Low,
    WHO_data.IncidencePer1000High,
    color="darkorchid",
    alpha=0.1,
)
plt.plot(
...
plt.plot(
    WHO_comm.Year, WHO_comm.NumSuspectedCasesTestedByRDT, color="darkorchid"
)  # WHO data
