...
extracted = extract_results(
    results_folder, module="tlo.methods.mockitis", key="summary", column="PropInf", index="date"
)
...
fig, ax = plt.subplots()
ax.bar(x=xvals, height=propinf_end.loc[:, (slice(None), "mean")].iloc[0].values, yerr=yerr)
ax.set_xticks(xvals)
