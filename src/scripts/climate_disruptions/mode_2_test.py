...
scenario_names = scenarios_of_interest
suffix = "mode_2_test"
scenario_colours = [
    "#823038",  # Baseline
    "#00566f",  # High
...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    def get_num_treatments(_df):
        return sum_event_counts(_df, "TREATMENT_ID")
    ...
    all_draws_non_blank_mean = []
    all_draws_non_blank_lower = []
    all_draws_non_blank_upper = []
    ...
    for draw in range(len(scenario_names)):
        ...
        make_graph_file_name = lambda stub: output_folder / f"_{stub}_{draw}.png"
        ...
        print(f"  Extracting non-blank HSIs...")
        num_non_blank_appts = summarize(
            extract_results(
                results_folder,
                module="tlo.methods.healthsystem.summary",
        ...
        if scenario_names[draw] == 0:
            ...
            baseline_non_blank_total = num_non_blank_appts["mean"].values[0]
        ...
        all_draws_non_blank_mean.append(num_non_blank_appts["mean"])
        all_draws_non_blank_lower.append(num_non_blank_appts["lower"])
        all_draws_non_blank_upper.append(num_non_blank_appts["upper"])
        ...
        pd.DataFrame({
            ...
            "non_blank_mean": num_non_blank_appts["mean"].values[0],
    ...
    non_blank_mean_values = series_to_value(all_draws_non_blank_mean)
    non_blank_lower_values = series_to_value(all_draws_non_blank_lower)
    non_blank_upper_values = series_to_value(all_draws_non_blank_upper)
    ...
    summary_df = pd.DataFrame({
        ...
        'non_blank_mean': non_blank_mean_values,
        'non_blank_lower': non_blank_lower_values,
        'non_blank_upper': non_blank_upper_values,
    ...
    }, index=[f"Draw {i}" for i, draw in enumerate(scenarios_of_interest)])
    ...
    non_blank_err = np.array([
        non_blank_mean_values - non_blank_lower_values,
        non_blank_upper_values - non_blank_mean_values
    ])
    ...
    if baseline_treatments_total is not None:
        ...
        diff_df = pd.DataFrame({
            ...
            'non_blank_diff': non_blank_mean_values - baseline_non_blank_total,
            'non_blank_pct_change': ((non_blank_mean_values - baseline_non_blank_total) /
                                     baseline_non_blank_total * 100),
        })
