...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path = None):
    ...
    def get_num_deaths_by_district(_df):
        ...
        return _df.loc[pd.to_datetime(_df.date).between(*TARGET_PERIOD)].groupby(_df["district_of_residence"]).size()
    ...
    for draw in range(len(scenario_names_all)):
        ...
        axes[1].legend(title="District", bbox_to_anchor=(1.0, 1), loc="upper left")
        axes[1].grid(False)
        ...
        axes[1].legend(title="Condition (by district)", bbox_to_anchor=(1.05, 1), loc="upper left", fontsize=8, ncol=2)
        axes[1].grid(False)
    ...
    deaths_upper_per_1000 - deaths_means_per_1000
    deaths_means_per_1000 - deaths_lower_per_1000

    dalys_upper_per_1000 - dalys_means_per_1000
    dalys_means_per_1000 - dalys_lower_per_1000
    ...
    df_dalys_by_district_all_scenarios_per_1000.to_csv(
        output_folder / "deaths_and_dalys_by_district_scenario.csv", index=False
    )
    fig.tight_layout()
    ...
    for i, scenario in enumerate(scenario_names[1:], start=1):
        ...
        difference_from_baseline_per_1000 = (
            ...
            - df_dalys_by_district_all_scenarios_per_1000["Baseline"]
        )
