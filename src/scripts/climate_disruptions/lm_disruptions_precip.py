...
max_year_for_analysis_baseline = 1940 + len(year_range) + 1
...
covid_months = range((2020 - min_year_for_analysis) * 12 + 4, (
    2020 - min_year_for_analysis) * 12 + 4 + 20)
cyclone_freddy_months_phalombe = range((2023 - min_year_for_analysis) * 12 + 4, (
    2020 - min_year_for_analysis) * 12 + 4 + 14)
cyclone_freddy_months_thumbwe = range((2023 - min_year_for_analysis) * 12 + 3, (
    2020 - min_year_for_analysis) * 12 + 3 + 12)
...
if use_all_weather:
    ...
    weather_data_five_day_cumulative_clean = weather_data_five_day_cumulative_original.drop(
    ...
    weather_data_five_day_cumulative_df = weather_data_five_day_cumulative_df.drop(
        weather_data_five_day_cumulative_df.index[-1:])
    ...
    weather_data_monthly = weather_data_monthly_df
    ...
    weather_data_five_day_cumulative = weather_data_five_day_cumulative.iloc[
...
monthly_reporting_by_facility = monthly_reporting_by_facility[common_columns]
...
year_flattened = year * len(monthly_reporting_by_facility.columns)
...
altitude = [float(x) for x in
            repeat_info(expanded_facility_info['A109__Altitude'], num_facilities, year_range, historical=True)]
minimum_distance = [float(x) for x in
                    repeat_info(expanded_facility_info['minimum_distance'], num_facilities, year_range,
                                historical=True)]
...
minimum_distance = np.nan_to_num(minimum_distance, nan=np.nan, posinf=np.nan, neginf=np.nan)
...
coefficient_names = ["year", "month", "altitude", "minimum_distance"] + list(resid_encoded.columns) + list(
    zone_encoded.columns) + list(owner_encoded.columns)
...
axs[0].scatter(data_ANC_predictions['Year_Month'], data_ANC_predictions['y_filtered'], color='#1C6E8C', alpha=0.5,
               label='Actual data')
axs[0].scatter(data_ANC_predictions['Year_Month'], data_ANC_predictions['y_pred'], color='#9AC4F8', alpha=0.7,
               label='Predictions')
axs[0].scatter(data_ANC_predictions_grouped['Year_Month'], data_ANC_predictions_grouped['y_filtered'], color='red',
               alpha=0.5, label='Mean Actual data')
axs[0].scatter(data_ANC_predictions_grouped['Year_Month'], data_ANC_predictions_grouped['y_pred'], color='yellow',
               alpha=0.7, label='Mean Predictions')
...
axs[1].scatter(data_ANC_predictions['Year_Month'],
               (data_ANC_predictions['y_filtered'] - data_ANC_predictions['y_pred']), color='#9AC4F8', alpha=0.7,
               label='Residuals')
...
included_weather, results_of_weather_model, y_pred_weather, mask_all_data = stepwise_selection(
    X_weather_standardized, y, poisson=poisson, log_y=log_y)

coefficient_names_weather = ["precip_monthly_total", "precip_5_day_max", "precip_monthly_total_2",
                             "precip_5_day_max_2", "precip_monthly_total_3", "precip_5_day_max_3",
                             "5_day_monthly", "year", "month",
...
axs[1].scatter(monthly_weather_predictions, np.exp(matched_y_pred_weather) - np.exp(matched_y_pred), color='red',
               alpha=0.5, label='Residuals')
...
data_weather_predictions_grouped[['Year', 'Month']] = data_weather_predictions_grouped['Year_Month'].str.split(
    '-', expand=True).astype(int)
data_weather_predictions_grouped = data_weather_predictions_grouped.sort_values(
...
ax.scatter(data_weather_predictions_grouped['Year_Month'],
           data_weather_predictions_grouped['difference'], color='#823038', alpha=0.7)
...
ax.stem(data_weather_predictions_grouped['Year_Month'][positive_mask],
        ...
        label="More appointments projected due to less precipitation")
ax.stem(data_weather_predictions_grouped['Year_Month'][negative_mask],
        ...
        label="Fewer appointments projected due to more precipitation")

ax.set_xlabel('Year-Month')
ax.set_ylabel(f'Difference in Predicted {service} Services (Without vs. With Precipitation)')
january_ticks = data_weather_predictions_grouped[
    data_weather_predictions_grouped['Year_Month'].str.endswith('-1')]
...
############## LR #########################################
log_likelihood_null = results.llf
log_likelihood_full = results_of_weather_model.llf
...
############## SAVE MODELS #########################################
SAVE_DIR = "/Users/rem76/Desktop/Climate_Change_Health/Data/saved_models"
os.makedirs(SAVE_DIR, exist_ok=True)

model_bundle = {
    "results": results,
    "results_of_weather_model": results_of_weather_model,
    "included": included,
    "included_weather": included_weather,
    "scaler": scaler,
    "resid_columns": list(resid_encoded.columns),
    "zone_columns": list(zone_encoded.columns),
    "owner_columns": list(owner_encoded.columns),
    "mean_altitude_train": mean_altitude,
    "service": service,
}

save_path = os.path.join(SAVE_DIR, f"model_bundle_{service}.pkl")
...
expanded_facility_info_all = pd.read_csv(
    f"{data_path}expanded_facility_info_by_all_facilities_lm.csv",
    index_col=0
)
# Transpose so facilities are rows
expanded_facility_info_all = expanded_facility_info_all.T

# Define facility level mapping
facility_levels_types = {
    "level_0": ["Health Post", "Village Health Committee", "Community Health Station",
                "Village Clinic", "Mobile Clinic", "Outreach Clinic"],
    "level_1a": ["Dispensary", "Rural Health Centre", "Urban Health Centre",
                 "Private Clinic", "Special Clinic", "Antenatal Clinic",
                 "Maternity Clinic", "Maternity Facility",
                 "Health Centre", "Clinic", "Maternity"],
    "level_1b": ["Community Hospital", "Rural Hospital", "CHAM Hospital",
                 "Rural/Community Hospital"],
    "level_2": ["District Hospital", "District Health Office", "Other Hospital"],
    "level_3": ["Kamuzu Central Hospital", "Mzuzu Central Hospital",
                "Zomba Central Hospital", "Queen Elizabeth Central Hospital",
                "Central Hospital"],
    "level_4": ["Zomba Mental Hospital"],
}
ftype_to_level = {ft: lvl for lvl, ftypes in facility_levels_types.items() for ft in ftypes}
...
for ssp_scenario in ssp_scenarios:
    for model_type in model_types:
        ...
        weather_data_prediction_flatten = np.vstack(
            (weather_data_prediction_monthly_flattened,
             weather_data_prediction_five_day_cumulative_flattened)).T
        ...
        ftype_info_prediction = repeat_info(expanded_facility_info_pred['Ftype'], num_facilities,
                                            year_range_prediction, historical=False)
        altitude_prediction = [float(x) for x in
                               repeat_info(expanded_facility_info_pred['A109__Altitude'],
                                           num_facilities, year_range_prediction, historical=False)]
        minimum_distance_prediction = [float(x) for x in
                                       repeat_info(expanded_facility_info_pred['minimum_distance'],
                                                   num_facilities, year_range_prediction, historical=False)]
        ...
        altitude_prediction = np.nan_to_num(altitude_prediction, nan=mean_altitude_prediction,
                                            posinf=mean_altitude_prediction, neginf=mean_altitude_prediction)
        altitude_prediction = list(altitude_prediction)

        minimum_distance_prediction = np.nan_to_num(minimum_distance_prediction, nan=np.nan,
        ...
        X_continuous_weather = np.column_stack([
            ...
            weather_data_prediction_flatten[:, 0] ** 3,
            weather_data_prediction_flatten[:, 1] ** 3,
        ...
        owner_encoded_prediction = align_dummies(owner_encoded_prediction, list(owner_encoded.columns))
        ...
        X_basis_weather = np.column_stack([X_continuous_weather, X_categorical_weather])
        X_basis_weather_filtered = X_basis_weather[X_basis_weather[:, 0] > mask_threshold]
        X_basis_weather_filtered = X_basis_weather_filtered[:, included_weather]

        year_month_labels = np.array(
            [f"{y}-{m}" for y, m in zip(X_basis_weather_filtered[:, 2], X_basis_weather[:, 3])])
        ...
        X_bases_ANC_standardized = X_bases_ANC_standardized[:, included]
        ...
        district_monthly_totals = full_data_weather_predictions.groupby(
            ['District', 'Year', 'Month']).agg({
        ...
        district_monthly_totals.to_csv(
            f"{data_path}district_monthly_totals_{ssp_scenario}_{model_type}_{service}.csv", index=False)

        district_level_monthly_totals = full_data_weather_predictions.groupby(
            ['District', 'Level', 'Year', 'Month']).agg({
            'Predicted_Weather_Model': 'sum',
            'Predicted_No_Weather_Model': 'sum',
            'Difference_in_Expectation': 'sum',
            'Precipitation': 'mean',
        ...
        district_level_monthly_totals.to_csv(
            f"{data_path}district_level_monthly_totals_{ssp_scenario}_{model_type}_{service}.csv", index=False)

        pd.DataFrame(X_basis_weather_filtered).to_csv(
            f'{data_path}X_basis_weather_filtered_predictions_{ssp_scenario}_{model_type}_{service}.csv',
            index=False)
