...
def _read_log(results_folder: Path, draw: int, run: int,
    ...
    for suffix in [".pickle", ".pkl"]:
        # Try dedicated key file first
        fpath = run_folder / f"{module}__{log_key}{suffix}"
        if fpath.exists():
            with open(fpath, "rb") as f:
                return pickle.load(f)
        # Try combined module file
...
def _disrupted_persons_by_district(
    ...
    # Union across both logs: drop duplicate (Person_ID, RealFacility_ID) pairs
    # so the same person at the same facility in both logs counts once.
    combined = pd.concat(frames, ignore_index=True).drop_duplicates()
    combined["district"] = combined["RealFacility_ID"].map(fac_to_district)
    combined = combined.dropna(subset=["district"])

    # A person who attended facilities in two different districts counts in each.
    # Within a district, each person is counted once regardless of how many
    # disruptions they experienced there.
...
def apply(results_folder: Path, output_folder: Path, resourcefilepath: Path):
    ...
    for draw in scenarios_of_interest:
        ...
        # For each run: compute mean-across-years proportion per district,
        # plus total disrupted persons and total population across the country.
        run_mean_proportions = []
        run_country_disrupted = []  # mean annual disrupted persons, whole country
        run_country_population = []  # mean annual population, whole country
        ...
        for run in range(n_runs):
            ...
            # Mean annual totals across years for this run
            ...
            print(f"    Run {run}: country mean proportion = {run_mean.mean() * 100:.3f}%  |  "
                  f"mean annual disrupted = {mean_disrupted:,.0f}  |  "
                  f"mean annual population = {mean_population:,.0f}")
        ...
        all_scenario_results[scen] = {
            ...
            # Country-level absolute numbers (mean annual, across runs)
            ...
            # Country-level proportion (computed from totals, not mean of district means)
    ...
    for scen, result in all_scenario_results.items():
        ...
        country_rows.append({
            "Scenario": scen,
            "mean_annual_disrupted_mean": round(result["country_disrupted_mean"], 1),
            "mean_annual_disrupted_lower": round(result["country_disrupted_lower"], 1),
            "mean_annual_disrupted_upper": round(result["country_disrupted_upper"], 1),
            "mean_annual_population": round(result["country_population_mean"], 1),
    ...
    for row in country_rows:
        print(
            f"  {row['Scenario']}: "
            f"{row['mean_annual_disrupted_mean']:,.0f} people/year "
            f"[{row['mean_annual_disrupted_lower']:,.0f}–{row['mean_annual_disrupted_upper']:,.0f}]  "
            f"({row['country_pct_disrupted_mean']:.2f}% of population)"
        )
