...
sim.modules["OesophagealCancer"].parameters[
    "init_prop_dysphagia_oes_cancer_by_stage"
] = [0.0] * 6
sim.modules["OesophagealCancer"].parameters[
    "init_prop_with_dysphagia_diagnosed_oes_cancer_by_stage"
] = [0.0] * 6
sim.modules["OesophagealCancer"].parameters["init_prop_treatment_status_oes_cancer"] = [
    0.0
] * 6
...
treated = pd.DataFrame(
    df.loc[
        df.index.isin(cohort) & ~pd.isnull(df.oc_date_treatment), "oc_date_treatment"
    ].copy()
)
...
cohort_treated["oc_date_treatment"] = pd.to_datetime(
    cohort_treated["oc_date_treatment"]
)
cohort_treated["days_treatment_to_death"] = (
    cohort_treated["date"] - cohort_treated["oc_date_treatment"]
).dt.days
...
1 - (
    len(
        cohort_treated.loc[
            (cohort_treated["cause"] == "OesophagealCancer")
