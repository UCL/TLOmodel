...
for seed in range(1, 11):
    ...
    if __name__ == "__main__":
        ...
        #-------------------------
            ...
            return deaths_per_facility, costs_df, need_oxygen_ds_with_SpO2lt90, total_full_oxygen_provided, total_ox_liters_used, total_ox_liters_used_perfect
        ...
        df.index.name = 'Seed number'

        if not os.path.exists(f'output_{scenario}{datestamp}.xlsx'):
            df.to_excel(f'./output_{scenario}{datestamp}.xlsx', sheet_name='Sheet_compile', index=True)

        new_row = pd.DataFrame([data])
        new_row.index = [f'{seed}']

        # Properly use ExcelWriter with existing workbook
        book = load_workbook(f'./output_{scenario}{datestamp}.xlsx')

        with pd.ExcelWriter(f'./output_{scenario}{datestamp}.xlsx', engine='openpyxl') as writer:
            writer.book = book
            writer.sheets = {ws.title: ws for ws in book.worksheets}
            new_row.to_excel(writer,
                             sheet_name='Sheet_compile',
                             header=False,
                             startrow=writer.sheets['Sheet_compile'].max_row,
                             index=True)


def confidence_interval(data, confidence=0.95):
    """
    Computes the confidence interval for a given set of data.
    ...
    n = len(data)
    mean = np.mean(data)
    ci_lower = data.quantile(0.025, axis=0)
    ci_upper = data.quantile(0.975, axis=0)

    # std_err = stats.sem(data)  # Standard error of the mean
    # t_value = stats.t.ppf((1 + confidence) / 2, df=n-1)  # t-score for CI
    # margin_of_error = t_value * std_err
    # return mean, mean - margin_of_error, mean + margin_of_error
    return mean, ci_lower, ci_upper


# Get the excel file with the data
data = pd.read_excel(f'output_{scenario}{datestamp}.xlsx', sheet_name='Sheet_compile')
mean, ci_lower, ci_upper = confidence_interval(data)

new_row_mean = pd.DataFrame([mean[1:]])  # remove the 'Seed number' means and CIs
new_row_ci_lower = pd.DataFrame([ci_lower[1:]])
new_row_ci_upper = pd.DataFrame([ci_upper[1:]])

# Convert each series to formatted strings
string_value_mean = mean[1:].apply(lambda x: "{:.2f}".format(x))
string_value_ci_lower = ci_lower[1:].apply(lambda x: "{:.2f}".format(x))
string_value_ci_upper = ci_upper[1:].apply(lambda x: "{:.2f}".format(x))

# Combine into the final format for each column
joint_mean_ci = string_value_mean + ' (' + string_value_ci_lower + ', ' + string_value_ci_upper + ')'
joint_mean_ci = joint_mean_ci.to_frame().T  # get to the same format as new_row_mean and ci
...
with pd.ExcelWriter(f'./output_{scenario}{datestamp}.xlsx', engine='openpyxl') as writer:
    writer.book = book
    writer.sheets = {ws.title: ws for ws in book.worksheets}
    new_row_mean.index = ['mean']
    new_row_mean.to_excel(writer,
                     sheet_name='Sheet_compile',
                     header=False,
                     startrow=writer.sheets['Sheet_compile'].max_row,
                     index=True)
    new_row_ci_lower.index = ['ci_lower']
    new_row_ci_lower.to_excel(writer,
                          sheet_name='Sheet_compile',
                          header=False,
                          startrow=writer.sheets['Sheet_compile'].max_row,
                          index=True)
    new_row_ci_upper.index = ['ci_upper']
    new_row_ci_upper.to_excel(writer,
                          sheet_name='Sheet_compile',
                          header=False,
                          startrow=writer.sheets['Sheet_compile'].max_row,
                          index=True)
    joint_mean_ci.index = ['mean (ci)']
    joint_mean_ci.to_excel(writer,
                              sheet_name='Sheet_compile',
                              header=False,
                              startrow=writer.sheets['Sheet_compile'].max_row,
                              index=True)
