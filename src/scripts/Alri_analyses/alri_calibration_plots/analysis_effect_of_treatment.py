...
hsi_with_perfect_diagnosis = HSI_Alri_Treatment(
    module=alri_module_with_perfect_diagnosis, person_id=None
)
...
def generate_case_mix() -> pd.DataFrame:
    ...
    def get_incident_case_mix() -> pd.DataFrame:
        ...
        probs_of_acquiring_pathogen = (
            alri_polling_event.get_probs_of_acquiring_pathogen(
        ...
        while len(new_alri) < MIN_SAMPLE_OF_NEW_CASES:
            new_alri.extend(
                [
                    (k, v)
                    for k, v in sample_outcome(
                        probs=probs_of_acquiring_pathogen,
                        rng=alri_module_with_perfect_diagnosis.rng,
                    ).items()
                ]
            )
        ...
        # under-nutrition).
        return (
            pd.DataFrame(columns=["person_id", "pathogen"], data=new_alri)
            .merge(
                sim1.population.props[["age_exact_years", "sex"]],
                right_index=True,
                left_on=["person_id"],
                how="left",
            )
    ...
    def char_of_incident_case(
        ...
        incident_case = AlriIncidentCase(
            module=alri_module_with_perfect_diagnosis, person_id=None, pathogen=None
        )
    ...
    overall_case_mix = (
        []
    )  # This will be the mix of cases, representing the characteristics of the case as well as the
    #                        pathogen and who they are incident to.

    for x in incident_case_mix.itertuples():
        overall_case_mix.extend(
            char_of_incident_case(
                sex=x.sex, age_exact_years=x.age_exact_years, pathogen=x.pathogen
            )
        )
...
def treatment_efficacy(
    ...
    classification_for_treatment_decision = (
        hsi._get_disease_classification_for_treatment_decision(
            age_exact_years=age_exact_years,
            symptoms=symptoms,
    ...
    treatment_fails = alri_module.models._prob_treatment_fails(
        ...
        oxygen_provided=(
            ultimate_treatment["oxygen_indicated"] if oxygen_available else False
        ),
...
if __name__ == "__main__":
    ...
    def summarize_by(
        df: pd.DataFrame, by: List[str], columns: [List[str]]
    ) -> pd.DataFrame:
        ...
        """
        return (
            pd.DataFrame({"fraction": df.groupby(by=by).size()})
            .apply(lambda x: x / x.sum(), axis=0)
            .join(df.groupby(by=by)[columns].mean())
        )

    # Examine case mix
    case_mix_by_disease_and_pathogen = summarize_by(
        table,
    ...
    risk_of_death = summarize_by(
        ...
        fraction_of_deaths=lambda df: (
            (df.fraction * df.prob_die_if_no_treatment)
            / (df.fraction * df.prob_die_if_no_treatment).sum()
        )
    )
    ...
    disease_classification = table[
        "classification_for_treatment_decision_with_oximeter_perfect_accuracy"
    ]
    low_oxygen = (table["oxygen_saturation"] == "<90%").replace(
        {True: "SpO2<90%", False: "SpO2>=90%"}
    )
    res = {
        "Perfect HW Dx Accuracy": {
            "No Treatment": table["prob_die_if_no_treatment"]
            .groupby(by=[disease_classification, low_oxygen])
            .sum(),
            "Antiobiotics only": (
    ...
    results = (100_000 / len(table)) * pd.concat(
        {k: pd.DataFrame(v) for k, v in res.items()}, axis=1
    )
