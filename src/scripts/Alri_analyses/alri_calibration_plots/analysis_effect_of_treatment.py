...
def generate_case_mix() -> pd.DataFrame:
    ...
    def get_incident_case_mix() -> pd.DataFrame:
            ...
            new_alri.extend(
                [
                    ...
                    for k, v in sample_outcome(
                        probs=probs_of_acquiring_pathogen, rng=alri_module_with_perfect_diagnosis.rng
                    ).items()
                ]
            )
        ...
        return (
            ...
            .merge(
                sim1.population.props[["age_exact_years", "sex"]], right_index=True, left_on=["person_id"], how="left"
            )
...
if __name__ == "__main__":
    ...
    case_mix_by_disease_and_pathogen = summarize_by(
        table, by=["disease_type", "pathogen"], columns=["has_danger_signs", "needs_oxygen", "prob_die_if_no_treatment"]
    ).reset_index()
    ...
    res = {
        ...
        "Normal HW Dx Accuracy": {
            ...
            "Antiobiotics only": (
                ...
                * (
                    1.0
                    - table["treatment_efficacy_if_normal_treatment_but_without_oximeter_or_oxygen_imperfect_hw_dx"]
                    / 100.0
                )
            )
            .groupby(by=[disease_classification, low_oxygen])
