...
if __name__ == "__main__":
    ...
    def scaled_baseline_tf(df, unscaled_intercept):
        ...
        unscaled_tf = unscaled_lm.predict(df,
                                          ...
                                          # referral_hc=df['seek_level'].apply(lambda x: True if '0' in x or '1a' in x else False) and
                                          #     df[f'referral_status_and_oxygen_scenario_{scenario}_{dx_accuracy}_hw_dx'].str[1] == 'not provided' and
    ...
    df_fb_pneumonia = df.loc[
                      (df[f"classification_for_treatment_decision_with_oximeter_perfect_accuracy_level2"] == 'fast_breathing_pneumonia'), :]
    get_base_oral_tf_fb_pneum = scaled_baseline_oral_tf(df=df_fb_pneumonia, unscaled_intercept=0.101)

    df_ci_pneumonia = df.loc[(df[f"classification_for_treatment_decision_with_oximeter_perfect_accuracy_level2"] == 'chest_indrawing_pneumonia'), :]
    ...
    def scaled_baseline_death(df, unscaled_intercept):
        ...
        unscaled_death_lm = \
            LinearModel(
                ...
                Predictor('below90SpO2', external=True).when(True, 5.04), # 5.04
                Predictor('moderate_hypoxaemia', external=True).when(True, 1.54), # 1.54
                Predictor('chest_indrawing', external=True).when(True, 1.26),
                Predictor('danger_signs', external=True).when(True, 2.45), # 2.45
                Predictor('respiratory_distress', external=True).when(True, 1.65),
                Predictor('bacteraemia', external=True).when(True, 2.51),
                # # under 2 mo
                # Predictor('danger_signs', external=True).when(True, 2.4), # 2.45
                # Predictor('moderate_hypoxaemia', external=True).when(True, 3.3), # 1.54
                # Predictor('below90SpO2', external=True).when(True, 4.5), # 5.04
            )
