...
for seed in range(1, 11):
    ...
    if __name__ == "__main__":
        ...
        risk_of_death = summarize_by(
            df=table,
            by=['oxygen_saturation', 'has_danger_signs', 'pulmonary_complication', 'bacteraemia', 'bacterial_infection',
                'hv_inf', 'un_clinical_acute_malnutrition',
                'classification_for_treatment_decision_without_oximeter_perfect_accuracy', 'disease_type'],
            columns=[
        ...
        if not os.path.exists(f'output_treatment_efficacies_10_runs{datestamp}.xlsx'):
            df.to_excel(f'./output_treatment_efficacies_10_runs{datestamp}.xlsx',
                        sheet_name=f'Sheet_seed{seed}', index=True)
        ...
        book = load_workbook(f'./output_treatment_efficacies_10_runs{datestamp}.xlsx')

        with pd.ExcelWriter(f'./output_treatment_efficacies_10_runs{datestamp}.xlsx', engine='openpyxl') as writer:
            writer.book = book
            writer.sheets = {ws.title: ws for ws in book.worksheets}
            new_data.to_excel(writer,
                              sheet_name=f'Sheet_seed{seed}',
                              header=True,
                              index=True)

        # -----------------------------
        # Append results of each loop
        seed_run_dfs[seed] = pd.DataFrame(table)


# Save the table output
with open(f'debug_output_treatment_efficacies_10_runs{datestamp}.pkl', 'wb') as f:
    pickle.dump(seed_run_dfs, f)

# Later, load it back
with open(f'debug_output_treatment_efficacies_10_runs__2025_02_20.pkl', 'rb') as f:
    ...
    for seed in range(1, 11):
        my_output[seed] = pd.DataFrame(my_output[seed])
        # # # Treatment efficacy table # # #
        risk_of_death = summarize_by(
            df=my_output[seed],
        ...
        if not os.path.exists(f'output_treatment_efficacies_10_runs{datestamp}_reduced.xlsx'):
            df.to_excel(f'./output_treatment_efficacies_10_runs{datestamp}_reduced.xlsx',
                        sheet_name=f'Sheet_seed{seed}', index=True)
        ...
        # Properly use ExcelWriter with existing workbook
        book = load_workbook(f'./output_treatment_efficacies_10_runs{datestamp}_reduced.xlsx')

        with pd.ExcelWriter(f'./output_treatment_efficacies_10_runs{datestamp}_reduced.xlsx', engine='openpyxl') as writer:
            writer.book = book
            writer.sheets = {ws.title: ws for ws in book.worksheets}
            new_data.to_excel(writer,
                              sheet_name=f'Sheet_seed{seed}',
...
def calculate_stats_with_indexes(excel_file, index_columns):
    """
    Calculate statistics using specified columns as indexes.

    Parameters:
    excel_file (str): Path to Excel file
    index_columns (list): List of 4 column names to use as indexes

    Returns:
    DataFrame with multi-index and statistics
    """
    ...
    for sheet_name in excel.sheet_names:
        ...
        # Verify all index columns exist in the sheet
        if not all(col in df.columns for col in index_columns):
            print(f"Warning: Some index columns missing in sheet {sheet_name}")
            continue

        # Add sheet name as a column if needed
        df['Sheet'] = sheet_name
        all_data.append(df)
    ...
    numeric_columns = [col for col in numeric_columns if col not in index_columns]

    combined_df['prob_die_if_no_treatment'] *= 100  # get the probability of death into % format

    # Group by the index columns and calculate statistics
    stats = (combined_df.groupby(index_columns)[numeric_columns]
             .agg(['mean',  # Corrected this line
                   lambda x: np.nanpercentile(x, 2.5),
                   lambda x: np.nanpercentile(x, 97.5)])
             .round(4))


    # # Rename the lambda function columns to be more descriptive
    # stats.columns = [f'{col[0]}_{col[1]}'.replace('<lambda_0>', 'ci_lower').replace('<lambda_1>', 'ci_upper')
    #                  for col in stats.columns]
    #
    # return stats
    # Combine statistics into a single string with 2 decimal formatting
    ...
    for col in numeric_columns:
        ...
        lower_vals = stats[col]['<lambda_0>']
        upper_vals = stats[col]['<lambda_1>']

        formatted_stats[col] = formatted_stats[col] = mean_vals.apply(format_number) + " (" + \
                                                      lower_vals.apply(format_number) + ", " + \
                                                      upper_vals.apply(format_number) + ")"

    return formatted_stats


# get the mean, ci lower and upper
excel_file = f'./output_treatment_efficacies_10_runs{datestamp}.xlsx'
index_columns = ['oxygen_saturation', 'has_danger_signs', 'pulmonary_complication',
                 'bacteraemia', 'bacterial_infection', 'hv_inf', 'un_clinical_acute_malnutrition',
                 'classification_for_treatment_decision_without_oximeter_perfect_accuracy', 'disease_type']
efficacies_stats = calculate_stats_with_indexes(excel_file, index_columns)
efficacies_stats.to_excel(f'treatment_efficacies_mean_ci{datestamp}.xlsx')
...
with open(f'debug_output_treatment_efficacies_10_runs__2025_02_20.pkl', 'rb') as f:
    ...
    for seed in range(1, 11):
        ...
        all_risk_of_death.append(risk_of_death)

    # Combine all sheets
    combined_df = pd.concat(all_risk_of_death, ignore_index=False)
    ...
    formatted_stats.to_excel(f'treatment_efficacies_mean_ci{datestamp}_reduced.xlsx')
