...
counts = counts.drop(columns="5+").rename(
    columns={"0": "0y", "1": "1y", "2-4": "2-4y"}
)  # for consistency
...
pop.set_index("date", drop=True, inplace=True)
...
for age_grp in ["0y", "1y", "2-4y"]:
    inc_rate[age_grp] = (
        counts[age_grp].apply(pd.Series).div(py[age_grp], axis=0).dropna()
    )
...
for ax_num, pathogen in enumerate(sim.modules["Alri"].all_pathogens):
    ...
    ax.hlines(
        y=calibration_incidence_rate_0_year_olds[
            pathogen
        ],  # axhlines is to plot horizontal lines at each y
...
plt.savefig(
    outputpath
    / ("ALRI_inc_rate_by_pathogen_and_time_0_year_olds" + datestamp + ".png"),
    format="png",
)
plt.tight_layout()
plt.show()
...
inc_mean["2-4y_calibrating_data"] = pd.Series(
    data=calibration_incidence_rate_2_to_4_year_olds
)
...
plt.tight_layout()
plt.savefig(
    outputpath / ("ALRI_inc_rate_calibration_0_year_olds" + datestamp + ".png"),
    format="png",
)
plt.show()
...
plt.tight_layout()
plt.savefig(
    outputpath / ("ALRI_inc_rate_calibration_1_year_olds" + datestamp + ".png"),
    format="png",
)
plt.show()
...
plt.tight_layout()
plt.savefig(
    outputpath / ("ALRI_inc_rate_calibration_2-4_year_olds" + datestamp + ".png"),
    format="png",
)
plt.show()
...
comparison.loc[
    ("2010-2014", slice(None), "0-4", "Lower respiratory infections")
].sum().plot.bar()
plt.title("Deaths per year due to ALRI, 2010-2014")
plt.tight_layout()
plt.savefig(
    outputpath / ("ALRI_death_calibration_plot" + datestamp + ".png"), format="png"
)
plt.show()
