""" This script will output the scenario outcomes for baseline antibiotics (baseline scenario) """
...
def treatment_efficacy(
    ...
    # incorrect_initial_dx = True if imci_symptom_based_classification != classification_for_treatment_decision else False
    # incorrect_initial_dx_or_complicated = incorrect_initial_dx or (len(complications) != 0)
...
if __name__ == "__main__":
    ...
    table['consultation_cost_seek_level'] = table['seek_level'].apply(
        lambda x: 2.58*2 if x == '2' else 2.47*2 if x == '1b' else 2.17*2 if x == '1a' else 1.76*2
    )
    ...
    table['inpatient_bed_cost_per_day'] = table[f'final_facility_scenario_{scenario}_{dx_accuracy}_hw_dx'].apply(
        lambda x: 7.94*2 if x == '2' else 6.89*2 if x == '1b' else 0)
