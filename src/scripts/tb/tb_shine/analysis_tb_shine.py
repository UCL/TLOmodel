...
# --------------- Consumables --------------- #
consumables = output['tlo.methods.healthsystem']['Consumables'].copy()
consumables = consumables.set_index('date')

keep = ((consumables.TREATMENT_ID == 'Hiv_Treatment_InitiationOrContinuation')) # change treatment type
HIV_treatment = consumables.loc[keep].copy()
HIV_treatment['year'] = HIV_treatment.index.year
HIV_treatment = HIV_treatment.groupby(by=['year']).size()
HIV_treatment.index = pd.to_datetime(HIV_treatment.index)

make_plot(
    title_str="Number of HIV Treatments",
    model=HIV_treatment,
)
