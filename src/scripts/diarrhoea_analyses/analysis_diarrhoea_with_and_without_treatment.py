...
def get_incidence_rate_and_death_numbers_from_logfile(logfile):
    ...
    deaths = (
        deaths_df.loc[deaths_df["cause"].str.startswith("Diarrhoea")]
        .groupby("year")
        .size()
    )
...
def plot_for_column_of_interest(results, column_of_interest):
    ...
    plt.tight_layout()
    plt.savefig(
        outputpath / ("Diarrhoea_inc_rate_by_scenario" + datestamp + ".png"),
        format="png",
    )
    plt.show()
...
plt.savefig(
    outputpath / ("Diarrhoea_deaths_by_scenario" + datestamp + ".png"), format="png"
)
plt.tight_layout()
plt.show()
