# This is a scratch file by Tim Hallett that will get the UNCIEF Master Facility Level data
# in the right format for importing
...
# Combine "Dispensaries" and "Health Centres";
# Combine 'Health Post', 'Outreach', and 'Village Clinic' as "Community Health Worker"
# (= the Community Health Worker under a tree in each village)
...
wb.loc[wb['Facility Type'] == 'District Hospital', ['District', 'Facility Type']].groupby(
    by=['District']).count()
...
# When used we will .loc onto this to find (CHW (Community Health Worker),
# Near-Hospital (Nearest Hospital),District Hospital,Referral Hospital)
...
df_CHW = wb.loc[
    wb['Facility Type'] == 'Community Health Worker', ['Village', 'Facility Type', 'Facility_ID']]
...
hospitals = wb.loc[
    (wb['Facility Type'] == 'Hospital') & (wb['Facility Type'] != 'Referral Hospital')]
...
for v in villages:
    ...
    dist_sq = np.power(hospitals['Eastings'] - vill_x, 2) + np.power(
        hospitals['Northings'] - vill_y, 2)

    nearest_hosp_id = hospitals.loc[dist_sq.idxmin(), 'Facility_ID']

    df_NearHospital = df_NearHospital.append(
        {'Village': v, 'Facility Type': 'Near Hospital', 'Facility_ID': nearest_hosp_id},
        ignore_index=True)
...
for v in villages:
    ...
    the_district_hosp = district_hosps.loc[
        district_hosps['District'] == the_district, 'Facility_ID']
...
for v in villages:
    ...
    if region == 'South':
        ref_hosp_facility_id = wb.loc[wb['Facility Name'] == 'QUEEN ELIZABETH'].Facility_ID.values[
            0]
    else:
        ref_hosp_facility_id = wb.loc[wb['Facility Name'] == 'KAMUZU CENTRAL HOSPITAL'].Facility_ID.values[0]

    df_ReferralHospital.loc[
        df_ReferralHospital['Village'] == v, 'Facility_ID'] = ref_hosp_facility_id
...
fac_details.Facility_ID = fac_details.Facility_ID.astype(
    np.int64)  # coerce the typing ready for the merge
