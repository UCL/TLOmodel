...
def add_footnote(footnote: str):
    ...
    plt.figtext(0.5, 0.01, footnote, ha="center", fontsize=10, bbox={"facecolor": "gray", "alpha": 0.3, "pad": 5})
...
class LifeStyleCalibration:
    ...
    def __init__(self, logs=None, path: str = None):
        ...
        self.en_props = {
            "li_urban": "currently urban",
            "li_wealth": "wealth level",
            "li_low_ex": "currently low exercise",
            "li_tob": "current using tobacco",
            "li_ex_alc": "current excess alcohol",
            "li_mar_stat": "marital status",
            "li_in_ed": "currently in education",
            ...
            "li_wood_burn_stove": "wood burn stove",
            "li_no_access_handwashing": " no access hand washing",
            "li_high_salt": "high salt",
            "li_high_sugar": "high sugar",
            "li_bmi": "bmi",
            "li_is_circ": "Male circumcision",
            "li_is_sexworker": "sex workers",
        }

        # ------------------------- OBSERVED DATA PROPORTIONS--------------------------------
        # 1. Lifestyle properties that are logged not by either rural urban or gender
        self.obs_data_prop = {
            "li_urban": {"ob_year": 2018, "source": "2018 Population census", "data": 0.16},
            "li_no_access_handwashing": {"ob_year": 2015, "source": "Malawi DHS 2015", "data": 0.58},
            "li_high_salt": {"ob_year": 2018, "source": "Price et al 2018; weighting by urban rural", "data": 0.27},
            "li_high_sugar": {
                "ob_year": 2018,
                "source": "Price et al 2018; 6 or more sugary drinks per day",
                "data": 0.37,
            },
            "li_is_sexworker": {"ob_year": 2012, "source": "UNAIDS", "data": 0.006},
        ...
        self.obs_data_prop_rural_urban = {
            "li_unimproved_sanitation": {"ob_year": 2015, "source": "Malawi DHS 2015", "data": [0.04, 0.19]},
            "li_no_clean_drinking_water": {"ob_year": 2015, "source": "Malawi DHS 2015", "data": [0.02, 0.15]},
            "li_wood_burn_stove": {"ob_year": 2015, "source": "Malawi DHS 2015", "data": [0.26, 0.94]},
            "li_ex_alc": {"ob_year": 2014, "source": "WHO 2014", "data": [0.15, 0.01]},
        }
        ...
        self.obs_data_prop_rural_urban_cat = {
            "li_wealth": {
                ...
                "data": {"urban": [0.75, 0.16, 0.05, 0.02, 0.02], "rural": [0.11, 0.21, 0.22, 0.23, 0.23]},
            },
            "li_low_ex": {
                "ob_year": 2011,
                "source": "Msyamboza et al; 2011; WHO STEPS",
                "data": {"urban": [0.32, 0.18], "rural": [0.11, 0.07]},
            },
            "li_bmi": {
                "ob_year": 2018,
                "source": "Price et al; median age ~ 30",
                "data": {
                    "urban": {"males": [0.04, 0.77, 0.15, 0.04, 0.00], "females": [0.02, 0.53, 0.26, 0.18, 0.01]},
                    "rural": {"males": [0.06, 0.85, 0.08, 0.01, 0.0], "females": [0.05, 0.68, 0.20, 0.08, 0.0]},
                },
            },
        }
        ...
        self.other_props = {
            "li_mar_stat": {
                "ob_year": 2015,
                "source": "Malawi DHS 2015",
                "data": {"males": [0.402, 0.565, 0.033], "females": [0.21, 0.657, 0.133]},
            },
            "li_tob": {
                "ob_year": 2015,
                "source": "Malawi DHS 2015",
                "data": {"males": 0.12, "females": 0.06},
            },
        }
        ...
        self._rural_urban_state = {"True": "urban", "False": "rural"}
    ...
    def construct_dfs(self, lifestyle_log) -> dict:
        ...
        return {
            ...
            for k, v in lifestyle_log.items()
            if k in self.en_props.keys()
        }
    ...
    def custom_axis_formatter(self, df: pd.DataFrame, ax, li_property: str):
        ...
        ax.legend(
            self.categories_desc[li_property]
            if li_property in self.categories_desc.keys()
            else [self.en_props[li_property]],
            loc="upper center",
        )
    ...
    def plot_no_cat_gender_age_group_properties(self, li_property: str):
        ...
        if li_property == "li_in_ed":
            ...
            for age_years in ["6", "12", "16", "19"]:
                pos_to_prop = re_ord_col_df["M"]["True"][age_years] + re_ord_col_df["F"]["True"][age_years]
            ...
            # get population positive to the property
            total_per_prop["model_output"] = self.dfs[li_property]["M"]["True"].sum(axis=1) + self.dfs[li_property][
                "F"
            ]["True"].sum(axis=1)
        # plotting
        if li_property == "li_in_ed":
            ...
            for age_years in ["6", "12", "16", "19"]:
                ...
                # do plotting
                ax = total_per_prop.plot.line(
                    y=[f"age_years_{age_years}"],
                    ylabel=f"{self.en_props[li_property]}" f" proportions",
                    ylim=[0, 1],
                    ax=axes[_row_counter, _column_counter],
                    title=f"Individuals aged {age_years} years {self.en_props[li_property]}",
                )
                total_per_prop.plot.line(y=["observed_data"], marker="^", color="red", ax=ax)
                # plt.ylim(0, 1)
    ...
    def plot_properties_by_urban_rural(self, li_property: str):
        ...
        for _value in self._rural_urban_state.keys() if not li_property == "li_ex_alc" else self.gender_des.keys():
            ...
            if li_property == "li_ex_alc":
                ...
                total_per_prop["total_pop"] = self.dfs[li_property]["True"][_value].sum(axis=1) + self.dfs[li_property][
                    "False"
                ][_value].sum(axis=1)

                # get population positive to excess alcohol
                total_per_prop["model_output"] = self.dfs[li_property]["True"][_value]["True"].sum(axis=1) + self.dfs[
                    li_property
                ]["False"][_value]["True"].sum(axis=1)
            else:
                # get totals male and females per each property
                total_per_prop["total_pop"] = self.dfs[li_property][_value]["M"].sum(axis=1) + self.dfs[li_property][
                    _value
                ]["F"].sum(axis=1)

                # get population positive to the property
                total_per_prop["model_output"] = self.dfs[li_property][_value]["M"]["True"].sum(axis=1) + self.dfs[
                    li_property
                ][_value]["F"]["True"].sum(axis=1)
            ...
            # set observed data value
            total_per_prop.loc[
                (total_per_prop.index == self.obs_data_prop_rural_urban[li_property]["ob_year"]), "observed_data"
            ] = self.obs_data_prop_rural_urban[li_property]["data"][_rows_counter]
            ...
            if not li_property == "li_ex_alc":
                ax = total_per_prop.plot.line(
                    y=["model_output"],
                    ax=axes[_rows_counter],
                    ylim=(0, ylim),
                    ylabel=f"{self.en_props[li_property]} proportions",
                    title=f"{self.en_props[li_property]}" f" {self._rural_urban_state[_value]}",
                )
            else:
                ax = total_per_prop.plot.line(
                    y=["model_output"],
                    ...
                    ylim=(0, ylim),
                    ylabel=f"{self.en_props[li_property]} proportions",
                    title=f"{self.en_props[li_property]}" f" {self.gender_des[_value]}",
                )
            total_per_prop.plot.line(y=["observed_data"], marker="^", color="red", ax=ax)
    ...
    def plot_properties_by_urban_rural_cat(self, li_property: str, categories: list = None):
        ...
        for urban_rural in self._rural_urban_state.keys():
                ...
                for gender, gender_desc in self.gender_des.items():
                    ...
                    for _category in categories:
                        total_per_prop[f"cat_{_category}"] = self.dfs[li_property][urban_rural][gender][_category].sum(
                            axis=1
                        ) + self.dfs[li_property][urban_rural][gender][_category].sum(axis=1)
            ...
            # for low exercise
                ...
                total_per_prop = pd.DataFrame()
                for gender, gender_desc in self.gender_des.items():
                    total_per_prop[gender_desc] = self.dfs[li_property][urban_rural][gender]["True"].sum(axis=1)
            ...
            # For the remaining properties
                ...
                for _category in categories:
                    total_per_prop[f"cat_{_category}"] = self.dfs[li_property][urban_rural]["M"][_category].sum(
                        axis=1
                    ) + self.dfs[li_property][urban_rural]["F"][_category].sum(axis=1)
                ...
                for urban_rural in self._rural_urban_state.keys():
                    ...
                    for gender_desc in self.gender_des.values():
                        ...
                        _prop_urb_rural[urban_rural][gender_desc].loc[
                            ...
                            self._rural_urban_state[urban_rural]
                        ][str.lower(gender_desc)][int(_category) - 1]
                        # do plotting
                        ax = _prop_urb_rural[urban_rural][gender_desc].plot.line(
                            y=[f"cat_{_category}"],
                            ylabel=f"{self.en_props[li_property]} proportions",
                            ax=axes[_column_counter, _rows_counter],
                            ylim=(0, ylim),
                        ...
                        _prop_urb_rural[urban_rural][gender_desc].plot.line(
                            y=["observed_data"], marker="^", color="red", ax=ax
                        )
                        ax.legend(["model_output", "observed_data"])
                        _rows_counter += 1
                    _column_counter += 1
        ...
        # plot low exercise
            ...
            for urban_rural in self._rural_urban_state.keys():
                ...
                for gender in self.gender_des.values():
                    # set observed data value
                    _prop_urb_rural[urban_rural].loc[
                        (
                            _prop_urb_rural[urban_rural].index
                            == self.obs_data_prop_rural_urban_cat[li_property]["ob_year"]
                        ),
                        "observed_data",
                    ...
                    # do plotting
                    ax = _prop_urb_rural[urban_rural].plot.line(
                        y=[gender],
                        ylabel=f"{self.en_props[li_property]} proportions",
                        ax=axes[_column_counter, _rows_counter],
                        ylim=(0, ylim),
                        title=f"{self.en_props[li_property]} {gender}" f" {self._rural_urban_state[urban_rural]}",
                    )

                    _prop_urb_rural[urban_rural].plot.line(y=["observed_data"], marker="^", color="red", ax=ax)
                    ax.legend(["model_output", "observed_data"])
                    _rows_counter += 1
                _column_counter += 1
            add_footnote(f"Data source: {self.obs_data_prop_rural_urban_cat[li_property]['source']}")
            ...
            for _category in categories:
                ...
                for urban_rural in self._rural_urban_state.keys():
                    ...
                    ax = _prop_urb_rural[urban_rural].plot.line(
                        ...
                        ax=axes[_rows_counter],
                        ylim=(0, ylim),
                        title=f"{self.en_props[li_property]} {_category}" f" {self._rural_urban_state[urban_rural]}",
                    )
            ...
            if li_property == "li_mar_stat":
                for _category in categories:
                    total_per_prop[f"cat_{_category}"] = self.dfs[li_property][gender][_category].sum(
                        axis=1
                    ) + self.dfs[li_property][gender][_category].sum(axis=1)
            ...
            for _category in categories:
                ...
                for gender_desc in self.gender_des.values():
                    ...
                    _props[gender_desc].loc[
                        (_props[gender_desc].index == self.other_props[li_property]["ob_year"]), "observed_data"
                    ] = self.other_props[li_property]["data"][str.lower(gender_desc)][int(_category) - 1]
                    # do plotting
                    ax = _props[gender_desc].plot.line(
                        y=[f"cat_{_category}"],
                        ylabel=f"{self.en_props[li_property]} proportions",
                        ax=axes[_rows_counter],
                        ylim=(0, ylim),
                        title=f" {gender_desc} " f"{self.categories_desc[li_property][int(_category) - 1]}",
                    )
        ...
        # plot tobacco
            ...
            for gender_desc in self.gender_des.values():
                ...
                ax = _props[gender_desc].plot.line(
                    ...
                    ax=axes[_rows_counter],
                    ylim=(0, ylim),
                    title=f" {gender_desc} " f"{self.en_props[li_property]}",
                )
