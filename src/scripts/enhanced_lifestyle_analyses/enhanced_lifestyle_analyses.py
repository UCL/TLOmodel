...
def add_footnote(fig: plt.Figure, footnote: str):
    ...
    graph is"""
    fig.figure.text(0.5, 0.01, footnote, ha="center", fontsize=10, bbox={"facecolor": "gray", "alpha": 0.3, "pad": 5})
...
class LifeStylePlots:
    ...
    def __init__(self, logs=None, path: Optional[str] = None):
        ...
        self.en_props = {
            ...
            "li_unimproved_sanitation": PlotDescriptor(
                "unimproved sanitation", "Sum of all individuals " "in urban or rural"
            ),
            "li_no_clean_drinking_water": PlotDescriptor(
                "no clean drinking water", "Sum of all individuals per" "urban or rural"
            ),
            ...
            "li_bmi": PlotDescriptor(
                "bmi", "Sum of 15+ individuals in all bmi categories in  individuals " "per urban or rural"
            ),
    ...
    def plot_categorical_properties_by_gender(self, li_property: str):
        ...
        fig, axes = plt.subplots(
            nrows=2 if li_property in self.cat_by_rural_urban_props or li_property == "li_ed_lev" else 1,
            ncols=2,
            figsize=(10, 5),
        )
        for gender, desc in self.gender_des.items():
            ...
            if li_property == "li_ed_lev" or li_property in self.cat_by_rural_urban_props:
                ...
                key_value_desc = (
                    self.wealth_desc.items() if li_property == "li_ed_lev" else self._rural_urban_state.items()
                )
            ...
            for _key in df_dict.keys():
                ...
                # do plotting
                ax = df_dict[_key].plot(
                    kind="bar",
                    stacked=True,
                    ax=axes[int(_key.split("_")[-1]), col]
                    if li_property in self.cat_by_rural_urban_props or li_property == "li_ed_lev"
                    else axes[col],
                    legend=None,
                    title=f"{_key.split('_')[1]} {desc} {self.en_props[li_property].label}" ' categories'
                    if li_property in self.cat_by_rural_urban_props or li_property == "li_ed_lev"
                ...
                if li_property in self.cat_by_rural_urban_props:
                    ...
                    for _key, _value in self._rural_urban_state.items():
                        df_dict[f"{gender}_{_value}_{_row}"] = self.dfs[li_property][_key][gender]["True"].sum(
                            axis=1
                        ) / self.dfs[li_property][_key][gender].sum(axis=1)
                        _row += 1

                else:
                    df = (
                        self.dfs[li_property].reorder_levels([0, 2, 1, 3], axis=1)
    ...
    def plot_categorical_properties_by_age_group(self, li_property: str):
        ...
        # create subplots
        fig, axes = plt.subplots(
            ncols=2 if li_property in self.cat_by_rural_urban_props or li_property == "li_ed_lev" else 1,
            figsize=(10, 5),
        )
        ...
        if li_property == "li_ed_lev" or li_property in self.cat_by_rural_urban_props:
            ...
            key_value_desc = self.wealth_desc.items() if li_property == "li_ed_lev" else self._rural_urban_state.items()
            for _key, _value in key_value_desc:
                gc_df = pd.DataFrame()
                for cat in categories:
                    if li_property == "li_ed_lev":
                        gc_df[f"cat_{cat}"] = self.dfs[li_property]["M"][_key][cat].sum(axis=0) + self.dfs[li_property][
                            "F"
                        ][_key][cat].sum(axis=0)
                    else:
                        gc_df[f"cat_{cat}"] = self.dfs[li_property][_key]["M"][cat].sum(axis=0) + self.dfs[li_property][
                            _key
                        ]["F"][cat].sum(axis=0)
                # add dataframe to dictionary
            ...
            gc_df = pd.DataFrame()
            for cat in categories:
                gc_df[f"cat_{cat}"] = self.dfs[li_property]["M"][cat].sum(axis=0) + self.dfs[li_property]["F"][cat].sum(
                    axis=0
                )
                # add to dataframe
        ...
        for _key in df_dict.keys():
            ...
            df_dict[_key].plot(
                kind="bar",
                stacked=True,
                ax=axes[int(_key.split("_")[-1])]
                if li_property in self.cat_by_rural_urban_props or li_property == "li_ed_lev"
                else axes,
                legend=None,
                title=f"{_key.split('_')[0]} {self.en_props[li_property].label}" ' categories by age group in 2021'
                if li_property in self.cat_by_rural_urban_props or li_property == "li_ed_lev"
    ...
    def plot_non_categorical_properties_by_age_group(self, li_property):
        ...
        # create subplots
        fig, axes = plt.subplots(
            nrows=2 if li_property in self.cat_by_rural_urban_props or li_property == "li_in_ed" else 1,
            figsize=(10, 5),
            sharex=True,
        )
        ...
        if li_property == "li_in_ed" or li_property in self.cat_by_rural_urban_props:
            _col: int = 0  # column counter
            key_value_desc = self.wealth_desc.items() if li_property == "li_in_ed" else self._rural_urban_state.items()
            for _key, _value in key_value_desc:
                temp_df = pd.DataFrame()
                for _bool_value in ["True", "False"]:
                    if li_property == "li_in_ed":
                        temp_df[_bool_value] = self.dfs[li_property]["M"][_key][_bool_value].sum(axis=0) + self.dfs[
                            li_property
                        ]["F"][_key][_bool_value].sum(axis=0)

                    else:
                        temp_df[_bool_value] = self.dfs[li_property][_key]["M"][_bool_value].sum(axis=0) + self.dfs[
                            li_property
                        ][_key]["F"][_bool_value].sum(axis=0)
            ...
            for _bool_value in ["True", "False"]:
                plot_df[_bool_value] = self.dfs[li_property]["M"][_bool_value].sum(axis=0) + self.dfs[li_property]["F"][
                    _bool_value
                ].sum(axis=0)
        ...
        for _key in df_dict.keys():
            ...
            df_dict[_key].plot(
                ...
                stacked=True,
                ax=axes[int(_key.split("_")[-1])]
                if li_property in self.cat_by_rural_urban_props or li_property == "li_in_ed"
                else axes,
                ylim=(0, y_lim),
                legend=None,
                color="darkturquoise",
                ...
                if li_property in self.cat_by_rural_urban_props or li_property == "li_in_ed"
                else f"{self.en_props[li_property].label} by age group in 2021",
                ylabel=f"{self.en_props[li_property].label} proportions",
                xlabel="Year",
            )
    ...
    def male_circumcision_and_sex_workers_plot(self, _property: str = None):
        ...
        ax = totals_df.plot(
            ...
            xlabel="Year",
            color="darkturquoise",
            title=f"{self.en_props[_property].label} Percentage",
        )
        # format x-axis
