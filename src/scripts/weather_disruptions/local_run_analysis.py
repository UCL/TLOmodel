...
print(f"  SSP / model       : ssp245 / mean")
print("-" * 60)
print(f"  Total HSIs seen   : {total_hsi_seen:,}")
print(f"  Total disrupted   : {total_disrupted:,}")
print(f"    Cancelled       : {total_cancelled:,}  ({total_cancelled / total_disrupted * 100:.1f}%)")
print(f"    Delayed         : {total_delayed:,}  ({total_delayed / total_disrupted * 100:.1f}%)")
print(f"  Mean monthly rate : {mean_monthly_rate:.2f}%")
print(f"  Peak month        : {peak_month.strftime('%Y-%m')}  ({peak_rate:.2f}%)")
print("=" * 60)
