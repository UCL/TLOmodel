...
calibration_data[budget_columns + expenditure_columns] = calibration_data[budget_columns + expenditure_columns].apply(
...
calibration_data = calibration_data.rename(
    columns={'EXPENDITURE (USD) (Jul 2018 - Jun 2019)': 'actual_expenditure_2019',
...
calibration_data = calibration_data.groupby('calibration_category')[
    ['actual_expenditure_2019', 'max_annual_budget_2020-22']].sum().reset_index()
...
input_costs = estimate_input_cost_of_scenarios(results_folder, resourcefilepath, _years=[2018], _draws=[0],
                                               summarize=True, cost_only_used_staff=False)
...
def assign_item_codes_to_consumables(_df):
    ...
    consumables_df = pd.read_csv(resourcefilepath / "costing" / "ResourceFile_Costing_Consumables.csv")[
        ['Item_Code', 'Items']]
...
art = [get_item_code("First-line ART regimen: adult"), get_item_code("Cotrimoxizole, 960mg pppy"),  # adult
       get_item_code("First line ART regimen: older child"), get_item_code("Cotrimoxazole 120mg_1000_CMST"),
       # Older children
...
tb_tests = [get_item_code("ZN Stain"), get_item_code("Sputum container"),
            get_item_code("Microscope slides, lime-soda-glass, pack of 50"),
...
other_drugs = set(consumables_costs_by_item_code['cost_subgroup'].unique()) - set(irs) - set(bednets) - set(
    undernutrition) - set(other_family_planning) - set(vaccines) \
...
def fill_calibration_costs_from_cost_groups(calibration_df: pd.DataFrame,
                                            mappings: list[
                                                tuple[pd.DataFrame, str, str, list[str], float]]) -> pd.DataFrame:
    """
...
mappings = [
    # Consumables
    (consumables_costs_by_item_code, 'cost_subgroup', 'Antiretrovirals', art, 82 / (0.103 * 365)),
    ...
    # HR
    (input_costs[input_costs['cost_category'] == 'human resources for health'], 'cost_subcategory',
     'Health Worker Salaries', ['salary_for_all_staff'], 1),
    (input_costs[input_costs['cost_category'] == 'human resources for health'], 'cost_subcategory',
     'Health Worker Training - Pre-Service', ['preservice_training_and_recruitment_cost_for_attrited_workers'], 1),
    (input_costs[input_costs['cost_category'] == 'human resources for health'], 'cost_subcategory',
     'Health Worker Training - In-Service', ['inservice_training_cost_for_all_staff'], 1),
    (input_costs[input_costs['cost_category'] == 'human resources for health'], 'cost_subcategory',
     'Mentorships & Supportive Supervision', ['mentorship_and_supportive_cost_for_all_staff'], 1),

    # Equipment
    (input_costs[input_costs['cost_category'] == 'medical equipment'], 'cost_subcategory',
     'Medical Equipment - Purchase', ['replacement_cost_annual_total'], 1),
    (input_costs[input_costs['cost_category'] == 'medical equipment'], 'cost_subcategory',
     'Medical Equipment - Maintenance',
     ['service_fee_annual_total', 'spare_parts_annual_total', 'major_corrective_maintenance_cost_annual_total'], 1),

    # Facility operation costs
    (input_costs, 'cost_subgroup', 'Facility utility bills',
     ['Electricity', 'Water', 'Cleaning', 'Security', 'Food for inpatient cases', 'Facility management'], 1),
...
list_of_consumables_costs_for_calibration_only_hiv = ['Voluntary Male Medical Circumcision',
                                                      'HIV Screening/Diagnostic Tests', 'Antiretrovirals']
list_of_consumables_costs_for_calibration_without_hiv = ['Indoor Residual Spray', 'Bednets', 'Malaria RDTs',
                                                         'Antimalarials', 'TB Tests (including RDTs)', 'TB Treatment',
                                                         'Vaccines',
                                                         'Condoms and Lubricants', 'Other family planning commodities',
                                                         'Undernutrition commodities',
                                                         'Other Drugs, medical supplies, and commodities']
list_of_hr_costs_for_calibration = ['Health Worker Salaries', 'Health Worker Training - In-Service',
                                    'Health Worker Training - Pre-Service', 'Mentorships & Supportive Supervision']
list_of_equipment_costs_for_calibration = ['Medical Equipment - Purchase', 'Medical Equipment - Maintenance']
list_of_operating_costs_for_calibration = ['Facility utility bills', 'Infrastructure - Rehabilitation',
                                           'Vehicles - Maintenance', 'Vehicles - Fuel and Maintenance']
...
all_consumable_costs = list_of_consumables_costs_for_calibration_only_hiv + list_of_consumables_costs_for_calibration_without_hiv + [
    'Supply Chain']
...
calibration_data_extract['cost_category'] = calibration_data_extract['calibration_category'].map(
    calibration_categories_dict)
...
calibration_data_extract.loc[calibration_data_extract[
                                 'Deviation of estimated cost from nearest benchmark (%)'] == 'nan%', 'Deviation of estimated cost from nearest benchmark (%)'] = 'NA'
# Replace if calibration is fine
calibration_condition_met = ((calibration_data_extract['model_cost'] > calibration_data_extract[
    ['actual_expenditure_2019', 'max_annual_budget_2020-22']].min(axis=1)) &
                             (calibration_data_extract['model_cost'] < calibration_data_extract[
                                 ['actual_expenditure_2019', 'max_annual_budget_2020-22']].max(axis=1)))
...
calibration_data_extract = calibration_data_extract[
    ['cost_category', 'calibration_category', 'actual_expenditure_2019', 'max_annual_budget_2020-22', 'model_cost',
     'Deviation of estimated cost from nearest benchmark (%)']]
...
def convert_df_to_latex(_df, _longtable=False, numeric_columns=[]):
    ...
    latex_table = latex_table.replace("Program Management & Administration",
                                      "Program Management \& Administration")  # Add \hline after each row
    latex_table = latex_table.replace("Mentorships & Supportive Supervision",
                                      "Mentorships \& Supportive Supervision")  # Add \hline after each row
...
total_expenditure = \
calibration_data_extract[calibration_data_extract['Cost Category'] != 'Not represented in TLO model'][
    'Recorded Expenditure (FY 2018/19)'].sum()
total_cost_estimate = \
calibration_data_extract[calibration_data_extract['Cost Category'] != 'Not represented in TLO model'][
    'Estimated cost (TLO Model, 2018)'].sum()
...
consumables_dispensed_summary = pd.concat(
    [consumables_dispensed_summary, consumables_dispensed[[idx['Item_Code'], idx['year']]]], axis=1)
consumables_dispensed_summary.columns = ['mean', 'lower', 'upper', 'Item_Code', 'year']
consumables_dispensed_dict = dict(
    zip(consumables_dispensed_summary['Item_Code'], consumables_dispensed_summary['mean']))
...
print(
    f"There is good correspondence between quantity of Malaria test kits (RDT) logged by the TLO model and LMIS data -  "
...
print(
    f"\item `ZN Stain' - No record in OpenLMIS; {consumables_dispensed_dict[str(zn_stain)]:,.0f} units dispensed as per modelled estimates"
...
print(
    f"\item `Jadelle (implant), box of 2\_CMST' -  53,585 units dispensed as per OpenLMIS, {consumables_dispensed_dict[str(jadelle)]:,.0f} units dispensed as per modelled estimates"
