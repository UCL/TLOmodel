...
input_costs_variable_discounting = estimate_input_cost_of_scenarios(results_folder, resourcefilepath,
                                                                    _draws=[0, 3, 5, 8],
                                                                    _years=list_of_relevant_years_for_costing,
                                                                    cost_only_used_staff=True,
...
# Abstract
consumable_cost_by_draw = \
input_costs[(input_costs.cost_category == 'medical consumables') & (input_costs.stat == 'mean')].groupby(['draw'])[
    'cost'].sum()
...
# Results 4
cost_of_hiv_testing = \
input_costs[(input_costs.cost_subgroup == 'Test, HIV EIA Elisa') & (input_costs.stat == 'mean')].groupby(['draw'])[
    'cost'].sum()
print(
    f"For instance, the cost of HIV testing consumables increases by {(cost_of_hiv_testing[3] / cost_of_hiv_testing[0] - 1) * 100:.2f}\% under the expanded HRH scenario and by "
...
revised_consumable_subcategories = {
    'cost_of_separately_managed_medical_supplies_dispensed': 'cost_of_consumables_dispensed',
    'cost_of_excess_separately_managed_medical_supplies_stocked': 'cost_of_excess_consumables_stocked',
    'supply_chain': 'supply_chain'}
input_costs_new = input_costs.copy()
input_costs_new['cost_subcategory'] = input_costs_new['cost_subcategory'].map(revised_consumable_subcategories).fillna(
    input_costs_new['cost_subcategory'])

# Figure 3: Estimated costs by cost sub-category
do_stacked_bar_plot_of_cost_by_category(_df=input_costs_new, _cost_category='medical consumables',
...
do_stacked_bar_plot_of_cost_by_category(_df=input_costs, _cost_category='medical equipment',
                                        ...
                                        _year=list_of_years_for_plot, show_title=False,
                                        _outputfilepath=figurespath, _scenario_dict=cost_scenarios)
do_stacked_bar_plot_of_cost_by_category(_df=input_costs, _cost_category='facility operating cost',
                                        _disaggregate_by_subgroup=False,
                                        _year=list_of_years_for_plot, show_title=False,
                                        _outputfilepath=figurespath, _scenario_dict=cost_scenarios)
...
def generate_detail_cost_table(_groupby_var, _groupby_var_name, _longtable=False):
    ...
    table_data = pd.concat([pivot_data[0], pivot_data[3], pivot_data[5], pivot_data[8]], axis=1,
                           keys=['draw=0', 'draw=3', 'draw=5', 'draw=8']).reset_index()

    # Rename columns for clarity
    table_data.columns = ['Cost Category', _groupby_var_name, 'Actual', 'Expanded HRH',
                          'Improved consumable availability', 'Expanded HRH +\n Improved consumable availability']
