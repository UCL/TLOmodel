...
def estimate_input_cost_of_scenarios(results_folder: Path,
    ...
    # todo replaced this (melt_model_output_draws_and_runs) after error
    ...
    years = list(range(first_year_of_simulation,
                       final_year_of_simulation + 1))  # this is the full period of the simulation but at the end of the function, years not needed for the final cost estimate are dropped
    ...
    TARGET_PERIOD = (Date(first_year_of_simulation, 1, 1), Date(final_year_of_simulation, 12,
                                                                31))  # Declare period for which the results will be generated (defined inclusively)
    ...
    hr_cost_parameters['Facility_Level'] = hr_cost_parameters['Facility_Level'].astype(
        str)  # Store Facility_Level as string
    ...
    unit_cost_equipment = unit_cost_equipment.rename(columns=unit_cost_equipment.iloc[7]).reset_index(drop=True).iloc[
                          8:]
    ...
    if _discount_rate == 0:
        unit_cost_equipment['replacement_cost_annual'] = unit_cost_equipment.apply(
            lambda row: row['unit_purchase_cost'] / row['Life span'],
            axis=1)  # straight line depreciation is discount rate is 0
    else:
        unit_cost_equipment['replacement_cost_annual'] = unit_cost_equipment.apply(
            lambda row: row['unit_purchase_cost'] / (
                    1 + (1 - (1 + annuitization_rate) ** (-row['Life span'] + 1)) / annuitization_rate),
            axis=1)  # Annuitised over the life span of the equipment assuming outlay at the beginning of the year
    unit_cost_equipment['service_fee_annual'] = unit_cost_equipment.apply(
    ...
    unit_cost_equipment['spare_parts_annual'] = unit_cost_equipment.apply(
    ...
    unit_cost_equipment['major_corrective_maintenance_cost_annual'] = unit_cost_equipment.apply(
    ...
    unit_cost_equipment = unit_cost_equipment[['Item_code', 'Equipment_tlo',
                                               'replacement_cost_annual', 'service_fee_annual', 'spare_parts_annual',
                                               'major_corrective_maintenance_cost_annual',
                                               'Health Post_prioritised', 'Community_prioritised',
                                               'Health Center_prioritised', 'District_prioritised',
                                               'Central_prioritised']]
    unit_cost_equipment = unit_cost_equipment.rename(
        columns={col: 'Quantity_' + col.replace('_prioritised', '') for col in unit_cost_equipment.columns if
                 col.endswith('_prioritised')})
    unit_cost_equipment = unit_cost_equipment.rename(
        columns={col: col.replace(' ', '_') for col in unit_cost_equipment.columns})
    ...
    # todo replacing the line above now commented out
    ...
    # ---------------------------------------------------------------------------------------------------------------
    preservice_training_cost = merge_cost_and_model_data(cost_df=hr_cost_parameters,
    ...
    if _discount_rate == 0:  # if the discount rate is 0, then the pre-service + recruitment cost simply needs to be divided by the number of years in tenure
        ...
        preservice_training_cost['annuitisation_rate'] = 1 + (1 - (1 + annuitization_rate) ** (
                -preservice_training_cost['average_length_of_tenure_in_the_public_sector'] + 1)) / annuitization_rate
    ...
    # ---------------------------------------------------------------------------------------------------------------
    inservice_training_cost = merge_cost_and_model_data(cost_df=hr_cost_parameters,
                                                        model_df=staff_size_chosen_for_costing,
    ...
    # ---------------------------------------------------------------------------------------------------------------
    mentorship_and_supportive_cost = merge_cost_and_model_data(cost_df=hr_cost_parameters,
                                                               model_df=staff_size_chosen_for_costing,
    ...
    unit_price_consumable.columns = pd.MultiIndex.from_arrays(
        [unit_price_consumable.columns, [''] * len(unit_price_consumable.columns)])

    cost_of_consumables_dispensed = consumables_dispensed.merge(unit_price_consumable, on=idx['Item_Code'],
    ...
    lmis_consumable_usage = pd.read_csv(
        path_for_consumable_resourcefiles / "ResourceFile_Consumables_availability_and_usage.csv")
    # TODO Generate a smaller version of this file
    # Collapse individual facilities
    lmis_consumable_usage_by_item_level_month = \
    lmis_consumable_usage.groupby(['category', 'item_code', 'district', 'fac_type_tlo', 'month'])[
    ...
    total_consumables_inflow_during_the_year = df.loc[
                                                   df.index.get_level_values('month') != 'January', 'received'].groupby(
        level=[0, 1, 2, 3]).sum() + \
    ...
    inflow_to_outflow_ratio = total_consumables_inflow_during_the_year.div(total_consumables_outflow_during_the_year,
                                                                           fill_value=1)

    # Edit outlier ratios
    inflow_to_outflow_ratio.loc[inflow_to_outflow_ratio < 1] = 1  # Ratio can't be less than 1
    inflow_to_outflow_ratio.loc[
        inflow_to_outflow_ratio > inflow_to_outflow_ratio.quantile(0.95)] = inflow_to_outflow_ratio.quantile(
        0.95)  # Trim values greater than the 95th percentile
    ...
    cost_of_excess_consumables_stocked = consumables_dispensed.merge(unit_price_consumable, left_on='Item_Code',
                                                                     right_on='Item_Code', validate='m:1', how='left')

    excess_stock_ratio.columns = pd.MultiIndex.from_arrays(
        [excess_stock_ratio.columns, [''] * len(excess_stock_ratio.columns)])  # TODO convert this into a funciton
    ...
    # todo this replaces function above which throws error KeyError: "The following id_vars or value_vars are not present in the DataFrame: ['year', 'Item_Code']"
    def melt_and_label_consumables_cost(_df, label):
        # Flatten row index and ensure 'Item_Code' and 'year' are columns
        _df = _df.reset_index()

        # If not a MultiIndex, convert to one with dummy second level
        ...
        # Select columns to melt: keep 'Item_Code' and 'year', and cost data columns where (int, int)
        selected_columns = [
            col for col in _df.columns
            if col[0] in ['Item_Code', 'year'] or (isinstance(col[0], int) and isinstance(col[1], int))
        ]
        _df = _df[selected_columns]

        # Melt
        melted_df = _df.melt(id_vars=[('Item_Code', ''), ('year', '')])
        # Rename columns correctly
        melted_df.columns = ['Item_Code', 'year', 'draw', 'run', 'cost']
        ...
        return melted_df[
            ['year', 'draw', 'run', 'Item_Code', 'consumable', 'cost_subcategory', 'Facility_Level', 'cost']
        ]

    # todo this was my first attempt which worked in some cases but not all
    # def melt_and_label_consumables_cost(_df, label):
    #     # Ensure row index is flattened and 'year' and 'Item_Code' are columns
    #     _df = _df.reset_index()
    #
    #     # If columns are not a MultiIndex but strings/tuples, try to convert where needed
    #     if not isinstance(_df.columns, pd.MultiIndex):
    #         try:
    #             _df.columns = pd.MultiIndex.from_tuples(_df.columns)
    #         except Exception:
    #             # Fallback: assume all non-metadata columns are cost data and create a synthetic MultiIndex
    #             _df.columns = pd.MultiIndex.from_product([_df.columns, ['']])
    #
    #     # Select columns: keep 'year' and 'Item_Code', and (int, int) columns
    #     selected_columns = [
    #         col for col in _df.columns
    #         if (col[0] in ['Item_Code', 'year']) or (isinstance(col[0], int) and isinstance(col[1], int))
    #     ]
    #     _df = _df[selected_columns]
    #
    #     # Melt with multi-indexed column names
    #     melted_df = _df.melt(id_vars=[('Item_Code', ''), ('year', '')])
    #     melted_df.columns = ['Item_Code', 'year', 'draw_run', 'cost']
    #     melted_df[['draw', 'run']] = pd.DataFrame(melted_df['draw_run'].tolist(), index=melted_df.index)
    #
    #     # Add other columns
    #     melted_df['consumable'] = melted_df['Item_Code'].map(consumables_dict)
    #     melted_df['cost_subcategory'] = label
    #     melted_df['Facility_Level'] = 'all'  # Temporary as noted
    #
    #     return melted_df[
    #         ['year', 'draw', 'run', 'Item_Code', 'consumable', 'cost_subcategory', 'Facility_Level', 'cost']]
    ...
    # todo this is giving error ValueError: Length mismatch: Expected axis has 5 elements, new values have 4 elements
    ...
    for d in _draws:
        for r in _runs:
            ...
            equipment_cost = pd.merge(equipment_df, unit_cost_equipment[
    ...
    facility_operation_cost = pd.merge(unit_cost_fac_operations, fac_count_by_level, on='Facility_Level', how='left',
                                       validate='m:m')
...
def generate_roi_plots(_monetary_value_of_incremental_health: pd.DataFrame,
    ...
    for draw_index, row in _monetary_value_of_incremental_health.iterrows():
        ...
        # Save
        plt.savefig(
            _outputfilepath / f'draw{draw_index}_{_scenario_dict[draw_index]}_ROI_at_{_value_of_life_suffix}.png',
            dpi=100,
            bbox_inches='tight')
        plt.close()
