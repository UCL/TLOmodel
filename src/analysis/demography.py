...
path = 'Demography_WorkingFile_Complete.xlsx'
...
Model_Years = sim.modules['Demography'].store_PopulationStats['Time']
Model_Pop = sim.modules['Demography'].store_PopulationStats['Population_Total']
Model_Pop_Normalised = 100 * np.asarray(Model_Pop) / np.asarray(Model_Pop[Model_Years == 2010])
...
m2015 = (df['Time'] >= Date(2015, 1, 1)) & (
        df['Time'] < Date(2016, 1, 1))  # create mask for the 2015 results
...
m2045 = (df['Time'] >= Date(2045, 1, 1)) & (
        df['Time'] < Date(2046, 1, 1))  # create mask for the 2015 results
...
df['agegrp'] = 1 + np.floor(df.age_to / 5)
df.loc[df.agegrp > 18, 'agegrp'] = 18
...
axes[0][0].barh(y, ModelOutput_Women_2015.flatten() / ModelOutput_Women_2015.flatten().sum(),
                align='center', color='red', zorder=10)
axes[0][0].set(title='Women, 2015')
axes[0][1].barh(y, ModelOutput_Men_2015.flatten() / ModelOutput_Men_2015.flatten().sum(),
                align='center', color='blue', zorder=10)
axes[0][1].set(title='Men, 2015')
axes[1][0].barh(y, ModelOutput_Women_2045.flatten() / ModelOutput_Women_2045.flatten().sum(),
                align='center', color='red', zorder=10)
axes[1][0].set(title='Women, 2045')
axes[1][1].barh(y, ModelOutput_Men_2045.flatten() / ModelOutput_Men_2045.flatten().sum(),
                align='center', color='blue', zorder=10)
...
CauseTemp = pd.DataFrame(np.where(np.random.random(len(df)) < 0.5, 'Other',
                                  'HIV'))  # create some dummy data to demonstarte
df['DeathEvent_Cause'] = CauseTemp
pd.DataFrame({'Cause = Other': df.groupby('DeathEvent_Cause').get_group('Other').DeathEvent_Age,
              'Cause = HIV': df.groupby('DeathEvent_Cause').get_group(
                  'HIV').DeathEvent_Age}).plot.hist(stacked=True)
plt.show()
