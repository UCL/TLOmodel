...
def test_storage_of_cause_of_death(seed):
    ...
    sim.register(demography.Demography(resourcefilepath=rfp), DummyModule())
    ...
    sim.modules["Demography"].do_death(
        individual_id=person_id, originating_module=sim.modules["DummyModule"], cause="a_cause"
    )
...
def test_cause_of_death_being_registered(tmpdir, seed):
    ...
    mapper_from_tlo_causes, mapper_from_gbd_causes = sim.modules[
        "Demography"
    ...
    assert (results["model"].sum() * 5.0) == approx(
        len(output["tlo.methods.demography"]["death"]) / sim.modules["Demography"].initial_model_to_data_popsize_ratio
    )
...
def test_calc_of_scaling_factor(tmpdir, seed):
    ...
    assert (
        output["tlo.methods.demography"]["scaling_factor"].at[0, "scaling_factor"]
