...
class HSI_Dummy(HSI_Event, IndividualScopeEventMixin):
    def __init__(self, module, person_id):
        ...
        self.EXPECTED_APPT_FOOTPRINT = sim.modules[
            "HealthSystem"
...
assert (
    sim.modules["HealthSystem"]
    .cons_item_code_availability_today.loc[item_code_for_consumable_that_is_available]
    .all()
)
assert (
    not sim.modules["HealthSystem"]
    .cons_item_code_availability_today.loc[
...
def test_create_duplicate_test_that_should_be_ignored():
    ...
    try:
        ...
        dx_manager.register_dx_test(
            my_test1=my_test1_property_only, my_test1_copy=my_test1_property_only
        )
    except ValueError:
        pass
    ...
    try:
        ...
        dx_manager.register_dx_test(
            my_list_of_tests1=(
                my_test1_property_and_consumable_and_sensspec,
                my_test1_property_only,
            ),
            my_list_of_tests1_copy=(
                my_test1_property_and_consumable_and_sensspec,
                my_test1_property_only,
            ),
        )
...
def test_create_dx_test_and_run():
    ...
    for person_id in df.loc[df.is_alive].index:
        ...
        result_from_dx_manager = dx_manager.run_dx_test(
            dx_tests_to_run="my_test1", hsi_event=hsi_event,
        )
...
def test_create_dx_tests_with_consumable_usage():
    ...
    assert None is not dx_manager.run_dx_test(
        dx_tests_to_run="my_test2", hsi_event=hsi_event,
    )

    assert None is not dx_manager.run_dx_test(
        dx_tests_to_run="my_test3", hsi_event=hsi_event,
    )
...
def test_create_dx_tests_with_consumable_useage_given_by_item_code_only():
    ...
    # Register the single and the compound tests with DxManager:
    dx_manager.register_dx_test(
        my_test1=my_test1_not_available, my_test2=my_test2_is_available,
    )
...
def test_run_batch_of_dx_test_in_one_call():
    ...
    result = dx_manager.run_dx_test(
        hsi_event=hsi_event, dx_tests_to_run=["my_test1", "my_test2"]
    )
...
def test_create_tuple_of_dx_tests_which_fail_and_require_chain_execution():
    ...
    dx_manager.register_dx_test(
        tuple_of_tests_with_first_not_available=(
    ...
    dx_manager.register_dx_test(
        tuple_of_tests_with_first_available=(
            my_test2_is_available,
            my_test1_not_available,
        )
    )
    ...
    result = dx_manager.run_dx_test(
        dx_tests_to_run="single_test_not_available", hsi_event=hsi_event
    )
...
def test_create_dx_test_and_run_with_imperfect_sensitivity():
    ...
    def run_test_sensitivity_and_specificty(sens=1.0, spec=1.0):
        ...
        for person_id in df.index:
            hsi_event.target = person_id
            result_from_dx_manager = dx_manager.run_dx_test(
                dx_tests_to_run="my_test", hsi_event=hsi_event
            )
...
def test_create_dx_test_and_run_with_cont_value_and_cutoff():
    ...
    dx_manager = DxManager(sim.modules["HealthSystem"])
    dx_manager.register_dx_test(
        my_test_on_above_threshold=my_test_on_above_threshold,
    ...
    hsi_event.target = person_id
    result = dx_manager.run_dx_test(
        dx_tests_to_run="my_test_on_above_threshold", hsi_event=hsi_event
    )
    assert result is True

    result = dx_manager.run_dx_test(
        dx_tests_to_run="my_test_on_below_threshold", hsi_event=hsi_event
    )
...
def test_create_dx_test_and_run_with_cont_dx_and_error():
    ...
    for person_id in df.loc[df.is_alive].index:
        ...
        result_from_test_with_zero_stdev.append(
            result_from_dx_manager["my_test_zero_stdev"]
        )
        result_from_test_with_nonzero_stdev.append(
            result_from_dx_manager["my_test_nonzero_stdev"]
        )
