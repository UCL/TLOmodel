...
def test_application_of_maternal_complications_and_care_seeking_postnatal_week_one_event(seed):
    ...
    params["prob_type_of_anaemia_pn"] = [[1, 0, 0], [1, 0, 0]]
    ...
    date_event, event = [
        ...
        if isinstance(ev[1], labour.HSI_Labour_ReceivesPostnatalCheck)
    ][0]
...
def test_application_of_neonatal_complications_and_care_seeking_postnatal_week_one_event(seed):
    ...
    date_event, event = [
        ...
        if isinstance(ev[1], newborn_outcomes.HSI_NewbornOutcomes_ReceivesPostnatalCheck)
    ][0]
    assert date_event == sim.date
...
def test_application_of_risk_of_hypertensive_disorders_postnatal_supervisor_event(seed):
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(mother_id_onset)
        if isinstance(ev[1], labour.HSI_Labour_ReceivesPostnatalCheck)
    ][0]
    assert date_event == sim.date
