...
def test_all_injuries_run(seed):
    ...
    assert len(
        set(sim.population.props["rt_injury_1"].dtypes.categories).intersection(set(sim.modules["RTI"].INJURY_CODES))
    ) == len(set(sim.modules["RTI"].INJURY_CODES))
    ...
    sim.modules["SymptomManager"].change_symptom(sim.population.props.index, "severe_trauma", "+", sim.modules["RTI"])
...
def test_module_properties(seed):
    ...
    assert (
        df.loc[df.is_alive & df.rt_road_traffic_inc & ~df.rt_imm_death, "rt_date_inj"] < date
        for date in df.loc[df.is_alive & df.rt_road_traffic_inc & ~df.rt_imm_death, rti.RTI.DATE_TO_REMOVE_DALY_COLUMNS]
    )
    check_dtypes(sim)
...
def test_no_capabilities(seed):
    ...
    sim.register(
        ...
        healthburden.HealthBurden(resourcefilepath=resourcefilepath),
        symptommanager.SymptomManager(resourcefilepath=resourcefilepath),
        healthsystem.HealthSystem(resourcefilepath=resourcefilepath, capabilities_coefficient=0.0),
        rti.RTI(resourcefilepath=resourcefilepath),
        simplified_births.SimplifiedBirths(resourcefilepath=resourcefilepath),
        healthseekingbehaviour.HealthSeekingBehaviour(resourcefilepath=resourcefilepath),
    )
