...
def test_event_scheduling_for_care_seeking_during_home_birth(seed):
    ...
    # Now run the event
    emergency_appt = HSI_GenericEmergencyFirstAppt(person_id=mother_id, module=sim.modules["Labour"])
...
def test_event_scheduling_for_labour_onset_and_facility_delivery(seed):
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(mother_id)
        if isinstance(ev[1], labour.HSI_Labour_ReceivesSkilledBirthAttendanceDuringLabour)
    ][0]
...
def test_event_scheduling_for_admissions_from_antenatal_inpatient_ward_for_caesarean_section(seed):
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(mother_id)
        if isinstance(ev[1], labour.HSI_Labour_ReceivesComprehensiveEmergencyObstetricCare)
    ][0]
    assert date_event == sim.date
...
def test_logic_within_death_and_still_birth_events(seed):
    ...
    causes = pregnancy_helper_functions.check_for_risk_of_death_from_cause_maternal(
        sim.modules["Labour"], mother_id, timing="intrapartum"
    )
    ...
    causes = pregnancy_helper_functions.check_for_risk_of_death_from_cause_maternal(
        sim.modules["Labour"], mother_id, timing="postnatal"
    )
...
def test_bemonc_treatments_are_delivered_correctly_with_no_cons_or_quality_constraints_via_functions(seed):
    ...
    sim.modules["Labour"].assessment_and_treatment_of_severe_pre_eclampsia_mgso4(hsi_event=hsi_event, labour_stage="ip")
    ...
    df.at[mother_id, "ps_htn_disorders"] = "eclampsia"
    sim.modules["Labour"].assessment_and_treatment_of_eclampsia(hsi_event=hsi_event, labour_stage="ip")
