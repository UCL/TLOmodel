...
def test_basic_run(seed):
    ...
    assert not pd.isnull(df.loc[~df.date_of_birth.isna(), ["tb_inf", "tb_strain", "tb_date_latent"]]).all().all()
    ...
    assert (
        not pd.isnull(
            df.loc[
                ~df.date_of_birth.isna() & df.is_alive,
                ["tb_on_treatment", "tb_date_treated", "tb_ever_treated", "tb_diagnosed"],
            ]
        )
        .all()
        .all()
...
def test_natural_history(seed):
    ...
    assert not pd.isnull(df.loc[~df.date_of_birth.isna() & df.is_alive, ["tb_scheduled_date_active"]]).all().all()
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(tb_case)
        if isinstance(ev[1], tb.HSI_Tb_ScreeningAndRefer)
    ][0]
    assert date_event > sim.date

    # test and treat this person
    list_of_hsi = ["tb.HSI_Tb_ScreeningAndRefer", "tb.HSI_Tb_StartTreatment"]

    for name_of_hsi in list_of_hsi:
        hsi_event = eval(name_of_hsi + "(person_id=" + str(tb_case) + ", " "module=sim.modules['Tb']," "" ")")
...
def test_treatment_schedule(seed):
    ...
    screening_appt = tb.HSI_Tb_ScreeningAndRefer(person_id=person_id, module=sim.modules["Tb"])
    ...
    tx_start = tb.HSI_Tb_StartTreatment(person_id=person_id, module=sim.modules["Tb"])
    ...
    date_event, event = [
        ...
        if isinstance(ev[1], tb.HSI_Tb_FollowUp)
    ][0]
    assert date_event > sim.date
...
def test_record_of_appt_of_tb_start_treatment_hsi(tmpdir, seed):
    ...
    def get_sim_for_appt_test_only(
        tmpdir,
        seed,
        use_simplified_birth=True,
        disable_HS=False,
        ignore_con_constraints=True,
        consumables_availability="all",
    ):
        """
        get sim with the checks for configuration of properties running in the TB module
        """
    ...
    def get_appt_footprints(_consumables_availability):
        ...
        # disable HS, all HSI events will run, but won't be in the HSI queue
        # they will enter the sim.event_queue
        sim = get_sim_for_appt_test_only(
            tmpdir,
            seed,
            use_simplified_birth=True,
            disable_HS=False,
        ...
        hsi_event = HSI_Tb_StartTreatment(module=sim.modules["Tb"], person_id=person_id)
        ...
        return hsi_run.loc[
            hsi_run.did_run & (hsi_run["Person_ID"] == person_id) & (hsi_run["TREATMENT_ID"] == "Tb_Treatment"),
...
def test_treatment_failure(seed):
    ...
    # screen and test person_id
    screening_appt = tb.HSI_Tb_ScreeningAndRefer(person_id=person_id, module=sim.modules["Tb"], facility_level="1a")
    ...
    screening_appt = tb.HSI_Tb_ScreeningAndRefer(person_id=person_id, module=sim.modules["Tb"], facility_level="2")
    test = screening_appt.apply(person_id=person_id, squeeze_factor=0.0)
...
def test_children_referrals(seed):
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if isinstance(ev[1], tb.HSI_Tb_Xray_level1b)
    ][0]
    assert date_event == sim.date

    # run HSI_Tb_Xray and check outcomes
    sim.modules["HealthSystem"].schedule_hsi_event(
        tb.HSI_Tb_Xray_level1b(person_id=person_id, module=sim.modules["Tb"]), topen=sim.date, tclose=None, priority=0
    )
...
def test_ipt_to_child_of_tb_mother(seed):
    ...
    date_event, event = [
        ...
        if isinstance(ev[1], tb.HSI_Tb_Start_or_Continue_Ipt)
    ][0]
    assert date_event == sim.date

    # give IPT to child
    ipt_appt = tb.HSI_Tb_Start_or_Continue_Ipt(person_id=child_id, module=sim.modules["Tb"])
    ...
    date_event, event = [
        ev for ev in sim.find_events_for_person(child_id) if isinstance(ev[1], tb.Tb_DecisionToContinueIPT)
    ][0]
...
def test_mdr(seed):
    ...
    # this calls clinical_monitoring which should schedule all follow-up appts
    tx_start = tb.HSI_Tb_StartTreatment(person_id=person_id, module=sim.modules["Tb"], facility_level="2")
    tx_start.apply(person_id=person_id, squeeze_factor=0.0)
...
def test_cause_of_death(seed):
    ...
    if result:
        sim.modules["Demography"].do_death(individual_id=person_id0, cause="TB", originating_module=sim.modules["Tb"])
    ...
    date_aids_event, aids_event = [
        ev for ev in sim.find_events_for_person(person_id1) if isinstance(ev[1], hiv.HivAidsOnsetEvent)
    ][0]
    ...
    date_aids_death_event, aids_death_event = [
        ev for ev in sim.find_events_for_person(person_id1) if isinstance(ev[1], hiv.HivAidsTbDeathEvent)
    ][0]
...
def test_hsi_scheduling(seed):
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if isinstance(ev[1], tb.HSI_Tb_StartTreatment)
    ][0]
    assert date_event == sim.date
    ...
    hsi_event = tb.HSI_Tb_ScreeningAndRefer(person_id=person_id, module=sim.modules["Tb"], facility_level="2")
    hsi_event.run(squeeze_factor=0)
