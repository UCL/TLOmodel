...
def hsi_checks(sim):
    ...
    assert pd.notnull(
        df.loc[df.nc_chronic_lower_back_pain_ever_diagnosed, "nc_chronic_lower_back_pain_date_last_test"]
    ).all()
    assert pd.notnull(
        df.loc[df.nc_chronic_kidney_disease_ever_diagnosed, "nc_chronic_kidney_disease_date_last_test"]
    ).all()
    ...
    assert df.loc[
        df.is_alive & df.nc_chronic_lower_back_pain_on_medication
    ].nc_chronic_lower_back_pain_ever_diagnosed.all()
    assert df.loc[
        df.is_alive & df.nc_chronic_kidney_disease_on_medication
    ].nc_chronic_kidney_disease_ever_diagnosed.all()
    assert df.loc[df.is_alive & df.nc_chronic_ischemic_hd_on_medication].nc_chronic_ischemic_hd_ever_diagnosed.all()
...
def test_if_health_system_cannot_run(seed):
    ...
    sim.register(
        ...
        healthsystem.HealthSystem(
            resourcefilepath=resourcefilepath, capabilities_coefficient=0.0, mode_appt_constraints=2
        ),
        symptommanager.SymptomManager(resourcefilepath=resourcefilepath),
        healthseekingbehaviour.HealthSeekingBehaviour(resourcefilepath=resourcefilepath),
        cardio_metabolic_disorders.CardioMetabolicDisorders(resourcefilepath=resourcefilepath),
    ...
    sim.modules["HealthSystem"].schedule_hsi_event(
        HSI_CardioMetabolicDisorders_StartWeightLossAndMedication(
            person_id=0, module=sim.modules["CardioMetabolicDisorders"], condition="diabetes"
        ),
        topen=sim.date,
    ...
    sim.modules["HealthSystem"].schedule_hsi_event(
        HSI_CardioMetabolicDisorders_Refill_Medication(
            person_id=1, module=sim.modules["CardioMetabolicDisorders"], condition="diabetes"
        ),
        topen=sim.date,
        tclose=sim.date + pd.DateOffset(days=1),
        priority=0,
...
def test_if_no_health_system_and_zero_death(seed):
    ...
    for condition in condition_list:
        ...
        sim.population.props.loc[
            sim.population.props.is_alive & (sim.population.props.age_years >= 20), f"nc_{condition}"
        ] = True
...
def test_if_no_health_system_and_high_risk_of_death(seed):
    ...
    for event in event_list:
        ...
        assert not (
            df.loc[~df.date_of_birth.isna() & pd.isnull(df[f"nc_{event}"]) & (df.age_years >= 20), "is_alive"]
        ).any()
...
def test_if_medication_prevents_all_death(seed):
    ...
    for condition in condition_list:
        ...
        sim.population.props.loc[
            sim.population.props.is_alive & (sim.population.props.age_years >= 20), f"nc_{condition}_on_medication"
        ] = True
        sim.population.props.loc[
            sim.population.props.is_alive & (sim.population.props.age_years >= 20),
            f"nc_{condition}_medication_prevents_death",
        ] = True
...
def test_hsi_investigation_not_following_symptoms(seed):
    ...
    for condition in condition_list:
        ...
        t = HSI_CardioMetabolicDisorders_Investigations(
            module=sim.modules["CardioMetabolicDisorders"],
            person_id=person_id,
            conditions_to_investigate=[f"{condition}"],
        )
        ...
        date_event, event = [
            ...
            if isinstance(ev[1], cardio_metabolic_disorders.HSI_CardioMetabolicDisorders_StartWeightLossAndMedication)
        ][0]
...
def test_hsi_weight_loss_and_medication(seed):
    ...
    for condition in condition_list:
        ...
        t = HSI_CardioMetabolicDisorders_StartWeightLossAndMedication(
            module=sim.modules["CardioMetabolicDisorders"], person_id=person_id, condition=f"{condition}"
        )
        t.apply(person_id=person_id, squeeze_factor=0.0)
        ...
        assert isinstance(
            sim.modules["HealthSystem"].HSI_EVENT_QUEUE[0].hsi_event, HSI_CardioMetabolicDisorders_Refill_Medication
        )
        ...
        if condition != "chronic_kidney_disease":  # those with CKD are not recommended to lose weight
            ...
            t = CardioMetabolicDisordersWeightLossEvent(
                module=sim.modules["CardioMetabolicDisorders"], person_id=person_id
            )
...
def test_hsi_emergency_events(seed):
    ...
    for event in event_list:
        ...
        t = HSI_CardioMetabolicDisorders_SeeksEmergencyCareAndGetsTreatment(
            module=sim.modules["CardioMetabolicDisorders"], person_id=person_id, events_to_investigate=[f"{event}"]
        )
        t.apply(person_id=person_id, squeeze_factor=0.0)
        ...
        assert isinstance(
            sim.modules["HealthSystem"].HSI_EVENT_QUEUE[0].hsi_event,
            HSI_CardioMetabolicDisorders_StartWeightLossAndMedication,
        )
...
def test_no_availability_of_consumables_for_conditions(seed):
    ...
    for condition in condition_list:
        ...
        t = HSI_CardioMetabolicDisorders_Refill_Medication(
            module=sim.modules["CardioMetabolicDisorders"], person_id=person_id, condition=f"{condition}"
        )
        t.apply(person_id=person_id, squeeze_factor=0.0)
...
def test_no_availability_of_consumables_for_events(seed):
    ...
    for event in event_list:
        ...
        t = CardioMetabolicDisordersEvent(
            module=sim.modules["CardioMetabolicDisorders"], person_id=person_id, event=event
        )
        t.apply(person_id=person_id)
        ...
        t = CardioMetabolicDisordersDeathEvent(
            module=sim.modules["CardioMetabolicDisorders"], person_id=person_id, originating_cause=f"{event}"
        )
        t.apply(person_id=person_id)
...
def test_logging_works_for_person_older_than_100(seed):
    ...
    age_update_event = AgeUpdateEvent(
        module=sim.modules["Demography"], age_range_lookup=sim.modules["Demography"].AGE_RANGE_LOOKUP
    )
