...
def seed_init_prev_in_first_stage_only(sim):
    # Set initial prevalence to zero:
    sim.modules["BladderCancer"].parameters["init_prop_bladder_cancer_stage"] = [0.0] * len(
...
def make_high_init_prev(sim):
    ...
    sim.modules["BladderCancer"].parameters["init_prop_bladder_cancer_stage"] = [0.1] * len(
        sim.modules["BladderCancer"].parameters["init_prop_bladder_cancer_stage"]
    )
    return sim
...
def check_configuration_of_population(sim):
    ...
    age_at_dx = (
        df.loc[~pd.isnull(df.bc_date_diagnosis)].bc_date_diagnosis
        - df.loc[~pd.isnull(df.bc_date_diagnosis)].date_of_birth
    )
    ...
    assert (
        df.loc[~pd.isnull(df.bc_date_treatment)].bc_date_diagnosis
        <= df.loc[~pd.isnull(df.bc_date_treatment)].bc_date_treatment
    ).all()
    ...
    assert (
        df.loc[~pd.isnull(df.bc_date_palliative_care)].bc_date_diagnosis
        <= df.loc[~pd.isnull(df.bc_date_palliative_care)].bc_date_diagnosis
    ).all()
...
def test_check_progression_through_stages_is_happening(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 15), "bc_status"
    ] = "tis_t1"
    check_configuration_of_population(sim)
...
def test_that_there_is_no_treatment_without_the_hsi_running(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 20), "bc_status"
    ] = "tis_t1"
    check_configuration_of_population(sim)
...
def test_check_progression_through_stages_is_blocked_by_treatment(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 15), "bc_date_diagnosis"
    ] = sim.date
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 15), "bc_date_treatment"
    ] = sim.date
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 15), "bc_stage_at_which_treatment_given"
    ] = "tis_t1"
    check_configuration_of_population(sim)
