...
def test_initial_prevalence_of_stunting(seed):
    ...
    prevalence_of_severe_stunting_given_any_stunting_by_age = (
        df.loc[df["any_stunted"]].groupby(by=["agegp"])["severely_stunted"].mean()
    )
...
def test_routine_assessment_for_chronic_undernutrition_if_stunted_and_correctly_diagnosed(seed):
    ...
    sim.modules["Stunting"].parameters["effectiveness_of_complementary_feeding_education_in_stunting_reduction"] = 1.0
    sim.modules["Stunting"].parameters["effectiveness_of_food_supplementation_in_stunting_reduction"] = 1.0
...
def test_math_of_incidence_calcs(seed):
    ...
    assert (
        annual_prob == approx(prop_simple) == approx(1.0 - (1.0 - monthly_risk) ** 12) == approx(prop_model, abs=0.008)
    )
