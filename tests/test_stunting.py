...
def check_configuration_of_properties(sim):
    ...
    assert (df.loc[df.is_alive & ~df.un_ever_stunted, 'un_HAZ_category'] == 'HAZ>=-2').all().all()

    # Those that were never stunted and should have no dates for stunting events
    # todo check this - this doesn't work
    # assert pd.isnull(df.loc[df.is_alive & ~df.un_ever_stunted, [
    #     'un_last_stunting_date_of_onset',
    #     'un_stunting_recovery_date',
    #     'un_stunting_tx_start_date']]).all().all()

    # Those that were ever stunted, should not have a HAZ score above <-2
    # todo- check this -- this doesn't work
    # assert (df.loc[df.is_alive & df.un_ever_stunted, 'un_HAZ_category'] != 'HAZ>=-2').all().all()
...
def test_initial_prevalence_of_stunting(tmpdir):
    ...
    sim.population.props.date_of_birth = sim.population.props['is_alive'].apply(lambda _:
        random_date(sim.date - pd.DateOffset(years=5), sim.date - pd.DateOffset(days=1), sim.rng)
    )
    ...
    prevalence_of_severe_stunting_given_any_stunting_by_age = df.loc[df['any_stunted']].groupby(by=['agegp'])['severely_stunted'].mean()
