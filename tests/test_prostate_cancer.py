...
def check_configuration_of_population(sim):
    ...
    assert set(df.index[~pd.isnull(df.pc_date_diagnosis)]).issubset(
        set(sim.modules["SymptomManager"].who_has("urinary")).union(
            sim.modules["SymptomManager"].who_has("pelvic_pain")
        )
    )

    # check that date diagnosed is consistent with the age of the person (ie. not before they were 35.0
    age_at_dx = (
        df.loc[~pd.isnull(df.pc_date_diagnosis)].pc_date_diagnosis
        - df.loc[~pd.isnull(df.pc_date_diagnosis)].date_of_birth
    )
    assert all([int(x.days / DAYS_IN_YEAR) >= 35 for x in age_at_dx])
    ...
    assert (
        df.loc[~pd.isnull(df.pc_date_treatment)].pc_date_diagnosis
        <= df.loc[~pd.isnull(df.pc_date_treatment)].pc_date_treatment
    ).all()
    ...
    assert (
        df.loc[~pd.isnull(df.pc_date_palliative_care)].pc_date_diagnosis
        <= df.loc[~pd.isnull(df.pc_date_palliative_care)].pc_date_diagnosis
    ).all()
...
def test_check_progression_through_stages_is_happeneing(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 35), "pc_status"
    ] = "prostate_confined"
    check_configuration_of_population(sim)
...
def test_check_progression_through_stages_is_blocked_by_treatment(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 35), "pc_date_diagnosis"
    ] = sim.date
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 35), "pc_date_treatment"
    ] = sim.date
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 35), "pc_stage_at_which_treatment_given"
    ] = "prostate_confined"
    check_configuration_of_population(sim)
