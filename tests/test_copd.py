...
def test_ch_lungfunction():
    ...
    assert all(copd.eligible_to_progress_to_next_lung_function(df)), (
...
def test_exacerbations():
    ...
    assert 0 == len(_individual_events), f"one or more events was scheduled for this " f"person {_individual_events}"
    ...
    _exacerbation_events = [
        ev[1] for ev in sim.find_events_for_person(df.index[0]) if isinstance(ev[1], CopdExacerbationEvent)
    ]
...
def test_severe_exacerbation():
    ...
    ge = [
        ev[1]
        for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if isinstance(ev[1], hsi_generic_first_appts.HSI_GenericEmergencyFirstAppt)
    ][0]
    ge.run(squeeze_factor=0.0)
    ...
    assert isinstance(
        [
            ev[1]
            for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
            if isinstance(ev[1], HSI_Copd_TreatmentOnSevereExacerbation)
        ][0],
...
def test_death_rate():
    ...
    assert all(~df.ch_will_die_this_episode), (
        "now that individuals have received perfect treatment," "death should be canceled"
    )
...
def test_initialise_lung_function():
    ...
    assert all(0 == sim.population.props.ch_lungfunction), (
        "individuals <15 years should have lung function of 0 at " "population initialisation"
    )
...
def test_referral_logic():
    ...
    hsi_emergency_event = [
        ...
        if isinstance(event_tuple[1], hsi_generic_first_appts.HSI_GenericEmergencyFirstAppt)
    ][0]
    ...
    sev_exacerbation_event = [
        event_tuple[1]
        for event_tuple in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if isinstance(event_tuple[1], HSI_Copd_TreatmentOnSevereExacerbation)
    ][0]
    ...
    assert "1a" != sev_exacerbation_event.facility_info.level, "referrals are not happening"
