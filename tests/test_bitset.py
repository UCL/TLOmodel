...
def dataframe():
    return pd.DataFrame(
        data={
            "symptoms": pd.Series(0, index=range(5), dtype=BitsetDType),
            "sy_stomachache": pd.Series(0, index=range(5), dtype=BitsetDType),
            "sy_injury": pd.Series(0, index=range(5), dtype=BitsetDType),
...
def symptoms(population):
    ...
    return BitsetHandler(population, "symptoms", ["fever", "cough", "nausea", "vomiting"])
...
def module_bitset_handler(population):
    return BitsetHandler(population, None, ["Module1", "Module2", "Module3", "Module4"])
...
def test_error_on_incorrect_column_dtype(population):
    with pytest.raises(AssertionError, match=str(BitsetDType)):
...
def test_has_all(dataframe, updated_symptoms):
    ...
    # who has a cough
    assert (updated_symptoms.has_all(dataframe.is_alive, "cough") == [True, False, True, True, True]).all()
    # who has both cough and vomiting
    assert (updated_symptoms.has_all(dataframe.is_alive, "cough", "vomiting") == [True, False, True, False, True]).all()


def test_has_all_multiple_columns(module_bitset_handler, sy_columns):
    module_bitset_handler.set([0], *(f"Module{i}" for i in range(1, 5)), columns=sy_columns[0])
    module_bitset_handler.set([1], "Module3", columns=sy_columns[1])
    has_all = module_bitset_handler.has_all([0, 1], "Module1", "Module3", columns=sy_columns)
...
def test_has_any(dataframe, updated_symptoms):
    assert (updated_symptoms.has_any(dataframe.is_alive, "cough", "vomiting") == [True, False, True, True, True]).all()
...
def test_has_any_multiple_columns(module_bitset_handler, sy_columns):
    ...
    module_bitset_handler.set([1], "Module3", columns=sy_columns[1])
    has_any = module_bitset_handler.has_any([0, 1], "Module1", "Module3", columns=sy_columns)
...
def test_uncompress_individual(updated_symptoms):
    ...
    assert not person_symp.fever and person_symp.cough and not person_symp.nausea and person_symp.vomiting
...
def test_linearmodel_with_bitset(dataframe, symptoms):
    ...
    lm = LinearModel(
        ...
        1.0,
        Predictor("symptoms").when(">0", 10).otherwise(20),
    )
    ...
    lm = LinearModel(
        LinearModelType.ADDITIVE,
        0.0,
        Predictor("symptoms").apply(lambda x: 2.0 if BitsetDType(x) & symptoms.element_repr("vomiting") else 20.0),
    )
    ...
    def symptom_coeff_calc(bitset):
        if BitsetDType(bitset) & symptoms.element_repr("fever"):
            if BitsetDType(bitset) & symptoms.element_repr("vomiting"):
                return 1
            else:
                return 2
        return 3

    lm = LinearModel(LinearModelType.ADDITIVE, 0.0, Predictor("symptoms").apply(symptom_coeff_calc))
    ...
    lm = LinearModel(
        LinearModelType.ADDITIVE, 0.0, Predictor("fever_and_vomiting", external=True).when(True, 1).otherwise(2)
    )

    out = lm.predict(
        dataframe.loc[dataframe.is_alive], fever_and_vomiting=symptoms.has_all(dataframe.is_alive, "fever", "vomiting")
    )
