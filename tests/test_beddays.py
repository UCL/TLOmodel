...
bed_types = list(
    pd.read_csv(resourcefilepath / "healthsystem" / "infrastructure_and_equipment" / "ResourceFile_Bed_Capacity.csv")
...
def test_beddays_in_isolation(tmpdir, seed):
    ...
    hs.parameters["BedCapacity"] = pd.DataFrame(
        data={"Facility_ID": level2_facility_ids, "bedtype1": cap_bedtype1, "bedtype2": cap_bedtype2}
    )
...
def test_beddays_allocation_resolution(tmpdir, seed):
    ...
    def assert_footprint_matches_expected(footprint: Dict[str, int], expected_footprint: Dict[str, int]):
        """
        ...
        """
        assert len(footprint) == len(expected_footprint), "Bed type footprints did not return same allocations."
    ...
    allocated_footprint = hs.bed_days.combine_footprints_for_same_patient(existing_footprint, incoming_footprint)
...
def test_bed_days_basics(tmpdir, seed):
    ...
    sim = Simulation(
        ...
        log_config={
            "filename": "bed_days",
            "directory": tmpdir,
            "custom_levels": {"tlo.methods.healthsystem": logging.INFO},
        },
    )
    sim.register(
        demography.Demography(resourcefilepath=resourcefilepath),
    ...
    for bed_type in [f"bed_tracker_{bed}" for bed in hs.bed_days.bed_types]:
        ...
        # Pandas versions
        date_range = pd.date_range(sim.start_date, sim.end_date - pd.DateOffset(days=1), freq="D")
        assert set(date_range) == set(dates_in_log)

        # Check columns (for each facility_ID) are as expected:
        assert (
            [str(x) for x in hs.parameters["BedCapacity"]["Facility_ID"].values]
            == log[bed_type].columns.drop(["date", "date"]).values
        ).all()
    ...
    assert (1 == diff.loc[(diff.index >= first_day) & (diff.index <= last_day)].sum(axis=1)).all()
...
def test_bed_days_property_is_inpatient(tmpdir, seed):
    ...
    class DummyModule(Module):
        ...
        def initialise_simulation(self, sim):
            ...
            self.sim.schedule_event(QueryInPatientStatus(self), self.sim.date + pd.DateOffset(days=1))
            self.in_patient_status = pd.DataFrame(
                index=pd.date_range(
                    self.sim.start_date, self.sim.start_date + pd.DateOffset(days=self.parameters["tracking_period"])
                ),
                columns=[0, 1, 2],
                data=False,
            )
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                HSI_Dummy(self, person_id=0), topen=Date(2010, 1, 3), tclose=None, priority=0
            )

            # Schedule person_id=1 to attend care on day 6th January
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                HSI_Dummy(self, person_id=1), topen=Date(2010, 1, 6), tclose=None, priority=0
            )

            # Schedule person_id=2 to attend care on 13th Jan, and then again on 15th Jan
            # [overlapping in-patient durations]
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                HSI_Dummy(self, person_id=2), topen=Date(2010, 1, 13), tclose=None, priority=0
            )

            self.sim.modules["HealthSystem"].schedule_hsi_event(
                HSI_Dummy(self, person_id=2), topen=Date(2010, 1, 15), tclose=None, priority=0
            )
    ...
    class QueryInPatientStatus(RegularEvent, PopulationScopeEventMixin):
        ...
        def apply(self, population):
            ...
            self.module.in_patient_status.loc[reporting_date] = population.props.loc[
                [0, 1, 2], "hs_is_inpatient"
            ].values
    ...
    false_ser = pd.Series(
        index=pd.date_range(sim.start_date, sim.end_date - pd.DateOffset(days=2), freq="D"), data=False
    )
    ...
    def assert_two_series_are_the_same_where_index_overlaps(a, b):
        return (
            pd.concat([tot_time_as_in_patient, beds_occupied], axis=1)
            .dropna()
...
def test_bed_days_released_on_death(tmpdir, seed):
    ...
    class DummyModule(Module):
        ...
        def initialise_simulation(self, sim):
            # Schedule event that will query the status of the property 'is_inpatient' each day
            self.sim.schedule_event(QueryInPatientStatus(self), self.sim.date)
            self.in_patient_status = pd.DataFrame(
                index=pd.date_range(
                    self.sim.start_date, self.sim.start_date + pd.DateOffset(days=days_simulation_duration)
                ),
                columns=[0, 1],
                data=False,
            )
            ...
            self.sim.modules["HealthSystem"].schedule_hsi_event(
                HSI_Dummy(self, person_id=1), topen=Date(2010, 1, 3), tclose=None, priority=0
            )

            # Schedule person_id=0 to die on 6th January
            self.sim.schedule_event(
                demography.InstantaneousDeath(self.sim.modules["Demography"], 0, "Other"), Date(2010, 1, 6)
            )
    ...
    class QueryInPatientStatus(RegularEvent, PopulationScopeEventMixin):
        ...
        def apply(self, population):
            self.module.in_patient_status.loc[self.sim.date] = population.props.loc[[0, 1], "hs_is_inpatient"].values
    ...
    expected_beds_occupied = (
        pd.Series(index=pd.date_range(sim.start_date, sim.end_date - pd.DateOffset(days=1), freq="D"), data=0)
        .add(
            # two persons occupy beds from 3rd Jan for 10 days
            pd.Series(index=pd.date_range(Date(2010, 1, 3), Date(2010, 1, 12)), data=2),
            fill_value=0,
        )
        .add(
            # death of one person, releases bed from 6th January - 12th January:
            pd.Series(index=pd.date_range(Date(2010, 1, 6), Date(2010, 1, 12)), data=-1),
            fill_value=0,
        )
    )
...
def test_bed_days_allocation_to_HSI(seed):
    ...
    def check_footprint_against_expectation(footprint_requested, fn_edit_bed_tracker, expected_footprint_sent_to_hsi):
        _sim = prepare_sim()
    ...
    def make_bed_a_and_b_available_on_first_day_only(_bed_tracker):
        return make_bed_b_available_on_first_day_only(make_bed_a_available_on_first_day_only(_bed_tracker))
...
def test_in_patient_admission_included_in_appt_footprint_if_any_bed_days():
    ...
    assert footprint_with_correct_inpatient_admission_and_inpatient_day == add_first_day_inpatient_appts_to_footprint(
        footprint_with_correct_inpatient_admission_and_inpatient_day
    )
    ...
    assert footprint_with_correct_inpatient_admission_and_inpatient_day == add_first_day_inpatient_appts_to_footprint(
        {**footprint, **IN_PATIENT_ADMISSION}
    )
    assert footprint_with_correct_inpatient_admission_and_inpatient_day == add_first_day_inpatient_appts_to_footprint(
        {**footprint, **IN_PATIENT_DAY_SUBSEQUENT_DAYS}
    )

    # If the in-patient admission is wrong, then it is corrected:
    assert footprint_with_correct_inpatient_admission_and_inpatient_day == add_first_day_inpatient_appts_to_footprint(
        {"Under5OPD": 1, "IPAdmission": 99, "InpatientDays": 99}
    )
...
def test_in_patient_appt_included_and_logged(tmpdir, seed):
    ...
    class DummyModule(Module):
        ...
        def initialise_simulation(self, sim):
            ...
            for person_id in range(num_persons):
                self.sim.modules["HealthSystem"].schedule_hsi_event(
                    HSI_Dummy(self, person_id=person_id), topen=date_of_admission, tclose=None, priority=0
                )
    ...
    # Load the logged tracker for general beds
    log_hsi = parse_log_file(sim.log_filepath, logging.DEBUG)["tlo.methods.healthsystem"]["HSI_Event"]
    log_hsi.index = pd.to_datetime(log_hsi.date)
    appts_freq_by_date = (
        log_hsi["Number_By_Appt_Type_Code"].apply(pd.Series).fillna(0).astype(int).groupby(level=0).sum()
    )
    ...
    expectation = (
        pd.concat(
            [
                pd.DataFrame(
                    index=[date_of_admission], data={k: num_persons * v for k, v in IN_PATIENT_ADMISSION.items()}
                ),
                pd.DataFrame(
                    index=pd.date_range(date_of_admission + pd.DateOffset(days=1), date_of_discharge),
                    data={k: num_persons * v for k, v in IN_PATIENT_DAY_SUBSEQUENT_DAYS.items()},
                ),
                pd.DataFrame(index=[date_of_admission], data={k: num_persons * v for k, v in footprint.items()}),
            ],
            axis=1,
        )
        .fillna(0)
    ...
    pd.testing.assert_frame_equal(
        appts_freq_by_date, expectation, check_dtype=False, check_names=False, check_freq=False
    )
...
def test_beddays_availability_switch(seed):
    ...
    assert not bed_days._scaled_capacity.any().any(), "At least one bed capacity was not set to 0"
    ...
    assert (
        ...
        and bedtype1.loc[start_date, facility_id_without_patient] == bedtype1_init_capacity
    ...
    assert (bedtype1.loc[day_4:, :] == 0).all().all(), "Day 4 onwards did not have correct capacity"

    # Bedtype 2 should have also have been updated, but there is no funny business here.
    assert (bedtype2.loc[day_2:, :] == 0).all().all(), "Bedtype 2 was not updated correctly"
    assert (
