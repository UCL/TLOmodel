...
def check_output_states_and_freq(rng, input_freq, prob_matrix, use_categorical_dtype=False, p_value_threshold=0.01):
    expected_output_freq = prob_matrix.to_numpy() @ input_freq
    input_states = np.repeat(prob_matrix.columns.to_list(), input_freq).tolist()
    input_states = pd.Series(pd.Categorical(input_states) if use_categorical_dtype else input_states)
    ...
    assert (
        chisquare(output_freq[expected_output_freq > 0], expected_output_freq[expected_output_freq > 0]).pvalue
        > p_value_threshold
    )
    return output_freq
...
def test_transition_states_fixed_state(rng, use_categorical_dtype):
    ...
    input_freq = np.full(len(states), 1000)
    output_freq = check_output_states_and_freq(rng, input_freq, prob_matrix, use_categorical_dtype)
...
def test_get_person_id_to_inherit_from(rng: np.random.RandomState):
    ...
    for child_id in rng.choice(population_dataframe.index[population_dataframe.is_alive], size=num_test):
        inherit_from_id = tlo.util.get_person_id_to_inherit_from(child_id, mother_id, population_dataframe, rng)
...
def test_hash_dataframe(rng):
    ...
    dataframes = [pd.DataFrame(rng.choice(list(string.ascii_lowercase), size=(4, 3))) for _ in range(10)]
    # account for lists
    for i in range(5):
...
def test_read_csv_method_output_matches_previously_used_read_excel(tmpdir):
    ...
    excel_file_path = Path(
        tmpdir_resource_filepath / "ResourceFile_test_convert_to_csv/ResourceFile_test_convert_to_csv.xlsx"
    )
    ...
    convert_excel_files_to_csv(
        folder=Path(tmpdir_resource_filepath / "ResourceFile_test_convert_to_csv"), files=[excel_file_path.name]
    )
    ...
    df_csv = read_csv_files(Path(str(excel_file_path).split(".")[0]), files=sheet_names)
...
def test_parse_values_in_mixed_datatypes_columns():
    ...
    # parse values
    mixed_data_df["param_values"] = mixed_data_df["param_values"].apply(
        parse_csv_values_for_columns_with_mixed_datatypes
    )
