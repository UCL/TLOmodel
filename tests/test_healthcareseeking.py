...
def get_dataframe_of_run_events_count(_sim):
    ...
    count_df["HSI_event"] = [
        event_details.event_name for event_details in _sim.modules["HealthSystem"].hsi_event_counts.keys()
    ]
    count_df["treatment_id"] = [
        event_details.treatment_id for event_details in _sim.modules["HealthSystem"].hsi_event_counts.keys()
    ]
    count_df["appt_footprint"] = [
        event_details.appt_footprint for event_details in _sim.modules["HealthSystem"].hsi_event_counts.keys()
    ]
    count_df["count"] = [
        _sim.modules["HealthSystem"].hsi_event_counts[event_details]
...
def test_healthcareseeking_does_occur_from_symptom_that_does_give_healthcareseeking_behaviour(seed, tmpdir):
    ...
    assert "Symptom_that_does_cause_healthcare_seeking" in sim.modules["SymptomManager"].symptom_names
    assert (
        "Symptom_that_does_cause_healthcare_seeking"
        in sim.modules["HealthSeekingBehaviour"].odds_ratio_health_seeking_in_children
    )
    assert (
        "Symptom_that_does_cause_healthcare_seeking"
        in sim.modules["HealthSeekingBehaviour"].odds_ratio_health_seeking_in_adults
    )

    # Check that everyone has the symptom
    assert idx_gets_symptom == set(sim.modules["SymptomManager"].who_has("Symptom_that_does_cause_healthcare_seeking"))
...
def test_healthcareseeking_does_not_occurs_from_symptom_that_do_not_give_healthcareseeking_behaviour(seed, tmpdir):
    ...
    assert "Symptom_that_does_not_cause_healthcare_seeking" in sim.modules["SymptomManager"].symptom_names
    assert (
        "Symptom_that_does_not_cause_healthcare_seeking"
        not in sim.modules["HealthSeekingBehaviour"].odds_ratio_health_seeking_in_children
    )
    assert (
        "Symptom_that_does_not_cause_healthcare_seeking"
        not in sim.modules["HealthSeekingBehaviour"].odds_ratio_health_seeking_in_adults
    )
...
def test_healthcareseeking_does_occur_from_symptom_that_does_give_emergency_healthcareseeking_behaviour(seed, tmpdir):
    ...
    assert (
        sim.modules["HealthSeekingBehaviour"].prob_seeks_emergency_appt_in_children[
            "Symptom_that_does_cause_emergency_healthcare_seeking"
        ]
        > 0
    )
    assert (
        sim.modules["HealthSeekingBehaviour"].prob_seeks_emergency_appt_in_adults[
            "Symptom_that_does_cause_emergency_healthcare_seeking"
        ]
        > 0
    )
...
def test_healthcareseeking_occurs_with_emergency_spurious_symptom_only(seed, tmpdir):
    ...
    assert (
        hsi_event_count_df.loc[hsi_event_count_df.treatment_id == "FirstAttendance_Emergency", "count"].sum()
...
def test_one_generic_emergency_hsi_scheduled_per_day_when_two_emergency_symptoms_are_onset(seed):
    ...
    class DummyDisease(Module):
        ...
        def read_parameters(self, data_folder):
            self.sim.modules["SymptomManager"].register_symptom(
                Symptom.emergency("EmergencySymptom1"), Symptom.emergency("EmergencySymptom2")
            )
...
def test_one_per_hsi_scheduled_per_day_when_emergency_and_non_emergency_symptoms_are_onset(seed):
    ...
    sim.register(
        ...
        healthseekingbehaviour.HealthSeekingBehaviour(
            resourcefilepath=resourcefilepath, force_any_symptom_to_lead_to_healthcareseeking=True
        ),
        DummyDisease(),
    )
...
def test_force_healthcare_seeking(seed):
    ...
    def hsi_scheduled_following_symptom_onset(
        force_any_symptom_to_lead_to_healthcare_seeking,
        ...
        class DummyDisease(Module):
            ...
            def read_parameters(self, data_folder):
                self.sim.modules["SymptomManager"].register_symptom(
                    Symptom(
                        name="NonEmergencySymptom",
                        odds_ratio_health_seeking_in_adults=(
                            0.0001  # <--- low chance of care seeking usually
                            if not no_healthcareseeking_in_adults
                            else None
                        ),
                        odds_ratio_health_seeking_in_children=(
                            0.0001  # <--- low chance of care seeking usually
                            if not no_healthcareseeking_in_children
                            else None
                        ),
            ...
            def initialise_simulation(self, sim):
                """Give person_id=0 both symptoms"""
                self.sim.modules["SymptomManager"].change_symptom(
                    person_id=[0], disease_module=self, symptom_string="NonEmergencySymptom", add_or_remove="+"
                )
    ...
    assert not hsi_scheduled_following_symptom_onset(
        force_any_symptom_to_lead_to_healthcare_seeking=False,
        no_healthcareseeking_in_children=True,
        no_healthcareseeking_in_adults=True,
    )
    assert not hsi_scheduled_following_symptom_onset(
        force_any_symptom_to_lead_to_healthcare_seeking=True,
        no_healthcareseeking_in_children=True,
        no_healthcareseeking_in_adults=True,
    )
...
def test_force_healthcare_seeking_control_of_behaviour_through_parameters_and_arguments(seed):
    ...
    value_in_resourcefile = bool(
        pd.read_csv(resourcefilepath / "ResourceFile_HealthSeekingBehaviour.csv")
        .set_index("parameter_name")
        .apply(pd.eval)
        .at["force_any_symptom_to_lead_to_healthcareseeking", "value"]
    )
    ...
    assert (
        False is sim.modules["HealthSeekingBehaviour"].force_any_symptom_to_lead_to_healthcareseeking
    ...
    sim.register(
        ...
        enhanced_lifestyle.Lifestyle(resourcefilepath=resourcefilepath),
        healthseekingbehaviour.HealthSeekingBehaviour(
            resourcefilepath=resourcefilepath, force_any_symptom_to_lead_to_healthcareseeking=value_in_argument
        ),
    )
...
def test_same_day_healthcare_seeking_for_emergency_symptoms(seed, tmpdir):
    ...
    assert count == 1
    assert only_event_that_ran.event_name == "HSI_GenericEmergencyFirstAppt"
...
def test_everyone_seeks_care_for_symptom_with_high_odds_ratio_of_seeking_care(seed):
    ...
    assert np.allclose(
        hsb.hsb_linear_models["children"].predict(
    ...
    assert np.allclose(
        hsb.hsb_linear_models["adults"].predict(
            df.loc[df.is_alive & (df.age_years >= 15)],
            subgroup="adults",
    ...
    for _person_id in df.loc[df.is_alive].index:
        evs = [x[1].TREATMENT_ID for x in sim.modules["HealthSystem"].find_events_for_person(_person_id)]
...
def test_care_seeking_from_symptoms_with_intermediate_level_of_care_seeking_and_emergency(seed):
    ...
    class DummyDisease(Module):
        ...
        def initialise_simulation(self, sim):
            ...
            idx_all_alive_persons = df.loc[df.is_alive].index.to_list()
            self.sim.modules["SymptomManager"].change_symptom(
                person_id=idx_all_alive_persons, disease_module=self, symptom_string="TestSymptom", add_or_remove="+"
            )
    ...
    evs = pd.Series(
        {
            _person_id: [x[1].TREATMENT_ID for x in sim.modules["HealthSystem"].find_events_for_person(_person_id)]
            for _person_id in df.loc[df.is_alive].index
        }
    ).explode()
...
def test_care_seeking_from_symptoms_with_different_levels_of_prob_emergency(seed):
    ...
    for prob_seeks_emergency_appt in [0.0, 0.25, 0.5, 0.75, 1.0]:
        ...
        assert np.isclose((evs == "FirstAttendance_Emergency").sum() / len(evs), prob_seeks_emergency_appt, atol=0.02)
        assert np.isclose(
            (evs == "FirstAttendance_NonEmergency").sum() / len(evs), (1.0 - prob_seeks_emergency_appt), atol=0.02
        )
...
def test_non_emergency_first_appt_can_be_levels_0_1a_1b_2(seed):
    ...
    def get_events_scheduled_following_hcs_poll(prob_non_emergency_care_seeking_by_level):
        ...
        sim.modules["HealthSeekingBehaviour"].parameters[
...
def test_custom_function_is_equivalent_to_linear_model(seed):
    ...
    person_ids_with_newly_onset_symptoms = sorted(sim.modules["SymptomManager"].get_persons_with_newly_onset_symptoms())
    ...
    for (
        ...
        subgroup_name,
        age_predictor,
        care_seeking_odds_ratios,
    ) in zip(
        (alive_newly_symptomatic_children, alive_newly_symptomatic_adults),
        ("children", "adults"),
        (
        ...
        will_seek_care_cf = (
            sim.modules["HealthSeekingBehaviour"]
            .hsb_linear_models[subgroup_name]
            .predict(
                subgroup.loc[_has_any_symptoms(subgroup, care_seeking_odds_ratios.keys())],
                subgroup=subgroup_name,
                care_seeking_odds_ratios=care_seeking_odds_ratios,
