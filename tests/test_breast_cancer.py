...
def check_configuration_of_population(sim):
    ...
    age_at_dx = (
        df.loc[~pd.isnull(df.brc_date_diagnosis)].brc_date_diagnosis
        - df.loc[~pd.isnull(df.brc_date_diagnosis)].date_of_birth
    )
    ...
    assert (
        df.loc[~pd.isnull(df.brc_date_treatment)].brc_date_diagnosis
        <= df.loc[~pd.isnull(df.brc_date_treatment)].brc_date_treatment
    ).all()
    ...
    assert (
        df.loc[~pd.isnull(df.brc_date_palliative_care)].brc_date_diagnosis
        <= df.loc[~pd.isnull(df.brc_date_palliative_care)].brc_date_diagnosis
    ).all()
