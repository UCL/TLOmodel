...
def _get_person_id(df, age_bounds: tuple = (0.0, np.inf)) -> int:
    ...
    (inclusively)."""
    return df.loc[df.is_alive & ~df["ri_current_infection_status"] & df["age_exact_years"].between(*age_bounds)].index[
        0
    ]
...
def get_sim(tmpdir, seed, cons_available, equip_available="all"):
    ...
    sim.register(
        ...
        healthsystem.HealthSystem(
            resourcefilepath=resourcefilepath, cons_availability=cons_available, equip_availability=equip_available
        ),
...
def sim_hs_default_consumables(tmpdir, seed):
    ...
    sim = Simulation(
        ...
        log_config={
            "filename": "tmp",
            "directory": tmpdir,
            "custom_levels": {"*": logging.WARNING, "tlo.methods.alri": logging.INFO},
        },
    )
    sim.register(
        demography.Demography(resourcefilepath=resourcefilepath),
...
def test_integrity_of_linear_models(sim_hs_all_consumables_and_equipment):
    ...
    for pathogen in alri_module.all_pathogens:
        res = models.compute_risk_of_acquisition(pathogen=pathogen, df=df.loc[df.is_alive & (df.age_years < 5)])
    ...
    assert (
        0.0
        == models.compute_risk_of_acquisition(
            pathogen="Strep_pneumoniae_PCV13", df=df.loc[df.is_alive & (df.age_years < 5)]
        )
    ).all()
    ...
    assert (
        0.0 == models.compute_risk_of_acquisition(pathogen="Hib", df=df.loc[df.is_alive & (df.age_years < 5)])
    ).all()
        ...
        for age in range(0, 100):
            for va_pneumo_all_doses in [True, False]:
                ...
                if patho in alri_module.pathogens["bacterial"]:
                    ...
                    assert (
                        pd.isnull(bacterial_coinfection) or bacterial_coinfection in alri_module.pathogens["bacterial"]
                    )
    ...
    for patho in alri_module.all_pathogens:
        for coinf in alri_module.pathogens["bacterial"] + [np.nan]:
            for disease_type in alri_module.disease_types:
                res = models.get_complications_that_onset(
    ...
    for classification_for_treatment_decision, age_exact_years in itertools.product(
        alri_module.classifications, np.arange(0, 2, 0.05)
    ):
        _ultimate_treatment = alri_module._ultimate_treatment_indicated_for_patient(
            classification_for_treatment_decision=classification_for_treatment_decision, age_exact_years=age_exact_years
        )
...
def test_nat_hist_recovery(sim_hs_all_consumables_and_equipment):
    ...
    params = sim.modules["Alri"].parameters
    all_symptoms = {"cough", "difficult_breathing", "tachypoea", "chest_indrawing", "danger_signs"}
    for p in params:
    ...
    assert 0 < len(sim.modules["SymptomManager"].has_what(person_id=person_id, disease_module=sim.modules["Alri"]))
    ...
    recov_event_tuple = [
        ...
        if isinstance(event_tuple[1], AlriNaturalRecoveryEvent)
    ][0]
...
def test_nat_hist_death(sim_hs_all_consumables_and_equipment):
    ...
    death_event_tuple = [
        event_tuple
        for event_tuple in sim.find_events_for_person(person_id)
        if isinstance(event_tuple[1], AlriDeathEvent)
    ][0]
...
def test_classification_based_on_symptoms_and_imci(sim_hs_all_consumables_and_equipment):
    ...
    imci_classification_on_symptoms = (
        ...
        ("chest_indrawing_pneumonia", {"symptoms": ["chest_indrawing"], "child_is_younger_than_2_months": False}),
        (
            "chest_indrawing_pneumonia",
            {"symptoms": ["chest_indrawing", "tachypnoea"], "child_is_younger_than_2_months": False},
        ),
        ("fast_breathing_pneumonia", {"symptoms": ["tachypnoea"], "child_is_younger_than_2_months": False}),
        ("danger_signs_pneumonia", {"symptoms": ["danger_signs"], "child_is_younger_than_2_months": False}),
        (
            "danger_signs_pneumonia",
            {"symptoms": ["danger_signs", "chest_indrawing"], "child_is_younger_than_2_months": False},
        ),
        ("chest_indrawing_pneumonia", {"symptoms": ["chest_indrawing"], "child_is_younger_than_2_months": False}),
        # -- Children younger than 2 months
        (
            "danger_signs_pneumonia",
            {"symptoms": ["danger_signs", "chest_indrawing"], "child_is_younger_than_2_months": True},
        ),
        ("fast_breathing_pneumonia", {"symptoms": ["tachypnoea"], "child_is_younger_than_2_months": True}),
        ("cough_or_cold", {"symptoms": ["cough"], "child_is_younger_than_2_months": True}),
        (
            "danger_signs_pneumonia",
            {
...
def test_do_effects_of_alri_treatment(sim_hs_all_consumables_and_equipment):
    ...
    pathogen = list(sim.modules["Alri"].all_pathogens)[0]
    incidentcase = AlriIncidentCase_Lethal_DangerSigns_Pneumonia(
        person_id=person_id, pathogen=pathogen, module=sim.modules["Alri"]
    )
    incidentcase.run()
    ...
    sim.modules["Alri"].do_effects_of_treatment_and_return_outcome(
        person_id=person_id, antibiotic_provided="1st_line_IV_antibiotics", oxygen_provided=True
    )
    ...
    cure_event = [
        event_tuple[1]
        for event_tuple in sim.find_events_for_person(person_id)
        if isinstance(event_tuple[1], AlriCureEvent)
    ][0]
...
def test_severe_pneumonia_referral_from_hsi_first_appts(sim_hs_all_consumables_and_equipment):
    ...
    pathogen = list(sim.modules["Alri"].all_pathogens)[0]
    incidentcase = AlriIncidentCase_Lethal_DangerSigns_Pneumonia(
        person_id=int(person_id), pathogen=pathogen, module=sim.modules["Alri"]
    )
    incidentcase.run()
    ...
    generic_appt = [
        event_tuple[1]
        for event_tuple in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if isinstance(event_tuple[1], HSI_GenericEmergencyFirstAppt)
    ][0]
    ...
    hsi2 = [
        ...
        for event_tuple in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if (
            isinstance(event_tuple[1], HSI_Alri_Treatment)
            and (event_tuple[1].TREATMENT_ID == "Alri_Pneumonia_Treatment_Inpatient")
        )
    ][0]
...
def generate_hsi_sequence(
    sim, incident_case_event, age_of_person_under_2_months=False, treatment_effect="perfectly_effective"
):
    ...
    df = parse_log_file(sim.log_filepath, level=logging.DEBUG)["tlo.methods.healthsystem"]["HSI_Event"].set_index(
        "date"
    )
...
def test_treatment_pathway_if_all_consumables_mild_case(tmpdir, seed):
    ...
    ] == generate_hsi_sequence(
        sim=get_sim(seed=seed, tmpdir=tmpdir, cons_available="all"),
...
def test_treatment_pathway_if_no_consumables_mild_case(seed, tmpdir):
    ...
    assert [
        ...
        sim=get_sim(seed=seed, tmpdir=tmpdir, cons_available="none"),
        incident_case_event=AlriIncidentCase_NonLethal_Fast_Breathing_Pneumonia,
        treatment_effect="perfectly_ineffective",
    )
...
def test_impact_of_all_hsi(seed, tmpdir):
    ...
    def get_number_of_deaths_from_cohort_of_children_with_alri(
        ...
        class DummyModule(Module):
            ...
            def initialise_simulation(self, sim):
                ...
                for idx in df[df.is_alive].index:
                    sim.schedule_event(
                        event=AlriIncidentCase(module=alri_module, person_id=idx, pathogen=self.rng.choice(pathogens)),
                        date=sim.date,
                    )
        ...
        print(
            f"persons_that_die_of_alri = "
            f"{df.loc[~df.is_alive & df['cause_of_death'].str.startswith('ALRI')].index.values}"
        )
        total_deaths_to_alri_with_severe_hypoxaemia = (
            sim.modules["Alri"].logging_event.trackers["deaths_among_persons_with_SpO2<90%"].report_current_total()
        )
