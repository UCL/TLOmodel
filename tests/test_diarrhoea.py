...
def get_combined_log(log_filepath):
    ...
    m = log["incident_case"][["person_id", "age_years", "date", "date_of_outcome", "will_die"]].merge(
        log["end_of_case"],
        left_on=["person_id", "date"],
        right_on=["person_id", "date_of_onset"],
        how="inner",
        suffixes=["_i", "_o"],
    )
...
def test_basic_run_of_diarrhoea_module_with_default_params(tmpdir, seed):
    ...
    log_config = {"filename": "tmpfile", "directory": tmpdir, "custom_levels": {"tlo.methods.diarrhoea": logging.INFO}}
...
def test_basic_run_of_diarrhoea_module_with_high_incidence_and_high_death_and_with_perfect_treatment(tmpdir, seed):
    ...
    def run(spurious_symptoms):
        ...
        log_config = {
            "filename": "tmpfile",
            "directory": tmpdir,
            "custom_levels": {"tlo.methods.diarrhoea": logging.INFO},
        }
        ...
        sim.register(
            ...
            symptommanager.SymptomManager(resourcefilepath=resourcefilepath, spurious_symptoms=spurious_symptoms),
...
def test_do_when_presentation_with_diarrhoea_severe_dehydration(seed):
    ...
    generic_hsi = HSI_GenericNonEmergencyFirstAppt(module=sim.modules["HealthSeekingBehaviour"], person_id=person_id)
    symptoms = {"diarrhoea"}
...
def test_do_treatment_for_those_that_will_die_if_consumables_available(seed):
    ...
    sim.modules["SymptomManager"].change_symptom(
        person_id=0, symptom_string="diarrhoea", add_or_remove="+", disease_module=sim.modules["Diarrhoea"]
    )

    # Run 'do_treatment' from an in-patient HSI.
    in_patient_hsi = HSI_Diarrhoea_Treatment_Inpatient(module=sim.modules["Diarrhoea"], person_id=person_id)
...
def test_do_treatment_for_those_that_will_die_if_consumables_not_available(seed):
    ...
    sim.register(
        ...
        simplified_births.SimplifiedBirths(resourcefilepath=resourcefilepath),
        enhanced_lifestyle.Lifestyle(resourcefilepath=resourcefilepath),
        healthsystem.HealthSystem(resourcefilepath=resourcefilepath, disable=False, cons_availability="none"),
        symptommanager.SymptomManager(resourcefilepath=resourcefilepath),
        healthseekingbehaviour.HealthSeekingBehaviour(
            resourcefilepath=resourcefilepath,
            force_any_symptom_to_lead_to_healthcareseeking=True,  # every symptom leads to health-care seeking
...
def test_do_treatment_for_those_that_will_not_die(seed):
    ...
    in_patient_hsi = HSI_Diarrhoea_Treatment_Outpatient(module=sim.modules["Diarrhoea"], person_id=person_id)
    sim.modules["Diarrhoea"].do_treatment(person_id=person_id, hsi_event=in_patient_hsi)
    ...
    assert evs[0][0] == scheduled_date_recovery - pd.DateOffset(
        days=sim.modules["Diarrhoea"].parameters["number_of_days_reduced_duration_with_zinc"]
    )
...
def test_effect_of_vaccine(seed):
    ...
    assert "severe" not in [
        get_dehydration(pathogen="rotavirus", va_rota_all_doses=True, age_years=1) for _ in range(100)
    ]
    assert "severe" not in [
        get_dehydration(pathogen="rotavirus", va_rota_all_doses=True, age_years=4) for _ in range(100)
    ]

    # ... but if no vaccine or infected with another pathogen, then sometimes it is severe dehydration.
    assert "severe" in [get_dehydration(pathogen="rotavirus", va_rota_all_doses=False, age_years=1) for _ in range(100)]
    assert "severe" in [get_dehydration(pathogen="shigella", va_rota_all_doses=True, age_years=1) for _ in range(100)]
    ...
    # dehydration.
    assert "severe" in [get_dehydration(pathogen="rotavirus", va_rota_all_doses=True, age_years=1) for _ in range(100)]
    assert "severe" in [get_dehydration(pathogen="rotavirus", va_rota_all_doses=True, age_years=2) for _ in range(100)]
...
def test_check_perfect_treatment_leads_to_zero_risk_of_death(seed):
    ...
    for (
        ...
        assert 1.0 == diarrhoea_module.models._get_probability_that_treatment_blocks_death(
            **_args
                ...
                for idx in df[df.is_alive].index:
                    sim.schedule_event(
                        event=DiarrhoeaIncidentCase(
                            module=diarrhoea_module, person_id=idx, pathogen=self.rng.choice(pathogens)
                        ),
                        date=sim.date,
                    )
