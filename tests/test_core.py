...
def test_categorical():
    ...
    with pytest.raises((ValueError, TypeError), match="Cannot setitem on a Categorical with a new category"):
        s[0] = "A"
...
class TestLoadParametersFromDataframe:
        ...
        class ParameterModule(Module):
            def __init__(self):
                ...
                self.PARAMETERS = {
                    ...
                    "categorical_basic": Parameter(
                        Types.CATEGORICAL, "categorical", categories=["category_1", "category_2"]
                    ),
    ...
    def test_parameter_df_not_in_class(self):
        ...
        resource = self.resource.copy()
        resource = pd.concat(
            (resource, pd.DataFrame({"parameter_name": ["new_value"], "value": [1]})), ignore_index=True
        )
