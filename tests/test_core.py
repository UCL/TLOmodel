...
class TestLoadParametersFromDataframe_Bools_From_Csv_And_Excel:
    ...
    def setup(self):
        ...
        resource_excel = pd.read_excel(
            pathlib.Path(os.path.dirname(__file__)) / "resources/ResourceFile_load-parameters.xlsx",
            sheet_name="parameter_values")
        resource_excel = resource_excel.loc[
            resource_excel.parameter_name.isin(['bool_true', 'bool_false'])]  # drop rows for other parameters

        resource_csv = pd.read_csv(
            pathlib.Path(os.path.dirname(__file__)) / "resources/ResourceFile_load-parameters.csv",
        )

        self.resources = {
            'csv': resource_csv,
            'excel': resource_excel,
        }
