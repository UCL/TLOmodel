...
def run_sim(
    ...
    if incr_prob_of_failure:
        ...
        sim.modules["Contraception"].processed_params["p_pregnancy_with_contraception_per_month"] = (
            sim.modules["Contraception"].processed_params["p_pregnancy_with_contraception_per_month"] * 100
        ).clip(upper=1.0)
        ...
        for date in pd.date_range(sim.date, Date(2019, 12, 31), freq=pd.DateOffset(months=1)):
            ...
            _usage_by_age[date] = (
                sim.population.props.loc[
                    (sim.population.props.sex == "F") & (sim.population.props.age_years.between(15, 49))
                ]
...
def test_pregnancies_and_births_occurring(tmpdir, seed):
    ...
    assert (set(births.loc[after9months, "mother"]) - set(births.loc[after9months, "mother"] < 0)).issubset(
        set(pregs["woman_id"])
    )
...
def test_woman_starting_contraceptive_after_birth(tmpdir, seed):
    ...
    contraception.parameters["Initiation_AfterBirth"].loc[0] = 1.0 / len(
        contraception.parameters["Initiation_AfterBirth"].loc[0].values
    )
    contraception.processed_params = contraception.process_params()

    # Select a woman to be a mother
    person_id = 0
    _props = {"is_alive": True, "sex": "F", "age_years": 30}
...
def test_occurrence_of_HSI_for_maintaining_on_and_switching_to_methods(tmpdir, seed):
    ...
    sim = run_sim(
        tmpdir, seed=seed, use_healthsystem=True, disable=False, consumables_available=True, end_date=Date(2010, 1, 1)
    )
    ...
    states_that_may_require_HSI_to_maintain_on = sorted(
    ...
    meth_spec_days_between_appt = sim.modules["Contraception"].parameters["days_between_appts_for_maintenance"][
        states_that_may_require_HSI_to_maintain_on.index(co_method)
    ]
    original_props = {
        "sex": "F",
        "age_years": 30,
...
def test_record_of_appt_footprint_for_switching_to_methods(tmpdir, seed):
    ...
    def get_appt_footprints(switch_from, switch_to, consumables_available) -> List[str]:
        ...
        hsi_event = HSI_Contraception_FamilyPlanningAppt(
            module=sim.modules["Contraception"], person_id=person_id, new_contraceptive=switch_to
        )
        ...
        return hsi_run.loc[
            ...
            & (hsi_run["TREATMENT_ID"] == "Contraception_Routine"),
    ...
    assert [{"MinorSurg": 1}] == get_appt_footprints(
        switch_from="not_using", switch_to="female_sterilization", consumables_available=True
    )
    #  - If switching to anything new => 'FamilyPlanning'
    assert [{"FamPlan": 1}] == get_appt_footprints(
        switch_from="not_using", switch_to="pill", consumables_available=True
    )
    #  - If maintaining on implant => 'FamilyPlanning'
    assert [{"FamPlan": 1}] == get_appt_footprints(
        switch_from="implant", switch_to="implant", consumables_available=True
    )
    #  - If maintaining on pill  => 'PharmDispensing'
    assert [{"PharmDispensing": 1}] == get_appt_footprints(
        switch_from="pill", switch_to="pill", consumables_available=True
    )
    ...
    def is_list_longer_than_length_of_one_and_with_first_element_nonblank_and_subsequent_blank(x):
        return (len(x) > 1) & (x[0] != {}) & (0 == len([_x for _i, _x in enumerate(x) if (_i != 0) and (_x != {})]))
...
def test_defaulting_off_method_if_no_healthsystem_or_consumable_at_individual_level(tmpdir, seed):
    ...
    def check_that_persons_on_contraceptive_default(sim):
        ...
        initial_conditions = pd.DataFrame(
            {"method": contraceptives * 2, "due_appt": [True] * n_contraceptives + [False] * n_contraceptives}
        )
        ...
        def set_last_appt(of_method, due_bool):
            ...
            assert min(sim.modules["Contraception"].parameters["days_between_appts_for_maintenance"]) > 31
            if due_bool:
                if of_method in states_that_may_require_HSI_to_maintain_on:
                    return (
                        sim.modules["Contraception"].parameters["days_between_appts_for_maintenance"][
                            states_that_may_require_HSI_to_maintain_on.index(of_method)
                        ]
                        - 25
                    )
                else:
        ...
        on_a_method_that_required_appointment_but_appointment_not_due = (
            initial_conditions.method.isin(states_that_may_require_HSI_to_maintain_on) & ~initial_conditions.due_appt
        )
        assert (
...
def test_defaulting_off_method_if_no_healthsystem_at_population_level(tmpdir, seed):
    ...
    max_days_between_appt_for_maintenance = max(
        sim.modules["Contraception"].parameters["days_between_appts_for_maintenance"]
    )
    ...
    assert (ys.loc[after_everyone_has_appt, sorted(states_that_may_require_HSI_to_maintain_on)] == 0).all().all()
    ...
    assert (
        changes.loc[changes["switch_from"].isin(states_that_may_require_HSI_to_maintain_on), "switch_to"]
        .isin(states_that_do_require_HSI_to_switch_to)
        .all()
    )
...
def test_defaulting_off_method_if_no_consumables_at_population_level(tmpdir, seed):
    ...
    # (allowing for the monthly occurrence of the poll).
    assert (
        (num_on_contraceptives.loc[after_everyone_has_appt, sorted(states_that_may_require_HSI_to_maintain_on)] == 0)
        .all()
        .all()
    )
...
def test_outcomes_same_if_using_or_not_using_healthsystem(tmpdir, seed):
    ...
    sim_does_not_use_healthsystem = run_sim(
        run=True,
        tmpdir=tmpdir,
        seed=seed,
        use_healthsystem=False,
    ...
    def report_availability_of_consumables():
        ...
        def find_average_availability(items: List, level: str):
            ...
            if items_being_requested_but_not_recognised != set():
                ...
                warnings.warn(
                    "\nWarning: item_code(s) "
                    + str(items_being_requested_but_not_recognised)
                    + " from method(s) "
                    + str(set(methods_with_unrecognised_items))
                    + " not recognised at level "
                    + level
                    + "."
            ...
            return np.prod([cons.loc[(slice(None), facilities, _item)].mean() for _item in items])
...
def test_input_probs_sum():
    ...
    resourcefilepath = Path(os.path.dirname(__file__)) / "../resources"
    workbook = read_csv_files(Path(resourcefilepath) / "contraception" / "ResourceFile_Contraception", files=None)
    sheet_names = [
        "Initiation_ByMethod",
        "Interventions_Pop",
        "Interventions_PPFP",
