...
def set_all_women_as_pregnant_and_reset_baseline_parity(sim):
    ...
    sim.modules["Labour"].la_linear_models["post_term_labour"] = LinearModel(LinearModelType.MULTIPLICATIVE, 0.0)
...
def turn_off_antenatal_pregnancy_loss(sim):
    ...
    sim.modules["PregnancySupervisor"].ps_linear_models["spontaneous_abortion"] = LinearModel(
...
def test_run_core_modules_high_volumes_of_pregnancy_hsis_cant_run(seed, tmpdir):
    ...
    sim.register(
        ...
        healthburden.HealthBurden(resourcefilepath=resourcefilepath),
        symptommanager.SymptomManager(resourcefilepath=resourcefilepath),
        healthsystem.HealthSystem(resourcefilepath=resourcefilepath, mode_appt_constraints=2, cons_availability="all"),
        newborn_outcomes.NewbornOutcomes(resourcefilepath=resourcefilepath),
        pregnancy_supervisor.PregnancySupervisor(resourcefilepath=resourcefilepath),
        care_of_women_during_pregnancy.CareOfWomenDuringPregnancy(resourcefilepath=resourcefilepath),
        labour.Labour(resourcefilepath=resourcefilepath),
...
def test_abortion_complications(seed):
    ...
    def check_abortion_logic(abortion_type):
        ...
        emergency_appt = HSI_GenericEmergencyFirstAppt(person_id=mother_id, module=sim.modules["PregnancySupervisor"])
        ...
        death_event = pregnancy_supervisor.EarlyPregnancyLossDeathEvent(
            module=sim.modules["PregnancySupervisor"], individual_id=mother_id, cause=f"{abortion_type}_abortion"
        )
...
def test_still_births_ends_pregnancies_as_expected(seed):
    ...
    sim.modules["PregnancySupervisor"].ps_linear_models["early_onset_labour"] = LinearModel(
        LinearModelType.MULTIPLICATIVE, 0.0
    )
...
def test_run_all_births_end_ectopic_no_care_seeking(seed):
    ...
    rupture_event = pregnancy_supervisor.EctopicPregnancyRuptureEvent(
        individual_id=mother_id, module=sim.modules["PregnancySupervisor"]
    )
    ...
    death_event = pregnancy_supervisor.EarlyPregnancyLossDeathEvent(
        individual_id=mother_id, module=sim.modules["PregnancySupervisor"], cause="ectopic_pregnancy"
    )
    death_event.apply(mother_id)
...
def test_preterm_labour_logic(seed):
    ...
    sim.modules["PregnancySupervisor"].ps_linear_models["early_initiation_anc4"] = LinearModel(
        LinearModelType.MULTIPLICATIVE, 0.0
    )
...
def test_pregnancy_supervisor_anaemia(seed):
    ...
    for person in pregnant_women.index:
        ...
        assert pd.isnull(
            sim.modules["PregnancySupervisor"].mother_and_newborn_info[person]["severe_anaemia_" "resolution"]
        )
