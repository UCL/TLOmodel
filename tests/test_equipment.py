...
equipment_item_code_that_is_available = [
    0,
    1,
]
equipment_item_code_that_is_not_available = [
    2,
    3,
]


def run_simulation_and_return_log(seed, tmpdir, equipment_in_init, equipment_in_apply) -> Dict:
    ...
    sim.register(
        ...
        DummyModule(essential_equipment=equipment_in_init, other_equipment=equipment_in_apply),
    )
...
def test_logging_of_equipment_from_multiple_hsi(seed, tmpdir):
    ...
    item_code_needed_at_each_level = {"0": set({0}), "1a": set({10}), "2": set({30}), "3": set({44}), "4": set()}

    class DummyHSIEvent(HSI_Event, IndividualScopeEventMixin):
        def __init__(self, module, person_id, level, equipment_item_code):
            super().__init__(module, person_id=person_id)
    ...
    assert (
        item_code_needed_at_each_level == df.groupby("Facility_Level")["EquipmentEverUsed"].sum().apply(set).to_dict()
    )
