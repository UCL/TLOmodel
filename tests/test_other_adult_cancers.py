...
def check_configuration_of_population(sim):
    ...
    age_at_dx = (
        df.loc[~pd.isnull(df.oac_date_diagnosis)].oac_date_diagnosis
        - df.loc[~pd.isnull(df.oac_date_diagnosis)].date_of_birth
    )
    ...
    assert (
        df.loc[~pd.isnull(df.oac_date_treatment)].oac_date_diagnosis
        <= df.loc[~pd.isnull(df.oac_date_treatment)].oac_date_treatment
    ).all()
    ...
    assert (
        df.loc[~pd.isnull(df.oac_date_palliative_care)].oac_date_diagnosis
        <= df.loc[~pd.isnull(df.oac_date_palliative_care)].oac_date_diagnosis
    ).all()
...
def test_check_progression_through_stages_is_happening(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 15), "oac_status"
    ] = "site_confined"
    check_configuration_of_population(sim)
...
def test_check_progression_through_stages_is_blocked_by_treatment(seed):
    ...
    sim.population.props.loc[
        ...
        & (sim.population.props.oac_status != "none"),
        "oac_date_diagnosis",
    ] = sim.date
    sim.population.props.loc[
        sim.population.props.is_alive
