...
def test_generation_of_natural_history_process_no_art():
    ...
    * The AIDS death event results in an actual death when no ART
    ...
    infection_event = hiv.HivInfectionEvent(
        module=sim.modules["Hiv"], person_id=person_id
    )
    ...
    date_aids_event, aids_event = [
        ...
        if isinstance(ev[1], hiv.HivAidsOnsetEvent)
    ][0]
    ...
    date_aids_death_event, aids_death_event = [
        ev
        for ev in sim.find_events_for_person(person_id)
        if isinstance(ev[1], hiv.HivAidsDeathEvent)
    ][0]
...
def test_test_and_refer_event_scheduled_by_main_event_poll():
    ...
    dates_of_tr_events = [
        ...
        if isinstance(ev[4], hiv.HSI_Hiv_TestAndRefer)
    ]
...
def test_aids_symptoms_lead_to_treatment_being_initiated():
    ...
    aids_event = HivAidsOnsetEvent(
        person_id=person_id, module=sim.modules["Hiv"], cause="Hiv"
    )
    aids_event.apply(person_id)
    ...
    ge = [
        ...
        if isinstance(
            ev[1], hsi_generic_first_appts.HSI_GenericFirstApptAtFacilityLevel1
        )
    ][0]
...
def test_art_is_initiated_for_infants():
    ...
    sim.modules["NewbornOutcomes"].newborn_care_info[child_id] = {
        "ga_at_birth": df.at[mother_id, "ps_gestational_age_in_weeks"],
        "vit_k": False,
        "tetra_eye_d": False,
        "proph_abx": False,
        "abx_for_prom_given": False,
    ...
    newborn_care = (
        newborn_outcomes.HSI_NewbornOutcomes_CareOfTheNewbornBySkilledAttendant(
            module=sim.modules["NewbornOutcomes"],
            person_id=child_id,
...
def test_hsi_testandrefer_and_prep():
    ...
    date_decision_event, decision_event = [
        ev
        for ev in sim.find_events_for_person(person_id)
        if isinstance(ev[1], hiv.Hiv_DecisionToContinueOnPrEP)
    ][0]
    ...
    assert 0 == len(
        [
            ...
            for ev in sim.find_events_for_person(person_id)
            if (
                isinstance(ev[1], hiv.Hiv_DecisionToContinueOnPrEP)
                & (ev[0] > date_decision_event)
            )
        ]
    )
...
def test_hsi_testandrefer_and_art():
    ...
    assert df.at[person_id, "hv_diagnosed"]
    assert df.at[person_id, "hv_number_tests"] > 0
    assert "aids_symptoms" not in sim.modules["SymptomManager"].has_what(
        person_id=person_id
    )
    ...
    date_decision_event, decision_event = [
        ev
        for ev in sim.find_events_for_person(person_id)
        if isinstance(ev[1], hiv.Hiv_DecisionToContinueTreatment)
    ][0]
    ...
    assert 0 == len(
        [
            ...
            for ev in sim.find_events_for_person(person_id)
            if (
                isinstance(ev[1], hiv.Hiv_DecisionToContinueTreatment)
                & (ev[0] > date_decision_event)
            )
        ]
    )
    ...
    assert 0 < len(
        [
            ...
            for ev in sim.find_events_for_person(person_id)
            if (
                isinstance(ev[1], hiv.HivAidsOnsetEvent)
                & (ev[0] >= date_decision_event)
            )
        ]
    )
