...
def test_initialisation(seed):
    ...
    for idx in before_aids_idx:
        ...
        assert all(
            date_of_event >= sim.date for date_of_event, _ in events_for_this_person
        ), "Not all dates in the event list are after the current date"
    ...
    for idx in aids:
        ...
        assert all(
            date >= sim.date for date, _ in events_for_this_person
        ), "Not all dates in the event list are after the current date"
...
def test_generation_of_natural_history_process_no_art(seed):
    ...
    * The AIDS death event results in an actual death when no ART
    ...
    date_aids_event, aids_event = [
        ev for ev in sim.find_events_for_person(person_id) if isinstance(ev[1], hiv.HivAidsOnsetEvent)
    ][0]
    ...
    date_aids_death_event, aids_death_event = [
        ev for ev in sim.find_events_for_person(person_id) if isinstance(ev[1], hiv.HivAidsDeathEvent)
    ][0]
...
def test_mtct_during_breastfeeding_if_mother_infected_during_breastfeeding(seed):
    ...
    * HIV infection events are created during breastfeeding if the mother is infected _whilst_ breastfeeding.
    ...
    assert 0 == len(
        [
            ev
            for ev in sim.find_events_for_person(child_id)
            if isinstance(ev[1], hiv.HivInfectionDuringBreastFeedingEvent)
        ]
    )
...
def test_test_and_refer_event_scheduled_by_main_event_poll(seed):
    ...
    dates_of_tr_events = [
        ev.topen
        for ev in sim.modules["HealthSystem"].HSI_EVENT_QUEUE
        if isinstance(ev.hsi_event, hiv.HSI_Hiv_TestAndRefer)
    ]
...
def test_aids_symptoms_lead_to_treatment_being_initiated(seed):
    ...
    ge = [
        ...
        if isinstance(ev[1], hsi_generic_first_appts.HSI_GenericNonEmergencyFirstAppt)
    ][0]
    ...
    assert 1 == len(
        [
            ev[0]
            for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
            if isinstance(ev[1], hiv.HSI_Hiv_TestAndRefer)
        ]
    )
...
def test_art_is_initiated_for_infants(seed):
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(child_id)
        if isinstance(ev[1], hiv.HSI_Hiv_TestAndRefer)
    ][0]
    ...
    assert 1 == len(
        [
            ev
            for ev in sim.modules["HealthSystem"].find_events_for_person(child_id)
            if isinstance(ev[1], hiv.HSI_Hiv_StartOrContinueTreatment)
        ]
    )
...
def test_hsi_testandrefer_and_circ(seed):
    ...
    date_event, event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if isinstance(ev[1], hiv.HSI_Hiv_Circ)
    ][0]
...
def test_child_circ(seed):
    ...
    def find_event_scheduled(cons_availability="all", prob=1.0):
        ...
        event = [
            ev[1]
            for ev in sim.modules["HealthSystem"].find_events_for_person(target_person)
            if isinstance(ev[1], hiv.HSI_Hiv_Circ)
        ]
...
def test_hsi_testandrefer_and_prep(seed):
    ...
    date_hsi_event, hsi_event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if isinstance(ev[1], hiv.HSI_Hiv_StartOrContinueOnPrep)
    ][0]
    ...
    date_next_hsi_event, next_hsi_event = [
        ev
        for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
        if (isinstance(ev[1], hiv.HSI_Hiv_StartOrContinueOnPrep) & (ev[0] >= date_decision_event))
    ][0]
    ...
    assert 0 == len(
        [
            ev[0]
            for ev in sim.find_events_for_person(person_id)
            if (isinstance(ev[1], hiv.Hiv_DecisionToContinueOnPrEP) & (ev[0] > date_decision_event))
        ]
    )
...
def test_hsi_testandrefer_and_art(seed):
    ...
    assert 1 == len(
        [
            ev
            for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
            if (isinstance(ev[1], hiv.HSI_Hiv_StartOrContinueTreatment) & (ev[0] >= date_decision_event))
        ]
    )
    ...
    assert 0 == len(
        [
            ev[0]
            for ev in sim.find_events_for_person(person_id)
            if (isinstance(ev[1], hiv.Hiv_DecisionToContinueTreatment) & (ev[0] > date_decision_event))
        ]
    )
    ...
    assert 0 < len(
        [
            ev[0]
            for ev in sim.find_events_for_person(person_id)
            if (isinstance(ev[1], hiv.HivAidsOnsetEvent) & (ev[0] >= date_decision_event))
        ]
    )
...
def test_hsi_art_stopped_due_to_no_drug_available_and_no_restart(seed):
    ...
    t = HSI_Hiv_StartOrContinueTreatment(
        module=sim.modules["Hiv"], person_id=person_id, facility_level_of_this_hsi="1a"
    )
    t.apply(person_id=person_id, squeeze_factor=0.0)
    ...
    assert 1 == len(
        [
            ev[0]
            for ev in sim.find_events_for_person(person_id)
            if (isinstance(ev[1], hiv.HivAidsOnsetEvent) & (ev[0] >= sim.date))
        ]
    )
    ...
    assert 1 == len(
        [
            ev[0]
            for ev in sim.modules["HealthSystem"].find_events_for_person(person_id)
            if (isinstance(ev[1], hiv.HSI_Hiv_StartOrContinueTreatment) & (ev[0] >= sim.date))
        ]
    )
...
def test_hsi_art_stopped_if_healthsystem_cannot_run_hsi_and_no_restart(seed):
    ...
    assert 0 == len(
        [
            ev[0]
            for ev in sim.modules["HealthSystem"].find_events_for_person(person_id=0)
            if (isinstance(ev[1], hiv.HSI_Hiv_StartOrContinueTreatment) & (ev[0] >= sim.date))
        ]
    )
    assert 0 == len(
        [
            ev[0]
            for ev in sim.modules["HealthSystem"].find_events_for_person(person_id=1)
            if (isinstance(ev[1], hiv.HSI_Hiv_StartOrContinueTreatment) & (ev[0] >= sim.date))
        ]
    )
    ...
    assert 0 == len(
        [
            ev[0]
            for ev in sim.find_events_for_person(person_id=0)
            if (isinstance(ev[1], hiv.HivAidsOnsetEvent) & (ev[0] >= sim.date))
        ]
    )
    ...
    assert 1 == len(
        [
            ev[0]
            for ev in sim.find_events_for_person(person_id=1)
            if (isinstance(ev[1], hiv.HivAidsOnsetEvent) & (ev[0] >= sim.date))
        ]
    )
...
def test_baseline_hiv_prevalence(seed):
    ...
    adult_prev_1549_data = (
        prev_data.loc[(prev_data.Year == 2010, "HIV prevalence among general population 15-49")].values[0] / 100
    )
    female_prev_1549_data = prev_data.loc[(prev_data.Year == 2010, "HIV prevalence among women 15-49")].values[0] / 100
    male_prev_1549_data = prev_data.loc[(prev_data.Year == 2010, "HIV prevalence among men 15-49")].values[0] / 100
    ...
    adult_prev_1549 = len(df[df.hv_inf & df.is_alive & df.age_years.between(15, 49)]) / len(
        df[df.is_alive & df.age_years.between(15, 49)]
    )
    assert np.isclose(adult_prev_1549, adult_prev_1549_data, rtol=0.05)

    female_prev_1549 = len(df[df.hv_inf & df.is_alive & df.age_years.between(15, 49) & (df.sex == "F")]) / len(
        df[df.is_alive & df.age_years.between(15, 49) & (df.sex == "F")]
    )
    assert np.isclose(female_prev_1549, female_prev_1549_data, rtol=0.05)

    male_prev_1549 = len(df[df.hv_inf & df.is_alive & df.age_years.between(15, 49) & (df.sex == "M")]) / len(
        df[df.is_alive & df.age_years.between(15, 49) & (df.sex == "M")]
    )
