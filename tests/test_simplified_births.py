...
def check_property_integrity(sim):
    ...
    if len(mothers) > 0:
        assert not df.loc[mothers, ["date_of_last_pregnancy", "si_date_of_last_delivery"]].isna().any().any()

        newborns = df.loc[~df.date_of_birth.isna() & (df.mother_id >= 0), ["date_of_birth", "mother_id"]]
        newborns = newborns.merge(
            df[["date_of_birth", "date_of_death"]], left_on="mother_id", right_index=True, suffixes=("_nb", "_mother")
        )
        assert (
    ...
    assert (
        df.loc[df.is_alive & (df.age_exact_years > 0.6) & (df.age_exact_years < 2.0), "nb_breastfeeding_status"].isin(
            ["none", "non_exclusive"]
        )
    ).all()
    ...
    assert (df.loc[df.is_alive & (df.age_exact_years > 2.1), "nb_breastfeeding_status"] == "none").all()
...
def test_standard_run_using_simplified_birth_module(seed):
    ...
    assert (
        df.loc[df.index.isin(born_in_sim.index) & (df.age_exact_years < 0.5), "nb_breastfeeding_status"] == "exclusive"
    ).all()
    check_dtypes(sim)
