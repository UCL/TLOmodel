...
def test_individual_death():
    ...
    pd.testing.assert_series_equal(pd.Series([True, True]), df.is_alive, check_names=False)
    ...
    pd.testing.assert_series_equal(pd.Series([False, False]), df.is_alive, check_names=False)
    pd.testing.assert_series_equal(pd.Series([Date(2010, 3, 1), Date(2010, 4, 1)]), df.date_of_death, check_names=False)
...
def test_make_test_property(seed):
    ...
    pd.testing.assert_series_equal(pd.Series([False, False, False]), sim.population.props.test, check_names=False)
...
def test_birth_and_death():
    ...
    pd.testing.assert_series_equal(
        pd.Series([True, False], index=[4, 5]), sim.population.props.loc[4:5, "is_alive"], check_names=False
    )
    ...
    for person in df.itertuples():
        ...
        for child1i, child2i in zip(person.children[:-1], person.children[1:]):
            child1, child2 = df.at[child1i, "date_of_birth"], df.at[child2i, "date_of_birth"]
