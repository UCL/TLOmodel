...
def check_configuration_of_population(sim):
    ...
    age_at_dx = (
        df.loc[~pd.isnull(df.oc_date_diagnosis)].oc_date_diagnosis
        - df.loc[~pd.isnull(df.oc_date_diagnosis)].date_of_birth
    )
    ...
    assert (
        df.loc[~pd.isnull(df.oc_date_treatment)].oc_date_diagnosis
        <= df.loc[~pd.isnull(df.oc_date_treatment)].oc_date_treatment
    ).all()
    ...
    assert (
        df.loc[~pd.isnull(df.oc_date_palliative_care)].oc_date_diagnosis
        <= df.loc[~pd.isnull(df.oc_date_palliative_care)].oc_date_diagnosis
    ).all()
...
def test_check_progression_through_stages_is_happeneing(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 20), "oc_status"
    ] = "low_grade_dysplasia"
    check_configuration_of_population(sim)
...
def test_check_progression_through_stages_is_blocked_by_treatment(seed):
    ...
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 20), "oc_date_diagnosis"
    ] = sim.date
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 20), "oc_date_treatment"
    ] = sim.date
    sim.population.props.loc[
        sim.population.props.is_alive & (sim.population.props.age_years >= 20), "oc_stage_at_which_treatment_applied"
    ] = "low_grade_dysplasia"
    check_configuration_of_population(sim)
