...
def register_all_modules(seed):
    ...
    sim.register(
        ...
        healthsystem.HealthSystem(
            resourcefilepath=resourcefilepath, service_availability=["*"], cons_availability="all"
        ),  # went set disable=true, cant check HSI queue,
...
def check_event_queue_for_event_and_return_scheduled_event_date(
    sim, queue_of_interest, individual_id, event_of_interest
):
    """Checks the hsi OR event queue for an event and returns scheduled date"""
    if queue_of_interest == "event":
        date_event, event = [
            ev for ev in sim.find_events_for_person(person_id=individual_id) if isinstance(ev[1], event_of_interest)
        ][0]
        return date_event
    else:
        date_event, event = [
            ev
            for ev in sim.modules["HealthSystem"].find_events_for_person(individual_id)
            if isinstance(ev[1], event_of_interest)
        ][0]
        return date_event
...
def test_perfect_run_of_anc_contacts_no_constraints(seed):
    ...
    recommended_ga_at_next_visit = sim.modules["CareOfWomenDuringPregnancy"].determine_gestational_age_for_next_contact(
        mother_id
    )
...
def test_anc_contacts_that_should_not_run_wont_run(seed):
    ...
    date_event = check_event_queue_for_event_and_return_scheduled_event_date(
        sim,
        queue_of_interest="hsi",
        individual_id=updated_mother_id,
...
def test_daisy_chain_care_seeking_logic_to_ensure_certain_number_of_contact(seed):
    ...
    sim.modules["CareOfWomenDuringPregnancy"].antenatal_care_scheduler(
        individual_id=updated_mother_id, visit_to_be_scheduled=2, recommended_gestation_next_anc=20
    )
    ...
    sim.modules["CareOfWomenDuringPregnancy"].antenatal_care_scheduler(
        individual_id=updated_mother_id, visit_to_be_scheduled=3, recommended_gestation_next_anc=26
    )
...
def test_initiation_of_treatment_for_maternal_anaemia_during_antenatal_inpatient_care(seed):
    ...
    assert (
        sim.modules["PregnancySupervisor"].mother_and_newborn_info[mother_id]["severe_anaemia_resolution"] == sim.date
    )
    ...
    hsi_events = find_and_return_hsi_events_list(sim, mother_id)
    assert (
        care_of_women_during_pregnancy.HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfAnaemia
        not in hsi_events
    )  # noqa: E501
...
def test_initiation_of_treatment_for_gestational_diabetes_during_antenatal_inpatient_care(seed):
    ...
    assert (
        care_of_women_during_pregnancy.HSI_CareOfWomenDuringPregnancy_AntenatalOutpatientManagementOfGestationalDiabetes
        in hsi_events
    )  # noqa: E501
...
def test_scheduling_and_treatment_effect_of_post_abortion_care(seed):
    ...
    # Run the event
    emergency_appt = HSI_GenericEmergencyFirstAppt(
        person_id=updated_mother_id, module=sim.modules["PregnancySupervisor"]
    )
    ...
    death_event = pregnancy_supervisor.EarlyPregnancyLossDeathEvent(
        module=sim.modules["PregnancySupervisor"], individual_id=mother_id, cause="spontaneous_abortion"
    )

    # Replicate the death equation from the event so risk of death == 1 but treatment is completely effective
    sim.modules["PregnancySupervisor"].ps_linear_models["spontaneous_abortion_death"] = LinearModel(
        LinearModelType.MULTIPLICATIVE, 1, Predictor("ac_received_post_abortion_care").when(True, 0)
    )
...
def test_scheduling_and_treatment_effect_of_ectopic_pregnancy_case_management(seed):
    ...
    ectopic_event = pregnancy_supervisor.EctopicPregnancyEvent(
        individual_id=mother_id, module=sim.modules["PregnancySupervisor"]
    )
    ...
    sim.modules["PregnancySupervisor"].ps_linear_models["ectopic_pregnancy_death"] = LinearModel(
        LinearModelType.MULTIPLICATIVE, 1, Predictor("ac_ectopic_pregnancy_treated").when(True, 0)
    )

    # Run the death event
    death_event = pregnancy_supervisor.EarlyPregnancyLossDeathEvent(
        module=sim.modules["PregnancySupervisor"], individual_id=mother_id, cause="ectopic_pregnancy"
    )
    death_event.apply(mother_id)
