...
def test_configuration_of_properties(seed):
    ...
    had_an_episode_now_resolved = ~pd.isnull(df["de_date_init_most_rec_depr"]) & (
...
def get_hsi_events_run(sim):
    return {event_details.event_name for event_details in sim.modules["HealthSystem"].hsi_event_counts.keys()}
...
def test_hsi_functions(tmpdir, seed):
    ...
    sim.modules["Depression"].linearModels["Risk_of_SelfHarm_per3mo"] = LinearModel(
        LinearModelType.MULTIPLICATIVE, sim.modules["Depression"].parameters["prob_3m_selfharm_depr"]
    )
