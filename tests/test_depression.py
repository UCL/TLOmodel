...
def test_configuration_of_properties():
    # --------------------------------------------------------------------------
    # Create and run a very short but big population simulation for use in the tests
    ...
    # (i.e. the specificity of DxTest = 1.0)
...
def test_hsi_functions():
    ...
    sim.population.props['de_cc'] = False
    ...
    assert df.loc[df['de_ever_had_talk_ther']].sum()

    # Check that there have been some uses of Anti-depressants
    assert df.loc[df['de_on_antidepr']].sum()
