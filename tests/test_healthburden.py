...
def test_run_with_healthburden_with_dummy_diseases(tmpdir, seed):
    ...
    assert set(dalys.set_index(["sex", "age_range", "year"]).columns) == {
        "Other",
        "Mockitis_Disability_And_Death",
        "ChronicSyndrome_Disability_And_Death",
    }
...
def test_cause_of_disability_being_registered(seed, tmpdir):
    ...
    assert set(disability_mapper_from_tlo_causes.keys()) == set(sim.modules["HealthBurden"].causes_of_disability.keys())
    assert set(disability_mapper_from_gbd_causes.keys()) == set(
        sim.modules["HealthBurden"].parameters["gbd_causes_of_disability"]
    )
    assert set(disability_mapper_from_gbd_causes.values()) == set(disability_mapper_from_tlo_causes.values())
...
def test_airthmetic_of_lifeyearslost(seed, tmpdir):
    ...
    assert yll.loc[("F", ["0-4", "5-9"], slice(None), slice(None), 2010)].sum().sum() == approx(
        1.0, abs=0.5 / DAYS_IN_YEAR
    )
...
def test_arithmetic_of_stacked_lifeyearslost(tmpdir, seed):
    ...
    sim = Simulation(
        ...
        log_config={
            "filename": "tmp",
            "directory": tmpdir,
            "custom_levels": {"tlo.methods.healthburden": logging.INFO},
        },
    )
    sim.register(
        demography.Demography(resourcefilepath=rfp),
    ...
    age_range_at_disability_onset = AGE_RANGE_LOOKUP[int(np.round(age_at_date(disability_onset_date, date_of_birth)))]
    ...
    age_groups_where_yll_are_accrued = set(
        [
            _grp
            for _age, _grp in AGE_RANGE_LOOKUP.items()
    ...
    marker_for_disability = (
        (yld.year == disability_onset_date.year) & (yld.age_range == age_range_at_disability_onset) & (yld.sex == sex)
    )
    ...
    yll_by_year_not_stacked = (
        yll_not_stacked.loc[(yll_not_stacked.sex == sex), ["year", "age_range", "cause_of_death_A"]]
        .groupby("year")["cause_of_death_A"]
        .sum()
    )
    assert all(
        [
            yll_by_year_not_stacked.loc[year] == approx(1.0, abs=1 / 364)
            for year in range(death_date.year, sim.end_date.year)
        ]
    )
    assert all(
        [
            yll_by_year_not_stacked.loc[year] == approx(0.0, abs=1 / 364)
            for year in range(sim.start_date.year, death_date.year)
        ]
    )
    ...
    yll_stacked_by_time = (
        yll_stacked_by_time.loc[(yll_stacked_by_time.sex == sex), ["year", "age_range", "cause_of_death_A"]]
        .groupby(["year", "age_range"])["cause_of_death_A"]
        .sum()
        .unstack()
    )
    assert (
        0.0 == yll_stacked_by_time.loc[yll_stacked_by_time.index != death_date.year].sum().sum()
    )  # No Yll for years other than year of death
    assert (
        approx(age_limit_for_yll - 2.0, 1 / 364) == yll_stacked_by_time.loc[death_date.year].sum()
    )  # In year of death, (age_limit_for_yll - 2)    years of lost life.
    assert (
        yll_stacked_by_time.loc[
            death_date.year,
            yll_stacked_by_time.columns[yll_stacked_by_time.columns.isin(age_groups_where_yll_are_accrued)],
        ]
        > 0
    ).all()
    assert (
        0.0 == yll_stacked_by_time[sorted(age_groups_where_yll_are_not_accrued)].sum().sum()
    )  # There should be no yll for ages above `age_limit_for_yll` because that is the definition
    ...
    yll_stacked_by_age_and_time = (
        yll_stacked_by_age_and_time.loc[
            (yll_stacked_by_age_and_time.sex == sex), ["year", "age_range", "cause_of_death_A"]
        ]
        .groupby(["year", "age_range"])["cause_of_death_A"]
        .sum()
        .unstack()
    ...
    assert (
        0
        == yll_stacked_by_age_and_time[
            yll_stacked_by_age_and_time.columns[yll_stacked_by_age_and_time.columns != age_range_at_death]
        ]
        .sum()
        .sum()
    )  # No YLL for ages other than age at
    #                                                                              death
    assert (
        0 == yll_stacked_by_age_and_time.loc[yll_stacked_by_age_and_time.index != death_date.year].sum().sum()
    )  # No Yll for years other than year of death
    assert yll_stacked_by_age_and_time.sum().sum() == approx(
        yll_stacked_by_time.sum().sum()
    )  # Total YLL matches YLL when stacked by time only

    # Check dalys (not stacked) is as expected:
    dalys_by_year_not_stacked = (
        log["dalys"].loc[(log["dalys"].sex == sex), ["year", "age_range", "Label_A"]].groupby("year")["Label_A"].sum()
    )
    ...
    dalys_by_year_stacked_by_time = (
        log["dalys_stacked"]
        .loc[(log["dalys"].sex == sex), ["year", "age_range", "Label_A"]]
        .groupby(["year", "age_range"])["Label_A"]
        .sum()
        .unstack()
    )
    ...
    assert all(
        [
            dalys_by_year_stacked_by_time.loc[year].sum() == (approx(0.0, 1 / 364))
            for year in range(death_date.year + 1, sim.end_date.year + 1)
        ]
    )

    # Check dalys_stacked_by_age_and_time is as expected
    dalys_by_year_stacked_by_age_and_time = (
        log["dalys_stacked_by_age_and_time"]
        .loc[(log["dalys"].sex == sex), ["year", "age_range", "Label_A"]]
        .groupby(["year", "age_range"])["Label_A"]
        .sum()
        .unstack()
    ...
    assert (
        0.0
        == dalys_by_year_stacked_by_age_and_time.loc[
    ...
    dalys_by_year_stacked_by_time_district = (
        log["dalys_by_district_stacked_by_age_and_time"]
        .loc[(log["dalys"].sex == sex), ["year", "district_of_residence", "Label_A"]]
        .groupby(["year", "district_of_residence"])["Label_A"]
        .sum()
        .unstack()
    )
