...
def test_sims(sim):
    ...
    for person in df.index[
        df.is_alive & (df.ma_inf_type == "clinical") & (sim.date >= df.ma_date_symptoms)
    ]:
...
def test_remove_malaria_test(seed):
    ...
    assert (
        df.cause_of_death.loc[~df.is_alive & ~df.date_of_birth.isna()].isin(
    ...
    dead_due_to_malaria = (
        ...
        & df.cause_of_death.isin({"severe_malaria", "Malaria"})
    )
    malaria_death_date_in_past = ~pd.isnull(df.ma_date_death) & (
        df.ma_date_death <= sim.date
    )
...
def test_dx_algorithm_for_non_malaria_outcomes(seed):
    ...
    person_id = 0
    sim.population.props.loc[person_id, ["ma_is_infected", " ma_inf_type"]] = (
        False,
        "none",
    )
...
def test_severe_malaria_deaths_perfect_treatment(sim):
    ...
    treatment_appt = malaria.HSI_Malaria_Treatment_Complicated(
        person_id=person_id, module=sim.modules["Malaria"]
    )
...
def test_individual_testing_and_treatment(sim):
    ...
    sim.modules["Malaria"].parameters[
        "prob_malaria_case_tests"
    ] = 1.0  # all cases referred for rdt
    sim.modules["Malaria"].parameters["sensitivity_rdt"] = 1.0
    ...
    assert set(sim.modules["SymptomManager"].has_what(person_id)) == {
        "fever",
        "headache",
        "vomiting",
        "stomachache",
    ...
    assert "malaria.HSI_Malaria_rdt" not in sim.modules[
        "HealthSystem"
    ].find_events_for_person(person_id)

    # screen and test person_id
    rdt_appt = malaria.HSI_Malaria_rdt(
        person_id=person_id, module=sim.modules["Malaria"]
    )
    ...
    # run treatment event and check person is treated and treatment counter incremented
    assert df.at[person_id, "ma_tx_counter"] == 0
    tx_appt = malaria.HSI_Malaria_Treatment(
        person_id=person_id, module=sim.modules["Malaria"]
    )
    ...
    # run treatment event and check person is treated and treatment counter incremented
    assert df.at[person_id, "ma_tx_counter"] == 0
    tx_appt = malaria.HSI_Malaria_Treatment_Complicated(
        person_id=person_id, module=sim.modules["Malaria"]
    )
    tx_appt.apply(person_id=person_id, squeeze_factor=0.0)
...
def test_population_testing_and_treatment(sim):
    ...
    for person in idx:
        assert 1 == len(
            [
                ev[0]
                for ev in sim.modules["HealthSystem"].find_events_for_person(
                    person_id=person
                )
    ...
    for person in idx:
        rdt_appt = malaria.HSI_Malaria_rdt(
            person_id=person, module=sim.modules["Malaria"]
        )
