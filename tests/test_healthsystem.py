...
def test_absenteeism_assumption(seed, tmpdir):
    """Check that we can use the parameter `absenteeism_mode` to manipulate the minutes of time available for healthcare
    workers."""

    def get_capabilities_today(absenteeism_mode: str) -> pd.Series:
        sim = Simulation(start_date=start_date, seed=seed)
        sim.register(
            demography.Demography(resourcefilepath=resourcefilepath),
            healthsystem.HealthSystem(resourcefilepath=resourcefilepath)
        )
        sim.modules['HealthSystem'].parameters['absenteeism_mode'] = absenteeism_mode
        sim.make_initial_population(n=100)
        sim.simulate(end_date=start_date + pd.DateOffset(days=0))
    ...
    caps = {
        _absenteeism_mode: get_capabilities_today(_absenteeism_mode)
        for _absenteeism_mode in ('default', 'data', 'custom')
    }
